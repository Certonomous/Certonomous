...
# `F7_runs` is a RUN ARCHIVE (R20) and the spec beside it is a campaign
# RECORD (R21): one directory, two destinations, which is why they are
# named through two different roots rather than one prefix.
F7 = str(lab_paths.RUNS / "F7_runs")
SPEC_PATH = str(lab_paths.CAMPAIGN / "F7a_REGATE_SPEC.md")
...
def _f7_modules():
    out = subprocess.check_output(
        ["git", "ls-files",
         str((lab_paths.RUNS / "F7_runs").relative_to(lab_paths.REPO))
         + "/*.py"],
        cwd=REPO).decode()
