...
TAG = "finer_relayered_ngrow0"
REFINEMENT = 5  # identical to the "finer" rung

# Attempt 2. Attempt 1 (naca4412_relayer_finer.py) applied nine relaxations at
# once and made layer coverage far WORSE, not better: 58.3% -> 4.36%, average
# layers 6 -> 0.362, cells 1,849,113 -> 1,284,878.
#
# Diagnosis: nGrow 0 -> 1 is the suspect. nGrow marks that many layers of faces
# CONNECTED to a non-extruded face as also-not-extruded, so on a mesh already
# failing to extrude 42% of the surface it cascades outward. Every other change
# in the set pushes toward MORE extrusion (featureAngle 60->70 extrudes more
# surface; minThickness 5%->1% keeps thinner layers instead of discarding them;
# the iteration and smoothing counts only buy more attempts).
#
# This attempt changes exactly one thing versus attempt 1: nGrow stays at 0.
# That makes the experiment decisive either way. If coverage now beats the
# original 58.3%, nGrow was the culprit AND the other eight relaxations help.
# If coverage lands back near 58.3%, the relaxations are inert and the ceiling
# is set by geometry. If it is still near 4%, the diagnosis is wrong.
#
# (literal text as written by chief_engineer.external_aero._snappy, old -> new)
RELAXED_LAYER_OVERRIDES = {
    # nGrow deliberately NOT overridden -- left at the original 0.
...
def main() -> int:
    ...
    result = {
        ...
        "note": ("Attempt 2 at re-meshing the 'finer' rung (refinement 5). "
                 "Identical to attempt 1 (naca4412_repair_finer_relayered.json) "
                 "EXCEPT nGrow is left at its original 0 instead of being raised "
                 "to 1. Attempt 1 drove layer coverage 58.3% -> 4.36% (average "
                 "layers 6 -> 0.362); nGrow was the suspected cause because it "
                 "propagates non-extrusion to connected faces, while every other "
                 "relaxation in the set pushes toward more extrusion. Applied "
                 "here: featureAngle 60->70, nRelaxIter 5->20, "
