...
def main(request: str | None = None, params: dict | None = None, emit=None) -> int:
    ...
    if all(v is not None for v in st_series):
        ...
        # wake blockMeshDict lays four O-grid blocks of (n_radial n_tangential
        # 1) with the front and back planes typed empty, so refinement moves in
        # exactly two directions: 24x26, 34x37 and 45x48 per block give the
        # 2496, 5032 and 8640 cells this ladder is fitted on. Left at the dim=3
        # default the representative size is the cube root of the same counts,
        # which stretches every observed order by exactly 1.5.
        band = uq_studies.eca_hoekstra_band(cells_series, st_series, dim=2)
