...
accepted `analyse_k0g.py`'s in-comparator extraction -- superseded section 8.1
...
WHAT IS COMPARED
  ...
  parameters, against `analyse_k0g.py`'s in-comparator reader, on the SAME case
...
def load_fields_at(case_dir, time_name):
    """Load the fields at an EXPLICIT time through analyse_k0g's PRODUCTION
    ...
    # K0g grades the fieldAverage outputs TMean/UMean (avg2), so the equivalence
    ...
    # THE BOUNDARY HALF, through the SAME production reader (K0g section V).
...
def selftest():
    ...
    fails = []
    print("=" * 70)
    print("check_k0g_extraction_equivalence.py -- SELFTEST")
    print("=" * 70)
...
def main(argv=None):
    ...
    print(f"  OpenFOAM postProcess -func sample  vs  analyse_k0g.py's reader")
    ...
    print("\n" + "=" * 70)
    if allok:
        print("VERDICT: AGREE -- the in-comparator reader of analyse_k0g.py")
