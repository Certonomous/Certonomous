...
section 7.2; superseded section 8.2 quoting T1b_L4_AMENDMENT.md section 7):
  ...
  5. the ExecutionTime line count == the "Time =" line count (the ADAPTIVE-
     ...
     instantiation, which K0h's adjustable time step does not satisfy; see the
...
completion field set, and mark_done_k0h.py REFUSES (exit 2) rather than infer
...
Usage:
    python3 mark_done_k0h.py --root <K0h_runs dir> [case ...]
    python3 mark_done_k0h.py --root <dir> --launch-guard <case>
    python3 mark_done_k0h.py --selftest
"""
...
#     scripts/mark_done_k0g.py
#     git blob 387b8b82b179291d4d7d177d6cc593255fd03914
...
SELF_REL = "scripts/mark_done_k0h.py"
GRADING_PATH = (
...
# K0h pre-registration section 5 (closures carried unchanged from K0f).
CLOSURE_OF_CASE = {
...
# K0h pre-registration section 7.2, the TRANSIENT completion field set table.
...
# [40,60] s window) that K0h actually grades, so a run without them at endTime is
# not complete.  Like phi they are SOLVER-GENERATED and are never staged in 0/
# (build_k0h.py's preflight excludes them); they must be PRESENT at endTime.
PER_CLOSURE_FIELDS = {
...
# K0h pre-registration section 7.1'/7.1''.  TRANSIENT: endTime is a PHYSICAL
...
def check(root, case):
    ...
    # --- clause 1 -------------------------------------------------------
    # AN ABSENT `STATUS` IS A REFUSAL, NOT A VERDICT (K0h section R6).
    #
    ...
    # deltaT 1, so #steps == endTime).  K0h is TRANSIENT with an ADJUSTABLE
    ...
    # confirmation for mark_done_k0h is being routed via the chief and is the
...
def main(argv=None):
    ...
    ap.add_argument("--root", default=None,
                    help="the K0h_runs directory holding the case directories")
...
def _synthetic(root, case, endtime=60, fields=None, break_clause=None,
    ...
    for f in emit:
        if gzip_fields:
            # The `.gz` branch is exercised even though K0h registers
...
def selftest():
    print("=" * 74)
    print("mark_done_k0h.py -- SELFTEST (planted controls)")
    print("=" * 74)
    fails = []
