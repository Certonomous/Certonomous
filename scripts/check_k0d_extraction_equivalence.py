...
accepted `analyse_k0d.py`'s in-comparator extraction -- superseded section 8.1
...
WHAT IS COMPARED
  ...
  parameters, against `analyse_k0d.py`'s in-comparator reader, on the SAME case
...
def load_fields_at(case_dir, time_name):
    """Load the fields at an EXPLICIT time through analyse_k0d's PRODUCTION
...
def compare(case_dir, verbose=True):
    ...
    try:
        ...
        for setname, which in (("vertical_midplane", "vertical"),
            ...
            # ---- T ----
            mine_T = A.sample_line(idx, cf["T"], which, A.SCHEME_GRADED)[1]
            ...
            mine_U = A.sample_line(idx, ux, which, A.SCHEME_GRADED)[1]
...
def selftest():
    ...
    fails = []
    print("=" * 70)
    print("check_k0d_extraction_equivalence.py -- SELFTEST")
    print("=" * 70)
...
def main(argv=None):
    ...
    print(f"  OpenFOAM postProcess -func sample  vs  analyse_k0d.py's reader")
    ...
    print("\n" + "=" * 70)
    if allok:
        print("VERDICT: AGREE -- the in-comparator reader of analyse_k0d.py")
