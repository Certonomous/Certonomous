#!/usr/bin/env bash
# ==========================================================================
# launch_k0g.sh SELFTEST -- the rc capture DRIVEN, not asserted.
#
...
for spec in "clean:exit 0:0:clean" \
    ...
    bash "$SELF/launch_k0g.sh" --case-dir "$d" --timeout 30 --solver fakeFoam --foam-bashrc none --no-detach >/dev/null 2>&1
...
bash "$SELF/launch_k0g.sh" --case-dir "$d" --timeout 1 --solver fakeFoam --foam-bashrc none --no-detach >/dev/null 2>&1
...
bash "$SELF/launch_k0g.sh" --case-dir "$d" --timeout 30 --solver fakeFoam --foam-bashrc none >/dev/null 2>&1
...
( cd "$W" && bash "$SELF/launch_k0g.sh" --case-dir "$d" --timeout 30 --solver fakeFoam --foam-bashrc none >/dev/null 2>&1 )
...
echo "-- ranks != 1 refuses (K0g is registered SERIAL)"
d=$(mkcase ranks); mksolver fakeFoam "exit 0"
bash "$SELF/launch_k0g.sh" --case-dir "$d" --timeout 30 --ranks 4 --solver fakeFoam --no-detach >/dev/null 2>&1
...
# THE ARM THAT WOULD HAVE CAUGHT K0g ATTEMPT 1 (seven cases, rc=127, wall=0).
...
if [ -f "$REAL_BASHRC" ]; then
    ...
    d=$(mkcase realenv)
    bash "$SELF/launch_k0g.sh" --case-dir "$d" --timeout 1 --solver "$REAL_SOLVER" --no-detach >/dev/null 2>&1
...
bash "$SELF/launch_k0g.sh" --case-dir "$d" --timeout 30 --solver definitelyNotASolver_k0g \
...
bash "$SELF/launch_k0g.sh" --case-dir "$d" --timeout 30 --solver fakeFoam \
