...
SUPERSEDED IN KIND -- THIS GENERATOR TARGETS A TREE THE OWNER OVERRULED, AND IT
REFUSES TO WRITE. Marked 2026-08-16 at `b0ab070d`. The code is kept, not
deleted: its classification frame, its safety control and its two counting
frames were re-used by the map that replaced it. Only the DESTINATIONS are
wrong, and they are wrong by reversal rather than by drift --

    this generator       ->  demo-output/website/**  becomes  web/site/**
    the owner's tree     ->  /web/ serves SITE FILES ONLY; everything else out

    this generator       ->  scripts/  becomes  ops/  wholesale
    the owner's tree     ->  /scripts/ retained; /ops/ takes launchers only

    this generator       ->  cases/  <-  models/ + demo-surfaces/
    the owner's tree     ->  cases/  =  one folder per physics family

The replacement is `campaign/MOVE_MAP_2026-08-16.md`, recorded at `330faa32`
and filed as docket D331. The superseded output, docs/PHASE2_MOVE_MAP.tsv, is
kept at its 20,202 rows with a banner; it is not deleted and history is not
rewritten.

WHY THIS REFUSES TO RUN INSTEAD OF MERELY SAYING SO. A docstring is read by
whoever OPENS the file; the risk here is whoever TYPES the command. An
executable artifact that silently emits 20,202 rows pointing at the wrong tree
is a loaded gun, and this lab ranks a tool that produces confident wrong output
above one that refuses. Pass --regenerate-superseded-target-tree to override,
which exists so the frame can still be re-measured for comparison, and which
names in the flag itself what the caller is asking for.
...
#: The override that lets the superseded frame still be re-measured. Named for
#: what the caller is asking for, so it cannot be typed by accident.
_OVERRIDE = "--regenerate-superseded-target-tree"


def main() -> int:
    if _OVERRIDE not in sys.argv[1:]:
        print(
            "REFUSED: this generator targets a SUPERSEDED target tree and "
            "wrote nothing.\n"
            "  superseded 2026-08-16; the owner's tree overrules it by "
            "REVERSAL, not drift:\n"
            "    web/site/** wholesale   ->  /web/ serves site files ONLY\n"
            "    scripts/ -> ops/        ->  /scripts/ retained\n"
            "    cases/ <- models/       ->  cases/ = one physics family each\n"
            "  replacement: campaign/MOVE_MAP_2026-08-16.md at 330faa32 "
            "(docket D331)\n"
            f"  to re-measure the old frame anyway: {_OVERRIDE}",
            file=sys.stderr)
        return 1
    ...
    with OUT.open("w") as f:
        f.write("# ===== SUPERSEDED IN KIND -- DO NOT EXECUTE ANY ROW BELOW "
                "=====\n")
        f.write("# The destinations in this file were OVERRULED BY REVERSAL, "
                "not made stale by drift:\n")
        f.write("#   web/site/** wholesale  ->  /web/ serves SITE FILES ONLY; "
                "everything else moves out\n")
        f.write("#   scripts/ -> ops/       ->  /scripts/ retained; /ops/ "
                "takes launchers only\n")
        f.write("#   cases/ <- models/      ->  cases/ = one folder per "
                "physics family\n")
        f.write("# Replacement: campaign/MOVE_MAP_2026-08-16.md at 330faa32, "
                "docket D331.\n")
        f.write("# Kept, not deleted; history is not rewritten.\n")
