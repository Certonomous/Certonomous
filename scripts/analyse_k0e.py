...
--------------------------------------
Exactly ONE row is GATED: **M4**, the bit-for-bit momentum control.  Its
...
The gating reason, in the form Sanaa's 2026-09-03 20:00Z ruling requires:
  *Without M4, the verdict on the Stanton attribution cannot be trusted, because
  a momentum field that moved means beta and g were not actually neutralised and
  the attribution of any Stanton-number error to the thermal closure is void.*
...
def wall_rows(case, t_wall, dt):
    ...
    for i in range(n_faces):
        ...
        xf, yf, _ = fc[i]
        xc, yc, _ = cc[o]
        if abs(xc - xf) > 1e-12 * max(1.0, abs(xf)):
            refuse(f"plate face {i}: owner cell centre x {xc!r} differs from "
                   f"face centre x {xf!r}.  The registration reads the wall "
                   f"gradient as a y-difference on an orthogonal mesh; refusing "
                   f"rather than projecting on an assumed normal.")
        d = abs(yc - yf)
        if d <= 0.0:
...
def analyse(root, reference, scratch, expect_sha=None):
    ...
    if not done[GRADED_ARM]:
        print(f"\nM4       {NOT_A_RESULT}   {GRADED_ARM} is NOT DONE under the "
              f"strict completion rule")
    ...
    # ---- M4, THE ONLY GATED ROW ---------------------------------------
    print("\n--- M4  MOMENTUM CONTROL, THE ONLY GATED ROW ---")
    ...
    if m4["worst_ulp"]:
        ...
        print(f"  worst pair                 {m4['worst_pair'][0]!r} vs "
              ...
              f"REPORTED beside the verdict, not a threshold)")
    m4_pass = (m4["worst_ulp"] == 0)
    print(f"  M4  {PASS if m4_pass else NOT_A_RESULT}")

    # ---- M4b, the DISCRIMINATOR, REPORTED not gated --------------------
    print("\n--- M4b SAME-SOLVER NEUTRALISATION CONTROL (REPORTED, not gated) ---")
    ...
    print("  field cannot enter the momentum equation, so this must be 0 ULP.")
    print("  It DISCRIMINATES a beta/g leak from a solver-identity difference,")
    print("  which M4 alone cannot separate.")
    if not done[CONTROL_ARM]:
        print(f"  M4b {UNMEASURED}  {CONTROL_ARM} is NOT DONE")
        m4b = None
    else:
        m4b = ulp_compare_U(case_g, case_c, GRADED_ARM, CONTROL_ARM)
        print(f"  worst ULP distance         {m4b['worst_ulp']}   "
              f"({m4b['components_nonzero']} components non-zero)")
        if m4b["worst_ulp"]:
            print(f"  worst at                   {m4b['where']}   "
                  f"|diff| {m4b['worst_abs']:.6e} m/s")
        print(f"  M4b {REPORTED}")
    ...
    print("\n" + "=" * 78)
    if m4_pass:
        print(f"RUNG K0e   {PASS}  on M4, the only gated row.  Every other row is")
        print("           REPORTED, not gated.  No band is armed on the")
        print("           correlation, so this rung CANNOT GATE FAIL and its")
        print("           Stanton agreement is NOT a validation of the thermal")
        print("           closure -- see the pre-registration section 2.1.3 on")
        print("           circularity.")
    else:
        print(f"RUNG K0e   {NOT_A_RESULT}.  M4 is non-zero: the momentum field")
        print("           moved, so beta and g were not demonstrated neutral by")
        print("           this control, and the attribution of any Stanton error")
        print("           to the thermal closure is VOID.  The reported rows above")
        print("           stand as measurements and as NOTHING MORE.")
        if m4b is not None:
            print(f"           M4b (same solver, same operators) reads "
                  f"{m4b['worst_ulp']} ULP -- see the pre-registration section 5.1")
            print("           for what that discriminates.")
    print("=" * 78)
    return EXIT_OK
