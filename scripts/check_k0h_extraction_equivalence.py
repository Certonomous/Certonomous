...
accepted `analyse_k0h.py`'s in-comparator extraction -- superseded section 8.1
...
WHAT IS COMPARED
  ...
  parameters, against `analyse_k0h.py`'s in-comparator reader, on the SAME case
...
#     scripts/check_k0g_extraction_equivalence.py
#     git blob 2506603bc2600028cb2d037d08ff4eba2de795b6
...
SELF_REL = "scripts/check_k0h_extraction_equivalence.py"
GRADING_PATH = (
...
def load_fields_at(case_dir, time_name):
    """Load the fields at an EXPLICIT time through analyse_k0h's PRODUCTION
    ...
    # K0h grades the fieldAverage outputs TMean/UMean (avg2), so the equivalence
    # gate compares the reader against OpenFOAM's sampler ON THOSE fields.
    tp = A.field_path(tdir, A.GRADED_T)
    if tp is None:
        refuse(f"{tdir}: no {A.GRADED_T} field")
    T = A.read_scalar_field(tp)
    idx = A.build_index(mesh, len(T))
    up = A.field_path(tdir, A.GRADED_U)
    if up is None:
        refuse(f"{tdir}: no {A.GRADED_U} field")
    U = A.read_vector_field(up, idx["nx"] * idx["ny"])
    # THE BOUNDARY HALF, through the SAME production reader (K0h section V).
...
def compare(case_dir, verbose=True):
    ...
    results = []
    try:
        # column layout of `fields ( TMean UMean )` under setFormat raw:
        #   distance, TMean, UMean.x, UMean.y, UMean.z
        #   (OpenFOAM orders scalars then vectors; there is no k column here)
        for setname, which in (("vertical_midplane", "vertical"),
            ...
            results.append((setname, "TMean", worst, CRITERION["T"]))
            # ---- UMean, x-component (column 2: distance, TMean, UMean.x, ...) ----
            ux = [v[0] for v in cf["U"]]
            mine_U = A.sample_line(idx, ux, which, A.SCHEME_GRADED,
                                   cf["bUx"])[1]
            theirs_U = [r[2] for r in rows]
            worstU = max(abs(a - b) for a, b in zip(mine_U, theirs_U))
            results.append((setname, "UMean.x", worstU, CRITERION["U"]))
    finally:
...
def selftest():
    ...
    fails = []
    print("=" * 70)
    print("check_k0h_extraction_equivalence.py -- SELFTEST")
    print("=" * 70)
...
def main(argv=None):
    ...
    print(f"  OpenFOAM postProcess -func sample  vs  analyse_k0h.py's reader")
    ...
    print("\n" + "=" * 70)
    if allok:
        print("VERDICT: AGREE -- the in-comparator reader of analyse_k0h.py")
