#!/usr/bin/env python3
"""K0g INSTRUMENT STANDARD -- no `assert` may carry a refusal, guard, control
or gate, and every registered refusal is DRIVEN under `python3 -O`.

REGISTERED AT: docs/campaigns/F14-cooling-ladder/K0g_PREREGISTRATION.md
...
THREE ARMS, because two of them are not enough:
  ...
  1. STATEMENT TYPE.  Every K0g instrument's own AST must carry ZERO `Assert`
...
INSTRUMENTS = ("analyse_k0g.py", "build_k0g.py", "check_k0g_mesh.py",
               "mark_done_k0g.py", "check_k0g_extraction_equivalence.py",
               "check_k0g_instrument_standard.py")
...
def refusal_drives(tmp):
    ...
    return (
        ("build_k0g.py: --preflight on a non-case",
         ["build_k0g.py", "--preflight", os.path.join(tmp, "nope")], 2),
        ("build_k0g.py: --root missing",
         ["build_k0g.py"], 2),
        ("mark_done_k0g.py: --root missing",
         ["mark_done_k0g.py"], 2),
        ("mark_done_k0g.py: unregistered case",
         ["mark_done_k0g.py", "--root", empty, "NOT_A_REGISTERED_CASE"], 2),
        ("analyse_k0g.py: --root is not a directory",
         ["analyse_k0g.py", "--root", os.path.join(tmp, "nope")], 2),
        ("analyse_k0g.py: freeze check on a wrong sha",
         ["analyse_k0g.py", "--root", empty, "--expect-sha", "0" * 40], 2),
        ("check_k0g_mesh.py: --root missing",
         ["check_k0g_mesh.py"], 2),
        ("check_k0g_extraction_equivalence.py: --case on a non-directory",
         ["check_k0g_extraction_equivalence.py", "--case",
...
def main():
    print("=" * 74)
    print("K0g INSTRUMENT STANDARD (section 3C)")
    print("=" * 74)
    bad = []
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        open(mut, "w").write(
            "import sys\n"
            "def guard(ranks):\n"
            "    assert ranks == 1, 'K0g is registered SERIAL'\n"
            "    return True\n")
