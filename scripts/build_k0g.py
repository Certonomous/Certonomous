...
Usage:
    python3 build_k0g.py --root <K0g_runs dir> [case ...]
    python3 build_k0g.py --selftest
"""
import argparse
import os
import re
...
# table was filled in.  The window was open (K0g_runs did not exist) and the
...
REGISTRATION_GAPS = {
    "writeFormat": dict(
        ...
        ruling="ascii is THE ONE SETTING UNDER WHICH EVERY REGISTERED "
               ...
               "check_k0g_mesh.py reads constant/polyMesh/{points,owner} "
    ...
    "writeCompression": dict(
        ...
        ruling="mark_done_k0g.py's field_path accepts T or T.gz, so the "
               ...
               "mark_done_k0g.py's selftest exercise the .gz branch anyway: one "
        ...
        why="decides whether the completion reader and the age guard of "
            "section 7.2 must find T or T.gz.  mark_done_k0g.py accepts either "
...
# --- controlDict timings, K0g section 7.1' -- TRANSIENT ------------------
...
# transient time-step control (K0g section 7.1')
...
# guard.
# K0g pre-registration section 8: per-arm hard stop = 10x that arm's POINT line.
...
# timeout_s = cap_core_min * 60 / ranks, ranks == 1 (K0g section 8):
...
# --- the FIVE authorised K0g arms (K0g section 5) ------------------------
...
#  build_k0g.py REFUSES (exit 2) rather than infer one."
SEED = {
...
# --- fvSolution, K0g section 7.1' -- TRANSIENT PIMPLE --------------------
...
# analyse_k0g.py on the time-average.
FV_SOLUTION = """solvers
{
    "(p_rgh|p_rghFinal)"
    {
...
def grading_pair(first_cell, length, n_cells):
    ...
    the form check_k0g_mesh.py condition D can catch when it is wrong."""
    m = max(n_cells // 2, 1)
...
# --- windowed time-average (K0g section 7.1', ITEM-A ROBUST FORM) ---------
...
# windows [20,40] and [40,60] are byte-identical to K0g section 7.1'.
...
# --------------------------------------------------------------------------
# THE CONSUMER-SIDE COMPLETENESS ASSERTION (K0g section 3A).
#
...
# at endTime, which mark_done_k0g.py checks).
...
def selftest():
    ...
    import tempfile
    print("=" * 74)
    print("build_k0g.py -- SELFTEST (planted controls)")
    print("=" * 74)
    fails = []
    ...
    check_("the L2 arms take the 963.4 core-min cap and L1 the 491.5 (K0g 8)",
    ...
    check_("the K0g timeout table reproduces cap_core_min * 60 / 1 EXACTLY on "
    ...
    #   SELFTEST ITSELF WOULD HAVE CREATED K0g_runs/ AND DESTROYED THE
    ...
    try:
        mutated = os.path.join(gap_tmp, "build_k0g_gap_reopened.py")
        ...
        ranks_mut = os.path.join(gap_tmp, "build_k0g_ranks4.py")
    ...
    try:
        ...
        check_("M2_m (RNGkEpsilon) 0/ carries epsilon and NOT omega",
               os.path.isfile(os.path.join(dm2, "0", "epsilon"))
               and not os.path.exists(os.path.join(dm2, "0", "omega")))
        ...
        r2 = subprocess.run([sys.executable, "-c",
                             "import sys; sys.path.insert(0, %r); import build_k0g as B;"
    ...
    print("    verification/runs/F14-cooling-ladder/K0g_runs until the")
