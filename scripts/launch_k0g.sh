#!/usr/bin/env bash
# ==========================================================================
# K0g CASE LAUNCHER -- rc-CAPTURING, DETACHED, AND QUEUE-COMPATIBLE.
#
# NO K0d ANCESTOR EXISTS.  This is the one instrument in the K0g set with no
...
usage() {
    cat >&2 <<'U'
usage: launch_k0g.sh --case-dir DIR --timeout SECONDS [--ranks N]
                     [--solver NAME] [--no-detach]

  --case-dir  the case directory; STATUS is written to its PARENT as
              STATUS.<basename>, which is where mark_done_k0g.py reads it
  ...
  --ranks     default 1; K0g is registered SERIAL (section 6)
  --solver    default buoyantBoussinesqPimpleFoam (K0g is TRANSIENT)
...
[ "$RANKS" = "1" ] || { echo "REFUSE: K0g is registered SERIAL (section 6); --ranks=$RANKS" >&2; exit 2; }
...
write_status() {   # rc wall checkmesh_rc note
    ...
    # STATUS at all -- which mark_done_k0g.py REFUSES on, rather than reading
...
#
# ATTEMPT 1 OF K0g DIED HERE, ON ALL SEVEN FIRED CASES: `rc=127 wall=0
...
# The negative-control arm in launch_k0g_selftest.sh resolves the REAL binary
...
if [ "$FOAM_BASHRC" != "none" ]; then
    [ -f "$FOAM_BASHRC" ] || { echo "REFUSE: no OpenFOAM environment at $FOAM_BASHRC; refusing rather than launching into a shell where the solver cannot be found (K0g attempt 1: seven cases, rc=127, wall=0)" >&2; exit 2; }
    ...
    # launch_k0g_selftest.sh BEFORE the re-fire, which is exactly the arm's
...
if ! python3 "$SELF/build_k0g.py" --preflight "$CASE_DIR"; then
