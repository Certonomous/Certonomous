#!/usr/bin/env python3
"""K0h INSTRUMENT STANDARD -- no `assert` may carry a refusal, guard, control
...
THREE ARMS, because two of them are not enough:
  ...
  1. STATEMENT TYPE.  Every K0h instrument's own AST must carry ZERO `Assert`
...
#     scripts/check_k0g_instrument_standard.py
#     git blob 0196356a0cbcafd7ef992642bac62840ffb73dc4
# registered at docs/campaigns/F14-cooling-ladder/K0h_PREREGISTRATION.md
...
SELF_REL = "scripts/check_k0h_instrument_standard.py"
GRADING_PATH = (
...
INSTRUMENTS = ("analyse_k0h.py", "build_k0h.py", "check_k0h_mesh.py",
               "mark_done_k0h.py", "check_k0h_extraction_equivalence.py",
               "check_k0h_instrument_standard.py")
...
def refusal_drives(tmp):
    ...
    return (
        ("build_k0h.py: --preflight on a non-case",
         ["build_k0h.py", "--preflight", os.path.join(tmp, "nope")], 2),
        ("build_k0h.py: --root missing",
         ["build_k0h.py"], 2),
        ("mark_done_k0h.py: --root missing",
         ["mark_done_k0h.py"], 2),
        ("mark_done_k0h.py: unregistered case",
         ["mark_done_k0h.py", "--root", empty, "NOT_A_REGISTERED_CASE"], 2),
        ("analyse_k0h.py: --root is not a directory",
         ["analyse_k0h.py", "--root", os.path.join(tmp, "nope")], 2),
        ("analyse_k0h.py: freeze check on a wrong sha",
         ["analyse_k0h.py", "--root", empty, "--expect-sha", "0" * 40], 2),
        ("check_k0h_mesh.py: --root missing",
         ["check_k0h_mesh.py"], 2),
        ("check_k0h_extraction_equivalence.py: --case on a non-directory",
         ["check_k0h_extraction_equivalence.py", "--case",
...
def main():
    print("=" * 74)
    print("K0h INSTRUMENT STANDARD (section 3C)")
    print("=" * 74)
    bad = []
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        open(mut, "w").write(
            "import sys\n"
            "def guard(ranks):\n"
            "    assert ranks == 1, 'K0h is registered SERIAL'\n"
            "    return True\n")
