...
Usage:
    python3 build_k0d.py --root <K0d_runs dir> [case ...]
    python3 build_k0d.py --selftest
"""
import argparse
import os
import re
...
# table was filled in.  The window was open (K0d_runs did not exist) and the
...
REGISTRATION_GAPS = {
    "writeFormat": dict(
        ...
        ruling="ascii is THE ONE SETTING UNDER WHICH EVERY REGISTERED "
               ...
               "check_k0d_mesh.py reads constant/polyMesh/{points,owner} "
    ...
    "writeCompression": dict(
        ...
        ruling="mark_done_k0d.py's field_path accepts T or T.gz, so the "
               ...
               "mark_done_k0d.py's selftest exercise the .gz branch anyway: one "
        ...
        why="decides whether the completion reader and the age guard of "
            "section 7.2 must find T or T.gz.  mark_done_k0d.py accepts either "
...
LEVELS = {
    ...
    "L2": dict(Nx=224, nA=17, nB=193, nC=14, cells=50176, first_cell=7.864662e-04),
...
def grading_pair(first_cell, length, n_cells):
    ...
    the form check_k0d_mesh.py condition D can catch when it is wrong."""
    m = max(n_cells // 2, 1)
...
def selftest():
    ...
    import tempfile
    print("=" * 74)
    print("build_k0d.py -- SELFTEST (planted controls)")
    print("=" * 74)
    fails = []
    ...
    #   SELFTEST ITSELF WOULD HAVE CREATED K0d_runs/ AND DESTROYED THE
    ...
    try:
        ...
        r2 = subprocess.run([sys.executable, "-c",
                             "import sys; sys.path.insert(0, %r); import build_k0d as B;"
    ...
    print("    verification/runs/F14-cooling-ladder/K0d_runs until the")
