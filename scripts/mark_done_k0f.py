...
completion field set, and mark_done_k0f.py REFUSES (exit 2) rather than infer
...
Usage:
    python3 mark_done_k0f.py --root <K0f_runs dir> [case ...]
    python3 mark_done_k0f.py --root <dir> --launch-guard <case>
    python3 mark_done_k0f.py --selftest
"""
import argparse
import os
import re
...
# TEN, not nine.  See the refusal comment in analyse_k0f.py.
...
def check(root, case):
    ...
    # --- clause 1 -------------------------------------------------------
    # AN ABSENT `STATUS` IS A REFUSAL, NOT A VERDICT (K0f section R6).
    #
...
def main(argv=None):
    ...
    ap.add_argument("--root", default=None,
                    help="the K0f_runs directory holding the case directories")
...
def _synthetic(root, case, endtime=40000, fields=None, break_clause=None,
    ...
    for f in emit:
        if gzip_fields:
            # AMENDMENT 2 section A2.1c registers writeCompression = off, so
            # this rung will not produce a gzipped field.  The branch is
            # exercised ANYWAY: a branch kept alive in the code and dead in the
            # test is a branch that will be believed the first time it is used,
            # and `field_path` accepts either form deliberately.
...
def selftest():
    print("=" * 74)
    print("mark_done_k0f.py -- SELFTEST (planted controls)")
    print("=" * 74)
    fails = []
    ...
    try:
        ...
        _synthetic(tmp, "B_hi", break_clause=None, gzip_fields=True)
        f, _ = check(tmp, "B_hi")
        check_("GZIPPED clean case still satisfies all seven clauses",
               not f, str(f))
        _synthetic(tmp, "I_hi", break_clause="clause6", gzip_fields=True)
        f, _ = check(tmp, "I_hi")
