...
Usage:
    python3 build_k0h.py --root <K0h_runs dir> [case ...]
    python3 build_k0h.py --selftest
"""
...
#     scripts/build_k0g.py
#     git blob 4dace6456803c15c5e2a6286f579c254573f2dbc
...
# table was filled in.  The window was open (K0h_runs did not exist) and the
...
REGISTRATION_GAPS = {
    "writeFormat": dict(
        ...
        ruling="ascii is THE ONE SETTING UNDER WHICH EVERY REGISTERED "
               ...
               "check_k0h_mesh.py reads constant/polyMesh/{points,owner} "
    ...
    "writeCompression": dict(
        ...
        ruling="mark_done_k0h.py's field_path accepts T or T.gz, so the "
               ...
               "mark_done_k0h.py's selftest exercise the .gz branch anyway: one "
        ...
        why="decides whether the completion reader and the age guard of "
            "section 7.2 must find T or T.gz.  mark_done_k0h.py accepts either "
...
# --- controlDict timings, K0h section 7.1' -- TRANSIENT ------------------
# endTime is a PHYSICAL TIME in seconds; deltaT is the INITIAL step (adjusted at
# runtime to hold maxCo).  writeInterval 20 s writes at 20, 40, 60 s (adjustable
# run-time write control), so avg1's window closes at 40 and avg2's at 60.
# purgeWrite 0 keeps every write directory (40 and 60 are both needed).
END_TIME, DELTA_T, WRITE_INTERVAL, PURGE_WRITE = 60, 0.001, 20, 0
# transient time-step control (K0h section 7.1')
MAX_CO, MAX_DELTA_T = 2.0, 0.05
SPINUP_END = 20            # s, discarded; avg1 = [20,40], avg2 = [40,60]
AVG1_START, AVG1_END = 20, 40
AVG2_START, AVG2_END = 40, 60
...
# guard.
# K0h pre-registration section 8: per-arm hard stop = 10x that arm's POINT line.
# POINT lines: L1 49.15, L2 96.34, C_lam 72.25 core-min (section 8 table).
CAP_CORE_MIN = {
    "M1_c": 491.5, "M2_c": 491.5,     # 10 x 49.15
    "M1_m": 963.4, "M2_m": 963.4,     # 10 x 96.34
    "C_lam": 722.5,                    # 10 x 72.25
}
# timeout_s = cap_core_min * 60 / ranks, ranks == 1 (K0h section 8):
#   491.5 -> 29490 s ; 963.4 -> 57804 s ; 722.5 -> 43350 s
FROZEN_TIMEOUT_S = {491.5: 29490, 963.4: 57804, 722.5: 43350}
...
# --- the FIVE authorised K0h arms (K0h section 5) ------------------------
# L1+L2, two turbulent closures, plus C_lam the discrimination control.  L3 and
# the guard/sweep arms (B_hi, I_hi, M1_m_seed) are DEFINED-AND-NOT-AUTHORISED.
CASES = {
...
#  build_k0h.py REFUSES (exit 2) rather than infer one."
SEED = {
...
# --- fvSolution, K0h section 7.1' -- TRANSIENT PIMPLE --------------------
# The Final variants get relTol 0 (converge the last corrector of each step).
# The outer-corrector residualControl (1e-4 on p_rgh and U) is a PER-TIME-STEP
# convergence floor -- NOT the stationarity test, which is is_stationary in
# analyse_k0h.py on the time-average.
FV_SOLUTION = """solvers
{
    "(p_rgh|p_rghFinal)"
    {
...
def grading_pair(first_cell, length, n_cells):
    ...
    the form check_k0h_mesh.py condition D can catch when it is wrong."""
    m = max(n_cells // 2, 1)
...
# --- windowed time-average (K0h section 7.1', ITEM-A ROBUST FORM) ---------
# ONE fieldAverage function object with restartOnOutput true, NOT two
# overlapping FOs.  Averaging begins at the end of the spin-up
# (timeStart AVG1_START = 20 s) and the accumulator is RESET at every write.
# With writeInterval WRITE_INTERVAL (20 s) the solver writes at 20, 40, 60 s, so
#     dir 20/ : a one-sample spin-up write, DISCARDED (never read)
#     dir 40/ : the [20, 40] s window == avg1 (analyse reads AVG1_TIME = "40")
#     dir 60/ : the [40, 60] s window == avg2 (analyse reads AVG2_TIME = "60",
#               the GRADED window)
# Because EXACTLY ONE FO writes each directory, the dir-40 = [20,40] /
# dir-60 = [40,60] provenance does NOT depend on any FO listing order or on
# which FO wins a shared-boundary write.  The earlier two-FO form (avg2 first,
# avg1 second) relied on avg1 winning the t=40 write collision -- behaviour
# OpenFOAM does not document, and a swap gave a SILENT false-stationary PASS.
# restartOnOutput removes the shared-boundary write entirely; the intended
# windows [20,40] and [40,60] are byte-identical to K0h section 7.1'.
FIELD_AVERAGE_FUNCTIONS = f"""functions
{{
    windowAvg
    {{
        type            fieldAverage;
        libs            (fieldFunctionObjects);
        timeStart       {AVG1_START};
...
def system_dir(case, thickness, gaps):
    ...
    cd = ("application     buoyantBoussinesqPimpleFoam;\nstartFrom       startTime;\n"
          "startTime       0;\nstopAt          endTime;\n"
          f"endTime         {END_TIME};\ndeltaT          {DELTA_T};\n"
...
# --------------------------------------------------------------------------
# THE CONSUMER-SIDE COMPLETENESS ASSERTION (K0h section 3A).
#
...
# phi, TMean and UMean are SOLVER/fieldAverage generated: never staged in 0/, so
# the preflight excludes them from the required-in-0/ set (they must be present
# at endTime, which mark_done_k0h.py checks).
SOLVER_GENERATED = ("phi", "TMean", "UMean")
...
def assert_zero_complete(case_dir, verbose=True):
    ...
    if required is None:
        refuse(f"{case_dir}: the required field set could not be enumerated. "
               f"{note}")
    # WHICH DIRECTORY IS ASSERTED AGAINST, AND WHY BOTH ARE REACHABLE.
    # Section 7.5's order of operations runs `build_k0h.py --preflight` BEFORE
    # `launch_k0h.sh`, and after the clause-7 repair there is no `0/` at that
    # point -- the launcher creates it from `0.orig` immediately before the
    # solver.  So the assertion falls back to the STAGING directory when `0/`
    # does not yet exist.  `0/` IS STILL PREFERRED WHEN IT EXISTS: the
    # launcher's own in-launch call (its section 1') runs AFTER the stage, and
    # that call must assert against the bytes THE SOLVER WILL ACTUALLY READ,
    # not against the copy they came from.  A fallback, inserted; the `0/`
    # path is not replaced.
    zdir = os.path.join(case_dir, "0")
    staged_from = "0/"
    if not os.path.isdir(zdir):
        zdir = os.path.join(case_dir, "0.orig")
        staged_from = "0.orig/ (pre-launch: the launcher has not staged 0/ yet)"
    if not os.path.isdir(zdir):
        refuse(f"{case_dir}: neither 0/ nor 0.orig/ exists, so no field is "
               f"staged at all")
    missing = [f for f in required
               if not (os.path.isfile(os.path.join(zdir, f))
                       or os.path.isfile(os.path.join(zdir, f + ".gz")))]
    if missing:
        refuse(f"{case_dir}: {staged_from} is MISSING {missing}, which the "
               f"solver REQUIRES for closure {closure!r}.  The assertion is against "
    ...
    if verbose:
        print(f"  PRE-FLIGHT OK  {os.path.basename(case_dir)}: closure "
              f"{closure}, requires {required}, all present in {staged_from}")
...
def write_case(root, case, gaps):
    ...
    # ---- STAGING DIRECTORY: `0.orig`, NOT `0`.  THE REPAIR OF THE DEAD ----
    # ---- CLAUSE-7 LEVER (K0H_AGE_GUARD_RESTART_RULING_2026-09-10 section 7.4)
    #
    # THE DEFECT THIS REPLACES.  This builder used to create `0/` ITSELF and
    # touch `0/T` here.  Clause 7 -- `mark_done_k0h.py --launch-guard`, which
    # REFUSES a case in which `0` or a numeric time directory already exists --
    # was therefore not merely uncalled but UNCALLABLE: invoked AFTER the build
    # it would have refused EVERY K0h case, and invoked BEFORE the build it had
    # nothing to judge.  A guard that cannot be placed anywhere in the sequence
    # is a lever nobody can pull.
    #
    # THE PATTERN ADOPTED IS THE LAB'S OWN, NOT A NEW ONE.
    # `T1b_L4_AMENDMENT.md` section 7: "`run_one_t1b_L4.sh` creates `0` from
    # `0.orig` and touches `0/T` LAST ... G3 refuses a case in which `0` or any
    # numeric time directory already exists".  Implemented at
    # `run_one_t1b_L4.sh:32,40,45-46` and `launch_t4d.sh:158-160`.
    #
    # ONE CHANGE, TWO CLAUSES REPAIRED.  With no `0/` at build time clause 7
    # becomes JUDGEABLE, and with `0/T` created and touched by the LAUNCHER
    # clause 6's age guard becomes MEANINGFUL -- `0/T` dates the run allowed to
    # produce the answer, not the build that staged it.  Under the old
    # ordering a case built once and launched twice carried a `0/T` older than
    # BOTH runs' fields, so the age guard could not tell them apart; clause 7
    # is what removes that case, and it could not be called.
    #
    # `location` IN THE FoamFile HEADER STAYS `"0"`.  The header names where
    # the file WILL BE READ FROM, which is `0/` after the launcher stages it,
    # not the staging directory it is written into.  `numeric_times()`
    # (`mark_done_k0h.py:...`, `re.fullmatch(r"[0-9]+(\.[0-9]+)?")`) does NOT
    # match `0.orig`, and OpenFOAM does not read it as a time directory, so the
    # staging directory is invisible to clause 7 and to the solver alike --
    # verified, not assumed (build selftest, ZERO_STAGING arm).
    for sub, hdr, files in (("0.orig", "0", zero_dir(case)),
                            ("constant", "constant", constant_dir(case)),
                            ("system", "system", system_dir(case, t, gaps))):
        os.makedirs(os.path.join(d, sub))
        for name, body in files.items():
            open(os.path.join(d, sub, name), "w").write(
                with_header(name, body, hdr))
    # RULING 4's two refutability assertions, READ BACK FROM DISK.
    assert_two_d_registration(d, t)
    # NO `0/T` TOUCH HERE, AND ITS ABSENCE IS THE POINT.  The age-guard datum
    # is created by `launch_k0h.sh` immediately before the solver (its section
    # 3', `cp -r 0.orig 0` then `touch 0/T` LAST).  What the builder owes is
    # that the datum WILL EXIST, so it refuses now rather than letting the
    # launcher discover it at launch time.
    if not os.path.isfile(os.path.join(d, "0.orig", "T")):
        refuse(f"{d}: 0.orig/T was not written, so the launcher would have no "
               f"age-guard datum to create 0/T from.  Refusing at build time "
               f"rather than at launch time.")
    return d
...
def selftest():
    ...
    import tempfile
    print("=" * 74)
    print("build_k0h.py -- SELFTEST (planted controls)")
    print("=" * 74)
    fails = []
    ...
    check_("FIVE authorised arms (L1+L2 + C_lam control)", len(CASES) == 5,
           str(len(CASES)))
    check_("the five arms are exactly M1_c M1_m M2_c M2_m C_lam (no L3, no "
           "B_hi/I_hi/M1_m_seed)",
           set(CASES) == {"M1_c", "M1_m", "M2_c", "M2_m", "C_lam"},
           ",".join(sorted(CASES)))
    ...
    check_("RANKS is asserted == 1, so cap_core_min * 60 / ranks is the "
           "identity", RANKS == 1)
    check_("every one of the five arms carries a registered cap",
           set(CAP_CORE_MIN) == set(CASES), str(len(CAP_CORE_MIN)))
    check_("the L2 arms take the 963.4 core-min cap and L1 the 491.5 (K0h 8)",
           CAP_CORE_MIN["M1_m"] == 963.4 and CAP_CORE_MIN["M1_c"] == 491.5)
    drift = []
    for c in CASES:
        frozen, computed = timeout_for(c)
        if abs(frozen - computed) > 0.5:
            drift.append((c, frozen, computed))
    check_("the K0h timeout table reproduces cap_core_min * 60 / 1 EXACTLY on "
           "every arm (no rounding drift)",
    ...
    #   SELFTEST ITSELF WOULD HAVE CREATED K0h_runs/ AND DESTROYED THE
    ...
    try:
        mutated = os.path.join(gap_tmp, "build_k0h_gap_reopened.py")
        ...
        if not os.path.isfile(foam_bashrc):
            ...
            # THE STAGE STEP, MODELLED ON THE LAUNCHER AND NOT SHORT-CUT.
            # After the clause-7 repair the builder writes `0.orig/`; the
            # solver reads `0/`.  This arm therefore performs the SAME two
            # steps `launch_k0h.sh` performs (`cp -r 0.orig 0`, then `touch
            # 0/T` LAST), so the arm exercises the real launch sequence
            # instead of a fixture that no launcher would produce.
            shutil.copytree(os.path.join(bm_case, "0.orig"),
                            os.path.join(bm_case, "0"))
            os.utime(os.path.join(bm_case, "0", "T"), None)
            cd_path = os.path.join(bm_case, "system", "controlDict")
            cd_txt = open(cd_path).read()
            open(cd_path, "w").write(
                cd_txt.replace("endTime         60;", "endTime         0.01;")
                      .replace("deltaT          0.001;", "deltaT          0.005;"))
            r5 = subprocess.run(
                ["bash", "-c",
                 f"source {foam_bashrc} >/dev/null 2>&1 && "
                 f"buoyantBoussinesqPimpleFoam -case {bm_case}"],
                capture_output=True, text=True)
            reached = "Time = " in r5.stdout
            check_("THE REAL SOLVER READS EVERY 0/ FIELD THIS BUILDER WRITES: "
                   "buoyantBoussinesqPimpleFoam runs a couple of steps and "
                   ...
                   f"rc={r5.returncode}, reached a Time line: {reached}")
        ...
        ranks_mut = os.path.join(gap_tmp, "build_k0h_ranks4.py")
    ...
    try:
        ...
        # ---- THE CLAUSE-7 STAGING ARM, BOTH DIRECTIONS -------------------
        # The builder must write `0.orig/` and must NOT write `0/`.  Both
        # halves are asserted: a builder that wrote BOTH would leave clause 7
        # exactly as dead as it was, and an arm that checked only for
        # `0.orig/` would not see that.
        check_("ZERO_STAGING: the builder writes 0.orig/ ...",
               os.path.isdir(os.path.join(d, "0.orig")))
        check_("ZERO_STAGING: ... and writes NO 0/, which is what makes "
               "clause 7 judgeable at launch time",
               not os.path.exists(os.path.join(d, "0")),
               str(sorted(os.listdir(d))))
        for f in ("T", "U", "p_rgh", "alphat", "k", "omega", "nut"):
            check_(f"M1_m 0.orig/{f} written",
                   os.path.isfile(os.path.join(d, "0.orig", f)))
        check_("M1_m has no 0.orig/epsilon (kOmegaSST)",
               not os.path.exists(os.path.join(d, "0.orig", "epsilon")))
        # THE STAGING DIRECTORY MUST NOT LEAK INTO THE FIELD BYTES.
        # MEASURED, and my first version of this arm asserted the OPPOSITE and
        # was wrong: `field_file()` emits its own FoamFile header carrying NO
        # `location` entry at all, so `with_header()` passes those bodies
        # through untouched.  The property that actually matters is therefore
        # not "location says 0" but "nothing in a staged field names the
        # staging directory" -- which is what makes `cp -r 0.orig 0` a
        # byte-faithful stage rather than a rewrite.
        leaked = [f for f in sorted(os.listdir(os.path.join(d, "0.orig")))
                  if "0.orig" in open(os.path.join(d, "0.orig", f)).read()]
        check_("NO staged field names the staging directory, so the launcher's "
               "`cp -r 0.orig 0` stages the solver's bytes unchanged",
               not leaked, str(leaked))
        check_("and the field files carry no `location` entry at all "
               "(field_file emits its own header) -- measured, not assumed",
               "location" not in open(os.path.join(d, "0.orig", "T")).read())
        dl = write_case(tmp, "C_lam", testonly)
        check_("C_lam 0.orig/ contains EXACTLY T U p_rgh alphat (A5.5.4)",
               sorted(os.listdir(os.path.join(dl, "0.orig")))
               == ["T", "U", "alphat", "p_rgh"],
               str(sorted(os.listdir(os.path.join(dl, "0.orig")))))
        dm2 = write_case(tmp, "M2_m", testonly)
        check_("M2_m (RNGkEpsilon) 0.orig/ carries epsilon and NOT omega",
               os.path.isfile(os.path.join(dm2, "0.orig", "epsilon"))
               and not os.path.exists(os.path.join(dm2, "0.orig", "omega")))
        tm = open(os.path.join(d, "0.orig", "T")).read()
        ...
        check_("controlDict carries the registered TRANSIENT timings and the "
               "transient solver",
               all(x in cd for x in ("application     buoyantBoussinesqPimpleFoam",
                                     "endTime         60", "deltaT          0.001",
                                     "writeInterval   20", "adjustTimeStep  yes",
                                     "maxCo           2.0")))
        check_("controlDict carries ONE restartOnOutput fieldAverage window "
               "(timeStart 20; resets at each write so dir40=[20,40], "
               "dir60=[40,60]) producing TMean/UMean -- order-independent, no "
               "shared-boundary collision (ITEM-A robust form)",
               "fieldAverage" in cd and "restartOnOutput true" in cd
               and "timeStart       20" in cd
               and cd.count("type            fieldAverage;") == 1
               and "prime2Mean on" in cd)
        fss = open(os.path.join(d, "system", "fvSchemes")).read()
        check_("fvSchemes ddtScheme is Euler (transient), not steadyState",
               "default         Euler" in fss and "steadyState" not in fss)
        fs = open(os.path.join(d, "system", "fvSolution")).read()
        check_("fvSolution carries a PIMPLE block with nOuterCorrectors 2 and a "
               "per-step residualControl (1e-4 on p_rgh and U)",
               "PIMPLE" in fs and "nOuterCorrectors 2" in fs
               and "residualControl" in fs and "SIMPLE\n" not in fs)
        ...
        r2 = subprocess.run([sys.executable, "-c",
                             "import sys; sys.path.insert(0, %r); import build_k0h as B;"
    ...
    print("    verification/runs/F14-cooling-ladder/K0h_runs until the")
