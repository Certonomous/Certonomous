...
Usage:
    python3 analyse_k0g.py --root <K0g_runs dir>
    python3 analyse_k0g.py --selftest
"""
import argparse
import math
import os
...
# NO ROACHE TRIPLE IS FORMED (a triple needs three levels; K0g authorises two).
...
# K0g GRADES THE TIME-AVERAGE, NOT THE INSTANTANEOUS FIELD.  The graded fields
...
# K0g pre-registration section 7.1' -- STATIONARITY, not steady convergence.
...
def is_stationary(case_dir, n_cells):
    """STATIONARY (K0g section 7.1') means the running TIME-AVERAGE is not
    ...
    graded rows NOT A RESULT (K0g section 7.1').
    ...
    measured a persistent limit cycle over 60 000 iterations), so K0g grades the
...
def load_case_fields(case_dir):
    ...
    # THE BOUNDARY HALF OF THE READER (K0g section V).  Loaded here, beside the
...
def row_verdict(row, levels_converged, triple, guard_verdict,
    ...
    # 1. the graded arm is NOT STATIONARY (K0g section 7.1' replaces K0f's
    ...
    # 2. the grid triple not CONVERGING -- NOT FORMED at L1+L2 (K0g grades no
...
def guard_DC(rows_lam, rows_m1m, lam_stationary):
    ...
    K0g predicts C_lam does NOT reach stationarity (a laminar model at Ra 2.14e9
...
def require_markers(root):
    """analyse_k0g.py refuses (exit 2) unless all TEN DONE.<case> markers are
...
def assert_extraction_equivalence(root, verbose=True):
    """THE STANDING PRE-GRADING GATE (K0g section V, the tightening).

    REFUSES (exit 2) unless `check_k0g_extraction_equivalence.py` PASSES on the
    ...
    checker = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           "check_k0g_extraction_equivalence.py")
    ...
    if verbose:
        print("\nPRE-GRADING EQUIVALENCE GATE (K0g section V) on "
...
def analyse(root, verbose=True, expect_sha=None, scratch=None):
    ...
    if verbose:
        print("=" * 74)
        print("K0d COMPARATOR -- analyse_k0g.py")
    ...
    scratch = scratch or os.path.join(root, ".analyse_k0g_plants")
    ...
    # STATIONARITY replaces steady convergence (K0g section 7.1'); NO triple is
    ...
    # guard_B / guard_I / guard_seed need B_hi / I_hi / M1_m_seed, which K0g does
    # NOT authorise (deferred to a K0g extension or K0h), so they are not run.
    ...
    if verbose:
        ...
        print("  (guards B / I / SEED are DEFERRED: their arms B_hi / I_hi / "
              "M1_m_seed are not authorised in K0g)")
    ...
    if verbose:
        ...
        if not held:
            ...
            print("  G -- a Roache triple needs THREE levels; K0g authorises "
                  "L1+L2 only,")
...
def selftest():
    ...
    _FAILS = []
    print("=" * 74)
    print("analyse_k0g.py -- SELFTEST.  Every probe: a POSITIVE that must stay")
    ...
    print("=" * 74)
    tmp = tempfile.mkdtemp(prefix="analyse_k0g_selftest_")
    root = os.path.join(tmp, "K0g_runs")
    ...
    # ---------------- Roache math: RETAINED, NOT USED IN K0g GRADING -------
    print("\n-- ROACHE MATH (retained; K0g forms NO triple at L1+L2) --")
    # The pure classify/order/GCI functions are kept and unit-tested so a future
    # L3 re-registration inherits proven math, but K0g's grading path forms no
    ...
    # K0g: clause 1 is STATIONARITY (not steady convergence), clause 2 (the
    # triple) is DISABLED (triple=None; K0g forms none at L1+L2).
    print("\n-- THE VERDICT LADDER, IN ITS REGISTERED ORDER --")
    ...
    check_("ORDER 4: no primary on disk gives BLOCKED, not GATE FAIL and not "
           "PASS (K0g's steady state: P NOT OBTAINED)", v5 == BLOCKED, v5)
    ...
    print("\n-- THE WALL READER (K0g section V), BOTH BRANCHES DRIVEN --")
    ix = cfx["idx"]
...
def main(argv=None):
    ...
    ap.add_argument("--root", help="the K0g_runs directory")
