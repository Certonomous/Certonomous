#!/usr/bin/env python3
"""
F21 -- BUILD ONE LADDER LEVEL into a destination directory (called by
run_f21.sh once per level).  REFUSES a destination holding `0/` or a numeric
...
tree.  Mesher and postProcess run here, so never call it against the case tree.
  ...
  4. write 0/p (uniform: the exact pressure is a constant), then 0/U LAST.
Zero `assert` (L-332).
"""
import os
import re
...
import exact_f21 as EX            # noqa: E402
import foam_io_f21 as FIO          # noqa: E402
...
def die(msg, rc=1):
    sys.stderr.write("ABORT (build_f21): %s\n" % msg)
    sys.exit(rc)
...
def main(argv):
    if len(argv) != 3 or argv[1] != "--level":
        die("usage: build_f21.py <dest_dir> --level <coarse|medium|fine>")
    ...
    for f in ("fvSchemes", "fvSolution", "fvOptions"):
