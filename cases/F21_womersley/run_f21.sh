#!/usr/bin/env bash
# F21 -- LAUNCHER for pulsatile Womersley channel flow, ladder 128^2 / 256^2 / 512^2
# (pimpleFoam in PISO mode, laminar, cosine body-force drive, alpha = 5).
#
...
# (here and again inside build_f21.py).
#
...
ROOT="/home/ubuntu/Certonomous/cases/F21_womersley"
CASE_SRC="$ROOT/case"
EXACT="$ROOT/exact_f21.py"
BUILD="$ROOT/build_f21.py"
GRADER="$ROOT/grade_f21.py"
FIO="$ROOT/foam_io_f21.py"
RUN_ROOT="/home/ubuntu/Certonomous/verification/runs/F21_runs"
FOAM_BASHRC="/usr/lib/openfoam/openfoam2606/etc/bashrc"

CAP_CORE_MIN=1500           # must equal grade_f21.py::CAP_CORE_MIN
END_TIME=12.25              # 12 T + T/4; must equal controlDict.template endTime and exact_f21.T_END

# name  N  steps  ranks
LEVELS=("coarse 128 1176 1" "medium 256 2352 1" "fine 512 4704 1")
# projected SERIAL seconds per level: base rate 10.71 us per cell-step MEASURED on
# THIS case's coarse grid (128^2, 24-step scratch smoke probe, ExecutionTime 4.21 s,
# DIC-PCG ~238 iterations per p solve) -- 2x the F18 icoFoam basis (5.47 us at
# 128^2) that the dispatch named; grown per doubling by F18's MEASURED +72 % / +75 %
# (verification/runs/F18_runs/{coarse,medium,fine}/log.icoFoam, PCG iterations
# growing with N) to 18.4 us at 256^2 and 32.2 us at 512^2, times cell-steps
# 1.93e7 / 1.54e8 / 1.23e9.  DERIVED, NOT MEASURED on the medium and fine levels.
# Used ONLY by the pre-level projected-cap check.
declare -A PROJ_SERIAL_S=( [coarse]=206 [medium]=2836 [fine]=39710 )
DECOMP_METHOD="none"
...
for f in "$CASE_SRC/0/U.template" "$CASE_SRC/0/p.template" \
         ...
         "$CASE_SRC/system/fvSchemes" "$CASE_SRC/system/fvSolution" "$CASE_SRC/system/fvOptions" \
...
python3 "$EXACT" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: exact_f21.py --selftest did not pass; NOTHING may be launched against it"; exit 1; }
python3 "$GRADER" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: grade_f21.py --selftest did not pass; the grading path is not established"; exit 1; }
python3 -O "$GRADER" --selftest > /dev/null 2>&1
[ $? -eq 2 ] || { echo "ABORT: grade_f21.py did not exit 2 under python3 -O; the -O refusal is not armed"; exit 1; }
say "INSTRUMENT GREEN: exact_f21 and grade_f21 selftests pass; grade_f21 exits 2 under -O."
...
python3 -c "import sys; sys.exit(0 if abs(float('$CAP_GRADER') - $CAP_CORE_MIN) < 1e-9 else 1)" \
  || { echo "ABORT: launcher cap $CAP_CORE_MIN disagrees with grade_f21.py CAP_CORE_MIN $CAP_GRADER"; exit 1; }
...
python3 - "$CASE_SRC/constant/transportProperties" "$CASE_SRC/system/controlDict.template" "$END_TIME" <<'PYCHK' \
  || { echo "ABORT: nu on disk disagrees with exact_f21.NU, or controlDict.template endTime is not the registered value"; exit 1; }
import re, sys
sys.path.insert(0, "/home/ubuntu/Certonomous/cases/F21_womersley")
import exact_f21 as EX
...
PYCHK
say "nu ON DISK AGREES with exact_f21.NU; controlDict.template endTime == registered $END_TIME."

if [ "$PREFLIGHT" = "1" ]; then
  say ""
  say "PREFLIGHT ONLY -- ZERO COMPUTE. blockMesh was NOT run. build_f21.py was NOT called."
  say "decomposition method: $DECOMP_METHOD"
  say "decomposition seed:   $DECOMP_SEED"
  for L in "${LEVELS[@]}"; do
    set -- $L
    say "  level $1: $2 x $2 x 1 cells, $3 steps (dt = $END_TIME/$3 s), ranks $4, target $RUN_ROOT/$1"
...
for L in "${LEVELS[@]}"; do
  ...
  python3 "$BUILD" "$CD" --level "$NAME" > "$CD/log.build" 2>&1 \
    || { echo "ABORT: build_f21.py failed at level $NAME; see $CD/log.build"; exit 1; }
  ...
  pimpleFoam -case "$CD" > "$CD/log.pimpleFoam" 2>&1
  RC=$?
  echo "$RC" > "$CD/RC.txt"
