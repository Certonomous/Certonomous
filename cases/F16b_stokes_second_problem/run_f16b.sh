#!/usr/bin/env bash
# F16b -- LAUNCHER for Stokes' second problem (oscillating wall), CASE F16b:
# +/-x CYCLIC (F16's `empty` x made Ux a non-solved component; F16_runs/
# RESULTS_R2.md Finding B). Copied from cases/F16_stokes_second_problem/run_f16.sh
# at 2aea29d9 (Amendment-2 source form); paths, grader, cap and the MEASURED
# per-level projections are the only edits.
#
...
ROOT="/home/ubuntu/Certonomous/cases/F16b_stokes_second_problem"
CASE_SRC="$ROOT/case"
EXACT="$ROOT/exact_stokes.py"
GRADER="$ROOT/grade_f16b.py"
RUN_ROOT="/home/ubuntu/Certonomous/verification/runs/F16b_runs"
FOAM_BASHRC="/usr/lib/openfoam/openfoam2606/etc/bashrc"

CAP_CORE_MIN=5              # must equal grade_f16b.py::CAP_CORE_MIN
END_TIME=40                 # 40 periods; must equal grade_f16b.py END_TIME

# name  ny  steps-per-period  ranks
LEVELS=("coarse 56 400 1" "medium 112 800 1" "fine 224 1600 1")
# projected SERIAL seconds per level -- MEASURED on F16 run 2aea29d9, the same
# dictionaries and step counts on this box (ClockTime 3 / 7 / 15 s; F16_runs/
# */log.icoFoam). Used ONLY by the pre-level projected-cap check.
declare -A PROJ_SERIAL_S=( [coarse]=3 [medium]=7 [fine]=15 )
...
python3 "$GRADER" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: grade_f16b.py --selftest did not pass; the grading path is not established"; exit 1; }
python3 -O "$GRADER" --selftest > /dev/null 2>&1
[ $? -eq 2 ] || { echo "ABORT: grade_f16b.py did not exit 2 under python3 -O; the -O refusal is not armed"; exit 1; }
say "INSTRUMENT GREEN: exact_stokes and grade_f16b selftests pass; grade_f16b exits 2 under -O."
...
python3 -c "import sys; sys.exit(0 if abs(float('$CAP_GRADER') - $CAP_CORE_MIN) < 1e-9 else 1)" \
  || { echo "ABORT: launcher cap $CAP_CORE_MIN disagrees with grade_f16b.py CAP_CORE_MIN $CAP_GRADER"; exit 1; }
...
import re, sys
sys.path.insert(0, "/home/ubuntu/Certonomous/cases/F16b_stokes_second_problem")
