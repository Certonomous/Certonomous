#!/usr/bin/env python3
"""F22 -- minimal reader/writer for OpenFOAM ascii volVectorField / volScalarField
files, used by build_f22.py (to write 0/U from the mesh's own 0/C) and by
grade_f22.py (to read U at endTime).  Refusals are `raise`; zero
`assert` (L-332)."""
import re
...
def plant_ramp_into_vector_file(src, dst, comp, slope, L, yc):
    """Copy an ascii volVectorField file adding `slope * (y_c + L)` to component
    `comp` of EVERY internalField entry (a ramp in y whose curl is exactly
    -slope for comp = 0), rewriting only that list.  Returns rows changed."""
    text = open(src).read()
    m = re.search(r"internalField\s+nonuniform\s+List<vector>\s*(\d+)\s*\(", text)
    if not m:
        raise FieldFormatError("%s: no nonuniform vector internalField to plant into" % src)
    n = int(m.group(1))
    if n != len(yc):
        raise FieldFormatError("%s: %d entries but %d cell centres" % (src, n, len(yc)))
    pos = m.end()
    pat = re.compile(r"\s*\(\s*(%s)\s+(%s)\s+(%s)\s*\)" % (_NUM, _NUM, _NUM))
    pieces = [text[:pos]]
    for k in range(n):
