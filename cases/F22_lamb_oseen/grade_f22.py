#!/usr/bin/env python3
"""
F22 -- THE GRADING PATH for the 2-D decaying Lamb-Oseen vortex (icoFoam, T = 4 s,
far-field velocity fixed to the potential vortex).  It calls
...
file, exact_f22.py, foam_io_f22.py, build_f22.py (L-332); hard `-O` refusal.

TWO GATES at t = T, both from ONE declared parameter (BAND_FACTOR) applied to
the discretisation model's prediction:
  G-F22-1  E2 = sqrt(mean |U_h - U_exact(T)|^2) / U_ref over every cell
  G-F22-2  PEAK VORTICITY at T = max over interior cells of the grader's own
           central-difference curl of the cell-centred U.  ITS REFERENCE IS THE
           SAME STENCIL APPLIED TO THE EXACT VELOCITY SAMPLED AT THE FINE LEVEL'S
           CELL CENTRES (the F17b AMENDMENT 1 lesson applied at registration):
           the stencil's own O(h^2) under-read of the Gaussian peak cancels, and
           a zero-error solver reads zero.  The pointwise exact peak
           Gamma/(4 pi nu t') is printed beside it and is NOT the reference.

THERE IS NO PLATEAU GATE, AND THAT IS SAID RATHER THAN HIDDEN: the graded
quantities are values at the fixed instant t = T of a decaying transient, so
`plateau_states` is passed as None and recorded as ABSENT (VERIFICATION_CHARTER
section 9).  Rule 5 limb (1) is a CENSUS over EVERY time step's final pressure
residual against the solver's own tolerance -- not a two-point sample.
...
if not __debug__:
    sys.stderr.write("REFUSED: grade_f22.py must not run under `python3 -O` (the shared "
...
import exact_f22 as EX                  # THE EXACT SOLUTION and the discretisation model.
import foam_io_f22 as FIO               # THE READERS.
...
BAND_FACTOR = 3.0                       # the one declared parameter; see exact_f22 docstring
P_SOLVER_TOL = 1.0e-9                   # matches system/fvSolution, checked below
CAP_CORE_MIN = 1100.0                  # see the pre-registration section 8
...
def e2_from_files(u_path, xc, yc, t=END_TIME):
    """G-F22-1: E2(T) = sqrt(mean |U_h - U_exact(x_c, T)|^2) / U0."""
    U = read_U(u_path)
    if U.shape[0] != len(xc):
...
def structured(xc, yc, n):
    ...
    h = EX.h_of(n)
    ii = np.rint((xc + EX.L) / h - 0.5).astype(int)
    jj = np.rint((yc + EX.L) / h - 0.5).astype(int)
    if np.max(np.abs((xc + EX.L) / h - 0.5 - ii)) > 1e-6 or np.max(np.abs((yc + EX.L) / h - 0.5 - jj)) > 1e-6:
...
def peak_vorticity_from_files(u_path, xc, yc, n):
    """G-F22-2: max over interior cells of the grader's central-difference curl."""
    U = read_U(u_path)
    if U.shape[0] != len(xc):
        refuse("%s carries %d cells but 0/C carries %d" % (u_path, U.shape[0], len(xc)))
    ii, jj = structured(xc, yc, n)
    Fu = np.empty((n, n)); Fv = np.empty((n, n))
    Fu[jj, ii] = U[:, 0]; Fv[jj, ii] = U[:, 1]
    return float(np.max(EX.curl_h(Fu, Fv, EX.h_of(n))))


def peak_vorticity_reference(n):
    """The G-F22-2 REFERENCE: the exact velocity at the level's own cell centres
    through the SAME curl stencil the solved field goes through."""
    _h, xg, yg = EX.grid(n)
    X, Y = np.meshgrid(xg, yg)
    return float(np.max(EX.curl_h(EX.u_exact(X, Y, END_TIME), EX.v_exact(X, Y, END_TIME), EX.h_of(n))))
...
def plant_control_e2(u_path, xc, yc, level):
    ...
    tmp = tempfile.mkdtemp(prefix="f22_plant_e2_")
    try:
...
def plant_control_peak(u_path, xc, yc, n, level):
    """A uniform Ux offset is invisible to a curl; the plant is a Ux ramp in y,
    u += d * (y + L) / (2 L), whose curl is exactly -d/(2 L) everywhere, so the
    peak must move by exactly -d/(2 L)."""
    d = RT.PLANT
    before = peak_vorticity_from_files(u_path, xc, yc, n)
    tmp = tempfile.mkdtemp(prefix="f22_plant_pk_")
    try:
        work = os.path.join(tmp, "U")
        if FIO.plant_ramp_into_vector_file(u_path, work, 0, d / (2.0 * EX.L), EX.L, yc) == 0:
            refuse("nothing to plant into %s" % u_path)
        after = peak_vorticity_from_files(work, xc, yc, n)
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
    expected = -d / (2.0 * EX.L)
    if abs((after - before) - expected) > 1e-10:
        refuse("PLANTED-ZERO CONTROL FAILED for peak vorticity on %s: a Ux ramp of curl %.6e must move the "
               "peak by exactly that; the reader moved it %.6e" % (u_path, expected, after - before))
    return RT.external_plant_control("peak_vorticity_from_files", before, after, plant=expected,
                                     artifact=u_path, level=level)


def control_peak_reference_same_stencil():
    """Driven at the FINE size: the exact field written in the real format and
    read through the real peak reader returns ZERO error against the reference
    (to round-off); a planted Ux ramp is read back exactly."""
    n = NSIDE["fine"]
    ref = peak_vorticity_reference(n)
    d = RT.PLANT
    tmp = tempfile.mkdtemp(prefix="f22_ref_")
    try:
        z = np.zeros((n, n))
        up, xc, yc = synth_case(tmp, n, z, z)
        v0 = peak_vorticity_from_files(up, xc, yc, n)
        work = os.path.join(tmp, "U_planted")
        if FIO.plant_ramp_into_vector_file(up, work, 0, d / (2.0 * EX.L), EX.L, yc) == 0:
            refuse("nothing to plant into %s" % up)
        v1 = peak_vorticity_from_files(work, xc, yc, n)
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
    if abs(v0 - ref) > 1e-11:
        refuse("SAME-STENCIL REFERENCE CONTROL FAILED: exact field read back as %.17g against reference %.17g" % (v0, ref))
    if abs((v1 - ref) + d / (2.0 * EX.L)) > 1e-10:
        refuse("SAME-STENCIL REFERENCE CONTROL FAILED: planted ramp curl %.6e read back as %.6e" % (-d / (2.0 * EX.L), v1 - ref))
    return dict(control="PZ-F22-PEAK_reference_same_stencil_zero_error_and_ramp_read_back", reference=ref,
                pointwise_exact=EX.omega_peak_exact(), stencil_error=ref - EX.omega_peak_exact(),
...
def control_field_classes_separate():
    tmp = tempfile.mkdtemp(prefix="f22_l342_")
    try:
        cd = os.path.join(tmp, "coarse")
        os.makedirs(os.path.join(cd, "0")); os.makedirs(os.path.join(cd, "4"))
        ...
        for f in ("U", "p"):
            open(os.path.join(cd, "4", f), "w").write("y")
        ...
        shutil.rmtree(tmp, ignore_errors=True)
    return dict(control="PZ-F22-L342_infrastructure_vs_physics_fields_driven_both_ways",
...
def control_grade_ladder_is_called():
    ...
    return dict(control="PZ-F22-GRADE_LADDER_CALLSITE_ast_census_plus_grep_both_ways", ast_call_nodes=calls, passed=True)
...
def control_solver_dicts_match():
    ...
    if not m or abs(float(m.group(1)) - EX.NU) > 1e-15:
        refuse("transportProperties nu does not equal exact_f22.NU")
    ...
    if not re.search(r"application\s+icoFoam\s*;", cd):
        refuse("controlDict.template application is not icoFoam")
    ut = open(os.path.join(HERE, "case", "0", "U.template")).read()
    pt = open(os.path.join(HERE, "case", "0", "p.template")).read()
    for patch in ("left", "right", "bottom", "top"):
        if not re.search(r"%s\s*\{\s*type\s+fixedValue;\s*value\s+nonuniform" % patch, ut):
            refuse("U.template patch %s is not a nonuniform fixedValue (the far-field datum)" % patch)
        if not re.search(r"%s\s*\{\s*type\s+fixedFluxPressure;" % patch, pt):
            refuse("p.template patch %s is not fixedFluxPressure" % patch)
    ml = re.search(r"^L\s+=\s*([0-9.]+)", open(os.path.join(HERE, "exact_f22.py")).read(), re.M)
    bm = open(os.path.join(HERE, "case", "system", "blockMeshDict.template")).read()
    if not ml or ("(-%s -%s 0)" % (ml.group(1), ml.group(1))) not in bm:
        refuse("blockMeshDict.template box does not match exact_f22.L")
    return dict(control="solver_dictionaries_agree_with_registration", nu=EX.NU, p_tol=P_SOLVER_TOL,
                endTime=END_TIME, ddt="backward", far_field="fixedValue U (potential vortex) / fixedFluxPressure p",
                box_half_width=float(ml.group(1)), passed=True)
...
def bands():
    ...
    fine = tab["fine"]
    e2p = fine["E2_pred"]
    pk = peak_vorticity_reference(NSIDE["fine"])
    tol = BAND_FACTOR * abs(fine["peak_err_pred"])
    return {
        "G-F22-1_E2_velocity_L2_at_T": dict(
            ...
            principle=("discretisation-model prediction E2(T) = %.9e at h_fine = %g, dt = %g (linearised "
                       "discrete equations with Dirichlet boundaries forced by the exact field's truncation "
                       "residual, integrated with BDF2 on 48^2 and 96^2, extrapolated with the model order), "
                       "times [1/%g, %g]" % (e2p, fine["h"], fine["dt"], BAND_FACTOR, BAND_FACTOR))),
        "G-F22-2_peak_vorticity_at_T": dict(
            band=(pk - tol, pk + tol), reference=pk, dim=DIM,
            principle=("exact velocity at the fine level's cell centres through the grader's own central-"
                       "difference curl, max over interior cells = %.12f (pointwise exact Gamma/(4 pi nu t') = "
                       "%.12f; the stencil under-reads the Gaussian peak by %.3e and that is removed from the "
                       "reference) +/- %g x the model's predicted peak error (%.9e) = +/- %.9e"
                       % (pk, EX.omega_peak_exact(), pk - EX.omega_peak_exact(), BAND_FACTOR, fine["peak_err_pred"], tol))),
    }
...
def synth_case(tmp, n, eu, ev):
    ...
    open(up, "w").write("FoamFile { version 2.0; format ascii; class volVectorField; object U; }\n"
                        ...
                        "boundaryField { left { type fixedValue; value uniform (0 0 0); } }\n" % FIO.fmt_list(U, "vector"))
    return up, xc, yc


def demonstrate(bnd):
    """ZERO COMPUTE.  The finest INTEGRATED grid's error field (96^2), prolongated
    piecewise-constant onto the FINE grid (768^2) and scaled by the model's
    96^2 -> fine E2 ratio, stands in for the fine field -- built AT the fine size
    so the peak reader's own stencil is the fine level's.  The peak error of the
    prolongated field is not the model's peak prediction (a piecewise-constant
    prolongation has a rougher curl), so the peak rows use the scaled model
    peak error added as a Gaussian bump of the exact vorticity's shape."""
    m = EX.integrated(*EX.MODEL_GRIDS[-1])
    n = NSIDE["fine"]
    k = n // m["n"]
    if k * m["n"] != n:
        refuse("fine grid %d is not an integer refinement of the integrated %d" % (n, m["n"]))
    ...
    # a velocity perturbation whose curl is a Gaussian bump at the origin of amplitude fine["peak_err_pred"]:
    # u_theta = A r_b^2/(2 r) (1 - exp(-r^2/r_b^2)) has curl A exp(-r^2/r_b^2)
    _h, c, _c = EX.grid(n); X, Y = np.meshgrid(c, c)
    rb = math.sqrt(4.0 * EX.NU * (END_TIME + EX.T0))
    R2 = X ** 2 + Y ** 2
    with np.errstate(divide="ignore", invalid="ignore"):
        f = np.where(R2 > 1e-300, (1.0 - np.exp(-R2 / rb ** 2)) / R2, 1.0 / rb ** 2) * rb ** 2 / 2.0
    bu, bv = -Y * f * fine["peak_err_pred"], X * f * fine["peak_err_pred"]
    rows = []
    tmp = tempfile.mkdtemp(prefix="f22_demo_")
    try:
        for scale, want in ((1.0, "inside"), (40.0, "outside")):
            ...
            lo, hi = bnd["G-F22-1_E2_velocity_L2_at_T"]["band"]
            rows.append(dict(gate="G-F22-1_E2_velocity_L2_at_T", construction="exact(T) + %g x scaled model error field" % scale,
                             value=v1, band=[lo, hi], inside=bool(lo <= v1 <= hi), intended=want))
            up2, xc, yc = synth_case(tmp, n, scale * bu, scale * bv)
            v2 = peak_vorticity_from_files(up2, xc, yc, n)
            lo, hi = bnd["G-F22-2_peak_vorticity_at_T"]["band"]
            rows.append(dict(gate="G-F22-2_peak_vorticity_at_T", construction="exact(T) + %g x Gaussian vorticity bump of the model's predicted peak error" % scale,
...
# THE GRADE
# ---------------------------------------------------------------------------
GATES = ("G-F22-1_E2_velocity_L2_at_T", "G-F22-2_peak_vorticity_at_T")
...
def grade_one(root, gate, bnd):
    ...
    for name in LEVEL_NAMES:
        ...
        val = e2_from_files(up, xc, yc) if gate == GATES[0] else peak_vorticity_from_files(up, xc, yc, n)
        it_state, it_detail = iterative_state(log)
        levels.append(dict(name=name, cells=CELLS[name], value=val))
        detail.append(dict(name=name, cells=CELLS[name], value=val, artifact=up, h=EX.h_of(n),
    ...
    pc = plant_control_e2(fd["artifact"], fd["xc"], fd["yc"], fd["name"]) if gate == GATES[0] \
        else plant_control_peak(fd["artifact"], fd["xc"], fd["yc"], fd["n"], fd["name"])
    for d in detail:
        d.pop("xc"); d.pop("yc")
    spec = bnd[gate]
...
def main(argv=None):
    ...
    ap.add_argument("--root", default=os.path.join(REPO, "verification", "runs", "F22_LAMB_OSEEN_runs"))
    ap.add_argument("--out", default=None)
    ap.add_argument("--prereg-commit")
    ap.add_argument("--selftest", action="store_true")
    a = ap.parse_args(argv)

    census = ast_no_asserts([os.path.abspath(__file__), os.path.join(HERE, "exact_f22.py"),
                             os.path.join(HERE, "foam_io_f22.py"), os.path.join(HERE, "build_f22.py")])
    controls = [EX.control_symbolic_substitution(), EX.control_substitution_is_able_to_fail(),
                ...
                control_peak_reference_same_stencil()]
    bnd = bands()
    ...
    out = a.out or os.path.join(a.root, "F22_GRADED.json")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    with open(out, "w") as f:
        json.dump(dict(rung="F22-LAMB-OSEEN", prereg_commit=a.prereg_commit,
                       prereg="verification/campaign/F22_LAMB_OSEEN_PREREGISTRATION.md",
    ...
    print("F22 -- 2-D DECAYING LAMB-OSEEN VORTEX -- TALLY")
    print("=" * 78)
    for r in rows:
