#!/usr/bin/env bash
# F22 -- LAUNCHER for the 2-D decaying Lamb-Oseen vortex, ladder 192^2 / 384^2 / 768^2
# (icoFoam, transient, far-field velocity fixed to the potential vortex).
#
...
# pre-existing `0/`, numeric time or processor* directory in the RUN ROOT or in
# a LEVEL directory is REFUSED, never deleted (here and again inside build_f22.py).
#
...
ROOT="/home/ubuntu/Certonomous/cases/F22_lamb_oseen"
CASE_SRC="$ROOT/case"
EXACT="$ROOT/exact_f22.py"
BUILD="$ROOT/build_f22.py"
GRADER="$ROOT/grade_f22.py"
FIO="$ROOT/foam_io_f22.py"
RUN_ROOT="/home/ubuntu/Certonomous/verification/runs/F22_LAMB_OSEEN_runs"
FOAM_BASHRC="/usr/lib/openfoam/openfoam2606/etc/bashrc"

CAP_CORE_MIN=1100            # must equal grade_f22.py::CAP_CORE_MIN
END_TIME=4                  # T = 4 s; must equal controlDict.template endTime and exact_f22.T_END

# name  N  steps  ranks
LEVELS=("coarse 192 400 1" "medium 384 800 1" "fine 768 1600 1")
# projected SERIAL seconds per level: base rate 10.00 us per cell-step MEASURED on
# THIS case's coarse grid (192^2 = 36,864 cells, 24-step scratch smoke arm, ExecutionTime
# 8.85 s, box load 26.6 on 16 cores -- contended, so conservative), grown per doubling
# of N per side by F18b's registered +75 % then +80 % (F18 MEASURED +61 % / +72 % at
# 64^2 -> 128^2 -> 256^2, verification/runs/F18_runs/*/log.icoFoam, DIC-PCG iterations
# growing with N per side; F18b_TG2D_EXT_PREREGISTRATION.md section 8), times cell-steps
# 1.475e7 / 1.180e8 / 9.437e8 = 147 / 2064 / 29727 s.  DERIVED, NOT MEASURED on the
# medium and fine levels.  Used ONLY by the pre-level projected-cap check.
declare -A PROJ_SERIAL_S=( [coarse]=147 [medium]=2064 [fine]=29727 )
DECOMP_METHOD="none"
...
python3 "$EXACT" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: exact_f22.py --selftest did not pass; NOTHING may be launched against it"; exit 1; }
python3 "$GRADER" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: grade_f22.py --selftest did not pass; the grading path is not established"; exit 1; }
python3 -O "$GRADER" --selftest > /dev/null 2>&1
[ $? -eq 2 ] || { echo "ABORT: grade_f22.py did not exit 2 under python3 -O; the -O refusal is not armed"; exit 1; }
say "INSTRUMENT GREEN: exact_f22 and grade_f22 selftests pass; grade_f22 exits 2 under -O."
...
python3 -c "import sys; sys.exit(0 if abs(float('$CAP_GRADER') - $CAP_CORE_MIN) < 1e-9 else 1)" \
  || { echo "ABORT: launcher cap $CAP_CORE_MIN disagrees with grade_f22.py CAP_CORE_MIN $CAP_GRADER"; exit 1; }
say "CAP AGREES between launcher and grader: $CAP_CORE_MIN core-minutes."

python3 - "$CASE_SRC/constant/transportProperties" "$CASE_SRC/system/controlDict.template" "$END_TIME" <<'PYCHK' \
  || { echo "ABORT: nu on disk disagrees with exact_f22.NU, or controlDict.template endTime is not the registered value"; exit 1; }
import re, sys
sys.path.insert(0, "/home/ubuntu/Certonomous/cases/F22_lamb_oseen")
import exact_f22 as EX
...
PYCHK
say "nu ON DISK AGREES with exact_f22.NU; controlDict.template endTime == registered $END_TIME."

if [ "$PREFLIGHT" = "1" ]; then
  say ""
  say "PREFLIGHT ONLY -- ZERO COMPUTE. blockMesh was NOT run. build_f22.py was NOT called."
  say "decomposition method: $DECOMP_METHOD"
  say "decomposition seed:   $DECOMP_SEED"
  for L in "${LEVELS[@]}"; do
    set -- $L
    say "  level $1: $2 x $2 x 1 cells, dt = $END_TIME/$3 s, ranks $4, target $RUN_ROOT/$1"
...
for L in "${LEVELS[@]}"; do
  ...
  # ---- BUILD: templating, blockMesh, checkMesh (gates), 0/C, 0/p, 0/U LAST
  python3 "$BUILD" "$CD" --level "$NAME" > "$CD/log.build" 2>&1 \
    || { echo "ABORT: build_f22.py failed at level $NAME; see $CD/log.build"; exit 1; }
