#!/usr/bin/env python3
"""
F19 -- BUILD ONE LADDER LEVEL into a destination directory (called by
run_f19.sh once per level; lineage cases/F18_taylor_green/build_f18.py).
REFUSES a destination holding `0/` or a numeric time directory; deletes
nothing; refuses a destination inside the tracked case tree.  Mesher and
postProcess run here, so never call it against the case tree.
  ...
  4. write 0/p and 0/T (exact at t = 0 at the mesh's own centres), then 0/U LAST.
Zero `assert` (L-332).
"""
import os
import re
...
import exact_f19 as EX            # noqa: E402
import foam_io_f19 as FIO          # noqa: E402
...
def die(msg, rc=1):
    sys.stderr.write("ABORT (build_f19): %s\n" % msg)
    sys.exit(rc)
...
def main(argv):
    if len(argv) != 3 or argv[1] != "--level":
        die("usage: build_f19.py <dest_dir> --level <coarse|medium|fine>")
    ...
    if not m or abs(float(m.group(1)) - EX.MOL_WEIGHT) > 1e-9:
        die("thermophysicalProperties molWeight is not exact_f19.MOL_WEIGHT")
    ...
    if cells != n:
        die("built mesh has %d cells, level %s registers %d" % (cells, level, n))
    ...
    xc = C["internal"][:, 0]
    if np.any(np.abs(xc - EX.X0) < 1e-12):
        die("a cell centre sits on the diaphragm; the initial state is ambiguous there")
    rho0 = np.where(xc < EX.X0, EX.RHO_L, EX.RHO_R)
    p0 = np.where(xc < EX.X0, EX.P_L, EX.P_R)
    T0 = p0 / (rho0 * EX.R_GAS)
    FIO.write_from_template(os.path.join(CASE_SRC, "0", "p.template"), os.path.join(dest, "0", "p"),
                            {"__INTERNALFIELD__": FIO.fmt_list(p0, "scalar")})
    FIO.write_from_template(os.path.join(CASE_SRC, "0", "T.template"), os.path.join(dest, "0", "T"),
                            {"__INTERNALFIELD__": FIO.fmt_list(T0, "scalar")})
    internal = np.zeros((cells, 3))
