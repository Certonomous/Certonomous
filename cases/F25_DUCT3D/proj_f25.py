#!/usr/bin/env python3
"""
F25 -- PRE-SPEND PROJECTOR (AMENDMENT 2, 2026-08-27).  ZERO COMPUTE.
...
formula applied 8.0x -- overstated about 4.3x.  Applied to F25 that projected
10,209 core-min of a 2,000 cap at the fine level and would have HALTED after
burning ~580 core-min on two levels that cannot form a Roache triple.
...
=====================================
The registered CAP (2000 core-min) does not move.  The post-level incremental
...
# --- the frozen ladder, identical to run_f25.sh and exact_f25.py -------------
N_ITER = 4000
RANKS = 4
# name -> (nr, nx); cells = nx * nr * nr (a duct: nx along x, nr per cross-section side)
LEVELS = (("coarse", 16, 128), ("medium", 32, 256), ("fine", 64, 512))
# frozen serial-equivalent core-seconds per level (AMENDMENT 1's measured basis)
FROZEN_CORE_S = {"coarse": 246.0, "medium": 4324.0, "fine": 75997.0}


def refuse(msg, rc=2):
    sys.stderr.write("REFUSED (proj_f25): %s\n" % msg)
    sys.exit(rc)
...
def cell_iters(name):
    for nm, nr, nx in LEVELS:
        if nm == name:
            return float(nx * nr * nr) * float(N_ITER)
...
def selftest():
    ...
    print("proj_f25.py --selftest  (zero compute; every box reading INJECTED)")

    # C1  the 8.0x pin is GONE: a 4-rank entry at free = 0.5 gets CONT_MAX, not 8.
    p1, b1, h1 = project("coarse", 0.0, 0.5, 4, [], 2000.0)
    ...
    p2, _b, _h = project("coarse", 0.0, 16.0, 4, [], 2000.0)
    ...
    p3, _b, _h = project("medium", 4.1, 0.5, 4, [("coarse", FROZEN_CORE_S["coarse"])], 2000.0)
    ...
    try:
        CONT_MAX = 8.0
        p8, _b, _h = project("coarse", 0.0, 0.5, 4, [], 2000.0)
    finally:
        CONT_MAX = keep
    ...
    try:
        project("medium", 0.0, 8.0, 4, [("coarse", 0.0)], 2000.0)
        rc9 = None
...
def main(argv=None):
    _o_guard()
    ap = argparse.ArgumentParser(description="F25 pre-spend projector (Amendment 2)")
