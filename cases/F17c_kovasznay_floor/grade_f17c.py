...
checked by AST parse across this file, exact_f17c.py, foam_io_f17c.py and
build_f17c.py (L-332).  `grade_ladder` reaches its own gate through asserts in
...
if not __debug__:
    sys.stderr.write(
        "REFUSED: grade_f17c.py must not run under `python3 -O`.\n"
...
import exact_f17c as EX                  # THE EXACT SOLUTION and the discretisation model.
import foam_io_f17c as FIO               # THE READERS.
...
END_TIME = dict((n, EX.n_iter(n)) for n in LEVEL_NAMES)   # PER LEVEL (L-346).
# There is no global iteration count in this rung and its absence is the point:
# ONE number serving three levels IS the defect L-346 records. exact_f17c.ITERS
# carries the derivation; exact_f17c.ITERS_BASIS carries it in one string.
DT = 1.0
WRITE_EVERY = EX.WRITE_EVERY

# THE ONE DECLARED PARAMETER BOTH BANDS ARE BUILT FROM: the multiplicative
# window around the discretisation-derived prediction in exact_f17c.  Declared
...
CAP_CORE_MIN = 370.0                    # ClockTime s * ranks / 60, summed over levels.
# 1.4942x the 247.62 core-min point estimate. THE ESTIMATE IS proj_f17c.py's OWN
# ARITHMETIC, not a second figure written beside it: 98,304,000 x 0.6751 +
# 786,432,000 x 0.9347 + 18,874,368,000 x 0.7447 core-microseconds / 60 =
# 1.106 + 12.251 + 234.262 = 247.619 core-min, printed by `proj_f17c.py
# --selftest`. (An earlier draft of this line read "1.4993x the 250.12"; 250.12
# is reproduced by no code in this case and is struck. cfd-R2, 2026-08-27.)
# The rates are MEASURED on THIS ladder: 0.6751 / 0.9347 / 0.7447
# core-microseconds per cell-iteration at 24,576 / 98,304 / 393,216 cells, taken
# as the MARGINAL rate from F17b's own logs by differencing the first and last
# ExecutionTime readings so startup is excluded. THE RATE IS NOT MONOTONE IN
# PROBLEM SIZE -- it rises 38 % from coarse to medium and falls 20 % from medium
# to fine -- so this rung registers NO per-doubling growth exponent. See the
# pre-registration section 8.
...
# R-RC-4 FENCE. A token here refuses the run whatever the rc record says. The
# SIGFPE case is separated because OpenFOAM prints `trapFpe ... enabled` as a
# STARTUP BANNER on every run: the banner is not a trap, and a fence that could
# not tell them apart would refuse every clean run on this box.
FATAL_TOKENS = ("FOAM FATAL ERROR", "FOAM FATAL IO ERROR", "SIGSEGV", "SIGABRT",
                "SIGFPE", "SIGBUS", "Aborted", "Floating point exception",
                # THE STACK-TRACE FRAMES, and they are the ones that matter.
                # This rung's own R-RC-4 control found the gap: a real SIGFPE
                # trap redirected into a log file leaves NO "SIGFPE" token at
                # all. The shell's "Floating point exception (core dumped)"
                # goes to the terminal, not through `> log 2>&1`, and what
                # lands in the log is the OpenFOAM stack trace. A fence built
                # only from signal NAMES reads a trapped run as clean.
                "sigHandler", "printStack", "Foam::sigFpe", "Foam::sigSegv")


def fatal_tokens_in(text):
    """Fatal and signal tokens in a solver log, with the trapFpe STARTUP BANNER
    excluded.

    OpenFOAM prints `trapFpe: Floating point exception trapping enabled
    (SIGFPE).` on EVERY run on this box, and that one line names TWO of the
    tokens above. It is a banner, not a trap. A fence that could not tell them
    apart would refuse every clean run this rung produces -- and the same
    confusion is on the record already: F6a attempt 2's only SIGFPE hit was
    this banner (docs/L342_GRADER_AUDIT.md, Addendum 2 section 2).

    The exclusion is BY LINE and is the narrowest that works: only lines
    containing `trapFpe` are skipped. A real trap prints a stack trace with
    `Foam::sigFpe::sigHandler` on its own lines, and those are still seen.
    """
    hits = []
...
def iterative_state(log_path, end_time):
...
# ---------------------------------------------------------------------------
def completion(case_dir, log_path, end_time):
    ...
    # ---- R-RC (APPROVED BY SANAA): an rc VALUE is physics; an rc RECORD is
    # INFRASTRUCTURE. An ABSENT record reads NOT MEASURED when the other four
    # rule-4 conditions hold AND the log carries no fatal or signal token. A
    # RECORDED non-zero rc still refuses; so does a fatal token, whatever the
    # record says. The fence is checked BEFORE the record, so a crashed run
    # whose launcher also died cannot be waved through by the absence.
    fence = fatal_tokens_in(text)
    if fence:
        return dict(out, done=False, crashed=True, rc_record="fenced",
                    why="R-RC-4 FENCE: the log carries %s. A fatal or signal token refuses "
                        "whatever the rc record says; NOT A RESULT" % fence)
    rc_path = os.path.join(case_dir, "RC.txt")
    if not os.path.isfile(rc_path):
        out["rc"] = None
        out["rc_record"] = ("ABSENT -- NOT MEASURED (R-RC). The launcher records rc in the same "
                            "process that ran the solver; an absent record is a bookkeeping loss, "
                            "not a physics finding. The R-RC-4 fence above is CLEAR and the other "
                            "four rule-4 conditions are evaluated below on their own evidence.")
    else:
        rc = open(rc_path).read().strip()
        out["rc"] = rc
        out["rc_record"] = "MEASURED"
        if rc != "0":
...
def control_solver_dicts_match():
    """nu on disk == exact_f17c.NU; residualControl ABSENT; endTime and
    ...
    if not m or abs(float(m.group(1)) - EX.NU) > 1e-15:
        refuse("transportProperties nu does not equal exact_f17c.NU = %.17g" % EX.NU)
    ...
    me = re.search(r"^\s*endTime\s+__ENDTIME__\s*;", cd, re.M)
    mw = re.search(r"^\s*writeInterval\s+([0-9]+)\s*;", cd, re.M)
    if not me:
        refuse("controlDict.template does not carry the __ENDTIME__ placeholder; the iteration "
               "count is PER LEVEL in this rung (L-346) and a hard-coded endTime would restore "
               "the single inherited floor that F17b was refused for")
    if not mw or int(mw.group(1)) != WRITE_EVERY:
        refuse("controlDict.template writeInterval is not the registered %d" % WRITE_EVERY)
    for name in LEVEL_NAMES:
        if END_TIME[name] != EX.ITERS[name] or END_TIME[name] <= 0:
            refuse("registered endTime for %s disagrees with exact_f17c.ITERS" % name)
        if END_TIME[name] % WRITE_EVERY != 0:
            refuse("endTime %d at %s is not a whole number of writeInterval %d: the last "
                   "checkpoint would not land on endTime" % (END_TIME[name], name, WRITE_EVERY))
    return dict(control="solver_dictionaries_agree_with_registration", nu=EX.NU,
                endTime=dict(END_TIME), iters_basis=EX.ITERS_BASIS,
                writeInterval=WRITE_EVERY, residualControl="absent", passed=True)
...
def grade_one(root, gate, bnd):
    ...
    for name in LEVEL_NAMES:
        ...
        comp = completion(case_dir, log, END_TIME[name])
        if not comp["done"]:
        ...
        it_state, it_detail = iterative_state(log, END_TIME[name])
...
def control_field_classes_separate():
    ...
    ET = 40                       # a synthetic level; completion() takes its own
                                  # endTime now, because in THIS rung there is no
                                  # single one (L-346).
    tmp = tempfile.mkdtemp(prefix="f17c_l342_")
    try:
        cd = os.path.join(tmp, "coarse")
        os.makedirs(os.path.join(cd, "0")); os.makedirs(os.path.join(cd, str(ET)))
        open(os.path.join(cd, "0", "U"), "w").write("x")
        import time as _t
        _t.sleep(0.02)
        body = "".join("Time = %d\n" % i for i in range(1, ET + 1)) + "ClockTime = 7 s\nEnd\n"
        open(os.path.join(cd, "log.simpleFoam"), "w").write(body)
        open(os.path.join(cd, "RC.txt"), "w").write("0\n")
        for f in ("U", "p"):
            open(os.path.join(cd, str(ET), f), "w").write("y")
        good = completion(cd, os.path.join(cd, "log.simpleFoam"), ET)
        if not good["done"]:
        ...
        cc = cost_claim(tmp)
        again = completion(cd, os.path.join(cd, "log.simpleFoam"), ET)
        ...
        bad = completion(cd, os.path.join(cd, "log.simpleFoam"), ET)
        ...
        bad2 = completion(cd, os.path.join(cd, "log.simpleFoam"), ET)
        ...
        # ---- R-RC, DRIVEN BOTH WAYS on the same synthetic level ----------
        # The rc RECORD is INFRASTRUCTURE; the rc VALUE is physics. An ABSENT
        # record must read NOT MEASURED when the other four rule-4 conditions
        # hold -- and must STILL refuse when the log carries a fatal token.
        open(os.path.join(cd, "log.simpleFoam"), "w").write(body)
        os.remove(os.path.join(cd, "RC.txt"))
        rrc_ok = completion(cd, os.path.join(cd, "log.simpleFoam"), ET)
        if not rrc_ok["done"] or rrc_ok.get("rc") is not None \
                or "NOT MEASURED" not in (rrc_ok.get("rc_record") or ""):
            refuse("R-RC CONTROL: an ABSENT rc record with the other four rule-4 conditions "
                   "holding and the fence clear did not read NOT MEASURED: %r" % rrc_ok.get("why"))
        open(os.path.join(cd, "log.simpleFoam"), "w").write(
            body.replace("End\n", "FOAM FATAL ERROR\nEnd\n"))
        rrc_fenced = completion(cd, os.path.join(cd, "log.simpleFoam"), ET)
        if rrc_fenced["done"] or not rrc_fenced.get("crashed"):
            refuse("R-RC-4 FENCE CONTROL: a FOAM FATAL token with the rc record ABSENT was "
                   "not refused; an absent record must never wave a crashed run through")
        # the banner must NOT fence: OpenFOAM prints trapFpe on every clean run
        open(os.path.join(cd, "log.simpleFoam"), "w").write(
            "trapFpe: Floating point exception trapping enabled (SIGFPE).\n" + body)
        rrc_banner = completion(cd, os.path.join(cd, "log.simpleFoam"), ET)
        if not rrc_banner["done"]:
            refuse("R-RC-4 FENCE CONTROL: the trapFpe STARTUP BANNER was read as a trap and "
                   "refused a clean run; the fence would refuse every run on this box")
        # ... and the banner must not BLIND the fence: a real trap arriving on a
        # run that also printed the banner is the case that matters, because
        # every run on this box prints the banner.
        open(os.path.join(cd, "log.simpleFoam"), "w").write(
            "trapFpe: Floating point exception trapping enabled (SIGFPE).\n"
            + body.replace("End\n", "#0  Foam::sigFpe::sigHandler(int) at ??:?\nEnd\n"))
        rrc_real = completion(cd, os.path.join(cd, "log.simpleFoam"), ET)
        if rrc_real["done"] or not rrc_real.get("crashed"):
            refuse("R-RC-4 FENCE CONTROL: a REAL SIGFPE trap on a run that also printed the "
                   "trapFpe banner was not refused; the banner exclusion has blinded the fence")
        open(os.path.join(cd, "RC.txt"), "w").write("0\n")

        # ---- L-346, ASSERTED ON THE REGISTRATION ITSELF ------------------
        # The defect this rung exists to repair is ONE iteration count serving
        # three levels. If ITERS is ever flattened back to a single number this
        # control fires, and it fires at selftest time, before any compute.
        if len(set(EX.ITERS.values())) < 2:
            refuse("L-346 CONTROL: exact_f17c.ITERS carries one iteration count for every "
                   "level (%r). An inherited floor uniform across a ladder IS the defect "
                   "F17b was refused for; this rung may not carry it." % EX.ITERS)
        if EX.ITERS["fine"] <= EX.ITERS["coarse"]:
            refuse("L-346 CONTROL: the fine level's registered count %d does not exceed the "
                   "coarse level's %d; the discretisation error falls 4x per level and the "
                   "floor must fall with it" % (EX.ITERS["fine"], EX.ITERS["coarse"]))
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
    return dict(control="PZ-F17c-L342_L346_RRC_infrastructure_vs_physics_driven_both_ways",
                infrastructure_deleted="verdict channel unchanged, cost claim refused, defect lines printed",
                physics_corrupted="NOT A RESULT",
                r_rc_absent_record="NOT MEASURED, run complete on the other four conditions",
                r_rc_fence="FOAM FATAL with the record ABSENT still refuses; the trapFpe banner does not",
                l346_iters=dict(EX.ITERS), passed=True)
...
def main(argv=None):
    ...
    ap.add_argument("--root", default=os.path.join(REPO, "verification", "runs", "F17c_runs"))
    ap.add_argument("--out", default=None)
    ap.add_argument("--prereg-commit")
    ap.add_argument("--selftest", action="store_true")
    a = ap.parse_args(argv)

    census = ast_no_asserts([os.path.abspath(__file__), os.path.join(HERE, "exact_f17c.py"),
                             os.path.join(HERE, "foam_io_f17c.py"), os.path.join(HERE, "build_f17c.py")])
    ...
    out = a.out or os.path.join(a.root, "F17c_GRADED.json")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    with open(out, "w") as f:
        json.dump(dict(rung="F17c-KV40-FLOOR", prereg_commit=a.prereg_commit,
                       prereg="verification/campaign/F17c_KV40_FLOOR_PREREGISTRATION.md",
    ...
    print("F17c -- KOVASZNAY FLOW, DERIVED ITERATIVE FLOOR (L-346) -- TALLY")
    print("=" * 78)
    for r in rows:
