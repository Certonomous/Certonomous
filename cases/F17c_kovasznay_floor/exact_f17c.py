...
if not __debug__:
    sys.stderr.write(
        "REFUSED: exact_f17c.py must not run under `python3 -O`.\n"
...
LEVELS = (("coarse", 192, 128), ("medium", 384, 256), ("fine", 768, 512))
# F17c re-registers F17b's THREE LEVELS UNCHANGED.  The mesh family, the case
# dictionaries, the discretisation model, the bands and the Class C tolerances
# are byte-for-byte F17b's.  EXACTLY ONE THING CHANGES: the iterative floor.
# The linear model is SOLVED on the grids below (F17's own medium and fine;
# 192x128 = 29.5 s, 2.85 GB RSS measured) and EXTRAPOLATED to the two finer
# levels with the model's own observed order between them.  384x256 measured
...
# ---------------------------------------------------------------------------
# THE ITERATIVE FLOOR (L-346), DERIVED FROM THIS LADDER AND NOTHING ELSE.
#
# L-346 was paid for by F17b, which inherited F17's floor BYTE-FOR-BYTE: a
# fixed 4,000 iterations at all three levels.  On F17's ladder that floor sat
# two orders below the discretisation error.  On THIS ladder it does not, and
# F17b returned NOT A RESULT x 2 because the registered Class C plateau gate --
# correctly -- refused a fine level that was still moving.
#
# THE DERIVATION, at ZERO NEW COMPUTE, from F17b's OWN 40 written checkpoints
# (verification/runs/F17b_runs/<level>/<t>/U, read by THIS repository's frozen
# reader).  For each level the graded quantity E2 was recomputed at every
# checkpoint and the increments of the last 20 were fitted as a geometric
# sequence, log(dE2) linear in iteration:
#
#   level   rho per 100 iterations   E2 at 4,000    remaining to convergence
#   coarse  (increments at round-off; not fitted)   1.423137e-04   0.001 %
#   medium  0.78558                                 3.558768e-05   0.026 %
#   fine    0.98517  (+2 sigma 0.98538)             6.155850e-06   41.4 %
#
# THE FINE LEVEL WAS 41.4 % SHORT OF ITS OWN CONVERGED VALUE at 4,000
# iterations -- a larger miss than F17b's record states, because 13.5 % is the
# drift over the last window and 41.4 % is the distance still to travel.
# Extrapolated converged E2_fine = 1.050478e-05, which sits inside the
# UNCHANGED registered band [2.299887e-06, 2.069898e-05].
#
# THE REQUIREMENT.  The registered Class C trend tolerance is 2.0e-04 relative
# drift over the trailing 1,100-iteration window and IS NOT TOUCHED: it is the
# gate that caught F17b and L-346's fix is to derive the ITERATION COUNT, never
# to relax the gate.  Solving drift(N) = rem(N) x (rho^-11 - 1) <= 2.0e-04 x
# E2_inf for N gives 5,183 at medium and 43,568 at fine on the point estimate,
# 5,262 and 44,110 at rho + 2 sigma, and 41,768 at fine using the last observed
# increment ratio 0.9844 instead of the fit.  The counts below round UP past
# the worst of those three routes.
#
# WHAT THIS BUYS, stated as a prediction and not as a result: at 48,000 the
# fine level's remaining movement is 0.11 % of E2_inf -- three orders below the
# discretisation error, where L-346 asks for one.
#
# A CONSIDERED ALTERNATIVE, REJECTED AND RECORDED.  The slow fine-level
# convergence is a property of SIMPLE with fixed relaxation as h falls, and a
# better pressure solve would reach the same discrete answer in fewer
# iterations.  It is NOT taken: it would change the case, break byte-identity
# with F17 and F17b, and be registered without a pilot.  Paying the iterations
# on a known-good configuration is the cheaper risk.
# ---------------------------------------------------------------------------
ITERS = dict(coarse=4000, medium=8000, fine=48000)
ITERS_BASIS = ("DERIVED, not inherited (L-346): from F17b's own E2 checkpoint trajectory "
               "at zero new compute. Geometric tail ratio per 100 iterations 0.78558 "
               "(medium) and 0.98517 (fine, +2 sigma 0.98538); the count is the one that "
               "brings the trailing 1,100-iteration relative drift of E2 under the "
               "UNCHANGED 2.0e-04 Class C trend tolerance -- 5,183 / 43,568 on the point "
               "estimate, 5,262 / 44,110 at +2 sigma, 41,768 at fine on the last observed "
               "increment ratio; registered 4,000 / 8,000 / 48,000. The coarse level is "
               "registered at 4,000 on the DIRECT measurement that its drift there was "
               "already 1.82e-06, 110x inside the tolerance, not on a decay fit.")


def n_iter(level):
    """The registered iteration count for one level.  There is no global count:
    that a single number served all three levels IS the L-346 defect."""
    if level not in ITERS:
        refuse("unknown level %r; the registered levels are %s" % (level, sorted(ITERS)))
    return ITERS[level]
...
def main(argv=None):
    ...
    if a.json:
        print(json.dumps(dict(constants=dict(Re=RE, nu=NU, lam=LAM, U0=U0,
                                             ...
                                             iters=ITERS, iters_basis=ITERS_BASIS, write_every=WRITE_EVERY),
