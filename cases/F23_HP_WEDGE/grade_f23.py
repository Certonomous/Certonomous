#!/usr/bin/env python3
"""
F23 -- THE GRADING PATH for Hagen-Poiseuille flow on an axisymmetric wedge
(simpleFoam, steady, Re_D = 100, streamwise cyclic, fixed body force, 4 ranks
at every level).  Lineage: cases/F17b_kovasznay_ext/grade_f17.py.  It calls
...
file, exact_f23.py, foam_io_f23.py, build_f23.py (L-332); hard `-O` refusal.
...
x-column of cells nearest x = L/2 + dx/2; its cell count must equal NR.
...
each is a vanishing channel (u_r = 0, u_z is the wedge's constrained
direction, p is uniform on the cyclic domain) whose relative residual is
normalised by a vanishing scale and carries no information -- measured on the
scratch instrument run: Uy residual 5e-2 with max|Uy| = 4e-16 in the field.
...
if not __debug__:
    sys.stderr.write("REFUSED: grade_f23.py must not run under `python3 -O` (the shared "
...
import exact_f23 as EX                  # THE EXACT SOLUTION and the discretisation model.
import foam_io_f23 as FIO               # THE READERS.
...
CAP_CORE_MIN = 1100.0                   # ClockTime s * ranks / 60, summed over levels
...
CLASS_C = dict(
    ...
    window=12,           # element 1: 12 checkpoints = 1200 iterations (F17's justification: > 400
                         #   e-foldings of the slowest SIMPLE relaxation mode at alpha_U = 0.7)
    trend_tol=2.0e-4,
    stat_tol=1.0e-4,
    var_ratio=(0.2, 5.0),
    floor=1.0e-14,
...
WRITE_PATH_NOTE = (
    ...
    "v2606, 120 cells), parsed at selftest as a live control.")
...
def read_geometry(pdirs):
    """(xc, yc, V) concatenated over the processor directories."""
    xs, ys, vs = [], [], []
    for p in pdirs:
...
def e2n_from_files(u_paths, yc, V, mask):
    """G-F23-1: volume-weighted L2 error of the normalised station profile."""
    U = read_U(u_paths)
    if U.shape[0] != len(yc):
...
def fre_from_files(u_paths, V, mask):
    """G-F23-2: f.Re from the imposed G and the READ station bulk velocity."""
    U = read_U(u_paths)
    if U.shape[0] != len(V):
...
def diagnostics_from_files(u_paths, V, mask):
    ...
    return dict(ubar_all_cells=ub_all, ubar_station=ub_st, x_nonuniformity=abs(ub_all - ub_st) / max(abs(ub_st), 1e-300),
...
def _planted_copies(u_paths, d):
    tmp = tempfile.mkdtemp(prefix="f23_plant_")
    work = []
...
def control_class_c_can_say_no():
    ...
    return dict(control="PZ-F23-CLASSC_four_limbs_each_shown_able_to_refuse",
...
def iterative_state(log_path, diag):
    ...
    detail = dict(basis="initial residual of Ux (the driven component) censused over every iteration in the "
                        ...
                        "transverse components checked in the FIELD at endTime instead",
...
def control_grade_ladder_is_called():
    ...
    return dict(control="PZ-F23-GRADE_LADDER_CALLSITE_ast_census_plus_grep_both_ways",
                ast_call_nodes=calls, grep_lines=grep_hits, passed=True)


def control_solver_dicts_match():
    """nu, G, ranks on disk == exact_f23; residualControl ABSENT; endTime and
    writeInterval == registered; the wedge/cyclic patch declarations present."""
    ...
    if not m or abs(float(m.group(1)) - EX.NU) > 1e-15:
        refuse("transportProperties nu does not equal exact_f23.NU = %.17g" % EX.NU)
    ...
    if not m or abs(float(m.group(1)) - EX.G) > 1e-15:
        refuse("fvOptions momentum source does not equal exact_f23.G = %.17g" % EX.G)
    ...
    for pat, what in ((r"inlet\s*\{\s*type cyclic;\s*neighbourPatch outlet;", "inlet cyclic -> outlet"),
                      ...
                      (r"wedge1\s*\{\s*type wedge;", "wedge1"), (r"wedge2\s*\{\s*type wedge;", "wedge2"),
...
def bands():
    ...
    return {
        "G-F23-1_E2_normalised_profile": dict(
            band=(e2p / BAND_FACTOR, e2p * BAND_FACTOR), reference=0.0, dim=DIM,
            principle=("discretisation-model prediction E2n = %.9e at h_fine = %g (the radial stencil simpleFoam "
                       "reduces to on the wedge mesh's own geometry, solved; wedge bias %.2e included), times [1/%g, %g]"
        ...
        "G-F23-2_f_Re": dict(
            ...
            principle=("exact f.Re = 64 +/- %g x the model's predicted fine-level error (%.6e; wedge bias %.2e "
                       "included) = +/- %.6e" % (BAND_FACTOR, fine["fRe_err_pred"], fine["wedge_bias_fRe"], tol))),
    }
...
def synth_level(tmp, nr, nx, u_station, tag):
    """A synthetic single-processor level: nx x-columns each carrying the given
    radial profile, with 0/C and 0/V from the model's wedge geometry."""
...
def demonstrate(bnd):
    ...
    model error) stands in for the fine field, on a 4-column synthetic level at
    the fine radial size: inside both bands; the error field x40 is outside
    G-F23-1; the profile x1.001 is outside G-F23-2."""
    m = EX.model("fine")
    nr, _nx_full = SHAPE["fine"]
    nx = 4
    err = m["u"] - EX.u_exact(m["yc"])
    rows = []
    tmp = tempfile.mkdtemp(prefix="f23_demo_")
    try:
        ...
        for k, (scale, mult, want1, want2) in enumerate(cases):
            ...
            if want1:
                ...
                lo, hi = bnd["G-F23-1_E2_normalised_profile"]["band"]
                rows.append(dict(gate="G-F23-1_E2_normalised_profile", construction=cons, value=v1, band=[lo, hi],
                                 inside=bool(lo <= v1 <= hi), intended=want1))
            if want2:
                v2 = fre_from_files(ups, V, mask)
                lo, hi = bnd["G-F23-2_f_Re"]["band"]
                rows.append(dict(gate="G-F23-2_f_Re", construction=cons, value=v2, band=[lo, hi],
...
def control_field_classes_separate():
    tmp = tempfile.mkdtemp(prefix="f23_l342_")
    try:
        ...
        open(log, "w").write(body.replace("Initial residual = 1e-12", "Initial residual = 1e-7", 1).replace(
            "Time = 4000\n", "Time = 4000\n", 1))
        ...
        shutil.rmtree(tmp, ignore_errors=True)
    return dict(control="PZ-F23-L342_infrastructure_vs_physics_fields_driven_both_ways",
...
# THE GRADE
# ---------------------------------------------------------------------------
GATES = ("G-F23-1_E2_normalised_profile", "G-F23-2_f_Re")
...
def grade_one(root, gate, bnd):
    ...
    for name in LEVEL_NAMES:
        ...
        for t, ups in checkpoint_paths(pdirs):
            ts.append(t)
            vs.append(e2n_from_files(ups, yc, V, mask) if gate == GATES[0] else fre_from_files(ups, V, mask))
            last = ups
        if last is None:
...
def selftest_predicate(controls, demo, census):
    ...
    return True, ("%d controls green; both gate quantities shown able to take a passing AND a failing value "
                  ...
                  "grade_ladder call node; __debug__ True" % len(controls))


def main(argv=None):
    ap = argparse.ArgumentParser()
    ap.add_argument("--root", default=os.path.join(REPO, "verification", "runs", "F23_HP_WEDGE_runs"))
    ap.add_argument("--out", default=None)
    ap.add_argument("--prereg-commit")
    ap.add_argument("--selftest", action="store_true")
    a = ap.parse_args(argv)

    census = ast_no_asserts([os.path.abspath(__file__), os.path.join(HERE, "exact_f23.py"),
                             os.path.join(HERE, "foam_io_f23.py"), os.path.join(HERE, "build_f23.py")])
    controls = [EX.control_symbolic_substitution(), EX.control_substitution_is_able_to_fail(),
                ...
                EX.control_model_is_solved_second_order_and_wedge_admissible(),
    ...
    if a.selftest:
        ...
        print(json.dumps(dict(assert_census=census, controls=controls,
                              ...
                              model_levels=EX.predictions(), model_orders=dict(wedge=EX.model_orders(), polar=EX.model_orders(polar=True)),
    ...
    out = a.out or os.path.join(a.root, "F23_GRADED.json")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    with open(out, "w") as f:
        json.dump(dict(rung="F23-HP_WEDGE", prereg_commit=a.prereg_commit,
                       prereg="verification/campaign/F23_HP_WEDGE_PREREGISTRATION.md",
    ...
    print("F23 -- HAGEN-POISEUILLE WEDGE -- TALLY")
    print("=" * 78)
    for r in rows:
