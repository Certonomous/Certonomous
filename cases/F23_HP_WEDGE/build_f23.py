#!/usr/bin/env python3
"""
F23 -- BUILD ONE LADDER LEVEL into a destination directory (called by
run_f23.sh once per level; lineage cases/F17b_kovasznay_ext/build_f17.py).
...
tree.  Mesher and postProcess run here, so never call it against the case tree.
  ...
  1. template blockMeshDict (NX, NR, L, wall vertices at EXACT radius R);
     copy the fixed dictionaries (controlDict, fvSchemes, fvSolution,
     decomposeParDict, transportProperties, turbulenceProperties, fvOptions)
  2. blockMesh, checkMesh -> `Mesh OK`, MESH_STANDARD section 3 gates enforced
     (non-orthogonality 70 deg, skewness 4); aspect ratio and the wedge angles
     RECORDED in MESH_LINE.txt (aspect ratio is advisory at 1000, section 3.3)
  3. mkdir 0/, copy 0/p, `postProcess -func writeCellCentres` -> 0/C and
     `postProcess -func writeCellVolumes` -> 0/V
...
import exact_f23 as EX            # noqa: E402
import foam_io_f23 as FIO          # noqa: E402
...
def die(msg, rc=1):
    sys.stderr.write("ABORT (build_f23): %s\n" % msg)
    sys.exit(rc)
...
def main(argv):
    usage = "usage: build_f23.py <dest_dir> --level <coarse|medium|fine>  |  <dest_dir> --scratch NR NX"
    ...
    a = math.radians(EX.HALF_ANGLE_DEG)
    FIO.write_from_template(os.path.join(CASE_SRC, "system", "blockMeshDict.template"),
                            ...
                            {"__NX__": str(nx), "__NR__": str(nr), "__L__": repr(EX.L),
                             ...
                             "__HALF_ANGLE_DEG__": repr(EX.HALF_ANGLE_DEG)})
    ...
    if not m or abs(float(m.group(1)) - EX.NU) > 1e-15:
        die("transportProperties nu is not exact_f23.NU")
    ...
    if not m or abs(float(m.group(1)) - EX.G) > 1e-15:
        die("fvOptions momentum source is not exact_f23.G = %.17g" % EX.G)
    ...
    m_wa = re.findall(r"Wedge (wedge[12]) with angle ([0-9.eE+-]+) degrees", cm)
    ...
    if len(m_wa) != 2:
        die("checkMesh did not report both wedge patches' angles")
    ...
    with open(os.path.join(dest, "MESH_LINE.txt"), "w") as f:
        f.write("level=%s nr=%d nx=%d cells=%d max_non_orthogonality_deg=%s max_skewness=%s max_aspect_ratio=%s "
                "aspect_%s=%g wedge_angles_deg=%s/%s gate_non_ortho=%g gate_skew=%g source=log.checkMesh\n"
                % (level, nr, nx, cells, m_no.group(1), m_sk.group(1), m_ar.group(1), aspect_note, ASPECT_ADVISORY,
                   m_wa[0][1], m_wa[1][1], MAX_NON_ORTHO, MAX_SKEW))
    ...
    # geometry cross-check against the model's own wedge geometry: the cell
    # centres' y and the volumes must be the model's (the band rests on it)
    g = EX.wedge_geometry(nr)
    yc = C["internal"][:, 1]
    dx = EX.L / nx
    j = np.rint((C["internal"][:, 1] / (EX.R * g["ca"])) * nr - 0.5).astype(int)
    j = np.clip(j, 0, nr - 1)
    if np.max(np.abs(yc - g["yc"][j])) > 1e-9 or np.max(np.abs(V["internal"] - g["vol"][j] * dx)) > 1e-9 * dx:
        die("mesh geometry disagrees with exact_f23.wedge_geometry (centres or volumes); the model would not "
            "describe this mesh")
    ...
    print("built level %s at %s: %d cells, non-ortho %s, skew %s, aspect %s, wedge %s/%s deg; geometry == model; 0/U written last"
          % (level, dest, cells, m_no.group(1), m_sk.group(1), m_ar.group(1), m_wa[0][1], m_wa[1][1]))
    return 0
