#!/usr/bin/env bash
# =============================================================================
# VMFL017-R3 graded-run driver -- Transonic Flow over an RAE 2822 Airfoil
...
# adjustTimeStep + maxCo, 2D C-mesh, characteristic far field, on a HIGH-Re
# WALL-FUNCTION mesh (y+ ~ 30 first cell, held constant across L1/L2/L3).
#
# Frozen pre-registration: cases/ansys_verification/VMFL017-R3/PREREGISTRATION.md
# Frozen comparator      : cases/ansys_verification/VMFL017-R3/grade_vmfl017_r3.py
# Registered driving numbers (prereg sec.12/13): maxCo 0.2, maxDeltaT 1e-5,
# deltaT 1e-9, endTime 0.05 s, writeInterval 0.05 (adjustableRunTime),
# forceCoeffs executeInterval 1e-4, yPlus executeInterval 1e-4.
#
# ---------------------------------------------------------------------------
# NO `set -u`.  CATEGORICALLY INCOMPATIBLE with OpenFOAM v2606: sourcing
# etc/bashrc dereferences WM_PROJECT_DIR before assigning it (rc 127). EVERY
# check gates EXPLICITLY with `|| { echo ABORT...; exit <n>; }`.
# ---------------------------------------------------------------------------
# CAPS ARE PER LEVEL, NOT a shared drawdown (prereg line 12; anti-starvation).
# core_minutes = wall_s * RANKS / 60 ;  timeout_s = cap_core_min * 60 / RANKS.
# An overrun STOPS the level (rc 124); endTime is NEVER silently reduced to fit
# a cap (prereg line 12; CLAUDE.md rule 12). Caps are sized from the answer-blind
# smoke's measured Delta t ~ 1.87e-7 s: L1 ~132 core-min to reach endTime, so
# L1 300 / L2 1500 / L3 12000 (L2 ~4x cells x finer streamwise; L3 x4 again).
# =============================================================================

RANKS=1
declare -A CAP=( [L1]=300 [L2]=1500 [L3]=12000 )   # core-min, frozen prereg line 12
...
echo "=== VMFL017-R3 launcher  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)  run_root=$RUN_ROOT"

# --- smoke mode (Amendment 3 item 6) -----------------------------------------
# Refuses any root that is not under the scratch area, so a smoke can never leave
# an answer where a graded run would look for one. SMOKE_ENDTIME is sized for R3's
# ~1.87e-7 s step (~50 steps), not R2's 1.85e-9 s step.
SMOKE_ENDTIME="1e-5"
SMOKE_FO_INTERVAL="2e-7"
...
PREREG_REL="cases/ansys_verification/VMFL017-R3/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL017-R3/grade_vmfl017_r3.py"
BIRTH_REL="cases/ansys_verification/VMFL017-R3/mesh_birth/BIRTH_CERTIFICATE.md"
...
# --- 5. MESH BIRTH CERTIFICATE + R3 blockMeshDict freeze check ----------------
# The R3 blockMeshDicts are a NEW high-Re wall-function family (birth-certified here).
# Each is verified identical to its OWN committed HEAD blob (rule 2), not the low-Re
# attempt-1 family.
...
for L in L1 L2 L3; do
  BREL="cases/ansys_verification/VMFL017-R3/case/system/blockMeshDict.$L"
  git -C "$REPO" cat-file -e "HEAD:$BREL" 2>/dev/null || { echo "ABORT: no R3 blockMeshDict.$L at HEAD -- the mesh IS part of the freeze"; exit 2; }
  A="$(git -C "$REPO" show "HEAD:$BREL" | git -C "$REPO" hash-object --stdin)" || { echo "ABORT: cannot hash HEAD blob of $BREL"; exit 2; }
  B="$(git -C "$REPO" hash-object "$CASE_DIR/system/blockMeshDict.$L")" || { echo "ABORT: cannot hash R3 blockMeshDict.$L on disk"; exit 2; }
  [ "$A" = "$B" ] || { echo "ABORT: blockMeshDict.$L on disk ($B) != HEAD blob ($A) -- the mesh that would run is NOT the one that was frozen"; exit 2; }
done
echo "  blockMeshDict L1/L2/L3 on disk identical to their committed HEAD blobs"

# --- REGISTERED-NUMBER ASSERTION (prereg sec.12/13) --------------------------
...
for PAIR in "endTime:$REG_ENDTIME" "deltaT:$REG_DELTAT" "maxCo:$REG_MAXCO" "maxDeltaT:$REG_MAXDELTAT"; do
  ...
  [ -n "$GOT" ] || { echo "ABORT: controlDict has no $K -- prereg registers $K = $WANT"; exit 2; }
  _num_eq "$GOT" "$WANT" || { echo "ABORT: controlDict $K = $GOT but prereg registers $WANT"; exit 2; }
...
_num_eq "$WI" "$REG_WRITEINTERVAL" || { echo "ABORT: controlDict writeInterval = $WI but prereg registers $REG_WRITEINTERVAL"; exit 2; }
grep -qE '^[[:space:]]*writeControl[[:space:]]+adjustableRunTime;' "$CD" || { echo "ABORT: controlDict writeControl is not adjustableRunTime"; exit 2; }
...
_num_eq "$FOEI" "$REG_FO_EXECINTERVAL" || { echo "ABORT: forceCoeffs executeInterval = $FOEI but prereg registers $REG_FO_EXECINTERVAL"; exit 2; }
echo "  registered OK  forceCoeffs1 executeInterval = $FOEI  (~500 samples over $REG_ENDTIME s)"
# the yPlus function object must exist and sample at the registered interval (regime precondition)
grep -qE '^[[:space:]]*type[[:space:]]+yPlus;' "$CD" || { echo "ABORT: controlDict has no yPlus function object -- the R3 wall-function regime precondition needs it"; exit 2; }
NYP="$(grep -c 'executeInterval' "$CD")"
python3 -c "import sys;sys.exit(0 if int(sys.argv[1])>=2 else 1)" "$NYP" || { echo "ABORT: expected both forceCoeffs and yPlus executeInterval entries"; exit 2; }
echo "  registered OK  yPlus1 function object present"
# comparator ENDTIME_PHYS must be the SAME number
CMP_ENDTIME="$(grep -oE '^ENDTIME_PHYS[[:space:]]*=[[:space:]]*[0-9.eE+-]+' "$SCRIPT_DIR/grade_vmfl017_r3.py" | head -1 | sed -E 's/.*=[[:space:]]*//')"
_num_eq "$CMP_ENDTIME" "$REG_ENDTIME" || { echo "ABORT: comparator ENDTIME_PHYS = $CMP_ENDTIME but controlDict/prereg register $REG_ENDTIME"; exit 2; }
echo "  registered OK  comparator ENDTIME_PHYS = $CMP_ENDTIME == controlDict endTime"
CMP_MINS="$(grep -oE '^PLATEAU_MIN_SAMPLES[[:space:]]*=[[:space:]]*[0-9]+' "$SCRIPT_DIR/grade_vmfl017_r3.py" | head -1 | sed -E 's/.*=[[:space:]]*//')"
CMP_WFRAC="$(grep -oE '^WINDOW_FRAC[[:space:]]*=[[:space:]]*[0-9.]+' "$SCRIPT_DIR/grade_vmfl017_r3.py" | head -1 | sed -E 's/.*=[[:space:]]*//')"
python3 -c "
import sys
...
# --- 4. PLANTED-ZERO + p-floor + y+ controls (CLAUDE.md rule 3) --------------
python3 "$SCRIPT_DIR/grade_vmfl017_r3.py" --selftest > "/tmp/vmfl017r3_selftest.$$" 2>&1 || { echo "ABORT: comparator --selftest is NOT green; see /tmp/vmfl017r3_selftest.$$"; exit 2; }
grep -q '^SELFTEST OK' "/tmp/vmfl017r3_selftest.$$" || { echo "ABORT: comparator --selftest printed no SELFTEST OK"; exit 2; }
grep -q 'plant seen' "/tmp/vmfl017r3_selftest.$$" || { echo "ABORT: comparator --selftest did not exercise the planted-zero control"; exit 2; }
grep -q 'regime OK' "/tmp/vmfl017r3_selftest.$$" || { echo "ABORT: comparator --selftest did not exercise the y+ regime precondition"; exit 2; }
echo "  planted-zero + p-floor + y+ regime controls OK (comparator --selftest green)"
rm -f "/tmp/vmfl017r3_selftest.$$"
...
{ echo "case = VMFL017-R3"
  ...
  echo "registered = endTime $REG_ENDTIME s ; deltaT $REG_DELTAT ; maxCo $REG_MAXCO ; maxDeltaT $REG_MAXDELTAT ; writeInterval $REG_WRITEINTERVAL adjustableRunTime ; forceCoeffs executeInterval $REG_FO_EXECINTERVAL ; yPlus executeInterval $REG_FO_EXECINTERVAL"
...
for L in $LEVELS_TO_RUN; do
  ...
  if [ -n "$VMFL_SMOKE" ]; then
    # SCRATCH COPY ONLY. The graded controlDict on disk is never touched.
    sed -i -e "s/^endTime         0.05;/endTime         $SMOKE_ENDTIME;/" \
           -e "s/^writeInterval   0.05;/writeInterval   $SMOKE_ENDTIME;/" \
           -e "s/executeInterval 1e-4;/executeInterval $SMOKE_FO_INTERVAL;/g" \
           -e "s/writeInterval   1e-4;/writeInterval   $SMOKE_FO_INTERVAL;/g" "$OUT/system/controlDict" \
  ...
  (
    ...
    # 0/U is touched LAST, immediately before the solver: the age-guard datum (rule 4).
    touch 0/U || exit 93
  ...
  if [ -n "$VMFL_SMOKE" ]; then
    ...
    [ -n "$CDAT" ] || { echo "ABORT smoke: forceCoeffs1 wrote NO coefficient.dat"; exit 2; }
    YDAT="$(find "$OUT/postProcessing/yPlus1" -name yPlus.dat 2>/dev/null | head -1)"
    [ -n "$YDAT" ] || { echo "ABORT smoke: yPlus1 wrote NO yPlus.dat -- the regime precondition would have nothing to read"; exit 2; }
    SMOKE_CD="$(grep -vE '^[[:space:]]*(#|$)' "$CDAT" | tail -1 | awk '{print $2}')"
    SMOKE_CL="$(grep -vE '^[[:space:]]*(#|$)' "$CDAT" | tail -1 | awk '{print $5}')"
    SMOKE_YMAX="$(grep -vE '^[[:space:]]*(#|$)' "$YDAT" | tail -1 | awk '{print $4}')"
    echo "  SMOKE forceCoeffs+yPlus OK: last Cd=$SMOKE_CD Cl=$SMOKE_CL yPlusMax=$SMOKE_YMAX (NOT a result -- ${SMOKE_ENDTIME}s of physical time, forces NOT read for grading)"
    echo "smoke = last_Cd=$SMOKE_CD last_Cl=$SMOKE_CL last_yPlusMax=$SMOKE_YMAX" >> "$LR"
  fi
...
echo "=== VMFL017-R3 launcher done, overall rc=$OVERALL_RC, total ${TOTAL_CORE_MIN} core-min"
echo "grade with: python3 $SCRIPT_DIR/grade_vmfl017_r3.py --run-root $RUN_ROOT"
exit "$OVERALL_RC"
