#!/usr/bin/env bash
# ============================================================================
# run_vmflgpu003.sh -- VMFLGPU003 graded-run driver.
#
# Laminar Flow in a Lid-Driven Triangular Cavity, manual p.229.
# CPU PARENT: VMFL011 (manual p.41).  Object under verification: THE LAB'S GPU
# SOLVER PATH (OpenFOAM v2606 + petsc4Foam + PETSc-CUDA on the L4).
#
# GATE QUANTITY: the normalised X-velocity profile on the vertical line that
# bisects the base of the cavity (manual Figure .gpu003.2), read from the
# `bisector` sets function object registered in
# case/system/controlDict.template.  That object writes ONE PROFILE PER
# ITERATION, so the SAME artifact carries the gate value (the directory named
# exactly endTime) and the plateau history the comparator reads.
#
...
#
# IT GRADES NOTHING.  Grading is grade_vmflgpu003.py, frozen separately and
# cited by sha in PREREGISTRATION.md.
#
# WHAT THIS LAUNCHER DOES NOT INHERIT.  VMFLGPU001's and VMFLGPU002's limb-A
# tells were broken before compute (supervisor ruling, 2026-08-27) and the
# repair is in the COMPARATOR, not here.  What this launcher owes limb A is that
# BOTH ARMS ARE RUN WITH `-log_view`, so PETSc's own per-event GPU accounting
# exists in both logs and the forced-CPU control can be read at all.  Neither
# arm may be run without it, and the check below refuses if either is.
#
# ---------------------------------------------------------------------------
...
#
# EXECUTE this script (`bash run_vmflgpu002.sh <run_root>`); NEVER `source` it.
# ============================================================================
set -o pipefail   # wanted and safe. `set -e` / `set -u` deliberately NOT set.

CASE_ID="VMFLGPU003"
RANKS=1                       # serial; core-minutes = wall_s * RANKS / 60
CAP_GPU_H=1.5                 # FROZEN in PREREGISTRATION.md section 11.
CAP_CPU_ARM_CORE_MIN=90       # FROZEN in PREREGISTRATION.md section 11.
# name:NB:NH:ENDTIME  -- NB cells across the base, NH cells down the height.
# The blockMeshDict lays ONE degenerate hex collapsed onto the cavity apex, so
# TOTAL CELLS = NB*NH exactly: 800 / 3200 / 12800.  That is the CPU parent
# VMFL011's own mesh family, r = 2 (NB and NH double together), unchanged.  NB
# is EVEN at every level so the bisector x = 0 is a cell-FACE plane at all three
# levels and the sample line is identically placed under refinement.
#
# THE endTime VALUES ARE THE PARENT'S MEASURED CONVERGENCE POINTS WITH MARGIN,
# not a guess.  In VMFL011-R2's own log.simpleFoam the initial residuals of Ux,
# Uy AND p all fall below 1e-7 and STAY below it from iteration 259 / 488 / 1455
# at L1 / L2 / L3, under the SAME outer numerics this case keeps character for
# character.  1000 / 1500 / 3500 is 3.9x / 3.1x / 2.4x those.  There is no
# residualControl, so the run always reaches endTime and rule 4's "last time ==
# endTime" clause and the comparator's `Time =` line-count clause both bite; the
# comparator judges convergence after, from the residuals and from the gate
# quantity's own plateau.
LEVELS="L1_20x40:20:40:1000 L2_40x80:40:80:1500 L3_80x160:80:160:3500"
...
# LIMB A DEPENDS ON BOTH ARMS CARRYING `-log_view`.  The comparator reads PETSc's
# own per-event GPU accounting (GPU %F and the host-to-device transfer count) on
# the GPU arm AND on the forced-CPU control; without the table on the CONTROL
# there is nothing to discriminate against and the row certifies nothing.  This
# is asserted here rather than assumed, at zero compute.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CASE_SRC="$SCRIPT_DIR/case"
RUN_ROOT="${1:?usage: run_vmflgpu003.sh <run_root>}"
BUILD_ROOT="${BUILD_ROOT:-$HOME/gpu_build}"
REPO="${REPO:-$HOME/Certonomous}"
PREREG_REL="cases/ansys_verification/VMFLGPU003/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFLGPU003/grade_vmflgpu003.py"
...
# ---------------------------------------------------------------------------
# STEP 2c  EXCLUSIVE DEVICE -- REGISTERED CONTROL, NEW IN VMFLGPU002
#          (PREREGISTRATION.md section 4.3).  NOT inherited from VMFLGPU001.
#
# TWO GPU SOLVES ON ONE L4 CORRUPT BOTH CASES' EVIDENCE, and neither notices:
#
#   * LIMB A IS READ OFF A SHARED INSTRUMENT.  Tell 2 samples
#     `nvidia-smi --query-compute-apps=pid,used_memory`, which reports EVERY
#     process on the device.  A foreign solver's device memory lands in this
#     case's gpusample.txt, so tell 2 could FIRE FOR A CASE WHOSE OWN LINEAR
#     ALGEBRA NEVER TOUCHED THE CARD.  That is a false GPU-execution proof, and
#     limb A is the binary, physics-critical limb.
#   * BOTH COST RECORDS OVERSTATE AND NEITHER SAYS SO.  This case's GPU-hours
#     ARE wall time on a billed instance; two cases sharing the card each pay
#     full wall time for half a card.
#   * EVERY WALL FIGURE BECOMES A NUMBER ABOUT THE OTHER CASE TOO.  Contention
#     is not separable after the fact, and CONTENTION is a named, separately
#     reported category (COMPUTE_BUDGET_CHARTER section 6) -- it may not be
#     silently folded into a misprediction ratio at calibration time.
#
# THE QUEUE RUNNER CANNOT KNOW THIS.  It gates on CPU busy-ness and memory; a
# box whose four vCPU are idle while a GPU solve runs is EXACTLY what it will
# launch into.  So the refusal lives HERE, in the case that owns the
# requirement, and it is exit 2 -- a REFUSAL TO CERTIFY, of the same kind as the
# smoke gate, not the failure of a step.
#
# NVSMI is indirected for ONE reason: so this guard's own refusal path can be
# DRIVEN with a shim that prints a fake pid.  A guard nobody has seen refuse is
# a guard nobody has.
# ---------------------------------------------------------------------------
NVSMI="${NVSMI:-nvidia-smi}"
DEV_APPS="$("$NVSMI" --query-compute-apps=pid --format=csv,noheader 2>/dev/null | tr -d '[:space:]')"
if [ -n "$DEV_APPS" ]; then
  echo "REFUSE (exit 2): the L4 is NOT EXCLUSIVE to this case. nvidia-smi --query-compute-apps already reports process(es) holding the device: $("$NVSMI" --query-compute-apps=pid,used_memory --format=csv,noheader 2>/dev/null | tr '\n' ';')"
  echo "    A second GPU solve on this card would make limb A's tell 2 read the OTHER process's device memory as this case's, and would make both cases' GPU-hour records overstate without either saying so. This script does not share a device it is about to make a claim about. Wait for the running case to finish, or run this one on its own instance."
  exit 2
fi
log "EXCLUSIVE DEVICE VERIFIED: $NVSMI --query-compute-apps reports NO process holding the L4"
...
# REGISTERED CONTROL: THE MOVING WALL IS THE ONE THAT DRIVES THE CAVITY.
#
# The manual (p.229) gives this cavity exactly one driving datum: "Velocity of
# the top (base) wall = 2 m/s", every other wall stationary.  The gate quantity
# is the X-velocity profile NORMALISED BY THAT VELOCITY, and the comparator
# divides by a HARD-CODED U_WALL = 2.0.  So if the case's own 0/U carried a
# different wall speed, or drove the wrong patch, EVERY graded number would be
# normalised by a constant the field never saw -- and NOTHING DOWNSTREAM WOULD
# SHOW IT: the solve converges, the mesh is clean, the residuals fall, and limbs
# A and B BOTH STILL PASS because both arms carry the SAME error.  Only limb C
# moves, so the case would report a physics miss when what it actually had was a
# boundary condition it never checked.
#
# It is therefore checked, once per materialised case, BEFORE the mesher runs:
# `movingWall` must be fixedValue (2 0 0) EXACTLY, and `sideWalls` must be a
# no-slip wall.  Exact, not banded.
moving_wall_bc() {
  local d="$1"
  python3 - "$d/0/U" <<'MWGUARD'
import re, sys
path = sys.argv[1]
U_WALL = 2.0
try:
    ...
    print("ABORT moving-wall guard: cannot read %s (%s)" % (path, e)); sys.exit(1)
m = re.search(r"\bboundaryField\b\s*\{", txt)
if not m:
    print("ABORT moving-wall guard: no boundaryField block in %s" % path); sys.exit(1)
...
while i < len(txt) and depth:
    ...
    i += 1
if depth:
    print("ABORT moving-wall guard: unbalanced boundaryField in %s" % path); sys.exit(1)
body = txt[start:i-1]
def patch_block(name):
    mm = re.search(r"\b%s\b\s*\{" % re.escape(name), body)
    if not mm:
        return None
    k = mm.end(); d2 = 1; st = k
    while k < len(body) and d2:
        ...
        k += 1
    return body[st:k-1]
mw = patch_block("movingWall")
if mw is None:
    print("ABORT moving-wall guard: patch 'movingWall' is absent from the boundaryField "
          "of %s -- the cavity has no driver" % path); sys.exit(1)
t = re.search(r"\btype\s+(\w+)\s*;", mw)
if not t or t.group(1) != "fixedValue":
    print("ABORT moving-wall guard: movingWall type is %r, not fixedValue"
          % (t.group(1) if t else None)); sys.exit(1)
v = re.search(r"\bvalue\s+uniform\s*\(([^)]*)\)", mw)
if not v:
    print("ABORT moving-wall guard: movingWall carries no `value uniform (...)`"); sys.exit(1)
try:
    comps = [float(x) for x in v.group(1).split()]
except ValueError:
    print("ABORT moving-wall guard: unparseable movingWall value %r" % v.group(1)); sys.exit(1)
if len(comps) != 3 or comps[0] != U_WALL or comps[1] != 0.0 or comps[2] != 0.0:
    print("ABORT moving-wall guard: movingWall value is %r; the manual (p.229) and the "
          "comparator's hard-coded U_WALL both require exactly (%g 0 0). Every graded "
          "number is normalised by that constant." % (comps, U_WALL)); sys.exit(1)
sw = patch_block("sideWalls")
if sw is None:
    print("ABORT moving-wall guard: patch 'sideWalls' is absent from %s" % path); sys.exit(1)
t2 = re.search(r"\btype\s+(\w+)\s*;", sw)
if not t2 or t2.group(1) not in ("noSlip", "fixedValue"):
    print("ABORT moving-wall guard: sideWalls type is %r, which is neither noSlip nor a "
          "fixedValue; the manual says the other walls are stationary"
          % (t2.group(1) if t2 else None)); sys.exit(1)
if t2.group(1) == "fixedValue":
    v2 = re.search(r"\bvalue\s+uniform\s*\(([^)]*)\)", sw)
    if not v2 or any(float(x) != 0.0 for x in v2.group(1).split()):
        print("ABORT moving-wall guard: sideWalls is fixedValue but not (0 0 0)"); sys.exit(1)
print("MOVING WALL OK: movingWall fixedValue (%g 0 0), sideWalls %s" % (U_WALL, t2.group(1)))
sys.exit(0)
MWGUARD
}
...
FAILED=0
for spec in $LEVELS; do
  IFS=: read -r NAME NB NH ETIME <<< "$spec"
  EXPECT_CELLS=$(( NB * NH ))
  for arm in gpu cpu; do
    if [ "$arm" = "gpu" ]; then MAT="$GPU_MAT"; VEC="$GPU_VEC"; POPTS="$GPU_PETSC_OPTIONS";
    else                        MAT="$CPU_MAT"; VEC="$CPU_VEC"; POPTS="$CPU_PETSC_OPTIONS"; fi
    D="$RUN_ROOT/$arm/$NAME"
    log "--- $arm / $NAME : NB=${NB} across the base x NH=${NH} down the height (${EXPECT_CELLS} cells), endTime=$ETIME, mat=$MAT vec=$VEC"
    ...
    sed -e "s/__NB__/${NB}/g" -e "s/__NH__/${NH}/g" \
    ...
    # ---- REGISTERED CONTROL: the moving wall -------------------------------
    MWG="$(moving_wall_bc "$D")" || { echo "$MWG"; exit 1; }
    case "$MWG" in *ABORT*) echo "$MWG"; exit 1;; esac
    log "  $MWG"

    # ---- LIMB A PRECONDITION: BOTH arms must carry -log_view ----------------
    # Zero compute, and it refuses rather than discovering it after the solve.
    case "$POPTS" in
      *-log_view*) : ;;
      *) echo "ABORT: PETSC_OPTIONS for $arm/$NAME does not carry -log_view. Limb A is "\
              "read from PETSc's own per-event GPU accounting on BOTH arms; without the "\
              "table on the forced-CPU control there is nothing to discriminate against "\
              "and the row would certify nothing."; exit 1;;
    esac
