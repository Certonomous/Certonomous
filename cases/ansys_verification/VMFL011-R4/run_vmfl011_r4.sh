#!/usr/bin/env bash
# =============================================================================
# VMFL011-R4 graded-run driver -- Laminar Flow in a Triangular Cavity.
...
#
# Frozen pre-registration: cases/ansys_verification/VMFL011-R4/PREREGISTRATION.md
# Frozen comparator      : cases/ansys_verification/VMFL011-R4/grade_vmfl011_r4.py
# Run root               : verification/runs/ansys_verification/VMFL011-R4/
#
# MODELLED LINE FOR LINE ON cases/ansys_verification/VMFL011-R3/run_vmfl011_r3.sh
# (register row #36, which ran this exact L1/L2/L3 family to completion), with TWO
# substantive changes, both disclosed in the frozen pre-registration:
#   (A) an L4 finer level (160x320 = 51200 cells, r=2 finer than L3) is ADDED to the
#       ladder;
#   (12) the single RUNNING-TOTAL cap becomes PER-LEVEL caps (PER_LEVEL_CAP below)
#       plus a running-total ceiling, because L4 (~64 core-min on a PCG/DIC pressure
#       solve at 51200 cells) dominates the ladder and a per-level stop is the honest
#       instrument for it (CLAUDE.md rule 12).
# ARGV: the run root is $1 and it is REQUIRED -- VMFL064R2-ENTRY-DEF-1 was a queue
# entry whose argv omitted it and whose launcher refused at zero compute.  Every
# queue entry for this launcher MUST pass <run_root>.
#
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# THE CAPS ARE A HARD STOP, NOT A SCHEDULE.  core_minutes = wall_s * RANKS / 60 ;
# a level's timeout_s = min(per-level cap, remaining total) * 60 / RANKS.  An overrun
# STOPS the run (rc 124); endTime is NEVER silently reduced to fit a cap
# (CLAUDE.md rule 12).
# =============================================================================

RANKS=1
# PER-LEVEL caps (core-min), sized from R3's MEASURED costs (row #36, calibration
# C-175: L1 0.2667, L2 1.0667, L3 8.2333 core-min) with L4 extrapolated from the
# measured L3/L2 = 7.72x PCG scaling (~64 core-min), each capped with headroom so a
# normal run completes and only a runaway stops.
declare -A PER_LEVEL_CAP=( [L1]=3 [L2]=8 [L3]=25 [L4]=130 )
CAP_CORE_MIN=170         # running-TOTAL backstop (> sum of per-level caps 166, so the
                         # per-level caps are the binding constraint; the total is a
                         # cumulative-overrun backstop)
ENDTIME=20000            # byte-identical to attempts 1/2/3

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CASE_DIR="$SCRIPT_DIR/case"
RUN_ROOT="${1:?usage: run_vmfl011_r4.sh <run_root> [levels...]}"
shift 2>/dev/null
LEVELS_TO_RUN="${*:-L1 L2 L3 L4}"

# r = 2 grid triple/quadruple: base cells NB and height cells NH double together.
# NB is EVEN at every level so the bisector x = 0 is a cell-face plane identically
# placed under refinement.  L4 continues the r=2 ladder one step finer than L3.
declare -A NB=( [L1]=20 [L2]=40 [L3]=80  [L4]=160 )
declare -A NH=( [L1]=40 [L2]=80 [L3]=160 [L4]=320 )

echo "=== VMFL011-R4 launcher  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)  run_root=$RUN_ROOT"
...
PREREG_REL="cases/ansys_verification/VMFL011-R4/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL011-R4/grade_vmfl011_r4.py"
...
# claim in a document.  R4 changes NO case input (the L4 level is a driver NB/NH
# addition on the same blockMeshDict.template) and the fvSchemes stays
# `div(phi,U) bounded Gauss linear` (the DATED-PLAN linearUpwind->linear clause was a
# NO-OP on a false premise and is STRUCK -- prereg change B, L-502).
for F in case/0/U case/0/p case/constant/transportProperties \
  ...
  B="$(git -C "$REPO" hash-object "$SCRIPT_DIR/$F")" || { echo "ABORT: cannot hash R4 $F"; exit 2; }
  [ "$A" = "$B" ] || { echo "ABORT: $F is NOT the attempt-1 input ($B != $A) -- R4 changes NO case input"; exit 2; }
done
...
# plant-sizing repair and L-347 placement repair on the REAL attempt-1/R2 bytes, the
# observed-order floor, the L-342 field classes, AND the four R4-new checks: the
# finest-three graded triple (change A / L-500), the settling diagnostic, the
# value-blind digitisation-floor characterisation (change C), and the gate-blind
# physical-range guard with its PRESENT and KNOWN-BAD arms (change D).  A comparator
# whose controls do not fire does not grade this run, so the launcher refuses BEFORE
# spending anything.  Checked under BOTH interpreters, because `python3 -O` deletes
# every assert and a control that exists only under one flag is not a control (L-332).
ST="/tmp/vmfl011r4_selftest.$$"
python3 "$SCRIPT_DIR/grade_vmfl011_r4.py" --selftest > "$ST" 2>&1 || { echo "ABORT: comparator --selftest is NOT green under python3; see $ST"; exit 2; }
python3 -O "$SCRIPT_DIR/grade_vmfl011_r4.py" --selftest > "$ST.O" 2>&1 || { echo "ABORT: comparator --selftest is NOT green under python3 -O; see $ST.O"; exit 2; }
...
grep -q 'planted-zero ACCEPTS a reader that DOES see the plant' "$ST" || { echo "ABORT: comparator --selftest did not exercise the planted-zero PRESENT arm"; exit 2; }
...
grep -q 'L-347 probe 4: a BLIND reader is still REFUSED under the argmin plant' "$ST" || { echo "ABORT: comparator --selftest did not show a blind reader still refused"; exit 2; }
...
grep -q 'R4 change A: GRADED_TRIPLE_LEVELS is the FINEST THREE levels (L-500: no drop)' "$ST" || { echo "ABORT: comparator --selftest did not prove the graded triple is the FINEST THREE (change A / L-500)"; exit 2; }
grep -q 'R4 change A: four_point_order reports BOTH overlapping triples' "$ST" || { echo "ABORT: comparator --selftest did not exercise the 4-point settling diagnostic (change A)"; exit 2; }
grep -q 'R4 change C: it CHARACTERISES only and cannot widen the gate (L-487)' "$ST" || { echo "ABORT: comparator --selftest did not show the digitisation-floor characterisation cannot widen the gate (change C)"; exit 2; }
grep -q 'R4 change D: PRESENT arm' "$ST" || { echo "ABORT: comparator --selftest did not exercise the physical-range guard PRESENT arm (change D)"; exit 2; }
grep -q 'R4 change D: the physical-range guard is GATE-BLIND' "$ST" || { echo "ABORT: comparator --selftest did not show the physical-range guard is gate-blind (change D)"; exit 2; }
NCHK="$(grep -c '^  \[PASS\]' "$ST")"
...
{ echo "case = VMFL011-R4"
  echo "supersedes = register row #36 (VMFL011-R3, GATE FAIL) -- a NEW row, the R3 row is never overwritten"
  ...
  echo "ranks = $RANKS"
  echo "per_level_cap_core_min = L1=${PER_LEVEL_CAP[L1]} L2=${PER_LEVEL_CAP[L2]} L3=${PER_LEVEL_CAP[L3]} L4=${PER_LEVEL_CAP[L4]}"
  echo "total_cap_core_min = $CAP_CORE_MIN  (running-TOTAL ceiling)"
  echo "endTime = $ENDTIME"
  echo "smoke_mode = ${VMFL_SMOKE:-no}"
} > "$LR" || { echo "ABORT: cannot write LAUNCH_RECORD.txt"; exit 2; }

# --- OpenFOAM ------------------------------------------------------------------
# L-343 USER PIN, BEFORE the bashrc is sourced and gated explicitly.  A cron-started
# queue runner carries LOGNAME but NO USER; without the pin FOAM_USER_LIBBIN resolves
# to a `user-v2606` directory that does not exist.  This case links no user library,
# so the pin cannot change a number here -- it is applied because the launcher must be
...
for L in $LEVELS_TO_RUN; do
  ...
  [ -n "${PER_LEVEL_CAP[$L]}" ] || { echo "ABORT: no per-level cap registered for $L"; exit 2; }
  ...
  # CAP ENFORCEMENT: the level's budget is the SMALLER of its per-level cap and the
  # remaining running total, in the executable path.  An overrun STOPS the run (rc 124).
  REMAIN_TOTAL="$(python3 -c "print(max(0.0, $CAP_CORE_MIN - $TOTAL_CORE_MIN))")" || { echo "ABORT: total cap arithmetic failed for $L"; exit 2; }
  LEVEL_BUDGET="$(python3 -c "print(min(${PER_LEVEL_CAP[$L]}, $REMAIN_TOTAL))")" || { echo "ABORT: per-level cap arithmetic failed for $L"; exit 2; }
  TIMEOUT_S="$(python3 -c "print(int($LEVEL_BUDGET * 60 / $RANKS))")" || { echo "ABORT: timeout arithmetic failed for $L"; exit 2; }
  [ "$TIMEOUT_S" -gt 0 ] || { echo "ABORT: BUDGET EXHAUSTED before $L (spent $TOTAL_CORE_MIN of $CAP_CORE_MIN core-min; per-level cap ${PER_LEVEL_CAP[$L]}). An overrun STOPS the run and does NOT get a new budget (rule 12)."; exit 2; }
  echo "--- $L  per-level cap ${PER_LEVEL_CAP[$L]}  remaining-total ${REMAIN_TOTAL} of ${CAP_CORE_MIN}  budget ${LEVEL_BUDGET} core-min  ranks ${RANKS}  timeout ${TIMEOUT_S}s"
...
PYBC
  ...
  printf 'level=%s\nstate=STARTED\nranks=%d\nper_level_cap_core_min=%s\ntotal_cap_core_min=%s\nlevel_budget_core_min=%s\ntimeout_s=%d\nendTime=%s\nprereg_blob=%s\ncomparator_blob=%s\nstarted_utc=%s\nnote=PER-LEVEL cap min(per-level, remaining-total). This file is an L-342 INFRASTRUCTURE artifact: the comparator gates on the physics artefacts and reports rc NOT MEASURED if this file is absent.\n' \
    "$L" "$RANKS" "${PER_LEVEL_CAP[$L]}" "$CAP_CORE_MIN" "$LEVEL_BUDGET" "$TIMEOUT_S" "$ENDTIME" "$PREREG_BLOB" "$GRADER_BLOB" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$RUN_ROOT/RUN_RC.$L" \
  ...
  # later stage of the wrapper (setsid-returns-zero trap).  Detachment is verified by
  # the WRAPPER's SID == PID (field 6 of /proc/<pid>/stat), never by the solver's ppid.
  rm -f "$OUT/.solver_rc"
  (
    cd "$OUT" || exit 90
  ...
  printf 'rc = %s\nlevel = %s\nstate = FINISHED\nwait_rc = %d\nwall_s = %d\nranks = %d\ncore_min = %s\ntotal_core_min_so_far = %s\ntimeout_s = %d\nper_level_cap_core_min = %s\ntotal_cap_core_min = %s\nendTime = %s\nlast_time_in_log = %s\nEnd_lines = %s\nlatest_time_dir = %s\nfield_at_latest_newer_than_0_U = %s\ndetach_wrapper_pid = %s\ndetach_wrapper_sid = %s\nprereg_blob = %s\ncomparator_blob = %s\nsolver_bin = %s\nutc = %s\nnote = rc is simpleFoam exit status under timeout, CAPTURED INSIDE the detached subshell; 124 means a cap (per-level or running-total) fired. endTime is NEVER reduced to fit a cap (rule 12). L-342: this file is INFRASTRUCTURE -- absent, the comparator reports rc NOT MEASURED and grades the physics artefacts.\n' \
    "$RC" "$L" "$WAIT_RC" "$WALL" "$RANKS" "$CORE_MIN" "$TOTAL_CORE_MIN" "$TIMEOUT_S" "${PER_LEVEL_CAP[$L]}" "$CAP_CORE_MIN" "$ENDTIME" "${LAST_TIME:-none}" "${ENDED:-0}" "${ENDT_DIR:-none}" "$ENDT_OK" "$SOLVER_PID" "${WSID:-unknown}" "$PREREG_BLOB" "$GRADER_BLOB" "$SOLVER_BIN" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  ...
  { echo "level $L : rc=$RC wall_s=$WALL core_min=$CORE_MIN total=$TOTAL_CORE_MIN per_level_cap=${PER_LEVEL_CAP[$L]} timeout_s=$TIMEOUT_S last_time=${LAST_TIME:-none} End_lines=${ENDED:-0} latest_time_dir=${ENDT_DIR:-none} field_newer_than_0_U=$ENDT_OK"; } >> "$LR"

  # STOP AT THE FIRST NON-ZERO rc.
  if [ "$RC" -ne 0 ]; then
    OVERALL_RC="$RC"
    echo "STOP $L: rc=$RC after ${WALL}s = ${CORE_MIN} core-min (running total ${TOTAL_CORE_MIN} of ${CAP_CORE_MIN}; per-level cap ${PER_LEVEL_CAP[$L]})."
    echo "  A non-zero rc is a FINDING, not a retry. rc=124 means a cap stopped it;"
...
printf 'total_core_min = %s\ntotal_cap_core_min = %s\nper_level_cap_core_min = L1=%s L2=%s L3=%s L4=%s\nranks = %d\noverall_rc = %s\nsmoke_mode = %s\ncost_basis = owner-stated rate $0.0513/core-h (c7a.4xlarge, Sanaa 2026-08-21/22); REPORTED-BY-OWNER, NOT MEASURED -- the box cannot read its own billing (COMPUTE_BUDGET_CHARTER.md sec.5). Dollars are DERIVED.\n' \
  "$TOTAL_CORE_MIN" "$CAP_CORE_MIN" "${PER_LEVEL_CAP[L1]}" "${PER_LEVEL_CAP[L2]}" "${PER_LEVEL_CAP[L3]}" "${PER_LEVEL_CAP[L4]}" "$RANKS" "$OVERALL_RC" "${VMFL_SMOKE:-no}" > "$RUN_ROOT/COST.txt" || { echo "ABORT: cannot write COST.txt"; exit 2; }
echo "total_core_min = $TOTAL_CORE_MIN" >> "$LR"
echo "finished_utc = $(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$LR"
echo "=== VMFL011-R4 launcher done, overall rc=$OVERALL_RC, total ${TOTAL_CORE_MIN} core-min of ${CAP_CORE_MIN}"
echo "grade with: python3 $SCRIPT_DIR/grade_vmfl011_r4.py --run-root $RUN_ROOT"
exit "$OVERALL_RC"
