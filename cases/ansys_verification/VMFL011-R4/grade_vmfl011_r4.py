#!/usr/bin/env python3
"""grade_vmfl011_r4.py -- comparator for VMFL011-R4, Laminar Flow in a Triangular
...
register row that cites the R3 row #36 and never overwrites it).  Parent (R3)
comparator blob 3975d9ee3a60bde8b2c1537c27abb1431662d65b, parent pre-registration
freeze 45c3e8a4ec3a805f98e7e2464cbbe770102c3f38.  R3 graded GATE FAIL: rms 0.034088
at L3 vs band <= 0.030, on a CONVERGING triple, with the rms sequence
L1/L2/L3 = 0.040264 / 0.034775 / 0.034088 STILL FALLING with refinement.

WHAT R4 CHANGES, AND WHAT IT DELIBERATELY DOES NOT.  The gate is UNCHANGED and
carried BYTE-IDENTICAL from R3 (L-487): rms_vs_benchmark at the FINEST level, band
BAND_RMS = 0.030, tier ceiling GATE REACHED, the reference blob, U_WALL, ENDTIME,
the readers (_read_rms, _read_umin), roache() with P_MIN, and the VERDICT-DECISION
block (the `if R['blocking'] / elif triple != CONVERGING / else band-check` cascade
that reads rms at LEVELS[-1] and maps it to GATE REACHED / GATE FAIL / NOT A RESULT).
Each of these four spans is proven EMPTY-DIFF against R3 in the pre-registration.
THE ONE GATE-BLOCK LINE THAT CHANGES IS `LEVELS`, which GAINS 'L4' (change A) -- a
finer level EXTENDS the ladder, it cannot move the band 0.030 or the ceiling; and the
triple-SELECTION in main() is re-pointed at the finest three (LEVELS[-3:]).  Adding a
finer level can only turn a genuinely-asymptotic-and-in-band solve into GATE REACHED;
a still-failing rms stays GATE FAIL, and a non-asymptotic triple is NOT A RESULT with
no level dropped (L-500).  R4 makes exactly two additive, off-gate changes plus two
diagnostics:
  (A) ADD an L4 finer grid level (r=2: 160x320 = 51200 cells) so the rms metric is
      read at a finer h and the question "does the still-falling rms enter the band
      as h->0, or plateau above it?" can be answered.  The GRADED Roache triple is
      FIXED A-PRIORI = the 3 FINEST levels (L2, L3, L4) -- LEVELS[-3:], no fallback
      search, no dropped level (rule 2, L-500/L-501 anti-circularity; mirrors
      VMFL054-R3).  A 4-point (L1..L4) settling diagnostic reports whether the order
      is settling but MOVES NO GATE QUANTITY.
  (B) The DATED-PLAN 'linearUpwind->linear' scheme clause is STRUCK as a NO-OP.
      R3's frozen system/fvSchemes uses `div(phi,U) bounded Gauss linear` -- 2nd-order
      central, the least-diffusive scheme, blob 5f997a3b9051bc7c4c284c065cebc946f66507ae,
      disk==HEAD, ZERO linearUpwind in the file's full git history.  There is no
      1st-order-blend to raise to 2nd order; the swap would change nothing and
      freezing it would put a false premise in a rule-6 record.  Same falsification
      as VMFL063-R2 (registry row, 2026-09-07).  R4 keeps the schemes and every case
      input BYTE-IDENTICAL to the attempt-1/R3 family.
  (C) DIGITISATION-ERROR CHARACTERISATION of the referent (digitisation_floor_diag).
      The reference is Ansys's digitisation of a FIGURE (Jyotsna & Vanka 1995, manual
      Figure .11.2; NO discrete target table).  The diagnostic bounds the referent's
      digitisation noise from its OWN internal evidence (the RMS of reference values
      in the physically-quiescent lower cavity, where the true velocity ~ 0) and from
      full-scale reading-error fractions.  It is a CHARACTERISATION printed off the
      gate path; it references no band and it CANNOT widen the 0.030 gate (L-487).
  (D) A GATE-BLIND PHYSICAL-RANGE REFUSAL (physical_range_refusal) per level: rms and
      u_min_norm are bounded by the physics (an incompressible lid-driven cavity
      cannot exceed the wall speed), so a NaN/inf or an absurd magnitude REFUSES
      (exit 2).  Its bounds reference NEITHER 0.030 NOR the benchmark -0.318062.

THE L-340 AND L-347 CONTROL REPAIRS ARE CARRIED FROM R3 BYTE-IDENTICAL and still
DRIVEN on the real attempt-1 and R2 bisector bytes embedded below; R1/R2 died on a
plant the reader could not see, and that fix is preserved in full.

WHAT THE R3 REFUSAL NAMED, AND THE ONLY THING THAT FILE CHANGED ON THE
...
def planted_zero(artifact, perturb_fn, read_fn, plant, label, exit_on_fail=True):
    ...
    try:
        ...
        seen = read_fn(cp)
        delta = abs(seen - base)
        rec = {'artifact': artifact, 'plant': plant, 'baseline': base,
        ...
        if not (delta > 0.1 * abs(plant)):
            ...
            rec['message'] = (
                ...
                '  planted %g into %s, reader moved by only %g (threshold %g).\n'
                '  A reader not shown able to see a non-zero cannot certify a zero '
                '(CLAUDE.md rule 3).' % (label, plant, artifact, delta,
                                         0.1 * abs(plant)))
            # L-347 clause 3: a control that exits HERE hides every control behind
            # it.  When exit_on_fail is False the caller collects EVERY channel's
            # result and refuses ONCE, with all of them printed.
            if exit_on_fail:
                sys.stderr.write('REFUSING (exit 2): %s\n' % rec['message'])
                sys.exit(2)
        return rec
...
ENDTIME  = 20000
FIELDS   = ['U', 'p']
LEVELS   = ['L1', 'L2', 'L3', 'L4']   # R4 ADDS L4 (160x320, r=2 finer than L3)
...
# --- the L-347 repair, and the whole of it ----------------------------------
# BYTE-IDENTICAL IN MAGNITUDE to the parent's u_min plant (-abs(PLANT)*100); the
# ONLY change is WHERE it is planted.  At the argmin row a min() reader moves by
# exactly |plant|, so delta/threshold = 10 by construction, for any data.
UMIN_PLANT      = -abs(1.234e-03) * 100   # == -abs(PLANT)*100, the
                                          # parent's magnitude EXACTLY
                                          # (-0.12340000000000001)
RMS_PLANT_FLOOR = 1.0e-6  # so a run that matched the benchmark exactly still plants

# --- R4 additive constants, ALL off the gate path ----------------------------
# The GRADED Roache triple is the 3 FINEST levels, fixed a-priori at this freeze.
GRADED_TRIPLE_LEVELS = LEVELS[-3:]        # ('L2','L3','L4'); no fallback, no drop
# (C) Digitisation-error characterisation of the FIGURE-digitised referent.  These
# feed digitisation_floor_diagnostic() only; NO verdict reads them, and none of them
# is BAND_RMS or the benchmark value (L-487: characterise, never widen the band).
QUIESCENT_Y      = -2.0                   # m; below this the triangular-cavity flow
                                          # is physically ~quiescent (truth u ~ 0),
                                          # so the reference's own scatter there is a
                                          # direct lower bound on its digitisation noise
FS_READ_ERR_PCT  = (0.5, 1.0, 2.0, 3.0)   # full-scale reading-error fractions to report
# (D) Gate-blind physical bounds.  |u/U_WALL| cannot much exceed 1 in an
# incompressible lid-driven cavity (max speed is the lid speed); an RMS of the
# difference of two such bounded profiles is bounded well below 2.  These reference
# the PHYSICS (the wall-speed bound), never the gate 0.030 or the benchmark -0.318.
U_MIN_PHYS_LO    = -1.5
U_MIN_PHYS_HI    =  1.5
RMS_PHYS_HI      =  2.0
...
def _argmin_row(path):
    """The index of the row the `min()` reader SELECTS.  L-347: a plant that does
    not land here cannot move a selecting reader, whatever its magnitude."""
    r = _rows(path)
    if not r:
        refuse('_argmin_row found NO data row in %s' % path)
    best = 0
...
def _perturb_at_argmin(path, plant):
    ...
    For a `min()` reader the argmin row carries the answer.  Adding a NEGATIVE
    plant there makes that row the minimum by exactly |plant|, so the read moves
    by EXACTLY |plant| -- which is > 0.1*|plant| for any non-zero plant, BY
    CONSTRUCTION and independently of the data.  The threshold rule
    `delta > 0.1*|plant|` is carried CHARACTER FOR CHARACTER from the parent; the
    repair is the LOCATION, never a loosened test (L-340's rule, carried).

    The parent's `_perturb` (row 0) is preserved below, OFF the grading path, so
    umin_placement_control() can show it failing on the real R2 bytes."""
    if plant >= 0:
        refuse('_perturb_at_argmin was handed a non-negative plant (%r); a min() '
    ...
    for i, ln in enumerate(lines):
        ...
        open(path, 'w').write('\n'.join(lines) + '\n')
        return
    refuse('_perturb_at_argmin could not reach the argmin row %d of %s' % (idx, path))

def digitisation_floor_diagnostic():
    """CHARACTERISE the FIGURE-digitised referent's digitisation noise (VMFL011-R4
    change (C); charter sec.2al/2am).  The reference is Ansys's digitisation of the
    Jyotsna & Vanka curve (manual Figure .11.2) -- a curve READ OFF A PLOT, carried
    to 5-6 stored significant figures whose PRECISION is not its ACCURACY.  Two
    honest, reproducible bounds, computed from the reference bytes themselves:

      * INTERNAL floor (a LOWER bound): in the physically-quiescent lower cavity
        (y <= QUIESCENT_Y) the true velocity is ~ 0, so the RMS of the reference
        values there directly measures the digitisation noise in the region where
        the digitiser could align to a nearly flat curve.
      * READING-ERROR floor: a curve digitised from a figure carries a per-point
        reading error of order a fraction of the plotted full-scale range.  For each
        fraction in FS_READ_ERR_PCT this reports sigma_dig = pct * span -- which, for
        ~independent zero-mean per-point errors, is the rms a PERFECT model would
        STILL show against the digitised curve: an rms FLOOR no model can beat.

    MOVES NO VERDICT.  It references no band and cannot widen the 0.030 gate (L-487).
    It answers only: given the referent's quality, is rms <= 0.030 even attainable,
    or is the band inside the referent's own digitisation noise?"""
    by, bv = _bench()
    span = max(bv) - min(bv)
    qui = [v for y, v in zip(by, bv) if y <= QUIESCENT_Y]
    internal = math.sqrt(sum(v * v for v in qui) / len(qui)) if qui else None
    floors = {('%.1f%%FS' % p): (p / 100.0) * span for p in FS_READ_ERR_PCT}
    return {'referent': 'figure-digitised (Jyotsna & Vanka Fig .11.2; NO target table)',
            'full_scale_span_u_norm': span,
            'quiescent_y_cutoff_m': QUIESCENT_Y,
            'n_quiescent_rows': len(qui),
            'internal_quiescent_rms_lower_bound': internal,
            'reading_error_rms_floors': floors,
            'note': 'CHARACTERISATION ONLY; references no band; cannot widen the gate (L-487). '
                    'A rms FLOOR at or above the frozen 0.030 band would mean GATE REACHED is '
                    'UNATTAINABLE against this referent whatever the lab solve does.'}

def physical_range_refusal(ch, level):
    """GATE-BLIND refusal (VMFL011-R4 change (D)).  rms and u_min_norm are bounded by
    the physics -- an incompressible lid-driven cavity cannot exceed the wall speed --
    so a NaN/inf or a physically impossible magnitude is a corrupt read and REFUSES
    (exit 2).  The bounds reference the wall-speed physics, NEVER the gate 0.030 or
    the benchmark -0.318062."""
    rms = ch['rms_vs_benchmark']; umin = ch['u_min_norm']
    if not math.isfinite(rms) or rms < 0.0 or rms > RMS_PHYS_HI:
        refuse('%s: rms_vs_benchmark = %r is not a physical RMS in [0, %g] -- an RMS is '
               'non-negative and, for two profiles bounded by the wall speed, well below '
               '%g; a value outside it is a corrupt read (gate-blind: references neither '
               'the band nor the benchmark)' % (level, rms, RMS_PHYS_HI, RMS_PHYS_HI))
    if not math.isfinite(umin) or not (U_MIN_PHYS_LO <= umin <= U_MIN_PHYS_HI):
        refuse('%s: u_min_norm = %r is outside the physical bound [%g, %g] -- |u/U_WALL| '
               'cannot much exceed 1 in an incompressible lid-driven cavity; a value '
               'outside it is a corrupt read (gate-blind: references neither the band '
               'nor the benchmark)' % (level, umin, U_MIN_PHYS_LO, U_MIN_PHYS_HI))

def four_point_order(umins, r=2.0):
    """4-POINT SETTLING DIAGNOSTIC (L1..L4).  Reports the two overlapping Roache
    triples -- the coarse (L1,L2,L3) and the GRADED finest (L2,L3,L4) -- so a reader
    can see whether the observed order is SETTLING toward the asymptotic range as
    h -> 0.  DIAGNOSTIC ONLY: it moves no gate quantity and no verdict reads it."""
    coarse = roache(umins[0], umins[1], umins[2], r)
    fine   = roache(umins[1], umins[2], umins[3], r)
    return {'triple_L1L2L3': {'state': coarse['state'], 'p': coarse.get('p'),
                              'ratio': coarse.get('ratio')},
            'triple_L2L3L4': {'state': fine['state'], 'p': fine.get('p'),
                              'ratio': fine.get('ratio')},
            'note': 'DIAGNOSTIC ONLY; the GRADED triple is the finest (L2,L3,L4); '
                    'this moves no gate quantity'}
...
def controls(level_dir):
    """The rule-3 controls, one per channel, each with the plant sized AND PLACED
    for ITS reader.

    L-347 clause 3, and it is the reason this function does not simply return a
    dict of calls: EVERY channel's control runs, and EVERY result is reported,
    BEFORE any exit.  The parent exited inside the first entry of a dict literal,
    so the second channel's control had never once executed on real data and its
    defect stayed invisible across two complete runs.  Here the failures are
    collected, all of them are printed, and the refusal happens ONCE at the end."""
    p = _xy(level_dir)
    base_rms = _read_rms(p)
    out = {}
    out['rms_vs_benchmark'] = planted_zero(
        p, _perturb_all, _read_rms, rms_plant_for(base_rms),
        'rms_vs_benchmark', exit_on_fail=False)
    out['u_min_norm'] = planted_zero(
        p, _perturb_at_argmin, _read_umin, UMIN_PLANT,
        'u_min_norm', exit_on_fail=False)
    bad = [k for k in sorted(out) if out[k]['control'] != 'PASS']
    if bad:
        lines = ['%d of %d rule-3 channel control(s) FAILED at %s; ALL controls were '
                 'run and ALL are reported (L-347).' % (len(bad), len(out), level_dir)]
        for k in sorted(out):
            r = out[k]
            if r['control'] == 'PASS':
                lines.append('  [control PASS] %s: plant %g moved the reader by %g '
                             '(threshold %g)' % (k, r['plant'], r['delta_seen'],
                                                 r['threshold']))
            else:
                lines.append('  [control FAIL] ' + r['message'].replace('\n', '\n  '))
        refuse('\n'.join(lines))
    return out
...
# ============================================================================
# THE REAL BYTES THAT MADE THE R2 REFUSE.
# verification/runs/ansys_verification/VMFL011-R2/L1/postProcessing/bisector/
# 20000/bisect_U.xy, verbatim, 401 sampled points, git blob 11c1577972244f7d5fde3ea98d1676dce0468126.
# It is embedded so the control that DRIVES this repair runs on the ACTUAL
# artifact of the refusal recorded in cases/ansys_verification/VMFL011-R2/
# RESULTS.md and register row #31, not on a construction that resembles it, and
# so the check survives in a scratch tree with no repository.
#
# USING AN EXISTING RUN'S OUTPUT TO **DRIVE A CONTROL** IS LEGITIMATE AND IS WHAT
# L-347 CLAUSE 2 REQUIRES.  USING IT TO SET A BAND, A THRESHOLD OR A GATE
# QUANTITY WOULD BE GATE-FITTING AND IS REFUSED (rule 2).  These bytes are read
# ONLY by umin_placement_control(); NO VERDICT READS THEM, and every gate-path
# constant in this file is byte-identical to the R2 freeze, which was made before
# the R2 run existed.
# ============================================================================
REAL_XY_L1_R2_BLOB = '11c1577972244f7d5fde3ea98d1676dce0468126'
REAL_XY_L1_R2_N    = 401
REAL_XY_L1_R2_UMIN = -0.2644944566075       # min(u)/U_WALL over the 401 samples
                                            # (raw min -0.528988913215 / U_WALL)
REAL_XY_L1_R2_ARGI = 300        # the row the min() reader SELECTS
REAL_XY_L1_R2_ARGY = -1.0        # its abscissa, m
REAL_XY_L1_R2_ROW0 = 0.0        # the row the PARENT planted into: u == 0 at
                                      # the collapsed-hex apex y = -4 m
REAL_XY_L1_R2 = r'''
-4 	0 	0 	0
-3.99 	6.24899441265e-12 	-6.13305354073e-17 	-1.97803336483e-20
-3.98 	1.24979888253e-11 	-1.22661070815e-16 	-3.95606672965e-20
...
def _write_real_r2_xy(path):
    open(path, 'w').write(REAL_XY_L1_R2.lstrip('\n'))

def umin_placement_control(quiet=False):
    """THE L-347 CONTROL -- the control that DRIVES this re-registration.

    It runs in --selftest AND in main() BEFORE any level is read.  Four probes,
    each refusing (exit 2) on its own failing path:

      (1) DRIVES THE FIX.  The R2 pair (row-0 plant, magnitude -0.1234) must
          STILL REFUSE on these bytes, moving the min() reader by EXACTLY 0.  If
          it did not refuse, nothing would justify this file existing.
      (2) THE PLACEMENT IS LOAD-BEARING, NOT THE MAGNITUDE.  The registered pair
          (argmin plant, THE SAME magnitude) must PASS, and its move must equal
          |plant| to 1e-12 -- the theorem, not a measurement.
      (3) THE DEFECT IS DATA-DEPENDENT, AND THAT IS WHY A FIXTURE MISSED IT.  On a
          profile whose argmin IS row 0, the parent's row-0 plant PASSES.  A
          selftest built on such a fixture would have certified a control that
          cannot see the real data -- which is exactly what happened.
      (4) A BLIND READER IS STILL REFUSED under the new placement, so the repair
          did not buy its pass by weakening anything."""
    import tempfile as _tf
    d = _tf.mkdtemp(prefix='umin_')
    try:
        real = os.path.join(d, 'bisect_U.xy')
        _write_real_r2_xy(real)
        got = _blob_sha1(real)
        if got != REAL_XY_L1_R2_BLOB:
            refuse('the embedded R2 L1 bisector is not the registered blob '
                   '(%s, registered %s) -- the bytes this repair is driven '
                   'against have drifted' % (got, REAL_XY_L1_R2_BLOB))
        r = _rows(real)
        if len(r) != REAL_XY_L1_R2_N:
            refuse('the embedded R2 L1 bisector carries %d rows, registered %d'
                   % (len(r), REAL_XY_L1_R2_N))
        base = _read_umin(real)
        if abs(base - REAL_XY_L1_R2_UMIN) > 1e-12:
            refuse('the embedded R2 L1 bisector reads min(u)/U_WALL = %r, '
                   'registered %r' % (base, REAL_XY_L1_R2_UMIN))
        ai = _argmin_row(real)
        if ai != REAL_XY_L1_R2_ARGI:
            refuse('the min() reader selects row %d of the embedded R2 bytes, '
                   'registered %d' % (ai, REAL_XY_L1_R2_ARGI))
        if abs(r[0][1] - REAL_XY_L1_R2_ROW0) > 1e-12:
            refuse('row 0 of the embedded R2 bytes reads u = %r, registered %r -- '
                   'the apex identity this lesson rests on has drifted'
                   % (r[0][1], REAL_XY_L1_R2_ROW0))

        # ---- probe (1): the R2 pair must STILL refuse on these bytes
        code1, msg1 = expect_refusal(d, planted_zero, real, _perturb, _read_umin,
                                     UMIN_PLANT, 'R2 pair (off-path probe)')
        parent_refused = (code1 == 2)
        cp = os.path.join(d, 'p_row0.xy'); shutil.copy2(real, cp)
        _perturb(cp, UMIN_PLANT)
        d_parent = abs(_read_umin(cp) - base)
        if not parent_refused:
            refuse('probe (1): the R2 row-0 pair did NOT refuse on the real R2 L1 '
                   'bytes, so NOTHING drives this re-registration (L-347)')
        if d_parent != 0.0:
            refuse('probe (1): the R2 row-0 plant moved the min() reader by %r, and '
                   'register row #31 recorded EXACTLY 0 -- the bytes or the reader '
                   'have drifted' % d_parent)

        # ---- probe (2): the registered pair passes, and by the theorem
        rec = planted_zero(real, _perturb_at_argmin, _read_umin, UMIN_PLANT,
                           'u_min_norm (registered pair)', exit_on_fail=False)
        if rec['control'] != 'PASS':
            refuse('probe (2): the REGISTERED argmin pair failed its own control on '
                   'the real R2 L1 bytes -- the repair does not work')
        expect = abs(UMIN_PLANT) / U_WALL
        if abs(rec['delta_seen'] - expect) > 1e-12:
            refuse('probe (2): the argmin plant moved the reader by %r, and the '
                   'theorem says EXACTLY |plant|/U_WALL = %r (the plant is applied '
                   'to the RAW column, the reader normalises); a reader that '
                   'responded differently from the theorem is refused'
                   % (rec['delta_seen'], expect))

        # ---- probe (3): the defect is DATA-DEPENDENT -- a fixture hides it
        fx = os.path.join(d, 'fixture.xy')
        with open(fx, 'w') as fh:
            fh.write('%r\t%r\t0\t0\n' % (-4.0, -1.0))
            for i in range(1, 5):
                fh.write('%r\t%r\t0\t0\n' % (-4.0 + i * 0.5, 0.1 * i))
        rec3 = planted_zero(fx, _perturb, _read_umin, UMIN_PLANT,
                            'row-0 plant on a fixture whose argmin IS row 0',
                            exit_on_fail=False)
        if rec3['control'] != 'PASS':
            refuse('probe (3): the parent pair failed even on a fixture whose argmin '
                   'is row 0; then the defect would not be data-dependent and this '
                   "file's account of why the freeze missed it is wrong")

        # ---- probe (4): a blind reader is STILL refused under the new placement
        code4, msg4 = expect_refusal(d, planted_zero, real, _perturb_at_argmin,
                                     (lambda _p: 1.0), UMIN_PLANT,
                                     'blind reader under the argmin plant')
        if code4 != 2:
            refuse('probe (4): a BLIND reader was not refused under the argmin '
                   'plant -- the repair bought its pass by weakening the control')

        out = {'embedded_blob': got, 'n': len(r), 'base_umin_norm': base,
               'theorem_delta': expect,
               'argmin_row': ai, 'argmin_y_m': r[ai][0], 'row0_u': r[0][1],
               'parent_row0_delta': d_parent, 'parent_refused': parent_refused,
               'registered_delta': rec['delta_seen'],
               'threshold': rec['threshold'],
               'fixture_row0_passes': rec3['control'] == 'PASS',
               'blind_reader_refused': code4 == 2,
               'parent_refusal_verbatim': msg1.replace('\n', ' | '),
               'result': 'L-347 placement repair DRIVEN on the real R2 L1 bytes'}
        if not quiet:
            print('  L-347 PLACEMENT REPAIR DRIVEN on the real R2 L1 bytes: '
                  'min(u)/U_WALL %r at row %d (y = %r); the R2 row-0 plant %g lands '
                  'on u = %r at the apex and moves the min() reader by EXACTLY %r '
                  '(REFUSES); the registered argmin plant moves it by %r > '
                  'threshold %r (PASSES, = |plant|/U_WALL by the theorem); a fixture whose '
                  'argmin IS row 0 lets the parent pair PASS (the defect is '
                  'data-dependent); a blind reader is STILL refused'
                  % (base, ai, r[ai][0], UMIN_PLANT, r[0][1], d_parent,
                     rec['delta_seen'], rec['threshold']))
        return out
    finally:
        shutil.rmtree(d, ignore_errors=True)
...
def frozen_constants_control():
    ...
    for name, got, want in (('U_WALL', U_WALL, 2.0),
                            ...
                            ('GRADED_TRIPLE_LEVELS', GRADED_TRIPLE_LEVELS, ['L2', 'L3', 'L4']),
                            ('FIELDS', FIELDS, ['U', 'p']),
                            ('P_MIN', P_MIN, 0.05),
                            ('RMS_PLANT_K', RMS_PLANT_K, 4.0),
                            ('UMIN_PLANT', UMIN_PLANT, -abs(PLANT) * 100),
                            ('QUIESCENT_Y', QUIESCENT_Y, -2.0),
                            ('U_MIN_PHYS_LO', U_MIN_PHYS_LO, -1.5),
                            ('U_MIN_PHYS_HI', U_MIN_PHYS_HI, 1.5),
                            ('RMS_PHYS_HI', RMS_PHYS_HI, 2.0)):
        if got != want:
    ...
    return {'checked': 18, 'reference_blob': got_blob, 'n_bench': len(by),
...
def selftest():
    ...
    def chk(name, cond):
        ...
        if cond:
            ...
            print('  [FAIL] ' + name)
    print('--- grade_vmfl011_r3.py --selftest')
    ...
    # ---- THE L-347 PLACEMENT CONTROL, on the REAL R2 L1 bytes
    u = umin_placement_control(quiet=True)
    chk('L-347 embedded R2 L1 bisector is the registered blob',
        u['embedded_blob'] == REAL_XY_L1_R2_BLOB)
    chk('L-347 probe 1: the R2 row-0 pair STILL REFUSES on the real R2 bytes',
        u['parent_refused'] is True)
    chk('L-347 probe 1: and it moved the min() reader by EXACTLY 0, as row #31 recorded',
        u['parent_row0_delta'] == 0.0)
    chk('L-347 probe 2: the REGISTERED argmin pair PASSES on the same bytes',
        u['registered_delta'] > u['threshold'])
    chk('L-347 probe 2: and its move equals |plant|/U_WALL EXACTLY (the theorem, not a fit)',
        abs(u['registered_delta'] - abs(UMIN_PLANT) / U_WALL) <= 1e-12)
    chk('L-347 probe 3: the defect is DATA-DEPENDENT (row-0 plant passes when argmin IS row 0)',
        u['fixture_row0_passes'] is True)
    chk('L-347 probe 4: a BLIND reader is still REFUSED under the argmin plant',
        u['blind_reader_refused'] is True)
    chk('L-347: the parent planted into row 0, where u == 0 at the apex, while the '
        'reader selects a different row',
        u['row0_u'] == 0.0 and u['argmin_row'] != 0)

    # ---- L-347 clause 3: ALL channel controls run and ALL are reported before exit
    # Driven by RECONSTRUCTING THE R2 FAILURE: the u_min plant is put back where
    # the parent put it (row 0) for the duration of this probe, on the REAL R2 L1
    # bytes, so the u_min control fails exactly as it did in register row #31 while
    # the rms control passes.  The parent exited inside the FIRST dict entry and
    # said nothing about the second; this file must name BOTH.
    d = tempfile.mkdtemp(prefix='st4_')
    saved = globals()['_perturb_at_argmin']
    try:
        lv = os.path.join(d, 'L9')
        os.makedirs(os.path.join(lv, 'postProcessing', 'bisector', str(ENDTIME)))
        _write_real_r2_xy(os.path.join(lv, 'postProcessing', 'bisector',
                                       str(ENDTIME), 'bisect_U.xy'))
        okc = controls(lv)
        chk('L-347 clause 3: with the REGISTERED placement both channel controls PASS '
            'on the real R2 L1 bytes',
            all(v['control'] == 'PASS' for v in okc.values()) and len(okc) == 2)
        globals()['_perturb_at_argmin'] = _perturb        # reconstruct the R2 defect
        codec, msgc = expect_refusal(d, controls, lv)
        chk('L-347 clause 3: a failing channel control REFUSES (exit 2)', codec == 2)
        chk('L-347 clause 3: the refusal names BOTH channels, not just the first',
            ('rms_vs_benchmark' in msgc) and ('u_min_norm' in msgc))
        chk('L-347 clause 3: the PASSING channel is reported too, not swallowed',
            '[control PASS] rms_vs_benchmark' in msgc)
        chk('L-347 clause 3: the FAILING channel is named as such',
            '[control FAIL] planted-zero control FAILED for u_min_norm' in msgc)
        chk('L-347 clause 3: the refusal says ALL controls were run and reported',
            'ALL controls were run and ALL are reported' in msgc)
        # and the harder direction: BOTH channels blinded at once (the L-340 defect
        # AND the L-347 defect together).  A comparator that exits inside the FIRST
        # failing control would name only rms_vs_benchmark and say nothing about
        # u_min_norm -- which is exactly the shape that hid this defect for two runs.
        savedall = globals()['_perturb_all']
        try:
            globals()['_perturb_all'] = _perturb
            code2, msg2 = expect_refusal(d, controls, lv)
            chk('L-347 clause 3: with BOTH channels blinded the refusal REFUSES (exit 2)',
                code2 == 2)
            chk('L-347 clause 3: with BOTH channels blinded BOTH are reported FAIL, so '
                'no control hides behind another',
                ('[control FAIL] planted-zero control FAILED for rms_vs_benchmark' in msg2)
                and ('[control FAIL] planted-zero control FAILED for u_min_norm' in msg2))
            chk('L-347 clause 3: and it says 2 of 2 controls failed',
                '2 of 2 rule-3 channel control(s) FAILED' in msg2)
        finally:
            globals()['_perturb_all'] = savedall
    finally:
        globals()['_perturb_at_argmin'] = saved
        shutil.rmtree(d, ignore_errors=True)
    ...
    # ---- R4 change (A): the GRADED triple is FIXED as the FINEST THREE (no drop, no search)
    chk('R4 change A: GRADED_TRIPLE_LEVELS is the FINEST THREE levels (L-500: no drop)',
        list(GRADED_TRIPLE_LEVELS) == ['L2', 'L3', 'L4'])
    chk('R4 change A: GRADED_TRIPLE_LEVELS is LEVELS[-3:], derived not hand-typed',
        list(GRADED_TRIPLE_LEVELS) == list(LEVELS[-3:]))
    chk('R4 change A: the band is read at the finest level, which is now L4',
        LEVELS[-1] == 'L4')
    # 4-point settling diagnostic: reports BOTH overlapping triples, moves no verdict.
    d4 = four_point_order([-0.264494, -0.319438, -0.337560, -0.343000])
    chk('R4 change A: four_point_order reports BOTH overlapping triples (L1L2L3 and L2L3L4)',
        ('triple_L1L2L3' in d4) and ('triple_L2L3L4' in d4))
    chk('R4 change A: the settling diagnostic is DIAGNOSTIC ONLY (moves no gate quantity)',
        'DIAGNOSTIC ONLY' in d4['note'] and 'moves no gate quantity' in d4['note'])

    # ---- R4 change (C): the digitisation-floor characterisation is VALUE-BLIND (off gate)
    dd = digitisation_floor_diagnostic()
    chk('R4 change C: digitisation floor reports per-point reading-error rms floors',
        isinstance(dd['reading_error_rms_floors'], dict) and len(dd['reading_error_rms_floors']) > 0)
    chk('R4 change C: it reports an internal quiescent-region lower bound from the referent bytes',
        dd['internal_quiescent_rms_lower_bound'] is not None and dd['full_scale_span_u_norm'] > 0.0)
    chk('R4 change C: the floors are computed from the referent span alone, NOT from the band 0.030',
        abs(dd['reading_error_rms_floors']['1.0%FS'] - 0.01 * dd['full_scale_span_u_norm']) <= 1e-15)
    chk('R4 change C: it CHARACTERISES only and cannot widen the gate (L-487)',
        'cannot widen the gate' in dd['note'])

    # ---- R4 change (D): the GATE-BLIND physical-range guard -- PRESENT and KNOWN-BAD arms (L-487)
    good = {'rms_vs_benchmark': 0.0341, 'u_min_norm': -0.3376, 'y_at_u_min_m': -1.0}
    codep, _ = expect_refusal('__NOSCRATCH__', physical_range_refusal, good, 'Lp')
    chk('R4 change D: PRESENT arm -- a PHYSICAL (rms, u_min) read PASSES the range guard (no exit)',
        codep is None)
    codeb1, msgb1 = expect_refusal('__NOSCRATCH__', physical_range_refusal,
                                   {'rms_vs_benchmark': float('nan'), 'u_min_norm': -0.34}, 'Lb')
    chk('R4 change D: KNOWN-BAD arm -- a NaN rms REFUSES (exit 2)',
        codeb1 == 2 and 'not a physical RMS' in msgb1)
    codeb2, msgb2 = expect_refusal('__NOSCRATCH__', physical_range_refusal,
                                   {'rms_vs_benchmark': RMS_PHYS_HI + 1.0, 'u_min_norm': -0.34}, 'Lb')
    chk('R4 change D: KNOWN-BAD arm -- an rms above the physical bound REFUSES (exit 2)',
        codeb2 == 2 and 'not a physical RMS' in msgb2)
    codeb3, msgb3 = expect_refusal('__NOSCRATCH__', physical_range_refusal,
                                   {'rms_vs_benchmark': 0.034, 'u_min_norm': U_MIN_PHYS_HI + 1.0}, 'Lb')
    chk('R4 change D: KNOWN-BAD arm -- a u_min outside [%g, %g] REFUSES (exit 2)'
        % (U_MIN_PHYS_LO, U_MIN_PHYS_HI),
        codeb3 == 2 and 'outside the physical bound' in msgb3)
    chk('R4 change D: the physical-range guard is GATE-BLIND (references neither 0.030 nor -0.318062)',
        ('gate-blind' in msgb1) and ('gate-blind' in msgb3))
...
def main():
    ...
    pre = {'frozen_constants': frozen_constants_control(),
           ...
           'umin_placement_L347': umin_placement_control()}
    R = {'case': 'VMFL011-R4', 'manual_page': 41, 'endTime': ENDTIME,
         'supersedes': 'register row #36 (VMFL011-R3, GATE FAIL -- rms 0.034088 at the '
                       'finest level L3 vs band <= 0.030, on a CONVERGING u_min triple, '
                       'with the rms sequence STILL FALLING with refinement). A NEW row; '
                       'the R3 row is never overwritten (ANSYS_VERIFICATION_CHARTER section 6).',
         'parent_comparator_blob': '3975d9ee3a60bde8b2c1537c27abb1431662d65b',
         ...
         'graded_triple_levels': list(GRADED_TRIPLE_LEVELS),
         'digitisation_floor': digitisation_floor_diagnostic(),
    ...
    umins = {}                          # u_min_norm per graded level, keyed by level
    for L in LEVELS:
        ...
        physical_range_refusal(c, L)         # gate-blind guard (change D); exits 2 on a corrupt read
        e['state'] = 'COMPLETE'
        if not ic:
            R['blocking'].append('%s: NOT iteratively converged (rule 5 step 1): %s' % (L, msg))
        umins[L] = c['u_min_norm']
        R['levels'][L] = e
    # 4-POINT SETTLING DIAGNOSTIC (change A) -- OFF GATE: reported whenever every
    # level graded, moves no verdict, reads no band.
    if all(L in umins for L in LEVELS):
        R['settling_diagnostic'] = four_point_order([umins[L] for L in LEVELS])
    # THE GRADED ROACHE TRIPLE IS FIXED A-PRIORI as the FINEST THREE (rule 5;
    # L-500/L-501: never drop the finest/failing level to rescue a non-asymptotic
    # triple, and never search for the triple that passes).  If any of the finest
    # three is missing the triple is INCOMPLETE and the row is NOT A RESULT -- there
    # is no fallback to a coarser triple.
    if all(L in umins for L in GRADED_TRIPLE_LEVELS):
        R['triple'] = roache(*[umins[L] for L in GRADED_TRIPLE_LEVELS])
        R['triple_on'] = 'u_min_norm'
    else:
        got = [L for L in GRADED_TRIPLE_LEVELS if L in umins]
        R['blocking'].append('the GRADED grid triple (finest %s) is INCOMPLETE -- %d of %d graded; '
                             'NO fallback to a coarser triple (L-500)'
                             % (tuple(GRADED_TRIPLE_LEVELS), len(got), len(GRADED_TRIPLE_LEVELS)))
    if R['blocking']:
    ...
    out = a.out or os.path.join(a.run_root, 'GRADING_VMFL011_R4.json')
