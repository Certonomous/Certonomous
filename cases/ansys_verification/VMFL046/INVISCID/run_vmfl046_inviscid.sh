#!/usr/bin/env bash
# =============================================================================
# VMFL046-INVISCID driver -- the Euler/inviscid arm of the CD-nozzle shock (§24.7).
# Ansys Fluid Dynamics Verification Manual, Release 2026 R1, p.155.
# Solver: rhoSimpleFoam (OpenFOAM v2606), steady compressible, INVISCID (mu=0),
# slip walls, 2-D half-nozzle. Controlled experiment: the frozen viscous VMFL046
# case with EXACTLY two changes (mu 1.7894e-05 -> 0.0; wall noSlip -> slip).
#
# Frozen pre-registration: cases/ansys_verification/VMFL046/INVISCID/PREREGISTRATION.md
# Frozen comparator      : cases/ansys_verification/VMFL046/grade_vmfl046.py  (REUSED unchanged,
#                          blob cbe98dc8...; same solver/log/fields/reference)
# Frozen reference        : cases/ansys_verification/VMFL046/quasi1d_reference.py  (inviscid, shock 1.250)
# Run root (cwd)         : verification/runs/ansys_verification/VMFL046_INVISCID/  (never the case dir)
#
# Modelled on run_vmfl046.sh. The driver SOURCES its own OpenFOAM env and asserts its
...
RANKS=1
CAP_CORE_MIN=84          # RUNNING TOTAL across all three solves (~3x the 28 core-min estimate, §26.2)
...
RUN_ROOT="${1:?usage: run_vmfl046_inviscid.sh <run_root> [levels...]}"
shift 2>/dev/null
...
echo "=== VMFL046-INVISCID launcher  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)  run_root=$RUN_ROOT  smoke=$SMOKE"
...
if [ "$SMOKE" != "1" ]; then
  ...
  while IFS= read -r f; do
    rel="cases/ansys_verification/VMFL046/INVISCID/case/${f#$CASE_DIR/}"
...
echo "=== VMFL046-INVISCID launcher done  core_min_used=$CORE_MIN_USED  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
