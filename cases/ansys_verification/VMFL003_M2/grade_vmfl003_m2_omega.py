...
COMPARATOR (omega family: kOmegaSST).  FIELD-NAME-ONLY variant of
grade_vmfl003_m2.py: epsilon -> omega, no grading-logic change.  Gate, band,
reference, plants, completion clauses and verdict order are byte-identical.
...
def grade(runroot, write_json=None):
    res = {"case": "VMFL003_M2_D_kOmegaSST", "runroot": runroot,
...
def selftest():
    ...
    print("VMFL003-M2-omega comparator --selftest: %d checks, %d failures"
          % (checks[0], len(fails)))
    for f in fails:
        print("  FAILED: " + f)
...
def verify_frozen(commit):
    ...
    mine.update(data)
    rel = "cases/ansys_verification/VMFL003_M2/grade_vmfl003_m2_omega.py"
    try:
