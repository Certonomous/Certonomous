#!/usr/bin/env bash
# VMFL004 graded-run driver -- Plain Couette Flow with Pressure Gradient (manual p.21), 2D cyclic
#
...
#
# Usage:  run_vmfl004.sh <run_root>            graded run
#         run_vmfl004.sh <scratch_root> smoke  exercises THIS LAUNCHER end-to-end
...
RUN_ROOT="${1:?usage: run_vmfl004.sh <run_root> [smoke]}"
MODE="${2:-graded}"
...
PREREG_REL="cases/ansys_verification/VMFL004/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL004/grade_vmfl004.py"
...
echo "VMFL004 $MODE complete (${SPENT}/${CAP_CORE_MIN} core-min) -> grade with grade_vmfl004.py --run-root $RUN_ROOT"
