#!/usr/bin/env bash
# =============================================================================
# VMFL054-R2 graded-run driver -- Laminar flow in a Trapezoidal Driven Cavity.
...
#
# WHY R2 (the ONLY change from R1): R1's frozen driver (freeze 05ec949e) did NOT
# source the OpenFOAM environment. The queue daemon runs the driver in a plain shell
# where FOAM_APPBIN is not on PATH, so `blockMesh` was `command not found` (rc 127)
# and R1 aborted at L1 at zero physics compute (2026-09-02T21:43:57Z; launcher_rc=10;
# the frozen comparator correctly REFUSED, exit 2, "RUN_RC absent"). This is the same
# CLASS as VMFL064 attempt-1 (a launcher defect that refused at zero compute), and the
# team's reference drivers VMFL063:177-178 / VMFL064-R2:158-159 both source the bashrc
# and assert the solver on PATH -- R1 dropped exactly those two lines. R2 restores them.
# NOTHING ELSE CHANGES: the comparator (grade_vmfl054_r2.py), the case inputs, the gate
# constants and both rulings are byte-identical to R1's frozen package.
#
...
RUN_ROOT="${1:?usage: run_vmfl054_r2.sh <run_root> [levels...]}"
shift 2>/dev/null
LEVELS_TO_RUN="${*:-L1 L2 L3}"
SMOKE="${VMFL054_R2_SMOKE:-0}"

declare -A NX=( [L1]=40 [L2]=80  [L3]=160 )
declare -A NY=( [L1]=40 [L2]=80  [L3]=160 )

echo "=== VMFL054-R2 launcher  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)  run_root=$RUN_ROOT  smoke=$SMOKE"

# --- OpenFOAM environment (THE R2 FIX): source it and assert the solver is on PATH,
#     UNCONDITIONALLY, so the driver works under the bare daemon shell and under a
#     manual smoke alike. This is the pair R1 was missing. ------------------------
...
#     to its blob at HEAD. (Caveat carried from R1's report: this pins to HEAD, not to the
...
if [ "$SMOKE" != "1" ]; then
  ...
  while IFS= read -r f; do
    rel="cases/ansys_verification/VMFL054-R2/case/${f#$CASE_DIR/}"
...
echo "=== VMFL054-R2 launcher done  core_min_used=$CORE_MIN_USED  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
