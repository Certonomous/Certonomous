...
def check_pressure_based_config(level_dir, level):
    ...
    mn = re.search(r"^(?:(?!//).)*?(?<![A-Za-z0-9_])min\s+([0-9.eE+\-]+)\s*;", otxt, re.M)
    mx = re.search(r"^(?:(?!//).)*?(?<![A-Za-z0-9_])max\s+([0-9.eE+\-]+)\s*;", otxt, re.M)
    act = re.search(r"^(?:(?!//).)*?(?<![A-Za-z0-9_])active\s+(\w+)\s*;", otxt, re.M)
    if not (mn and mx):
