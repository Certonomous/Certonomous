#!/usr/bin/env python3
"""VMFLGPU001 -- PARSER FIX, DRAFT ONLY.  NOT APPLIED.  NOT COMMITTED.  NOT A GRADER.

=====================================================================
  DRAFT.  This file grades NOTHING and is on NO grading path.
  It does not import, modify, wrap or monkey-patch grade_vmflgpu001.py.
  grade_vmflgpu001.py is UNTOUCHED by this draft; its blob is 21fa23871de61e04d344ea04e5b1f2ab1da6c39c.
=====================================================================

WHAT IS DEFECTIVE IN THE FROZEN GRADER
--------------------------------------
grade_vmflgpu001.py parses every `key = value` artifact with
...
-------
  ...
  * an EXPLICIT REFUSAL (never a bare `assert`; `python3 -O` strips those,
    ...
    call site asserts it.  There are THREE call sites in grade_vmflgpu001.py:
...
Nothing any verdict depends on.  Measured, per real file, in selftest():
  * every key READ by grade_vmflgpu001.py keeps a BYTE-IDENTICAL value;
  * new keys appear in the LAUNCH_RECORD dict (`smoke_rc`, `end`, `note`,
    `arm`, `level`, `wall_s`, `cap_fired`, `utc`, `rc`) -- none of them is read
    by grade_vmflgpu001.py, and the grader lands only four named LAUNCH_RECORD keys
...
CASE = "VMFLGPU001"
FROZEN_GRADER = "grade_vmflgpu001.py"
FROZEN_BLOB = "21fa23871de61e04d344ea04e5b1f2ab1da6c39c"
RUN_ROOT_DEFAULT = "/home/ubuntu/Certonomous/verification/runs/ansys_verification/VMFLGPU001"
