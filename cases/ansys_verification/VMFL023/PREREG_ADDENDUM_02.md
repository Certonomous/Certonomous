# VMFL023 — PRE-REGISTRATION ADDENDUM 02 — A COMPARATOR REPAIR UNDER §2d.1
...
blob at `754f4f66decd10709d52e1210bfbb4e682b0cbd7`. **No gate, threshold, band, reference
value, cap, window, level list or label moved.**

## THE DEFECT

`run_vmfl023.sh` writes each level's exit record as
`rc = 0` — **with spaces around the `=`**. `grade_vmfl023.py` parsed
that file with
...
this case's eventual `RESULTS.md`.

## OPERATIONAL CONSEQUENCE, STATED SO IT IS NOT A SURPRISE

`grade_vmfl023.py` on disk **no longer hashes equal to its blob at
`754f4f66decd10709d52e1210bfbb4e682b0cbd7`**. That is correct and intended: any
