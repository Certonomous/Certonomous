...
# R2 CHANGE (d): per-level endTime.  Run 1 showed L3 needs more than 3000 SIMPLE
# iterations to converge below 1e-6 (its final Ux initial residual was
# 1.19876e-06 at iteration 3000, plateau ptp 2.772e-05 m/s > 1e-6); L1 and L2
# were already at 1e-14 / 1e-12 at 3000.  N=6000 for L3 was fixed BEFORE this
# run from a log10(residual)-vs-iteration fit over run 1's last 1000 iterations
# (slope -9.6065e-04 decades/iter; extrapolated residual 1e-8 at ~5163 iter;
# rounded up to a multiple of 1000; predicted residual at 6000 ~1.57e-9, two
# decades of margin below 1e-6).  See R2/PREREGISTRATION.md section 4.1.
...
def _endtime_for(level_dir):
    ...
    if name not in ENDTIME_BY_LEVEL:
        refuse("no registered endTime for level %r (R2 CHANGE d)" % name)
    return ENDTIME_BY_LEVEL[name]

# R2 CHANGE (d): end of per-level endTime block
...
# R2 CHANGE (e): the run tree and grading JSON live under VMFL001/R2 -- this is
# a NEW rung citing run 1, not a re-grade of run 1's tree (charter section 6).
RUN_ROOT = "/home/ubuntu/Certonomous/verification/runs/ansys_verification/VMFL001/R2"
OUT_JSON = os.path.join(RUN_ROOT, "GRADING_VMFL001_R2.json")
...
# ---------------------------------------------------------------------------
def read_raw_set(path, cols=None):
    """Parse an OpenFOAM sampled coordSet file.  Returns (columns, rows).

    R2 CHANGE (b): v2606's `sets` writer emits the file HEADERLESS, named
    <setName>_<fields alphabetical>.xy (run 1 REFUSED because the R1 reader
    required a `#` column header that this writer never writes).  When `cols`
    is supplied -- by find_set_file, derived from the filename's field list
    (R2 CHANGE a) -- a headerless file is accepted and those columns are used.
    A file whose data rows disagree in width with the layout is still REFUSED,
    never guessed; if no layout is supplied and there is no `#` header, it is
    also REFUSED.
    """
    ...
    with open(path) as fh:
        for line in fh:
            ...
            if s.startswith("#"):
                toks = s.lstrip("#").split()
                if cols is None and len(toks) >= 4:   # R2 CHANGE (b): header still honoured if present
                    cols = toks
                continue
    ...
    if cols is None:
        refuse("no column layout for %s -- neither a filename-derived layout "
               "(R2 CHANGE a) nor a '#' header; this comparator does not guess"
               % path)
    if not rows:
        refuse("no data rows in " + path)
    for r in rows:
        if len(r) != len(cols):
            refuse("layout has %d columns but a data row has %d in %s (R2 CHANGE b)"
                   % (len(cols), len(r), path))
    return cols, rows
...
def vtheta_from_raw(path, cols):
    """Return [(r, theta, v_theta), ...] from a sampled U file.
    ...
    R2 CHANGE (a): the column layout `cols` is supplied by find_set_file, derived
    from the (headerless) filename, and passed to the reader.
    """
    cols, rows = read_raw_set(path, cols)   # R2 CHANGE (a): layout from the filename
...
# R2 CHANGE (a): v2606 writes postProcessing/<fo>/<time>/<setName>_<fields
# alphabetical>.<ext>, HEADERLESS (verified against run 1: gateAxis_p_U.xy, seven
# columns).  The column layout is DERIVED from the filename: coords x y z, then
# each field after the set name in the order written, scalar 1 column, vector 3
# (p -> 1, U -> 3), so gateAxis_p_U.xy -> x y z p U_x U_y U_z.  It REFUSES if the
# field list is not exactly {p, U} in some order, and read_raw_set REFUSES if a
# row width != 3 + sum of field widths.  This is the reader run 1 taught us to
# write: it never guesses a column.
FIELD_WIDTHS = {"p": 1, "U": 3}


def _layout_from_filename(fname, set_name):
    base = os.path.basename(fname)
    stem = base.rsplit(".", 1)[0]                  # drop the .xy / .raw extension
    prefix = set_name + "_"
    if not stem.startswith(prefix):
        refuse("sampled file %r does not begin with the set name %r (R2 CHANGE a)"
               % (base, prefix))
    fields = stem[len(prefix):].split("_")
    if set(fields) != {"p", "U"}:
        refuse("sampled file %r field list %r is not exactly {p, U} (R2 CHANGE a)"
               % (base, fields))
    cols = ["x", "y", "z"]
    for f in fields:
        if f == "U":
            ...
            cols += [f]                            # scalar: one column
    return cols


def find_set_file(level_dir, set_name, field="U", time=None):
    """postProcessing/<fo>/<time>/<setName>_<fields>.<ext> (v2606 naming).

    R2 CHANGE (a): returns (path, cols) -- exactly one hit is required, and the
    column layout is derived from the filename by _layout_from_filename.  The
    endTime is the per-level value (R2 CHANGE d).  `field` is retained for the
    signature but is no longer used to select the file (the set name does).
    """
    t = str(_endtime_for(level_dir)) if time is None else str(time)   # R2 CHANGE (d)
    pat = os.path.join(level_dir, "postProcessing", "*", t, "%s_*" % set_name)
    hits = sorted(glob.glob(pat))
    if len(hits) > 1:
        refuse("ambiguous sampled files for set %r in %s: %s"
               % (set_name, level_dir, ", ".join(os.path.basename(h) for h in hits)))
    if len(hits) != 1:
        refuse("no sampled file for set %r at time %s under %s/postProcessing "
               "(expected one %s_*.xy)" % (set_name, t, level_dir, set_name))
    cols = _layout_from_filename(hits[0], set_name)
    return hits[0], cols


def gate_values(level_dir):
    """The four gate radii -> v_theta, from the frozen probe procedure."""
    path, cols = find_set_file(level_dir, SET_NAME)   # R2 CHANGE (a): (path, cols)
    got = vtheta_from_raw(path, cols)                  # R2 CHANGE (a)
    out = {}
...
def plant_into_raw(path, r_target, plant, cols):
    ...
    R2 CHANGE (c): the layout `cols` is supplied by find_set_file (the file is
    headerless), and the plant goes into the DERIVED U_y column index -- for
    gateAxis_p_U.xy that is index 5 of x y z p U_x U_y U_z.  A row whose width
    disagrees with the layout is skipped, never mis-indexed.
    """
    ix, iy, j = cols.index("x"), cols.index("y"), cols.index("U_y")   # R2 CHANGE (c): derived indices
    ...
    for i, line in enumerate(lines):
        ...
        if abs(math.hypot(vals[ix], vals[iy]) - r_target) < 1e-6:
            before = vals[j]
            vals[j] = before + plant
            ...
            for l2 in open(path).read().split("\n"):
                ...
                if len(v2) != len(cols):   # R2 CHANGE (c): width guard on re-read, derived indices
                    continue
                if abs(math.hypot(v2[ix], v2[iy]) - r_target) < 1e-6:
                    back = v2[j]
                    break
            if back is None:
...
def planted_zero_control(level_dir):
    ...
    src, cols = find_set_file(level_dir, SET_NAME)   # R2 CHANGE (a): (path, cols)
    ...
    try:
        ...
        before, after = plant_into_raw(work, TRIPLE_RADIUS, PLANT, cols)   # R2 CHANGE (c): layout supplied
        got = vtheta_from_raw(work, cols)            # R2 CHANGE (a)
...
def strict_completion(level_dir):
    ...
    endtime = _endtime_for(level_dir)   # R2 CHANGE (d): this level's registered endTime
    ...
    if not times or float(times[-1]) != float(endtime):   # R2 CHANGE (d): per-level endTime
    ...
    if n_exec != endtime:                                 # R2 CHANGE (d): per-level endTime
...
def azimuthal_spread(level_dir):
    ...
    try:
        path, cols = find_set_file(level_dir, DIAG_SET_NAME)   # R2 CHANGE (a): (path, cols)
    except SystemExit:
        return "unavailable"
    got = vtheta_from_raw(path, cols)                          # R2 CHANGE (a)
    out = {}
...
def selftest():
    ...
    try:
        # R2 CHANGE (f): the reader fixture is written in the ACTUAL v2606 writer
        # format -- HEADERLESS, named <setName>_<fields alphabetical>.xy, columns
        # x y z p U_x U_y U_z (run 1's real files look exactly like this).  On the
        # +x axis v_theta == U_y, so U_y carries v_exact and U_x = U_z = 0.
        print("--- selftest: raw reader on the REAL v2606 format (headerless gateAxis_p_U.xy) ---")
        path = os.path.join(tmp, "gateAxis_p_U.xy")
        with open(path, "w") as fh:
            for r in RADII:
                fh.write("%r 0 0.0025 0 0 %r 0\n" % (r, v_exact(r)))
        cols = _layout_from_filename(path, "gateAxis")   # R2 CHANGE (a): layout from filename
        check("filename layout is x y z p U_x U_y U_z",
              cols == ["x", "y", "z", "p", "U_x", "U_y", "U_z"], cols)
        got = dict((round(a, 6), c) for a, b, c in vtheta_from_raw(path, cols))
        check("reader recovers v_theta at all four radii from a headerless file",
              all(abs(got[round(r, 6)] - v_exact(r)) < 1e-15 for r in RADII))

        # R2 CHANGE (f): headerless is now ACCEPTED when the layout comes from the
        # filename; what must still be REFUSED is a row whose width disagrees with
        # the layout, and a filename whose field list is not {p, U}.
        print("--- selftest: reader refuses a width mismatch and a bad field list ---")
        ...
        wbad = os.path.join(tmp, "width_p_U.xy")
        open(wbad, "w").write("0.02 0 0.0025 0 0 0.015\n")   # 6 cols, layout wants 7
        check("a data row narrower than the layout is REFUSED with exit 2",
              refuses(lambda: read_raw_set(wbad, ["x", "y", "z", "p", "U_x", "U_y", "U_z"])))
        check("a filename whose fields are not {p, U} is REFUSED with exit 2",
              refuses(lambda: _layout_from_filename("gateAxis_U.xy", "gateAxis")))

        print("--- selftest: planted-zero, positive arm ---")
        work = os.path.join(tmp, "plant_p_U.xy")
        shutil.copy(path, work)
        before_v = dict((round(a, 6), c) for a, b, c in vtheta_from_raw(work, cols))
        b, a = plant_into_raw(work, TRIPLE_RADIUS, PLANT, cols)   # R2 CHANGE (c): layout supplied
        after_v = dict((round(a2, 6), c) for a2, b2, c in vtheta_from_raw(work, cols))
        delta = after_v[round(TRIPLE_RADIUS, 6)] - before_v[round(TRIPLE_RADIUS, 6)]
        check("the reader sees the planted %g m/s in the U_y column" % PLANT,
        ...
        work2 = os.path.join(tmp, "noplant_p_U.xy")
        shutil.copy(path, work2)
        v2 = dict((round(a2, 6), c) for a2, b2, c in vtheta_from_raw(work2, cols))
...
# ---------------------------------------------------------------------------
# R2 CHANGE (f): --dryrun-reader -- run the reader on a REAL run file and print
# ONLY whether it parsed and the row count, NEVER a value.  This is the
# pre-freeze external-reader check that run 1 taught us to do: the R1 comparator
# had never seen a real v2606 sampled file, so its naming/format assumption went
# undetected until it refused at grade time.  The set name is taken from the
# filename (text before the first "_"); the layout is derived by
# _layout_from_filename; and the reader is read_raw_set.
# ---------------------------------------------------------------------------
def dryrun_reader(path):
    set_name = os.path.basename(path).split("_")[0]
    cols = _layout_from_filename(path, set_name)
    _, rows = read_raw_set(path, cols)
...
if __name__ == "__main__":
    if "--selftest" in sys.argv:
        sys.exit(selftest())
    if "--dryrun-reader" in sys.argv:                 # R2 CHANGE (f)
