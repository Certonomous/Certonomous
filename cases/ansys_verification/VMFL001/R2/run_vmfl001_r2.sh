#!/usr/bin/env bash
# VMFL001-R2 -- flow between rotating and stationary concentric cylinders.
# Builds and runs the three-level Roache family.  It does NOT grade anything;
# grading is grade_vmfl001_r2.py, frozen separately and cited by sha in
# R2/PREREGISTRATION.md.
#
# R2 (re-run after run 1 = NOT A RESULT): reader matched to v2606's headerless
# <set>_p_U.xy writer, and a PER-LEVEL endTime (L3 needs 6000 iterations, not
# 3000, to converge below 1e-6).  The endTime is substituted per level into
# system/controlDict from controlDict.template (__ENDTIME__ placeholder).
#
# GUARDS, in order:
...
CASE_SRC="/home/ubuntu/Certonomous/cases/ansys_verification/VMFL001/R2/case"
RUN_ROOT="/home/ubuntu/Certonomous/verification/runs/ansys_verification/VMFL001/R2"
PREREG="cases/ansys_verification/VMFL001/R2/PREREGISTRATION.md"
CAP_CORE_MIN=10          # pre-registered cap (R2 estimate 3.72 core-min x 2.5, rounded up); overrun stops the run
RANKS=1                  # serial: core-minutes = wall_s * RANKS / 60
# name:nr:naz_per_block:endTime -- per-level endTime (R2: L3 = 6000, not 3000)
...
for spec in $LEVELS; do
  IFS=: read -r name NR NAZB ETIME <<< "$spec"   # R2: 4th field is this level's endTime
  d="${RUN_ROOT}/${name}"
  echo "=== ${name}: nr=${NR}, azimuthal per block=${NAZB} (total azimuthal $((4*NAZB))), endTime=${ETIME} ==="
  ...
  # R2: substitute this level's endTime into controlDict (endTime + writeInterval)
...
echo "grade with: python3 cases/ansys_verification/VMFL001/R2/grade_vmfl001_r2.py"
