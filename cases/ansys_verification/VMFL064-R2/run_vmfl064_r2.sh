#!/usr/bin/env bash
# =============================================================================
# VMFL064-R2 graded-run driver -- Low Reynolds Number Flow in a Channel with
# Sudden Asymmetric Expansion (backward-facing step, Armaly LR/s = 5.0).
# Ansys Fluid Dynamics Verification Manual, Release 2026 R1, p.195/196.
# Solver: simpleFoam (OpenFOAM v2606), steady laminar SIMPLEC, Re_D = 200, 2-D.
#
# Frozen pre-registration: cases/ansys_verification/VMFL064-R2/PREREGISTRATION.md
# Frozen comparator      : cases/ansys_verification/VMFL064-R2/grade_vmfl064_r2.py
# Run root               : verification/runs/ansys_verification/VMFL064-R2/
#
# THIS FILE IS MODELLED LINE FOR LINE ON
#   cases/ansys_verification/VMFL017/R2/run_vmfl017_r2.sh
# (the team's reference R2 driver), with the VMFL064 case specifics substituted.
#
# ---------------------------------------------------------------------------
...
#   2. cap enforcement in the executable path, timeout_s = remaining*60/RANKS  -> below
#   3. no `set -u`, reason named                                              -> above
#   4. planted-zero control (comparator --selftest, before a core-minute)      -> below
#   5. mesh birth certificate (checkMesh, minted per level)                    -> below
#   6. a pre-flight smoke test that exercises THIS LAUNCHER                    -> VMFL_SMOKE
#
# THE CAP IS BYTE-IDENTICAL TO ATTEMPT 1 AND IS A RUNNING TOTAL, NOT A PER-LEVEL
# CAP. VMFL017-R2 registers PER-LEVEL caps; VMFL064 registered a 90 core-min
# RUNNING-TOTAL drawdown (PREREGISTRATION.md line 10, blob
# 7b9fd0dcaa141cacf4e0f3dee5a6ceab0b9f48b5) and the R2 re-registration keeps the
# cap byte-identical, so the DRAWDOWN semantics come from attempt 1, not from the
# VMFL017-R2 template. Deliberate, registered, and named here so it cannot read as
...
RANKS=1
CAP_CORE_MIN=90          # running TOTAL across all three levels -- frozen, byte-identical
ENDTIME=20000            # byte-identical to attempt 1

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CASE_DIR="$SCRIPT_DIR/case"
RUN_ROOT="${1:?usage: run_vmfl064_r2.sh <run_root> [levels...]}"
shift 2>/dev/null
LEVELS_TO_RUN="${*:-L1 L2 L3}"

# r = 2 grid triple: every level doubles NXU, NXD and NY (frozen prereg line 6).
declare -A NXU=( [L1]=64 [L2]=128 [L3]=256 )
declare -A NXD=( [L1]=64 [L2]=128 [L3]=256 )
declare -A NY=(  [L1]=16 [L2]=32  [L3]=64  )

echo "=== VMFL064-R2 launcher  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)  run_root=$RUN_ROOT"
...
PREREG_REL="cases/ansys_verification/VMFL064-R2/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL064-R2/grade_vmfl064_r2.py"
...
# --- 5. MESH FAMILY + CASE INPUTS, BYTE-IDENTICAL TO ATTEMPT 1 ----------------
# The R2 case directory is the attempt-1 case directory REUSED BYTE FOR BYTE
# (PREREGISTRATION.md "WHAT IS BYTE-IDENTICAL"). Every input file is hashed
# against the ATTEMPT-1 HEAD blob, so "the mesh family did not change" is a
# CHECK at launch, not a claim in a document. Its birth certificate is minted
# per level from checkMesh below (MESH_STANDARD sec.6).
for F in 0/U 0/p constant/momentumTransport constant/transportProperties \
  ...
  A="$(git -C "$REPO" rev-parse "HEAD:cases/ansys_verification/VMFL064/case/$F" 2>/dev/null)" || { echo "ABORT: no attempt-1 case/$F at HEAD"; exit 2; }
  B="$(git -C "$REPO" hash-object "$CASE_DIR/$F")" || { echo "ABORT: cannot hash R2 case/$F"; exit 2; }
  [ "$A" = "$B" ] || { echo "ABORT: case/$F is NOT the attempt-1 input ($B != $A) -- the R2 changes the READER, nothing else"; exit 2; }
done
echo "  case inputs byte-identical to the attempt-1 family (9 files, blockMeshDict.template included)"

# --- 4. PLANTED-ZERO + CORNER-VORTEX + P-FLOOR CONTROLS (rule 3) --------------
# The comparator's own --selftest DRIVES the planted-zero, the corner-vortex
# control that drives this R2's reader repair, and the observed-order floor. A
# comparator whose controls do not fire does not grade this run, so the launcher
# refuses BEFORE spending a core-minute. Checked under BOTH interpreters, because
# `python3 -O` deletes every assert and a control that exists only under one flag
# is not a control (L-332).
ST="/tmp/vmfl064r2_selftest.$$"
python3 "$SCRIPT_DIR/grade_vmfl064_r2.py" --selftest > "$ST" 2>&1 || { echo "ABORT: comparator --selftest is NOT green under python3; see $ST"; exit 2; }
python3 -O "$SCRIPT_DIR/grade_vmfl064_r2.py" --selftest > "$ST.O" 2>&1 || { echo "ABORT: comparator --selftest is NOT green under python3 -O; see $ST.O"; exit 2; }
...
grep -q 'planted-zero sees the plant' "$ST" || { echo "ABORT: comparator --selftest did not exercise the planted-zero control (rule 3)"; exit 2; }
grep -q 'attempt-1 logic REFUSES (exit 2) on the SAME bytes' "$ST" || { echo "ABORT: comparator --selftest did not DRIVE the corner-vortex repair control"; exit 2; }
grep -q 'NOT A RESULT, no GCI' "$ST" || { echo "ABORT: comparator --selftest did not exercise the observed-order floor (P_MIN)"; exit 2; }
NCHK="$(grep -c '^  \[PASS\]' "$ST")"
...
{ echo "case = VMFL064-R2"
  echo "supersedes = register row #29 (VMFL064, NOT A RESULT) -- a NEW row, the old one is never overwritten"
  ...
  echo "ranks = $RANKS"
  echo "cap_core_min = $CAP_CORE_MIN  (RUNNING TOTAL across levels, byte-identical to attempt 1 prereg line 10)"
...
for L in $LEVELS_TO_RUN; do
  ...
  grep -q '__NXU__\|__NXD__\|__NY__' "$OUT/system/blockMeshDict" && { echo "ABORT: blockMeshDict substitution did not take at $L"; exit 2; }
  ...
  # --- 5. MESH BIRTH CERTIFICATE, minted from this level's own checkMesh --------
...
PYBC
  ...
  printf 'level=%s\nstate=STARTED\nranks=%d\ncap_core_min=%s\nremaining_core_min=%s\ntimeout_s=%d\nendTime=%s\nprereg_blob=%s\ncomparator_blob=%s\nstarted_utc=%s\nnote=RUNNING-TOTAL drawdown (attempt-1 prereg line 10, byte-identical). This file is an L-342 INFRASTRUCTURE artifact: the comparator gates on the physics artefacts and reports rc NOT MEASURED if this file is absent.\n' \
  ...
  # that reaches RUN_RC.$L is the SOLVER's own status and not a status invented by
  # any later stage of the wrapper. The outer `wait` rc is recorded alongside it.
  rm -f "$OUT/.solver_rc"
  (
    cd "$OUT" || exit 90
  ...
  # The grader needs Cx, Cy at the converged time (the reattachment abscissae).
  # Written with postProcess writeCellCentres on the LATEST time, only on success.
  if [ "$RC" -eq 0 ]; then
  ...
  printf 'rc = %s\nlevel = %s\nstate = FINISHED\nwait_rc = %d\nwall_s = %d\nranks = %d\ncore_min = %s\ntotal_core_min_so_far = %s\ntimeout_s = %d\ncap_core_min = %s\nendTime = %s\nlast_time_in_log = %s\nEnd_lines = %s\nSIMPLE_converged_lines = %s\nlatest_time_dir = %s\nfield_at_latest_newer_than_0_U = %s\ndetach_wrapper_pid = %s\ndetach_wrapper_sid = %s\nprereg_blob = %s\ncomparator_blob = %s\nsolver_bin = %s\nutc = %s\nnote = rc is simpleFoam exit status under timeout, CAPTURED INSIDE the detached subshell; 124 means the RUNNING-TOTAL cap fired. endTime is NEVER reduced to fit a cap (rule 12). L-342: this file is INFRASTRUCTURE -- absent, the comparator reports rc NOT MEASURED and grades the physics artefacts.\n' \
    "$RC" "$L" "$WAIT_RC" "$WALL" "$RANKS" "$CORE_MIN" "$TOTAL_CORE_MIN" "$TIMEOUT_S" "$CAP_CORE_MIN" "$ENDTIME" "${LAST_TIME:-none}" "${ENDED:-0}" "${CONVERGED:-0}" "${ENDT_DIR:-none}" "$ENDT_OK" "$SOLVER_PID" "${WSID:-unknown}" "$PREREG_BLOB" "$GRADER_BLOB" "$SOLVER_BIN" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
...
echo "=== VMFL064-R2 launcher done, overall rc=$OVERALL_RC, total ${TOTAL_CORE_MIN} core-min of ${CAP_CORE_MIN}"
echo "grade with: python3 $SCRIPT_DIR/grade_vmfl064_r2.py --run-root $RUN_ROOT"
exit "$OVERALL_RC"
