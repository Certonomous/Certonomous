#!/usr/bin/env bash
# =============================================================================
# VMFL069-R2 graded-run driver -- Two Phase Poiseuille Flow.
...
#
# Frozen pre-registration: cases/ansys_verification/VMFL069-R2/PREREGISTRATION.md
# Frozen comparator      : cases/ansys_verification/VMFL069-R2/grade_vmfl069_r2.py
...
RANKS=1
CAP_CORE_MIN=5000        # RUNNING TOTAL across all three solves (PREREGISTRATION-R2 sec.7).
                         # LARGE because a Courant-limited transient-to-steady solve at
                         # maxCo=1 is inherently ~4e5 steps at L3; an overrun still STOPS
                         # the run (rc 124) and endTime is NEVER reduced to fit a cap.
ENDTIME=1000             # s; under adjustTimeStep yes the STEP COUNT is variable and does
                         # NOT equal this number (R1 was 2000 with deltaT 1 s == step count)

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CASE_DIR="$SCRIPT_DIR/case"
RUN_ROOT="${1:?usage: run_vmfl069_r2.sh <run_root> [levels...]}"
shift 2>/dev/null
...
echo "=== VMFL069-R2 launcher  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)  run_root=$RUN_ROOT"
...
PREREG_REL="cases/ansys_verification/VMFL069-R2/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL069-R2/grade_vmfl069_r2.py"
...
for F in 0/U 0/p_rgh 0/alpha.fluid1 \
  ...
  A="$(git -C "$REPO" rev-parse "HEAD:cases/ansys_verification/VMFL069-R2/case/$F" 2>/dev/null)" || { echo "ABORT: case/$F is not committed at HEAD"; exit 2; }
...
ST="/tmp/vmfl069_r2_selftest.$$"
rm -rf "$SCRIPT_DIR/__pycache__"
python3    "$SCRIPT_DIR/grade_vmfl069_r2.py" --selftest > "$ST"   2>&1; RC_PLAIN=$?
[ "$RC_PLAIN" = "0" ] || { echo "ABORT: comparator --selftest is NOT green under python3 (rc $RC_PLAIN); see $ST"; exit 2; }
rm -rf "$SCRIPT_DIR/__pycache__"
python3 -O "$SCRIPT_DIR/grade_vmfl069_r2.py" --selftest > "$ST.O" 2>&1; RC_O=$?
...
for MARK in \
  ...
  'completion REFUSES when the ExecutionTime count != the Time-line count' \
  'completion REFUSES when adjustTimeStep is not yes (R1'"'"'s fatal setting)' \
  'completion REFUSES when maxCo is not the registered value' \
...
{ echo "case = VMFL069-R2"
  echo "manual_page = 205"
  echo "supersedes = nothing -- R2 SUCCESSOR to VMFL069 R1 (NOT A RESULT, rc 136 SIGFPE); R2 does NOT supersede R1, it is a NEW registration with a NEW row"
...
echo "=== VMFL069-R2 launcher done, overall rc=$OVERALL_RC, total ${TOTAL_CORE_MIN} core-min of ${CAP_CORE_MIN}"
echo "grade with: python3 $SCRIPT_DIR/grade_vmfl069_r2.py --run-root $RUN_ROOT --out $RUN_ROOT/GRADING_VMFL069_R2.json"
exit "$OVERALL_RC"
