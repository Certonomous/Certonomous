#!/usr/bin/env python3
# =============================================================================
# VMFL069 COMPARATOR -- Two Phase Poiseuille Flow
...
# this case WRITES time directories 500, 1000, 1500, 2000, whose LEXICOGRAPHIC
# maximum is `500` and whose NUMERIC maximum is `2000` -- the 19-site hazard of
# this territory, live in this very case. `numeric_latest_time_dir()` sorts with
# key=float AND is cross-checked against the solver log's own last Time.
# =============================================================================
...
RUN_ROOT = os.path.join(HERE, "..", "..", "..",
                        "verification", "runs", "ansys_verification", "VMFL069")
...
ENDTIME       = 2000        # s, == the number of 1 s time steps
PLATEAU_TIME  = 1500        # s, the previous written time
DELTAT        = 1
...
def completion(run_root, level):
    ...
    PHYSICS-CRITICAL -- each gates, any failure REFUSES (exit 2):
      ...
      * last Time == endTime                              (literal rule 4)
      * ExecutionTime count == endTime                    (literal: deltaT = 1 s,
                                                           so #steps == endTime)
      ...
      * PLATEAU: the limb quantities at t = 1500 and t = 2000 agree (rule 5 limb 1)
    ...
    if not mE or not mD:
        raise SystemExit2("no endTime/deltaT in %s" % cdp)
    out["endTime"], out["deltaT"] = float(mE.group(1)), float(mD.group(1))
    if out["endTime"] != float(ENDTIME) or out["deltaT"] != float(DELTAT):
        raise SystemExit2("%s carries endTime %g deltaT %g; the registered pair is %g / %g"
                          % (cdp, out["endTime"], out["deltaT"], float(ENDTIME), float(DELTAT)))
    ...
    if out["n_exec"] != int(out["endTime"]):
        raise SystemExit2("%s: ExecutionTime count %d != endTime %d. deltaT is 1 s, so the "
                          "step count and the numeric endTime are the same number and rule 4's "
                          "clause holds literally." % (logp, out["n_exec"], int(out["endTime"])))
...
def _log_text(endtime, nexec=None, lasttime=None, end_line=True, fvopt=True):
    ...
    for k in range(1, n):
        L.append("Time = %g\n" % k)
        L.append("ExecutionTime = 0.01 s  ClockTime = 0 s\n")
    L.append("Time = %g\n" % last)
    L.append("ExecutionTime = 0.01 s  ClockTime = 0 s\n")
    if end_line:
        L.append("End\n")
...
def _write_level(root, level, err=0.0, endtime=ENDTIME, nexec=None, lasttime=None,
                 ...
                 plateau_shift=0.0, fields=FIELDS, times=(500, 1000, 1500, 2000),
                 age_ok=True, deltat=DELTAT, cells_override=None):
    ...
    gate to either side of its band without ever touching a real run."""
    ...
    try:
        fh.write(_log_text(endtime, nexec, lasttime, end_line, fvopt))
    finally:
        fh.close()
...
def selftest():
    print("SELFTEST VMFL069 -- Two Phase Poiseuille Flow (VM2026R1 p.205)")
    ...
    try:
        ...
        ck("completion() records that the LEXICOGRAPHIC latest dir would have misread",
           ...
           and c["numeric_latest_time_dir"] == "2000")
        ...
        ck("completion REFUSES when last Time != endTime", one_bad("shorttime", lasttime=1999))
        ck("completion REFUSES when the ExecutionTime count != endTime",
           one_bad("execcount", nexec=1999))
        ...
        ck("completion REFUSES when controlDict carries an unregistered endTime",
           one_bad("wrongend", endtime=1000, times=(500, 1000)))
...
def verify_frozen():
    ...
    rc = 0
    for rel in ("cases/ansys_verification/VMFL069/PREREGISTRATION.md",
                "cases/ansys_verification/VMFL069/grade_vmfl069.py"):
...
def main(argv):
    ...
    R = {"case": "VMFL069", "manual_page": 205,
    ...
    print("VMFL069  (VM2026R1 p.205)  ROW VERDICT: %s" % R["verdict"])
    for k in ("A", "B", "C"):
        L = R["limbs"][k]
        t = L["triple"]
