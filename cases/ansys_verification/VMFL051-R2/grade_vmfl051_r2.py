#!/usr/bin/env python3
"""VMFL051-R2 comparator -- isentropic expansion of supersonic flow over a
convex corner.  SUCCESSOR to VMFL051 run 1 (register #4, NOT A RESULT).
...
cites it by blob sha, and before any GRADED solver runs (CLAUDE.md rule 2;
VERIFICATION_CHARTER section 2d).  Nothing in it may change once the first graded
R2 solve has started; instrumentation not on the grading path may be added later
...
======================================================================
WHAT CHANGED FROM VMFL051 run 1, AND WHY (the ONE lever)
======================================================================
Run 1 graded the post-expansion Mach number as a SINGLE-endTime SNAPSHOT of the
volume-average of `Ma` over the frozen cellZone `gateZone`.  Its Roache triple
came out OSCILLATORY (3.2279 / 3.2233 / 3.2294, R = -1.3486) and two of three
levels failed the snapshot plateau clause.

An ANSWER-BLIND L1 smoke (documented in PREREGISTRATION.md section 3; the graded
Mach value was NEVER read -- only peak-to-peak and mean-DIFFERENCES) established
the mechanism:
  * the coarse-level gate signal carries a TEMPORALLY-PERSISTENT numerical
    oscillation (Kurganov-Tadmor noise seeded at the corner singularity) that
    does NOT decay with integration time -- its peak-to-peak is flat at
    ~6.6e-3 in Mach out to 16 flow-throughs (so MORE endTime is NOT the lever);
  * the oscillation amplitude FALLS with mesh refinement (L1 6.6e-3, L2 3.5e-3,
    L3 0.85e-3 -- it converges away);
  * the oscillation is TEMPORAL, not a spatial-reduction artefact: the strictly
    inner zone carries an IDENTICAL peak-to-peak (6.689e-3 vs 6.687e-3), so a
    downstream-LINE spatial sample would carry the same noise (and, with fewer
    cells, likely more) -- the dated plan's spatial-line hypothesis is FALSIFIED;
  * the TIME-MEAN of the gate signal is STABLE: over averaging windows from the
    last 20% to the last 60% of the run it moves by only +/-1.2e-4 in Mach, and
    mean(last 50%) vs mean(last 25%) agree to 7.6e-5.

THE LEVER (single; keeps ALL THREE mesh levels; the gate is UNCHANGED):
  * REDUCTION: the per-level gate value is the TIME-MEAN of volAverage(Ma) over
    the last WINDOW_FRAC of rows (the settled window, past the initial
    transient), NOT the endTime snapshot.  endTime is UNCHANGED at 7.0e-3 s.
  * SETTLEDNESS: run 1's snapshot-plateau clause (peak-to-peak of the snapshot
    series < tol) is replaced by the correct convergence test for a TIME-MEAN --
    WINDOW-LENGTH INSENSITIVITY: |mean(last WINDOW_FRAC) - mean(last
    WINDOW_CHECK_FRAC)| <= TOL_STAT.  A mean still in transient fails this; a
    stationary-oscillation mean passes it.  This is the settledness of the
    OBSERVABLE THAT IS GRADED and changes in lockstep with the reduction -- it is
    one lever, not two, and it does NOT widen the 0.5% gate and does NOT drop a
    level (CLAUDE.md rule 5's one-way street; L-500; L-487).

The gate (3.2370 +/- 0.5%), the mesh triple (L1/L2/L3, r = 2), the solver, the
scheme, the endTime and the sampling zone are all IDENTICAL to run 1.  The solve
is byte-identical; only the reduction of the recorded series changed.

The reader is built directly against the v2606 writer source, NOT a belief about
it (the lesson of VMFL001 run 1 = N-AV4 / L-286):

  * output path  postProcessing/<foName>/<startTime>/volFieldValue.dat
  * header    "# Region : cellZone gateZone", "# Cells  : N", "# Volume : V",
              then "# Time" followed by <tab>volAverage(<field>) per field.
  * data      writeCurrentTime, then `file() << tab << sresult` per field, at the
              object's writePrecision (12).
Columns are located BY HEADER NAME, never by position.

    python3 grade_vmfl051_r2.py                       # grade the run tree
    python3 grade_vmfl051_r2.py --selftest            # all controls, zero compute
    python3 grade_vmfl051_r2.py --dryrun-reader FILE  # parse only; prints no value
    python3 grade_vmfl051_r2.py --verify-frozen SHA   # hash self against a commit
"""
...
SELF_REL = "cases/ansys_verification/VMFL051-R2/grade_vmfl051_r2.py"
RUN_ROOT = os.path.join(REPO, "verification/runs/ansys_verification/VMFL051-R2")
OUT_JSON = os.path.join(RUN_ROOT, "GRADING_VMFL051_R2.json")
...
# THE GATE IS AGAINST THIS.  Table .51.1, "Target" column.  UNCHANGED from run 1.
...
#
# OpenFOAM v2606's universal gas constant, which is what the SOLVER uses:
#   RR = 1e3 * NA * k = 1e3 * 6.0221417930e+23 * 1.38065e-23 = 8314.47006650545
# The CODATA-2018 value 8314.46261815324 moves M2 by 6.6e-7 in Mach = 2.0e-5 %,
# four orders of magnitude inside the gate.  Both are printed by --selftest.
# ---------------------------------------------------------------------------
...
PM_TABLE_TOL = 5e-5   # degrees; Anderson's Appendix C is printed to 4 decimals

# ---------------------------------------------------------------------------
# 3.  THE GATE AND THE DIAGNOSTIC BANDS -- UNCHANGED from run 1 (L-487).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3a.  GATE-BLIND PHYSICAL-RANGE REFUSAL.  A post-expansion Mach number that is
#      not physically a supersonic Mach number is not a number to grade.  This
#      references NEITHER the 0.5 % band NOR 3.2370: it is a physics sanity floor
#      and ceiling only.  M must be supersonic (the flow is supersonic
#      everywhere) and not absurd.
# ---------------------------------------------------------------------------
PHYS_MACH_LO = 1.0        # strictly supersonic
PHYS_MACH_HI = 20.0       # not physically credible for this case above this
...
#
# endTime is UNCHANGED at 7.0e-3 s (single lever: only the reduction changed).
# For rhoCentralFoam (transient, adjustTimeStep yes) the "last time == endTime"
# clause DOES apply -- the solver stops AT endTime (unlike a steady residualControl
# solver that may stop earlier on convergence) -- but in the DECLARED-DEPARTURE-1
# form |t_last - endTime| <= maxDeltaT, because the adaptive step overshoots
# endTime by a fraction of a step (smoke: t_last = 0.0280005 for endTime 0.028,
# overshoot 5e-7 << maxDeltaT 1e-5).
# ---------------------------------------------------------------------------
ENDTIME = 7.0e-3         # s, IDENTICAL at every level and UNCHANGED from run 1
WRITEINTERVAL = 3.5e-3   # s
MAXDELTAT = 1.0e-5       # s, the declared C3 tolerance
REQUIRED_FIELDS = ("T", "U", "p", "rho", "Ma")

# THE REDUCTION.  Per-level gate value = time-mean of volAverage(Ma) over the
# last WINDOW_FRAC of rows.  0.50 = [~3.5e-3, 7e-3] ~= 12 oscillation periods,
# past the ~2-flow-through startup transient (a flow-through is ~1.73e-3 s).
WINDOW_FRAC = 0.50

# THE SETTLEDNESS GATE (can only produce NOT A RESULT, never a PASS).  A time-mean
# is settled iff it is insensitive to the averaging-window length: the mean over
# the last WINDOW_FRAC must agree with the mean over the last WINDOW_CHECK_FRAC.
# A mean still in transient differs strongly (the smoke measured
# |mean(first 20%) - mean(last 50%)| = 0.546; the same test on the settled window
# measured 7.6e-5 at L1, the coarsest/worst level).  TOL_STAT sits between them:
# 6.6x above the worst settled value and ~13x below the oscillation amplitude.
WINDOW_CHECK_FRAC = 0.25
TOL_STAT = 5.0e-4

# The inner-zone consistency clause (DECLARED; can only produce NOT A RESULT).
# Applied to the TIME-MEANS of the two zones.
ZONE_CONSISTENCY_TOL = 1.0e-2

# ---------------------------------------------------------------------------
# 6.  PLANTED-ZERO CONTROLS (CLAUDE.md rule 3; L-487 -- the plant must move the
#     SPECIFIC REDUCTION the gate reads).  The reduction is now a TIME-MEAN, so
#     the plant is a PROPER SUBSET of the averaged set and the expected shift is
#     scaled by the planted fraction (a whole-set plant would cancel to the plant
#     identically and could not fail -- the comparator REFUSES that degenerate
#     configuration, so the defect cannot be silently reintroduced).  None is
#     optional; none can be skipped by a flag; the comparator EXITS 2 on failure.
# ---------------------------------------------------------------------------
PLANT_DAT = 1.234e-03      # Mach, planted into a PROPER SUBSET of the .dat window
PLANT_DAT_TOL = 1e-12
...
#     every level doubles nxA, nxB and ny, so h halves exactly.  UNCHANGED.
# ---------------------------------------------------------------------------
...
# The frozen inlet stagnation state, from the manual's own inlet condition; used
# only for the two DIAGNOSTIC Mach re-derivations.
...
def value_at_endtime(series, name, endtime, tol):
    """Used only by the completion check (C3) -- the GATE no longer reads a
    single endTime row."""
...
def tail_indices(nrows, frac):
    """0-based indices of the last `frac` fraction of rows (>= 2)."""
    k = max(2, int(round(frac * nrows)))
    k = min(k, nrows)
    return list(range(nrows - k, nrows)), k


def window_mean(series, name, frac):
    """THE REDUCTION: time-mean of a column over the last `frac` of rows."""
    j = column(series, name)
...
def window_ptp(series, name, frac):
    """DIAGNOSTIC: peak-to-peak (oscillation amplitude) over the same window."""
    j = column(series, name)
    idx, k = tail_indices(len(series["rows"]), frac)
    vals = [series["rows"][i][j] for i in idx]
    return max(vals) - min(vals), k


def window_periods(series, name, frac):
    """DIAGNOSTIC: approximate number of oscillation periods in the window, via
    zero-crossings of the mean-subtracted signal."""
    j = column(series, name)
    idx, k = tail_indices(len(series["rows"]), frac)
    vals = [series["rows"][i][j] for i in idx]
    m = sum(vals) / len(vals)
    d = [v - m for v in vals]
    zc = sum(1 for i in range(1, len(d)) if d[i - 1] * d[i] < 0.0)
    return zc / 2.0
...
def plant_into_scalar_field_subset(src, dst, delta, n_planted):
    """Write a copy of an ascii volScalarField with `delta` added to the FIRST
    n_planted internal values only (a PROPER SUBSET -- L-487).  The RUN TREE IS
    NEVER TOUCHED -- only this copy.  Returns (n_total, n_planted)."""
...
def plant_into_dat_subset(src, dst, colname, delta, plant_row_pred):
    ...
    the data rows for which plant_row_pred(data_row_index) is True (a PROPER
    SUBSET when the predicate does not select every row -- L-487).  The RUN TREE
    IS NEVER TOUCHED -- only this copy.  Returns (n_data_rows, n_planted)."""
    cols = None
    out = []
    n_data = 0
    n_planted = 0
...
# ===========================================================================
# Roache triple (CLAUDE.md rule 5).  Semantics fixed; thresholds above.
# ===========================================================================
...
# face of PREREGISTRATION.md section 8.
# ===========================================================================
...
def completion_check(level_dir):
    ...
    # is a STEADY-ITERATION clause; this solver is transient with an adaptive
    # step.  The invariant the clause protects -- the log is not truncated
    # mid-step -- is checked directly: one ExecutionTime line per Time line.
    ...
    # C6 AGE GUARD, STRICTER than the rule: dates the run from the LATEST mtime
    # anywhere in the case's own 0/.
...
# ===========================================================================
# One level -> the gate value (TIME-MEAN) and its diagnostics
# ===========================================================================
def read_level(level_dir):
    ...
    # THE REDUCTION -- time-mean over the settled window.
    ma, k, n = window_mean(gate, "volAverage(Ma)", WINDOW_FRAC)
    pa, _, _ = window_mean(gate, "volAverage(p)", WINDOW_FRAC)
    ta, _, _ = window_mean(gate, "volAverage(T)", WINDOW_FRAC)
    ma_in, _, _ = window_mean(diag, "volAverage(Ma)", WINDOW_FRAC)

    # SETTLEDNESS -- window-length insensitivity of the mean.
    ma_short, k_s, _ = window_mean(gate, "volAverage(Ma)", WINDOW_CHECK_FRAC)
    stat = abs(ma - ma_short)

    ptp, _ = window_ptp(gate, "volAverage(Ma)", WINDOW_FRAC)
    periods = window_periods(gate, "volAverage(Ma)", WINDOW_FRAC)

    # GATE-BLIND physical-range refusal (references neither the band nor 3.2370).
    for label, val in (("gate", ma), ("inner", ma_in)):
        if not math.isfinite(val) or not (PHYS_MACH_LO < val < PHYS_MACH_HI):
            refuse("PHYS", "the %s-zone time-mean Mach is %r in %s -- not a "
                           "physical supersonic Mach number in (%.1f, %.1f); "
                           "this references neither the gate band nor the target"
    ...
    return {
        ...
        "Ma_short_window": ma_short, "stationarity": stat,
        "window_rows": k, "window_check_rows": k_s, "series_rows": n,
        "ptp_window": ptp, "periods_window": periods,
...
# ===========================================================================
# PLANTED-ZERO CONTROLS (CLAUDE.md rule 3; L-487) -- against the REAL artifacts
# ===========================================================================
def control_pz1(level_dir):
    """Plant into a COPY of the gated .dat and require the TIME-MEAN reduction
    to move by the plant scaled by the planted FRACTION of the window (L-487:
    the plant is a proper subset of the averaged set; a whole-set plant would
    cancel to the plant identically and is refused)."""
    src = os.path.join(level_dir, "postProcessing", GATE_FO, "0",
                       "volFieldValue.dat")
    series = read_volfieldvalue(src)
    n_rows = len(series["rows"])
    win_idx, k_win = tail_indices(n_rows, WINDOW_FRAC)
    win_set = set(win_idx)
    # plant onto the SECOND HALF of the window rows -> a proper subset.
    plant_from = win_idx[len(win_idx) // 2]

    def pred(i):
        return i >= plant_from

    base, _, _ = window_mean(series, "volAverage(Ma)", WINDOW_FRAC)
    tmp = tempfile.mkdtemp(prefix="vmfl051r2_pz1_")
    try:
        # NEGATIVE ARM: an unplanted copy must show NO signal.
        flat = os.path.join(tmp, "flat.dat")
        shutil.copy2(src, flat)
        v_flat, _, _ = window_mean(read_volfieldvalue(flat),
                                   "volAverage(Ma)", WINDOW_FRAC)
        ...
        # POSITIVE ARM: a proper-subset plant must move the mean by the
        # plant times the planted fraction of the window.
        planted = os.path.join(tmp, "planted.dat")
        n_data, n_planted = plant_into_dat_subset(src, planted,
                                                  "volAverage(Ma)", PLANT_DAT, pred)
        n_planted_in_window = sum(1 for i in win_idx if pred(i))
        if n_planted_in_window == 0 or n_planted_in_window >= k_win:
            refuse("PZ-1", "the plant selected %d of %d window rows -- it must "
                           "be a PROPER, NON-EMPTY subset (L-487)"
                   % (n_planted_in_window, k_win))
        v_planted, _, _ = window_mean(read_volfieldvalue(planted),
                                      "volAverage(Ma)", WINDOW_FRAC)
        seen = v_planted - base
        expected = PLANT_DAT * (n_planted_in_window / float(k_win))
        if abs(seen - expected) > PLANT_DAT_TOL:
            refuse("PZ-1", "planted %g into %d of %d window rows of a copy of "
                           "%s; expected the mean to move by %g, saw %g "
                           "(error %g > %g) -- THE READER CANNOT SEE THE PLANT "
                           "IN THE REDUCTION IT GRADES"
                   % (PLANT_DAT, n_planted_in_window, k_win, src, expected,
                      seen, seen - expected, PLANT_DAT_TOL))
...
def control_pz2(level_dir, endtime_dir):
    """Plant into a COPY of the Ma FIELD ITSELF at endTime and require the field
    reader to see the plant in the field mean (scaled by the planted fraction of
    cells -- L-487).  This is the control that demonstrates the comparator can
    read a NON-ZERO out of the Mach field on disk."""
    ...
    n_plant = n_total // 2          # a PROPER subset
    tmp = tempfile.mkdtemp(prefix="vmfl051r2_pz2_")
    try:
        ...
        if abs(seen - expected) > PLANT_FIELD_TOL:
            refuse("PZ-2", "planted %g into %d of %d cells of a copy of the Ma "
                           "field at %s; expected mean shift %g, saw %g "
                           "(error %g) -- THE FIELD READER CANNOT SEE A NON-ZERO"
                   % (PLANT_FIELD, np_, n_total, src, expected, seen,
                      seen - expected))
        return {"plant": PLANT_FIELD, "seen": seen, "expected": expected,
                "n_cells": n_total, "n_planted": np_,
...
def grade():
    ...
    # step 1: any level whose graded time-mean is NOT settled -> NOT A RESULT
    for name, _, _, _, _ in LEVELS:
        L = per_level[name]
        if L["stationarity"] > TOL_STAT:
            verdict = "NOT A RESULT"
            why = ("level %s time-mean is NOT settled: |mean(last %.0f%%) - "
                   "mean(last %.0f%%)| = %.3e > %.3e (rule 5 step 1; the "
                   "averaging window is still in transient)"
                   % (name, 100 * WINDOW_FRAC, 100 * WINDOW_CHECK_FRAC,
                      L["stationarity"], TOL_STAT))
            break
    # the declared inner-zone consistency clause -- NOT A RESULT only.
    if verdict is None:
        L = per_level[finest]
        ...
        if zc > ZONE_CONSISTENCY_TOL:
            ...
            why = ("the gate zone and its strictly-inner diagnostic zone "
                   "time-means disagree by %.3e > %.3e at %s -- the gate zone is "
                   "being contaminated from an edge"
    ...
    result = {
        "case": "VMFL051-R2",
        "successor_of": "VMFL051 (register #4, NOT A RESULT)",
        "manual_page": MANUAL_PAGE,
        "verdict": verdict,
        "why": why,
        "reduction": {"kind": "time-mean of volAverage(Ma) over the last "
                              "%.0f%% of rows" % (100 * WINDOW_FRAC),
                      "endTime": ENDTIME, "window_frac": WINDOW_FRAC,
                      "settledness": "window-length insensitivity "
                                     "|mean(last %.0f%%)-mean(last %.0f%%)| "
                                     "<= %g" % (100 * WINDOW_FRAC,
                                                100 * WINDOW_CHECK_FRAC, TOL_STAT)},
...
def report(res):
    P = print
    P("=" * 78)
    P("VMFL051-R2 -- Isentropic Expansion of Supersonic Flow Over a Convex Corner")
    P("Successor to VMFL051 run 1 (register #4, NOT A RESULT -- OSCILLATORY triple)")
    P("Ansys Fluid Dynamics Verification Manual, Release 2026 R1, pp. %s"
      % res["manual_page"])
    P("=" * 78)
    P("")
    P("REDUCTION: %s" % res["reduction"]["kind"])
    P("           endTime %.4g s (UNCHANGED from run 1); settledness: %s"
    ...
    P("")
    P("--- levels (gate value = time-mean; ptp/periods are DIAGNOSTIC) ---")
    for name, _, _, _, cells in LEVELS:
        L = res["levels"][name]
        P("  %-14s cells %6d  zone %5d cells  <Ma>_t = %.10f  (inner %.10f)"
          % (name, cells, L["gate_ncells"], L["Ma"], L["Ma_inner"]))
        P("  %-14s   settledness |d(window)| = %.3e (tol %.1e)  "
          "osc ptp %.3e over ~%.1f periods (%d/%d rows)"
          % ("", L["stationarity"], TOL_STAT, L["ptp_window"],
             L["periods_window"], L["window_rows"], L["series_rows"]))
        P("  %-14s   diagnostics: M from p = %.10f, M from T = %.10f, "
          "<p> = %.4f Pa, <T> = %.4f K"
          % ("", L["M_from_p"], L["M_from_T"], L["p"], L["T"]))
    P("")
    t = res["triple"]
    P("--- Roache triple on the TIME-MEAN post-expansion Mach, r = %.1f, Fs = %.2f ---"
      % (t["ratio"], t["fs"]))
    ...
    P("--- THE GATE (manual printed target, Table .51.1; UNCHANGED from run 1) ---")
...
def verify_frozen(commitish):
    ...
    if mine != blob:
        ...
        return 2
    print("FREEZE VERIFIED: grade_vmfl051_r2.py is byte-identical to %s:%s "
...
# ===========================================================================
# A stationary-oscillation series (settled): a small square-ish oscillation about
# a fixed mean, on top of the physical value; endTime row present at t=0.007.
def _stationary_fixture(mean=3.2355, amp=3.0e-3, n=400, endt=ENDTIME):
    lines = ["# Region : cellZone gateZone",
             "# Cells  : 116",
             "# Volume : 3.40820048026e-05",
             "# Time\tvolAverage(Ma)\tvolAverage(p)\tvolAverage(T)"]
    for i in range(n):
        t = endt * (i + 1) / n
        v = mean + (amp if (i % 2 == 0) else -amp)   # stationary +/- amp
        lines.append("%.8g\t%.10g\t%.6g\t%.6g" % (t, v, 66414.0, 218.25))
    return "\n".join(lines) + "\n"


# A drifting series (NOT settled): the mean ramps across the window.
def _drifting_fixture(m0=3.20, m1=3.2355, n=400, endt=ENDTIME):
    lines = ["# Region : cellZone gateZone",
             "# Cells  : 116",
             "# Volume : 3.40820048026e-05",
             "# Time\tvolAverage(Ma)\tvolAverage(p)\tvolAverage(T)"]
    for i in range(n):
        t = endt * (i + 1) / n
        v = m0 + (m1 - m0) * (i / (n - 1.0))          # monotone ramp
        lines.append("%.8g\t%.10g\t%.6g\t%.6g" % (t, v, 66414.0, 218.25))
    return "\n".join(lines) + "\n"
...
def selftest():
    ...
    print("VMFL051-R2 comparator selftest -- no solver, no run tree touched")
    print("")
    ...
    print("")
    print("[C] the frozen reference values (UNCHANGED from run 1)")
    m2 = M2_prandtl_meyer(MANUAL_M1, TURN_DEG, GAMMA)
    ...
    print("")
    print("[D] the reader and the TIME-MEAN reduction, on the REAL .dat format")
    tmp = tempfile.mkdtemp(prefix="vmfl051r2_selftest_")
    try:
        f = os.path.join(tmp, "volFieldValue.dat")
        open(f, "w").write(_stationary_fixture())
        ...
        mean, k, n = window_mean(s, "volAverage(Ma)", WINDOW_FRAC)
        check("time-mean of a stationary +/- osc equals the mean",
              abs(mean - 3.2355) < 1e-9, "<Ma>_t = %.10f over %d/%d rows" % (mean, k, n))
        ptp, _ = window_ptp(s, "volAverage(Ma)", WINDOW_FRAC)
        check("ptp diagnostic sees the oscillation amplitude",
              abs(ptp - 6.0e-3) < 1e-9, "ptp = %.4e" % ptp)
        m_long, _, _ = window_mean(s, "volAverage(Ma)", WINDOW_FRAC)
        m_short, _, _ = window_mean(s, "volAverage(Ma)", WINDOW_CHECK_FRAC)
        check("a STATIONARY series is SETTLED (window-insensitive)",
              abs(m_long - m_short) <= TOL_STAT,
              "|d| = %.3e <= %.1e" % (abs(m_long - m_short), TOL_STAT))

        fd = os.path.join(tmp, "drift.dat")
        open(fd, "w").write(_drifting_fixture())
        sd = read_volfieldvalue(fd)
        md_long, _, _ = window_mean(sd, "volAverage(Ma)", WINDOW_FRAC)
        md_short, _, _ = window_mean(sd, "volAverage(Ma)", WINDOW_CHECK_FRAC)
        check("a DRIFTING series is NOT settled (the clause can say no)",
              abs(md_long - md_short) > TOL_STAT,
              "|d| = %.3e > %.1e -> NOT A RESULT" % (abs(md_long - md_short), TOL_STAT))

        # refusal arms
        f2 = os.path.join(tmp, "nocells.dat")
        open(f2, "w").write(_stationary_fixture().replace("# Cells  : 116",
                                                          "# Cells  : 0"))
        try:
        ...
        print("")
        print("[E] planted-zero, .dat TIME-MEAN reader (PZ-1), proper-subset (L-487)")
        base, _, _ = window_mean(read_volfieldvalue(f), "volAverage(Ma)", WINDOW_FRAC)
        n_rows = len(s["rows"])
        win_idx, k_win = tail_indices(n_rows, WINDOW_FRAC)
        plant_from = win_idx[len(win_idx) // 2]
        # negative arm
        flat = os.path.join(tmp, "flat.dat")
        shutil.copy2(f, flat)
        vf, _, _ = window_mean(read_volfieldvalue(flat), "volAverage(Ma)", WINDOW_FRAC)
        check("unplanted copy shows NO signal", abs(vf - base) < PLANT_DAT_TOL,
              "delta %.3e" % (vf - base))
        # positive arm: proper subset -> fractional shift
        pl = os.path.join(tmp, "planted.dat")
        plant_into_dat_subset(f, pl, "volAverage(Ma)", PLANT_DAT,
                              lambda i: i >= plant_from)
        n_pw = sum(1 for i in win_idx if i >= plant_from)
        vp, _, _ = window_mean(read_volfieldvalue(pl), "volAverage(Ma)", WINDOW_FRAC)
        ...
        # the "can say no" arm: a WHOLE-window plant is refused as inert (L-487)
        whole = os.path.join(tmp, "whole.dat")
        plant_into_dat_subset(f, whole, "volAverage(Ma)", PLANT_DAT,
                              lambda i: True)
        # emulate control_pz1's inert-config guard
        n_all = sum(1 for i in win_idx if True)
        check("a WHOLE-window plant is the INERT config the control refuses",
              n_all >= k_win, "n_planted_in_window %d >= window %d -> REFUSE" % (n_all, k_win))
        ...
        # can say no: a whole-set field plant is refused as inert
        try:
    ...
    check("oscillatory family -> OSCILLATORY (run 1's own state)",
    ...
    print("")
    print("[I] the gate, both arms (band UNCHANGED from run 1)")
    ...
    check("a subsonic mean (M=0.5) is out of the physical range",
          not (PHYS_MACH_LO < 0.5 < PHYS_MACH_HI),
          "M=0.5 -> REFUSE (supersonic required)")
    ...
    check("a physical supersonic mean (M=3.2) is in range",
          ...
          "M=3.2 -> accepted; range (%.1f,%.1f) mentions neither 3.2370 nor 0.5%%"
...
def main(argv):
    ...
    if "--dryrun-reader" in argv:
        ...
        # built so it cannot leak the answer.
