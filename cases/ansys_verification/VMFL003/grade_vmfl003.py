#!/usr/bin/env python3
"""
VMFL003 -- Pressure Drop in Turbulent Flow Through a Pipe.  THE COMPARATOR.
...
YPLUS_LO   = 25.0                 # one-way NOT-A-RESULT band, frozen
YPLUS_HI   = 65.0
...
LEVELS = [
    ...
    ("L1_250x5",     250,    5,  1250,  6000),
    ("L2_500x5",     500,    5,  2500,  8000),
    ("L3_1000x5",   1000,    5,  5000, 12000),
]
LADDER = [
    ("D_500x3",      500,    3,  1500,  8000),
    ("D_500x4",      500,    4,  2000,  8000),
    ("L2_500x5",     500,    5,  2500,  8000),   # reused, not re-run
    ("D_500x6",      500,    6,  3000,  8000),
]
FIELDS_REQUIRED = ["U", "p", "k", "epsilon", "nut"]

RUNROOT_DEFAULT = "verification/runs/ansys_verification/VMFL003"
...
def gate_verdict(dp_pa):
    ...
    return ("PASS" if abs(dev) <= GATE_BAND else "GATE FAIL"), dev


def grade(runroot, write_json=None):
    res = {"case": "VMFL003", "runroot": runroot,
    ...
    for name, nx, nr, cells, endt in LEVELS:
        ...
        yavg = yrows[-1][3]
        lv["yplus"] = {"min": yrows[-1][1], "max": yrows[-1][2],
                       "average": yavg, "predicted": YPLUS_PRED}
        lv["yplus_ok"] = (YPLUS_LO <= yavg <= YPLUS_HI)
        if not lv["yplus_ok"]:
            res["not_a_result_reasons"].append(
                "%s: mean wall y+ = %.6g is outside the frozen wall-function "
                "validity band [%g, %g] -- nutkWallFunction was applied outside "
...
def selftest():
    ...
    ck(gate_verdict(TARGET_DP_PA)[0] == "PASS", "exact target passes")
    ck(gate_verdict(DP_COLEBROOK)[0] == "PASS", "Colebrook passes")
    ...
    ck(gate_verdict(TARGET_DP_PA * 1.024)[0] == "PASS", "+2.4 % passes")
    ck(gate_verdict(TARGET_DP_PA * 0.976)[0] == "PASS", "-2.4 % passes")
    # DECLARED before any compute: this band passes both Ansys solvers.
    ck(gate_verdict(ANSYS_CONTEXT["Fluent"])[0] == "PASS",
       "DECLARED: Fluent's own 21480 Pa (-1.214 %) passes this band")
    ck(gate_verdict(ANSYS_CONTEXT["CFX"])[0] == "PASS",
       "DECLARED: CFX's own 21740 Pa (-0.018 %) passes this band")
    ...
    print("VMFL003 comparator --selftest: %d checks, %d failures"
          % (checks[0], len(fails)))
    for f in fails:
        print("  FAILED: " + f)
...
def verify_frozen(commit):
    ...
    mine.update(data)
    rel = "cases/ansys_verification/VMFL003/grade_vmfl003.py"
    try:
        want = subprocess.check_output(
            ["git", "rev-parse", "%s:%s" % (commit, rel)],
            cwd=os.path.dirname(os.path.dirname(os.path.dirname(here))),
            text=True).strip()
