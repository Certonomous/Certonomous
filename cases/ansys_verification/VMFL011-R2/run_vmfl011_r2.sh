#!/usr/bin/env bash
# =============================================================================
# VMFL011-R2 graded-run driver -- Laminar Flow in a Triangular Cavity.
...
#
# Frozen pre-registration: cases/ansys_verification/VMFL011-R2/PREREGISTRATION.md
# Frozen comparator      : cases/ansys_verification/VMFL011-R2/grade_vmfl011_r2.py
# Run root               : verification/runs/ansys_verification/VMFL011-R2/
#
...
RUN_ROOT="${1:?usage: run_vmfl011_r2.sh <run_root> [levels...]}"
shift 2>/dev/null
...
echo "=== VMFL011-R2 launcher  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)  run_root=$RUN_ROOT"
...
PREREG_REL="cases/ansys_verification/VMFL011-R2/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL011-R2/grade_vmfl011_r2.py"
...
# claim in a document.  The R2 changes the PLANT SIZING inside the rule-3 control
# and nothing else on the grading path.
for F in case/0/U case/0/p case/constant/transportProperties \
  ...
  [ "$A" = "$B" ] || { echo "ABORT: $F is NOT the attempt-1 input ($B != $A) -- the R2 changes the PLANT SIZING, nothing else"; exit 2; }
done
...
python3 "$SCRIPT_DIR/grade_vmfl011_r2.py" --selftest > "$ST" 2>&1 || { echo "ABORT: comparator --selftest is NOT green under python3; see $ST"; exit 2; }
python3 -O "$SCRIPT_DIR/grade_vmfl011_r2.py" --selftest > "$ST.O" 2>&1 || { echo "ABORT: comparator --selftest is NOT green under python3 -O; see $ST.O"; exit 2; }
...
echo "=== VMFL011-R2 launcher done, overall rc=$OVERALL_RC, total ${TOTAL_CORE_MIN} core-min of ${CAP_CORE_MIN}"
echo "grade with: python3 $SCRIPT_DIR/grade_vmfl011_r2.py --run-root $RUN_ROOT"
exit "$OVERALL_RC"
