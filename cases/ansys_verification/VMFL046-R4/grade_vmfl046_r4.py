#!/usr/bin/env python3
"""VMFL046-R4 -- Supersonic flow with a normal shock in a converging-diverging nozzle.
THE FROZEN GRADING PATH.  Ansys Fluid Dynamics Verification Manual VM2026R1 p.155.

R4 IS A COST FIX AND A READER-WINDOW FIX.  IT IS NOT A PHYSICS FIX AND IT IS NOT A
NUMERICS FIX.  Every one of the eleven case/ inputs is BYTE-IDENTICAL to R3's, and the
driver ABORTS if any one of them differs (run_vmfl046_r4.sh, exit 7).
...
WHAT IS CARRIED FORWARD FROM R3'S FROZEN COMPARATOR, UNCHANGED IN BEHAVIOUR
  ...
  N4  the limitTemperature bounds must be NON-BINDING at endTime
...
USAGE
    grade_vmfl046_r4.py <run_root>          grade
    grade_vmfl046_r4.py --paths <run_root>  §39.5 path enumeration against a real run root
    grade_vmfl046_r4.py --selftest          planted-failure self-tests
"""
...
# N4 -- the frozen constant/fvOptions bounds.
...
def check_limiters_nonbinding(t_path, level):
    ...
    and R3.  During EVERY failed transient probe of R3 §5.3 that limiter bound at BOTH ends
    within a few time steps of the blow-up.  A run whose T field touches those bounds at
    endTime is not reporting the registered physics, and printing that as a diagnostic
    beside a verdict would be worse than never computing it.  The physical range is
    T in [328, 500] K, so this limb is non-binding by construction unless something has
    gone wrong."""
...
# ---- grade ------------------------------------------------------------------
def grade(run_root):
    print("VMFL046-R4  Supersonic flow with a normal shock in a CD nozzle (VM2026R1 p.155)")
    print("solver: rhoPimpleFoam (TRANSIENT), ddtSchemes Euler, Gauss vanLeer(V) 1 --")
    print("        BYTE-IDENTICAL to R3.  R4 changes the COST ENVELOPE and the READER's")
    print("        WINDOW, and NOTHING about the physics or the numerics.")
    ...
    try:
        ...
        if args and args[0] == "--paths":
            if len(args) != 2:
                ...
                return 2
            print("§39.5 path enumeration for VMFL046-R4 against %s" % args[1])
            for p in READ_PATHS:
                print("   reads: %s" % p)
            print()
