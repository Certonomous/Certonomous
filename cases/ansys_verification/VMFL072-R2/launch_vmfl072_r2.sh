#!/bin/bash
# ---------------------------------------------------------------------------
# VMFL072-R2 -- THE LAB DRIVER.  One registered level per invocation.
#
#     VMFL072R2_PREREG_SHA=<freeze commit> ./launch_vmfl072_r2.sh <L1|L2|L3|B2|C1>
#
...
#   G-01  VMFL072R2_PREREG_SHA is set
...
usage() { echo "usage: VMFL072R2_PREREG_SHA=<sha> $0 <L1|L2|L3|B2|C1>" >&2; exit 3; }
...
RELDIR="cases/ansys_verification/VMFL072-R2"

# --- registered per-level caps, core-minutes (R2 8.2). SERIAL: 1 rank, so ---
...
FROZEN_FILES=(
  "PREREGISTRATION.md"
  "compare_vmfl072_r2.py"
  "apply_level.sh"
  "launch_vmfl072_r2.sh"
)
CASE_INPUTS=(
  "base/constant/g"
...
# ===========================================================================
if [ -z "${VMFL072R2_PREREG_SHA:-}" ]; then
  echo "G-01 REFUSED: VMFL072R2_PREREG_SHA is not set. The frozen file must be" >&2
...
CMP="$(git -C "${REPO}" hash-object "${HERE}/compare_vmfl072_r2.py")"
if [ "${CMP}" != "${PIN}" ]; then
...
if ! git -C "${REPO}" rev-parse --verify --quiet "${VMFL072R2_PREREG_SHA}^{commit}" >/dev/null; then
  echo "G-02 REFUSED: '${VMFL072R2_PREREG_SHA}' is not a commit in this repository." >&2
  exit 2
fi
for f in "${FROZEN_FILES[@]}"; do
  have="$(git -C "${REPO}" hash-object "${HERE}/${f}")"
  want="$(git -C "${REPO}" rev-parse --verify --quiet \
            "${VMFL072R2_PREREG_SHA}:${RELDIR}/${f}" || true)"
  if ! [[ "${want}" =~ ^[0-9a-f]{40}$ ]]; then
    echo "G-02 REFUSED: ${RELDIR}/${f} does not exist at ${VMFL072R2_PREREG_SHA}" >&2
    exit 2
  fi
  if [ "${have}" != "${want}" ]; then
...
# ===========================================================================
RUNROOT="${REPO}/verification/runs/ansys_verification/VMFL072-R2/${LEVEL}"
case "${RUNROOT}" in
...
case            VMFL072-R2
level           ${LEVEL}
prereg sha      ${VMFL072R2_PREREG_SHA}
comparator pin  ${PIN}
comparator blob ${CMP}
run root        ${RUNROOT}
...
echo "  python3 ${HERE}/compare_vmfl072_r2.py --runroot ${REPO}/verification/runs/ansys_verification/VMFL072-R2"
