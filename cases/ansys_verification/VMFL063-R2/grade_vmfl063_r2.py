#!/usr/bin/env python3
# =============================================================================
# VMFL063-R2 COMPARATOR -- Separated Laminar Flow Over a Blunt Plate
# Ansys Fluid Dynamics Verification Manual, Release 2026 R1, p. 193 (Table .63.1).
#
# SUCCESSOR to VMFL063 (register #44, GATE FAIL). CORRECTED LEVER: RESOLUTION-ONLY.
# The base's dated plan said 'linearUpwind->linear'; the base fvSchemes was ALREADY
# `bounded Gauss linear` (2nd-order central, the least-diffusive scheme), blob
# 0b7e18377a27, verified disk==HEAD -- there is NO linearUpwind to switch from and
# nothing higher-order than central to switch to. The R2 case inputs are
# BYTE-IDENTICAL to VMFL063's (all 9 blobs equal); the ONLY change is a finer r=2
# triple 23040 / 92160 / 368640 (the base's own grading inherited verbatim, the
# family shifted up one octave). The gate LR/(2t)=4.0 +/-10 % is BYTE-IDENTICAL to
# the base (L-487): TOL, window, sign ref, P_MIN, Fs, ratio all unchanged. This
# comparator IS the base VMFL063 comparator (blob fc339a79) adapted ONLY by:
# run-root / path / tempfile-prefix renames; a gate-blind PHYSICAL-RANGE refusal
# that references neither the Target 4.0 nor the +/-10 % band; and an explicit
# L-487 inert-arm demonstration in --selftest. The reduction (the LAST reversed-to-
# attached crossing) is UNCHANGED, so the well-tested reader is preserved intact.
#
...
RUN_ROOT = os.path.join(HERE, "..", "..", "..",
                        "verification", "runs", "ansys_verification", "VMFL063-R2")
...
def selftest():
    print("SELFTEST VMFL063-R2 -- Separated Laminar Flow Over a Blunt Plate (VM2026R1 p.193)")
...
def verify_frozen():
    ...
    rc = 0
    for rel in ("cases/ansys_verification/VMFL063-R2/PREREGISTRATION.md",
                "cases/ansys_verification/VMFL063-R2/grade_vmfl063_r2.py"):
...
def main(argv):
    ...
    R = {"case": "VMFL063-R2", "manual_page": 193,
    ...
    print("VMFL063-R2  (VM2026R1 p.193)  ROW VERDICT: %s" % R["verdict"])
