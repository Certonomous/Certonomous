#!/usr/bin/env bash
# =============================================================================
# VMFL063-R2 graded-run driver -- Separated Laminar Flow Over a Blunt Plate.
...
# The base dated 'linearUpwind->linear' clause was FALSIFIED -- the base fvSchemes
# was ALREADY `bounded Gauss linear` (2nd-order central), blob 0b7e1837. R2 changes
# ONLY the cell counts (finer r=2 triple 23040/92160/368640); the case inputs are
# byte-identical to VMFL063's and every gate constant is unchanged.
#
# Frozen pre-registration: cases/ansys_verification/VMFL063-R2/PREREGISTRATION.md
# Frozen comparator      : cases/ansys_verification/VMFL063-R2/grade_vmfl063_r2.py
# Run root               : verification/runs/ansys_verification/VMFL063-R2/
#
...
#   (a) EVERY case input is hashed against its OWN HEAD blob at the VMFL063-R2
#       path -- the files that run ARE the files that were frozen. They are also
#       byte-identical to VMFL063's inputs, which is what makes R2 a resolution-only
#       change (PREREGISTRATION sec.1.4); the freeze-pin here is the operative check.
...
RANKS=1
CAP_CORE_MIN=320         # RUNNING TOTAL across all four solves (PREREGISTRATION sec.7)
ENDTIME=30000            # iteration ceiling; residualControl stops it earlier

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CASE_DIR="$SCRIPT_DIR/case"
RUN_ROOT="${1:?usage: run_vmfl063_r2.sh <run_root> [levels...]}"
shift 2>/dev/null
...
declare -A NXU=( [L1]=64  [L1D]=64  [L2]=128 [L3]=256 )
declare -A NXD=( [L1]=160 [L1D]=160 [L2]=320 [L3]=640 )
declare -A NYL=( [L1]=24  [L1D]=24  [L2]=48  [L3]=96  )
declare -A NYU=( [L1]=96  [L1D]=96  [L2]=192 [L3]=384 )

echo "=== VMFL063-R2 launcher  utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)  run_root=$RUN_ROOT"
...
PREREG_REL="cases/ansys_verification/VMFL063-R2/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL063-R2/grade_vmfl063_r2.py"
...
for F in 0/U 0/p constant/transportProperties constant/momentumTransport \
  ...
  A="$(git -C "$REPO" rev-parse "HEAD:cases/ansys_verification/VMFL063-R2/case/$F" 2>/dev/null)" || { echo "ABORT: case/$F is not committed at HEAD"; exit 2; }
...
# tempfile.mkdtemp(prefix="vmfl063_r2_selftest_") in grade_vmfl063_r2.py and many
...
ST="/tmp/vmfl063_r2_selftest.$$"
rm -rf "$SCRIPT_DIR/__pycache__"
python3    "$SCRIPT_DIR/grade_vmfl063_r2.py" --selftest > "$ST"   2>&1; RC_PLAIN=$?
[ "$RC_PLAIN" = "0" ] || { echo "ABORT: comparator --selftest is NOT green under python3 (rc $RC_PLAIN); see $ST"; exit 2; }
rm -rf "$SCRIPT_DIR/__pycache__"
python3 -O "$SCRIPT_DIR/grade_vmfl063_r2.py" --selftest > "$ST.O" 2>&1; RC_O=$?
...
for MARK in \
  ...
  'ROW verdict can NEVER be PASS while limb A is capped at GATE REACHED' \
  'physical-range REFUSES an LR beyond the frozen search window' \
  'L-487 INERT arm' ; do
...
{ echo "case = VMFL063-R2"
  echo "manual_page = 193"
  echo "predecessor = VMFL063 register #44 (GATE FAIL); base freeze 2df23798 -> 3602cbf3"
  echo "supersedes = the OWED dated plan of VMFL063 (register #44, GATE FAIL); row #44 stands"
...
echo "=== VMFL063-R2 launcher done, overall rc=$OVERALL_RC, total ${TOTAL_CORE_MIN} core-min of ${CAP_CORE_MIN}"
echo "grade with: python3 $SCRIPT_DIR/grade_vmfl063_r2.py --run-root $RUN_ROOT --out $RUN_ROOT/GRADING_VMFL063_R2.json"
exit "$OVERALL_RC"
