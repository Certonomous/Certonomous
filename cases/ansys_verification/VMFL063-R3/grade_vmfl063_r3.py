#!/usr/bin/env python3
# =============================================================================
# VMFL063-R3 COMPARATOR -- Separated Laminar Flow Over a Blunt Plate
# Ansys Fluid Dynamics Verification Manual, Release 2026 R1, p. 193 (Table .63.1).
#
# SUCCESSOR to VMFL063-R2 (register #68, GATE FAIL). The R2 grid ladder is EXHAUSTED
# (p=2.505, GCI_fine=0.48 % on a CONVERGING triple converging to LR/(2t)~5.5, 37.5 %
# out). The case is LAMINAR, so a grid-converged code-to-code gap cannot be turbulence
# model-form: it can only be a SETUP/DOMAIN difference (PREREGISTRATION sec.1.2). R2
# established GRID-independence but never DOMAIN-independence. R3's lever is therefore
# a DOMAIN-INDEPENDENCE study at a FIXED grid-converged resolution (R2-L3's): enlarge
# the domain over a frozen ladder and de-confine the far-field top BC (symmetryPlane ->
# a non-confining constant-pressure open boundary).
#
# THE GATE IS BYTE-IDENTICAL TO R2 (L-487). Every gate/control FUNCTION and CONSTANT in
# this file below the FROZEN CONSTANTS and DOMAIN LADDER blocks is CARRIED OVER FROM THE
# R2 COMPARATOR VERBATIM -- REF_LR2T, TOL, window, sign ref, TAU_EPS, P_MIN, Fs, ratio,
# the last-crossing reader, the from-data sign convention, the cross-instrument check,
# the gate-blind PHYSICAL-RANGE refusal, the two-stage/two-channel planted-zero control,
# the AST/cardinality/numeric-time-dir guards, roache(), and the three verdict functions.
# R3 ADDS ONLY, and touches nothing on the gate path: (1) the frozen domain ladder and
# self-convergence tolerance DOMAIN_TOL; (2) domain_self_convergence(), which fixes D*
# from the ladder and DOMAIN_TOL alone (never the Target 4.0); (3) main() wiring that
# grades the ladder + the BC-isolation diagnostic, fixes D*, and then runs the UNCHANGED
# R2 gate on the r=2 triple built at D* (under run_root/GATE). The domain check is
# reported BESIDE the gate and enters neither verdict_for_limb_a() nor the band test.
#
...
RUN_ROOT = os.path.join(HERE, "..", "..", "..",
                        "verification", "runs", "ansys_verification", "VMFL063-R3")
...
# GATE-BLIND PHYSICAL-RANGE refusal (supervisor's R2 requirement). A reattachment
# length is a POSITIVE distance downstream of the blunt face and, being located
# inside the frozen search window, cannot exceed the window's own extent. This
# bound is GEOMETRIC -- it references the frozen window (X_WIN_HI) and the plate
# thickness (TWO_T) ONLY, and NEITHER the Target 4.0 NOR the +/-10 % band -- so it
# cannot be a disguised gate. A value inside the bound is NOT thereby PASS; the
# band still decides. LR2T_PHYS_MAX = X_WIN_HI/TWO_T = 13.333...
LR2T_PHYS_MAX = X_WIN_HI / TWO_T
...
# ------------------------------------------------- R3 DOMAIN-INDEPENDENCE LAYER -
# NEW in R3, and the ONLY addition to the frozen constants. The gate above is
# byte-identical to R2. The domain ladder tests DOMAIN-independence at a FIXED
# grid-converged resolution (R2-L3's), the un-ruled-out laminar SETUP lever
# (PREREGISTRATION sec.1, sec.4). Extents are geometric multiples of the plate
# thickness 2t, fixed a priori -- NO LR value enters their choice.
DOMAINS    = ["D0", "D1", "D2"]     # Lu/H = 10/20, 20/40, 40/80 (x 2t); PREREG sec.4.3
BC_ISOLATION = "D0_CONFINED"        # D0 with the OLD symmetryPlane top; diagnostic only
GATE_SUBDIR  = "GATE"               # the r=2 gate triple + L1D are built under here at D*
# ANSWER-BLIND self-convergence stop (PREREGISTRATION sec.4.4), DERIVED FROM THE GATE
# TOLERANCE by a rule fixed in advance -- NEVER from an observed floor
# (ANSYS_VERIFICATION_CHARTER sec.16.2 / sec.20.2: a plateau threshold is justified from
# the gate tolerance so iteration/ladder noise cannot move the verdict, and whatever
# number falls out is BINDING, including if it fails). The derivation is EXECUTABLE here,
# not merely recited:  DOMAIN_TOL = TOL / 10  = a fixed 10x headroom below the gate band,
# so residual per-rung domain drift cannot move the verdict across the +/-10 % band.
# It references NEITHER the Target 4.0 NOR the answer -- only the frozen band it is one-
# tenth of -- so it is not a disguised gate: a ladder that self-converges FAR from 4.0
# (e.g. ~5.5) still reports SELF_CONVERGED (--selftest proves this).
# A-PRIORI ACHIEVABILITY (sec.21.1, commensurate units, NOT the derivation): the crossing
# reader resolves LR/(2t) to ~0.42 % (R2-L3 dx ~2.06 mm / 2t over LR/2t ~5.5), FINER than
# 1 %, so the stop is satisfiable and not sub-resolution.
# GENUINE PLATEAU (sec.16.2): SELF_CONVERGED requires the TERMINAL ladder step within
# DOMAIN_TOL (a within-tol step followed by an out-of-tol one is a fortuitous single step,
# NOT a plateau); D* = the EARLIEST domain from which EVERY remaining step stays within
# DOMAIN_TOL. If none exists the ladder is NOT extended: the row is NOT A RESULT.
DOMAIN_TOL = TOL / 10.0             # = 0.010; derived from the gate band, not observed
DOMAIN_TOL_RULE = "TOL / 10.0"      # the fixed derivation, recorded for the grading record

# GATE PROVENANCE (L-487). Every gate/control CONSTANT and FUNCTION below the FROZEN
# CONSTANTS / DOMAIN blocks is carried VERBATIM from the R2 frozen comparator. Its blob
# is recorded here and --verify-frozen re-hashes the R2 file and REFUSES on any drift, so
# "the gate is byte-identical to R2" is a machine-checked invariant, not a prose claim.
R2_COMPARATOR_REL  = "cases/ansys_verification/VMFL063-R2/grade_vmfl063_r2.py"
R2_COMPARATOR_BLOB = "5d94fecbfa35013943b60e758ff433ad50ef00f7"
...
def physical_range_refuse(lr_m, lr2t, level):
    """GATE-BLIND physical-range refusal. A reattachment length must be a positive
    distance lying inside the frozen (geometric) search window. This references the
    window and the plate thickness ONLY -- never the Target and never the band -- so
    it cannot be a disguised gate. It catches a nonphysical reader result (a
    zero/negative crossing, or one beyond the domain window) before any value is
    graded; a plausible value is NOT thereby a PASS. Returns None on success."""
    if not (lr_m > 0.0):
        raise SystemExit2("%s: PHYSICAL-RANGE refusal -- LR = %.9g m is not a positive "
                          "distance downstream of the blunt face" % (level, lr_m))
    if not (lr_m <= X_WIN_HI):
        raise SystemExit2("%s: PHYSICAL-RANGE refusal -- LR = %.9g m lies beyond the frozen "
                          "search-window upper bound %.9g m" % (level, lr_m, X_WIN_HI))
    if not (0.0 < lr2t <= LR2T_PHYS_MAX):
        raise SystemExit2("%s: PHYSICAL-RANGE refusal -- LR/(2t) = %.9g is outside the "
                          "geometric bound (0, %.6g] (window/2t; references neither the "
                          "Target nor the band)" % (level, lr2t, LR2T_PHYS_MAX))
...
def planted_zero_tau(level_dir, t):
    ...
    tmp = tempfile.mkdtemp(prefix="vmfl063_r2_plant_tau_")
    try:
...
def planted_zero_u(level_dir, t):
    ...
    tmp = tempfile.mkdtemp(prefix="vmfl063_r2_plant_u_")
    try:
...
# --------------------------------------------- R3 domain self-convergence ------
def domain_self_convergence(lr2t_by_domain, order=DOMAINS, tol=DOMAIN_TOL):
    """NEW in R3, and it touches NOTHING on the gate path. Given LR/(2t) at each
    domain of the frozen ladder (in ladder order), decide DOMAIN-independence by
    SELF-CONVERGENCE and fix D*.

    ANSWER-BLIND (CLAUDE.md rule 2; PREREGISTRATION sec.4.4): the ONLY reference is
    `tol` (DOMAIN_TOL). The Target 4.0 (REF_LR2T) and the band (TOL) do NOT appear
    here -- a ladder that self-converges to a value far from 4.0 (e.g. ~5.5) still
    reports SELF_CONVERGED, which --selftest proves.

    GENUINE PLATEAU (ANSYS_VERIFICATION_CHARTER sec.16.2, sec.16.3): the ladder
    SELF_CONVERGES only if the FINAL step is within `tol`. A within-tol step followed
    by an out-of-tol step is a fortuitous single step, NOT a plateau, and reports
    NOT_CONVERGED -- otherwise a ladder that resumed moving would be certified
    domain-independent (the R2-grid analogue of a non-CONVERGING Roache triple). D* is
    then the EARLIEST domain from which EVERY remaining step stays within `tol` -- the
    smallest domain beyond which enlargement no longer moves LR. If the final step is
    out of `tol`, the state is NOT_CONVERGED and the caller emits NOT A RESULT WITHOUT
    extending the frozen ladder."""
    present = [d for d in order if d in lr2t_by_domain]
    steps = []
    for i in range(1, len(present)):
        prev, cur = present[i - 1], present[i]
        a, b = lr2t_by_domain[prev], lr2t_by_domain[cur]
        rel = abs(b - a) / abs(a) if a != 0.0 else float("inf")
        steps.append({"from": prev, "to": cur, "lr2t_from": a, "lr2t_to": b,
                      "rel_change": rel, "within_tol": rel <= tol})
    d_star = None
    if steps and steps[-1]["within_tol"]:
        c = len(steps) - 1
        while c - 1 >= 0 and steps[c - 1]["within_tol"]:
            c -= 1
        d_star = steps[c]["to"]
    state = "SELF_CONVERGED" if d_star is not None else "NOT_CONVERGED"
    return {"state": state, "d_star": d_star, "tol": tol, "tol_rule": DOMAIN_TOL_RULE,
            "ladder_order": list(order), "steps": steps,
            "note": ("SELF_CONVERGED requires the TERMINAL ladder step within DOMAIN_TOL "
                     "(a genuine plateau, not a fortuitous single step, sec.16.2); D* is "
                     "the earliest domain from which every remaining step stays within "
                     "DOMAIN_TOL; references neither the Target nor the band")}


def _case_lr2t(case_root, case_id, store):
    """Read LR/(2t) for ONE completed case dir with the SAME functions the gate
    uses (completion + last-crossing reduction + physical-range refusal +
    cross-instrument check). Module scope so both main()'s ladder and pick_dstar()
    share it. This is OFF the gate path: it reads the SAME quantity, but the gate
    verdict is decided only by verdict_for_limb_a()/roache() on the D* triple."""
    c = completion(case_root, case_id)
    ...
    if lr is None:
        raise SystemExit2("%s: the plate-top wall shear has NO reversed-to-attached "
                          "crossing inside the frozen window (%g, %g] m -- no "
                          "reattachment length to grade" % (case_id, X_WIN_LO, X_WIN_HI))
    physical_range_refuse(lr, lr / TWO_T, case_id)
    ...
    if lr_u is None:
        raise SystemExit2("%s: the INDEPENDENT near-wall u_x instrument finds no "
                          "reversal in the window; the value is not certified" % case_id)
    dx = local_dx_at(x, lr)
    if not (abs(lr - lr_u) <= CROSS_TOL_CELLS * dx):
        raise SystemExit2("%s: CROSS-INSTRUMENT DISAGREEMENT -- wall shear LR = %.9g m, "
                          "near-wall u_x LR = %.9g m, apart by %.9g m > %g local cell "
                          "widths (%.9g m)" % (case_id, lr, lr_u, abs(lr - lr_u),
                                               CROSS_TOL_CELLS, CROSS_TOL_CELLS * dx))
...
def pick_dstar(run_root):
    """OFF-GATE launcher helper. Grade ONLY the domain ladder (no gate) and print
    the frozen self-convergence decision so the launcher can build the gate triple
    at D*. It reuses domain_self_convergence() and touches NO gate function.
    Prints one line:  D_STAR=<Dn>  or  D_STAR=NOT_CONVERGED."""
    run_root = os.path.abspath(run_root)
    lr2t = {d: _case_lr2t(run_root, d, None) for d in DOMAINS}
    dsc = domain_self_convergence(lr2t)
    for d in DOMAINS:
        print("# %s LR/(2t) = %.9g" % (d, lr2t[d]))
    print("D_STAR=%s" % (dsc["d_star"] if dsc["state"] == "SELF_CONVERGED" else "NOT_CONVERGED"))
    return 0
...
def selftest():
    print("SELFTEST VMFL063-R3 -- Separated Laminar Flow Over a Blunt Plate (VM2026R1 p.193)")

    ck("AST guard: ast.Assert count is 0 in this file", _ast_guard() == 0)

    tmp = tempfile.mkdtemp(prefix="vmfl063_r2_selftest_")
    try:
        # ---- cardinality guard ------------------------------------------------
        ...
        # ---- gate-blind physical-range refusal (geometric; not the gate) ------
        ck("physical-range: a plausible LR passes the geometric guard",
           physical_range_refuse(0.36, 0.36 / TWO_T, "L?") is None)
        ck("physical-range REFUSES a non-positive LR",
           _refuses(physical_range_refuse, 0.0, 0.0, "L?"))
        ck("physical-range REFUSES an LR beyond the frozen search window",
           _refuses(physical_range_refuse, X_WIN_HI + 0.1, (X_WIN_HI + 0.1) / TWO_T, "L?"))
        ck("physical-range bound is GEOMETRIC (window/2t), NOT the Target 4.0",
           abs(LR2T_PHYS_MAX - X_WIN_HI / TWO_T) < 1e-12 and LR2T_PHYS_MAX != REF_LR2T)
        ...
        # ---- L-487: the plant is matched to the REDUCTION (a crossing locator) --
        # A crossing reduction must move ONLY when the feature moves. Two arms on
        # the SAME bubble prove the design is load-bearing, not decorative:
        #   GOOD arm  -- a whole-profile positive shift moves the crossing upstream;
        #   INERT arm -- an equal shift applied ONLY to already-attached faces
        #                DOWNSTREAM of the crossing leaves the last crossing PUT.
        # If the inert arm moved the crossing, the reader would be responding to
        # values away from the feature -- exactly L-487's failure to avoid.
        xb487, tb487 = _bubble_tau(0.36)
        lr0_487 = last_sign_change_in_window(xb487, tb487)
        bump487 = 0.05 * max(abs(v) for v in tb487)
        good_487 = last_sign_change_in_window(xb487, [v + bump487 for v in tb487])
        ck("L-487 GOOD arm: a whole-profile positive shift moves the crossing upstream",
           good_487 is not None and good_487 < lr0_487)
        inert487 = [v + (bump487 if x > lr0_487 + 0.2 else 0.0) for x, v in zip(xb487, tb487)]
        inert_487 = last_sign_change_in_window(xb487, inert487)
        ck("L-487 INERT arm: a shift applied ONLY downstream of the crossing (attached "
           "faces) leaves the last crossing UNMOVED -- the reduction tracks the FEATURE",
           inert_487 is not None and abs(inert_487 - lr0_487) < 1.0e-9)
        ...
        # ---- R3 DOMAIN self-convergence (the NEW layer; the gate is untouched) --
        # A ladder that keeps moving does NOT self-converge.
        d_diverge = domain_self_convergence({"D0": 6.20, "D1": 5.60, "D2": 5.05})
        ck("domain: a still-moving ladder (6.20/5.60/5.05) is NOT_CONVERGED",
           d_diverge["state"] == "NOT_CONVERGED" and d_diverge["d_star"] is None)
        # A ladder that settles within DOMAIN_TOL self-converges and fixes D*.
        d_conv = domain_self_convergence({"D0": 5.20, "D1": 5.00, "D2": 5.005})
        ck("domain: a settled ladder (5.20/5.00/5.005) is SELF_CONVERGED at D* = D2",
           d_conv["state"] == "SELF_CONVERGED" and d_conv["d_star"] == "D2")
        # THE ANSWER-BLIND PROOF: a ladder that settles FAR from the Target 4.0
        # (here at ~5.5) still reports SELF_CONVERGED -- the rule references
        # DOMAIN_TOL only, never REF_LR2T. If this used the Target it would refuse.
        d_far = domain_self_convergence({"D0": 5.60, "D1": 5.50, "D2": 5.499})
        ck("domain: ANSWER-BLIND -- a ladder settling at ~5.5 (far from Target 4.0) is "
           "SELF_CONVERGED (the rule references DOMAIN_TOL only, not the Target)",
           d_far["state"] == "SELF_CONVERGED" and d_far["d_star"] == "D2"
           and REF_LR2T not in (d_far["tol"],) and d_far["tol"] == DOMAIN_TOL)
        # GENUINE-PLATEAU CONTROL (sec.16.2/16.4): a within-tol step FOLLOWED BY an
        # out-of-tol step is a fortuitous single step, NOT a plateau. The detector must
        # be shown able to REJECT it -- a null (`it self-converged`) is trusted only from
        # a reader shown able to see the non-null.
        d_false = domain_self_convergence({"D0": 5.50, "D1": 5.505, "D2": 5.20})
        ck("domain: a FALSE plateau (5.50/5.505/5.20 -- tiny step then a 5.5 %% jump) is "
           "NOT_CONVERGED (the terminal step is out of tol; sec.16.2 genuine plateau)",
           d_false["state"] == "NOT_CONVERGED" and d_false["d_star"] is None)
        # A GENUINE early plateau (both steps within tol) fixes D* at the earliest domain.
        d_early = domain_self_convergence({"D0": 5.50, "D1": 5.495, "D2": 5.49})
        ck("domain: a genuine early plateau (5.50/5.495/5.49, both steps within tol) is "
           "SELF_CONVERGED at the EARLIEST such domain D* = D1",
           d_early["state"] == "SELF_CONVERGED" and d_early["d_star"] == "D1")
        ck("domain: DOMAIN_TOL is DERIVED from the gate band (TOL/10 = 0.010), not observed, "
           "and equals neither the band nor the Target",
           abs(DOMAIN_TOL - 0.01) < 1e-12 and DOMAIN_TOL == TOL / 10.0
           and DOMAIN_TOL != TOL and DOMAIN_TOL != REF_LR2T)
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
...
def verify_frozen():
    ...
    rc = 0
    for rel in ("cases/ansys_verification/VMFL063-R3/PREREGISTRATION.md",
                "cases/ansys_verification/VMFL063-R3/grade_vmfl063_r3.py"):
        ...
        if not same:
            rc = 2
    # GATE PROVENANCE (L-487): the R2 comparator this file carries VERBATIM must be intact
    # and byte-identical to the recorded blob, else the "gate == R2" claim is unfounded.
    r2disk = subprocess.run(["git", "-C", repo, "hash-object",
                             os.path.join(repo, R2_COMPARATOR_REL)],
                            capture_output=True, text=True).stdout.strip()
    r2ok = (r2disk == R2_COMPARATOR_BLOB)
    print("%-64s disk=%s expect=%s %s"
          % (R2_COMPARATOR_REL, r2disk or "?", R2_COMPARATOR_BLOB,
             "OK (gate carried byte-identical)" if r2ok else "MISMATCH -- gate provenance"))
    if not r2ok:
        rc = 2
    return rc
...
def main(argv):
    ...
    # OFF-GATE launcher helper: grade only the domain ladder and print D*.
    if "--pick-dstar" in argv:
        return pick_dstar(run_root)

    R = {"case": "VMFL063-R3", "manual_page": 193,
         ...
         "field_classes": FIELD_CLASSES, "run_root": run_root,
         "domain_ladder": DOMAINS, "domain_tol": DOMAIN_TOL,
         "domain_tol_rule": DOMAIN_TOL_RULE,
         "gate_provenance": {"r2_comparator": R2_COMPARATOR_REL, "r2_blob": R2_COMPARATOR_BLOB},
         "domains": {}, "levels": {}}
    ...
    # ========================================================================
    # R3 DOMAIN LADDER (NEW; off the gate path). Each domain is one completed
    # solve at the FIXED R2-L3 resolution with the de-confined top BC. The reader
    # _case_lr2t() uses the SAME functions the gate uses -- LR is read identically
    # everywhere; the gate verdict is still decided only on the D* grid triple.
    # ========================================================================
    lr2t_dom = {d: _case_lr2t(run_root, d, R["domains"]) for d in DOMAINS}
    # BC-isolation diagnostic (does NOT gate): D0 de-confined vs D0 with the OLD
    # symmetryPlane top. It isolates the top-BC-type effect at fixed domain/mesh.
    bc_conf = _case_lr2t(run_root, BC_ISOLATION, R["domains"])
    R["bc_isolation"] = {"D0_deconfined_lr2t": lr2t_dom["D0"], "D0_confined_lr2t": bc_conf,
                         "confinement_delta_lr2t": lr2t_dom["D0"] - bc_conf,
                         "note": "diagnostic only; does not enter the gate or the band"}

    dsc = domain_self_convergence(lr2t_dom)
    R["domain_self_convergence"] = dsc

    print("VMFL063-R3  (VM2026R1 p.193)  DOMAIN-INDEPENDENCE LADDER (fixed R2-L3 grid):")
    for d in DOMAINS:
        print("  %-3s LR/(2t) = %.6f" % (d, lr2t_dom[d]))
    print("  BC isolation: D0 de-confined %.6f vs D0 confined %.6f  (delta %.6f)"
          % (lr2t_dom["D0"], bc_conf, lr2t_dom["D0"] - bc_conf))
    print("  self-convergence: %s, D* = %s, DOMAIN_TOL = %g (references neither Target nor band)"
          % (dsc["state"], dsc["d_star"], DOMAIN_TOL))

    if dsc["state"] != "SELF_CONVERGED":
        R["verdict"] = "NOT A RESULT"
        R["verdict_note"] = ("domain ladder did NOT self-converge within the frozen extents "
                             "(DOMAIN_TOL=%g); the ladder is NOT extended (PREREGISTRATION "
                             "sec.4.4, outcome 5) and the domain question is escalated, not "
                             "tuned" % DOMAIN_TOL)
        print("  ROW VERDICT: NOT A RESULT  (domain NOT_CONVERGED; the gate is not graded "
              "without a D*)")
        if out_path:
            fh = open(out_path, "w")
            try:
    ...
    # ========================================================================
    # THE GATE -- BYTE-IDENTICAL TO R2, evaluated at D* under run_root/GATE.
    # Everything below is the R2 gate path, unchanged but for the case-id string
    # and pointing at gate_root instead of run_root (PREREGISTRATION sec.5).
    # ========================================================================
    gate_root = os.path.join(run_root, GATE_SUBDIR)
    R["gate_root"] = gate_root
    R["d_star"] = dsc["d_star"]

    # ---- strict completion at every level, twin included ----------------------
    for lv in LEVELS + [DET_TWIN]:
        R["levels"][lv] = completion(gate_root, lv)
    ...
    for lv in LEVELS:
        ...
        physical_range_refuse(lr, lr / TWO_T, lv)
        lr_u = last_sign_change_in_window(*nearwall_u_profile(c["level_dir"], c["time_dir"]))
        if lr_u is None:
            raise SystemExit2("%s: the INDEPENDENT near-wall u_x instrument finds no "
    ...
    print("VMFL063-R3  (VM2026R1 p.193)  ROW VERDICT: %s   [gate at D* = %s]"
          % (R["verdict"], R["d_star"]))
