#!/usr/bin/env python3
"""VMFLGPU001-R2 comparator -- THE LAB'S GPU SOLVER PATH, exercised on flow between
...
R2 SUCCESSOR NOTE (fresh registration; VERIFICATION_CHARTER 2c/2d, ANSYS_VERIFICATION
_CHARTER 6).  This is the successor to VMFLGPU001 (R1), register row #33, which was
`NOT A RESULT`: R1's FROZEN plateau clause I5 refused on the L1 probe because a
perfectly converged channel and a dead one look identical to a peak-to-peak tolerance.
Sanaa's 2026-08-27 anti-gaming clause (frozen gates never edited post-compute;
converged-but-wrong = NOT HELD, never a parameter hunt) is WHY R1's clause stands and
why R2 is a fresh registration rather than an amendment.  R2 changes ONLY the CONTROLS:
(a) the plateau gets a LIVENESS FLOOR (a dead channel that never moved refuses; a
demonstrably-live channel is no longer refused merely because its last window is
bit-identical), and (b) limb A's GPU tell is re-based on PETSc's GPU %F table value
(R1's tell1/tell3 were loose/miscalibrated for this build, exactly as VMFLGPU002
Post-compute Amendment 5 found and the supervisor ruled).  EVERY gate constant --
reference, band, ceiling, mesh family, cap, endTime, gate quantity -- is byte-identical
to R1.  Every guard here is DRIVEN in --selftest (L-314), never merely read.

    limb A  GPU EXECUTION      GPU %F table value + tell2 + the forced-CPU control
...
# R2 LIVENESS FLOOR (registered BEFORE compute; a NUMBER, not a measured range).
# The plateau reads v_theta at TRIPLE_RADIUS; the initial field is at rest
# (v_theta = 0) and a correct solve MUST traverse to the exact analytic value.
# The floor is 10 %% of that EXACT value -- DERIVED FROM THE PHYSICS (the closed
# form), NOT from R1's measured range (4.639e-3 m/s, which would be fitting).  A
# dead channel (full-history range 0) fails; the real solve (range ~4.5e-3)
# clears it ~10x; a barely-moved channel fails.  = 4.547810e-04 m/s.
LIVENESS_FLOOR = 0.1 * abs(v_exact(TRIPLE_RADIUS))
...
def iterative_convergence(level_dir):
    ...
    # ---- R2 LIVENESS CONTROL (replaces R1's unconditional null-range refusal) --
    # LIVENESS FIRST: a channel that never moved cannot be certified as
    # converged.  R1's I5 refused on a NULL last-window range, which cannot tell
    # a DEAD channel from a PERFECTLY CONVERGED one -- and it over-refused the L1
    # channel (row #33), which had risen from ~0 to 4.639e-3 and then gone
    # bit-identical.  R2 gates liveness on the WHOLE history: the full-history
    # range must clear the registered LIVENESS_FLOOR (a physics-derived number).
    # This is what R1 lacked, and it still REFUSES a dead channel.
    full_range = max(vals) - min(vals)
    if full_range < LIVENESS_FLOOR:
        refuse("I5", "%s: the probe channel is DEAD -- its full-history range %g m/s is "
                     "below the registered liveness floor %g m/s. A channel that never "
                     "moved cannot be certified as converged (R2 liveness control; the "
                     "control R1's row #33 lacked). Liveness is gated on the WHOLE "
                     "history, which peak-to-peak over the last window cannot see."
               % (name, full_range, LIVENESS_FLOOR))
    ...
    # A NULL last-window range (ptp == 0) is NOT refused when the channel is
    # demonstrably LIVE (full_range >= LIVENESS_FLOOR): that is a perfectly
    # converged double-precision fixed point -- exactly the L1 channel R1's I5
    # over-refused.  The liveness leg above, not peak-to-peak, separates dead
    # from converged.
    return dict(level=name, final_residuals=finals, residual_tol=RES_TOL,
                plateau_ptp=ptp, plateau_tol=PLATEAU_TOL,
                full_history_range=full_range, liveness_floor=LIVENESS_FLOOR,
...
# R2 LIMB-A READER (VMFLGPU002 Post-compute Amendment 5, applied fresh here):
# the GENUINE GPU-work discriminator reads PETSc's GPU %F table VALUE (0 on a CPU
# solve, > 0 on a GPU one), NOT tell1's arm-independent -log_view legend.
...
def gpu_flops_on_device(solver_log_text):
    """True iff a GPU-eligible -log_view event row reports GPU %F > 0.  Gated on
    the presence of the CpuToGpu column so the last field is known to be GPU %F;
    absent columns -> False (fail-safe)."""
    if not re.search(r"CpuToGpu", solver_log_text):
        return False
    for line in solver_log_text.splitlines():
        t = line.strip()
        if not _GPU_EVENT_RE.match(t):
            continue
        parts = t.split()
        try:
...
def limb_A(run_root, level):
    ...
    # R2 limb A (VMFLGPU002 Post-compute Amendment 5, applied fresh): the GPU
    # tell is PETSc's GPU %F table VALUE, 0 on a CPU solve and > 0 on a GPU one.
    # R1's tell1 matched the arm-INDEPENDENT -log_view legend (fires on both
    # arms of a CUDA build) and tell3 expected a `type: aijcusparse` ksp_view
    # line this build echoes only in the options block -- both miscalibrated for
    # this build.  The forced-CPU control refuses only on GENUINE device work in
    # the control arm; tell2 (a PID holding device memory) is kept; tell3 is
    # dropped from the conjunction.  Limb A's MEANING is unchanged.
    ...
    return dict(level=level, gpu_flops_on_device=True, tell2_pid_on_gpu=True,
                ...
                tell1_note="tell1 (legend) NOT USED: loose on a CUDA build",
                tell3_note="tell3 (ksp_view type) DROPPED: options-block echo on this build")
...
def grade(run_root, out_json=None):
    ...
    for lvl in LEVELS:
        ...
        print("  LIMB A HELD at %s: the GPU arm shows GPU %%F > 0 on -log_view event "
              "rows and holds device memory (tell2), and the forced-CPU control showed "
              "GPU-ABSENT (R2 limb-A control)" % lvl)
        rec["levels"][lvl] = lrec
...
def _build_level(root, arm, level, *, gpu_tells=True, end_line=True, with_rc=True,
                 vfun=None, gpusample=True, petsc_init=False, alive_null=False):
    ...
    _write(os.path.join(d, "postProcessing", "radialProbes", str(et),
                        "%s_p_U.xy" % SET_NAME), "\n".join(rows) + "\n")
    # per-iteration probe series: a LIVE channel that RISES from ~0 to vf(r)
    # over the first half of the run (so its full-history range ~= vf, which
    # EXERCISES the R2 liveness floor rather than sidestepping it), then settles.
    # alive_null: settle to a BIT-IDENTICAL value (last-window ptp exactly 0) --
    # a perfectly converged live channel, exactly what R1's I5 over-refused and
    # what R2 must now PASS.  Otherwise settle with a tiny sub-tolerance wobble.
    pr = []
    for i in range(1, et + 1):
        frac = min(1.0, 2.0 * i / et)
        if 2 * i > et:
            wob = 0.0 if alive_null else 1e-9 * ((i % 3) - 1)
        else:
            wob = 0.0
        pr.append("%d " % i + " ".join("(%.17g %.17g %.17g)" % (0.0, vf(r) * frac + wob, 0.0)
                                       for r in RADII))
...
def drive_refusal(kind):
    ...
    try:
        if kind == "endline":
            ...
            # A DEAD channel: EVERY iteration bit-identical from the start, so
            # the full-history range is EXACTLY 0 -- the channel never moved.
            # R2's liveness floor (I5) must REFUSE it. This is what R1's
            # null-range refusal did too, but R2 refuses it on the WHOLE history,
            # which is what lets R2 PASS a live channel with a null last window.
            _build_run(tmp)
            f = os.path.join(tmp, GPU_ARM, LEVELS[0], "postProcessing", PROBE_FO, "0", "U")
            ...
            # A GPU arm log that LOOKS GPU-configured -- cusparse type AND the
            # -log_view CpuToGpu legend, so R1's loose tell1/tell3 both fire --
            # but with GPU %F = 0 on every event row: it ran on the CPU. The
            # repaired reader reads the VALUE and REFUSES (limb A). The forged
            # mutation the supervisor and Sanaa require (L-314).
            _build_run(tmp)
            for lvl in LEVELS:
                ...
                _write(f, body +
                       ...
                       "End\n")
            grade(tmp)
        elif kind == "norc-noend":
            # R-RC (Sanaa 2026-08-27): an absent rc RECORD is infrastructure and
            # gives NOT MEASURED (not a refusal) -- ONLY WHEN the other four
            # rule-4 conditions hold. Here rc is absent AND the End line is gone,
            # so the conjunction is broken and completion must REFUSE (C4), never
            # a blanket pass on a missing rc file.
            _build_run(tmp, end_line=False)
            os.remove(os.path.join(tmp, GPU_ARM, LEVELS[0], "RUN_RC.txt"))
            os.remove(os.path.join(tmp, "RUN_RC.%s.%s" % (LEVELS[0], GPU_ARM)))
            grade(tmp)
    ...
    try:
        if kind == "petsc-exec-shape":
            ...
            _write(os.path.join(tmp, "COST.txt"),
                   ...
                   "cost_basis = derived, not measured\n" % CASE)
            grade(tmp)
        elif kind == "live-converged":
            # A clean CONVERGING run whose probes RISE to vf(r) then go
            # BIT-IDENTICAL (last-window ptp EXACTLY 0) -- a perfectly converged
            # LIVE channel. R1's I5 refused exactly this (row #33 L1). R2's
            # liveness floor separates dead from converged, so this must GRADE
            # to the tier ceiling, NOT refuse. The forward fix, driven.
            for lvl, sc in zip(LEVELS, (4e-3, 1e-3, 2.5e-4)):
                _build_level(tmp, GPU_ARM, lvl, vfun=_converging_vfun(sc), alive_null=True)
                _build_level(tmp, CPU_ARM, lvl, gpu_tells=False,
                             vfun=_converging_vfun(sc), alive_null=True)
            _write(os.path.join(tmp, "LAUNCH_RECORD.txt"),
                   "case_id = %s\nprereg_sha_head = %s\nprereg_sha_disk = %s\n"
                   "comparator_sha_head = %s\ncomparator_sha_disk = %s\n"
...
def selftest():
    ...
    try:
        ...
        chk("a DEAD channel (full-history range 0 -- never moved) REFUSES on the "
            "R2 liveness floor %g m/s, which peak-to-peak over the last window "
            "cannot see" % LIVENESS_FLOOR,
            _drives_exit2("dead-channel"))
        chk("a LIVE channel that converged to a bit-identical fixed point (null "
            "LAST-window range but full-history range above the liveness floor) "
            "GRADES to the tier ceiling -- R2 resolves R1's I5 over-refusal (row #33)",
            _drives_verdict("live-converged", TIER_CEILING))
        ...
        chk("a FORGED GPU-arm log with GPU %F = 0 (cusparse type + -log_view "
            "legend present, so R1's loose tell1/tell3 would fire) REFUSES -- the "
            "R2 reader reads the GPU %F table VALUE (forged mutation, L-314)",
            _drives_exit2("gpu-zero-pctf"))
        chk("R-RC conjunction: an absent rc RECORD with the End line ALSO gone "
            "REFUSES (C4) -- absent rc is infrastructure-only when the other four "
            "rule-4 conditions hold, never a blanket pass (Sanaa 2026-08-27)",
            _drives_exit2("norc-noend"))
