#!/usr/bin/env bash
# VMFL051-R3 -- Isentropic Expansion of Supersonic Flow Over a Convex Corner.
# SUCCESSOR to VMFL051-R2 (register Row #73, NOT A RESULT -- OSCILLATORY triple).
#
# The SOLVE is byte-identical to R2 / run 1: same three-level r=2 family, same
# endTime (7.0e-3 s), same scheme, same 0/ fields, same thermophysical and
# turbulence dictionaries.  The ONLY changes from R2 are the two SAMPLING
# dictionaries (system/topoSetDict adds the cross-plane faceZones postExpPlane
# and postExpPlane2; system/controlDict.template adds the pStag exprField and the
# two flux-weighted surfaceFieldValue FOs) and the COMPARATOR (grade_vmfl051_r3.py).
# This script builds and runs the family; it does NOT grade anything.
#
# GUARDS, in order:
#   1. FREEZE-PIN (rule 2): the pre-registration AND the comparator on disk must
#      be BYTE-IDENTICAL to their committed HEAD blobs.  No solver starts otherwise.
#   2. AGE GUARD (rule 4): refuses if the run directory for a level already exists.
#   3. refuses if topoSet did not put faces into the GATE cross-plane faceZone
#      (postExpPlane) -- an empty plane reads as a number and must never be graded
#      -- and refuses on a face count above ny (a single x-face column in the band
#      cannot exceed ny; a larger count means topoSet caught more than one column).
...
# one statement after the solver call (the setsid-parent-returns-zero trap).
set -u -o pipefail

REPO="/home/ubuntu/Certonomous"
CASE_SRC="${REPO}/cases/ansys_verification/VMFL051-R3/case"
RUN_ROOT="${REPO}/verification/runs/ansys_verification/VMFL051-R3"
PREREG_REL="cases/ansys_verification/VMFL051-R3/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL051-R3/grade_vmfl051_r3.py"

CAP_CORE_MIN=28          # pre-registered CEILING; UNCHANGED from R2 (which ran the
                         # identical solve in 9.8 core-min, 35% of cap).  An
                         # overrun STOPS the run; it does not get a new budget.
RANKS=1                  # serial: core-minutes = wall_s * RANKS / 60;
                         # timeout seconds = CAP_CORE_MIN * 60 / RANKS.

ENDTIME="0.007"          # s -- IDENTICAL at every level and UNCHANGED from R2
WRITEINTERVAL="0.0035"   # s
...
# the GATE cross-plane faceZone that must be non-empty and single-column
GATE_ZONE="postExpPlane"
DIAG_ZONE="postExpPlane2"
...
for spec in $LEVELS; do
  ...
  # --- guard 3: the GATE cross-plane faceZone must be non-empty & single-column
  ng=$(grep -oE "${GATE_ZONE} now size [0-9]+" "$d/log.topoSet" | tail -1 | grep -oE '[0-9]+$')
  [ -n "${ng:-}" ] || refuse "${name}: cannot read the size of ${GATE_ZONE} from log.topoSet"
  [ "$ng" -gt 0 ] || refuse "${name}: the frozen GATE cross-plane ${GATE_ZONE} is EMPTY -- an empty plane reads as a number and must never be graded"
  [ "$ng" -le "$NY" ] || refuse "${name}: ${GATE_ZONE} holds ${ng} faces > ny=${NY} -- a single x-face column in the y-band cannot exceed ny; topoSet caught more than one column"
  echo "${name}: ${GATE_ZONE} holds ${ng} faces (ny=${NY})"
  nd=$(grep -oE "${DIAG_ZONE} now size [0-9]+" "$d/log.topoSet" | tail -1 | grep -oE '[0-9]+$')
  [ -n "${nd:-}" ] || refuse "${name}: cannot read the size of ${DIAG_ZONE} from log.topoSet"
  [ "$nd" -gt 0 ] || refuse "${name}: the diagnostic cross-plane ${DIAG_ZONE} is EMPTY"
  echo "${name}: ${DIAG_ZONE} holds ${nd} faces"
...
echo "grade with: python3 cases/ansys_verification/VMFL051-R3/grade_vmfl051_r3.py"
