...
VERSION = "VMFLGPU005-comparator-1.0"
...
# ==========================================================================
# PLANTED-ZERO CONTROL (rule 3). Plant into a COPY of the mid-height traverse at
# the row C1b's max() SELECTS, read it back, refuse if the reader cannot see it.
# ==========================================================================
def planted_zero_control(rows, clause="P1"):
    base = c1b_vmax(rows)
    idx = max(range(len(rows)), key=lambda i: rows[i][1])   # the argmax row C1b reads
    planted = list(rows)
    x, uy, t = planted[idx]
    planted[idx] = (x, uy + PLANT, t)
    moved = c1b_vmax(planted)
    # NEGATIVE arm: an unplanted copy must see no move.
    same = c1b_vmax(list(rows))
    if abs(same - base) > 0.0:
        refuse(clause, "the NEGATIVE arm moved: an unplanted copy read %.9g against "
                       "a baseline of %.9g. The reader is not deterministic and no "
                       "zero it reports is evidence." % (same, base))
    delta = abs(moved - base)
    expect = PLANT
    if delta <= PLANT_MIN_FRACTION * expect:
        refuse(clause, "PLANT-BLIND at the argmax row (index %d, x=%.5f): a plant of "
                       "%.6g m/s should have moved max U_y by ~%.6g but it moved by "
                       "%.6g. The reader cannot see a known non-zero, so a zero from "
                       "it is not evidence (rule 3)." % (idx, rows[idx][0], PLANT, expect, delta))
    return dict(row=idx, plant=PLANT, expected=expect, observed=delta)
...
def grade(run_root):
    ...
    for level, cells in LEVELS:
        ...
        control = planted_zero_control(mid_g, "P1")
...
def selftest():
    ...
    # planted control fires
    okp = False
    try:
        rows = [(0.001 * i, 0.10 + 0.001 * i, 290.0 + 0.01 * i) for i in range(50)]
        r = planted_zero_control(rows)
        okp = r["observed"] > PLANT_MIN_FRACTION * r["expected"]
    except Refusal:
        okp = False
    check(okp, "planted_zero_fires", "a %g m/s plant moves max U_y by the expected amount" % PLANT)
    # planted control REFUSES a blind reader
    okpb = False
    try:
        planted_zero_control_blind = planted_zero_control  # same fn; feed a reader that ignores the plant
    except Exception:
        pass
