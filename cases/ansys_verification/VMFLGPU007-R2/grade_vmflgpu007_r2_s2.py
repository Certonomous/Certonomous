...
VERSION = "VMFLGPU007-R2-comparator-1.0-s2"
# ---------------------------------------------------------------- SUCCESSOR S2
# S2 (2026-08-28) is grade_vmflgpu007_r2.py's SUCCESSOR under CLAUDE.md rule 2:
# the parent (blob aae498976139cd8489c9b3de48427f1f425f875c) is FROZEN and is
# NOT edited. S2 differs from it in ONE thing only -- the planted-zero control
# (rule 3) -- rebuilt to meet Sanaa's 2026-08-28 birth requirement: the plant
# must travel the REAL production path (written into the real heated-wall sample
# file on disk, read BACK through the parent's own unmodified read_wall_T()),
# not injected downstream of the reader as the parent did (it mutated the
# already-parsed rows and re-ran only the nusselt/peak_of reducer). Every band,
# threshold, ceiling, tier, limb, refusal clause, completion clause, Roache
# classifier and physics constant is byte-identical to the parent; the only
# other differences are this header, the two imports added above, the one
# call-site line that feeds the control a case directory and endTime instead of
# parsed rows, and the control's --selftest arms.
...
# only the nusselt/peak_of reducer, so it exercised an argmax reducer, never the
# FILE reader. S2 plants a SIZED offset into the T column of EVERY row of a COPY
# of the real heated-wall sample file, reads it BACK FROM DISK THROUGH the
# parent's own unmodified read_wall_T(), and then runs the SAME planted file
# through the FULL gate functional nusselt()/peak_of(). Sized to the channel (a
# fraction of the in-window max wall-temperature rise, never a fixed absolute an
# averaging reader could dilute -- L-340), and applied to EVERY row so it can
# never land outside the reader's support (L-347). Refuses via refuse() (never
# assert; python3 -O safe); the --selftest suppress arm proves it can FAIL, not
# merely pass. Raising T UNIFORMLY lowers every Nu, so peak Nu (the gate
# quantity) must move DOWN by the plant -- the gate-functional sensitivity check.
# ==========================================================================
def planted_zero_control(gpu_dir, endtime, clause="P1", _selftest_suppress_plant=False):
    plant_k = 0.05          # plant size as a fraction of the in-window max dT
    rb_tol = 1.0e-9         # read-back tolerance, relative to the plant scale
    base_dir = os.path.join(gpu_dir, "postProcessing", "wallT", endtime)
    if not os.path.isdir(base_dir):
        refuse(clause, "the gate reader directory %s does not exist; the "
                       "planted-zero control has no real production artifact to "
                       "travel through." % base_dir)
    ...
    src = one_match(os.path.join(base_dir, "*heatedWall*.raw"), clause,
                    "the heated-wall temperature sample (planted-zero source)")
    base_rows = read_wall_T(gpu_dir, endtime, clause)
    base_nu = nusselt(base_rows, clause)
    base_peak, _ = peak_of(base_nu)
    dTs = [tw - T_INLET for _xh, tw in base_rows]
    signal = max(dTs)
    if signal <= 0.0:
        refuse(clause, "the heated-wall temperature-rise channel is non-positive "
                       "everywhere, so there is nothing to size a plant to.")
    plant = plant_k * signal
    dT_min = min(dTs)

    def _plant_T_column(path, delta):
        out = []
        for line in open(path):
            s = line.strip()
            if not s or s.startswith("#"):
    ...
    tmp = tempfile.mkdtemp(prefix="vmflgpu007_r2_s2_plant_")
    try:
        ...
        # THROUGH read_wall_T() -- proving the reader is deterministic.
        neg_rows = read_wall_T(tmp, endtime, clause)
        if len(neg_rows) != len(base_rows) or any(
            ...
            refuse(clause, "the NEGATIVE arm moved: an unplanted copy of %s did "
                           ...
                           "read_wall_T(). The reader is not deterministic and no "
                           "zero it reports is evidence." % src)
        # PLANT into EVERY T on the real bytes (unless the selftest suppresses it
        # to prove the control can FAIL). The EXPECTATION is always `plant`.
        if not _selftest_suppress_plant:
            _plant_T_column(dst, plant)
        seen_rows = read_wall_T(tmp, endtime, clause)
        ...
        if not (worst <= rb_tol * scale):
            refuse(clause, "PLANT-BLIND (P1a, reader sensitivity): a plant of "
                           "%.9g K was written into the T column of EVERY row of "
                           "%s, but read back through read_wall_T() the worst "
        ...
        # P1b: the SAME planted file through the FULL gate functional. A uniform
        # +plant on T lifts every dT, so peak Nu = q''H/(k*(dT_min+plant)) DROPS.
        seen_nu = nusselt(seen_rows, clause)
        seen_peak, _ = peak_of(seen_nu)
        delta = base_peak - seen_peak
        expect = abs(Q_FLUX * H_STEP / (KAPPA * dT_min)
                     - Q_FLUX * H_STEP / (KAPPA * (dT_min + plant)))
        if expect <= 0.0:
            refuse(clause, "the planted perturbation is not resolvable on this "
                           "channel: the expected peak-Nu move is %.6g." % expect)
        if delta <= PLANT_MIN_FRACTION * expect:
            refuse(clause, "PLANT-BLIND (P1b, gate-functional sensitivity): a "
                           "uniform plant of %.9g K into T should have moved peak "
                           "Nu (the gate quantity) DOWN by ~%.9g but it moved by "
                           "%.9g. A plant the raw reader sees but the gate does "
                           "not is the register-row-#31 failure (rule 3)."
                           % (plant, expect, delta))
        return dict(row=len(base_rows), x_over_H=base_rows[0][0], plant=plant,
                    expected=expect, observed=abs(delta),
                    worst_readback=worst, n_rows=len(base_rows), file=src)
    finally:
...
def grade(run_root):
    ...
    for level, cells in LEVELS:
        ...
        controls[level] = planted_zero_control(gd, et_g, "P1")
...
def selftest():
    ...
    # ---- PLANTED-ZERO CONTROL (rule 3) -- SUCCESSOR S2: DRIVEN TO BOTH -------
    # OUTCOMES on a REAL-SHAPED wallT file written to disk, read back through the
    # parent's own read_wall_T().  The parent's flat-profile `p1_plant_blind`
    # arm is retired: a uniform every-row plant is not defeated by a flat
    # profile, so the P1a refusal is driven instead by the SUPPRESS arm below,
    # which withholds the plant from disk along the real production path.
    _pdir = os.path.join(root, "s2_plant_fixture")
    _make_arm(_pdir, "L1", "gpu", LEVEL_ENDTIME["L1"], REF_PEAK_NU)
    _pgpu = os.path.join(_pdir, "gpu", "L1")
    _pet = str(LEVEL_ENDTIME["L1"])
    ok_suppress = False
    try:
        planted_zero_control(_pgpu, _pet, "P1", _selftest_suppress_plant=True)
    except Refusal as _r:
        ok_suppress = "PLANT-BLIND (P1a" in str(_r)
    results.append((ok_suppress, "p1_refuses_when_plant_suppressed",
                    "with the plant suppressed from disk the control refuses P1a "
                    "-- the control is shown able to FAIL, not merely to pass"))
    ...
    # ---- THE PLANT FIRES ON THE REAL-SHAPED DISK FIXTURE (positive arm) -----
    ok_p = False
    try:
        pc = planted_zero_control(_pgpu, _pet, "P1")
        ok_p = pc["observed"] > PLANT_MIN_FRACTION * pc["expected"] > 0.0
        detail_p = ("a plant sized to the wall-temperature-rise channel, written "
                    "into EVERY row on disk and read back through read_wall_T(), "
                    "moved peak Nu by %.6g against an expected %.6g"
                    % (pc["observed"], pc["expected"]))
    except Exception as exc:
        detail_p = "CRASHED: %r" % (exc,)
    results.append((ok_p, "p1_plant_fires_on_disk_fixture", detail_p))
