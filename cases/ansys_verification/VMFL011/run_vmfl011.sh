#!/usr/bin/env bash
# VMFL011 graded-run driver -- Laminar Flow in a Triangular Cavity (manual p.41), 2D
#
...
#
# Usage:  run_vmfl011.sh <run_root>            graded run
#         run_vmfl011.sh <scratch_root> smoke  exercises THIS LAUNCHER end-to-end
...
RUN_ROOT="${1:?usage: run_vmfl011.sh <run_root> [smoke]}"
MODE="${2:-graded}"
...
PREREG_REL="cases/ansys_verification/VMFL011/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL011/grade_vmfl011.py"
...
for L in $LEVELS; do
  ...
  sed -e "s/__NB__/${A[$L]}/g" -e "s/__NH__/${B[$L]}/g" "$OUT/system/blockMeshDict.template" > "$OUT/system/blockMeshDict" || { echo "ABORT sed blockMeshDict"; exit 1; }
...
PYBC
  ...
  T0=$(date +%s)
  # DETACHED SOLVER (supervisor course-correction 2026-08-26).  setsid makes the solver
  # its OWN session leader so it survives the launching lane's death -- the failure mode
  # that killed work last night.  Backgrounded + waited so rc is still captured on BOTH
  # paths.  Detachment is VERIFIED by the WRAPPER's SID == its own PID (field 6 of
  # /proc/<pid>/stat), never by the solver's ppid (a solver whose ppid is the timeout pid
  # is what CORRECT setsid looks like, so ppid proves nothing).
...
echo "VMFL011 $MODE complete (${SPENT}/${CAP_CORE_MIN} core-min) -> grade with grade_vmfl011.py --run-root $RUN_ROOT"
