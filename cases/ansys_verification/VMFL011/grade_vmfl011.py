#!/usr/bin/env python3
"""grade_vmfl011.py -- comparator for VMFL011, Laminar Flow in a Triangular Cavity
(Ansys Fluid Dynamics Verification Manual, VM2026R1, p.41).

FROZEN GRADING PATH (CLAUDE.md rule 2).  The gate, the band and the tier ceiling
are fixed in cases/ansys_verification/VMFL011/PREREGISTRATION.md and are committed
...
BAND_RMS = 0.030          # frozen; justified in PREREGISTRATION.md sec.6
...
def _xy(level_dir):
    f = sorted(glob.glob(os.path.join(level_dir, 'postProcessing', 'bisector', '*',
                                      'bisect_U.xy')))
    if not f:
        raise RuntimeError('no bisect_U.xy under %s' % level_dir)
    return f[-1]
...
def controls(level_dir):
    return {'rms_vs_benchmark': planted_zero(_xy(level_dir), _perturb, _read_rms,
                                             PLANT, 'rms_vs_benchmark'),
            'u_min_norm': planted_zero(_xy(level_dir), _perturb, _read_umin,
...
def main():
    ...
    R = {'case': 'VMFL011', 'manual_page': 41, 'endTime': ENDTIME,
    ...
    out = a.out or os.path.join(a.run_root, 'GRADING_VMFL011.json')
