#!/usr/bin/env bash
# VMFL004-R2 graded-run driver -- Plain Couette Flow with Pressure Gradient (manual p.21),
# 2-D cyclic.  Re-run of VMFL004 (register row #25) with the iterative-convergence channel
# corrected to the driven component (L-338).  The CASE PHYSICS is byte-identical to VMFL004;
# only the grader's convergence channel changed.
#
# NO `set -u`.  It is CATEGORICALLY INCOMPATIBLE with OpenFOAM v2606: sourcing etc/bashrc
# dereferences WM_PROJECT_DIR (bashrc line 184) BEFORE assigning it.  MEASURED: rc 127.
...
#
# THE COST INSTRUMENT (CLAUDE.md rule 12).  RANKS is in BOTH formulae so a parallel copy
# inherits a correct cap automatically:
...
# sec.12.  AN OVERRUN STOPS THE RUN; IT DOES NOT GET A NEW BUDGET.
#
# Usage:  run_vmfl004_r2.sh <run_root>            graded run
#         run_vmfl004_r2.sh <scratch_root> smoke  exercises THIS LAUNCHER end-to-end
#                                            (template Amendment 3 item 6) at L1 with a tiny
#                                            endTime; grades nothing.
RANKS=1
CAP_CORE_MIN=30
ENDTIME=20000
CASE_DIR="$(cd "$(dirname "$0")" && pwd)/case"
RUN_ROOT="${1:?usage: run_vmfl004_r2.sh <run_root> [smoke]}"
MODE="${2:-graded}"
...
PREREG_REL="cases/ansys_verification/VMFL004-R2/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFL004-R2/grade_vmfl004_r2.py"
...
declare -A A=( [L1]=15 [L2]=30 [L3]=60 )
declare -A B=( [L1]=40 [L2]=80 [L3]=160 )
SPENT=0; TOTAL_WALL=0
for L in $LEVELS; do
  OUT="$RUN_ROOT/$L"
  # AGE GUARD (rule 4): refuse a case where 0/ or a time dir already exists.  Time dirs are
  # matched by the REGEX ^[0-9]+(\.[0-9]+)?$ -- NEVER a [0-9]* glob, which matches 0.orig.
...
PYAGE
  ...
  sed -e "s/__NX__/${A[$L]}/g" -e "s/__NY__/${B[$L]}/g" "$OUT/system/blockMeshDict.template" > "$OUT/system/blockMeshDict" || { echo "ABORT sed blockMeshDict"; exit 1; }
...
PYBC
  ...
  # so "field at endTime newer than 0/" is a real test of THIS run.
  touch "$OUT"/0/* || { echo "ABORT: cannot touch $OUT/0"; exit 1; }
  T0=$(date +%s)
  # DETACHED SOLVER (supervisor course-correction 2026-08-26; L-336).  setsid makes the
  # solver its OWN session leader so it survives the launching lane's death.  Backgrounded +
  # waited so rc is still captured on BOTH paths.  Detachment is VERIFIED by the WRAPPER's
  # SID == its own PID (field 6 of /proc/<pid>/stat), NEVER by the solver's ppid (a solver
  # whose ppid is the timeout pid is what CORRECT setsid looks like, so ppid proves nothing).
  ...
  # THE MEASURED rc GOES TO DISK on BOTH the success and abort paths.  Strict completion
  # (rule 4) requires rc = 0, and a rule you cannot evaluate is a rule you are not applying.
  printf 'rc = %d\nlevel = %s\nendTime = %s\nwall_s = %d\nranks = %d\ncore_min = %s\ntimeout_s = %d\nprereg_sha = %s\ncomparator_sha = %s\nfinished_utc = %s\n' \
...
echo "VMFL004-R2 $MODE complete (${SPENT}/${CAP_CORE_MIN} core-min) -> grade with grade_vmfl004_r2.py --run-root $RUN_ROOT"
