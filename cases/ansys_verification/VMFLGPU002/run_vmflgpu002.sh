#!/usr/bin/env bash
# ============================================================================
# run_vmflgpu002.sh -- VMFLGPU002 graded-run driver.
#
# Laminar Flow in a 90 deg Planar Dividing Tee-Junction, manual p.227.
# CPU PARENT: VMFL010.  Object under verification: THE LAB'S GPU SOLVER PATH
...
# read from the surfaceFieldValue function objects registered in
# case/system/controlDict.template, at t == endTime.
#
...
#
# IT GRADES NOTHING.  Grading is grade_vmflgpu002.py, frozen separately and
...
CAP_GPU_H=1.0                 # FROZEN in PREREGISTRATION.md section 11.
CAP_CPU_ARM_CORE_MIN=60       # FROZEN in PREREGISTRATION.md section 11.
# name:NW:ENDTIME  -- NW = cells across the channel width W.  The blockMeshDict
# lays four blocks (inletleg 1x2, junction 1x1, mainleg 1x3, branchleg 3x1 in
# units of W), so TOTAL CELLS = 9*NW*NW exactly: 3600 / 14400 / 57600.  That is
# the CPU parent VMFL010's own mesh family, r = 2, unchanged.
#
# THE endTime VALUES ARE THE PARENT'S MEASURED CONVERGENCE POINTS WITH MARGIN,
# not a guess: VMFL010 reached its residualControl at 729 / 1125 / 1575 SIMPLE
# iterations (VMFL010/RESULTS.md), under the SAME outer numerics this case keeps
# character for character.  1200 / 1600 / 2200 is 1.65x / 1.42x / 1.40x those.
# residualControl is REMOVED here so the run always reaches endTime and rule 4's
# "last time == endTime" clause bites; the comparator judges convergence after.
LEVELS="L1_N20:20:1200 L2_N40:40:1600 L3_N80:80:2200"
...
RUN_ROOT="${1:?usage: run_vmflgpu002.sh <run_root>}"
BUILD_ROOT="${BUILD_ROOT:-$HOME/gpu_build}"
REPO="${REPO:-$HOME/Certonomous}"
PREREG_REL="cases/ansys_verification/VMFLGPU002/PREREGISTRATION.md"
GRADER_REL="cases/ansys_verification/VMFLGPU002/grade_vmflgpu002.py"
...
# REGISTERED CONTROL: OUTLET PRESSURE SYMMETRY (PREREGISTRATION.md section 4.4).
#
# The draft pre-registration's RISK line, promoted from prose to an executable
# guard.  The manual (p.227) holds BOTH exit planes at THE SAME STATIC PRESSURE,
# and the flow split is set by almost nothing else: the two branches compete for
# the same inlet flux, and the only thing telling them how to divide it is the
# pressure they are held at.  An asymmetric outlet p BC -- a typo, a merge, a
# dict copied from a case with a back-pressure -- BIASES THE SPLIT DIRECTLY, and
# NOTHING DOWNSTREAM WOULD SHOW IT: the solve converges, the mesh is clean, the
# residuals fall, and limbs A and B BOTH STILL PASS because both arms would
# carry the SAME bias.  Only limb C moves -- so the case would report a physics
# miss when what it actually had was a boundary condition it never checked.
#
# So it is checked, once per materialised case, BEFORE the mesher runs.
outlet_p_symmetric() {
  local d="$1"
  python3 - "$d/0/p" <<'OPGUARD'
import re, sys
path = sys.argv[1]
try:
    txt = open(path).read()
except OSError as e:
    print("ABORT outlet-p guard: cannot read %s (%s)" % (path, e)); sys.exit(1)
m = re.search(r"\bboundaryField\b\s*\{", txt)
if not m:
    print("ABORT outlet-p guard: no boundaryField block in %s" % path); sys.exit(1)
...
while i < len(txt) and depth:
    ...
    i += 1
if depth:
    print("ABORT outlet-p guard: unbalanced boundaryField in %s" % path); sys.exit(1)
body = txt[start:i-1]
patches, j = {}, 0
while True:
    mm = re.compile(r"([A-Za-z_][\w.]*)\s*\{").search(body, j)
    if not mm: break
    k = mm.end(); depth = 1
    while k < len(body) and depth:
        ...
        k += 1
    patches[mm.group(1)] = body[mm.end():k-1]
    j = k
need = ("mainOutlet", "branchOutlet")
missing = [q for q in need if q not in patches]
if missing:
    print("ABORT outlet-p guard: %s absent from the boundaryField of %s -- this case's split "
          "is DEFINED by two exits held at equal static pressure, and a p file without both of "
          "them is not this case" % (", ".join(missing), path))
    sys.exit(1)
spec = {}
for q in need:
    b = patches[q]
    # NOT anchored to line start: the frozen 0/p writes each patch on ONE line
    # (`mainOutlet { type fixedValue; value uniform 0; }`), and a `^`-anchored
    # regex silently matches nothing there.  MEASURED: the first draft of this
    # guard refused the frozen case for exactly that reason, which is why it was
    # driven against the real file before being committed.
    t = re.search(r"\btype\s+(\w+)\s*;", b)
    v = re.search(r"\bvalue\s+uniform\s+([-+0-9eE.]+)\s*;", b)
    if not t:
        print("ABORT outlet-p guard: patch %s in %s has no `type`" % (q, path)); sys.exit(1)
    if t.group(1) != "fixedValue":
        print("ABORT outlet-p guard: patch %s is `%s`, not `fixedValue`. The manual holds BOTH "
              "exit planes at the SAME STATIC PRESSURE; an exit that is not fixedValue does not "
              "hold one at all, and the flow split is set by exactly this." % (q, t.group(1)))
        sys.exit(1)
    if not v:
        print("ABORT outlet-p guard: patch %s is fixedValue with no `value uniform <x>;`" % q)
        sys.exit(1)
    spec[q] = float(v.group(1))
a, b = spec["mainOutlet"], spec["branchOutlet"]
if a != b:
    print("ABORT outlet-p guard: THE TWO EXITS ARE NOT AT THE SAME STATIC PRESSURE -- mainOutlet "
          "p = %r, branchOutlet p = %r (difference %r). The manual (p.227) holds both exit planes "
          "at the same static pressure and the flow split is set by the pressure difference "
          "between them: an asymmetric exit BC biases the gate quantity directly, and every other "
          "check in this launcher would still pass. Refusing." % (a, b, a - b))
    sys.exit(1)
print("OUTLET PRESSURE SYMMETRY OK: mainOutlet and branchOutlet are both fixedValue p = %r" % a)
OPGUARD
}
...
FAILED=0
for spec in $LEVELS; do
  IFS=: read -r NAME NW ETIME <<< "$spec"
  EXPECT_CELLS=$(( 9 * NW * NW ))
  for arm in gpu cpu; do
    if [ "$arm" = "gpu" ]; then MAT="$GPU_MAT"; VEC="$GPU_VEC"; POPTS="$GPU_PETSC_OPTIONS";
    else                        MAT="$CPU_MAT"; VEC="$CPU_VEC"; POPTS="$CPU_PETSC_OPTIONS"; fi
    D="$RUN_ROOT/$arm/$NAME"
    log "--- $arm / $NAME : NW=${NW} across the width W (${EXPECT_CELLS} cells), endTime=$ETIME, mat=$MAT vec=$VEC"
    ...
    sed -e "s/__2N__/$(( 2 * NW ))/g" -e "s/__3N__/$(( 3 * NW ))/g" -e "s/__N__/${NW}/g" \
    ...
    # ---- REGISTERED CONTROL: outlet pressure symmetry ----------------------
    OPS="$(outlet_p_symmetric "$D")" || { echo "$OPS"; exit 1; }
    case "$OPS" in *ABORT*) echo "$OPS"; exit 1;; esac
    log "  $OPS"
