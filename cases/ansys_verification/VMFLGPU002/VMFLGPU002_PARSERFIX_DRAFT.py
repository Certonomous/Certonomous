#!/usr/bin/env python3
"""VMFLGPU002 -- PARSER FIX, DRAFT ONLY.  NOT APPLIED.  NOT COMMITTED.  NOT A GRADER.

=====================================================================
  DRAFT.  This file grades NOTHING and is on NO grading path.
  It does not import, modify, wrap or monkey-patch grade_vmflgpu002.py.
  grade_vmflgpu002.py is UNTOUCHED by this draft; its blob is 92a82426e25c0ee9556f1d20565b6ee720524aec.
=====================================================================

WHAT IS DEFECTIVE IN THE FROZEN GRADER
--------------------------------------
grade_vmflgpu002.py parses every `key = value` artifact with
...
-------
  ...
  * an EXPLICIT REFUSAL (never a bare `assert`; `python3 -O` strips those,
    ...
    call site asserts it.  There are THREE call sites in grade_vmflgpu002.py:
...
Nothing any verdict depends on.  Measured, per real file, in selftest():
  * every key READ by grade_vmflgpu002.py keeps a BYTE-IDENTICAL value;
  * new keys appear in the LAUNCH_RECORD dict (`smoke_rc`, `end`, `note`,
    `arm`, `level`, `wall_s`, `cap_fired`, `utc`, `rc`) -- none of them is read
    by grade_vmflgpu002.py, and the grader lands only four named LAUNCH_RECORD keys
...
CASE = "VMFLGPU002"
FROZEN_GRADER = "grade_vmflgpu002.py"
FROZEN_BLOB = "92a82426e25c0ee9556f1d20565b6ee720524aec"
RUN_ROOT_DEFAULT = "/home/ubuntu/Certonomous/verification/runs/ansys_verification/VMFLGPU002"
