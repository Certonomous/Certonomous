#!/usr/bin/env python3
# ---------------------------------------------------------------------------
# VMFL072-R3 -- the frozen comparator.
#
...
#
# WHAT CHANGED FROM THE R2 COMPARATOR, AND WHAT DID NOT (R3 9.4).
#   CHANGED: exactly ONE frozen constant -- D_NSTAR, the Limb B reference. R3
#   raises the film precursor h0 from 1e-7 to 1e-5 m (the dewetting remedy,
#   R3 3.8), so the solver's EXACT DISCRETE state h^2(h+h0)=3nu q/gs moves from
#   5.500814601e-04 to 5.468015742732e-04 m. Limb B compares delta_mon to that
#   state, so the reference tracks the solver and Limb B stays exact.
#   UNCHANGED: every FIELD NAME this comparator reads, every reader, the whole
#   verdict logic, the two plants, the monitor window, the completion and
#   geometry refusals, the bands and T_FLOOR. THE REMEDY IS A SCALAR DICTIONARY
#   VALUE (base/0.orig/U: h0 1e-7 -> 1e-5); it writes the SAME files R2 wrote.
#   The disk evidence table above -- proven in R2 by BUILDING AND RUNNING the
#   registered case (register #58: four of five levels wrote it to endTime) --
#   therefore still holds byte-for-byte for R3, so the 39.5 trap that struck the
#   original VMFL072 (a comparator reading Cf_film/magSf_film, names in ZERO
#   code paths) is avoided by CONSTRUCTION here: no field name changed.
#
# NO ROACHE TRIPLE IS COMPUTED HERE. R3 7 declines it with the statement
# CLAUDE.md rule 5 requires. This file contains no Richardson extrapolation, no
# observed order and no GCI, deliberately. It gates level-INDEPENDENCE (R3 6.6)
...
#
# USAGE
#   compare_vmfl072_r3.py --runroot <dir>          grade
#   compare_vmfl072_r3.py --selftest               instrument self-test
# ---------------------------------------------------------------------------
...
# --- the references -------------------------------------------------------
D_MANUAL      = 0.555e-3          # m   R3 1   Ansys VM 2026R1 Table .72.1 target
D_NSTAR       = 5.468015742732e-04 # m  R3 2.4 EXACT DISCRETE state h^2(h+h0)=3nu q/gs, h0=1e-5
D_NUSSELT     = 5.501147914e-04   # m   R3 2.6 h0-free Nusselt; CONTEXT ONLY, never a gate
...
C1_RETENTION_MAX = 5.0e-4         # -   R3 6.4  max over plate |h-dN*|/dN* < 0.05 %
...
def verdict(state):
    ...
    # -- one-way gates (R3 7.2 order 3-4): they can only turn a limb INTO
...
def grade(runroot):
    out = []
    out.append("VMFL072-R3 -- Liquid Water Flow Over a Flat Plate Under Gravity")
    out.append("run root: %s" % runroot)
    out.append("")

    scratch = tempfile.mkdtemp(prefix="vmfl072r3_plant_")
    try:
        levels = {}
        ...
        out.append("     NO ROACHE TRIPLE IS DECLARED (R3 7). No order, no GCI, no")
...
# this file and by R3 3.5, not here.  This banner is deliberate.
# ===========================================================================
...
def selftest():
    ...
    root = tempfile.mkdtemp(prefix="vmfl072r3_st_clean_")
    ...
    for clause in ("C-01", "C-02", "C-03", "C-04", "C-05", "C-06", "C-07",
                   "C-14", "C-15", "C-16", "C-17"):
        r = tempfile.mkdtemp(prefix="vmfl072r3_st_%s_" % clause)
    ...
    r = tempfile.mkdtemp(prefix="vmfl072r3_st_C-13_")
    ...
    def limb(tag, mut, want_a, want_b):
        rr = tempfile.mkdtemp(prefix="vmfl072r3_st_%s_" % tag)
        _build_run(rr, mut)
        try:
            rc, lines = grade(rr)
    ...
    r = tempfile.mkdtemp(prefix="vmfl072r3_st_C-11_")
    _build_run(r, base)
    lv = Level(r, "L3")
    ...
    r = tempfile.mkdtemp(prefix="vmfl072r3_st_plant_")
    ...
    scratch = tempfile.mkdtemp(prefix="vmfl072r3_st_pl_")
    rep = []
    ...
    # HONEST LIMIT, CARRIED SO NO READER OVERRATES THIS CONTROL (R3 6.8):
    ...
    ok.append("DECLARED LIMIT: on the UNIFORM production meshes the unweighted "
              ...
              "there, not two readers (R3 6.8 limit 1)")
    ...
    # ---- report ----------------------------------------------------------
    print("VMFL072-R3 comparator --selftest")
    print("=" * 78)
