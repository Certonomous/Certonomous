#!/bin/bash
# ---------------------------------------------------------------------------
# VMFL072-R3 -- apply a registered level to a case copied from base/.
#
...
#
# EVERY VALUE BELOW IS READ OUT OF VMFL072-R3/PREREGISTRATION.md. Nothing here
# is a choice this script is entitled to make.
#
#   Mesh (R3 6.6). Refinement is in the FILM plane only, ratio r = 2:
...
#     B2  L3 geometry (R3 6.5 -- the bracket is at the GRADED level)
#     C1  L2 geometry (R3 6.4)
#
#   NZ = 1 AT EVERY LEVEL, AND IT IS A MEASUREMENT, NOT AN ASSUMPTION (R3 3.6).
...
#   a planted +1.234e-09 m in one face. See R3 3.6 for the control.
#
#   Inlet state. R3 RAISES h0 to 1e-5 m (the precursor-film remedy, R3 3.8),
#   so the EXACT DISCRETE STATE moves: delta_N* = 5.468015742732e-04 m solves
#   h^2(h+h0) = 3*nu*q/gs with h0 = 1e-5 m, and is -0.602277 % from the h0-free
#   Nusselt delta_N = 5.501147914e-04 m (context only). Limb B's reference IS
#   this dN*, so the shift is exact and non-circular (R3 2.4). Gamma is
#   IDENTICAL at every level -- rho*h_in*U_in = 0.381000000 kg/m/s exactly,
#   which is what R3 6.5's bracket requires. THE 1.30 dN* OVER-THICK INLET IS
#   UNCHANGED (L-487 anti-circularity, at FULL strength):
#     L1 L2 L3  h_in = 1.30 dN*  = 7.1084204656e-04   U_in = 0.537381243
#     B2        h_in = 0.70 dN*  = 3.8276110199e-04   U_in = 0.997993736
#     C1        h_in = 1.00 dN*  = 5.4680157427e-04   U_in = 0.698595615
#
#   Time step (R3 8.1). Fixed, never adjustTimeStep: OpenFOAM computes the
...
#   velocity is 0.998 m/s, not 0.699 m/s. The R2 dt values are UNCHANGED: with
#   the R3 inlet velocities the film Courant stays <= 0.45 < maxCo 0.5 at every
#   level (checked: L1/L2/L3 Co=0.447 on U_eq, B2 0.399 on its inlet).
#     L1 dt 5.00e-03 WI 10  2000 steps  Co_film 0.4471
#     L2 dt 2.50e-03 WI 20  4000 steps  Co_film 0.4471
#     L3 dt 1.25e-03 WI 40  8000 steps  Co_film 0.4471
#     B2 dt 7.8125e-04 WI 64 12800 steps Co_film 0.3992
#     C1 dt 2.50e-03 WI 20  4000 steps  Co_film 0.4471
#
...
case "${LEVEL}" in
  L1) NX=64;  NY=16; NZ=1; H_IN=7.1084204656e-04; U_IN=0.537381243; DT=5.0e-03;    WI=10 ;;
  L2) NX=128; NY=32; NZ=1; H_IN=7.1084204656e-04; U_IN=0.537381243; DT=2.5e-03;    WI=20 ;;
  L3) NX=256; NY=64; NZ=1; H_IN=7.1084204656e-04; U_IN=0.537381243; DT=1.25e-03;   WI=40 ;;
  B2) NX=256; NY=64; NZ=1; H_IN=3.8276110199e-04; U_IN=0.997993736; DT=7.8125e-04; WI=64 ;;
  C1) NX=128; NY=32; NZ=1; H_IN=5.4680157427e-04; U_IN=0.698595615; DT=2.5e-03;    WI=20 ;;
  *)
