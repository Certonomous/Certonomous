#!/usr/bin/env python3
"""F26 -- minimal reader/writer for OpenFOAM ascii volVectorField / volScalarField
files (lineage: cases/F23_HP_WEDGE/foam_io_f23.py, itself from F20), used by
build_f26.py (to write 0/p, 0/T, 0/U from the mesh's own 0/C, and to REMAP
constant/polyMesh/points onto the exact flow-net) and by grade_f26.py (to read
rho, p, T, U at every checkpoint and 0/C, 0/V).  Refusals are `raise`; zero
`assert` (L-332)."""
import re
...
def read_points(path):
    """constant/polyMesh/points (ascii): returns (header_text, ndarray (N, 3), trailer_text)."""
    text = open(path).read()
    m = re.search(r"\n(\d+)\s*\n\(", text)
    if not m:
        raise FieldFormatError("%s: no `N (` vectorField list" % path)
    n = int(m.group(1))
    pos = m.end()
    pat = re.compile(r"\s*\(\s*(%s)\s+(%s)\s+(%s)\s*\)" % (_NUM, _NUM, _NUM))
    out = np.empty((n, 3))
    for k in range(n):
        mm = pat.match(text, pos)
        if not mm:
            raise FieldFormatError("%s: bad point %d of %d" % (path, k, n))
        out[k] = (float(mm.group(1)), float(mm.group(2)), float(mm.group(3)))
        pos = mm.end()
    mm = re.compile(r"\s*\)").match(text, pos)
    if not mm:
        raise FieldFormatError("%s: point list not closed" % path)
    return text[:m.start()], out, text[mm.end():]


def write_points(path, header, pts, trailer):
    body = "\n".join("(%.17g %.17g %.17g)" % tuple(r) for r in pts)
    open(path, "w").write("%s\n%d\n(\n%s\n)%s" % (header, len(pts), body, trailer))
