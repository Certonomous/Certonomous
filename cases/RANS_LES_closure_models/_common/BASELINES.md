...
**Two training-cell counts appear in this programme and they are different**
**quantities, not a discrepancy.** 342014 is the count on the *LES-only* mask
(`k_LES` above the anisotropy floor, so `b_LES` is defined) -- what a model
that never touches `b_RANS` can train on, and the number quoted by
`Ling2016_TBNN`. **341717** is the count on the stricter mask used here, which
also requires `b_RANS` to be finite. The 297 cells between them are all
non-finite `b_RANS`: `b_RANS = tau_RANS/(2 k_RANS) - I/3` is undefined where
the converged RANS `k` underflows although `k_LES` does not (567 such cells
across all 40 cases, 5-52 per case on 34 of them; the extended features and
`b_LES` contribute none). The masks are named `valid_les_only` and `valid` in
`score_prediction.py`; **quote the mask name, never just the count.**
...
Cell mask: k_LES above the anisotropy floor (which defines b_LES) AND b_RANS finite. b_RANS = tau_RANS/(2 k_RANS) - I/3 is undefined where the converged RANS k underflows even though k_LES does not: 567 such cells across the 40 cases, 5-52 per case on 34 of them, and every one of the 567 is a non-finite b_RANS (the extended features and b_LES contribute none). Because this mask needs b_RANS it is STRICTER than the LES-only mask used where a model never touches b_RANS - see the note in this section - so the SST numbers here differ in the fourth decimal from BASELINES.md sections 3-5
