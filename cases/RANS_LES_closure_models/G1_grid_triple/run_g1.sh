#!/bin/bash
# ---------------------------------------------------------------------------
# G1 grid-triple driver.  Invoke as:  /bin/bash run_g1.sh
# (There is deliberately NO shebang: line 1 is the draft banner, and the queue
# entry's launch_cmd names /bin/bash explicitly.)
#
# RC CAPTURE.  Every child's rc is captured on the line AFTER the child, in
# this script's own foreground.  This script contains NO setsid and NO nohup.
# `setsid timeout cmd` exits 0 for EVERY outcome, so an rc taken around a
# setsid line is not the child's rc -- it is the wrapper's.  Detaching is the
# queue daemon's job, not this script's; if anyone later wraps a command here
# in setsid, the rc must be captured INSIDE the detached wrapper.
#
...
CASE_DIR="/home/ubuntu/Certonomous/cases/RANS_LES_closure_models/G1_grid_triple"
RUN_ROOT="/home/ubuntu/closure-data/g1"
FOAM_BASHRC="/usr/lib/openfoam/openfoam2606/etc/bashrc"

# Registered cap (PREREGISTRATION.md section 6).  ranks = 1, so core-min == wall-min.
CAP_CORE_MIN=600
CAP_S=36000

# level  nCells_expected  endTime  timeout_s   -- frozen, must match build_g1.py LEVELS
LEVELS="L1:3840:20000:900 L2:15360:30000:5400 L3:61440:60000:29700"
...
# --- 1. stage (build_g1.py refuses over an existing tree: the age guard) ---
python3 "${CASE_DIR}/build_g1.py" > "${RUN_ROOT}/log.build" 2>&1
RC=$?
log "STAGE rc=${RC}"
[ "${RC}" -eq 0 ] || die "build_g1.py refused; see ${RUN_ROOT}/log.build"
...
for SPEC in ${LEVELS}; do
  ...
  for F in wallShearStress yPlus writeCellVolumes writeCellCentres; do
...
log "NEXT: grade with  python3 ${CASE_DIR}/grade_g1.py"
log "CHAIN rc=0"
exit 0
