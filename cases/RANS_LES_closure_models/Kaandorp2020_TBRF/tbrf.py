...
def _fit_sse(Gv, c, s, gamma, rcond=1e-8):
    """Ridge/pseudo-inverse fit and residual SSE for a batch of nodes.

    The 10-tensor basis is RANK-DEFICIENT on this benchmark: every case is a
    statistically two-dimensional mean flow, and Pope 1975 p. 335 [VERIFIED-PDF]
    states that in two dimensions the basis collapses to three tensors. A plain
    ridge solve with Kaandorp's Gamma = 1e-12 therefore returns coefficients of
    order 1/Gamma along the null directions (measured: training b_rms 0.81, worse
    than predicting b = 0). We use a symmetric eigendecomposition and invert only
    eigenvalues above `rcond * lambda_max`, which is the minimum-norm solution in
    the identifiable subspace. DEPARTURE, disclosed in ../Ling2016_TBNN/RESULTS.md
    sec. 7b.
    """
    G = _unpack(Gv)
    tr = np.trace(G, axis1=1, axis2=2) / NB
    Greg = G + (gamma * np.maximum(tr, 1e-30))[:, None, None] * np.eye(NB)[None]
    w, V = np.linalg.eigh(Greg)
    wmax = np.maximum(w[:, -1:], 1e-300)
    inv = np.where(w > rcond * wmax, 1.0 / np.maximum(w, 1e-300), 0.0)
    g = np.einsum("kab,kb,kcb,kc->ka", V, inv, V, c)
    sse = s - 2.0 * (g * c).sum(1) + np.einsum("ka,kab,kb->k", g, G, g)
    return g, np.maximum(sse, 0.0)
