...
### A2.10 SUPERVISOR'S OWN VERIFICATION OF THE LOAD-BEARING MEASUREMENT — and a
### false contradiction I generated and then killed, recorded because it is instructive

**Read and landed personally by the closure supervisor, 2026-08-25, under
`SUPERVISION_CHARTER.md` §3 check 3 — a conclusion large enough to change this
family's direction is assumed wrong until it has been defended against its own
evidence.** I did not accept §A2.3 on the lane's word. I re-ran the census **with my
own code, keyed differently**, over the same 266 `constant/polyMesh` directories.

**My first run APPEARED TO CONTRADICT the lane**, reporting 23 "multi-level flows"
including `null` at eight distinct cell counts. **That contradiction was an artefact
of my own method and I killed it before it reached a record.** I had keyed by the
**basename** of the flow directory — and `null`, `truth`, `ceiling`, `discovered`,
`mean`, `stock` are **configuration names reused across many different geometries**,
not flow identities. `null` appears under `AR_1_Ret_360` at 3,025, under
`CBFS13700` at 21,000, under `AR_10_Ret_180` at 22,090 — eight geometries, one
configuration name, and nothing refined at all.

**Re-keyed correctly, the lane's claim holds and is stronger than its own statement
of it.** Keyed by **full path**: all 266 map to 266 distinct paths, each with exactly
**one** cell count — no directory holds two. Keyed by **geometry token** extracted
from the path (`AR_n_Ret_m`, `CBFS*`, `PHLL*`, `alpha_*`, hump, PH): **40 distinct
geometries, and ZERO of them exist at more than one cell count.** 11 distinct counts
overall, 2,209 to 51,626, so the reader is demonstrated able to see a difference.

**BELIEVED. `G` is empty on every closure row, and no re-run of what is on this box
could have filled it.** The lesson I take from my own false start, and record against
myself: **a census is only as good as its key, and a key that collides silently
manufactures exactly the positive a null-hunting search is least equipped to doubt.**
The wrong answer here was the *interesting* one, which is precisely why it needed a
second method rather than a second reading.

**One classification I did NOT adopt.** The lane flagged its Row 3 caveat — that a
refined ceiling run would have to interpolate a truth-extracted source — as **its own
reading, backed by no landed record**, and said so unprompted. I leave **Row 3 as
GAP** on the chief's own discriminator: the quantity is a solve output on a mesh that
exists, and awkwardness in constructing the refined source is a **design problem, not
an undefinedness**. Recorded so a future session can overturn it knowingly rather than
inherit it silently.

**No tier moved. No verdict moved. No letter moved. Zero compute: 0.000
core-minutes, 0.000 GPU-hours.**
