# =====================================================================
# UNFROZEN SUCCESSOR TO M1b -- FIXES THE G1 MODEL_RE ARM-APPLICATION
# DEFECT (L-509).  DO NOT GRADE TO A VERDICT.  DO NOT FREEZE.
# ---------------------------------------------------------------------
# THE DEFECT M1d REMOVES (L-509).  grade_m1b.py's MODEL_RE was
#     re.compile(r"Selecting\s+(?:RAS\s+)?turbulence model\s+(\w+)")
# (grade_m1b.py:394, byte-identical to the frozen grade_m1.py:224),
# applied with `.search()` -- FIRST MATCH WINS.  Every real OpenFOAM log
# prints, IN THIS ORDER:
#     Selecting turbulence model type RAS      <- MODEL_RE matched THIS
#                                                 first, captured "type"
#     Selecting RAS turbulence model kOmega    <- never reached
# So model_from_log == "type" on all 78 M1 rows, and G1 flagged a mismatch
# against the (correct) dict model -> NOT A RESULT on ALL 78 rows.  The arm
# WAS applied correctly; the READER was wrong.  M1b's synthetic fixture
# (grade_m1b.py:1105) wrote ONLY the second line and omitted the first, so
# the defect was INVISIBLE to a green selftest -- the same "fixture cleaner
# than a real log" failure L-402 / the G2 precedent name.
# THE FIX: MODEL_RE now requires the "RAS " token BEFORE "turbulence model"
# (mandatory, not optional), binding ONLY the model-specific line; the
# synthetic fixture now carries the REAL two-line banner; a new L-314
# planted-failure proof and a §2j model-channel limb on REAL bytes defend
# the fix in both directions.  EVERYTHING ELSE IS INHERITED FROM M1b
# VERBATIM -- all 12 gate constants, the C1 predicate (the L-508 repair),
# the fatal channel, the non-finite/anti-windowing channel.
# ---------------------------------------------------------------------
# This file is UNFROZEN and is NOT a frozen grading path under CLAUDE.md
# rule 2: no pre-registration accompanies it and NO commit fixes it by
# sha256.  The closure-supervisor performs the SUPERVISION_CHARTER sec.3
# check-1 diff-read and the freeze in a DELIBERATE session (sec.3 check 3,
# a big claim).  Until that freeze:
# NOTHING THIS FILE PRODUCES IS A RESULT.  DO NOT GRADE.  DO NOT FREEZE.
# =====================================================================
"""
M1d multi-model sweep G1-FIX REGRADE -- SUCCESSOR TO M1b.
...
Usage
  python3 grade_m1d.py --root /home/ubuntu/closure-data/multimodel_sweep
  python3 -O grade_m1d.py --selftest
  python3 grade_m1d.py --birth            # section 2j, real producer bytes
"""
...
# M1d writes its OWN birth record (a separate directory from M1b's), because
# self_sha256() differs from grade_m1b.py and verify_birth_record() refuses a
# record produced by a different build.  This path must be regenerated with
# `python3 grade_m1d.py --birth` before grading; it does NOT clobber M1b's.
BIRTH_DIR = "/home/ubuntu/closure-data/m1d_birth"
BIRTH_RECORD = os.path.join(BIRTH_DIR, "m1d_birth_demonstration.json")
...
# --------------------------------------------------------------------------
# THE G1 MODEL_RE FIX -- M1d's ONE SUBSTANTIVE CHANGE (L-509).
#
...
# and parse_log applies it with `.search()`, first-match-wins.  Every real
# OpenFOAM log prints TWO lines, IN THIS ORDER:
#     Selecting turbulence model type RAS      <- the OPTIONAL-RAS form matches
#                                                 THIS first, capturing "type"
#     Selecting RAS turbulence model kOmega    <- the model-specific line, never
#                                                 reached once model!=None
# So model_from_log was the literal "type" on all 78 rows, and G1 read a mismatch
# against the correct dict model -> NOT A RESULT on ALL 78.  The arm was applied
# correctly; the reader captured a keyword.
#
# THE FIX: make the "RAS " token before "turbulence model" MANDATORY, not
# optional.  This binds MODEL_RE to ONLY the model-specific line.  It matches
# "Selecting RAS turbulence model kOmega" (captures "kOmega") and does NOT match
# "Selecting turbulence model type RAS" (no "RAS " between "Selecting" and
# "turbulence model"), so first-match-wins now lands on the right line.
MODEL_RE = re.compile(r"Selecting RAS turbulence model\s+(\w+)")
# The fallback.  Inherited from M1b VERBATIM and NOT used in parse_log (MODEL_RE
# alone is applied there).  Verified by inspection to be safe: it is line-
# anchored (`^\s*(?:RAS\s+)?[Mm]odel\s+(\w+)\s*$`), so on the generic banner line
# "Selecting turbulence model type RAS" it does NOT match at all (the line begins
# with "Selecting", not an optional "RAS " then "Model"/"model"), and therefore
# cannot reintroduce the "type" capture the fix above removes.
MODEL_RE2 = re.compile(r"^\s*(?:RAS\s+)?[Mm]odel\s+(\w+)\s*$")
# L-509: the DEFECTIVE predecessor pattern, retained ONLY so the selftest and the
# §2j birth demonstration can PROVE -- on synthetic and on REAL producer bytes --
# what it WOULD have captured ("type") from the two-line banner.  IT NEVER GATES
# ANYTHING HERE; it is evidence, exactly as RE_BROAD_DEFECTIVE is for the fatal
# channel.
MODEL_RE_DEFECTIVE_PREDECESSOR = re.compile(
    r"Selecting\s+(?:RAS\s+)?turbulence model\s+(\w+)")


def _first_model_match(path, regex, max_lines=5000):
    """First model-name capture over the leading lines of a log, replicating
    parse_log's first-match-wins semantics.  Bounded so a 20 MB log is not read
    whole for this evidence-only reading; the model banner is within the first
    ~60 lines of every OpenFOAM log."""
...
def verify_birth_record():
    ...
    if not os.path.isfile(BIRTH_RECORD):
        refuse("BIRTH-ABSENT",
               ...
               f"comparator may not grade. Run: python3 grade_m1d.py --birth")
    ...
    # NEW IN M1d (L-509): the MODEL channel is the very reader M1d fixes, so its
    # birth demonstration on real bytes is a PRECONDITION OF GRADING too.  A
    # record without a satisfied model limb refuses -- a green synthetic selftest
    # is not a demonstration that the FIXED reader works on real producer output.
    mdl = rec.get("model_limb", {})
    if not mdl.get("passed"):
        refuse("BIRTH-MODEL",
               "the MODEL limb is not satisfied: grade_m1d's MODEL_RE has not "
               "been shown, on REAL producer bytes, to read a genuine model name "
               "where the defective predecessor pattern captured 'type' (L-509). "
               "Run: python3 grade_m1d.py --birth")
...
def main(argv):
    ap = argparse.ArgumentParser(description="M1d successor comparator (G1 MODEL_RE fix, L-509)")
...
def grade(run_root, bench_root, out_path=None):
    ...
    print("M1d MULTI-MODEL SWEEP REGRADE -- SUCCESSOR TO M1b, G1 MODEL_RE FIX (L-509)")
    ...
    out = dict(
        tool="grade_m1d.py", instrument_sha256=self_sha256(),
        predecessor="cases/RANS_LES_closure_models/M1b_multimodel_sweep_regrade/grade_m1b.py",
    ...
    out_path = out_path or os.path.join(run_root, "gate_m1d.json")
...
def _fake_log(n_iter, k_res, om_res, model="kOmegaSST", end=True, exec_lines=None,
    ...
    #
    # NEW IN M1d (L-509).  THE FIXTURE NOW CARRIES THE REAL TWO-LINE MODEL BANNER.
    # M1b's fixture wrote ONLY "Selecting RAS turbulence model {model}" and OMITTED
    # the generic "Selecting turbulence model type RAS" line that every real log
    # prints FIRST -- so the fixture was cleaner than any real log and could not
    # exhibit the first-match-wins "type" capture that condemned all 78 M1 rows.
    # Prepending the generic line makes the synthetic log reproduce the real
    # banner: the OLD optional-RAS pattern would now capture "type" from it, while
    # the fixed MODEL_RE binds only the model-specific second line.
    out = [TRAPFPE_BANNER,
           "Selecting turbulence model type RAS",
           f"Selecting RAS turbulence model {model}"]
...
def selftest():
    ...
    # ---- NEW IN M1d: L-509 MODEL-CHANNEL PLANTED-FAILURE PROOF (L-314) -------
    # THE DEFECT M1d FIXES.  A real OpenFOAM log prints, in order:
    #     Selecting turbulence model type RAS      <- generic; captures "type"
    #     Selecting RAS turbulence model kOmega    <- the model-specific line
    # grade_m1b's MODEL_RE (r"Selecting\s+(?:RAS\s+)?turbulence model\s+(\w+)")
    # applied with .search() (first match wins) captured "type" from the FIRST
    # line, so G1 read model_from_log="type" and flagged a mismatch -> NOT A
    # RESULT on all 78 rows.  M1d's MODEL_RE makes the "RAS " token mandatory and
    # binds only the second line; the fixture (above) now carries BOTH lines like
    # a real log, so it can EXHIBIT the defect the fix removes (L-314).
    _two_line = _fake_log(3, lambda i: 1e-9, lambda i: 1e-11, model="kOmega")
    ok &= _check("L-509: the synthetic fixture carries the REAL two-line banner "
                 "(generic 'type RAS' line THEN the model-specific line)",
                 "Selecting turbulence model type RAS" in _two_line
                 and "Selecting RAS turbulence model kOmega" in _two_line)
    _lg2 = parse_log_from_text(_two_line)
    ok &= _check("L-509 (a): grade_m1d MODEL_RE extracts the CORRECT model "
                 "('kOmega') from the real two-line banner, NOT 'type'",
                 _lg2["model_from_log"] == "kOmega",
                 f"model_from_log={_lg2['model_from_log']!r}")
    # (b) the OLD defective pattern, applied first-match-wins exactly as parse_log
    # applied MODEL_RE, WOULD have captured "type" from the SAME bytes.
    _old_capture = None
    for _ln in _two_line.split("\n"):
        _m = MODEL_RE_DEFECTIVE_PREDECESSOR.search(_ln)
        if _m:
            _old_capture = _m.group(1)
            break
    ok &= _check("L-509 (b): the OLD defective pattern captures 'type' from the "
                 "SAME two-line bytes (so the fixture can exhibit the defect the "
                 "fix removes)", _old_capture == "type",
                 f"old_capture={_old_capture!r}")
    ok &= _check("L-509: MODEL_RE requires the mandatory 'RAS ' token -- it does "
                 "NOT match the generic 'Selecting turbulence model type RAS' line "
                 "and DOES match the model-specific line",
                 MODEL_RE.search("Selecting turbulence model type RAS") is None
                 and (MODEL_RE.search("Selecting RAS turbulence model kOmega")
                      is not None))
    ok &= _check("L-509: the fallback MODEL_RE2 does not reintroduce a 'type' "
                 "capture on the generic banner line",
                 MODEL_RE2.search("Selecting turbulence model type RAS") is None)
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="m1b_b_") as tb:
            ...
            ok &= _check("section 2j: a record with ONLY a positive limb REFUSES "
                         ...
                         _refuses(verify_birth_record))
            # NEW IN M1d: a record whose fatal limbs pass but whose MODEL limb
            # does NOT must refuse at BIRTH-MODEL (the reader M1d fixes is
            # unproven on real bytes).  model_limb absent == not passed.
            json.dump(dict(instrument_sha256=self_sha256(),
                           positive_limb=dict(passed=True, n_fixtures=1,
                                              fixtures=[dict(produced_by="OpenFOAM")]),
                           negative_limb=dict(passed=True, n_fixtures=1,
                                              fixtures=[dict(produced_by="OpenFOAM")]),
                           model_limb=dict(passed=False, n_fixtures=0, fixtures=[])),
                      open(bad, "w"))
            ok &= _check("section 2j (L-509): a record with satisfied fatal limbs "
                         "but an UNSATISFIED model limb REFUSES (BIRTH-MODEL)",
                         _refuses(verify_birth_record))
            # model_limb passed=True so this record reaches the BIRTH-SYNTHETIC
            # clause it is meant to exercise.
            json.dump(dict(instrument_sha256=self_sha256(),
                           positive_limb=dict(passed=True, n_fixtures=1,
                                              fixtures=[dict(produced_by="the control itself")]),
                           negative_limb=dict(passed=True, n_fixtures=1,
                                              fixtures=[dict(produced_by="OpenFOAM")]),
                           model_limb=dict(passed=True, n_fixtures=1, fixtures=[])),
                      open(bad, "w"))
...
def _birth_read(path, why):
    ...
    return dict(path=os.path.abspath(path), role_note=why,
                ...
                # NEW IN M1d (L-509): the MODEL channel on REAL producer bytes.
                # reader_model is what the FIXED MODEL_RE extracts (via parse_log);
                # old_regex_model is what the DEFECTIVE predecessor pattern would
                # have captured, applied first-match-wins over the same file.
                reader_model=log["model_from_log"],
                old_regex_model=_first_model_match(
                    path, MODEL_RE_DEFECTIVE_PREDECESSOR))
...
def birth_demonstration():
    ...
    p("M1d SECTION 2j BIRTH DEMONSTRATION -- FATAL CHANNEL (BOTH LIMBS) + "
      "MODEL CHANNEL (L-509)")
    ...
    # NEW IN M1d (L-509).  LIMB 3 -- THE MODEL CHANNEL ON REAL PRODUCER BYTES.
    # The defect M1d fixes was in the ARM-APPLICATION reader, so the birth
    # demonstration must exercise it on real bytes, not only synthetically.  On
    # the same real clean M1 logs, the FIXED MODEL_RE must extract a genuine model
    # name (never the literal "type"), while the OLD defective pattern captured
    # "type" from the generic banner line -- the whole L-509 defect, on real data.
    p("LIMB 3 -- MODEL CHANNEL (L-509), ON REAL PRODUCER BYTES.  The FIXED")
    p("MODEL_RE must read the CORRECT model from each real log, where the OLD")
    p("defective optional-RAS pattern captured the literal 'type'.")
    for f in neg:
        p(f"  {os.path.basename(os.path.dirname(f['path']))}/"
          f"{os.path.basename(f['path'])}")
        p(f"      grade_m1d MODEL_RE reads    : {f['reader_model']!r}")
        p(f"      OLD defective pattern reads : {f['old_regex_model']!r}")
    model_ok = (len(neg) > 0
                and all(f["reader_model"] not in (None, "type", "RAS")
                        for f in neg)
                and all(f["old_regex_model"] == "type" for f in neg))
    p(f"  LIMB 3: {'SATISFIED' if model_ok else 'NOT SATISFIED'} "
      f"({sum(1 for f in neg if f['reader_model'] not in (None, 'type', 'RAS'))} "
      f"of {len(neg)} real logs read a genuine model name under the FIXED reader, "
      f"and all {sum(1 for f in neg if f['old_regex_model'] == 'type')} of them "
      f"read 'type' under the OLD pattern -- so the fix is load-bearing on real "
      f"bytes, not just synthetically)")
    p("")

    rec = dict(
        item="M1d", generated_utc=now,
        ...
        model_re=MODEL_RE.pattern,
        defective_model_re=MODEL_RE_DEFECTIVE_PREDECESSOR.pattern,
        ...
        model_limb=dict(passed=model_ok, n_fixtures=len(neg),
                        fixtures=[dict(path=f["path"], reader_model=f["reader_model"],
                                       old_regex_model=f["old_regex_model"])
                                  for f in neg]),
    )
    json.dump(rec, open(BIRTH_RECORD, "w"), indent=2, sort_keys=True, default=str)
    txt = os.path.join(BIRTH_DIR, "m1d_birth_demonstration.txt")
    open(txt, "w").write("\n".join(lines) + "\n")
    p(f"wrote {BIRTH_RECORD}")
    p(f"wrote {txt}")
    open(txt, "w").write("\n".join(lines) + "\n")

    if not (pos_ok and neg_ok and model_ok):
        print("BIRTH DEMONSTRATION: NOT SATISFIED -- this instrument may not "
              "grade.", file=sys.stderr)
        return 2
    print("BIRTH DEMONSTRATION: ALL THREE LIMBS (fatal positive, fatal negative, "
          "MODEL channel) SATISFIED ON REAL PRODUCER BYTES.")
    return 0
