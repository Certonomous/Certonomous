...
from of_read import (read_field, sym_to_full, anisotropy, realisability_violation,
...
CFGS = ["null", "truth", "mean", "ml_s0", "ml_s1", "ml_s2"]
...
def main():
    ...
    for case, sdir in SRC.items():
        ...
        for cfg in CFGS:
            ...
            # converged anisotropy and its realisability
            tr = os.path.join(d, t, "tauijRecon")
            if os.path.exists(tr):
                b, vv = anisotropy(sym_to_full(read_field(tr)), k)
                v, _ = realisability_violation(b[vv], tol=1e-6)
                rec["viol_converged_b"] = float(v.mean())
                rec["b_rms_vs_LES"] = float(np.sqrt(
                    ((b[vv & validL] - bL[vv & validL]) ** 2).sum(axis=(1, 2)).mean()))
            out[case]["configs"][cfg] = rec
    ...
    for case, c in out.items():
        ...
        ml = [c["configs"][k]["U_rms"] for k in ("ml_s0", "ml_s1", "ml_s2")
