#!/usr/bin/env python3
"""
F20 -- BUILD ONE LADDER LEVEL into a destination directory (called by
run_f20.sh once per level; lineage cases/F19_SOD/build_f19.py).
...
postProcess run here, so never call it against the case tree.
  ...
  4. write 0/p and 0/T (exact vortex at t = 0 at the mesh's own centres), then 0/U LAST.
Zero `assert` (L-332).
"""
import os
import re
...
import exact_f20 as EX            # noqa: E402
import foam_io_f20 as FIO          # noqa: E402
...
def die(msg, rc=1):
    sys.stderr.write("ABORT (build_f20): %s\n" % msg)
    sys.exit(rc)
...
def main(argv):
    if len(argv) != 3 or argv[1] != "--level":
        die("usage: build_f20.py <dest_dir> --level <coarse|medium|fine>")
    ...
    if not m or abs(float(m.group(1)) - EX.MOL_WEIGHT) > 1e-9:
        die("thermophysicalProperties molWeight is not exact_f20.MOL_WEIGHT")
    ...
    rho0, u0, v0, p0, T0 = EX.fields_exact(xc, yc, 0.0)
    ...
    internal = np.column_stack([u0, v0, np.zeros(cells)])
