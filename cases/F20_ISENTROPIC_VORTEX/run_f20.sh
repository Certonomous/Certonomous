#!/usr/bin/env bash
# F20 -- LAUNCHER for the 2-D isentropic vortex (rhoCentralFoam, periodic, one
# period, three-level ladder).  Lineage: cases/F19_SOD/run_f19.sh.
#
...
# directory is REFUSED, never deleted (here and again inside build_f20.py).
#
...
ROOT="/home/ubuntu/Certonomous/cases/F20_ISENTROPIC_VORTEX"
CASE_SRC="$ROOT/case"
EXACT="$ROOT/exact_f20.py"
BUILD="$ROOT/build_f20.py"
GRADER="$ROOT/grade_f20.py"
FIO="$ROOT/foam_io_f20.py"
RUN_ROOT="/home/ubuntu/Certonomous/verification/runs/F20_ISENTROPIC_VORTEX_runs"
FOAM_BASHRC="/usr/lib/openfoam/openfoam2606/etc/bashrc"
SOLVER="rhoCentralFoam"

CAP_CORE_MIN=240            # must equal grade_f20.py::CAP_CORE_MIN
END_TIME=10                 # one period; must equal controlDict.template endTime and exact_f20.T_END

# name  N  steps  ranks
LEVELS=("coarse 128 4000 1" "medium 256 8000 1" "fine 512 16000 1")
...
# us/cell-step; the 1k-cell rows expose ~0.3-0.4 ms/step fixed overhead): 16384 x 4000,
# 65536 x 8000, 262144 x 16000 cell-steps.  NOT MEASURED on this case.  Used ONLY by the pre-level projected-cap check.
declare -A PROJ_SERIAL_S=( [coarse]=51 [medium]=396 [fine]=3152 )
DECOMP_METHOD="none"
...
python3 "$EXACT" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: exact_f20.py --selftest did not pass; NOTHING may be launched against it"; exit 1; }
python3 "$GRADER" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: grade_f20.py --selftest did not pass; the grading path is not established"; exit 1; }
python3 -O "$GRADER" --selftest > /dev/null 2>&1
[ $? -eq 2 ] || { echo "ABORT: grade_f20.py did not exit 2 under python3 -O; the -O refusal is not armed"; exit 1; }
say "INSTRUMENT GREEN: exact_f20 and grade_f20 selftests pass; grade_f20 exits 2 under -O."
...
python3 -c "import sys; sys.exit(0 if abs(float('$CAP_GRADER') - $CAP_CORE_MIN) < 1e-9 else 1)" \
  || { echo "ABORT: launcher cap $CAP_CORE_MIN disagrees with grade_f20.py CAP_CORE_MIN $CAP_GRADER"; exit 1; }
...
python3 - "$CASE_SRC/constant/thermophysicalProperties" "$CASE_SRC/system/controlDict.template" "$END_TIME" <<'PYCHK' \
  || { echo "ABORT: molWeight/Cp on disk disagree with exact_f20, or controlDict.template endTime is not the registered value"; exit 1; }
import re, sys
sys.path.insert(0, "/home/ubuntu/Certonomous/cases/F20_ISENTROPIC_VORTEX")
import exact_f20 as EX
...
PYCHK
say "molWeight AND Cp ON DISK AGREE with exact_f20 (R = 1, gamma = 1.4); controlDict.template endTime == registered $END_TIME."

if [ "$PREFLIGHT" = "1" ]; then
  say ""
  say "PREFLIGHT ONLY -- ZERO COMPUTE. blockMesh was NOT run. build_f20.py was NOT called."
  say "decomposition method: $DECOMP_METHOD"
  say "decomposition seed:   $DECOMP_SEED"
  for L in "${LEVELS[@]}"; do
    set -- $L
    say "  level $1: $2 x $2 x 1 cells, dt = $END_TIME/$3, ranks $4, target $RUN_ROOT/$1"
...
for L in "${LEVELS[@]}"; do
  ...
  python3 "$BUILD" "$CD" --level "$NAME" > "$CD/log.build" 2>&1 \
    || { echo "ABORT: build_f20.py failed at level $NAME; see $CD/log.build"; exit 1; }
