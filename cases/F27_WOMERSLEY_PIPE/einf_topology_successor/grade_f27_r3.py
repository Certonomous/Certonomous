#!/usr/bin/env python3
"""
F27 EINF-TOPOLOGY SUCCESSOR (R3) GRADER -- NEW file (rule 6: the frozen
grade_f27.py / grade_f27_successor*.py are NEVER edited). It is byte-
identical to grade_f27_successor_r2.py EXCEPT four NON-MEASUREMENT lines:
(a) it imports exact_f27_r3 (the R3 exact model: new W_REF_MESH for the
    ROUNDED-SQUARE core/ring interface; bands BYTE-IDENTICAL -- E2_pred,
    Einf_pred and r_eff are bit-identical to exact_f27, proven in the R3
    pre-registration; W_REF_MESH is a REPORTED-NOT-GATED reference and
    enters NO band, gate or rule-4/5 verdict, only the reported
    W_minus_ref channel);
(b) the --root default is the R3 run root;
(c) the L-332 zero-assert census names the R3 instrument files
    (exact_f27_r3.py, build_f27_r3.py) instead of the R2 ones -- a census
    over the files THIS grader actually depends on, not a verdict change;
    and
(d) this note.
EVERY reader (decomposed, never reconstructed), EVERY gated quantity
(e2_of, einf_of), the two-sided planted-zero controls (rule 3), the
rule-4 strict-completion + age guard, the rule-5 Roache gating via the
single AST-censused RT.grade_ladder call, every exit-2 refusal, the fixed
verdict vocabulary and CAP_CORE_MIN (625.0, retained: R3 has IDENTICAL
cell counts / dt / numerics to R2, whose 625 cap R2 completed under at
417.667 core-min) are BYTE-IDENTICAL to grade_f27_successor_r2.py. The
gate NAMES (G-F27R-1 / G-F27R-2) are RETAINED byte-identical; the R3 run
is distinguished by its run root and pre-registration commit (as R2 was
distinguished from the parent). The mesh-topology change is a BUILDER
concern (blockMeshDict.template); it does NOT alter how Einf is measured
-- einf_of already takes the max over EVERY cell of |u_z - u_exact(r)|,
the correct instrument on any D4-symmetric butterfly topology.

F27 NUMERICS SUCCESSOR -- THE GRADING PATH for pulsatile Womersley flow in a
CIRCULAR PIPE on a GENUINE 3-D BUTTERFLY (O-GRID) MESH (pimpleFoam in PISO mode,
alpha = 4, locked phase t = 5.25 = 5 T + T/4), with the NUMERICS CHANGED per
verification/campaign/F27_NUMERICS_SUCCESSOR_PREREGISTRATION.md.  It calls
`scripts/roache_triple.py::grade_ladder` and rule 5 is reached through that call
and through NOTHING ELSE (exactly one call node, AST-censused).

WHY A PARALLEL GRADER (the frozen parent grade_f27.py could not be re-used).
The parent grade_f27.py::control_solver_dicts_match() is METHOD-LOCKED to the
PARENT numerics: it hard-refuses (exit 2) unless it finds nNonOrthogonalCorrectors
1 / laplacian `Gauss linear corrected` / snGrad `corrected`.  The successor CHANGES
exactly those levers, so the parent grader would refuse it.  This grader is
byte-for-byte the parent EXCEPT that control_solver_dicts_match() below checks the
SUCCESSOR numerics -- nNonOrthogonalCorrectors 3, laplacian `Gauss linear limited
0.5`, snGrad `limited 0.5`, grad `cellLimited Gauss linear 1` -- and REFUSES a
case carrying the parent method.  EVERY gated quantity, EVERY band, BAND_FACTOR,
the four reported-not-gated channels and all rule-4 / rule-5 gating are IDENTICAL
to the frozen parent (the reference is the EXACT Womersley solution, which no
numerics change can move; bands are held EXACTLY, not widened by a single digit).
The shared model / readers / gate (exact_f27, foam_io_f27, proj_f27, roache_triple)
are imported UNCHANGED from beside the frozen parent.

REFUSAL DISCIPLINE: `raise` / `sys.exit(2)` only; ZERO `assert` across this
file, exact_f27.py, foam_io_f27.py, build_f27_successor.py, proj_f27.py (L-332);
hard `-O` refusal.
...
if not __debug__:
    sys.stderr.write("REFUSED: grade_f27_successor.py must not run under `python3 -O` (the shared "
...
HERE = os.path.dirname(os.path.abspath(__file__))          # .../F27_WOMERSLEY_PIPE/successor_numerics
PARENT = os.path.dirname(HERE)                              # .../F27_WOMERSLEY_PIPE (the frozen parent instrument dir)
REPO = os.path.dirname(os.path.dirname(PARENT))            # .../Certonomous
sys.path.insert(0, os.path.join(REPO, "scripts"))
sys.path.insert(0, PARENT)                                  # exact_f27, foam_io_f27, proj_f27 live beside the frozen parent

import roache_triple as RT              # THE GATE.
import exact_f27_r3 as EX               # R3 EXACT MODEL: new W_REF_MESH (reported ref); bands BYTE-IDENTICAL.
import foam_io_f27 as FIO               # THE READERS (SHARED, unchanged).

# The frozen parent case supplies every dictionary the numerics change does NOT
# touch (nu, drive, tolerances*, endTime, decomposition, geometry templates); the
# successor's OWN system/ supplies ONLY the two changed dicts (fvSchemes, fvSolution).
# *the solver tolerances live in the CHANGED fvSolution, so they are read from SUCC_SYSTEM.
PARENT_CASE = os.path.join(PARENT, "case")
SUCC_SYSTEM = os.path.join(HERE, "system")
...
CAP_CORE_MIN = 625.0                    # HARD CAP, F27_NUMERICS_SUCCESSOR_PREREGISTRATION.md section 5;
                                        # run_f27_successor.sh must agree (est 372, cap 500)
P_SOLVER_TOL = EX.P_SOLVER_TOL          # matches successor system/fvSolution, checked below
U_SOLVER_TOL = EX.U_SOLVER_TOL          # matches successor system/fvSolution, checked below

GATES = ("G-F27R-1_E2_velocity_locked_phase", "G-F27R-2_Einf_axial_velocity_locked_phase")
...
def control_solver_dicts_match():
    """THE METHOD-LOCK FOR THE SUCCESSOR NUMERICS.  The frozen parent's analogue
    (grade_f27.py::control_solver_dicts_match) hard-refuses unless it finds the
    PARENT method (nNonOrthogonalCorrectors 1 / `Gauss linear corrected` /
    `corrected`).  THIS ONE refuses unless it finds the SUCCESSOR method
    (nNonOrthogonalCorrectors 3 / `Gauss linear limited 0.5` / `limited 0.5` /
    `cellLimited Gauss linear 1`) AND EXPLICITLY refuses a case still carrying the
    parent method -- so a case that silently ran the parent numerics is rejected.

    The two CHANGED dictionaries (fvSchemes, fvSolution) are read from the
    successor's OWN system/ (SUCC_SYSTEM); every UNCHANGED physics dictionary
    (nu, turbulence, drive, endTime, decomposition, geometry templates) is read
    from the frozen parent case (PARENT_CASE), because the numerics change moves
    none of them.  The solver tolerances live inside the CHANGED fvSolution, so
    they are read from SUCC_SYSTEM."""
    # ---- UNCHANGED physics, from the frozen parent case -----------------------
    tp = open(os.path.join(PARENT_CASE, "constant", "transportProperties")).read()
    ...
    if not re.search(r"simulationType\s+laminar\s*;",
                     open(os.path.join(PARENT_CASE, "constant", "turbulenceProperties")).read()):
        refuse("turbulenceProperties is not laminar")
    # ---- solver tolerances and PISO, from the CHANGED successor fvSolution -----
    fs = open(os.path.join(SUCC_SYSTEM, "fvSolution")).read()
    m = re.search(r"\bp\b[^}]*?tolerance\s+([0-9eE+\-.]+)\s*;", fs, re.S)
    if not m or float(m.group(1)) != P_SOLVER_TOL:
        refuse("P_SOLVER_TOL = %g but successor fvSolution sets %s" % (P_SOLVER_TOL, m.group(1) if m else None))
    m = re.search(r"\(U\|UFinal\)\"[^}]*?tolerance\s+([0-9eE+\-.]+)\s*;", fs, re.S)
    if not m or float(m.group(1)) != U_SOLVER_TOL:
        refuse("U_SOLVER_TOL = %g but successor fvSolution sets %s" % (U_SOLVER_TOL, m.group(1) if m else None))
    if not re.search(r"nOuterCorrectors\s+1\s*;", fs):
        refuse("PIMPLE nOuterCorrectors is not 1 (PISO mode is registered, unchanged from the parent)")
    # ---- CHANGED LEVER 1: nNonOrthogonalCorrectors 1 -> 3 ---------------------
    if re.search(r"nNonOrthogonalCorrectors\s+1\s*;", fs):
        refuse("SUCCESSOR METHOD-LOCK: successor fvSolution still carries nNonOrthogonalCorrectors 1 -- the "
               "PARENT method. The numerics-change successor registers nNonOrthogonalCorrectors 3; this case "
               "silently ran the parent numerics. REFUSED.")
    if not re.search(r"nNonOrthogonalCorrectors\s+3\s*;", fs):
        refuse("SUCCESSOR METHOD-LOCK: successor fvSolution does not set nNonOrthogonalCorrectors 3 (the "
               "registered change: the explicit non-orthogonal correction is iterated to convergence within "
               "each time step at up to %g deg non-orthogonality)" % 40.5)
    # ---- CHANGED LEVERS 2-3 in fvSchemes; ddt unchanged -----------------------
    sch = open(os.path.join(SUCC_SYSTEM, "fvSchemes")).read()
    if not re.search(r"ddtSchemes\s*\{\s*default\s+backward\s*;", sch):
        refuse("fvSchemes ddt is not `backward`; the model integrates BDF2 (unchanged from the parent)")
    if re.search(r"laplacianSchemes\s*\{\s*default\s+Gauss\s+linear\s+corrected\s*;", sch):
        refuse("SUCCESSOR METHOD-LOCK: successor fvSchemes still carries laplacian `Gauss linear corrected` -- "
               "the PARENT method. The successor registers `Gauss linear limited 0.5`. REFUSED.")
    if not re.search(r"laplacianSchemes\s*\{\s*default\s+Gauss\s+linear\s+limited\s+0\.5\s*;", sch):
        refuse("SUCCESSOR METHOD-LOCK: successor fvSchemes laplacian is not `Gauss linear limited 0.5` (bounded "
               "non-orthogonal correction, the registered robustness choice above ~35 deg non-orthogonality)")
    if re.search(r"snGradSchemes\s*\{\s*default\s+corrected\s*;", sch):
        refuse("SUCCESSOR METHOD-LOCK: successor fvSchemes still carries snGrad `corrected` -- the PARENT "
               "method. The successor registers `limited 0.5`. REFUSED.")
    if not re.search(r"snGradSchemes\s*\{\s*default\s+limited\s+0\.5\s*;", sch):
        refuse("SUCCESSOR METHOD-LOCK: successor fvSchemes snGrad is not `limited 0.5`")
    # ---- CHANGED LEVER: gradient default Gauss linear -> cellLimited Gauss linear 1
    if re.search(r"gradSchemes\s*\{\s*default\s+Gauss\s+linear\s*;", sch):
        refuse("SUCCESSOR METHOD-LOCK: successor fvSchemes still carries an UNLIMITED gradient `Gauss linear` -- "
               "the PARENT method. The successor registers `cellLimited Gauss linear 1`. REFUSED.")
    if not re.search(r"gradSchemes\s*\{\s*default\s+cellLimited\s+Gauss\s+linear\s+1\s*;", sch):
        refuse("SUCCESSOR METHOD-LOCK: successor fvSchemes gradient is not `cellLimited Gauss linear 1` (bounds "
               "reconstructed gradients, suppressing the cross-plane over/undershoot)")
    # ---- div(phi,U) STAYS the parent's `Gauss linear` (NOT part of this freeze) -
    if not re.search(r"div\(phi,U\)\s+Gauss\s+linear\s*;", sch):
        refuse("SUCCESSOR METHOD-LOCK: div(phi,U) is not the parent's `Gauss linear`; a bounded divergence is a "
               "SEPARATE, NON-registered future successor (prereg section 2) and must NOT appear here")
    # ---- UNCHANGED: controlDict, drive, decomposition, from the frozen parent -
    cd = open(os.path.join(PARENT_CASE, "system", "controlDict.template")).read()
    ...
    fo = open(os.path.join(PARENT_CASE, "constant", "fvOptions")).read()
    ...
    dp = open(os.path.join(PARENT_CASE, "system", "decomposeParDict")).read()
    ...
    return dict(control="successor_solver_dictionaries_agree_with_registration_and_reject_the_parent_method",
                method_source=dict(changed_dicts_from=SUCC_SYSTEM, unchanged_dicts_from=PARENT_CASE),
                nu=EX.NU, p_tol=P_SOLVER_TOL, U_tol=U_SOLVER_TOL, endTime=END_TIME, ddt="backward",
                laplacian="Gauss linear limited 0.5", snGrad="limited 0.5",
                gradient="cellLimited Gauss linear 1", nNonOrthogonalCorrectors=3,
                div_phi_U="Gauss linear (parent, unchanged; a bounded divergence is a separate future successor)",
                drive="cosine f=1 A=1 z-hat, specific, all", decomposition="simple (1 1 4)", passed=True)
...
def main(argv=None):
    ...
    ap.add_argument("--root", default=os.path.join(REPO, "verification", "runs", "F27_NUMERICS_SUCCESSOR_R3_runs"))
    ap.add_argument("--out", default=None)
    ap.add_argument("--prereg-commit")
    ap.add_argument("--selftest", action="store_true")
    a = ap.parse_args(argv)

    census = ast_no_asserts([os.path.abspath(__file__), os.path.join(PARENT, "exact_f27_r3.py"),
                             os.path.join(PARENT, "foam_io_f27.py"), os.path.join(HERE, "build_f27_r3.py"),
                             os.path.join(PARENT, "proj_f27.py")])
    ...
    with open(out, "w") as f:
        json.dump(dict(rung="F27-WOMERSLEY-PIPE-3D-NUMERICS-SUCCESSOR", prereg_commit=a.prereg_commit,
                       prereg="verification/campaign/F27_NUMERICS_SUCCESSOR_PREREGISTRATION.md",
    ...
    print("F27 -- WOMERSLEY PULSATILE PIPE, 3-D BUTTERFLY LADDER -- NUMERICS SUCCESSOR -- TALLY")
    print("=" * 78)
    for r in rows:
