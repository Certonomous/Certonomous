#!/usr/bin/env python3
"""
F27 NUMERICS SUCCESSOR -- BUILD ONE LADDER LEVEL into a destination directory
(called by run_f27_successor.sh once per level; lineage build_f27_successor.py -> this R3 successor).
F27 EINF-TOPOLOGY SUCCESSOR (R3). NEW file (rule 6: frozen build_f27.py /
build_f27_successor.py NEVER edited). IDENTICAL to build_f27_successor.py EXCEPT:
(a) it imports exact_f27_r3 (new W_REF_MESH; bands BYTE-IDENTICAL); (b) the
blockMeshDict.template comes from the R3 system/ (the ROUNDED-SQUARE core/ring
interface fix, arc bulge rm = 0.5*sqrt(2/pi)) instead of the frozen parent case;
(c) it ADDS the A_MESH band-pin proving the built mesh reproduces the registered
cross-sectional area (hence the band). The CHANGED numerics dictionaries
(fvSchemes, fvSolution) are copied from the R3 system/ (byte-identical to the R2
successor numerics); every OTHER file -- unchanged dictionaries and the
controlDict/p/U templates -- is taken from the frozen parent case UNCHANGED.
The parent instrument files are NEVER edited or written to.

REFUSES a destination holding `0/`, a numeric time directory or processor*
directories; deletes nothing; refuses a destination inside the tracked case tree
OR inside the successor instrument tree.  Mesher and postProcess run here, so
never call it against either instrument tree.

  1. template blockMeshDict (NC, NR, NZ) and controlDict (DT) from the PARENT case;
     copy the fixed dictionaries (decomposeParDict, transportProperties,
     turbulenceProperties, fvOptions) from the PARENT case; copy the CHANGED
     numerics dictionaries (fvSchemes, fvSolution) from the SUCCESSOR system/
...
if not __debug__:
    sys.stderr.write("REFUSED: build_f27_successor.py must not run under `python3 -O`.\n")
    sys.exit(2)
...
sys.path.insert(0, PARENT)                                  # exact_f27, foam_io_f27 live beside the frozen parent
import exact_f27_r3 as EX         # noqa: E402  (R3 model: new W_REF_MESH; bands BYTE-IDENTICAL)
import foam_io_f27 as FIO         # noqa: E402
import numpy as np                # noqa: E402

CASE_SRC = os.path.join(PARENT, "case")                    # the frozen parent case: templates + UNCHANGED dicts
SUCC_SYSTEM = os.path.join(HERE, "system")                 # the successor's OWN CHANGED dicts (fvSchemes, fvSolution)
...
def main(argv):
    ...
    if os.path.realpath(dest).startswith(os.path.realpath(CASE_SRC)):
        die("destination %s lies inside the frozen parent case tree; refused" % dest, rc=2)
    if os.path.realpath(dest).startswith(os.path.realpath(HERE)):
        die("destination %s lies inside the successor instrument tree; refused" % dest, rc=2)
    refuse_if_answered(dest)

    for sub in ("system", "constant"):
        os.makedirs(os.path.join(dest, sub), exist_ok=True)
    # CHANGED numerics dictionaries from the successor's OWN system/
    for f in ("fvSchemes", "fvSolution"):
        shutil.copy(os.path.join(SUCC_SYSTEM, f), os.path.join(dest, "system", f))
    # UNCHANGED dictionaries from the frozen parent case
    for f in ("decomposeParDict",):
    ...
    FIO.write_from_template(os.path.join(SUCC_SYSTEM, "blockMeshDict.template"), bm,
    ...
    # ---- THE SUCCESSOR NUMERICS MUST HAVE LANDED IN THE BUILT CASE ------------
    # Defense-in-depth against a build that silently assembled the PARENT method:
    # read the CHANGED dictionaries back OUT OF THE DESTINATION and require the
    # registered successor levers (the grader's control_solver_dicts_match is the
    # authority; this refuses at build time so the parent numerics never even run).
    sch_out = open(os.path.join(dest, "system", "fvSchemes")).read()
    fs_out = open(os.path.join(dest, "system", "fvSolution")).read()
    if not re.search(r"laplacianSchemes\s*\{\s*default\s+Gauss\s+linear\s+limited\s+0\.5\s*;", sch_out):
        die("built fvSchemes laplacian is not the successor `Gauss linear limited 0.5`")
    if not re.search(r"snGradSchemes\s*\{\s*default\s+limited\s+0\.5\s*;", sch_out):
        die("built fvSchemes snGrad is not the successor `limited 0.5`")
    if not re.search(r"gradSchemes\s*\{\s*default\s+cellLimited\s+Gauss\s+linear\s+1\s*;", sch_out):
        die("built fvSchemes gradient is not the successor `cellLimited Gauss linear 1`")
    if not re.search(r"nNonOrthogonalCorrectors\s+3\s*;", fs_out):
        die("built fvSolution does not set the successor nNonOrthogonalCorrectors 3")
    if re.search(r"nNonOrthogonalCorrectors\s+1\s*;", fs_out) \
            or re.search(r"laplacianSchemes\s*\{\s*default\s+Gauss\s+linear\s+corrected\s*;", sch_out) \
            or re.search(r"snGradSchemes\s*\{\s*default\s+corrected\s*;", sch_out):
        die("built case still carries a PARENT numerics lever; the successor levers did not fully replace it")
    ...
    # ---- THE BAND PIN (R3): the built mesh's own cross-sectional area, which the
    # gate bands depend on through exact_f27_r3.r_eff_of, must reproduce the
    # registered A_MESH. The R3 mesh changes ONLY the interior core/ring interface
    # (a rounded square); the outer wall is byte-identical to the frozen parent, so
    # A_MESH is unchanged and the bands are byte-identical. This pin refuses a mesh
    # whose area (hence band) drifted.
    a_mesh_built = float(vol.sum()) / EX.LZ
    a_pin = None
    if pin_reference:
        a_reg = EX.A_MESH[level]
        if abs(a_mesh_built - a_reg) > EX.A_MESH_TOL:
            die("A_MESH BAND PIN FAILED at level %s: the built mesh gives A_MESH = %.15f, the pre-registration "
                "froze %.15f (difference %.3e > %.1e). The gate bands depend on A_MESH; this mesh would grade "
                "against a DIFFERENT band." % (level, a_mesh_built, a_reg, a_mesh_built - a_reg, EX.A_MESH_TOL), rc=2)
        a_pin = dict(registered=a_reg, rebuilt=a_mesh_built, difference=a_mesh_built - a_reg, tolerance=EX.A_MESH_TOL)
    ...
    mesh_line = dict(level=level, nc=nc, nr=nr, nz=nz, cells=cells, steps=steps, dt=EX.dt_of(steps),
                     ...
                     grading_readback=grading, W_ref_mesh=w_ref, W_ref_pin=pin, A_mesh_built=a_mesh_built, A_mesh_band_pin=a_pin,
