...
# PIPE, EINF-TOPOLOGY SUCCESSOR (R3). NEW file (rule 6: run_f27.sh /
# run_f27_successor_r2.sh are NEVER edited). Byte-for-byte the R2 successor driver
# EXCEPT the pointers below: it builds with build_f27_r3.py (which uses the R3
# ROUNDED-SQUARE core/ring interface blockMeshDict.template, arc bulge
# rm = 0.5*sqrt(2/pi)) and grades with grade_f27_r3.py (imports exact_f27_r3;
# bands BYTE-IDENTICAL), writes to F27_NUMERICS_SUCCESSOR_R3_runs, and RETAINS the
# 625 core-min cap (R3 has identical cells/dt/numerics to R2). The successor
# NUMERICS are UNCHANGED from R2; the ONLY physical change is the mesh topology.
# Original R2 header follows.
...
# EXCEPT it (i) builds with build_f27_r3.py from the SUCCESSOR schemes,
...
# (iii) writes to a FRESH run root F27_NUMERICS_SUCCESSOR_R3_runs, (iv) carries the
# HARD CAP 625 core-min (F27_NUMERICS_SUCCESSOR_R3_PREREGISTRATION.md; R2 actual 417.667),
...
# build_f27_r3.py).  The RUN ROOT existing at all is exit 3.
#
...
HERE="$PARENT/einf_topology_successor"                       # the R3 einf-topology successor instrument dir
...
EXACT="$PARENT/exact_f27_r3.py"                              # R3 model: new W_REF_MESH; bands BYTE-IDENTICAL
BUILD="$HERE/build_f27_r3.py"
GRADER="$HERE/grade_f27_r3.py"
PROJ_MOD="$PARENT/proj_f27.py"                              # SHARED, unchanged
FIO="$PARENT/foam_io_f27.py"                                # SHARED, unchanged
RUN_ROOT="/home/ubuntu/Certonomous/verification/runs/F27_NUMERICS_SUCCESSOR_R3_runs"
FOAM_BASHRC="/usr/lib/openfoam/openfoam2606/etc/bashrc"
STATUS="$HERE/STATUS.F27_NUMERICS_SUCCESSOR_R3"
GRADER_REL="cases/F27_WOMERSLEY_PIPE/einf_topology_successor/grade_f27_r3.py"  # for the rule-2 blob hash

CAP_CORE_MIN=625            # HARD CAP; must equal grade_f27_r3.py::CAP_CORE_MIN and the pre-registration
...
write_status() {
  local R=$1
  printf 'launcher_rc=%s end=%s note=exit-status-of-run_f27_r3.sh-NOT-the-solver-rc cap_core_min=%s spent_core_min=%s\n' \
...
for f in "$CASE_SRC/0/U.template" "$CASE_SRC/0/p.template" \
         ...
         "$SUCC_SYSTEM/blockMeshDict.template" "$CASE_SRC/system/controlDict.template" \
         "$EXACT" "$BUILD" "$GRADER" "$PROJ_MOD" "$FIO" "$FOAM_BASHRC"; do
  [ -f "$f" ] || { echo "ABORT: required file does not exist: $f"; exit 1; }
done
say "PATHS RESOLVED against disk in this invocation: 16 files, 5 directories (fvSchemes/fvSolution AND blockMeshDict.template from the R3 successor system)."
...
python3 "$GRADER" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: grade_f27_r3.py --selftest did not pass; the grading path is not established"; exit 1; }
python3 -O "$GRADER" --selftest > /dev/null 2>&1
[ $? -eq 2 ] || { echo "ABORT: grade_f27_r3.py did not exit 2 under python3 -O; the -O refusal is not armed"; exit 1; }
...
say "INSTRUMENT GREEN: exact_f27_r3, grade_f27_r3 and proj_f27 selftests pass; grade_f27_r3 and proj_f27 exit 2 under -O."
...
python3 -c "import sys; sys.exit(0 if abs(float('$CAP_GRADER') - $CAP_CORE_MIN) < 1e-9 else 1)" \
  || { echo "ABORT: launcher cap $CAP_CORE_MIN disagrees with grade_f27_r3.py CAP_CORE_MIN $CAP_GRADER"; exit 1; }
...
[ "$DISK_BLOB" = "$HEAD_BLOB" ] \
  || { echo "ABORT (rule 2): grade_f27_r3.py on disk ($DISK_BLOB) is NOT the committed blob ($HEAD_BLOB); the grading path is fixed at the pre-registration commit and must not drift"; exit 1; }
say "RULE 2: grade_f27_r3.py on disk == committed blob $HEAD_BLOB."
...
if [ "$PREFLIGHT" = "1" ]; then
  say ""
  say "PREFLIGHT ONLY -- ZERO COMPUTE. blockMesh was NOT run. build_f27_r3.py was NOT called. decomposePar was NOT run."
...
if [ "$DETACH" = "1" ]; then
  trap - EXIT
  ME="$HERE/run_f27_r3.sh"
  OUT="$HERE/launcher.detached.out"
  nohup setsid bash -c "cd '$HERE' && bash '$ME' --prereg-commit=$PREREG_COMMIT > '$OUT' 2>&1; R=\$?; printf 'launcher_rc=%s end=%s note=exit-status-of-run_f27_r3.sh-NOT-the-solver-rc\n' \"\$R\" \"\$(date -u +%Y-%m-%dT%H:%M:%SZ)\" > '$STATUS'" \
...
for L in "${LEVELS[@]}"; do
  ...
  python3 "$BUILD" "$CD" --level "$NAME" > "$CD/log.build" 2>&1 \
    || { echo "ABORT: build_f27_r3.py failed at level $NAME; see $CD/log.build"; exit 1; }
