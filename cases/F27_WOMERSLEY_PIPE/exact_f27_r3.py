#!/usr/bin/env python3
"""
F27 EINF-TOPOLOGY SUCCESSOR (R3) NOTE -- this file is a NEW file (rule 6:
the frozen exact_f27.py is NEVER edited). It is byte-identical to
exact_f27.py EXCEPT the three W_REF_MESH literals (the rounded-square
R3 mesh re-registration; a reported-not-gated reference, NOT a band
input) and this note. u_exact, discrete_solution, predictions(),
A_MESH, BAND_FACTOR, MODEL_NR and STEPS are unchanged, so both gate
bands are byte-identical to the frozen model.
...
# F27 EINF-TOPOLOGY SUCCESSOR (R3): re-registered for the ROUNDED-SQUARE
# core/ring interface (blockMeshDict.template arc bulge radius rm = 0.5*sqrt(2/pi)
# = 0.3989422804014327, the AREA-EQUIVALENT circle of the frozen square core).
# The interior cell distribution changes, so the volume-weighted same-stencil
# reference W_REF_MESH moves (a REPORTED-NOT-GATED reference; it enters NO band).
# RECOMPUTED from the built R3 mesh's own 0/C, 0/V (build_f27_r3.py pins each to
# W_REF_TOL). A_MESH, MODEL_NR and STEPS are UNCHANGED, so predictions() and both
# bands are BYTE-IDENTICAL to the frozen exact_f27.py -- verified in the R3
# pre-registration. Frozen exact_f27.py values were 0.649820058760990 /
# 0.644677639755022 / 0.643390292465464.
W_REF_MESH = dict(coarse=0.6497988002770876,
                  medium=0.6446723400039349,
                  fine=0.6433889691657636)
