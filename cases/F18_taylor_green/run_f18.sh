...
# (here and again inside build_f18.py).
#
...
CASE_SRC="$ROOT/case"
EXACT="$ROOT/exact_f18.py"
BUILD="$ROOT/build_f18.py"
GRADER="$ROOT/grade_f18.py"
FIO="$ROOT/foam_io_f18.py"
...
END_TIME=2                  # T = 2 s; must equal controlDict.template endTime and exact_f18.T_END
...
python3 "$EXACT" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: exact_f18.py --selftest did not pass; NOTHING may be launched against it"; exit 1; }
python3 "$GRADER" --selftest > /dev/null 2>&1 \
  || { echo "ABORT: grade_f18.py --selftest did not pass; the grading path is not established"; exit 1; }
python3 -O "$GRADER" --selftest > /dev/null 2>&1
[ $? -eq 2 ] || { echo "ABORT: grade_f18.py did not exit 2 under python3 -O; the -O refusal is not armed"; exit 1; }
say "INSTRUMENT GREEN: exact_f18 and grade_f18 selftests pass; grade_f18 exits 2 under -O."
...
python3 -c "import sys; sys.exit(0 if abs(float('$CAP_GRADER') - $CAP_CORE_MIN) < 1e-9 else 1)" \
  || { echo "ABORT: launcher cap $CAP_CORE_MIN disagrees with grade_f18.py CAP_CORE_MIN $CAP_GRADER"; exit 1; }
...
python3 - "$CASE_SRC/constant/transportProperties" "$CASE_SRC/system/controlDict.template" "$END_TIME" <<'PYCHK' \
  || { echo "ABORT: nu on disk disagrees with exact_f18.NU, or controlDict.template endTime is not the registered value"; exit 1; }
import re, sys
sys.path.insert(0, "/home/ubuntu/Certonomous/cases/F18_taylor_green")
import exact_f18 as EX
...
PYCHK
say "nu ON DISK AGREES with exact_f18.NU; controlDict.template endTime == registered $END_TIME."

if [ "$PREFLIGHT" = "1" ]; then
  say ""
  say "PREFLIGHT ONLY -- ZERO COMPUTE. blockMesh was NOT run. build_f18.py was NOT called."
...
for L in "${LEVELS[@]}"; do
  ...
  python3 "$BUILD" "$CD" --level "$NAME" > "$CD/log.build" 2>&1 \
    || { echo "ABORT: build_f18.py failed at level $NAME; see $CD/log.build"; exit 1; }
