#!/usr/bin/env python3
"""
F15-R3 -- THIRD COMPARATOR REGISTRATION UNDER L-342 (2026-08-26). ZERO COMPUTE.
===============================================================================
PARENT (R3): cases/F15_oblique_shock_reflection/grade_f15_r2.py, blob
        133820b314baeae42281da26b1d1469a7e0189fb, frozen at 893cdeaf (R2 prereg).
GRANDPARENT: grade_f15.py, blob e4076c03ca68d67e8247f73a4038d179cd7a20f2 (2aea29d9).
THE THIRD READER DEFECT (measured; RESULTS_R2.md section 0(b), GRADE_F15_R2.err):
  (iii) series_for() feeds x_wall_from_xy EVERY sampled time of the Class C
        series, from t = 0.05. The half-rise crossing detector is therefore
        applied to PRE-ARRIVAL samples: the whole wall line still reads the
        free-stream p1 = 0.714286 (the run is initialised uniform at region 1,
        0/p internalField 0.714285714285714, and the incident shock is driven
        in from the top boundary), so the detector refuses (exit 2) on
        coarse/postProcessing/lineWall/0.05/lineWall_p.xy before any graded
        sample is reached. The first sample with a crossing is t = 0.75.
R3 REPAIR, AND NOTHING ELSE: the series is WINDOWED to samples at
        t >= T_ARRIVAL, the exact arrival time of the incident shock at the
        wall station derived from the FROZEN geometry (exact_osr.py:
        M1 = 2.9, sigma = 29 deg from x, station x_w = X_MIN + 1/tan(sigma)):
            the shock enters the domain at the top-left corner (X_MIN, Y_MAX),
            its foot is at (x_w, Y_MIN); path length along the shock line
                L_s = (Y_MAX - Y_MIN) / sin(sigma)
            the disturbance that establishes the shock is carried along that
            line at the tangential (shock-parallel) velocity, which is the
            same on both sides of the shock and equals u1 cos(sigma), so
                T_ARRIVAL = L_s / (u1 cos sigma) = 1 / (u1 sin sigma cos sigma)
                          = 2 / (u1 sin 2 sigma) = 0.813205... at u1 = 2.9,
                            sigma = 29 deg.
        The first admitted sample at SAMPLE_INTERVAL = 0.05 is t = 0.85.
        T_ARRIVAL is COMPUTED from EX.solve() at every run, never a literal.
        The window applies to every series_for() call (both gates) so the
        two series are built from the same samples; G-F15-1's reader carries
        no crossing detector, so for it the window only drops pre-arrival
        samples, which Class C (last `window` = 60 samples) and the graded
        value (the last sample) never touched.
DIFF SCOPE against the R2 parent, and NOTHING ELSE:
  (1) this header block;
  (2) PHYSICS_CRITICAL gains the window line; T_ARRIVAL_NOTE;
  (3) arrival_time(sol) and series_for(..., t_min=None) -- t_min defaults to
      arrival_time(sol); the guard is `if t < t_min - 1e-9: continue`;
  (4) two driven controls appended: control_window_guard_flips (a synthetic
      series with a spurious pre-arrival crossing at x = +0.27 and the true
      post-arrival crossing at x_w: the guard returns the latter; the guard
      mutated to a no-op (t_min = 0.0) returns the former -- the control
      FLIPS) and control_gate_blocks_identical_to_r2 (the AST dump of every
      gate-bearing definition in this file equals the R2 file's: EMPTY diff);
  (5) the JSON `rung` label F15-OSR29-R3, parent blob, the R3 addendum path;
      default output F15_R3_GRADED.json; the tally title.
GATES, BANDS, THRESHOLDS, CAP AND LABELS ARE BYTE-IDENTICAL to R2 and to the
grandparent: N_CAPTURE_CELLS, END_TIME, SAMPLE_INTERVAL, CAP_CORE_MIN, RANKS,
LEVELS, CLASS_C, bands(), smeared_profile(), demonstrate(), class_c(),
x_wall_from_xy(), e1_from_xy(), grade_one() and the single grade_ladder call
are untouched lines, and control_gate_blocks_identical_to_r2 refuses if not.

(R2 header, retained verbatim below.)
...
PHYSICS_CRITICAL = (
    ...
    "the series window t >= T_ARRIVAL (R3; computed from the frozen geometry)",
)
T_ARRIVAL_NOTE = ("R3: T_ARRIVAL = 1 / (u1 sin sigma cos sigma) = 2 / (u1 sin 2 sigma), "
                  "the shock-line path (Y_MAX - Y_MIN)/sin sigma from the top-left "
                  "corner to the wall foot x_w, travelled at the shock-tangential "
                  "velocity u1 cos sigma; u1 and sigma from exact_osr.solve()")
INFRASTRUCTURE = (
...
# ---------------------------------------------------------------------------
# R3 DRIVEN CONTROLS -- the window guard shown to flip, and the gate blocks
# shown identical to R2 by AST
# ---------------------------------------------------------------------------
def control_window_guard_flips(sol):
    """A synthetic lineWall series: at t = 0.5 (pre-arrival) a spurious
    crossing at x = +0.27 (the shape R2 measured at t = 0.75 on the coarse
    level); at t = T_ARRIVAL + 3 SAMPLE_INTERVAL the true crossing at x_w.
    The windowed series must return ONLY the post-arrival value; the guard
    mutated to a no-op (t_min = 0.0) must return the spurious one FIRST."""
    t_arr = arrival_time(sol)
    xw = sol["x_wall_impingement"]
    dx = DX["fine"]
    x = np.arange(EX.X_MIN + dx / 2.0, EX.X_MAX, dx)
    tmp = tempfile.mkdtemp(prefix="f15_r3_window_")
    try:
        case = os.path.join(tmp, "case")
        pre_t = 0.5
        # the first admitted sample time, plus two intervals, on the 0.05 grid the
        # functionObject writes (a %g directory name must round-trip exactly)
        post_t = round(math.ceil(t_arr / SAMPLE_INTERVAL - 1e-9) * SAMPLE_INTERVAL
                       + 2 * SAMPLE_INTERVAL, 2)
        sol_sp = dict(sol); sol_sp["x_wall_impingement"] = 0.27
        for t, s in ((pre_t, sol_sp), (post_t, sol)):
            d = os.path.join(case, "postProcessing", "lineWall", "%g" % t)
            os.makedirs(d)
            synth_xy(os.path.join(d, "lineWall_p.xy"),
                     x, smeared_profile(x, s, 3, dx, on_wall=True))
        # a third, no-crossing pre-arrival sample: the R2 defect itself
        d0 = os.path.join(case, "postProcessing", "lineWall", "0.05")
        os.makedirs(d0)
        synth_xy(os.path.join(d0, "lineWall_p.xy"), x, np.full_like(x, sol["region1"]["p"]))
        ts_w, vs_w, _ = series_for(case, "lineWall", x_wall_from_xy, sol)
        probe = ("import sys; sys.path.insert(0, %r)\n"
                 "import " + os.path.splitext(os.path.basename(__file__))[0] + " as g\n"
                 "import exact_osr as EX\n"
                 "try:\n"
                 "    ts, vs, _ = g.series_for(%r, 'lineWall', g.x_wall_from_xy, EX.solve(), t_min=0.0)\n"
                 "    print('NOGUARD', ts, vs)\n"
                 "except SystemExit as e:\n"
                 "    print('REFUSED' if e.code == 2 else 'WRONG_CODE')\n") % (HERE, case)
        pr = subprocess.run([sys.executable, "-c", probe], stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
        mutated = pr.stdout.decode().strip()
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
    if not (abs(t_arr - 2.0 / (sol["region1"]["u"] * math.sin(2.0 * abs(math.radians(
            sol["sigma_incident_deg_from_x"]))))) < 1e-12):
        refuse("R3 WINDOW CONTROL: arrival_time does not equal 2/(u1 sin 2 sigma)")
    if len(ts_w) != 1 or abs(ts_w[0] - post_t) > 1e-9 or abs(vs_w[0] - xw) > dx:
        refuse("R3 WINDOW CONTROL FAILED: the windowed series returned t=%r "
               "x_w=%r; expected the single post-arrival sample at t=%.6g with "
               "the crossing at x_w=%.6g" % (ts_w, vs_w, post_t, xw))
    if not mutated.startswith("REFUSED"):
        refuse("R3 WINDOW CONTROL DID NOT FLIP: with the guard mutated to a no-op "
               "(t_min=0.0) the series must hit the no-crossing t=0.05 sample and "
               "refuse exactly as R2 did; got %r" % mutated[:120])
    # and with the no-crossing sample absent, the no-op guard returns the SPURIOUS
    # crossing first -- the second half of the flip
    tmp = tempfile.mkdtemp(prefix="f15_r3_window2_")
    try:
        case = os.path.join(tmp, "case")
        for t, s in ((pre_t, sol_sp), (post_t, sol)):
            d = os.path.join(case, "postProcessing", "lineWall", "%g" % t)
            ...
            synth_xy(os.path.join(d, "lineWall_p.xy"),
                     x, smeared_profile(x, s, 3, dx, on_wall=True))
        ts_n, vs_n, _ = series_for(case, "lineWall", x_wall_from_xy, sol, t_min=0.0)
        ts_g, vs_g, _ = series_for(case, "lineWall", x_wall_from_xy, sol)
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
    if not (len(vs_n) == 2 and abs(vs_n[0] - 0.27) <= dx and len(vs_g) == 1
            and abs(vs_g[0] - xw) <= dx):
        refuse("R3 WINDOW CONTROL DID NOT FLIP: no-op guard returned %r, windowed "
               "guard returned %r" % (vs_n, vs_g))
    return dict(control="PZ-F15-R3-WINDOW_GUARD_driven_both_ways",
                t_arrival=t_arr, formula="1/(u1 sin sigma cos sigma) = 2/(u1 sin 2 sigma)",
                u1=sol["region1"]["u"], sigma_deg=sol["sigma_incident_deg_from_x"],
                first_admitted_sample=math.ceil(t_arr / SAMPLE_INTERVAL - 1e-9) * SAMPLE_INTERVAL,
                spurious_pre_arrival=dict(t=pre_t, x=0.27),
                true_post_arrival=dict(t=post_t, x=xw),
                windowed=dict(t=ts_w, x=vs_w),
                guard_mutated_with_no_crossing_sample=mutated[:40],
                guard_mutated_without=dict(t=ts_n, x=vs_n), passed=True)


GATE_BLOCK_NAMES = ("N_CAPTURE_CELLS", "END_TIME", "SAMPLE_INTERVAL", "CAP_CORE_MIN",
                    "RANKS", "LEVELS", "CELLS", "DX", "CLASS_C", "DIM", "VERDICTS",
                    "SETS", "bands", "smeared_profile", "demonstrate", "class_c",
                    "x_wall_from_xy", "e1_from_xy", "read_xy_p", "grade_one",
                    "completion", "iterative_series", "plant_control_e1",
                    "plant_control_x_wall")


def _gate_blocks(path):
    out = {}
    for node in ast.parse(open(path).read()).body:
        ...
        for n in names:
            if n in GATE_BLOCK_NAMES:
                out[n] = ast.dump(node, include_attributes=False)
    return out


def control_gate_blocks_identical_to_r2():
    """The gate-bearing definitions of THIS file and of the R2 parent, compared
    as AST dumps (docstrings included, line numbers excluded). The diff must be
    EMPTY. Driven: a copy of R2 with CAP_CORE_MIN altered must be seen to differ."""
    mine = _gate_blocks(os.path.abspath(__file__))
    r2p = os.path.join(HERE, "grade_f15_r2.py")
    if not os.path.isfile(r2p):
        refuse("R3 GATE-BLOCK CONTROL: parent %s absent" % r2p)
    theirs = _gate_blocks(r2p)
    missing = [n for n in GATE_BLOCK_NAMES if n not in mine or n not in theirs]
    if missing:
        refuse("R3 GATE-BLOCK CONTROL: definitions not found in both files: %s" % missing)
    differing = [n for n in GATE_BLOCK_NAMES if mine[n] != theirs[n]]
    if differing:
        refuse("R3 GATE-BLOCK CONTROL FAILED: these gate-bearing definitions differ "
               "from R2 by AST: %s. Gates must be byte-identical." % differing)
    tmp = tempfile.mkdtemp(prefix="f15_r3_gateblk_")
    try:
        mut = os.path.join(tmp, "mut.py")
        open(mut, "w").write(open(r2p).read().replace("CAP_CORE_MIN = 200.0",
                                                     "CAP_CORE_MIN = 201.0", 1))
        seen = [n for n in GATE_BLOCK_NAMES if _gate_blocks(mut)[n] != theirs[n]]
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
    if seen != ["CAP_CORE_MIN"]:
        refuse("R3 GATE-BLOCK CONTROL: the comparator did not see a planted cap "
               "change (saw %s)" % seen)
    return dict(control="PZ-F15-R3-GATE_BLOCKS_ast_identical_to_R2_plus_planted_diff",
                compared=list(GATE_BLOCK_NAMES), differing=[], diff="EMPTY",
                planted_change_seen=seen, parent="grade_f15_r2.py", passed=True)
...
def arrival_time(sol):
    """R3: the exact arrival time of the incident shock at the wall station,
    from the FROZEN geometry and nothing measured. See T_ARRIVAL_NOTE."""
    u1 = float(sol["region1"]["u"])
    # the incident shock DESCENDS: exact_osr reports its angle from x as -29 deg;
    # the path length and tangential speed depend on |sigma| only
    sigma = abs(math.radians(float(sol["sigma_incident_deg_from_x"])))
    if not (0.0 < sigma < math.pi / 2.0) or u1 <= 0.0:
        refuse("arrival_time: geometry out of range (u1 %.6g, sigma %.6g rad)"
               % (u1, sigma))
    return 1.0 / (u1 * math.sin(sigma) * math.cos(sigma))


def series_for(case_dir, set_name, reader, sol, t_min=None):
    """The plateau series: the GRADED QUANTITY itself, recomputed at every
    sampled time from that time's own artifact. Not a proxy.

    R3: samples at t < t_min are NOT read (the reader's crossing detector is
    a steady-state instrument and a pre-arrival wall line has no crossing to
    detect). t_min defaults to arrival_time(sol); the selftest passes 0.0 as
    the mutated no-op guard and requires the answer to flip."""
    t_min = arrival_time(sol) if t_min is None else float(t_min)
    root = os.path.join(case_dir, "postProcessing", set_name)
    ts, vs, last = [], [], None
    for t, d in numeric_time_dirs(root):
        if t < t_min - 1e-9:
            continue                        # R3 window guard
...
def main(argv=None):
    ...
    controls = [EX.control_paper_agreement(sol),
                ...
                control_window_guard_flips(sol),
                control_gate_blocks_identical_to_r2()]
    bnd = bands(sol)
    ...
    out = a.out or os.path.join(a.root, "F15_R3_GRADED.json")
    report = dict(rung="F15-OSR29-R3", parent_grader_blob='133820b314baeae42281da26b1d1469a7e0189fb',
                  grandparent_grader_blob='e4076c03ca68d67e8247f73a4038d179cd7a20f2',
                  t_arrival=arrival_time(sol), t_arrival_note=T_ARRIVAL_NOTE,
                  prereg_commit=a.prereg_commit,
                  prereg=("verification/campaign/"
                          "F15_OSR29_R2_PREREGISTRATION.md (ADDENDUM R3 at the foot)"),
    ...
    print("F15-R3 -- OBLIQUE SHOCK REFLECTION M=2.9 -- TALLY (L-342 third registration; "
          "series windowed to t >= %.6f)" % arrival_time(sol))
    print("=" * 78)
    for r in rows:
