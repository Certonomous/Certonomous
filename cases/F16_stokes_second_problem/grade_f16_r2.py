#!/usr/bin/env python3
"""
F16-R2 -- COMPARATOR RE-REGISTRATION UNDER L-342 (2026-08-26). ZERO COMPUTE.
==========================================================================
PARENT: cases/F16_stokes_second_problem/grade_f16.py, blob 679823ff5688c8b9bcbfbaf646f420cd392c7224
        (identical on disk, at HEAD and at the freeze commit 2aea29d9).
DEFECT: the parent's TIME_RE is ^-anchored and applied with .finditer() to the
        WHOLE log text WITHOUT re.MULTILINE, so `^` matches only at byte 0 and
        the reader returned 0 of the 16000 `Time =` lines in
        verification/runs/F16_runs/coarse/log.icoFoam -> both gates PENDING at
        commit 6f8048b9 on a run that is complete by rule 4.
RULE:   L-342 -- a bookkeeping failure invalidates the bookkeeping, never the
        ...
        registration that grades the PRESERVED artefacts of run 2aea29d9.
DIFF SCOPE against the parent, and NOTHING ELSE:
  (1) this header block;
  (2) the L-342 field-class declaration (PHYSICS_CRITICAL / INFRASTRUCTURE)
      directly below VERDICTS, plus the infrastructure census that prints
      `BOOKKEEPING DEFECT` lines beside the tally instead of refusing;
  (3) TIME_RE compiled with re.MULTILINE (the only change in `completion`);
  (4) the selftest: `control_completion_reads_a_real_log` -- a driven control
      on a VERBATIM excerpt of the real coarse log (5 `Time =` lines planted,
      4 ExecutionTime/ClockTime lines, 1 `End`), positive and negative --
      appended to the controls list, and the class-C probe importing THIS
      module by its own basename instead of the parent's;
  (5) the JSON `rung` label reads F16-SL2-R2 and names the parent blob.
GATES, BANDS, THRESHOLDS, CAP AND LABELS ARE BYTE-IDENTICAL to the parent:
BAND_FACTOR, P_SOLVER_TOL, CAP_CORE_MIN, CLASS_C, bands(), demonstrate(),
grade_one() and the single grade_ladder call are untouched lines.
...
record only; ClockTime is not read by the parent at all).
    ...
    for name, _ny, _ns in EX.LEVELS:
        ...
        if rc is None:
            defects.append("BOOKKEEPING DEFECT: level %s RC.txt absent "
                           "(infrastructure field; grade proceeds)" % name)
    st = os.path.join(root, "STATUS.F16")
    census["status_file"] = open(st).read().strip() if os.path.isfile(st) else None
    if census["status_file"] is None:
        defects.append("BOOKKEEPING DEFECT: STATUS.F16 absent (infrastructure "
...
def main(argv=None):
    ...
    out = a.out or os.path.join(a.root, "F16_R2_GRADED.json")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    with open(out, "w") as f:
        json.dump(dict(rung="F16-SL2-R2", parent_grader_blob='679823ff5688c8b9bcbfbaf646f420cd392c7224',
                       prereg_commit=a.prereg_commit,
                       prereg="verification/campaign/F16_SL2_R2_PREREGISTRATION.md",
    ...
    print("F16-R2 -- STOKES' SECOND PROBLEM -- TALLY (L-342 re-registration)")
    print("=" * 78)
    for r in rows:
