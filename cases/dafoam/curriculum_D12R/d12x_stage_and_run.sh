#!/usr/bin/env bash
# =============================================================================
# CURRICULUM D12R -- THE LAUNCHER.  FROZEN INSTRUMENT.
...
BASE="${BASE:-/home/ubuntu/certonomous-runs/CURRICULUM-D12R-cylinder-unsteady}"
TUT="${TUT:-/home/ubuntu/dafoam-tutorials/Cylinder}"
SRC="${SRC:-/home/ubuntu/Certonomous/cases/dafoam/curriculum_D12R}"
RUNPY="$SRC/d12x_run_script.py"
GRADEPY="$SRC/d12x_grade.py"
...
PLANT_PROBE="d12x_run_script.py"      # known to exist; the planted control
missing=""
for want in d12x_run_script.py d12x_grade.py d12x_stage_and_run.sh; do
...
if [ "$PHASE" = "1" ]; then
  ...
  chmod 777 "$ROOT"
  : > "$MANIFEST"
  { echo "ITEM=CURRICULUM-D12R"; echo "ROW=$ROW"; echo "IMAGE=$IMG"; echo "IMAGE_ID=$GOT_ID";
...
run_stage () {
  ...
  local D="$ROOT/$STAGE" LOG="$ROOT/${STAGE}_${STAMP}.log" NAME="d12x_${STAGE}_${STAMP}"
...
PYCD
  ...
  if [ "$TASK" = "shell" ]; then
    ...
    CMD="mpirun --allow-run-as-root --bind-to none -np 1 python d12x_run_script.py --task=$TASK --dvIndex=$IDX --dvDelta=$DELTA --out=d12x_${STAGE}.json"
  fi
  T0=$(date -u +%s)
...
if [ "$PHASE" = "1" ]; then
  ...
  MA=$(memavail_gib)
  T0=$(date -u +%s)
  timeout 900 sudo -n docker run --name "d12x_S0_${STAMP}" --user 0:0 --cpus=1 \
  ...
  mrc=$?; T1=$(date -u +%s)
  sudo -n docker rm "d12x_S0_${STAMP}" >/dev/null 2>&1
