#!/usr/bin/env bash
# Curriculum D18 arm launcher -- Cone_Supersonic 2D wedge (DAHisaFoam, M 1.96) baseline
...
# section 7 and recorded in d18_run_arm_DELTAS_from_d16.diff (item names, the
# Cone_Supersonic tutorial, cpuset 12,15, the cap/memory table -- X arms 8g because
# they carry the adjoint, D18's and D8's roots forbidden); the D5 lineage:
...
# ===========================================================================
ITEM=D18
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D18-cone-hypersonic
...
/home/ubuntu/dafoam-tutorials/Cone_Supersonic
...
# ---- can be correct and its ledger still be a foreign one moved in.  D18
...
#
# D18 REGISTERED DELTA (PREREGISTRATION.md section 5): cpuset 12,15 -- the
# dafoam-supervisor's FIFTEENTH-session placement; DISJOINT from D5 8,10,11,13,
# D4-SHIPPED 5,6,7,9, D15 2,3, D16 4,14, D6 2,3,4,14; not core 0.  OVERLAPS the
# supervisor's D8R placement 0,1,12,15 -- disclosed; the chain driver's G-CPUSET
# wait-and-retry refuses to start an arm while a live container shares a core.
CPUSET=12,15
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d18_" | tr '\n' ',' | sed 's/,$//'
}

# ---- REGISTERED CAP TABLE (PREREGISTRATION.md §4, verbatim) ---------------
#   arm    ranks  core-min cap   in-container wall   memory cap
#   MESH     1        5.0            300 s              4g
#   X-S      2       90.0           2700 s              8g   (carries the adjoint)
#   X-P      2       90.0           2700 s              8g   (carries the adjoint)
#   F-S      2      300.0           9000 s              4g   (primals only)
#   F-P      2      300.0           9000 s              4g   (primals only)
cap_core_min() {
  case "$1" in
    MESH)     echo 5.0 ;;
    X-S|X-P)  echo 90.0 ;;
...
MD5_RUNSCRIPT=e6f0baf0e07a4ede7bff555e5e0ed332   # d18_runScript.py = the tutorial runScript.py with D18's ONE REGISTERED DELTA (U0 680.0 -> 1736.0); NOT a byte copy
MD5_XF=3e63a9fa945bc6d7b3196e1e3e084a7f                                 # d18_xf.py (set at freeze)
MD5_DECOMP=68ecc827562886fb43c3aedb0627b344      # d18_decomposeParDict (numberOfSubdomains 2, scotch; the tutorial's says 8)

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: d18_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: d18_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d18_${ARM}_" 2>/dev/null | grep "^d18_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/d18_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/d18_xf.py"                     | md5sum -c - || { echo "ABORT xf md5"; exit 4; }
echo "$MD5_DECOMP  $BASE/base/system/decomposeParDict" | md5sum -c - || { echo "ABORT decomposeParDict md5"; exit 4; }
# ---- D18: the ONE REGISTERED DELTA and the gate across its two carriers -------------
echo "2c18bb481b762706ab03bbbac3a91a4d  $BASE/base/0.orig/include/freestreamConditions" | md5sum -c - || { echo "ABORT freestreamConditions md5"; exit 4; }
echo "d6a5b389f0976801416bfd81909e0176  $BASE/d18_mach_agreement.py" | md5sum -c - || { echo "ABORT G-MACH instrument md5"; exit 4; }
# G-MACH runs BEFORE EVERY ARM, not once at staging: the BC file the solver reads and the
# producer that normalises CD carry the freestream INDEPENDENTLY, and a disagreement is a
# plausible-but-wrong CD that every other check in this family would pass.
D18_MACH=$(python3 "$BASE/d18_mach_agreement.py" --bc "$BASE/base/0.orig/include/freestreamConditions" --producer "$BASE/d18_runScript.py") || { echo "ABORT G-MACH: $D18_MACH"; exit 4; }
echo "D18_G_MACH_PASS $D18_MACH"
...
NAME="d18_${ARM}_${STAMP}"
WORK="$BASE/$ARM"
LOG="$BASE/${ARM}_${STAMP}.log"

# ---- stage a pristine copy for the arm ------------------------------------
# D16 REGISTERED DELTA (kept): MESH stages from base/ (the tutorial's inputs, no mesh);
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.d18_age_datum"
  ...
  rm -f "$WORK/d18_cmd.sh" "$WORK/checkMesh.log" "$WORK/log.blockMesh" "$WORK/log.mirrorMesh" "$WORK/log.renumberMesh" "$WORK/.d18_age_datum" 2>/dev/null
  cp -a "$BASE/d18_runScript.py" "$BASE/d18_xf.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/d18_X.json" "$WORK/d18_F.json" "$WORK/d18_F.jsonl" "$WORK/d18_X.jsonl" "$WORK/dRdWColoring_2.bin"; do
...
case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo D18_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d18_xf.py -mode X" ;;
  F-S|F-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d18_xf.py -mode F" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/d18_cmd.sh" > /dev/null 2>&1 \
