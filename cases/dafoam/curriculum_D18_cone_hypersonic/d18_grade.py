#!/usr/bin/env python3
"""Curriculum D18 COMPARATOR -- Cone_Supersonic 2D wedge (DAHisaFoam, M 1.96) baseline gradient,
...
NOT A RESULT / BLOCKED / PENDING).  Derived from curriculum_D16/d16_grade.py (d18_grade_DELTAS_from_d16.diff: item, caps, ceiling,
...
WHAT IT GRADES (PREREGISTRATION.md section 3):
  G1   completion, ARM-KIND AWARE: every arm needs kernel rc == 0 (docker inspect, and
       ...
       need the instrument's TERMINAL MARKER in the arm log (D18_X_WRITTEN / D18_F_WRITTEN);
...
ARTEFACT = {"MESH": "checkMesh.log", "X-S": "d18_X.json", "F-S": "d18_F.json", "X-P": "d18_X.json", "F-P": "d18_F.json"}
TERMINAL = {"X-S": "D18_X_WRITTEN", "F-S": "D18_F_WRITTEN", "X-P": "D18_X_WRITTEN", "F-P": "D18_F_WRITTEN"}
...
# D18: re-priced on D17's OWN MEASURED arms (C-165 anchor: MESH 0.167, X-S 7.833,
# F-S 75.733, X-P 7.100, F-P 82.000 core-min, total 172.833 against 180.3 predicted,
# ratio 0.959) -- the same mesh, the same solver, the same FIXED 500 pseudo-steps per
# primal, so the wall per primal is bounded by the step count and not by the Mach.
PREDICTED_CORE_MIN = {"MESH": 0.2, "X-S": 9.0, "F-S": 82.0, "X-P": 9.0, "F-P": 82.0}
...
MD5_MACH_REGISTERED = "d6a5b389f0976801416bfd81909e0176"   # d18_mach_agreement.py at the freeze commit
DELIVERED_CORES_FLOOR = 1.5
# D18 P3: the band is NOT D17's. It is the oblique-shock model CALIBRATED ON D17'S OWN
# MEASUREMENT and re-evaluated at M 5.0002. CD = (A_proj/A0) * p2/q with A_proj/A0 = 0.4:
# at M 1.9586 that model gives 0.269849 against D17's MEASURED 0.269886 -- 0.014 % -- so
# it is a validated model, not a guess. At M 5.0002 (beta 20.615 deg, Mn1 1.7605,
# p2/p1 3.4491, q 1.7733e6 Pa) it gives 0.078842. Band [0.055, 0.105] = -30 %/+33 %.
# THE FALSIFIER IS SHARP: the model's D17 residual was 0.014 %, so a miss wider than
# 30 % says the 1-D oblique-shock reading of this configuration BREAKS DOWN at
# hypersonic Mach -- which is itself the finding, not a defect of the item.
PRED = {"P2_CL_abs_max": 1.0e-3, "P3_CD_band": (0.055, 0.105), "P3_CD_point": 0.078842,
        "P7_SN_min_to_discriminate": 1.0, "P4_patched_min_pass": 4,
...
EXPECTED_UNITS = 29        # D17's 22 + G-MACH's 5 (U16-U20) + P3 and P7 (U21, U22)
...
def arm_datum(base, arm):
    ...
    p = os.path.join(d, ".d18_age_datum")
...
# ================= G9 / G10 / G12 ======================================================
def g_mach(root):
    """G-MACH at GRADE TIME -- the flow condition the run ACTUALLY used, re-read from the run
    root rather than taken from the launcher's word for it.

    THE VALUE IS PHYSICS; THE LEDGER LINE IS A RECORD (L-342 / R-RC).  So this reads the
    STAGED BYTES -- `base/0.orig/include/freestreamConditions`, which the solver read, and
    `d18_runScript.py`, which normalised the objective -- through the STAGED COPY of the gate
    instrument, whose md5 is asserted against the registered one first.  That is the freeze
    discipline applied to the flow condition: verify the instrument that RAN, then re-read the
    physics with it.  The ledger line is corroboration and its absence is INFRASTRUCTURE.
    If the staged bytes cannot be read, this REFUSES -- the Mach a run used is not optional."""
    import importlib.util, hashlib
    mp = os.path.join(root, "d18_mach_agreement.py")
    if not os.path.isfile(mp):
        refuse("G-MACH", {"staged_instrument_absent": mp,
                          "note": "the freestream this run used cannot be established"})
    got = hashlib.md5(open(mp, "rb").read()).hexdigest()
    if got != MD5_MACH_REGISTERED:
        refuse("G-MACH", {"staged_instrument_md5": got, "registered": MD5_MACH_REGISTERED,
                          "note": "the gate that ran is not the gate that was frozen"})
    spec = importlib.util.spec_from_file_location("d18_mach_staged", mp)
    mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
    bc = os.path.join(root, "base", "0.orig", "include", "freestreamConditions")
    pr = os.path.join(root, "d18_runScript.py")
    try:
        res = mod.check(bc, pr)
    except Exception as e:
        refuse("G-MACH", {"refusal": str(e),
                          "note": "re-read at grade time from the staged bytes, and it does not hold"})
    # corroboration only: the ledger line the driver teed. Absent -> INFRASTRUCTURE, never fatal.
    lp = os.path.join(root, "ledger.txt")
    seen = False
    if os.path.isfile(lp):
        for line in open(lp, errors="replace"):
            if '"gate": "G-MACH"' in line and '"ok": true' in line:
                seen = True; break
    res = dict(res)
    res["ledger_corroboration"] = "PRESENT" if seen else "NOT MEASURED (INFRASTRUCTURE, L-342)"
    res["verdict"] = "PASS"
    return res
...
def grade(root):
    ...
    # ---- rows
    X = {"S": read_X(os.path.join(root, "X-S", "d18_X.json")), "P": read_X(os.path.join(root, "X-P", "d18_X.json"))}
    F = {"S": read_F(os.path.join(root, "F-S", "d18_F.json")), "P": read_F(os.path.join(root, "F-P", "d18_F.json"))}
    ...
    controls = {"S": ctrl_control(F["S"]), "P": ctrl_control(F["P"]),
                "grader_plant_S": grader_plant_control(root, os.path.join(root, "F-S", "d18_F.json"), "S"),
                "grader_plant_P": grader_plant_control(root, os.path.join(root, "F-P", "d18_F.json"), "P")}
    g5 = {}
    for rk in ("S", "P"):
    ...
    # ---- P7 THE DISCRIMINATION PREDICTION, registered before the run because D17 taught
    # ---- this family that a PASS can imply a discrimination it never had. SIGNAL = the worst
    # ---- shipped-vs-patched divergence; NOISE = the worst PATCHED-row FD error on the same
    # ---- objective, the common-mode error both rows carry. S/N <= 1 means the two rows are
    # ---- INDISTINGUISHABLE and the item's value is the CELL, not the row comparison.
    worst_div = max([d["divergence_pct"] for d in div], default=None)
    pc = [c for c in g5["P"]["G5_CD"]["components"] if isinstance(c.get("rel_err_pct"), (int, float))]
    worst_noise = max([c["rel_err_pct"] for c in pc], default=None)
    if worst_div is None or worst_noise in (None, 0):
        sn = None
        preds["P7_two_rows_NOT_discriminating"] = NOT_MEASURED
    else:
        sn = worst_div / worst_noise
        preds["P7_two_rows_NOT_discriminating"] = "HIT" if sn <= PRED["P7_SN_min_to_discriminate"] else "MISS"
    preds["_P7_signal_to_noise"] = None if sn is None else round(sn, 4)
    preds["_P7_worst_divergence_pct"] = worst_div
    preds["_P7_worst_patched_FD_error_pct"] = worst_noise
    ...
    return {"item": "CURRICULUM-%s" % ITEM, "verdict": verdict,
            ...
            "gates": {"G1_completion": "PASS", "G-M2_mesh_identity": gm2, "G5_SHIPPED": g5["S"], "G5_PATCHED": g5["P"],
                      ...
                      "G-MACH_freestream_agreement": gmach},
            ...
            "capability_grid_cell": "2D . steady . hypersonic -- gradients computed + FD-verified; this item moves ONLY that column",
            "mach": gmach.get("mach"),
            "real_gas_caveat": "PERFECT GAS, CALORICALLY PERFECT. thermophysicalProperties carries a "
                               "constant Cp and mu 0; at M 5 real air begins to show vibrational excitation "
                               "and, higher still, dissociation. NONE OF THAT IS MODELLED. This cell is "
                               "hypersonic IN THE MACH NUMBER ONLY, and the record says so rather than "
                               "letting the grid imply a physical regime the solver never solved."}
...
def _fix(tmp, tweak=None):
    ...
    k = {"rc": {a: 0 for a in ARMS_REQUIRED}, "ke": {}, "oom": {a: "false" for a in ARMS_REQUIRED},
         ...
         "drop_row": set(), "inspect_file": set(), "dl": "1.99 n=10 max_nr_throttled=0", "CL": 0.0,
         # D18's registered P3 point, not D17's 0.27 -- a fixture carrying the PREVIOUS
         # item's baseline would make P3 read MISS on a clean fixture and teach nothing.
         "CD": PRED["P3_CD_point"], "mach": "ok"}
    if tweak:
        tweak(k)
    ...
    for arm in ARMS_REQUIRED:
        ...
        open(os.path.join(d, ".d18_age_datum"), "w").write("%d\n" % t0)
        log = "%s_x.log" % arm
        so = k["so"][arm]
        ...
        if arm == "MESH":
            ...
            if arm.startswith("X"):
                ...
                json.dump({"item": "D18", "mode": "X", "identity": ident, "nprocs": 2, "CD_baseline": repr(k["CD"]),
                ...
                json.dump({"item": "D18", "mode": "F", "identity": ident, "components_requested": COMPONENTS_REGISTERED,
    ...
    # ---- G-MACH's three staged artefacts. The gate re-reads the FLOW CONDITION from the run
    # ---- root at grade time, so the fixture must carry what a real run would carry.
    _here = os.path.dirname(os.path.abspath(__file__))
    _inc = os.path.join(root, "base", "0.orig", "include")
    os.makedirs(_inc, exist_ok=True)
    _src_bc = os.path.join(_here, "d18_freestreamConditions")
    _src_pr = os.path.join(_here, "d18_runScript.py")
    _src_mm = os.path.join(_here, "d18_mach_agreement.py")
    if k["mach"] != "no_instrument":
        _txt = open(_src_mm).read()
        if k["mach"] == "instrument_drift":
            _txt += "\n# planted drift -- the gate that ran is not the gate that was frozen\n"
        open(os.path.join(root, "d18_mach_agreement.py"), "w").write(_txt)
    open(os.path.join(_inc, "freestreamConditions"), "w").write(open(_src_bc).read())
    _pr = open(_src_pr).read()
    if k["mach"] == "producer_stale":
        _pr = _pr.replace("U0 = 1736.0", "U0 = 680.0")     # D17's value left behind
    if k["mach"] == "bc_stale":
        open(os.path.join(_inc, "freestreamConditions"), "w").write(
            open(_src_bc).read().replace("1736.0", "680.0"))
    open(os.path.join(root, "d18_runScript.py"), "w").write(_pr)
...
def selftest(tmp):
    ...
    # ---- G-MACH: D18's NEW gate. Planted-failure proofs (Sanaa sec.1, L-314). ----
    r = grade(_fix(tmp))
    unit("U23 clean fixture -> G-MACH PASS at the REGISTERED M 5.0002, re-read from the run root",
         r["gates"]["G-MACH_freestream_agreement"]["verdict"] == "PASS"
         and r["gates"]["G-MACH_freestream_agreement"]["mach"] == 5.0002 and r["mach"] == 5.0002)
    unit("U24 PLANTED the producer left at D17's U0 680.0 while the BC file carries 1736.0 -> "
         "REFUSAL. Every other limb of this grader would have passed and CD would have been "
         "wrong by the square of a ratio", refused(_fix(tmp, tw(mach="producer_stale"))))
    unit("U25 PLANTED the BC file left at D17's 680.0 while the producer carries 1736.0 -> REFUSAL "
         "(the flow would never have left D17's Mach)", refused(_fix(tmp, tw(mach="bc_stale"))))
    unit("U26 PLANTED the STAGED G-MACH instrument drifted from its registered md5 -> REFUSAL: "
         "the gate that ran is not the gate that was frozen", refused(_fix(tmp, tw(mach="instrument_drift"))))
    unit("U27 PLANTED the staged G-MACH instrument ABSENT -> REFUSAL, never a pass. The freestream "
         "a run used is not optional", refused(_fix(tmp, tw(mach="no_instrument"))))
    r = grade(_fix(tmp))
    unit("U28 P3 scores HIT on the registered D18 point %.6f (band %r), NOT D17's 0.27"
         % (PRED["P3_CD_point"], PRED["P3_CD_band"]),
         r["predictions"]["P3_CD_baseline_in_band"] == "HIT")
    unit("U29 P7 is SCORED and its signal-to-noise is PRINTED beside it, so a PASS can never "
         "imply a discrimination the item did not have (the D17 lesson)",
         r["predictions"]["P7_two_rows_NOT_discriminating"] in ("HIT", "MISS", NOT_MEASURED)
         and "_P7_signal_to_noise" in r["predictions"])
    ...
    unit("U21 ast.Assert count = 0 in d18_grade.py and d18_xf.py", count_asserts(os.path.abspath(__file__)) == 0 and count_asserts(os.path.join(here, "d18_xf.py")) == 0)
    p = os.path.join(tmp, "planted_assert.py")
    open(p, "w").write("x = 1\nassert x == 1\n")
    unit("U22 the assert counter sees a planted assert (=1)", count_asserts(p) == 1)
    print("D18 GRADER SELFTEST units=%d expected=%d failures=%d python_O=%s" % (n, EXPECTED_UNITS, len(fails), not __debug__))
    if n != EXPECTED_UNITS or fails:
        print("SELFTEST FAIL: %s" % (fails or "unit count %d != %d" % (n, EXPECTED_UNITS)))
        return 2
    print("D18 GRADER SELFTEST PASS %d/%d (counted against the frozen EXPECTED_UNITS)" % (n, EXPECTED_UNITS))
    return 0
...
def main():
    ...
    if a.selftest:
        d = os.path.join(a.tmpdir, "d18_selftest_%d" % os.getpid()); os.makedirs(d, exist_ok=True)
        return selftest(d)
    if not a.root:
        print("usage: d18_grade.py --root <run root> [--out FILE] | --selftest"); return 64
    try:
        r = grade(a.root)
    except Refusal as e:
