#!/usr/bin/env bash
# D18 G-ROOT.5 SELFTEST -- d16_groot5_selftest.sh re-pointed (d18_groot5_selftest_DELTAS_from_d16.diff); the d5_groot5_selftest.sh pattern (md5 32e19fffc33a43efd9959a02dfab5062),
# driven against D18's OWN launcher (G-ROOT.5 from birth, standing rule of 2026-08-26).
...
# (d) G-ROOT.1 refusals on D4's, D5's, D16's and D8's roots;
...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; L="$HERE/d18_run_arm.sh"
...
echo "D18 G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1)"
...
CN="d18_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
sudo -n docker run -d --name "$CN" --cpus=0.1 --cpuset-cpus=15 --memory=64m --memory-swap=64m "$IMG" bash -c "sleep 180" >/dev/null 2>&1 || { bad "could not start sacrificial container"; rmdir "$BASE"; exit 2; }
...
kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; rm -f "$BASE/d18_driver.pid"
# (b2) STALE pidfile (dead pid) must NOT block
echo "999999" > "$BASE/d18_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
# (d) G-ROOT.1/.2 from birth: D4's, D5's, D16's and D8's roots REFUSE
for forb in /home/ubuntu/certonomous-runs/CURRICULUM-D4-a2-wing-cdmin /home/ubuntu/certonomous-runs/CURRICULUM-D5-a2-wing-ffd-density /home/ubuntu/certonomous-runs/CURRICULUM-D16-a1-naca0012-transonic /home/ubuntu/certonomous-runs/CURRICULUM-D8-a6-twist-opt; do
...
# (e) G-ROW from birth, with the REAL images, on the 777 root holding the staged
# instruments.  D18 ADDS TWO: the G-MACH instrument and the freestreamConditions overlay
# that carries this item's ONE REGISTERED DELTA.  The launcher asserts both before G-ROW,
# so a fixture that omitted them would abort on an md5 and the (e) legs would report a
# G-ROW failure that is really a staging omission -- which is exactly what happened at
# 17:32:55Z on the first drive of this file and is recorded rather than tidied.
chmod 777 "$BASE"; mkdir -p "$BASE/base/system" "$BASE/base/0.orig/include"
cp -a "$HERE/d18_runScript.py" "$HERE/d18_xf.py" "$HERE/d18_mach_agreement.py" "$BASE/" \
  && cp -a "$HERE/d18_decomposeParDict" "$BASE/base/system/decomposeParDict" \
  && cp -a "$HERE/d18_freestreamConditions" "$BASE/base/0.orig/include/freestreamConditions"
...
# D18: the cleanup must name EVERY file the (e) fixture staged, or the freeze condition
# below (root ABSENT afterwards) fails on a file the test itself left behind.
rm -rf "$BASE/base" "$BASE/d18_runScript.py" "$BASE/d18_xf.py" "$BASE/d18_mach_agreement.py"
chmod 775 "$BASE"
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^d18_" && bad "a d18_ container survives" || ok "no d18_ container survives"
echo "D18 G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
