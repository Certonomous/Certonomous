...
(see --gatelist and --selftest):
    ...
    G12R-0b  manifest<->ledger stage binding       REFUSAL only   [D12R2-DEF-2]
...
# completion in their own log and filesystem evidence instead.  This is the constant
# that makes the status limb stage-kind aware [D12R2-DEF-1].  It is a REGISTERED
# TAXONOMY, not a lookup built from whatever the manifest happens to contain: a
# taxonomy derived from the data it classifies cannot refuse an unknown member.
JSON_TASKS     = ("run_model", "compute_totals")
EVIDENCE_TASKS = ("shell", "mesh")

# THE FROZEN ROW COUNT [D12R2-DEF-2].  Phase 1's stage graph is fully determined by
# the pre-registration, so its stage count is a NUMBER THIS DOCUMENT TYPES, not a
# length measured from the thing being checked:
#   S0 1 + S1a 1 + S1b 1 + S2a 1 + S2b 1 + S3 3 + S3b 16 + S4 6 + S5 1 + S7 2 = 33
# Deriving it as len(<the stage list>) and comparing it against that same list is the
# TAUTOLOGY this lab amended against: it can only ever be true.  33 is typed here so
# that a manifest which silently loses a stage DISAGREES WITH A CONSTANT.
EXPECTED_STAGE_ROWS_PHASE1 = 33
...
# present the evidence a mesh stage actually has: checkMesh's own verdict, a positive
# cell count read from the mesh it wrote, and the polyMesh files themselves.
REQUIRED_ROW_KEYS_MESH = ("mesh_check_ok", "mesh_n_cells", "mesh_polymesh_files")
MESH_POLYMESH_REQUIRED = ("points", "faces", "owner", "neighbour", "boundary")
...
GATES_EMITTED = ["G12R-0", "G12R-0b", "G12R-1", "G12R-2", "G12R-3", "G12R-3b", "G12R-4", "G12R-5",
...
def g0_completion(stages):
    ...
    for st in stages:
        ...
        # FOUND BY THIS FILE'S OWN UNIT U-15e, and recorded rather than quietly moved.
        # The first draft put this block after the `rc` and `oomkilled` limbs, so a row
        # MISSING `oomkilled` refused with "OOMKilled=None" -- a message that blames the
        # container for a key the row never carried.  BOTH are refusals, so no test that
        # merely asserts "it refused" could tell them apart; U-15e requires the refusal
        # to NAME the missing key, and that is what caught it.
        #
        # `.get()` collapses "absent" and "present but None" into the same value.  Where
        # the value is gated that collapse is harmless; where it is not, ABSENCE READS AS
        # A PASS.  So presence is established for every required key BEFORE any value is
        # read, for every stage, whatever its task.  L-302 at the row level.
        missing = [k for k in REQUIRED_ROW_KEYS if k not in st]
        if st.get("task") == "mesh":
            missing += [k for k in REQUIRED_ROW_KEYS_MESH if k not in st]
        if missing:
            raise Refusal("G12R-0 %s: manifest row is MISSING the key(s) %s. A row that "
        ...
        # D12R DIED HERE, and the way it died is the lesson.  This limb was
        # UNCONDITIONAL while its SIBLING step-count limb below carried an explicit
        # [D4] stage-kind repair: THE SAME BLINDNESS WAS FIXED IN ONE LIMB AND LEFT IN
        # THE OTHER, thirty-six lines apart, in the same function, in the same commit.
        #
        # THE RULE IS NOT "ACCEPT None".  It is: the task decides WHICH EVIDENCE OF
        # COMPLETION IS ADMISSIBLE, and every task must present some.  A `shell` or
        # `mesh` stage runs no run-script and writes no per-stage JSON BY DESIGN of the
        # launcher, so demanding one asks a `run_model` question of a stage that is not
        # one -- the same shape as reading CL where the objective is CD.
        task = st.get("task")
        ...
        if task in JSON_TASKS:
            # These run the run-script, which writes the per-stage JSON.  A missing or
            # non-COMPLETE status here is EXACTLY the D12R refusal, PRESERVED: the
            # repair widens the question, it does not weaken it.
            if st.get("status") != "COMPLETE":
                raise Refusal("G12R-0 %s: task=%r WRITES a per-stage JSON and its status "
                              "is %r, not 'COMPLETE'." % (nm, task, st.get("status")))
        else:
            # These write NO per-stage JSON.  This branch is NOT a waiver -- it is a
            # DIFFERENT question, and it refuses too: a status here means the launcher's
            # model of the stage and the comparator's disagree, and a disagreement
            # between two instruments about what a stage IS is never graded through.
        ...
        # ---- [D12R2-DEF-1] THE TIME LIMB IS TASK-CONDITIONAL, AND ITS MESH
        # ---- REPLACEMENT IS NOT A WAIVER BUT A HARDER QUESTION ---------------------
        if st.get("task") == "mesh":
            # checkMesh's OWN verdict, not this gate's opinion of the mesh.
            if st.get("mesh_check_ok") is not True:
                raise Refusal("G12R-0 %s: checkMesh did not report `Mesh OK.` "
                              "(mesh_check_ok=%r). A mesh stage whose own checker did "
                              "not pass has not completed, whatever its rc."
                              % (nm, st.get("mesh_check_ok")))
            nc = st.get("mesh_n_cells")
            if not isinstance(nc, int) or isinstance(nc, bool) or nc <= 0:
                raise Refusal("G12R-0 %s: mesh_n_cells=%r. A mesh stage that cannot "
                              "state a POSITIVE cell count read from the mesh it wrote "
                              "has produced nothing this item can solve on." % (nm, nc))
            files = st.get("mesh_polymesh_files")
            if not isinstance(files, list) or not files:
                raise Refusal("G12R-0 %s: mesh_polymesh_files=%r -- the mesh stage did "
                              "not enumerate the polyMesh it claims to have written, so "
                              "its cell count has no artifact behind it." % (nm, files))
            # `.gz` is stripped by the launcher before this list is written: the D2
            # defect of the superseded item was a typed datum `0/U` against a real
            # `U.gz`, and the same compression applies to every polyMesh file here.
            absent = [w for w in MESH_POLYMESH_REQUIRED if w not in files]
            if absent:
                raise Refusal("G12R-0 %s: polyMesh is MISSING %s (present: %s). A mesh "
                              "that cannot be read is not a mesh."
                              % (nm, ", ".join(absent), ", ".join(sorted(files))))
        else:
...
def g0b_manifest_ledger_binding(rows, ledger_path, expected_rows):
    """G12R-0b -- THE LIMB THAT WOULD HAVE CAUGHT S0.  REFUSAL only.

    D12R's phase 1 ran THIRTY-THREE stages and its manifest carried THIRTY-TWO.  The
    mesh stage S0 did not fail a check -- IT VANISHED -- and NOTHING REFUSED, because
    the completion gate can only grade rows it is handed and had no way to know a row
    was missing.  That is L-302 at the manifest level: an instrument that cannot say
    "I measured nothing" will report a number it did not measure.  The gate below is
    the one that says it.

    WHY THE LEDGER IS AN ADMISSIBLE SECOND WITNESS, when a second md5 reader was ruled
    NOT to be one.  The pre-registration's own words: INDEPENDENCE OF READERS IS NOT
    INDEPENDENCE OF QUESTIONS -- two readers of the same manifest are powerless against
    a shared wrong question.  The ledger is not a second reader of the manifest.  It is
    a SEPARATE ARTIFACT, appended by a DIFFERENT statement in the launcher (the shell
    `echo ... | tee -a`) than the one that appends the manifest row (a python `open(...,
    "a")`), and in D12R THOSE TWO STATEMENTS DISAGREED -- which is precisely how the
    defect is visible at all.  A stage that reaches the ledger and not the manifest has
    been dropped by one of the two, and this gate does not care which.

    AND THE COUNT IS CHECKED AGAINST A CONSTANT THIS FILE TYPES.  Comparing the
    manifest against the ledger alone would still pass if BOTH lost the same stage.
    `expected_rows` is a registered number derived from the PRE-REGISTRATION's stage
    graph, not measured from either artifact, so all three must agree.
    """
    names_manifest = [r.get("name") for r in rows]
    if any(n is None for n in names_manifest):
        raise Refusal("G12R-0b: a manifest row carries no `name`; the binding to the "
                      "ledger cannot be made and is not guessed at")
    if len(set(names_manifest)) != len(names_manifest):
        dupes = sorted(set(n for n in names_manifest if names_manifest.count(n) > 1))
        raise Refusal("G12R-0b: DUPLICATE manifest rows for %s -- a duplicate can mask "
    ...
    if not names_ledger:
        raise Refusal("G12R-0b: ledger %s holds NO `STAGE=` lines. THE READER MUST BE "
                      "SHOWN ABLE TO SEE A STAGE BEFORE ITS ZERO IS EVIDENCE (CLAUDE.md "
                      "rule 3); a zero here means the ledger format moved, not that no "
                      "stage ran." % ledger_path)

    only_ledger   = [n for n in names_ledger   if n not in set(names_manifest)]
    only_manifest = [n for n in names_manifest if n not in set(names_ledger)]
    if only_ledger or only_manifest:
        raise Refusal("G12R-0b: the manifest and the ledger DISAGREE about which stages "
                      "ran. In the ledger but NOT the manifest: %s. In the manifest but "
                      "NOT the ledger: %s. manifest=%d rows, ledger=%d STAGE= lines. A "
                      "stage that ran and left no row is UNGRADED AND SILENT."
                      % (", ".join(only_ledger) or "(none)",
                         ", ".join(only_manifest) or "(none)",
                         len(names_manifest), len(names_ledger)))
    if len(names_manifest) != expected_rows or len(names_ledger) != expected_rows:
        raise Refusal("G12R-0b: REGISTERED stage count is %d; manifest has %d rows and "
                      "the ledger %d `STAGE=` lines. Both artifacts agreeing with each "
                      "other is NOT enough -- they must agree with the number the "
                      "pre-registration fixed."
                      % (expected_rows, len(names_manifest), len(names_ledger)))
    return {"gate": "G12R-0b", "verdict": "PASS", "n_rows": len(names_manifest),
            "n_ledger_stage_lines": len(names_ledger), "registered": expected_rows}
...
def selftest(tmpdir):
    ...
    def base_stage(**kw):
        ...
        # [D12R2] `task` added for the same reason the RULING-5 limbs were: a fixture
        # missing a limb is a stage missing evidence, and the gate refuses absence.
    ...
    # ================= [D12R2] UNITS FOR THE TWO NEW LIMBS =====================
    # These do not merely call the new code; each one MAKES THE GUARDED CONDITION
    # OCCUR and requires the refusal to fire.  A unit that only exercises the happy
    # path proves the branch is reachable, not that it refuses.
    def _shellrow(**kw):
        row = {"name": "S1a", "rc": 0, "oomkilled": "false", "status": None,
               "task": "shell", "stage_kind": "steady",
               "end_line_present": True, "last_time": 500.0, "endTime": 500.0,
               "coldstart_ok": True, "age_guard_ok": True, "age_guard_detail": "ok",
               "expected_steps": 500, "time_line_count": 6,
               "execution_time_count": 7, "memavail_GiB": 20.0,
               "n_cd_lines": 0, "n_cd_negative": 0}
        row.update(kw)
        return row

    def _meshrow(**kw):
        row = {"name": "S0", "rc": 0, "oomkilled": "false", "status": None,
               "task": "mesh", "stage_kind": "mesh",
               "end_line_present": True, "last_time": None, "endTime": None,
               "coldstart_ok": True, "age_guard_ok": True, "age_guard_detail": "ok",
               "memavail_GiB": 20.0, "n_cd_lines": 0, "n_cd_negative": 0,
               "mesh_check_ok": True, "mesh_n_cells": 2450,
               "mesh_polymesh_files": ["boundary", "faces", "neighbour",
                                       "owner", "points"]}
        row.update(kw)
        return row

    def u15():
        # THE REPAIR ITSELF.  D12R refused exactly this row and produced nothing.
        exercised.add("G12R-0")
        check(g0_completion([_shellrow()])["verdict"] == "PASS",
              "G12R-0 still refuses a `shell` stage carrying no per-stage JSON")
        check(g0_completion([_meshrow()])["verdict"] == "PASS",
              "G12R-0 still refuses a `mesh` stage carrying no per-stage JSON")

    def u15b():
        # THE D12R PROTECTION, PRESERVED.  The repair widens the question; it must not
        # weaken it.  A task that DOES write a JSON is still refused without one.
        for t in ("run_model", "compute_totals"):
            for bad in (None, "STARTED", "FAILED"):
                _refuses(lambda t=t, bad=bad: g0_completion(
                    [_g0row(task=t, status=bad)]),
                    "G12R-0 accepted task=%s with status=%r" % (t, bad))

    def u15c():
        # THE EVIDENCE BRANCH IS NOT A WAIVER, and this is the unit that says so.
        # A `shell`/`mesh` stage that genuinely failed must still refuse.
        common = [("rc", 1), ("oomkilled", "true"), ("end_line_present", False),
                  ("coldstart_ok", False), ("age_guard_ok", False), ("age_guard_ok", None)]
        for mut, val in common + [("last_time", 499.0)]:
            _refuses(lambda mut=mut, val=val: g0_completion([_shellrow(**{mut: val})]),
                     "G12R-0 waved through a FAILED shell stage on %s=%r" % (mut, val))
        # `last_time` is DELIBERATELY ABSENT from the mesh list, and the omission is a
        # finding this unit made rather than a gap it left.  The first draft mutated it
        # on a mesh row too and the gate did not refuse -- CORRECTLY: a mesh stage has no
        # time, which is exactly why its `last time == endTime` limb was replaced by the
        # mesh evidence limbs (U-15f) instead of being left as a 0.0 == 0.0 that cannot
        # fail.  Asserting a refusal here would have forced a limb back that means
        # nothing, so the unit was corrected to the gate, and the reason recorded.
        for mut, val in common:
            _refuses(lambda mut=mut, val=val: g0_completion([_meshrow(**{mut: val})]),
                     "G12R-0 waved through a FAILED mesh stage on %s=%r" % (mut, val))

    def u15d():
        # A status on a no-JSON task is a DISAGREEMENT BETWEEN INSTRUMENTS, refused.
        _refuses(lambda: g0_completion([_shellrow(status="COMPLETE")]),
                 "G12R-0 accepted a `shell` row carrying a status")
        # and an unregistered task is refused rather than guessed at
        for t in (None, "unknown", "", "run_model2"):
            _refuses(lambda t=t: g0_completion([_shellrow(task=t)]),
                     "G12R-0 accepted an unregistered task=%r" % (t,))

    def u15e():
        # ABSENCE IS REFUSED BY NAME.  Every required key, deleted one at a time.
        for k in REQUIRED_ROW_KEYS:
            row = _g0row()
            row.pop(k, None)
            try:
                g0_completion([row])
                raise SelfTestFailure("G12R-0 accepted a row with %r DELETED" % k)
            except Refusal as exc:
                check(k in str(exc),
                      "G12R-0 refused a row missing %r without NAMING it: %s" % (k, exc))

    def u15f():
        # THE MESH EVIDENCE LIMBS.  A mesh stage has no time, so `last time == endTime`
        # would be 0.0 == 0.0 -- a limb that cannot fail.  These are the limbs that
        # replaced it, and each one is MADE TO FAIL here.
        exercised.add("G12R-0")
        check(g0_completion([_meshrow()])["verdict"] == "PASS",
              "G12R-0 refused a well-formed mesh row")
        _refuses(lambda: g0_completion([_meshrow(mesh_check_ok=False)]),
                 "G12R-0 accepted a mesh whose own checkMesh did not pass")
        _refuses(lambda: g0_completion([_meshrow(mesh_check_ok=None)]),
                 "G12R-0 accepted a mesh with NO checkMesh verdict")
        for nc in (0, -1, None, "2450", True, 2450.0):
            _refuses(lambda nc=nc: g0_completion([_meshrow(mesh_n_cells=nc)]),
                     "G12R-0 accepted mesh_n_cells=%r" % (nc,))
        for fl in ([], None, "points faces owner neighbour boundary"):
            _refuses(lambda fl=fl: g0_completion([_meshrow(mesh_polymesh_files=fl)]),
                     "G12R-0 accepted mesh_polymesh_files=%r" % (fl,))
        for drop in MESH_POLYMESH_REQUIRED:
            keep = [w for w in MESH_POLYMESH_REQUIRED if w != drop]
            try:
                g0_completion([_meshrow(mesh_polymesh_files=keep)])
                raise SelfTestFailure("G12R-0 accepted a polyMesh missing %r" % drop)
            except Refusal as exc:
                check(drop in str(exc),
                      "G12R-0 refused a polyMesh missing %r without naming it" % drop)
        # and the mesh row must still carry its OWN required keys
        for k in REQUIRED_ROW_KEYS_MESH:
            row = _meshrow(); row.pop(k, None)
            try:
                g0_completion([row])
                raise SelfTestFailure("G12R-0 accepted a mesh row with %r deleted" % k)
            except Refusal as exc:
                check(k in str(exc), "refusal did not name the missing %r" % k)

    def _write_pair(d, names, ledger_names=None):
        os.makedirs(d, exist_ok=True)
        rows = [{"name": n} for n in names]
        lp = os.path.join(d, "ledger.txt")
        with open(lp, "w") as f:
            f.write("PREREG_SHA=deadbeef\n")
            for n in (names if ledger_names is None else ledger_names):
                f.write("STAGE=%s TASK=x rc=0 wall_s=1 ranks=1 core_min=0.1\n" % n)
        return rows, lp

    _P1 = ["S0", "S1a", "S1b", "S2a", "S2b"] + \
          ["S3_r%d" % i for i in (1, 2, 3)] + \
          ["S3b_c%d_%s" % (c, s2) for c in range(4) for s2 in ("ap", "am", "bp", "bm")] + \
          ["S4_n%d_r%d" % (n, r) for n in (20, 40, 80) for r in (1, 2)] + \
          ["S5", "S7_plant", "S7_clean"]

    def u16():
        exercised.add("G12R-0b")
        # the registered graph is 33 stages; the fixture must BE 33 or the unit is
        # testing a different graph than the one the constant was registered for
        check(len(_P1) == EXPECTED_STAGE_ROWS_PHASE1,
              "the selftest's own phase-1 name list is %d, not the registered %d"
              % (len(_P1), EXPECTED_STAGE_ROWS_PHASE1))
        rows, lp = _write_pair(os.path.join(tmpdir, "g0b_ok"), _P1)
        r = g0b_manifest_ledger_binding(rows, lp, EXPECTED_STAGE_ROWS_PHASE1)
        check(r["verdict"] == "PASS" and r["n_rows"] == 33 and
              r["n_ledger_stage_lines"] == 33, "G12R-0b did not pass a well-formed pair")

    def u16b():
        # THE S0 CASE, MADE TO HAPPEN.  The ledger keeps 33, the manifest loses S0 --
        # byte-for-byte the D12R defect -- and the gate must refuse AND NAME S0.
        exercised.add("G12R-0b")
        d = os.path.join(tmpdir, "g0b_drop")
        rows, lp = _write_pair(d, _P1)
        dropped = [r for r in rows if r["name"] != "S0"]
        check(len(dropped) == 32, "the drop fixture is not 32 rows")
        try:
            g0b_manifest_ledger_binding(dropped, lp, EXPECTED_STAGE_ROWS_PHASE1)
            raise SelfTestFailure("G12R-0b DID NOT REFUSE a manifest missing S0 -- "
                                  "this is the exact D12R defect and it escaped")
        except Refusal as exc:
            check("S0" in str(exc), "G12R-0b refused but did not NAME S0: %s" % exc)
        # and the mirror: a row present in the manifest that never reached the ledger
        try:
            g0b_manifest_ledger_binding(rows + [{"name": "S9_ghost"}], lp,
                                        EXPECTED_STAGE_ROWS_PHASE1)
            raise SelfTestFailure("G12R-0b accepted a manifest row with no ledger line")
        except Refusal as exc:
            check("S9_ghost" in str(exc), "G12R-0b did not name the ghost row")

    def u16c():
        # BOTH ARTIFACTS AGREEING IS NOT ENOUGH.  Drop the stage from BOTH, so the two
        # witnesses are consistent and both wrong -- only the frozen constant catches it.
        exercised.add("G12R-0b")
        d = os.path.join(tmpdir, "g0b_both")
        short = [n for n in _P1 if n != "S0"]
        rows, lp = _write_pair(d, short)
        try:
            g0b_manifest_ledger_binding(rows, lp, EXPECTED_STAGE_ROWS_PHASE1)
            raise SelfTestFailure("G12R-0b accepted a SELF-CONSISTENT pair that both "
                                  "lost the same stage -- the frozen count did nothing")
        except Refusal as exc:
            check("33" in str(exc), "G12R-0b did not cite the registered count: %s" % exc)

    def u16d():
        # THE READER MUST BE SHOWN ABLE TO SEE A STAGE (rule 3), plus the two shapes
        # that would let a bare count be fooled.
        exercised.add("G12R-0b")
        d = os.path.join(tmpdir, "g0b_blind")
        os.makedirs(d, exist_ok=True)
        rows, lp = _write_pair(d, _P1)
        # leg 1: the reader DOES see stages in a real ledger -- proved by u16 passing.
        # leg 2: a ledger with no STAGE= lines must REFUSE, not read as "nothing ran"
        empty = os.path.join(d, "empty_ledger.txt")
        with open(empty, "w") as f:
            f.write("PREREG_SHA=deadbeef\nSPENT_CORE_MIN=0.0 of cap 600.0\n")
        ...
        _refuses(lambda: g0b_manifest_ledger_binding(dup, lp,
                                                     EXPECTED_STAGE_ROWS_PHASE1),
                 "G12R-0b let a DUPLICATE row pad a manifest back to the right count")
        # leg 5: a row with no name cannot be bound and is not guessed at
        _refuses(lambda: g0b_manifest_ledger_binding(rows + [{}], lp,
                                                     EXPECTED_STAGE_ROWS_PHASE1),
                 "G12R-0b accepted a nameless row")
    ...
    _UNIT_LIST = [
        ...
        ("U-15  [D12R2-DEF-1] G12R-0 ACCEPTS a shell/mesh stage with no JSON", u15),
        ("U-15b [D12R2-DEF-1] G12R-0 STILL refuses a JSON task without COMPLETE", u15b),
        ("U-15c [D12R2-DEF-1] the evidence branch is NOT a waiver: failed shell/mesh refuse", u15c),
        ("U-15d [D12R2-DEF-1] G12R-0 refuses a status on a no-JSON task, and an unknown task", u15d),
        ("U-15e [D12R2-DEF-1] G12R-0 refuses a MISSING required key, BY NAME", u15e),
        ("U-15f [D12R2-DEF-1] mesh evidence limbs replace the vacuous time limb", u15f),
...
def plan(manifest_path, root, man_meta=None):
    ...
    out = {"gates": []}
    # [D12R2-DEF-2] THE BINDING RUNS FIRST, because every gate after it grades a set of
    # rows whose COMPLETENESS is exactly what this gate establishes.  Grading first and
    # counting afterwards would publish numbers from a set already known to be short.
    out["gates"].append(g0b_manifest_ledger_binding(
        rows, os.path.join(root, "ledger.txt"), EXPECTED_STAGE_ROWS_PHASE1))
...
# ---------------------------------------------------------------- demonstration
# THE RULING THIS IMPLEMENTS (dafoam-supervisor, 2026-08-26):
#   "DEMONSTRATE every new limb by making the condition it guards actually occur --
#    on sacrificial copies ... Under plain `python3` AND under `python3 -O`. A count
#    of units is not a demonstration, and a selftest that passes under `-O` is the
#    weak test -- what matters is whether the REFUSALS FIRE."
#
# So this mode does not assert anything about the gates.  It BUILDS BROKEN ARTIFACTS,
# runs THIS FILE ON THEM IN A CHILD PROCESS under both flags, and requires the child
# to have refused.  A selftest calls a function; this runs the program.
#
# SEEDED FROM THE REAL D12R PHASE-1 MANIFEST when one is supplied, so the rows under
# test are the actual bytes that failed on 2026-08-25 rather than a hand-written
# idealisation of them.  The S0 row is the one FIXTURE -- D12R never wrote one, which
# is the defect -- and it is labelled as a fixture wherever it appears.

_DEMO_MESH_FIXTURE = {"name": "S0", "task": "mesh", "stage_kind": "mesh",
                      "rc": 0, "oomkilled": "false", "end_line_present": True,
                      "last_time": None, "endTime": None, "coldstart_ok": True,
                      "age_guard_ok": True, "age_guard_detail": "fixture",
                      "status": None, "memavail_GiB": 20.0,
                      "n_cd_lines": 0, "n_cd_negative": 0,
                      "mesh_check_ok": True, "mesh_n_cells": 2450,
                      "mesh_polymesh_files": ["boundary", "faces", "neighbour",
                                              "owner", "points"]}
...
def _demo_child(d):
    """--demo-child : run G12R-0b then G12R-0 over one sacrificial bundle.

    Exit 0 = both gates passed.  Exit 2 = a gate REFUSED (with its message).  This is
    the process the demonstration runs under `python3` and under `python3 -O`.
    """
    rows = read_json(os.path.join(d, "rows.json"))
    try:
        g0b_manifest_ledger_binding(rows, os.path.join(d, "ledger.txt"),
                                    EXPECTED_STAGE_ROWS_PHASE1)
        g0_completion(rows)
    except Refusal as exc:
        print("REFUSAL: %s" % exc)
...
def _demo_write(d, rows, ledger_names):
    os.makedirs(d, exist_ok=True)
    with open(os.path.join(d, "rows.json"), "w") as f:
        json.dump(rows, f)
    with open(os.path.join(d, "ledger.txt"), "w") as f:
        f.write("PREREG_SHA=demo\n")
        for n in ledger_names:
            f.write("STAGE=%s TASK=x rc=0 wall_s=1 ranks=1 core_min=0.1\n" % n)
    return d


def demo(tmpdir, seed_manifest=None):
    root = os.path.join(tmpdir, "d12y_demo")
    if os.path.isdir(root):
        import shutil
        shutil.rmtree(root)
    os.makedirs(root)

    # ---- the seed.  REAL ROWS where we have them. ----------------------------
    if seed_manifest and os.path.isfile(seed_manifest):
        seed = _read_manifest_rows(seed_manifest)
        origin = "REAL D12R phase-1 rows from %s" % seed_manifest
    else:
        seed = []
        origin = "SYNTHETIC rows (no --manifest supplied)"
    if not seed:
        sys.stderr.write("REFUSAL: --demo needs a seed manifest with rows\n")
        return 2

    # D12R's S1a row is `shell` with status null -- the row that killed it.  Every
    # other row is carried through untouched.
    base = [dict(r) for r in seed]
    for r in base:
        if "task" not in r:
            sys.stderr.write("REFUSAL: seed row %r carries no `task`\n" % r.get("name"))
            return 2
    names = [r["name"] for r in base]
    if "S0" in names:
        sys.stderr.write("REFUSAL: the seed already carries an S0 row; this demo is "
                         "written for the D12R manifest, which does not\n")
        return 2
    healthy = [dict(_DEMO_MESH_FIXTURE)] + base       # 32 real + 1 fixture = 33
    hnames = [r["name"] for r in healthy]

    def mutate(idx_name, **kw):
        out = [dict(r) for r in healthy]
        for r in out:
            if r["name"] == idx_name:
                r.update(kw)
        return out

    def delkey(idx_name, key):
        out = [dict(r) for r in healthy]
        for r in out:
            if r["name"] == idx_name:
                r.pop(key, None)
    ...
    scen = []
    # LEG 0 -- THE PLANTED CONTROL.  A demonstration in which everything refuses
    # proves only that the file refuses.  This leg must PASS or the rest is worthless.
    scen.append(("CONTROL  healthy 33-row pair MUST PASS", 0,
                 _demo_write(os.path.join(root, "s0_control"), healthy, hnames), None))
    # ---- D12R2-DEF-1 --------------------------------------------------------
    scen.append(("DEF-1 a  S1a as a JSON task with status null  (THE D12R REFUSAL, PRESERVED)", 2,
                 _demo_write(os.path.join(root, "s1a"),
                             mutate("S1a", task="run_model"), hnames), "status"))
    scen.append(("DEF-1 b  S1a shell with rc=1                  (evidence branch is NO waiver)", 2,
                 _demo_write(os.path.join(root, "s1b"),
                             mutate("S1a", rc=1), hnames), "rc="))
    scen.append(("DEF-1 c  S1a shell, age guard FAILED", 2,
                 _demo_write(os.path.join(root, "s1c"),
                             mutate("S1a", age_guard_ok=False), hnames), "AGE GUARD"))
    scen.append(("DEF-1 d  S1a shell carrying a status          (instruments disagree)", 2,
                 _demo_write(os.path.join(root, "s1d"),
                             mutate("S1a", status="COMPLETE"), hnames), "disagree"))
    scen.append(("DEF-1 e  S1a with `age_guard_ok` DELETED      (absence named, not read as pass)", 2,
                 _demo_write(os.path.join(root, "s1e"),
                             delkey("S1a", "age_guard_ok"), hnames), "age_guard_ok"))
    scen.append(("DEF-1 f  S1a with an unregistered task", 2,
                 _demo_write(os.path.join(root, "s1f"),
                             mutate("S1a", task="wibble"), hnames), "taxonomy"))
    scen.append(("DEF-1 g  S0 mesh: checkMesh did NOT pass", 2,
                 _demo_write(os.path.join(root, "s1g"),
                             mutate("S0", mesh_check_ok=False), hnames), "checkMesh"))
    scen.append(("DEF-1 h  S0 mesh: ZERO cells                   (a vacuous 0==0 limb would pass)", 2,
                 _demo_write(os.path.join(root, "s1h"),
                             mutate("S0", mesh_n_cells=0), hnames), "mesh_n_cells"))
    scen.append(("DEF-1 i  S0 mesh: polyMesh missing `owner`", 2,
                 _demo_write(os.path.join(root, "s1i"),
                             mutate("S0", mesh_polymesh_files=["boundary", "faces",
                                                               "neighbour", "points"]),
                             hnames), "owner"))
    # ---- D12R2-DEF-2 --------------------------------------------------------
    scen.append(("DEF-2 a  S0 DROPPED from the manifest, kept in the ledger  (THE D12R DEFECT)", 2,
                 _demo_write(os.path.join(root, "s2a"),
                             [r for r in healthy if r["name"] != "S0"], hnames), "S0"))
    scen.append(("DEF-2 b  S0 dropped from BOTH                 (self-consistent, both wrong)", 2,
                 _demo_write(os.path.join(root, "s2b"),
                             [r for r in healthy if r["name"] != "S0"],
                             [n for n in hnames if n != "S0"]), "33"))
    scen.append(("DEF-2 c  a ledger with ZERO STAGE= lines      (rule 3: a blind reader)", 2,
                 _demo_write(os.path.join(root, "s2c"), healthy, []), "STAGE="))

    path = os.path.abspath(__file__)
    print("D12R2 DEMONSTRATION -- every limb's guarded condition MADE TO OCCUR")
    print("seed: %s" % origin)
    print("each scenario runs `%s --demo-child <dir>` as a CHILD PROCESS under BOTH flags\n"
          % os.path.basename(path))
    print("%-72s %-8s %-9s %-9s %s" % ("scenario", "expect", "python3", "python3 -O", "verdict"))
    bad = 0
    for label, want, d, needle in scen:
        got = {}
        outs = {}
        for flag in (False, True):
            cmd = [sys.executable] + (["-O"] if flag else []) + [path, "--demo-child", d]
            pr = subprocess.run(cmd, capture_output=True, text=True)
            got[flag] = pr.returncode
            outs[flag] = (pr.stdout or "") + (pr.stderr or "")
        ok = (got[False] == want and got[True] == want)
        # the refusal must also SAY WHY -- an exit code alone does not identify the limb
        if ok and needle:
            ok = all(needle in outs[f] for f in (False, True))
        if not ok:
            bad += 1
        print("%-72s %-8d %-9d %-9d %s"
              % (label, want, got[False], got[True], "ok" if ok else "*** WRONG ***"))
        if not ok:
            print("      under python3   : %s" % outs[False].strip()[:200])
            print("      under python3 -O: %s" % outs[True].strip()[:200])
    print("")
    if bad:
        print("DEMONSTRATION FAILED: %d of %d scenarios did not behave as registered"
              % (bad, len(scen)))
        return 1
    print("DEMONSTRATION PASSED: %d of %d scenarios, IDENTICAL under python3 and "
          "python3 -O." % (len(scen) - bad, len(scen)))
    print("The control PASSED and every defect REFUSED, so the refusals are "
          "discriminating and not a file that simply says no to everything.")
    return 0
