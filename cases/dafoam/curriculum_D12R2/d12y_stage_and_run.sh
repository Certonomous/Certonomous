#!/usr/bin/env bash
# =============================================================================
# CURRICULUM D12R2 -- THE LAUNCHER.  FROZEN INSTRUMENT.
...
BASE="${BASE:-/home/ubuntu/certonomous-runs/CURRICULUM-D12R2-cylinder-unsteady}"
...
missing=""
for want in d12y_run_script.py d12y_grade.py d12y_stage_and_run.sh; do
...
if [ "$PHASE" = "1" ]; then
  ...
  chmod 777 "$ROOT"
  : > "$MANIFEST"
  { echo "ITEM=CURRICULUM-D12R2"; echo "ROW=$ROW"; echo "IMAGE=$IMG"; echo "IMAGE_ID=$GOT_ID";
...
if [ "$PHASE" = "1" ]; then
  ...
  MA=$(memavail_gib)
  T0=$(date -u +%s)
  timeout 900 sudo -n docker run --name "d12y_S0_${STAMP}" --user 0:0 --cpus=1 \
