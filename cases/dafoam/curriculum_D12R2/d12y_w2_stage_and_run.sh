#!/usr/bin/env bash
# =============================================================================
# CURRICULUM D12R2-W2 -- THE CONTINGENCY-WINDOW LAUNCHER.  FROZEN INSTRUMENT.
#
...
BASE="${BASE:-/home/ubuntu/certonomous-runs/CURRICULUM-D12R2W2-cylinder-unsteady}"
...
for f in "$RUNPY" "$GRADEPY" "$SRC/d12y_w2_stage_and_run.sh"; do
...
missing=""
for want in d12y_run_script.py d12y_grade.py d12y_w2_stage_and_run.sh; do
...
if [ "$PHASE" = "1" ]; then
  ...
  chmod 777 "$ROOT"
  : > "$MANIFEST"
  { echo "ITEM=CURRICULUM-D12R2W2"; echo "ROW=$ROW"; echo "IMAGE=$IMG"; echo "IMAGE_ID=$GOT_ID";
...
aggregate_cap_assert () {
  ...
  if python3 -c "import sys; sys.exit(0 if $agg > $total else 1)"; then
    echo "REFUSE: AGGREGATE memory caps ${agg} GiB EXCEED physical ${total} GiB. A cap every" | tee -a "$LEDGER"
    echo "container could claim at once is a promise the box cannot keep. NOT LAUNCHED." | tee -a "$LEDGER"
    return 1
  fi
  return 0
}
...
run_stage () {
  ...
  aggregate_cap_assert || { echo "STAGE=$STAGE BLOCKED by the aggregate cap assert -- NOT LAUNCHED" | tee -a "$LEDGER"; return 9; }
