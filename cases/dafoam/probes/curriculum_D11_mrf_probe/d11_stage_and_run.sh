#!/usr/bin/env bash
# CURRICULUM D11 CAPABILITY PROBE launcher.  Frozen instrument.
...
BASE="${BASE:-/home/ubuntu/certonomous-runs/CURRICULUM-PROBES-D10-D11-D12/D11}"
SRC="${SRC:-/home/ubuntu/Certonomous/cases/dafoam/probes/curriculum_D11_mrf_probe/d11_case}"
RUNPY="${RUNPY:-/home/ubuntu/Certonomous/cases/dafoam/probes/curriculum_D11_mrf_probe/d11_run_script.py}"
TMO="${TMO:-240}"
...
timeout 300 sudo -n docker run --name "d11_mesh_${STAMP}" --user 0:0 --cpus=1 \
...
sudo -n docker rm "d11_mesh_${STAMP}" >/dev/null 2>&1
...
run_stage () {
  ...
  grep -qE "^\s+omega\s+${OMEGA};" "$BASE/$STAGE/constant/MRFProperties" \
