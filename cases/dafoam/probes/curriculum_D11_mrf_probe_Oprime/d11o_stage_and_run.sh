#!/usr/bin/env bash
# CURRICULUM D11-O' CAPABILITY PROBE launcher (the omega re-buy, ATTEMPT THREE AND LAST).
# Frozen instrument.
#
...
BASE="${BASE:-/home/ubuntu/certonomous-runs/CURRICULUM-PROBES-D10-D11-D12/D11O}"
SRC="${SRC:-/home/ubuntu/Certonomous/cases/dafoam/probes/curriculum_D11_mrf_probe_Oprime/d11o_case}"
RUNPY="${RUNPY:-/home/ubuntu/Certonomous/cases/dafoam/probes/curriculum_D11_mrf_probe_Oprime/d11o_run_script.py}"
TMO="${TMO:-240}"
...
timeout 300 sudo -n docker run --name "d11o_mesh_${STAMP}" --user 0:0 --cpus=1 \
...
sudo -n docker rm "d11o_mesh_${STAMP}" >/dev/null 2>&1
...
run_stage () {
  ...
  local NAME="d11o_${STAGE}_${STAMP}"
  ...
  timeout "$TMO" sudo -n docker run --name "$NAME" \
      ...
      "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
       mpirun --allow-run-as-root --bind-to none -np 1 python d11o_run_script.py -task $TASK -uOffset $UOFF -out d11o_${STAGE}.json" \
      > "$LOG" 2>&1
  rc=$?
