#!/usr/bin/env bash
# CURRICULUM D11-D' CAPABILITY PROBE launcher (the dictionary re-buy).  Frozen instrument.
#
...
BASE="${BASE:-/home/ubuntu/certonomous-runs/CURRICULUM-PROBES-D10-D11-D12/D11P}"
SRC="${SRC:-/home/ubuntu/Certonomous/cases/dafoam/probes/curriculum_D11_mrf_probe_Dprime/d11p_case}"
RUNPY="${RUNPY:-/home/ubuntu/Certonomous/cases/dafoam/probes/curriculum_D11_mrf_probe_Dprime/d11p_run_script.py}"
TMO="${TMO:-240}"
...
SPENT_CORE_MIN=0
OMEGA_PLANT="300.0"     # rad/s -- THE PLANT
OMEGA_INERT="0.0"
...
timeout 300 sudo -n docker run --name "d11p_mesh_${STAMP}" --user 0:0 --cpus=1 \
...
sudo -n docker rm "d11p_mesh_${STAMP}" >/dev/null 2>&1
...
run_stage () {
  ...
  local NAME="d11p_${STAGE}_${STAMP}"
  ...
  timeout "$TMO" sudo -n docker run --name "$NAME" \
      ...
      "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
       mpirun --allow-run-as-root --bind-to none -np 1 python d11p_run_script.py -task $TASK -uOffset $UOFF -out d11p_${STAGE}.json" \
      > "$LOG" 2>&1
  rc=$?
