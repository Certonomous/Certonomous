#!/usr/bin/env python3
"""
CURRICULUM D11-Fprime CAPABILITY PROBE grader (the omega re-buy, attempt three and last).  Frozen instrument.
...
def selftest():
    ...
    try:
        ...
        pass
    print("D11-Fprime GRADER SELFTEST: 7/7 PASS (A healthy, B inert-MRF refuses, C zero-adjoint GATE FAIL, "
          "D empty-set refuses, E bit-identical GATE FAIL, F FD-disagreement GATE FAIL, G noisy-clean refuses)")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--root", default="/home/ubuntu/certonomous-runs/CURRICULUM-PROBES-D10-D11-D12/D11F")
    ...
    try:
        ...
        oP = read_json(os.path.join(a.root, "omegaP", "d11f_omegaP.json"))
        o0 = read_json(os.path.join(a.root, "omega0", "d11f_omega0.json"))
        cl = read_json(os.path.join(a.root, "clean", "d11f_clean.json"))
        fp = read_json(os.path.join(a.root, "fdp", "d11f_fdp.json"))
        fm = read_json(os.path.join(a.root, "fdm", "d11f_fdm.json"))
        verdict, gates = grade(oP, o0, cl, fp, fm)
    except Refusal as e:
        print("D11-Fprime PROBE VERDICT: NOT A RESULT")
        print("GRADER REFUSED (exit 2): %s" % e)
        return 2
    for k in sorted(gates):
        print("  %-8s %-12s %s" % (k, gates[k][0], gates[k][1]))
    print("D11-Fprime PROBE VERDICT: %s" % verdict)
    return 0
