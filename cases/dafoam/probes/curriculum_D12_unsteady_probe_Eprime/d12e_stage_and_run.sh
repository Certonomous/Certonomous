#!/usr/bin/env bash
# CURRICULUM D12-E' ENVELOPE SECOND POINT launcher.  Frozen instrument.
#
# D12 (GATE REACHED, frozen c2913dcd) measured the checkpoint envelope at ONE window
# length, 5 steps, which cannot separate the FIXED cost of the dRdWTPC preconditioner
# from the PER-STEP cost of the checkpoints.  A single point extrapolates to anything.
# This runs the identical case at 10 steps so a two-point fit measures the slope.
# ONE stage, primal + unsteady adjoint.  No plant: this probe grades NO capability and
# reaches NO verdict about DAFoam -- it measures two numbers already gated elsewhere.
...
BASE="${BASE:-/home/ubuntu/certonomous-runs/CURRICULUM-PROBES-D10-D11-D12/D12E}"
TUT="${TUT:-/home/ubuntu/dafoam-tutorials/Cylinder}"
CDICT="${CDICT:-/home/ubuntu/Certonomous/cases/dafoam/probes/curriculum_D12_unsteady_probe_Eprime/d12e_controlDict_probe}"
RUNPY="${RUNPY:-/home/ubuntu/Certonomous/cases/dafoam/probes/curriculum_D12_unsteady_probe_Eprime/d12e_run_script.py}"
TMO="${TMO:-600}"
...
grep -q "^endTime         0.10;" "$BASE/mesh/system/controlDict" \
  || { echo "ABORT: probe controlDict endTime is not 0.10"; exit 1; }

timeout 600 sudo -n docker run --name "d12e_mesh_${STAMP}" --user 0:0 --cpus=1 \
...
sudo -n docker rm "d12e_mesh_${STAMP}" >/dev/null 2>&1
...
run_stage () {
  ...
  local NAME="d12e_${STAGE}_${STAMP}"
  ...
  for bad in "$BASE/$STAGE/0.01" "$BASE/$STAGE/0.10"; do
  ...
  timeout "$TMO" sudo -n docker run --name "$NAME" \
      ...
      "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
       mpirun --allow-run-as-root --bind-to none -np 1 python d12e_run_script.py -task $TASK -shapePlant $PLANT -out d12e_${STAGE}.json" \
      > "$LOG" 2>&1
  rc=$?
