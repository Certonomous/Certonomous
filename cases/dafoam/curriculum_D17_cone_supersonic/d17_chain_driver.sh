#!/usr/bin/env bash
# D17 chain driver -- DERIVED from curriculum_D16/d16_chain_driver.sh (itself from
# D15's and curriculum_D5/d5_chain_driver.sh, Addendum 2) with the REGISTERED DELTAS in
# PREREGISTRATION.md section 7 and d17_chain_driver_DELTAS_from_d16.diff (item
...
#       the ledger opens with an exact `ITEM=D17` line and a STAGED line
...
#   setsid nohup bash d17_chain_driver.sh MESH X-S F-S X-P F-P > <root>/chain_launch.out 2>&1 &
...
LAUNCHER="$HERE/d17_run_arm.sh"
GRADER="$HERE/d17_grade.py"
...
MD5_LAUNCHER=1de86c92e903a72dee10260b911b6ede
MD5_GRADER=a940b0ee51c9102bb27dd6aa4516f366
MD5_RUNSCRIPT=8621862522cf2af72f381f0d97660b8a
MD5_XF=37b0861594065fd3a292224e1446d25a
...
test $# -ge 1 || { echo "ABORT usage: d17_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/d17_driver.pid"
cd "$HERE" || exit 4
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$HERE/d17_decomposeParDict" "$BASE/base/system/decomposeParDict" || { echo "ABORT overlay decomposeParDict"; exit 4; }
  cp -a "$HERE/d17_runScript.py" "$HERE/d17_xf.py" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  ...
  echo "D17_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "D17_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT  $BASE/d17_runScript.py"; echo "$MD5_XF  $BASE/d17_xf.py"; echo "$MD5_DECOMP  $BASE/base/system/decomposeParDict";
  tut_md5_list "$BASE/base" | grep -v "/runScript.py$" | grep -v "/system/decomposeParDict$"; } | md5sum -c - || { echo "ABORT staged instrument/input md5"; exit 4; }
...
echo "D17_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
for ARM in $ARMS; do
  ...
  echo "D17_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW file=$(basename "$H5_FILE")"
  ...
  while true; do
    AGG=$(python3 "$HERE/d17_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
  ...
  echo "D17_AGGREGATE arm=$ARM waited=$WAITED $AGG"
  ...
  while true; do
    CPU=$(python3 "$HERE/d17_cpuset_overlap.py" "$CPUSET_REGISTERED" "d17_")
  ...
  echo "D17_CPUSET arm=$ARM waited=$CWAITED $CPU"
  echo "D17_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "D17_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
if echo "$MD5_GRADER  $GRADER" | md5sum -c - > /dev/null; then
  python3 "$GRADER" --root "$BASE" --out "$BASE/D17_grade_${GSTAMP}.json" > "$BASE/D17_grade_${GSTAMP}.out" 2>&1; GRC=$?
  echo "grader_rc=$GRC stamp=$GSTAMP out=D17_grade_${GSTAMP}.json note=comparator-exit-status-NOT-the-verdict" >> "$STATUS"
else
  echo "grader_rc=NOT_RUN stamp=$GSTAMP note=grader-md5-drifted-at-chain-end" >> "$STATUS"
fi
echo "D17_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC"
exit "$CHAIN_RC"
