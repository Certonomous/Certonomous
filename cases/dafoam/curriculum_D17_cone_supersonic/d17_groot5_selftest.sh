#!/usr/bin/env bash
# D17 G-ROOT.5 SELFTEST -- d16_groot5_selftest.sh re-pointed (d17_groot5_selftest_DELTAS_from_d16.diff); the d5_groot5_selftest.sh pattern (md5 32e19fffc33a43efd9959a02dfab5062),
# driven against D17's OWN launcher (G-ROOT.5 from birth, standing rule of 2026-08-26).
...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; L="$HERE/d17_run_arm.sh"
...
echo "D17 G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1)"
...
CN="d17_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
...
kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; rm -f "$BASE/d17_driver.pid"
# (b2) STALE pidfile (dead pid) must NOT block
echo "999999" > "$BASE/d17_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
cp -a "$HERE/d17_runScript.py" "$HERE/d17_xf.py" "$BASE/" && cp -a "$HERE/d17_decomposeParDict" "$BASE/base/system/decomposeParDict"
...
rm -rf "$BASE/base" "$BASE/d17_runScript.py" "$BASE/d17_xf.py"
chmod 775 "$BASE"
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^d17_" && bad "a d17_ container survives" || ok "no d17_ container survives"
echo "D17 G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
