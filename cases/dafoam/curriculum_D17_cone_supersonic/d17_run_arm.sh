#!/usr/bin/env bash
# Curriculum D17 arm launcher -- Cone_Supersonic 2D wedge (DAHisaFoam, M 1.96) baseline
...
# section 7 and recorded in d17_run_arm_DELTAS_from_d16.diff (item names, the
# Cone_Supersonic tutorial, cpuset 12,15, the cap/memory table -- X arms 8g because
# they carry the adjoint, D17's and D8's roots forbidden); the D5 lineage:
...
# ===========================================================================
ITEM=D17
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D17-cone-supersonic
...
# ---- can be correct and its ledger still be a foreign one moved in.  D17
...
#
# D17 REGISTERED DELTA (PREREGISTRATION.md section 5): cpuset 12,15 -- the
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d17_" | tr '\n' ',' | sed 's/,$//'
}
...
MD5_RUNSCRIPT=8621862522cf2af72f381f0d97660b8a   # d17_runScript.py = byte copy of the shipped Cone_Supersonic tutorial runScript.py
MD5_XF=37b0861594065fd3a292224e1446d25a                                 # d17_xf.py (set at freeze)
MD5_DECOMP=68ecc827562886fb43c3aedb0627b344      # d17_decomposeParDict (numberOfSubdomains 2, scotch; the tutorial's says 8)

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: d17_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: d17_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d17_${ARM}_" 2>/dev/null | grep "^d17_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/d17_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/d17_xf.py"                     | md5sum -c - || { echo "ABORT xf md5"; exit 4; }
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.d17_age_datum"
  ...
  rm -f "$WORK/d17_cmd.sh" "$WORK/checkMesh.log" "$WORK/log.blockMesh" "$WORK/log.mirrorMesh" "$WORK/log.renumberMesh" "$WORK/.d17_age_datum" 2>/dev/null
  cp -a "$BASE/d17_runScript.py" "$BASE/d17_xf.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/d17_X.json" "$WORK/d17_F.json" "$WORK/d17_F.jsonl" "$WORK/d17_X.jsonl" "$WORK/dRdWColoring_2.bin"; do
...
case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo D17_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d17_xf.py -mode X" ;;
  F-S|F-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d17_xf.py -mode F" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/d17_cmd.sh" > /dev/null 2>&1 \
