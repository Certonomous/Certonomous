...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d8g_" | tr '\n' ',' | sed 's/,$//'
}
...
# ---- block.
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d8g_${ARM}_" 2>/dev/null | grep "^d8g_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
case "$MODE" in
  P) CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d8g_of.py -mode P -level $LEVEL" ;;
  A) CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d8g_of.py -mode A -level $LEVEL" ;;
  F) CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d8g_of.py -mode F -level $LEVEL" ;;
esac
...
while true; do
  RUNNING=$(sudo -n docker inspect --format '{{.State.Running}}' "$CID" 2>>"$LA_TMP/la.stderr")
  RIRC=$?
  ...
  if [ "$(python3 -c "print(1 if $CM > $CEILING else 0)")" = "1" ]; then
    ...
    sudo -n docker stop -t 30 "$CID" >/dev/null 2>>"$LA_TMP/la.stderr"
    break
  fi
  sleep 10
done
...
rc=$(sudo -n docker inspect --format '{{.State.ExitCode}}' "$CID" 2>>"$LA_TMP/la.stderr")
test -n "$rc" || rc=125
T1=$(date -u +%s)
WALL=$((T1-T0))
kill $SAMPLER 2>/dev/null; wait $SAMPLER 2>/dev/null
...
INSPECT=$(sudo -n docker inspect --format '{{.State.ExitCode}} {{.State.OOMKilled}}' "$CID" 2>>"$LA_TMP/la.stderr")
echo "$(sudo -n docker inspect --format '{{.State.ExitCode}} {{.State.OOMKilled}} {{.State.StartedAt}} {{.State.FinishedAt}} {{.HostConfig.CpusetCpus}} {{.HostConfig.Memory}}' "$CID" 2>>"$LA_TMP/la.stderr") $GOT_DIGEST" > "$BASE/${ARM}_${STAMP}.inspect.txt"   # the surviving kernel record (the grader's L-342 fallback)
sudo -n docker rm "$CID" >/dev/null 2>&1
