#!/usr/bin/env bash
# D8R G-ROOT.5 SELFTEST -- d16_groot5_selftest.sh re-pointed (d8r_groot5_selftest_DELTAS_from_d16.diff); the d5_groot5_selftest.sh pattern (md5 32e19fffc33a43efd9959a02dfab5062),
# driven against D8R's OWN launcher (G-ROOT.5 from birth, standing rule of 2026-08-26).
...
# (d) G-ROOT.1 refusals on D4's, D5's, D8's and D17's roots;
...
#     holding ONLY the three staged instruments (no base/), so a launcher that
#     got past G-ROW still refuses for want of base/ with nothing to remove; the copies are removed afterwards and the root's emptiness asserted.
...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; L="$HERE/d8r_run_arm.sh"
BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D8R-a6-twist-opt-conv
IMG=dafoam/opt-packages:latest; IMG_P=dafoam-idwarp-rot:v1; BOGUS=no-such-image:selftest; ARM=O-S
PASS=0; FAIL=0
ok()  { echo "  [OK ] $1"; PASS=$((PASS+1)); }
bad() { echo "  [BAD] $1"; FAIL=$((FAIL+1)); }
echo "D8R G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1)"
...
CN="d8r_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
...
kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; rm -f "$BASE/d8r_driver.pid"
# (b2) STALE pidfile (dead pid) must NOT block
echo "999999" > "$BASE/d8r_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
# (d) G-ROOT.1/.2 from birth: D4's, D5's, D8's and D17's roots REFUSE
for forb in /home/ubuntu/certonomous-runs/CURRICULUM-D4-a2-wing-cdmin /home/ubuntu/certonomous-runs/CURRICULUM-D5-a2-wing-ffd-density /home/ubuntu/certonomous-runs/CURRICULUM-D8-a6-twist-opt /home/ubuntu/certonomous-runs/CURRICULUM-D17-cone-supersonic; do
...
cp -a "$HERE/d8r_runScript.py" "$HERE/d8r_of.py" "$BASE/" && cp -a "$HERE/d8r_decomposeParDict" "$BASE/base/system/decomposeParDict"
...
if [ "$rc" -eq 4 ] && echo "$out" | grep -q "ABORT G-ROW arm F-S is registered on the SHIPPED row; got ROW=PATCHED"; then ok "(e2) F-S on the PATCHED image -> rc=4 G-ROW REFUSED before any staging"; else bad "(e2) rc=$rc: $(echo "$out" | tail -2)"; fi
out=$(bash "$L" O-S "$IMG" 2>&1); rc=$?
if [ "$rc" -eq 5 ] && echo "$out" | grep -q "D4S_G_ROW_PASS row=SHIPPED" && echo "$out" | grep -q "ABORT base/ carries no constant/polyMesh/points.gz"; then ok "(e3) O-S on the SHIPPED image -> G-ROW passes, then the arm REFUSES (rc=5) for want of D8's staged mesh -- nothing staged"; else bad "(e3) rc=$rc: $(echo "$out" | tail -2)"; fi
test -e "$BASE/O-S" && bad "(e3) left an O-S directory" || ok "(e3) no arm directory was created"
rm -rf "$BASE/base" "$BASE/d8r_runScript.py" "$BASE/d8r_of.py"
chmod 775 "$BASE"
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^d8r_" && bad "a d8r_ container survives" || ok "no d8r_ container survives"
echo "D8R G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
