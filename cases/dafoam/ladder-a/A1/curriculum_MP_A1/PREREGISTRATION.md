...
## §11. DATED ADDENDUM — 2026-09-07, RULE-2 **PRE-COMPUTE** REPAIR OF THE LIVE STALE `PRODUCER_MD5` INSIDE `mpa1_xf.py` + `MD5_XF` / `MD5_LAUNCHER` CASCADE RE-PINS (STRIKES NOTHING; NO GATE MOVED)

This is a rule-2 amendment taken **before first compute** (MP-A1 has recorded **zero** solver core-minutes; before-compute amendments are legal, `CLAUDE.md` rule 2). It strikes nothing above, alters **no** gate, threshold, band, cap, deadline, label or instrument **logic**, and does **not** re-open the c4e84348 freeze's **gates**. `lines whose number changed above this section: 0.`

**CONDITION.** §10's line-411 latent observation is now repaired under supervisor direction. `mpa1_xf.py:127` held `PRODUCER_MD5 = "0c026d72047b605099125258e3152f93"` — the SO-3 **parent** runScript md5 — while its own comment (lines 123–126) requires it to equal `md5(mpa1_runScript.py)`. This constant is dereferenced in a **live runtime check** at `mpa1_xf.py:559–563`: `got = md5_of(PRODUCER)` where `PRODUCER = "mpa1_runScript.py"` (line 101), and `if got != PRODUCER_MD5: sys.stderr.write("MPA1_XF REFUSE producer md5 ...") ; sys.exit(2)`. Confirmed by read: `got` hashes the raw bytes of `mpa1_runScript.py` (frozen md5 `bb3ba3a61b19dc8564e247cdb11e9147`) via `hashlib.md5` (line 193–195). With the stale pin, the correct frozen producer would hash to `bb3ba3a6…` ≠ `0c026d72…`, so the check would **REFUSE (exit 2)** on every invocation. `mpa1_xf.py` runs only in the **XE/FE arms** (launcher invocation, not the MESH pre-container path), so this did **not** cause the earlier MESH `rc=4` (§9/§10) — but it **would abort the XE/FE arms** the moment they ran. This is the same rename-omission class as §9/§10, one instrument-internal layer deeper.

**HOW CHECKED — EXHAUSTIVE constant audit (by role, not by name-shape).** Every 32-hex constant across **all** MP-A1 `.py`/`.sh` files was enumerated and, for each that is compared against a file's md5 (both `MD5_*`-named pins **and** non-prefixed ones such as `PRODUCER_MD5`), checked pin-vs-guarded-file's-actual-md5 on disk. Findings: (a) the sole stale **functional** pin was `mpa1_xf.py:127 PRODUCER_MD5` (below); (b) all `MD5_*` pins in `mpa1_chain_driver.sh` and `mpa1_run_arm.sh`, plus the six `MD5_TUT_*` external-tutorial pins, were re-verified **OK** as §9/§10 left them, **before** the cascade; (c) constants deliberately **out of scope** because they are **not** repo-file md5 pins: `ROW_BY_SO_MD5`/`SO_MD5` (container `libidwarp.so` toolchain identities `85f59e87…`/`f0fcb488…`, verified against the `.so` inside the container at runtime) and `IMG_*_DIGEST`/`IMG_DIGEST` (sha256 docker-image digests); and two **prose** occurrences of the old `0c026d72…` value that are true historical derivation notes, not checked constants (`mpa1_runScript.py:79` docstring "derived from curriculum_SO3 … parent md5 0c026d72…"; `mpa1_run_arm.sh:492` comment) — **left unchanged** as accurate history.

**THE FIX + CASCADE + FIXPOINT.** Old value verified on each line before editing; hex-for-hex; comment alignment preserved; **no logic touched**. Because `PRODUCER_MD5` lives **inside** `mpa1_xf.py`, editing it changes that instrument's md5, which cascades into the two pins that guard `mpa1_xf.py` (`MD5_XF` in both the driver and the launcher) and, since the launcher's md5 then changes, into the driver's `MD5_LAUNCHER` that guards the launcher. Iterated to a fixpoint (the driver's own md5 is guarded by **no** live runtime pin — verified — so the chain terminates there):

| iter | file | pin / constant | guarded file | old | new |
|---|---|---|---|---|---|
| 1 | `mpa1_xf.py:127` | `PRODUCER_MD5` | `mpa1_runScript.py` | `0c026d72047b605099125258e3152f93` | `bb3ba3a61b19dc8564e247cdb11e9147` |
| — | ⇒ `mpa1_xf.py` md5 | (guarded file) | — | `8036ca85d502276dc172c626becd11d5` | `d25891fd41cd975bc85ff393fa61b23c` |
| 2 | `mpa1_run_arm.sh:516` | `MD5_XF` | `mpa1_xf.py` | `8036ca85d502276dc172c626becd11d5` | `d25891fd41cd975bc85ff393fa61b23c` |
| 2 | `mpa1_chain_driver.sh:93` | `MD5_XF` | `mpa1_xf.py` | `8036ca85d502276dc172c626becd11d5` | `d25891fd41cd975bc85ff393fa61b23c` |
| — | ⇒ `mpa1_run_arm.sh` md5 | (guarded file) | — | `3fab25cc046588f3fe7e528250c083aa` | `e4bf43ef8d5b3d88dcca19421e040df1` |
| 3 | `mpa1_chain_driver.sh:84` | `MD5_LAUNCHER` | `mpa1_run_arm.sh` | `3fab25cc046588f3fe7e528250c083aa` | `e4bf43ef8d5b3d88dcca19421e040df1` |
| — | ⇒ `mpa1_chain_driver.sh` md5 | **unguarded — fixpoint** | — | `e2a537afb8799ac634f8acff18147671` | `080bb287da87985578e6649eb636d350` |

Three edit-iterations; a fourth re-audit pass found **zero** stale. No circular dependency (the driver, top of chain, is not self-pinned).

**POST-REPAIR VERIFICATION — `ALL_PINS_MATCH=1`.** Full re-audit of every functional file-md5 pin (the `PRODUCER_MD5` internal pin, all nine driver instrument pins, the three launcher pins, and the six `MD5_TUT_*` external pins) vs disk: **every pin equals its guarded file's actual md5**, including the cascade re-pins (`MD5_XF = d25891fd… ×2`, `MD5_LAUNCHER = e4bf43ef…`). Sandbox dry-run of the live `PRODUCER_MD5` gate path (`mpa1_xf.py -mode X`, on a verbatim `cp -a` copy, **no** container): the gate **PASSED** — no `MPA1_XF REFUSE producer md5` emitted (the run then stops later, post-gate, on the host-only `openmdao` import that exists inside the container, unrelated to any pin). **Planted control:** a one-byte corruption appended to the copied `mpa1_runScript.py` flips its md5 and the gate **FIRES fail-closed** — `rc=2`, `MPA1_XF REFUSE producer md5 e241282a… != frozen bb3ba3a61b19dc8564e247cdb11e9147` (the reference now correctly the true frozen value, not the parent) — so the reader is not blind; the corrupted copy was discarded and the copy restored byte-identical (the frozen repo file was never touched — planting was on the scratch copy). The **MESH pre-container** instrument-integrity block (driver `md5sum -c` on all nine pins) was independently re-confirmed **rc=0** with the post-cascade driver pins.

**FAILED STAGING ROOT.** The registered run root `/home/ubuntu/certonomous-runs/CURRICULUM-MP-A1-a1-naca0012-alpha-multipoint-fixedlift-optimisation` is **ABSENT** (removed by the §10 lane), so a clean cold re-launch under the age-guard is possible; no re-creation was performed here.

**SCOPE.** This addendum changes only integrity constants — one instrument-internal `PRODUCER_MD5` reconciled to `md5(mpa1_runScript.py)`, and the `MD5_XF`/`MD5_LAUNCHER` pins cascaded to the resulting instrument bytes — reconciling them **to** the producer §8 already hash-locks. It moves **no** gate, threshold, band, cap, label or instrument **logic**; the grading path stays exactly the §8 files (the grader `mpa1_grade.py` md5 is unchanged). The instrument-content change to `mpa1_xf.py` is a legal **pre-compute** repair; the c4e84348 freeze's **gates are NOT re-opened**. The `NOT_FROZEN` sentinel (removed at `2f2e0321`) is **not** re-added; **re-launch remains the supervisor's act** after the check-1 read of these deltas. **ZERO SOLVER CORE-MINUTES (pure static pin reconciliation + a sandbox dry-run). STILL NOT ENQUEUED, SUBMISSIONS PARKED.**
