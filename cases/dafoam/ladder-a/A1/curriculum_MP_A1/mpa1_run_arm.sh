#!/usr/bin/env bash
# Curriculum MP-A1 arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam),
...
CPUSET=8   # MP-A1 registered cpuset 8 (PROVISIONAL; re-read live occupancy at freeze).
           # G12 gates this against mpa1_grade.py:CPUSET_REGISTERED.  NOT core 0.
...
# ===========================================================================
ITEM=MPA1
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-MP-A1-a1-naca0012-alpha-multipoint-fixedlift-optimisation
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: mpa1_run_arm.sh <MESH|O-S|XE-S|FE-S|O-P|XE-P|FE-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: mpa1_run_arm.sh <MESH|O-S|XE-S|FE-S|O-P|XE-P|FE-P> <image>"; exit 64; }
...
echo "$MD5_XF  $BASE/mpa1_xf.py"                     | md5sum -c - || { echo "ABORT mpa1_xf.py md5"; exit 4; }
...
if [ "$ARM" = "MESH" ]; then
  ...
  cp -a "$BASE/mpa1_runScript.py" "$BASE/mpa1_xf.py" "$BASE/mpa1_stall.py" "$WORK/" \
  ...
  if [ -n "$XOPT_ARM" ]; then
    XOPT_SRC="$BASE/$XOPT_ARM/mpa1_xopt.json"
    test -f "$XOPT_SRC" || {
...
case "$ARM" in
  MESH)       CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo MPA1_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  O-S|O-P)    CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python mpa1_runScript.py -task run_driver -optimizer IPOPT" ;;
  XE-S|XE-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python mpa1_xf.py -mode X -xopt mpa1_xopt.json" ;;
  FE-S|FE-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python mpa1_xf.py -mode F -xopt mpa1_xopt.json" ;;
esac
test -n "${CMD:-}" || {
  echo "ABORT arm $ARM carries no registered command.  MP-A1 declares"
