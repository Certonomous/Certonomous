#!/usr/bin/env python3
"""AVWC EVIDENCE DUMP -- writes the FULL grade payload the frozen comparator's `main()`
drops for brevity (`avwc_grade.py` pops the "grade" key before serialising).
...
`AVWC_regrade.json` and REFUSES if any of them differ, so this file cannot quietly become a
second, softer reading of the same run.  Read `AVWC_regrade.json` as the verdict of record;
...
import avwc_grade as G                                                     # noqa: E402
...
def main():
    ...
    out = os.path.join(HERE, "AVWC_full_grades.json")
    ref = json.load(open(os.path.join(HERE, "AVWC_regrade.json")))
    d = os.path.join(tmp, "avwc_dump_%d" % os.getpid())
    ...
    for it in sorted(G.ITEMS):
        ...
        for k in SHARED:
            if r.get(k) != ref[it].get(k):
                print("REFUSAL: %s field %r disagrees with AVWC_regrade.json: %r vs %r"
