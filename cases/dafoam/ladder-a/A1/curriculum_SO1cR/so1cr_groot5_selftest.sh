#!/usr/bin/env bash
# SO-1cR LAUNCHER AND DRIVER GUARD SELFTEST -- every guard DRIVEN to FIRE, not
# asserted to exist.  DERIVED from curriculum_SO1a/so1a_groot5_selftest.sh with
# the registered deltas in so1cr_groot5_selftest_DELTAS_from_so1a.diff.
#
...
LAUNCHER="$HERE/so1cr_run_arm.sh"
DRIVER="$HERE/so1cr_chain_driver.sh"
REG_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO1cR-a1-naca0012-dragmin-npinv
TMP="${TMPDIR:-/tmp}/so1cr_groot5_$$"
N=0; FAIL=0
mkdir -p "$TMP" || exit 4
...
echo "SO1cR GUARD SELFTEST  stamp=$(date -u +%Y%m%dT%H%M%SZ)"
...
grep -q "CURRICULUM-SO1b-a1-naca0012-dragmin-opt" "$HERE/so1cr_run_arm.sh"
legc "(a4) SO-1a's and SO-1b's run roots are BOTH in FORBIDDEN_ROOTS" $?
grep -q "CURRICULUM-SO1a-a1-naca0012-dragmin-gradient" "$HERE/so1cr_run_arm.sh"
legc "(a5) ... and SO-1a's is too" $?
grep -q "CURRICULUM-SO1bR-a1-naca0012-dragmin-opt" "$HERE/so1cr_run_arm.sh"
...
# FORBIDDEN_ROOTS changed so1cr_run_arm.sh and stale-ed MD5_LAUNCHER; the suite stayed
...
# one-byte mutation of so1cr_run_arm.sh drove it [BAD] naming pinned-vs-actual).
...
#     exit 4; a one-byte edit to so1cr_decomposeParDict_simple left this leg GREEN
...
PINFAIL=0; PINSEEN=0
for pv in MD5_LAUNCHER:so1cr_run_arm.sh MD5_GRADER:so1cr_grade.py MD5_XN:so1cr_xn.py \
          MD5_RUNSCRIPT:so1cr_runScript.py \
          MD5_DECOMP_SCOTCH:so1cr_decomposeParDict_scotch \
          MD5_DECOMP_SIMPLE:so1cr_decomposeParDict_simple; do
  v=${pv%%:*}; f=${pv##*:}
  p=$(grep -oP "(?<=^$v=)[0-9a-f]{32}" "$HERE/so1cr_chain_driver.sh")
...
TUT_SRC=$(grep -oP '(?<=^TUT_SRC=)\S+' "$HERE/so1cr_chain_driver.sh")
TUTFAIL=0; TUTSEEN=0
if [ -z "$TUT_SRC" ]; then
...
PINS_IN_DRIVER=$(grep -cE '^MD5_[A-Z_0-9]+=[0-9a-f]{32}' "$HERE/so1cr_chain_driver.sh")
...
FOREIGN=$!
echo "$FOREIGN" > "$TMP/root5/so1cr_driver.pid"
...
if [ -f "$TMP/pr/ok.md" ]; then
  ...
  grep -v "SO1CR-CAP-MANIFEST" "$TMP/pr/ok.md" > "$TMP/pr/nomanifest.md"
...
L_OD=$(firstexec 'SO1CR_G_OPTDEP_PASS' "$LAUNCHER")
L_CP=$(firstexec 'SO1CR_G_CAP_PREREG_PASS' "$LAUNCHER")
...
legc "(e6) G-SO1B READS THE SPECIFIC GATES, NOT SO-1b's ITEM VERDICT -- gating on the item verdict would kill SO-1cR in SO-1b's own predicted outcome (SO-1b predicts item GATE FAIL)" $?
...
# =====================================================================
# SO-1cR LEG GROUP (r14) -- RULE 14 MADE MECHANICAL: **EVERY** CALL SITE.
#
# `CLAUDE.md` rule 14: "a lesson is not applied until EVERY call site asserts
# it".  SO-1c's amendment R8 registered a fence that said "the row-label
# comparison" -- SINGULAR -- and repaired the ONE site the lane had found.
# THREE were live: the launcher's G-OPTDEP (refused the REAL artefact in
# PREFLIGHT, rc=5, chain dead at its second arm), the grader's G-XSTAR (would
# have refused at GRADING), and the grader's own FIXTURE, which wrote a label
# the real producer never writes and so kept G-XSTAR GREEN.
#
# THE SWEEP IS `so1cr_rowlabel_sweep.py`, ONE IMPLEMENTATION, CALLED HERE.  A
# guard suite that runs its own copy of the logic proves nothing about the logic
# that ships, so this suite does not re-implement the sweep -- it drives it.
#
# IT IS NEVER BELIEVED ON A GREEN ALONE.  Its registered drive is a PAIR: RED on
# the KNOWN-BROKEN PREDECESSOR, CLEAN on the repaired tree.  That pairing is not
# ceremony -- an earlier version of the sweep was CLEAN ON THE BROKEN
# PREDECESSOR because it had dropped the string tokens it matches on, and only
# the positive control caught it.
# =====================================================================
SWEEP="$HERE/so1cr_rowlabel_sweep.py"
md5sum "$HERE/so1cr_chain_driver.sh" "$HERE/so1cr_run_arm.sh" "$HERE/so1cr_grade.py" \
       "$SWEEP" > "$TMP/r14_pins.md5"

# ---- (r14a) CLEAN ON THE SHIPPED TREE.
python3 "$SWEEP" "$HERE" > "$TMP/r14_real.out" 2>&1; R14_RC=$?
legc "(r14a) RULE 14 SWEEP over the item's THREE row-label consumers -- chain driver, launcher, grader -- finds NO unrepaired comparison, NO label derived as row[0], the SAME registered mapping in all three, and the FIXTURE deriving its label from that mapping rather than from the bare directory name" $R14_RC
[ "$R14_RC" = 0 ] || sed 's/^/         /' "$TMP/r14_real.out"

# ---- (r14b) THE POSITIVE CONTROL: RED ON THE REAL, BROKEN PREDECESSOR.
# SO-1c's OWN instruments, copied under SO-1cR's names and swept.  This is the
# strongest available ability-to-fail drive: not a synthetic mutant but THE
# CODE THAT ACTUALLY DIED, and the sweep must name every site that killed it.
R14_PRED="$TMP/r14_pred"
rm -rf "$R14_PRED"; mkdir -p "$R14_PRED"
PRED_SRC="$(cd "$HERE/../curriculum_SO1c" 2>/dev/null && pwd)"
N=$((N+1))
if [ -n "$PRED_SRC" ] && [ -f "$PRED_SRC/so1c_run_arm.sh" ] && [ -f "$PRED_SRC/so1c_grade.py" ] \
   && [ -f "$PRED_SRC/so1c_chain_driver.sh" ]; then
  cp -a "$PRED_SRC/so1c_chain_driver.sh" "$R14_PRED/so1cr_chain_driver.sh"
  cp -a "$PRED_SRC/so1c_run_arm.sh"      "$R14_PRED/so1cr_run_arm.sh"
  cp -a "$PRED_SRC/so1c_grade.py"        "$R14_PRED/so1cr_grade.py"
  python3 "$SWEEP" "$R14_PRED" > "$TMP/r14_pred.out" 2>&1; R14_PRED_RC=$?
  if [ "$R14_PRED_RC" = 1 ] \
     && grep -q "so1cr_run_arm.sh:468" "$TMP/r14_pred.out" \
     && grep -q "so1cr_grade.py:997"   "$TMP/r14_pred.out" \
     && grep -q "FIXTURE WRITES THE BARE DIRECTORY NAME" "$TMP/r14_pred.out"; then
    echo "  [OK ] (r14b) POSITIVE CONTROL -- THE SWEEP IS RED ON THE INSTRUMENTS THAT ACTUALLY DIED.  Driven against curriculum_SO1c's OWN frozen files it names run_arm.sh:468 (the preflight abort that killed the chain), grade.py:997 (G-XSTAR, which would have refused at grading) AND the concealing fixture.  A sweep shown red only on a synthetic mutant has not been shown to find the real thing"
    sed 's/^/         /' "$TMP/r14_pred.out"
  else
    echo "  [BAD] (r14b) THE SWEEP DID NOT GO RED, OR DID NOT NAME ALL THREE SITES, ON THE REAL BROKEN PREDECESSOR (rc=$R14_PRED_RC).  It has not been shown able to find the break it exists for"; FAIL=$((FAIL+1))
    sed 's/^/         /' "$TMP/r14_pred.out"
  fi
else
  echo "  [BAD] (r14b) curriculum_SO1c's frozen instruments are not on this box, so the POSITIVE CONTROL could not be driven.  NOT counted as passing -- a sweep believed on a green alone is exactly what this item is repairing"; FAIL=$((FAIL+1))
fi
rm -rf "$R14_PRED"

# ---- (r14c) AND A FOURTH CALL SITE NOBODY HAS THOUGHT OF: the sweep is a
# ---- DETECTOR, not a whitelist, so a NEW unrepaired site must turn it red too.
R14_MUT="$TMP/r14_mutant"
rm -rf "$R14_MUT"; mkdir -p "$R14_MUT"
cp -a "$HERE/so1cr_chain_driver.sh" "$HERE/so1cr_run_arm.sh" "$HERE/so1cr_grade.py" "$R14_MUT/"
python3 - "$R14_MUT/so1cr_grade.py" <<'PLANT'
import sys
p = sys.argv[1]
s = open(p).read()
anchor = "def g_xstar(root, meta, arm):"
plant = ('def g_rowcheck_PLANTED(ref, arm):\n'
         '    if ref.get("row") != ARM_ROW[arm]:\n'
         '        refuse("G-PLANTED", {"arm": arm})\n'
         '    return True\n\n\n')
if anchor not in s:
    raise SystemExit("PLANT ANCHOR MISSING -- the mutation could not be applied, so this "
                     "drive is NOT DRIVEN and must not be reported as passing")
open(p, "w").write(s.replace(anchor, plant + anchor, 1))
PLANT
PLANT_RC=$?
N=$((N+1))
if [ "$PLANT_RC" != 0 ]; then
  echo "  [BAD] (r14c) THE PLANT COULD NOT BE APPLIED -- the fourth-call-site drive did not run"; FAIL=$((FAIL+1))
else
  python3 "$SWEEP" "$R14_MUT" > "$TMP/r14_mut.out" 2>&1; R14_MUT_RC=$?
  if [ "$R14_MUT_RC" = 1 ] && grep -q "UNREPAIRED ROW-LABEL CALL SITE" "$TMP/r14_mut.out"; then
    echo "  [OK ] (r14c) A FOURTH CALL SITE, PLANTED INTO A COPY OF THE REPAIRED GRADER, TURNS THE SWEEP RED and is named by file and line.  The sweep had never seen this site and carries no entry for it -- it is a detector of the FORM, which is what rule 14 asks for"
    sed 's/^/         /' "$TMP/r14_mut.out"
  else
    echo "  [BAD] (r14c) THE SWEEP DID NOT FAIL ON A PLANTED FOURTH CALL SITE -- it is a whitelist, not a detector, and must not be trusted"; FAIL=$((FAIL+1))
  fi
fi
rm -rf "$R14_MUT"

# ---- (r14d) THE SHIPPED BYTES DID NOT MOVE ACROSS THE DRIVES.
N=$((N+1))
if md5sum -c --status "$TMP/r14_pins.md5" 2>/dev/null; then
  echo "  [OK ] (r14d) THE SHIPPED FILES ARE BYTE-IDENTICAL AFTER BOTH ABILITY-TO-FAIL DRIVES, proved by md5 against hashes taken before them -- a mutation drive that leaves the shipped bytes changed has repaired nothing and broken something"
else
  echo "  [BAD] (r14d) a shipped file's md5 MOVED across the ability-to-fail drives"; FAIL=$((FAIL+1))
fi

# ---- (r14e) THE SWEEP FAILS CLOSED ON A MISSING CONSUMER.
R14_GONE="$TMP/r14_gone"; rm -rf "$R14_GONE"; mkdir -p "$R14_GONE"
cp -a "$HERE/so1cr_chain_driver.sh" "$R14_GONE/"
python3 "$SWEEP" "$R14_GONE" > "$TMP/r14_gone.out" 2>&1
R14_GONE_RC=$?
[ "$R14_GONE_RC" = 1 ] && grep -q "MISSING CONSUMER" "$TMP/r14_gone.out"
legc "(r14e) THE SWEEP FAILS CLOSED: a consumer that has been RENAMED OR DELETED is reported MISSING, never treated as clean.  A file the sweep cannot see is not a file the sweep has cleared" $?
rm -rf "$R14_GONE"
...
cat <<'NOTDRIVEN'
  [NOT DRIVEN] (a0+) G-ROOT.5 leg (a) POSITIVE side -- "a RUNNING container already
      ...
      (no so1cr_ container exists, and the guard passes).  The one-line command that
      drives the positive side, for the supervisor to run BEFORE first compute:
        sudo -n docker run -d --name so1cr_Ns-P_selftest --cpus=0.1 --cpuset-cpus=15 \
             --memory=64m alpine sleep 30 >/dev/null && \
        BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO1cR-a1-naca0012-dragmin-npinv \
             bash so1cr_run_arm.sh Ns-P dafoam-idwarp-rot:v1 ; echo rc=$? ; \
        sudo -n docker rm -f so1cr_Ns-P_selftest
      EXPECTED: rc=3, "ABORT G-ROOT.5 a RUNNING container already carries this item's
      prefix and arm".
NOTDRIVEN

echo
LIVE_SO1C=$(sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -c "^so1cr_")
echo "(a0) NEGATIVE side of G-ROOT.5 leg (a): so1cr_ containers RUNNING = $LIVE_SO1C (expected 0)"
...
rm -rf "$TMP"
echo
echo "SO1cR guard selftest: $N legs driven, $FAIL fail, 1 leg NOT DRIVEN and named (never counted)"
