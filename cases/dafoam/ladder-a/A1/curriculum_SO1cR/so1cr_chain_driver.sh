#!/usr/bin/env bash
# SO-1cR chain driver -- DERIVED from curriculum_SO1a/so1a_chain_driver.sh with
# the REGISTERED DELTAS listed in PREREGISTRATION.md section 7 and recorded in
# so1cr_chain_driver_DELTAS_from_so1a.diff.  THE DELTAS ARE:
...
MD5_LAUNCHER=52d991ce80a1af30523a9f7253286e3b   # RE-PINNED by AMENDMENT R7, 2026-08-31 (was 777c33117dd65d882a9be04d27c07526; SO-1bR's run root ADDED to FORBIDDEN_ROOTS)
MD5_GRADER=a13cd4e7b53c5051abe697da485c3433   # RE-PINNED by AMENDMENT R6, 2026-08-28 (was 32af1c494db6884144151c7e7d7e81af)
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f
MD5_XN=1ffadf39206d5bb732dcfcf0288ba628
...
test $# -ge 1 || { echo "ABORT usage: so1cr_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/so1cr_driver.pid"
cd "$HERE" || exit 4
...
# FIRST LAUNCHER CALL.  SO-1cR verifies the gradient AT SO-1b's OPTIMUM, so what
...
# FAILURE IS SO-1b's FINDING.  Gating SO-1cR on SO-1b's item verdict would kill
# SO-1cR in exactly the case SO-1b expects.  The same defect was caught in this
...
#
# WHAT SO-1cR ACTUALLY CONSUMES, PER ROW:
...
# AND IT DELIBERATELY DOES NOT READ SO-1b's `G5E` FD GATE ON EITHER ROW.  SO-1cR
...
#   * SHIPPED accepts `PASS` OR `GATE REACHED`.  SO-1cR's shipped arm asks what a
...
if [ "$SO1B_GRADE_N" -eq 0 ]; then
  ...
  echo "  SO-1cR verifies the gradient AT SO-1b's OPTIMUM; with no graded optimum"
...
# THAT IS FALSE OF THE ARTEFACT: every gate SO-1cR asks for exists one level down
...
for row in ("PATCHED", "SHIPPED"):
    ...
    if gopt not in ACCEPT[row]:
        refuse("SO-1b's %s row G-OPT reads %r; SO-1cR accepts %s on that row.  "
    ...
    # ---- and the E artefact SO-1cR actually reads
...
print("SO1CR_G_SO1B_PASS grade=%s gates_at=%s rows=PATCHED,SHIPPED gopt=%s/%s gcl=PASS/PASS "
...
GRC=$?
if [ "$GRC" -ne 0 ]; then
  echo "ABORT G-SO1B refused (rc=$GRC).  SO-1cR is BLOCKED at ZERO core-minutes; nothing was staged and no container started."
  exit 7
fi
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$HERE/so1cr_decomposeParDict_scotch" "$BASE/base/system/decomposeParDict" || { echo "ABORT overlay decomposeParDict"; exit 4; }
  cp -a "$HERE/so1cr_runScript.py" "$HERE/so1cr_xn.py" "$HERE/so1cr_decomposeParDict_scotch" "$HERE/so1cr_decomposeParDict_simple" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  ...
  # ---- point ON THE SAME MESH and image".  SO-1cR regenerates the mesh in its own
  ...
  # root; the grader's is the SUBJECT, SO-1cR's own.  BOTH SIDES NOW REFUSE ON A
  ...
  if [ "$SO1B_MESH_N" -ne 1 ]; then
    ...
    echo "  The mesh fingerprint SO-1cR compares against must come from ONE NAMED file."
  ...
  # <<< G-MESHREF-SELECT
  echo "SO1CR_OPTREF_STAGED patched=$(md5sum "$BASE/optref/PATCHED/so1b_E.json" | cut -d\  -f1) shipped=$(md5sum "$BASE/optref/SHIPPED/so1b_E.json" | cut -d\  -f1) mesh_ref_lines=$(wc -l < "$BASE/optref/so1b_mesh_points_sha256.txt") source=$SO1B_BASE"
  ...
  echo "ITEM=SO1cR" > "$BASE/ledger.txt"
  ...
  echo "SO1CR_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "SO1CR_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT  $BASE/so1cr_runScript.py"; echo "$MD5_XN  $BASE/so1cr_xn.py";
  echo "$MD5_DECOMP_SCOTCH  $BASE/so1cr_decomposeParDict_scotch"; echo "$MD5_DECOMP_SIMPLE  $BASE/so1cr_decomposeParDict_simple";
...
echo "SO1CR_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
for ARM in $ARMS; do
  ...
  echo "SO1CR_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW file=$(basename "$H5_FILE")"
  ...
  while true; do
    AGG=$(python3 "$HERE/so1cr_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
  ...
  echo "SO1CR_AGGREGATE arm=$ARM waited=$WAITED $AGG"
  echo "SO1CR_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "SO1CR_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
echo "SO1CR_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC"
exit "$CHAIN_RC"
