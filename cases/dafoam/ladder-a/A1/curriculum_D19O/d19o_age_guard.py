#!/usr/bin/env python3
r"""Curriculum D19O -- THE REPAIRED AGE DATUM, AND THE CONTROLS THAT PROVE IT CAN
STILL REFUSE.

DERIVED from `curriculum_D19R/d19r_age_guard.py` (md5 RE-COMPUTED at this freeze
...
graded artefact name (`d19o_F.json`).  NO CLAUSE IS WEAKENED, NO EXCLUSION IS
...
THE ONE THING D19O ADDS, AND IT IS A REGISTRATION RATHER THAN A CODE CHANGE
===========================================================================
...
**D19O RUNS np = 1 ON EVERY ARM, SO `decomposePar` NEVER RUNS AND THE MUTATION
...
L1  THE DATUM LEAVES THE SOLVER'S WRITE SET, STRUCTURALLY.  The launcher stamps a
    ...
    `<parent of run root>/.d19o_datums/<ARM>.sentinel`.  This is ASSERTED, not
...
Usage:  d19o_age_guard.py --selftest
...
def _fixture(root, arm="S1"):
    ...
    _touch(os.path.join(d, "d19o_xf.py"), T, b"instrument\n")
    return d, T
...
INPUT_SUBDIRS = ["0.orig", "constant", "system", "d19o_xf.py"]


def selftest():
    ok = True
    tmp = tempfile.mkdtemp(prefix="d19o_guard_")
    try:
        run_root = os.path.join(tmp, "CURRICULUM-D19O-fixture")
        ...
        print()
        print("D19O PLANTED CONTROL -- RELATIVE, K REGISTERED AT FREEZE")
        ...
        print()
        print("D19O AGE GUARD SELFTEST: %s" % ("OK -- every red leg fired, both green legs passed"
                                               if ok else "FAILED"))
