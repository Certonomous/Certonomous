#!/usr/bin/env bash
# ===========================================================================
# Curriculum D19O -- THE CHAIN DRIVER'S CONTROLS.
#
...
TMP=$(mktemp -d /tmp/d19o_chain_XXXXXX)
trap 'rm -rf "$TMP"' EXIT
FAILED=0
...
echo "D19O CHAIN DRIVER SELFTEST"
...
grep -q "is not a registered D19O arm" "$TMP/c1.out" \
...
i_exist = src.index("D19O_18_3_EXISTENCE_OK")
...
if not_in_guard:
    ...
    allowed = {"d19o_xf_selftest.py", "d19o_grade_selftest.py"}
...
if unset:
    sys.stderr.write("PIN STILL UNSET: %r -- run d19o_repin.sh\n" % unset); sys.exit(1)
...
def marker(grade):
    subprocess.run(["bash", os.path.join(here, "d19o_stop_marker.sh"), root,
                    "selftest", grade], capture_output=True, text=True)
    return json.load(open(os.path.join(root, "D19O_STOP_MARKER.json")))
...
import sys
src = open(sys.argv[1]).read()
i_self = src.index("D19O_SELFTESTS_OK")
...
echo
if [ "$FAILED" -eq 0 ]; then
  echo "D19O CHAIN DRIVER SELFTEST: OK -- every leg fired"
  exit 0
fi
echo "D19O CHAIN DRIVER SELFTEST: FAILED -- $FAILED leg(s)"
exit 1
