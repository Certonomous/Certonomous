#!/usr/bin/env bash
# ===========================================================================
# Curriculum D19O -- THE LAUNCHER'S CONTROLS.
#
...
TMP=$(mktemp -d /tmp/d19o_runarm_XXXXXX)
trap 'rm -rf "$TMP"' EXIT
FAILED=0
...
ROW=$(bash -c ". \"$LAUNCHER\" --source-only; d19o_ledger_row O-P PATCHED img:p sha256:d 0 454 1 7.567 40.0 2220 40.0 12g '0 false' 27.0 27.0 11 '0.99 n=5' '' '' L.log S1")
python3 - <<EOF
import re, sys
...
d = ("/home/ubuntu/certonomous-runs/CURRICULUM-D19R-a1-naca0012-subsonic-plateau/"
...
print("    (r8) all %d mesh identity pins RE-COMPUTED from D19R's own MESH output "
...
echo
if [ "$FAILED" -eq 0 ]; then
  echo "D19O LAUNCHER SELFTEST: OK -- every refusal leg fired"
  exit 0
fi
echo "D19O LAUNCHER SELFTEST: FAILED -- $FAILED leg(s)"
exit 1
