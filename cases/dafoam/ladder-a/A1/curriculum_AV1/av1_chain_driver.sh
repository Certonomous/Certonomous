#!/usr/bin/env bash
# AV1 chain driver -- DERIVED from curriculum_D15/d15_chain_driver.sh (frozen at
# 8fc2bdeb, md5 4656f1a0e6381b9c03923cd2470d2814) with the REGISTERED DELTAS listed
# in PREREGISTRATION.md section 7 and recorded in av1_chain_driver_DELTAS_from_d15.diff:
...
#       used is this item's 4 GiB per arm (6 GiB for the np = 4 arms);
...
#       (the np = 4 dict IS the tutorial's own; the np = 2 dict is D15's, both
#       overlaid per arm by the launcher), copies the instruments from this case
...
#       the ledger opens with an exact `ITEM=AV1` line and a STAGED line
...
#   setsid nohup bash av1_chain_driver.sh MESH X1-S X2-S X4-S X1-P X2-P X4-P > <root>/chain_launch.out 2>&1 &
...
LAUNCHER="$HERE/av1_run_arm.sh"
GRADER="$HERE/av1_grade.py"
IMG_SHIPPED=dafoam/opt-packages:latest
IMG_PATCHED=dafoam-idwarp-rot:v1
BASE=/home/ubuntu/certonomous-runs/CURRICULUM-AV1-a1-naca0012-npinv
...
MD5_LAUNCHER=9996cfddf5c1f8637a498027cd48385d
MD5_GRADER=87f15e05130cfdb3cbf195d1daba6154
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f
MD5_XF=74011a9c5e6c760b8aec1c78e928b4d2
MD5_DECOMP_NP2=68ecc827562886fb43c3aedb0627b344
MD5_DECOMP_NP4=816f5ba44075fde47fa5db4269877bc8
...
test $# -ge 1 || { echo "ABORT usage: av1_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/av1_driver.pid"
cd "$HERE" || exit 4
...
if [ ! -d "$BASE" ]; then
  ...
  echo "$MD5_DECOMP_NP4  $BASE/base/system/decomposeParDict" | md5sum -c - || { echo "ABORT the tutorial's decomposeParDict is not the registered np4 dict"; exit 4; }
  cp -a "$HERE/av1_runScript.py" "$HERE/av1_x.py" "$HERE/av1_decomposeParDict_np2" "$HERE/av1_decomposeParDict_np4" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  ...
  echo "AV1_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "AV1_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT  $BASE/av1_runScript.py"; echo "$MD5_XF  $BASE/av1_x.py"; echo "$MD5_DECOMP_NP2  $BASE/av1_decomposeParDict_np2"; echo "$MD5_DECOMP_NP4  $BASE/av1_decomposeParDict_np4"; echo "$MD5_DECOMP_NP4  $BASE/base/system/decomposeParDict";
...
echo "AV1_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
cap_mem_gib() { case "$1" in X4-S|X4-P) echo 6 ;; *) echo 4 ;; esac; }   # 4g every arm, 6g at np = 4 (PREREGISTRATION.md section 4)
...
for ARM in $ARMS; do
  ...
  echo "AV1_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW file=$(basename "$H5_FILE")"
  ...
  while true; do
    AGG=$(python3 "$HERE/av1_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
  ...
  echo "AV1_AGGREGATE arm=$ARM waited=$WAITED $AGG"
  echo "AV1_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "AV1_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
if echo "$MD5_GRADER  $GRADER" | md5sum -c - > /dev/null; then
  python3 "$GRADER" --root "$BASE" --out "$BASE/AV1_grade_${GSTAMP}.json" > "$BASE/AV1_grade_${GSTAMP}.out" 2>&1; GRC=$?
  echo "grader_rc=$GRC stamp=$GSTAMP out=AV1_grade_${GSTAMP}.json note=comparator-exit-status-NOT-the-verdict" >> "$STATUS"
else
  echo "grader_rc=NOT_RUN stamp=$GSTAMP note=grader-md5-drifted-at-chain-end" >> "$STATUS"
fi
echo "AV1_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC"
exit "$CHAIN_RC"
