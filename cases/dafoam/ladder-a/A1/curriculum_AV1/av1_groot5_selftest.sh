#!/usr/bin/env bash
# AV1 G-ROOT.5 SELFTEST -- the d15_groot5_selftest.sh pattern (8fc2bdeb, md5 8a2a7df64b2db87615a20fab2d9208f8),
# driven against AV1's OWN launcher (G-ROOT.5 from birth, standing rule of 2026-08-26).
...
# (d) G-ROOT.1 refusals on D4's, D5's, D13's, D15's and D16's roots;
...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; L="$HERE/av1_run_arm.sh"
BASE=/home/ubuntu/certonomous-runs/CURRICULUM-AV1-a1-naca0012-npinv
IMG=dafoam/opt-packages:latest; IMG_P=dafoam-idwarp-rot:v1; BOGUS=no-such-image:selftest; ARM=X1-S
PASS=0; FAIL=0
ok()  { echo "  [OK ] $1"; PASS=$((PASS+1)); }
bad() { echo "  [BAD] $1"; FAIL=$((FAIL+1)); }
echo "AV1 G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1)"
...
CN="av1_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
...
kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; rm -f "$BASE/av1_driver.pid"
# (b2) STALE pidfile (dead pid) must NOT block
echo "999999" > "$BASE/av1_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
for forb in /home/ubuntu/certonomous-runs/CURRICULUM-D4-a2-wing-cdmin /home/ubuntu/certonomous-runs/CURRICULUM-D5-a2-wing-ffd-density /home/ubuntu/certonomous-runs/CURRICULUM-D13-a1-basin-restart /home/ubuntu/certonomous-runs/CURRICULUM-D15-a1-naca0012-subsonic /home/ubuntu/certonomous-runs/CURRICULUM-D16-a1-naca0012-transonic; do
...
cp -a "$HERE/av1_runScript.py" "$HERE/av1_x.py" "$HERE/av1_decomposeParDict_np2" "$HERE/av1_decomposeParDict_np4" "$BASE/" && cp -a "$HERE/av1_decomposeParDict_np4" "$BASE/base/system/decomposeParDict"
out=$(bash "$L" X1-P "$IMG" 2>&1); rc=$?
if [ "$rc" -eq 4 ] && echo "$out" | grep -q "ABORT G-ROW arm X1-P is registered on the PATCHED row; got ROW=SHIPPED"; then ok "(e1) X1-P on the SHIPPED image -> rc=4 G-ROW REFUSED before any staging"; else bad "(e1) rc=$rc: $(echo "$out" | tail -2)"; fi
...
out=$(bash "$L" X4-S "$IMG" 2>&1); rc=$?
if [ "$rc" -eq 5 ] && echo "$out" | grep -q "D4S_G_ROW_PASS row=SHIPPED" && echo "$out" | grep -q "ABORT arm X4-S expects an existing MESH/"; then ok "(e3) X4-S on the SHIPPED image -> G-ROW passes, then the arm REFUSES (rc=5) for want of the MESH arm's output -- nothing staged"; else bad "(e3) rc=$rc: $(echo "$out" | tail -2)"; fi
test -e "$BASE/X4-S" && bad "(e3) left an X4-S directory" || ok "(e3) no arm directory was created"
rm -rf "$BASE/base" "$BASE/av1_runScript.py" "$BASE/av1_x.py" "$BASE/av1_decomposeParDict_np2" "$BASE/av1_decomposeParDict_np4"
chmod 775 "$BASE"
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^av1_" && bad "an av1_ container survives" || ok "no av1_ container survives"
echo "AV1 G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
