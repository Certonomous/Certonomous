#!/usr/bin/env python3
"""Curriculum SO-2M COMPARATOR -- NACA0012 INCOMPRESSIBLE, the FD-VERIFIED
...
with the registered deltas in so2m_grade_DELTAS_from_so1a_grade.diff.  SIX of them
...
ARTEFACT = {"MESH": "checkMesh.log", "X-S": "so2m_X.json", "G-S": "so2m_F.json",
            "X-P": "so2m_X.json", "G-P": "so2m_F.json"}
TERMINAL = {"X-S": "SO2M_X_WRITTEN", "G-S": "SO2M_F_WRITTEN",
            "X-P": "SO2M_X_WRITTEN", "G-P": "SO2M_F_WRITTEN"}
...
# ---- the OUTPUT vocabulary, mirroring so2m_xm.py's registration ----------------------
OUTPUTS = ("CD", "CL", "CMZ")
...
def instrument_constants(path):
    """Read so2m_xm.py's OWN declared constants out of its source, by AST.
...
def flip_control_g5m(base, xpath, X, F, live, row_label):
    ...
    artefact with PLANT added to ONE d(CMZ)/d(shape) entry must read GATE FAIL.

    WHICH ENTRY IS THE COMPARATOR'S CHOICE AND IS REGISTERED HERE: the GRADED pair
    with the SMALLEST |d_ref|, because the plant is ABSOLUTE and its relative
    effect is PLANT/|d_ref| -- the smallest reference is where an absolute plant
    has the largest chance of crossing band D.  The arithmetic is printed either
    way, so a control that cannot cross says so with numbers."""
    ...
    target = min(cands, key=lambda c: abs(c["d_ref"]))
    j = json.load(open(xpath))
    idx = target["idx"]
    vals = list(j["adjoint"][GRADED_OUTPUT]["shape"])
    vals[idx] = repr(float(vals[idx]) + PLANT)
    ...
    if planted["verdict"] != "GATE FAIL":
        refuse("CONTROL", {"G5m_did_NOT_flip_to_GATE_FAIL_under_the_planted_copy": rec,
                           "note": "PREREGISTRATION.md section 7 direction B REFUSES unless "
                                   "G5m reads GATE FAIL on the planted copy.  The arithmetic "
                                   "above says whether the registered PLANT is large enough "
                                   "to cross band D on this row's smallest graded reference."})
    return rec
...
def grade(root):
    ...
    return {"item": "CURRICULUM-%s" % ITEM, "verdict": verdict, "rows": row_verdicts,
            ...
            "instrument_cross_check": instrument_constants(os.path.join(here, "so2m_xm.py"))
                                      if os.path.isfile(os.path.join(here, "so2m_xm.py")) else None}
...
# locations, and the key vocabulary CROSS-CHECKED against so2m_xm.py's OWN bytes.
...
#
# TWO SCALES, AND THE REASON IS A MEASURED FINDING AGAINST THE FROZEN DOCUMENT.
# PREREGISTRATION.md section 7 direction B registers an ABSOLUTE plant of
# PLANT = 1.234e-03 added to one d(CMZ)/d(shape) entry, and REFUSES unless G5m
# reads GATE FAIL on that copy.  Crossing band D needs
#     PLANT > 0.05 * |d_ref|,  i.e.  |d_ref| < PLANT / 0.05 = 0.02468.
# The only MEASURED neighbours on this exact case are SO-1a's real adjoint
# derivatives at the same normalisation: |d(CL)/d(shape)| runs 0.462 .. 3.157 over
# the four registered shape components (X-S/so1a_X.json), and |d(CD)/d(shape)|
# runs 0.0035 .. 0.0124.  `CMZ` carries CL's normalisation with an added moment
# arm, so if it lands in the CL class the registered plant is one to two orders of
# magnitude too small and the registered control CANNOT flip the gate.
#   * FIXTURE_CMZ_PHYSICAL is the CL-class scale.  Leg B1 drives it and shows the
#     comparator REFUSING with the arithmetic printed -- a property of the FROZEN
#     DOCUMENT, measured here rather than discovered after 11 core-min.
#   * FIXTURE_CMZ_FROM_CL is a scale small enough for the control to fire, so the
#     direction-B MACHINERY is itself driven end to end and the positive path is
#     exercised.  It is named as a chosen scale, never as a physical claim.
...
def _augment_real_X(j, adj_err_pct=None, flip=(), nz_zero=False, cmz_base=None,
                    cmz_scale=None):
    """Add the SO-2M keys to a REAL SO-1a X artefact, at the REGISTERED locations."""
...
def selftest(tmp):
    ...
    unit("L1 ast.Assert count = 0 in so2m_grade.py and so2m_xm.py",
         count_asserts(self_path) == 0
         and count_asserts(os.path.join(here, "so2m_xm.py")) == 0)
    p = os.path.join(tmp, "planted_assert.py")
    open(p, "w").write("x = 1\nassert x == 1\n")
    unit("L2 PLANTED: the assert counter sees a planted assert (=1)", count_asserts(p) == 1)
    ic = instrument_constants(os.path.join(here, "so2m_xm.py"))
    unit("S1 (schema) the INSTRUMENT's own declared key vocabulary, read out of "
         "so2m_xm.py's bytes by AST, EQUALS this comparator's registered vocabulary -- "
    ...
    unit("F6 (rule 3, DIRECTION B) G5m FLIPPED to GATE FAIL on a planted copy of the REAL "
         "X artefact, on both rows, in the SAME invocation as the verdict it licenses",
    ...
    unit("F7 (rule 3, DIRECTION B) G-NZ FLIPPED to GATE FAIL when the aoa entry was forced "
         ...
         all(r["controls"]["flip_G_NZ"][rl]["planted_verdict"] == "GATE FAIL"
             for rl in ROW_LABELS))
    unit("B1 (DIRECTION B, THE FINDING) at the CL-CLASS scale the registered ABSOLUTE "
         "PLANT = 1.234e-03 CANNOT cross band D on the smallest graded d(CMZ)/d(shape), so "
         "the comparator REFUSES exactly as PREREGISTRATION.md section 7 direction B "
         "instructs.  This is a measured property of the FROZEN DOCUMENT, driven here at "
         "ZERO core-min instead of after 11.",
         refused(_real_fixture(tmp, tw(cmz_scale={rl: FIXTURE_CMZ_PHYSICAL
                                                  for rl in ROW_LABELS}))))
    _bar = FD_BAND_PCT / 100.0 * abs(FIXTURE_CMZ_PHYSICAL
                                     * min(abs(float(v)) for v in
                                           json.load(open(REAL_PARENT_X))["adjoint"]["CL"]["shape"]))
    unit("B2 (DIRECTION B, THE ARITHMETIC) the plant needed to cross band D at that scale "
         "is %.6g and the registered plant is %.6g -- a factor of %.1f short.  The "
         "MACHINERY is sound (F6 flips it at a smaller scale); the registered MAGNITUDE is "
         "what cannot fire, and the choice of magnitude is frozen."
         % (_bar, PLANT, _bar / PLANT),
         _bar > PLANT)
    ...
    print("SO2M GRADER SELFTEST units=%d expected=%d failures=%d python_O=%s"
    ...
    if n != EXPECTED_UNITS or fails:
        ...
        return 2
    print("SO2M GRADER SELFTEST PASS %d/%d (counted against the frozen EXPECTED_UNITS)"
          % (n, EXPECTED_UNITS))
    return 0
...
def main():
    ...
    if count_asserts(os.path.abspath(__file__)) != 0:
        ...
        return 2
    if a.selftest:
        d = os.path.join(a.tmpdir, "so2m_selftest_%d" % os.getpid())
    ...
    if not a.root:
        print("usage: so2m_grade.py --root <run root> [--out FILE] | --selftest")
        return 64
    try:
        r = grade(a.root)
    except Refusal as e:
