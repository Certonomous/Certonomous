#!/usr/bin/env bash
# Curriculum D15 arm launcher -- NACA0012 SUBSONIC (DARhoSimpleFoam) baseline
# gradient, TWO ROWS (shipped + patched), adjoint X + central-FD table F.
#
# DERIVED from `cases/dafoam/ladder-a/A2/curriculum_D5/d5_run_arm.sh` (Addendum 1
# md5 50a976780e357998238ede3bbb8e5521) with the REGISTERED DELTAS listed in
# PREREGISTRATION.md section 7 and recorded in d15_run_arm_DELTAS_from_d5.diff:
...
# ===========================================================================
ITEM=D15
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D15-a1-naca0012-subsonic
...
#
# D15 REGISTERED DELTA (PREREGISTRATION.md section 5): cpuset 2,3 -- the
# dafoam-supervisor's FOURTEENTH-session placement; DISJOINT from D5 8,10,11,13,
# D4-SHIPPED 5,6,7,9, W2R 12, D16 4,14; not core 0.
CPUSET=2,3
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d15_" | tr '\n' ',' | sed 's/,$//'
}
...
#   X-S      2       20.0            600 s              4g
#   X-P      2       20.0            600 s              4g
#   F-S      2       60.0           1800 s              4g
#   F-P      2       60.0           1800 s              4g
cap_core_min() {
  case "$1" in
    MESH)     echo 5.0 ;;
    X-S|X-P)  echo 20.0 ;;
...
MD5_RUNSCRIPT=6537fa7641c4ccb20056f60f96f63b11   # d15_runScript.py = byte copy of the shipped subsonic tutorial runScript.py
MD5_XF=8a664a976de77c5fe7c3fed5746be4de                                 # d15_xf.py (set at freeze)
MD5_DECOMP=68ecc827562886fb43c3aedb0627b344      # d15_decomposeParDict (numberOfSubdomains 2, scotch)

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: d15_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: d15_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d15_${ARM}_" 2>/dev/null | grep "^d15_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/d15_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/d15_xf.py"                     | md5sum -c - || { echo "ABORT xf md5"; exit 4; }
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.d15_age_datum"
  ...
  rm -f "$WORK/d15_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.d15_age_datum" 2>/dev/null
  cp -a "$BASE/d15_runScript.py" "$BASE/d15_xf.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/d15_X.json" "$WORK/d15_F.json" "$WORK/d15_F.jsonl" "$WORK/d15_X.jsonl" "$WORK/dRdWColoring_2.bin"; do
...
case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo D15_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d15_xf.py -mode X" ;;
  F-S|F-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d15_xf.py -mode F" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/d15_cmd.sh" > /dev/null 2>&1 \
