#!/usr/bin/env bash
# D15 G-ROOT.5 SELFTEST -- the d5_groot5_selftest.sh pattern (md5 32e19fffc33a43efd9959a02dfab5062),
# driven against D15's OWN launcher (G-ROOT.5 from birth, standing rule of 2026-08-26).
...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; L="$HERE/d15_run_arm.sh"
...
echo "D15 G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1)"
...
CN="d15_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
sudo -n docker run -d --name "$CN" --cpus=0.1 --cpuset-cpus=3 --memory=64m --memory-swap=64m "$IMG" bash -c "sleep 180" >/dev/null 2>&1 || { bad "could not start sacrificial container"; rmdir "$BASE"; exit 2; }
...
kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; rm -f "$BASE/d15_driver.pid"
# (b2) STALE pidfile (dead pid) must NOT block
echo "999999" > "$BASE/d15_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
for forb in /home/ubuntu/certonomous-runs/CURRICULUM-D4-a2-wing-cdmin /home/ubuntu/certonomous-runs/CURRICULUM-D5-a2-wing-ffd-density /home/ubuntu/certonomous-runs/CURRICULUM-D13-a1-basin-restart /home/ubuntu/certonomous-runs/CURRICULUM-D16-a1-naca0012-transonic; do
...
cp -a "$HERE/d15_runScript.py" "$HERE/d15_xf.py" "$BASE/" && cp -a "$HERE/d15_decomposeParDict" "$BASE/base/system/decomposeParDict"
...
rm -rf "$BASE/base" "$BASE/d15_runScript.py" "$BASE/d15_xf.py"
chmod 775 "$BASE"
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^d15_" && bad "a d15_ container survives" || ok "no d15_ container survives"
echo "D15 G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
