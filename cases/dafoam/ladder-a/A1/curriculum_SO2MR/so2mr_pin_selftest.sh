#!/usr/bin/env bash
# SO-2MR PIN AND NO-LAUNCH SELFTEST.
#
...
#   /home/ubuntu/certonomous-runs/CURRICULUM-SO2MR-a1-naca0012-moment-gradient
...
DRIVER="$HERE/so2mr_chain_driver.sh"
LAUNCHER="$HERE/so2mr_run_arm.sh"
REAL_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO2MR-a1-naca0012-moment-gradient
TMP="${1:-/tmp}/so2mr_pin_selftest_$$"
PASS=0; FAIL=0
...
echo "SO2MR PIN + NO-LAUNCH SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "  driver_md5=$(md5sum "$DRIVER" | cut -d' ' -f1) launcher_md5=$(md5sum "$LAUNCHER" | cut -d' ' -f1)"
CASE_BEFORE=$(ls -1a "$HERE" | sort | md5sum | cut -d' ' -f1)
DOCKER_PRE=$(sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -c '^so2mr_')
...
# =============================================================================
if [ -e "$REAL_BASE" ]; then
  bad "(o0) the SO-2MR run root EXISTS: $REAL_BASE -- refusing to run over a real root"
  exit 2
fi
KP=/home/ubuntu/certonomous-runs/CURRICULUM-SO2a-a1-naca0012-geometric-constraint-gradient
if [ -e "$KP" ]; then
  ok "(o0) SO-2MR run root ABSENT and the SAME lister SEES SO-2a's existing root -- the absence is a reading of the disk, not of a broken test"
else
...
target_of() {   # THE ONE registered table.  A pin with no entry here FAILS.
  case "$1" in
    MD5_LAUNCHER)       echo "$HERE/so2mr_run_arm.sh" ;;
    MD5_GRADER)         echo "$HERE/so2mr_grade.py" ;;
    MD5_RUNSCRIPT)      echo "$HERE/so2mr_runScript.py" ;;
    MD5_XM)             echo "$HERE/so2mr_xm.py" ;;
    MD5_DECOMP)         echo "$HERE/so2mr_decomposeParDict" ;;
...
CGOT=$(md5sum "$HERE/so2mr_xm.py" | cut -d' ' -f1)
if [ "$CVAL" != "$CGOT" ]; then
...
SB="$TMP/nl4"; mine "$SB"
mkdir -p "$SB"
cp -a "$HERE"/so2mr_*.sh "$HERE"/so2mr_*.py "$HERE"/so2mr_decomposeParDict "$SB/" 2>/dev/null
printf '\n# planted drift\n' >> "$SB/so2mr_grade.py"
out=$(bash "$SB/so2mr_chain_driver.sh" MESH 2>&1); rc=$?
if [ "$rc" -eq 5 ] && [ -f "$SB/NOLAUNCH_FREEZE.txt" ] && echo "$out" | grep -q 'SO2MR_NOLAUNCH FREEZE rc=5'; then
...
mkdir -p "$SB2"
cp -a "$HERE"/so2mr_*.sh "$HERE"/so2mr_*.py "$HERE"/so2mr_decomposeParDict "$SB2/" 2>/dev/null
printf '\n# an edit the registration does not name\n' >> "$SB2/so2mr_runScript.py"
sed -i "s/^MD5_RUNSCRIPT=.*/MD5_RUNSCRIPT=$(md5sum "$SB2/so2mr_runScript.py" | cut -d' ' -f1)/" "$SB2/so2mr_chain_driver.sh"
out=$(bash "$SB2/so2mr_chain_driver.sh" MESH 2>&1); rc=$?
...
mkdir -p "$SB2b"
cp -a "$HERE"/so2mr_*.sh "$HERE"/so2mr_*.py "$HERE"/so2mr_decomposeParDict "$SB2b/" 2>/dev/null
sed -i "s|^BASE=.*|BASE=$TMP/nl2b_root|" "$SB2b/so2mr_chain_driver.sh"
...
out=$(bash "$SB2b/so2mr_chain_driver.sh" MESH 2>&1); rc=$?
if echo "$out" | grep -q 'SO2MR_PRODUCER_OK'; then
...
mkdir -p "$SB3"
cp -a "$HERE"/so2mr_*.sh "$HERE"/so2mr_*.py "$HERE"/so2mr_decomposeParDict "$SB3/" 2>/dev/null
FAKE_ROOT="$TMP/nl3_root"; mkdir -p "$FAKE_ROOT"; mine "$FAKE_ROOT"
sed -i "s|^BASE=.*|BASE=$FAKE_ROOT|" "$SB3/so2mr_chain_driver.sh"
echo "  [--] NL-3 SANDBOX DELTA (the only difference from the frozen driver):"
diff "$HERE/so2mr_chain_driver.sh" "$SB3/so2mr_chain_driver.sh" | sed 's/^/       /'
NDELTA=$(diff "$HERE/so2mr_chain_driver.sh" "$SB3/so2mr_chain_driver.sh" | grep -c '^[<>]')
if [ "$NDELTA" -eq 2 ]; then
...
out=$(bash "$SB3/so2mr_chain_driver.sh" MESH 2>&1); rc=$?
if [ "$rc" -eq 3 ] && [ -f "$SB3/NOLAUNCH_ROOT.txt" ] && echo "$out" | grep -q 'SO2MR_NOLAUNCH ROOT rc=3'; then
...
mkdir -p "$SB1"
cp -a "$HERE"/so2mr_*.sh "$HERE"/so2mr_*.py "$HERE"/so2mr_decomposeParDict "$SB1/" 2>/dev/null
NL1_ROOT="$TMP/nl1_root"; mine "$NL1_ROOT"
sed -i "s|^BASE=.*|BASE=$NL1_ROOT|; s/^H5_FLOOR_GIB=.*/H5_FLOOR_GIB=8.0; H5_SAMPLES=2; H5_WINDOW_S=2; AGG_CEILING_GIB=0.0001/; s/^AGG_POLL_S=.*/AGG_POLL_S=1; AGG_BOUND_S=0/" "$SB1/so2mr_chain_driver.sh"
T0=$(date -u +%s)
out=$(bash "$SB1/so2mr_chain_driver.sh" MESH 2>&1); rc=$?
...
if ! echo "$out" | grep -q 'SO2MR_DRIVER arm=MESH image='; then
...
#
# WHY THIS EXISTS.  so2mr_run_arm.sh derives the row TWICE from two independent
...
cp -a "$HERE/so2mr_runScript.py" "$HERE/so2mr_xm.py" "$GROW_ROOT/"
cp -a "$HERE/so2mr_decomposeParDict" "$GROW_ROOT/base/system/decomposeParDict"
...
if [ ! -e "$REAL_BASE" ]; then
  ok "(z3) the SO-2MR run root is STILL ABSENT: the freeze condition of PREREGISTRATION.md section 2 survives this selftest"
else bad "(z3) the SO-2MR run root now EXISTS"; fi
DOCKER_POST=$(sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -c '^so2mr_')
if [ "$DOCKER_POST" = "$DOCKER_PRE" ]; then
  ok "(z4) so2mr_ container count is $DOCKER_POST before and after -- this item has still started NO container"
else bad "(z4) so2mr_ container count moved $DOCKER_PRE -> $DOCKER_POST"; fi

echo "SO2MR PIN + NO-LAUNCH SELFTEST pass=$PASS fail=$FAIL pins_exist=$N_EXIST pins_driven=$N_DRIVEN $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
