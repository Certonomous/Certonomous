#!/usr/bin/env bash
# SO-2MR chain driver -- DERIVED from curriculum_SO1a/so1a_chain_driver.sh
...
# so2mr_chain_driver_DELTAS_from_so1a.diff).
#
# THE REGISTERED DELTAS, and the four that are NEW MACHINERY rather than renaming:
#   (1) item names, run root, arms MESH X-S G-S X-P G-P, instrument so2mr_xm.py,
#       producer so2mr_runScript.py with its NEW md5 (the third functional CMZ is
...
#   setsid nohup bash so2mr_chain_driver.sh MESH X-S G-S X-P G-P > <root>/chain_launch.out 2>&1 &
...
LAUNCHER="$HERE/so2mr_run_arm.sh"
GRADER="$HERE/so2mr_grade.py"
...
# what runs.  EVERY pin below is DRIVEN by so2mr_pin_selftest.sh, which enumerates
...
# PIN PROVENANCE.  The three values above the tutorial block were INHERITED FROM
# SO-2M THROUGH THE RENAME AND WERE STALE THE MOMENT THE RENAME RAN: the item-token
# rename REWRITES the bytes of the launcher and the instrument, so a pin carried
# across it names a file that no longer exists at that hash.  Left as inherited,
# NL-4 FREEZE (rc=5) would have refused this item's own first arm.  RE-PINNED
# 2026-08-31 AT SO-2MR's STAGE-2 FREEZE, PRE-COMPUTE -- this item has burned 0
# core-min, started no container and created no run root, so this is a pre-freeze
# edit to an unfrozen, untracked file and NOT an amendment to a frozen one:
#   MD5_LAUNCHER e12d510a35300e3d3412cb474c4a566c -> c27a40f507f60be4bfdc4bead696a612
#   MD5_GRADER   e08238ccbc38725c0186b73049cf518e -> 4f625c58c1c5f7b9a9c8f6cdda9c530c
#   MD5_XM       19c9eb89f6d12d93b22be96a65320650 -> f9da5713cd762f3da5ff350d8403f3ae
# No gate, threshold, band, cap or label moved with them; every pin is DRIVEN
# against its registered target by so2mr_pin_selftest.sh leg (p1).
MD5_LAUNCHER=c27a40f507f60be4bfdc4bead696a612
MD5_GRADER=4f625c58c1c5f7b9a9c8f6cdda9c530c
MD5_RUNSCRIPT=ae4a73429dd6972dc804b76d9a44aa05
MD5_XM=f9da5713cd762f3da5ff350d8403f3ae
...
test $# -ge 1 || { echo "ABORT usage: so2mr_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/so2mr_driver.pid"
cd "$HERE" || exit 4
...
nolaunch() {
  ...
  {
    echo "SO2MR NO-LAUNCH BRANCH $NLNAME"
  ...
  echo "SO2MR_NOLAUNCH $NLNAME rc=$NLRC verdict=BLOCKED file=$HERE/NOLAUNCH_${NLNAME}.txt"
}
...
# =============================================================================
test -f "$HERE/so2mr_runScript.py" || { nolaunch PRODUCER 4 "so2mr_runScript.py is absent from $HERE"; exit 4; }
PRODUCER_REPORT=$(python3 - "$TUT_SRC/runScript.py" "$HERE/so2mr_runScript.py" "$MD5_TUT_RUNSCRIPT" "$MD5_RUNSCRIPT" <<'PYEOF'
...
echo "SO2MR_PRODUCER_OK $(printf '%s' "$PRODUCER_REPORT" | tr -d '\n' | cut -c1-220)"
...
# =============================================================================
LIVE_PREFIX=$(sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep '^so2mr_' | head -5 | tr '\n' ',' | sed 's/,$//')
...
cp -a "$HERE/so2mr_decomposeParDict" "$BASE/base/system/decomposeParDict" || { echo "ABORT overlay decomposeParDict"; exit 4; }
cp -a "$HERE/so2mr_runScript.py" "$HERE/so2mr_xm.py" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
echo "ITEM=SO2MR" > "$BASE/ledger.txt"
...
echo "SO2MR_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"

{ echo "$MD5_RUNSCRIPT  $BASE/so2mr_runScript.py"; echo "$MD5_XM  $BASE/so2mr_xm.py"; echo "$MD5_DECOMP  $BASE/base/system/decomposeParDict";
...
echo "SO2MR_DRIVER start=$STAMP0 pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
for one run is the defect.  The existing rc=0 rows are KEPT, never re-run and never deleted."
  ...
  echo "SO2MR_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW file=$(basename "$H5_FILE")"
...
series: $(basename "$H5_FILE")"
  ...
  while true; do
    AGG=$(python3 "$HERE/so2mr_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
...
series: $(basename "$AGG_SERIES")"
  ...
  echo "SO2MR_AGGREGATE arm=$ARM waited=$WAITED $AGG"
  echo "SO2MR_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  # rc CAPTURED DIRECTLY from the launcher, never through a pipe.
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "SO2MR_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
RC_GE=$?
if [ "$RC_GE" -eq 0 ]; then
  python3 "$GRADER" --root "$BASE" --out "$BASE/SO2MR_grade_${GSTAMP}.json" > "$BASE/SO2MR_grade_${GSTAMP}.out" 2>&1
  GRC=$?
  echo "grader_rc=$GRC stamp=$GSTAMP out=SO2MR_grade_${GSTAMP}.json note=comparator-exit-status-NOT-the-verdict" >> "$STATUS"
else
...
echo "SO2MR_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC arms_bought=$BOUGHT of $DECLARED_ARMS"
exit "$CHAIN_RC"
