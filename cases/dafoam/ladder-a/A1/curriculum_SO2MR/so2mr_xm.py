#!/usr/bin/env python
"""Curriculum SO-2MR -- NACA0012 INCOMPRESSIBLE, the FD-VERIFIED PITCHING-MOMENT
...
EXACTLY these registered deltas and no other (so2mr_xm_DELTAS_from_so1a_xf.diff):

  * THE PRODUCER IS NO LONGER A BYTE COPY OF THE TUTORIAL, AND THAT IS SAID
    OPENLY RATHER THAN QUIETLY.  `so2mr_runScript.py` is the tutorial's
  ...
  * Printed tokens SO1A_* -> SO2MR_*; artefact names so1a_* -> so2mr_*.
...
OUT_X = "so2mr_X.json"
OUT_F = "so2mr_F.json"
JSONL_F = "so2mr_F.jsonl"
JSONL_X = "so2mr_X.jsonl"
...
def parse_mode(argv):
    ...
    for i, a in enumerate(argv):
        if a == "-mode" and i + 1 < len(argv):
            mode = argv[i + 1]
    if mode not in ("X", "G"):
        sys.stderr.write("SO2MR_XM usage: so2mr_xm.py -mode X|G\n")
        sys.exit(64)
    return mode
...
def main():
    ...
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("SO2MR_XM REFUSE producer md5 %s != frozen %s\n"
                         % (got, PRODUCER_MD5))
        sys.exit(2)
    ...
    if delta.get("reachable") and not (delta["tutorial_md5_ok"] and delta["insertions_exact"]):
        sys.stderr.write("SO2MR_XM REFUSE producer differs from the tutorial by "
    ...
    if src.count(ANCHOR) != 1:
        sys.stderr.write("SO2MR_XM REFUSE anchor %r appears %d times\n"
    ...
    ns = {"__name__": "so2mr_frozen_header", "__file__": PRODUCER}
    ...
    if GRADED_OUTPUT not in declared:
        sys.stderr.write("SO2MR_XM REFUSE %s is not declared in daOptions['function']: %r\n"
                         % (GRADED_OUTPUT, declared))
        sys.exit(2)
    sys.stdout.write("SO2MR_FUNCTIONS_DECLARED %s\n" % (",".join(declared),))
    ...
    if mode == "X":
        ...
        if rank == 0:
            ...
            with open(OUT_X, "w") as fh:
                ...
                fh.flush()
                os.fsync(fh.fileno())
            sys.stdout.write("SO2MR_X_WRITTEN %s\n" % OUT_X)
    ...
    if rank == 0:
        ...
        if problem is not None:
            sys.stderr.write("SO2MR_XM REFUSE planted control not seen on read-back: %s\n" % problem)
            sys.exit(2)
        sys.stdout.write("SO2MR_PLANTED_CONTROL_SEEN tuple=%s zero=%r plant=%r\n"
        ...
        with open(OUT_F, "w") as fh:
            ...
            fh.flush()
            os.fsync(fh.fileno())
        sys.stdout.write("SO2MR_F_WRITTEN %s n_rows=%d\n" % (OUT_F, len(rows)))
    MPI.COMM_WORLD.Barrier()
