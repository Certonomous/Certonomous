...
# PREREGISTRATION.md section 7 and recorded in so2mr_run_arm_DELTAS_from_d15.diff:
...
# `.so2mr_age_datum_ref` beside the epoch, so the grader is not left inferring it.
#
...
# ===========================================================================
ITEM=SO2MR
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO2MR-a1-naca0012-moment-gradient
...
# does not apply (so2mr_grade.py g_placement: dl_ok is True when ARM_RANKS == 1),
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^so2mr_" | tr '\n' ',' | sed 's/,$//'
}
...
MD5_RUNSCRIPT=ae4a73429dd6972dc804b76d9a44aa05   # so2mr_runScript.py = byte copy of the shipped INCOMPRESSIBLE tutorial runScript.py
MD5_XM=f9da5713cd762f3da5ff350d8403f3ae   # so2mr_xm.py, RE-PINNED at SO-2MR's Stage-2 freeze 2026-08-31 (the inherited 19c9eb89f6d12d93b22be96a65320650 named SO-2M's bytes, which the item-token rename had already rewritten)
MD5_DECOMP=e6f1b0060944bc86d6dff56480ad2bd4      # so2mr_decomposeParDict (numberOfSubdomains 1, scotch)

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: so2mr_run_arm.sh <MESH|X-S|G-S|X-P|G-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: so2mr_run_arm.sh <MESH|X-S|G-S|X-P|G-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^so2mr_${ARM}_" 2>/dev/null | grep "^so2mr_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/so2mr_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XM  $BASE/so2mr_xm.py"                     | md5sum -c - || { echo "ABORT xf md5"; exit 4; }
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.so2mr_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so2mr_age_datum_ref"
  ...
  rm -f "$WORK/so2mr_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.so2mr_age_datum" "$WORK/.so2mr_age_datum_ref" 2>/dev/null
  cp -a "$BASE/so2mr_runScript.py" "$BASE/so2mr_xm.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/so2mr_X.json" "$WORK/so2mr_F.json" "$WORK/so2mr_F.jsonl" "$WORK/so2mr_X.jsonl" "$WORK/dRdWColoring_2.bin"; do
  ...
  echo "$AGE_DATUM" > "$WORK/.so2mr_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so2mr_age_datum_ref"
  echo "SO2MR_WRITE_COMPRESSION $(grep -a writeCompression "$WORK/system/controlDict" 2>/dev/null | head -1 | tr -d ';')"
  echo "D4_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM datum_file=$DATUM_NAME resolved_by=EXISTENCE"
fi

case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo SO2MR_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so2mr_xm.py -mode X" ;;
  G-S|G-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so2mr_xm.py -mode G" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/so2mr_cmd.sh" > /dev/null 2>&1 \
