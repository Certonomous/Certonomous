#!/usr/bin/env python3
"""Curriculum D16 COMPARATOR -- NACA0012 TRANSONIC (DARhoSimpleCFoam) baseline gradient,
...
NOT A RESULT / BLOCKED / PENDING).  Derived from curriculum_D15/d15_grade.py (d16_grade_DELTAS_from_d15.diff: item, caps, ceiling,
predictions, cpuset).  Shape: curriculum_D14/d14m_grade.py (built-in
...
WHAT IT GRADES (PREREGISTRATION.md section 3):
  G1   completion, ARM-KIND AWARE: every arm needs kernel rc == 0 (docker inspect, and
       ...
       need the instrument's TERMINAL MARKER in the arm log (D16_X_WRITTEN / D16_F_WRITTEN);
  ...
  G-M2 mesh identity: cells == 4,032 (A1's mesh; same generator, same profiles) ->
       PASS else GATE FAIL (a different mesh is a different item).
  G5   per ROW, the bright line: per registered component, the FD reference is the
       ...
       on CD (the objective) and G5c on CL (the constraint) with the same bands; a row is
       PASS only if both are.
  ...
  G12  placement: cpuset == the registered 2,3 on every row; delivered cores >= 1.5
...
ITEM = "D16"
BASE = "/home/ubuntu/certonomous-runs/CURRICULUM-D16-a1-naca0012-transonic"
...
ARTEFACT = {"MESH": "checkMesh.log", "X-S": "d16_X.json", "F-S": "d16_F.json", "X-P": "d16_X.json", "F-P": "d16_F.json"}
TERMINAL = {"X-S": "D16_X_WRITTEN", "F-S": "D16_F_WRITTEN", "X-P": "D16_X_WRITTEN", "F-P": "D16_F_WRITTEN"}
DATUM_REF = {"MESH": "0.orig/U", "X-S": "0/U", "F-S": "0/U", "X-P": "0/U", "F-P": "0/U"}
CAPS = {"MESH": 5.0, "X-S": 30.0, "F-S": 90.0, "X-P": 30.0, "F-P": 90.0}
ITEM_CEILING_CORE_MIN = 245.0
PREDICTED_CORE_MIN = {"MESH": 0.3, "X-S": 6.0, "F-S": 14.0, "X-P": 6.0, "F-P": 14.0}
CELLS_EXPECTED = 4032
...
CPUSET_REGISTERED = "4,14"
DELIVERED_CORES_FLOOR = 1.5
PRED = {"P2_CL_band": (0.30, 0.60), "P3_CD_band": (0.012, 0.045), "P4_patched_min_pass": 4,
        "P6_core_min_band": (15.0, 120.0), "P6_mesh_wall_s_max": 120.0}
...
def arm_datum(base, arm):
    ...
    p = os.path.join(d, ".d16_age_datum")
...
def grade(root):
    ...
    # ---- rows
    X = {"S": read_X(os.path.join(root, "X-S", "d16_X.json")), "P": read_X(os.path.join(root, "X-P", "d16_X.json"))}
    F = {"S": read_F(os.path.join(root, "F-S", "d16_F.json")), "P": read_F(os.path.join(root, "F-P", "d16_F.json"))}
    ...
    controls = {"S": ctrl_control(F["S"]), "P": ctrl_control(F["P"]),
                "grader_plant_S": grader_plant_control(root, os.path.join(root, "F-S", "d16_F.json"), "S"),
                "grader_plant_P": grader_plant_control(root, os.path.join(root, "F-P", "d16_F.json"), "P")}
    g5 = {}
    for rk in ("S", "P"):
        cd = grade_components(X[rk], F[rk], "CD")
        cl = grade_components(X[rk], F[rk], "CL")
        g5[rk] = {"G5_CD": cd, "G5c_CL": cl, "row_verdict": compose_row(cd, cl),
    ...
    return {"item": "CURRICULUM-%s" % ITEM, "verdict": verdict,
            ...
            "capability_grid_cell": "2D . steady . transonic -- gradients computed + FD-verified; this item moves ONLY that column"}
...
def _fix(tmp, tweak=None):
    ...
    k = {"rc": {a: 0 for a in ARMS_REQUIRED}, "ke": {}, "oom": {a: "false" for a in ARMS_REQUIRED},
         ...
         "drop_row": set(), "inspect_file": set(), "dl": "1.99 n=10 max_nr_throttled=0", "CL": 0.45, "CD": 0.02}
    if tweak:
        tweak(k)
    ...
    for arm in ARMS_REQUIRED:
        ...
        open(os.path.join(d, ".d16_age_datum"), "w").write("%d\n" % t0)
        log = "%s_x.log" % arm
        so = k["so"][arm]
        ...
        if arm == "MESH":
            ...
            if arm.startswith("X"):
                ...
                json.dump({"item": "D16", "mode": "X", "identity": ident, "nprocs": 2, "CD_baseline": repr(k["CD"]),
                ...
                json.dump({"item": "D16", "mode": "F", "identity": ident, "components_requested": COMPONENTS_REGISTERED,
...
def selftest(tmp):
    ...
    unit("U2 clean fixture -> P1-P4, P6 HIT; P5 MISS (shape[6] clean by construction)",
         all(r["predictions"][k] == "HIT" for k in ("P1_cells_4032", "P2_CL_baseline_in_band", "P3_CD_baseline_in_band", "P4_patched_row_CD_PASS_ge4", "P6_total_core_min_band", "P6b_mesh_wall_le_120s")) and r["predictions"]["P5_shipped_shape6_outside_band_D_or_flipped"] == "MISS")
    ...
    r = grade(_fix(tmp, tw(cm={"F-S": 91.0})))
    unit("U13 F-S core_min 91.0 > cap 90.0 -> G10 GATE FAIL", r["gates"]["G10_caps"]["verdict"] == "GATE FAIL" and r["gates"]["G10_caps"]["per_arm"]["F-S"]["crossed"])
    r = grade(_fix(tmp, tw(cs={"X-S": "2,3"})))
    unit("U14 X-S on cpuset 2,3 -> G12 GATE FAIL", r["gates"]["G12_placement"]["verdict"] == "GATE FAIL")
    ...
    unit("U21 ast.Assert count = 0 in d16_grade.py and d16_xf.py", count_asserts(os.path.abspath(__file__)) == 0 and count_asserts(os.path.join(here, "d16_xf.py")) == 0)
    p = os.path.join(tmp, "planted_assert.py")
    open(p, "w").write("x = 1\nassert x == 1\n")
    unit("U22 the assert counter sees a planted assert (=1)", count_asserts(p) == 1)
    print("D16 GRADER SELFTEST units=%d expected=%d failures=%d python_O=%s" % (n, EXPECTED_UNITS, len(fails), not __debug__))
    if n != EXPECTED_UNITS or fails:
        print("SELFTEST FAIL: %s" % (fails or "unit count %d != %d" % (n, EXPECTED_UNITS)))
        return 2
    print("D16 GRADER SELFTEST PASS %d/%d (counted against the frozen EXPECTED_UNITS)" % (n, EXPECTED_UNITS))
    return 0
...
def main():
    ...
    if a.selftest:
        d = os.path.join(a.tmpdir, "d16_selftest_%d" % os.getpid()); os.makedirs(d, exist_ok=True)
        return selftest(d)
    if not a.root:
        print("usage: d16_grade.py --root <run root> [--out FILE] | --selftest"); return 64
    try:
        r = grade(a.root)
    except Refusal as e:
