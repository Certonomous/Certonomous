#!/usr/bin/env bash
# D16 chain driver -- DERIVED from curriculum_D15/d15_chain_driver.sh (itself from
# curriculum_D5/d5_chain_driver.sh, Addendum 2) with the REGISTERED DELTAS in
# PREREGISTRATION.md section 7 and d16_chain_driver_DELTAS_from_d15.diff (item
# names, the transonic tutorial source and its runScript md5); the D5 lineage:
...
#       the ledger opens with an exact `ITEM=D16` line and a STAGED line
...
#   setsid nohup bash d16_chain_driver.sh MESH X-S F-S X-P F-P > <root>/chain_launch.out 2>&1 &
...
LAUNCHER="$HERE/d16_run_arm.sh"
GRADER="$HERE/d16_grade.py"
...
TUT_SRC=/home/ubuntu/dafoam-tutorials/NACA0012_Airfoil/transonic
PERMISSION=bc0e687e
...
MD5_LAUNCHER=bf29bd90700eebfb37bf01ff5300fa74
MD5_GRADER=0b8338b3e483548da3515e79da28762a
MD5_RUNSCRIPT=420cf2e79aaabe7d67b4701bbb99a314
MD5_XF=c78eaaaabe1b4afb0bd74270843ddf77
MD5_DECOMP=68ecc827562886fb43c3aedb0627b344
# the shipped tutorial's INPUT bytes, frozen here because the checkout is not
MD5_TUT_RUNSCRIPT=420cf2e79aaabe7d67b4701bbb99a314
...
test $# -ge 1 || { echo "ABORT usage: d16_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/d16_driver.pid"
cd "$HERE" || exit 4
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$HERE/d16_decomposeParDict" "$BASE/base/system/decomposeParDict" || { echo "ABORT overlay decomposeParDict"; exit 4; }
  cp -a "$HERE/d16_runScript.py" "$HERE/d16_xf.py" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  ...
  echo "D16_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "D16_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT  $BASE/d16_runScript.py"; echo "$MD5_XF  $BASE/d16_xf.py"; echo "$MD5_DECOMP  $BASE/base/system/decomposeParDict";
...
echo "D16_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
for ARM in $ARMS; do
  ...
  echo "D16_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW file=$(basename "$H5_FILE")"
  ...
  while true; do
    AGG=$(python3 "$HERE/d16_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
  ...
  echo "D16_AGGREGATE arm=$ARM waited=$WAITED $AGG"
  echo "D16_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "D16_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
if echo "$MD5_GRADER  $GRADER" | md5sum -c - > /dev/null; then
  python3 "$GRADER" --root "$BASE" --out "$BASE/D16_grade_${GSTAMP}.json" > "$BASE/D16_grade_${GSTAMP}.out" 2>&1; GRC=$?
  echo "grader_rc=$GRC stamp=$GSTAMP out=D16_grade_${GSTAMP}.json note=comparator-exit-status-NOT-the-verdict" >> "$STATUS"
else
  echo "grader_rc=NOT_RUN stamp=$GSTAMP note=grader-md5-drifted-at-chain-end" >> "$STATUS"
fi
echo "D16_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC"
exit "$CHAIN_RC"
