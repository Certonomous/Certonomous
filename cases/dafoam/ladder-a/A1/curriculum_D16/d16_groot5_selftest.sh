#!/usr/bin/env bash
# D16 G-ROOT.5 SELFTEST -- d15_groot5_selftest.sh re-pointed (d16_groot5_selftest_DELTAS_from_d15.diff); the d5_groot5_selftest.sh pattern (md5 32e19fffc33a43efd9959a02dfab5062),
# driven against D16's OWN launcher (G-ROOT.5 from birth, standing rule of 2026-08-26).
...
# (d) G-ROOT.1 refusals on D4's, D5's, D13's and D15's roots;
...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; L="$HERE/d16_run_arm.sh"
...
echo "D16 G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1)"
...
CN="d16_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
sudo -n docker run -d --name "$CN" --cpus=0.1 --cpuset-cpus=14 --memory=64m --memory-swap=64m "$IMG" bash -c "sleep 180" >/dev/null 2>&1 || { bad "could not start sacrificial container"; rmdir "$BASE"; exit 2; }
...
kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; rm -f "$BASE/d16_driver.pid"
# (b2) STALE pidfile (dead pid) must NOT block
echo "999999" > "$BASE/d16_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
# (d) G-ROOT.1/.2 from birth: D4's, D5's, D13's and D15's roots REFUSE
for forb in /home/ubuntu/certonomous-runs/CURRICULUM-D4-a2-wing-cdmin /home/ubuntu/certonomous-runs/CURRICULUM-D5-a2-wing-ffd-density /home/ubuntu/certonomous-runs/CURRICULUM-D13-a1-basin-restart /home/ubuntu/certonomous-runs/CURRICULUM-D15-a1-naca0012-subsonic; do
...
cp -a "$HERE/d16_runScript.py" "$HERE/d16_xf.py" "$BASE/" && cp -a "$HERE/d16_decomposeParDict" "$BASE/base/system/decomposeParDict"
...
rm -rf "$BASE/base" "$BASE/d16_runScript.py" "$BASE/d16_xf.py"
chmod 775 "$BASE"
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^d16_" && bad "a d16_ container survives" || ok "no d16_ container survives"
echo "D16 G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
