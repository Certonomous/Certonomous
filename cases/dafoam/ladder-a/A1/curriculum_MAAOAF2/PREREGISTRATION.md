...
## AMENDMENT 1 — 2026-09-11, **PRE-COMPUTE** (condition: run root `/home/ubuntu/certonomous-runs/MAAOAF2-ma288-init` DID NOT EXIST, checked by `ls` immediately before and after; no arm had run; `G-FREEZE.0` had refused all four). **Two supervisor errors, both mine. NO GATE, THRESHOLD, CAP, LABEL OR PREDICTION MOVES.**

**ERROR 1 — I SUBSTITUTED THE WRONG SLOT NAMES AND FROZE ANYWAY.** The runner's slots are `IMAGETAG`, `IMAGEID`, `SRCMD5`, `RUNROOT`, `CPUSETM1..M4`. I substituted against names I had assumed (`IMAGE`, `RUNSCRIPTMD5`, `CPUM1..M4`), so **only 3 of 8 landed and I committed and launched a runner carrying 5 live placeholders.**

**`G-FREEZE.0` REFUSED ALL FOUR ARMS — `ABORT G-FREEZE.0 this file still carries 5 unfrozen placeholders`, exit 3 on each. No container started, no run root was created, no compute was spent, and no row exists that a pre-registration does not cover.** That guard is the one whose off-by-one twin I repaired on `A5P2` hours earlier and whose `LA.0c` sibling I repaired on `MAAOAF`; tonight it caught its own supervisor. **The lesson is not that the guard worked — it is that I did not verify my own substitution before committing, and the only thing between that and a bogus row was a check I had spent the night doubting.** The corrected substitution leaves **0** suffixed tokens; runner md5 is now `575d6fc66e3fc7bdb6f4849f75192918`.

**ERROR 2 — THE FREEZE STAMP ABOVE IS CORRUPTED AND IS STRUCK.** I wrote it with an **unquoted heredoc**, so every backticked span was **command-substituted by the shell** and its content deleted from the file — `trim.log:508` became empty, `div(phi,rho)` produced a syntax error, and so on. The mangled §"FREEZE STAMP" block above is **STRUCK IN ITS ENTIRETY** and replaced by this amendment; it is not edited, per rule 6. This is the lab's recorded *"backticks kill the commit"* trap, arriving through a heredoc instead of a `-m`.

**THE FREEZE VALUES, RESTATED INTACT.** Runner md5 `575d6fc66e3fc7bdb6f4849f75192918` (post-substitution, `bash -n` clean, 0 slots); grader md5 `f8ce662eeebbe90a11c8d48a2dd83790`; image `dafoam-idwarp-rot:v1` @ `sha256:2927768a16acdea0330180fff95c8879c1dda9efcf6028728523b7dee30f6d35`, read from the daemon; staged instrument md5 `bd22df020be42fbd2eef14cbc25182f7`; cpusets M1 8, M2 9, M3 10, M4 11.

**THE TWO MEASUREMENTS, RESTATED INTACT — both re-derived by me, both changed the ladder.**
**(i) MA288's max non-orthogonality is 31.503 deg** (average 1.710), from the run's own `checkMesh` printed inside `trim.log` at line 508, beside maxAR 212103.67 and max skewness 1.3968. **THE TRAP IS REAL: the file `MA288/case/checkMesh.log` reads 22.749 deg — but it describes a 4,032-CELL GENERATION-STAGE MESH, not the 130,304-cell run mesh.** Two non-orthogonality figures for "MA288" sit on disk and only one is the case. **So A5's lesson transfers a fortiori: A5P2 broke a fixed point with two correctors at 3.512 deg, and MA288 is 8.97x LESS orthogonal.** The aspect ratio of 212,103 is explicitly not the argument.
**(ii) M3 AS ORIGINALLY BRIEFED WAS A NO-OP ON BOTH LIMBS.** Verified by me: every div(phi,X) term except div(phi,U) is **already** bounded Gauss upwind, and **div(phi,rho) does not exist — zero occurrences** — because DARhoSimpleFoam carries density as rho = psi*p rather than a transport equation. div(phi,U) had already been MAAOAF's N3. **The lane refused the arm its supervisor asked for and said why; that is the behaviour I want.** M3 is redirected to the one unbounded discrete operator remaining, gradSchemes -> cellLimited Gauss linear 1.

**THE L7 CONDITION, RESTATED INTACT:** unless some arm achieves onset **> 1** and **>= 250** and a last-block max initRes **< 0.6170**, this item declares a **FORMULATION** question and proposes no fifth settings rung. R0's last-block max initRes is 0.7286090 and N3's 0.6178434, so **N2's 0.6170459 is the lowest of the three arms that reached 500** — the bar is the incumbent's own number, not a round one.

**AMENDMENT 1a — 2026-09-11, still PRE-COMPUTE (run root absent; `G-IMG` had refused all four arms, no container, no compute).** A third error of mine, from the same bad substitution: `IMAGE` is a **PREFIX of `IMAGETAG`**, so my first sed rewrote `__MAAOAF2_UNFROZEN__IMAGETAG` into the literal `dafoam-idwarp-rot:v1TAG` at line 85, and the corrected second pass found no token left to fix. **`G-IMG` refused all four arms — `could not resolve image 'dafoam-idwarp-rot:v1TAG'` — so a SECOND independent guard caught me before anything ran.** Damage scanned against the committed blob and confined to that one line; repaired. **Substituting a token that is a prefix of another token is the trap, and the guards, not my reading, are what caught it — twice in ten minutes.** Runner md5 now `6c1da569f213ff9011892a00cf3a8fc3`. No gate, threshold, cap, label or prediction moves.
