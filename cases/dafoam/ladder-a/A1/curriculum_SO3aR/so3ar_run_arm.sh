#!/usr/bin/env bash
# Curriculum SO-3aR arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam),
...
# PREREGISTRATION.md section 7 and recorded in so3ar_run_arm_DELTAS_from_d15.diff:
...
# `.so3ar_age_datum_ref` beside the epoch, so the grader is not left inferring it.
#
...
# `so3ar_grade.py:_ledger_rows_via_launcher` runs
#     bash -c '. so3ar_run_arm.sh --source-only; so3ar_ledger_row ...'
...
# ===========================================================================
# THE REGISTERED TABLES LIVE ABOVE `--source-only`, AND THAT IS A SO-3aR DELTA.
#
...
# Moving them above the early return exposes them to so3ar_groot5_selftest.sh's
...
# ===========================================================================
ITEM=SO3aR
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO3aR-a1-naca0012-alpha-multipoint-gradient
...
#
# ---- SO-3aR REGISTERED DELTA, AND IT IS A REPAIR OF AN INHERITED FICTION.
...
# ----   CURRICULUM-SO3aRF-a1-naca0012-alpha-feasibility
...
# ---- so3ar_groot5_selftest.sh drives every entry that is ON DISK and prints
...
/home/ubuntu/certonomous-runs/CURRICULUM-SO3aRF-a1-naca0012-alpha-feasibility
...
#
# SO-3aR STAGE-2 PLACEMENT (PREREGISTRATION.md section 5: "cpuset fixed at Stage 2
...
# 14 is disjoint from both, so SO-3aR does NOT inherit the parent's DISCLOSED
# OVERLAP with D4-SHIPPED's registered 5,6,7,9.  At np = 1 the grader's
# delivered-cores floor does not apply (so3ar_grade.py g_placement: dl_ok is True
...
# reporting the exposure.  G12 compares this value against so3ar_grade.py's
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^so3ar_" | tr '\n' ',' | sed 's/,$//'
}

# ---- REGISTERED CAP TABLE (PREREGISTRATION.md §4, verbatim) ---------------
# SO-3aR's registered caps (PREREGISTRATION.md section 4).  THIS TABLE IS A
...
# carried SO-3aR's, so the two disagreed for a while and only the code was right.
# so3ar_groot5_selftest.sh now PARSES BOTH and refuses if they diverge, because a
...
# before -- even though so3ar_runScript.py and so3ar_xf.py now exist and their md5s
...
# it.  so3ar_chain_driver.sh:60-71 makes the same choice for the same reason.
...
MD5_RUNSCRIPT=53ba67c95461f86a585cb7ec7cdc2b39   # so3ar_runScript.py
MD5_XF=aa213384d92f6e07de90c40167854ce9          # so3ar_xf.py
# so3ar_decomposeParDict is ADOPTED BYTE-IDENTICALLY from the parent (one line
# changed from the tutorial's: numberOfSubdomains 1), it EXISTS, and this is the
# value so3ar_chain_driver.sh:70 already pins it at.  It is not a partial table: it
# is the one file whose bytes are unchanged from an already-frozen ancestor.
MD5_DECOMP=e6f1b0060944bc86d6dff56480ad2bd4

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: so3ar_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: so3ar_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^so3ar_${ARM}_" 2>/dev/null | grep "^so3ar_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/so3ar_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/so3ar_xf.py"                     | md5sum -c - || { echo "ABORT so3ar_xf.py md5"; exit 4; }
...
# empty case and REFUSES, and it is the same mapping so3ar_grade.py registers as
# ARM_ROW (MESH/X-S/F-S -> SHIPPED, X-P/F-P -> PATCHED).
WANT_ROW=$(row_of "$ARM")
test -n "$WANT_ROW" || {
  echo "ABORT arm $ARM carries no registered row.  SO-3aR declares MESH X-S F-S X-P F-P"
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.so3ar_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so3ar_age_datum_ref"
  ...
  rm -f "$WORK/so3ar_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.so3ar_age_datum" "$WORK/.so3ar_age_datum_ref" 2>/dev/null
  cp -a "$BASE/so3ar_runScript.py" "$BASE/so3ar_xf.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/so3ar_X.json" "$WORK/so3ar_F.json" "$WORK/so3ar_F.jsonl" "$WORK/so3ar_X.jsonl" "$WORK/dRdWColoring_2.bin"; do
  ...
  echo "$AGE_DATUM" > "$WORK/.so3ar_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so3ar_age_datum_ref"
  echo "SO3AR_WRITE_COMPRESSION $(grep -a writeCompression "$WORK/system/controlDict" 2>/dev/null | head -1 | tr -d ';')"
  echo "D4_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM datum_file=$DATUM_NAME resolved_by=EXISTENCE"
fi

case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo SO3AR_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so3ar_xf.py -mode X" ;;
  F-S|F-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so3ar_xf.py -mode F" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/so3ar_cmd.sh" > /dev/null 2>&1 \
...
{
  so3ar_ledger_row "$ARM" "$ROW" "$IMG" "$GOT_DIGEST" "$rc" "$WALL" "$RANKS" "$CORE_MIN" "$CAP" "$TMO" "$BACKCHECK" "$MEM" "$INSPECT" "$MEMAVAIL_GIB" "$MEMAVAIL_POST" "$CPUSET" "$DELIVERED" "$SIBLINGS_PRE" "$SIBLINGS_POST" "$(basename "$LOG")" "$STAMP"
