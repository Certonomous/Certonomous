#!/usr/bin/env bash
# SO-3aR G-ROOT.5 SELFTEST -- ADOPTED from curriculum_SO1b/so1b_groot5_selftest.sh
# (md5 read at run time and printed below), driven against SO-3aR's OWN FROZEN
# launcher so3ar_run_arm.sh.  Discharges the requirement PREREGISTRATION.md:227
# (section 7.3) registered against this item:
#
#   "before this item is enqueued, the G-ROOT.5 container legs must be driven --
#    including a sacrificial container named so3ar_<ARM>_... and a sacrificial
#    live pid with cwd inside the run root -- and the three new forbidden roots
#    must each be shown to abort at G-ROOT.1."
#
# WHAT SO-3aR ACTUALLY REGISTERS, stated plainly because a sibling family
# registers something else.  SO-3aR's G-ROOT.5 is "A LIVE ARM IS NEVER
# RE-STAGED" (so3ar_run_arm.sh:234-267) and its refusal code is EXIT 3, on two
# live readings taken BEFORE any destructive act:
#   (a) a RUNNING container carrying so3ar_<ARM>_ ;
#   (b) a driver pidfile in the run root naming a LIVE pid that is not an
#       ancestor of the launcher, or whose cwd IS the run root.
# A STALE pidfile does NOT block.  SO-3aR's launcher does NOT refuse merely
# because the run root exists -- that clause belongs to the D12R2/W3 chain
# driver's phase 1 (exit 6) and SO-3aR's driver instead re-stages on the FIRST
# fire only (so3ar_chain_driver.sh:72-91).  Both directions are driven below and
...
# `sudo -n rm -rf "$WORK"` (so3ar_run_arm.sh:338), and every leg here refuses
...
L="$HERE/so3ar_run_arm.sh"
GRADER="$HERE/so3ar_grade.py"
PRE="$HERE/PREREGISTRATION.md"
BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO3aR-a1-naca0012-alpha-multipoint-gradient
IMG_S=dafoam/opt-packages:latest
IMG_P=dafoam-idwarp-rot:v1
BOGUS=no-such-image:so3ar-selftest
ARM=X-S
CPUSET=14
PASS=0; FAIL=0
...
echo "SO3aR G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1) grader_md5=$(md5sum "$GRADER" | cut -d' ' -f1)"
...
echo "$SPID" > "$BASE/so3ar_driver.pid"; mine "$BASE/so3ar_driver.pid"
...
rm -f "$BASE/so3ar_driver.pid"

# (b2) a STALE pidfile must NOT block -- the guard must discriminate, not refuse.
echo "999999" > "$BASE/so3ar_driver.pid"; mine "$BASE/so3ar_driver.pid"
...
echo "$out" | grep -q "D4S_G_ROOT_PASS item=SO3aR" && ok "(c2) G-ROOT.1-.3 also PASS on this item's own root, ledger_clean=yes" || bad "(c2) G-ROOT.1-.3 did not pass on the item's own root"
...
echo "ITEM=SO3aR" > "$BASE/ledger.txt"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
if [ "$rc" -eq 4 ] && echo "$out" | grep -q "ledger_clean=yes"; then
  ok "(d4) DRIVEN CONTROL: the SAME ledger carrying ITEM=SO3aR is ACCEPTED -- (d3)'s refusal is a reading of the line, not of the file's existence"
...
cp -a "$HERE/so3ar_runScript.py" "$BASE/so3ar_runScript.py"; mine "$BASE/so3ar_runScript.py"
cp -a "$HERE/so3ar_xf.py" "$BASE/so3ar_xf.py"; mine "$BASE/so3ar_xf.py"
cp -a "$HERE/so3ar_decomposeParDict" "$BASE/base/system/decomposeParDict"; mine "$BASE/base/system/decomposeParDict"

# ---- SO-3aR REGISTERED STATE OF THESE LEGS, AND WHY THEY ASSERT A REFUSAL THE
# ---- PARENT'S ASSERTED A PASS FOR.
# The parent's (e1)-(e3) expected the staged-instrument md5s to VERIFY and the
# run to proceed to G-ROW.  SO-3aR's instrument pins are deliberately the
# FAIL-CLOSED SENTINEL until the Stage-2 amendment (so3ar_run_arm.sh MD5_UNSET),
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^so3ar_" && bad "(z4) a so3ar_ container survives" || ok "(z4) no so3ar_ container survives, running or stopped"
...
#
# On 2026-08-31 so3ar_run_arm.sh was mutated four ways and the grader's 91-unit
...
# already carried SO-3aR's, and only the code was right.  A comment table that
...
NINE="so3ar_chain_driver.sh so3ar_run_arm.sh so3ar_xf.py so3ar_grade.py so3ar_runScript.py
so3ar_aggregate_memory.py so3ar_groot5_selftest.sh so3ar_decomposeParDict so3ar_stop_marker.sh"
MISSING=""
...
# so3ar_xf.py REFUSES (exit 2) otherwise, and this lane shipped a docstring that
...
ANCH=$(python3 -c "
import sys; sys.path.insert(0,'$HERE')
import so3ar_xf as XF; print(XF.ANCHOR)")
NANCH=$(grep -Fc -- "$ANCH" "$HERE/so3ar_runScript.py")
if [ "$NANCH" -eq 1 ]; then
  ok "(x7) the producer carries the instrument's anchor EXACTLY ONCE (count=$NANCH)"
else bad "(x7) the producer carries the anchor $NANCH times -- so3ar_xf.py:478 would REFUSE every arm"; fi
...
src = open(HERE + "/so3ar_runScript.py").read()
ns = {"__name__": "hdr", "__file__": "so3ar_runScript.py"}
saved = list(sys.argv); sys.argv = ["so3ar_runScript.py", "-task", "run_model"]
exec(compile(src.split(XF.ANCHOR)[0], "so3ar_runScript.py", "exec"), ns)
sys.argv = saved
bad = []
...
cp -a "$HERE/so3ar_runScript.py" "$KP/producer_planted.py"
...
import so3ar_grade as G
present, absent = G.so3ar_instrument_files()
...
# consumer -- so3ar_chain_driver.sh's `img_of` -- derived the row from an ARM-NAME
...
DRV="$HERE/so3ar_chain_driver.sh"
...
DRVF="$HERE/so3ar_chain_driver.sh"
...
MD5_LAUNCHER so3ar_run_arm.sh
MD5_GRADER so3ar_grade.py
MD5_RUNSCRIPT so3ar_runScript.py
MD5_XF so3ar_xf.py
MD5_AGG so3ar_aggregate_memory.py
MD5_DECOMP so3ar_decomposeParDict
MD5_STOP_MARKER so3ar_stop_marker.sh
PINS
...
KACT=$(md5sum "$HERE/so3ar_grade.py" | cut -d' ' -f1)
[ -n "$KPIN" ] && [ "$KPIN" != "$KACT" ] \
...
#
# THIS IS SO-3aR's REASON TO EXIST.  SO-3a's (x12)-(x14) above are RIGHT about
# what they assert and BLIND to what killed it: they select pins by the SHAPE OF
# THE VALUE, inside ONE FILE.  `so3ar_xf.py:PRODUCER_MD5` held a value chosen
# DELIBERATELY not to be md5-shaped, in a different file, and (x14) reported
# 13 of 13 while the item could not launch.
#
# `so3ar_pin_census.py` sweeps BY ROLE across every instrument.  Below it is
...
CENSUS="$HERE/so3ar_pin_census.py"
if [ ! -f "$CENSUS" ]; then
  bad "(x15) so3ar_pin_census.py ABSENT -- the by-role census is the item's own repair and cannot be optional"
else
...
census_mutant() {   # $1 label  $2 expected-assertion-token  $3.. the mutation
  ...
  ( cd "$M" && "$@" ) >/dev/null 2>&1
  local o rc
  o="$(python3 "$M/so3ar_pin_census.py" --root "$M" 2>&1)"; rc=$?
...
census_mutant "(x15a) SENTINEL PLANTED (the SO-3a death mode)" "A2" \
  sed -i 's/^MD5_XF=[0-9a-f]\{32\}/MD5_XF=UNSET-PIN-SENTINEL-FAILS-CLOSED/' so3ar_chain_driver.sh
...
census_mutant "(x15b) STALE WELL-FORMED md5 PLANTED" "A3" \
  sed -i 's/^MD5_GRADER=[0-9a-f]\{32\}/MD5_GRADER=deadbeefdeadbeefdeadbeefdeadbeef/' so3ar_chain_driver.sh
...
census_mutant "(x15c) NEW UNPINNED CONSUMER PLANTED" "A5" \
  bash -c 'printf "echo \"\$MD5_NOSUCHPIN  \$BASE/so3ar_grade.py\" | md5sum -c -\n" >> so3ar_run_arm.sh'
...
census_mutant "(x15f) CONSUMER DELETED, PIN LEFT DEAD" "A4" \
  sed -i '/\$MD5_AGG  \$AGG_SCRIPT/d;/MD5_AGG/!b' so3ar_chain_driver.sh
...
python3 - "$PB/so3ar_pin_census.py" <<'BLIND'
import re, sys
p = sys.argv[1]
s = open(p).read()
...
open(p, "w").write(s)
BLIND
PB_OUT="$(python3 "$PB/so3ar_pin_census.py" --root "$PB" 2>&1)"; PB_RC=$?
...
cp -a "$HERE/so3ar_xf.py" "$HERE/so3ar_runScript.py" "$PD/"
PROD_BEFORE=$(md5sum "$PD/so3ar_runScript.py" | cut -d' ' -f1)
PIN_FROZEN=$(python3 -c 'import re,sys; print(re.search(r"^PRODUCER_MD5 = \"([^\"]*)\"", open(sys.argv[1]).read(), re.M).group(1))' "$PD/so3ar_xf.py")

if [ "$PIN_FROZEN" = "$PROD_BEFORE" ]; then
  ok "(p1) THE PRODUCER PIN IS SET AND CORRECT: so3ar_xf.py:PRODUCER_MD5 = $PIN_FROZEN = md5(so3ar_runScript.py).  SO-3a shipped this constant holding UNSET-PRODUCER-PIN-SENTINEL-FAILS-CLOSED and died on it in a container"
else
  bad "(p1) PRODUCER_MD5=$PIN_FROZEN but md5(so3ar_runScript.py)=$PROD_BEFORE"
fi

# ---- MUTATE THE PRODUCER: the pin must REFUSE through the real code path ----
printf '\n# (p2) mutation: one comment line, appended\n' >> "$PD/so3ar_runScript.py"
PROD_MUT=$(md5sum "$PD/so3ar_runScript.py" | cut -d' ' -f1)
MUT_ERR=$( cd "$PD" && python3 so3ar_xf.py -mode X 2>&1 >/dev/null ); MUT_RC=$?
if [ "$MUT_RC" -eq 2 ] && printf '%s\n' "$MUT_ERR" | grep -q "SO3AR_XF REFUSE producer md5 $PROD_MUT"; then
  ok "(p2) PIN REFUSES ON A MUTATED PRODUCER: exit 2, 'SO3AR_XF REFUSE producer md5 ${PROD_MUT:0:12}... != frozen ${PIN_FROZEN:0:12}...' -- the pin is live, not decorative"
else
...
cp -a "$HERE/so3ar_runScript.py" "$PD/so3ar_runScript.py"
PROD_AFTER=$(md5sum "$PD/so3ar_runScript.py" | cut -d' ' -f1)
...
REAL_ERR=$( cd "$PD" && python3 so3ar_xf.py -mode X 2>&1 >/dev/null ); REAL_RC=$?
if printf '%s\n' "$REAL_ERR" | grep -q "SO3AR_XF REFUSE producer md5"; then
...
echo "SO3aR G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
