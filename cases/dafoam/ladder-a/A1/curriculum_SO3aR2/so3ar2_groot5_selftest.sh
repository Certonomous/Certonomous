#!/usr/bin/env bash
# SO-3aR2 G-ROOT.5 SELFTEST -- ADOPTED from curriculum_SO1b/so1b_groot5_selftest.sh
# (md5 read at run time and printed below), driven against SO-3aR2's OWN FROZEN
# launcher so3ar2_run_arm.sh.  Discharges the requirement PREREGISTRATION.md:227
# (section 7.3) registered against this item:
#
#   "before this item is enqueued, the G-ROOT.5 container legs must be driven --
#    including a sacrificial container named so3ar2_<ARM>_... and a sacrificial
#    live pid with cwd inside the run root -- and the three new forbidden roots
#    must each be shown to abort at G-ROOT.1."
#
# WHAT SO-3aR2 ACTUALLY REGISTERS, stated plainly because a sibling family
# registers something else.  SO-3aR2's G-ROOT.5 is "A LIVE ARM IS NEVER
# RE-STAGED" (so3ar2_run_arm.sh:234-267) and its refusal code is EXIT 3, on two
# live readings taken BEFORE any destructive act:
#   (a) a RUNNING container carrying so3ar2_<ARM>_ ;
#   (b) a driver pidfile in the run root naming a LIVE pid that is not an
#       ancestor of the launcher, or whose cwd IS the run root.
# A STALE pidfile does NOT block.  SO-3aR2's launcher does NOT refuse merely
# because the run root exists -- that clause belongs to the D12R2/W3 chain
# driver's phase 1 (exit 6) and SO-3aR2's driver instead re-stages on the FIRST
# fire only (so3ar2_chain_driver.sh:72-91).  Both directions are driven below and
...
# `sudo -n rm -rf "$WORK"` (so3ar2_run_arm.sh:338), and every leg here refuses
...
BOGUS=no-such-image:so3ar2-selftest
ARM=X-S
CPUSET=14
PASS=0; FAIL=0
...
echo "SO3aR2 G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1) grader_md5=$(md5sum "$GRADER" | cut -d' ' -f1)"
...
# =============================================================================
# (a) a sacrificial RUNNING container carrying so3ar2_<ARM>_ .  ONE cpuset (the
# item's registered 9), 0.1 core, 64 MB, an explicit name, a bounded sleep, and
# `docker rm -f` by that name at the end of the leg.
CN="so3ar2_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
...
echo "$SPID" > "$BASE/so3ar2_driver.pid"; mine "$BASE/so3ar2_driver.pid"
...
rm -f "$BASE/so3ar2_driver.pid"

# (b2) a STALE pidfile must NOT block -- the guard must discriminate, not refuse.
echo "999999" > "$BASE/so3ar2_driver.pid"; mine "$BASE/so3ar2_driver.pid"
...
echo "$out" | grep -q "D4S_G_ROOT_PASS item=SO3aR2" && ok "(c2) G-ROOT.1-.3 also PASS on this item's own root, ledger_clean=yes" || bad "(c2) G-ROOT.1-.3 did not pass on the item's own root"
...
if [ "$rc" -eq 4 ] && echo "$out" | grep -q "ledger_clean=yes"; then
  ok "(d4) DRIVEN CONTROL: the SAME ledger carrying ITEM=SO3aR2 is ACCEPTED -- (d3)'s refusal is a reading of the line, not of the file's existence"
...
cp -a "$HERE/so3ar2_runScript.py" "$BASE/so3ar2_runScript.py"; mine "$BASE/so3ar2_runScript.py"
cp -a "$HERE/so3ar2_xf.py" "$BASE/so3ar2_xf.py"; mine "$BASE/so3ar2_xf.py"
cp -a "$HERE/so3ar2_decomposeParDict" "$BASE/base/system/decomposeParDict"; mine "$BASE/base/system/decomposeParDict"

# ---- SO-3aR2 REGISTERED STATE OF THESE LEGS, AND WHY THEY ASSERT A REFUSAL THE
# ---- PARENT'S ASSERTED A PASS FOR.
# The parent's (e1)-(e3) expected the staged-instrument md5s to VERIFY and the
# run to proceed to G-ROW.  SO-3aR2's instrument pins are deliberately the
# FAIL-CLOSED SENTINEL until the Stage-2 amendment (so3ar2_run_arm.sh MD5_UNSET),
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^so3ar2_" && bad "(z4) a so3ar2_ container survives" || ok "(z4) no so3ar2_ container survives, running or stopped"
...
#
# On 2026-08-31 so3ar2_run_arm.sh was mutated four ways and the grader's 91-unit
...
# already carried SO-3aR2's, and only the code was right.  A comment table that
...
NINE="so3ar2_chain_driver.sh so3ar2_run_arm.sh so3ar2_xf.py so3ar2_grade.py so3ar2_runScript.py
so3ar2_aggregate_memory.py so3ar2_groot5_selftest.sh so3ar2_decomposeParDict so3ar2_stop_marker.sh"
MISSING=""
...
# so3ar2_xf.py REFUSES (exit 2) otherwise, and this lane shipped a docstring that
...
ANCH=$(python3 -c "
import sys; sys.path.insert(0,'$HERE')
import so3ar2_xf as XF; print(XF.ANCHOR)")
NANCH=$(grep -Fc -- "$ANCH" "$HERE/so3ar2_runScript.py")
if [ "$NANCH" -eq 1 ]; then
  ok "(x7) the producer carries the instrument's anchor EXACTLY ONCE (count=$NANCH)"
else bad "(x7) the producer carries the anchor $NANCH times -- so3ar2_xf.py:478 would REFUSE every arm"; fi
...
import ast
import so3ar2_xf as XF
src = open(HERE + "/so3ar2_runScript.py").read()
ns = {"__name__": "hdr", "__file__": "so3ar2_runScript.py"}
saved = list(sys.argv); sys.argv = ["so3ar2_runScript.py", "-task", "run_model"]
exec(compile(src.split(XF.ANCHOR)[0], "so3ar2_runScript.py", "exec"), ns)
sys.argv = saved
bad = []
...
cp -a "$HERE/so3ar2_runScript.py" "$KP/producer_planted.py"
...
import so3ar2_grade as G
present, absent = G.so3ar2_instrument_files()
...
# consumer -- so3ar2_chain_driver.sh's `img_of` -- derived the row from an ARM-NAME
...
DRV="$HERE/so3ar2_chain_driver.sh"
...
DRVF="$HERE/so3ar2_chain_driver.sh"
...
MD5_LAUNCHER so3ar2_run_arm.sh
MD5_GRADER so3ar2_grade.py
MD5_RUNSCRIPT so3ar2_runScript.py
MD5_XF so3ar2_xf.py
MD5_AGG so3ar2_aggregate_memory.py
MD5_DECOMP so3ar2_decomposeParDict
MD5_STOP_MARKER so3ar2_stop_marker.sh
PINS
...
KACT=$(md5sum "$HERE/so3ar2_grade.py" | cut -d' ' -f1)
[ -n "$KPIN" ] && [ "$KPIN" != "$KACT" ] \
...
# =============================================================================
# (x15) THE BY-ROLE PIN CENSUS, AND (p1)-(p4) THE PRODUCER PIN DRIVEN
#
# THIS IS SO-3aR2's REASON TO EXIST.  SO-3a's (x12)-(x14) above are RIGHT about
# what they assert and BLIND to what killed it: they select pins by the SHAPE OF
# THE VALUE, inside ONE FILE.  `so3ar2_xf.py:PRODUCER_MD5` held a value chosen
# DELIBERATELY not to be md5-shaped, in a different file, and (x14) reported
# 13 of 13 while the item could not launch.
#
# `so3ar2_pin_census.py` sweeps BY ROLE across every instrument.  Below it is
# driven GREEN on the real tree and RED in SIX independent directions, EACH
# FROM A SINGLE MUTATION IN ITS OWN BYTE-IDENTICAL COPY.  One mutation at a
# time is not fastidiousness: a compound mutant cannot attribute a detection,
# because one assertion's refusal masks another's silence.
# =============================================================================
echo ""
echo "(x15/p) BY-ROLE PIN CENSUS and THE PRODUCER PIN"
CENSUS="$HERE/so3ar2_pin_census.py"
if [ ! -f "$CENSUS" ]; then
  bad "(x15) so3ar2_pin_census.py ABSENT -- the by-role census is the item's own repair and cannot be optional"
else

# ---- (x15) the CONTROL: green on the unmutated tree -------------------------
CEN_OUT="$(python3 "$CENSUS" --root "$HERE" 2>&1)"; CEN_RC=$?
NPINS=$(printf '%s\n' "$CEN_OUT" | grep -oP '^ENUMERATED \K[0-9]+' | head -1)
[ "$CEN_RC" -eq 0 ] \
  && ok "(x15) PIN CENSUS GREEN on the real tree: $NPINS pin-like constants enumerated BY ROLE across every instrument, each with a registered disposition, each md5-kind pin holding a real md5 and equal to the file it pins" \
  || { bad "(x15) PIN CENSUS rc=$CEN_RC on the real tree"; printf '%s\n' "$CEN_OUT" | sed -n '/CENSUS RED/,$p' | head -20; }

# ---- one helper: a byte-identical copy, ONE mutation, one reading ----------
census_mutant() {   # $1 label  $2 expected-assertion-token  $3.. the mutation
  local label="$1"; local want="$2"; shift 2
  local M; M="$(mktemp -d)"; CREATED+=("$M")
  cp -a "$HERE/." "$M/" 2>/dev/null
  ( cd "$M" && "$@" ) >/dev/null 2>&1
  local o rc
  o="$(python3 "$M/so3ar2_pin_census.py" --root "$M" 2>&1)"; rc=$?
  local hit; hit=$(printf '%s\n' "$o" | grep -c "^  $want ")
  if [ "$rc" -eq 1 ] && [ "$hit" -ge 1 ]; then
    ok "$label -- census RED on $want, $hit finding(s), from ONE mutation"
  else
    bad "$label -- expected rc=1 with a $want finding, got rc=$rc hits=$hit"
    printf '%s\n' "$o" | sed -n '/CENSUS RED/,$p' | head -8
  fi
  rm -rf "$M"
}

# DIRECTION 1 -- A SENTINEL VALUE.  THE SO-3a DEATH, PLANTED.  A census keyed
# on the shape of a value cannot see a value chosen for not having that shape.
census_mutant "(x15a) SENTINEL PLANTED (the SO-3a death mode)" "A2" \
  sed -i 's/^MD5_XF=[0-9a-f]\{32\}/MD5_XF=UNSET-PIN-SENTINEL-FAILS-CLOSED/' so3ar2_chain_driver.sh

# DIRECTION 2 -- A STALE BUT WELL-FORMED md5.  The value is md5-shaped and
# wrong; only a comparison against the file can see it.
census_mutant "(x15b) STALE WELL-FORMED md5 PLANTED" "A3" \
  sed -i 's/^MD5_GRADER=[0-9a-f]\{32\}/MD5_GRADER=deadbeefdeadbeefdeadbeefdeadbeef/' so3ar2_chain_driver.sh

# DIRECTION 3 -- A NEW UNPINNED CONSUMER: an md5sum checkfile line reading a
# variable no pin declares.
census_mutant "(x15c) NEW UNPINNED CONSUMER PLANTED" "A5" \
  bash -c 'printf "echo \"\$MD5_NOSUCHPIN  \$BASE/so3ar2_grade.py\" | md5sum -c -\n" >> so3ar2_run_arm.sh'

# DIRECTION 4 -- A NEW PIN WITH NO DISPOSITION: the SO-1c gap, one layer up.
census_mutant "(x15d) NEW UNDISPOSED PIN PLANTED (name-role)" "A1" \
  bash -c 'printf "MD5_NEWTHING=00000000000000000000000000000000\n" >> so3ar2_chain_driver.sh'

# DIRECTION 5 -- A PIN THE NAME RULE CANNOT SEE: an md5-shaped value under an
# identifier carrying no MD5 token.  E-NAME alone would miss it; E-SHAPE
# catches it.  THE TWO ENUMERATORS COVER EACH OTHER'S BLINDNESS, and this leg
# is the proof, not the claim.
census_mutant "(x15e) SHAPE-ONLY PIN PLANTED (no MD5 in the name)" "A1" \
  bash -c 'printf "\nPRODUCER_FINGERPRINT = \"00000000000000000000000000000000\"\n" >> so3ar2_xf.py'

# DIRECTION 6 -- A DEAD PIN: its only consumer deleted.  A dead pin sits in the
# tree one edit from fail-open re-use, which is why S2-1 section 4 deleted
# MD5_UNSET rather than leaving it defined.
census_mutant "(x15f) CONSUMER DELETED, PIN LEFT DEAD" "A4" \
  sed -i '/\$MD5_AGG  \$AGG_SCRIPT/d;/MD5_AGG/!b' so3ar2_chain_driver.sh

# ---- (p4) THE CENSUS'S OWN PLANTED CONTROL, SHOWN REFUSING -----------------
# A census that parses nothing reports zero problems.  Blind its python
# extractor in a copy and it must REFUSE (exit 2), not report green.
PB="$(mktemp -d)"; CREATED+=("$PB")
cp -a "$HERE/." "$PB/" 2>/dev/null
python3 - "$PB/so3ar2_pin_census.py" <<'BLIND'
import re, sys
p = sys.argv[1]
s = open(p).read()
s = s.replace("    tree = ast.parse(text, filename=path)\n",
              "    tree = ast.parse(text, filename=path)\n    return []          # BLINDED BY (p4)\n", 1)
open(p, "w").write(s)
BLIND
PB_OUT="$(python3 "$PB/so3ar2_pin_census.py" --root "$PB" 2>&1)"; PB_RC=$?
if [ "$PB_RC" -eq 2 ] && printf '%s\n' "$PB_OUT" | grep -q "REFUSE the census's own extractor"; then
  ok "(p4) RULE 3: with its python extractor blinded the census REFUSES (exit 2) and reports NOTHING -- a zero from a reader not shown able to see a non-zero is not evidence"
else
  bad "(p4) a blinded census did not refuse (rc=$PB_RC)"
fi
rm -rf "$PB"

# ---- (p1)-(p3) THE PRODUCER PIN, DRIVEN THROUGH THE REAL CODE PATH ---------
# SO-3a's producer pin was never driven at all: it was read for the first time
# inside a container, on the item's own second arm.  Here it is driven on the
# HOST, in a copy, in BOTH directions, one mutation at a time.
PD="$(mktemp -d)"; CREATED+=("$PD")
cp -a "$HERE/so3ar2_xf.py" "$HERE/so3ar2_runScript.py" "$PD/"
PROD_BEFORE=$(md5sum "$PD/so3ar2_runScript.py" | cut -d' ' -f1)
PIN_FROZEN=$(python3 -c 'import re,sys; print(re.search(r"^PRODUCER_MD5 = \"([^\"]*)\"", open(sys.argv[1]).read(), re.M).group(1))' "$PD/so3ar2_xf.py")

if [ "$PIN_FROZEN" = "$PROD_BEFORE" ]; then
  ok "(p1) THE PRODUCER PIN IS SET AND CORRECT: so3ar2_xf.py:PRODUCER_MD5 = $PIN_FROZEN = md5(so3ar2_runScript.py).  SO-3a shipped this constant holding UNSET-PRODUCER-PIN-SENTINEL-FAILS-CLOSED and died on it in a container"
else
  bad "(p1) PRODUCER_MD5=$PIN_FROZEN but md5(so3ar2_runScript.py)=$PROD_BEFORE"
fi

# ---- MUTATE THE PRODUCER: the pin must REFUSE through the real code path ----
printf '\n# (p2) mutation: one comment line, appended\n' >> "$PD/so3ar2_runScript.py"
PROD_MUT=$(md5sum "$PD/so3ar2_runScript.py" | cut -d' ' -f1)
MUT_ERR=$( cd "$PD" && python3 so3ar2_xf.py -mode X 2>&1 >/dev/null ); MUT_RC=$?
if [ "$MUT_RC" -eq 2 ] && printf '%s\n' "$MUT_ERR" | grep -q "SO3AR2_XF REFUSE producer md5 $PROD_MUT"; then
  ok "(p2) PIN REFUSES ON A MUTATED PRODUCER: exit 2, 'SO3AR2_XF REFUSE producer md5 ${PROD_MUT:0:12}... != frozen ${PIN_FROZEN:0:12}...' -- the pin is live, not decorative"
else
  bad "(p2) a mutated producer was not refused (rc=$MUT_RC): $(printf '%s' "$MUT_ERR" | tail -2)"
fi

# ---- RESTORE, PROVED BY HASH, THEN DRIVE THE REAL PRODUCER -----------------
cp -a "$HERE/so3ar2_runScript.py" "$PD/so3ar2_runScript.py"
PROD_AFTER=$(md5sum "$PD/so3ar2_runScript.py" | cut -d' ' -f1)
if [ "$PROD_AFTER" = "$PROD_BEFORE" ] && [ "$PROD_AFTER" != "$PROD_MUT" ]; then
  ok "(p3) RESTORE PROVED BY HASH: md5 before=$PROD_BEFORE, mutated=$PROD_MUT, after restore=$PROD_AFTER -- byte-identical to the original and demonstrably different from the mutant"
else
  bad "(p3) restore not proved (before=$PROD_BEFORE mutated=$PROD_MUT after=$PROD_AFTER)"
fi
REAL_ERR=$( cd "$PD" && python3 so3ar2_xf.py -mode X 2>&1 >/dev/null ); REAL_RC=$?
if printf '%s\n' "$REAL_ERR" | grep -q "SO3AR2_XF REFUSE producer md5"; then
  bad "(p3b) the REAL producer was refused by its own pin -- the pin is wrong"
else
  ok "(p3b) PIN PASSES ON THE REAL PRODUCER: no 'REFUSE producer md5' on stderr, so execution reached the checks BEYOND the pin.  Stated at its true strength: on this HOST the next step needs mpi4py and fails there (rc=$REAL_RC), which is a DIFFERENT failure and is why the discriminator is the refusal TOKEN and never the exit status"
fi
rm -rf "$PD"
...
# ===========================================================================
# (c1)-(c6) THE COLLISION LEG, DRIVEN -- SO-3aR2's REASON TO EXIST
#
# `so3ar2_collision_leg.py` is itself an instrument, and an instrument that has
# not been driven RED is a claim.  These legs drive it from OUTSIDE: they build a
# byte-identical copy of the item's tree in a temp directory that MIRRORS THE REAL
# LAYOUT (so the leg's read-only reference to SO-3aR's frozen producer resolves
# the same way it does in the repository), apply ONE mutation at a time, and
# require the leg to come back RED or REFUSE.
#
# WHY THE COPY AND NOT THE REAL TREE: SO-3aR's `so3ar_runScript.py` is a CLOSED
# item's frozen file.  CLAUDE.md rule 6 -- a frozen file is never edited.  Nothing
# below writes into `curriculum_SO3aR/`; the parent's producer is COPIED and the
# copy's md5 is asserted equal to the original before anything is driven.
# ===========================================================================
if [ -f "$HERE/so3ar2_collision_leg.py" ]; then
CT=$(mktemp -d "${TMPDIR:-/tmp}/so3ar2_leg.XXXXXX")
trap 'rm -rf "$CT"' EXIT
mkdir -p "$CT/curriculum_SO3aR2" "$CT/curriculum_SO3aR"
cp -a "$HERE/so3ar2_collision_leg.py" "$HERE/so3ar2_runScript.py" "$HERE/so3ar2_run_arm.sh" \
      "$CT/curriculum_SO3aR2/"
cp -a "$HERE/../curriculum_SO3aR/so3ar_runScript.py" "$CT/curriculum_SO3aR/"
CP_SRC=$(md5sum "$HERE/../curriculum_SO3aR/so3ar_runScript.py" | cut -d' ' -f1)
CP_DST=$(md5sum "$CT/curriculum_SO3aR/so3ar_runScript.py" | cut -d' ' -f1)
CC="$CT/curriculum_SO3aR2"
if [ "$CP_SRC" = "$CP_DST" ]; then
  ok "(c0) the parent's frozen producer is COPIED, not touched: md5 source=$CP_SRC copy=$CP_DST, byte-identical, and nothing below writes into curriculum_SO3aR/"
else
  bad "(c0) the parent copy is not byte-identical (source=$CP_SRC copy=$CP_DST)"
fi

# ---- (c1) GREEN on the frozen tree, and the RED evidence is IN the green run --
C1_OUT=$(cd "$CC" && python3 so3ar2_collision_leg.py --root "$CC" 2>&1); C1_RC=$?
if [ "$C1_RC" -eq 0 ]; then
  ok "(c1) THE COLLISION LEG IS GREEN ON THE FROZEN TREE: rc=0 -- three DISTINCT run directories, three real os.rename calls on a real filesystem, no raise"
else
  bad "(c1) the collision leg is not green on the frozen tree (rc=$C1_RC): $(printf '%s' "$C1_OUT" | grep FINDING | head -3)"
fi
if printf '%s\n' "$C1_OUT" | grep -q "'label': 'L2_mutant_no_run_directory'.*'collided': True" \
   && printf '%s\n' "$C1_OUT" | grep -q "'label': 'L3_parent_real_bytes'.*'collided': True" \
   && printf '%s\n' "$C1_OUT" | grep -q "already exists, moving failed!"; then
  ok "(c1b) THE RED IS INSIDE THE GREEN RUN, WHICH IS THE POINT: the same invocation that reports rc=0 also reports L2 collided=True on the SAME bytes with ONE mutation and L3 collided=True on SO-3aR's OWN frozen producer, both carrying the literal 'already exists, moving failed!'.  A green that has not first seen red is not evidence"
else
  bad "(c1b) the green run does not carry its own RED limbs: $(printf '%s' "$C1_OUT" | grep -c collided) collided readings seen"
fi

# ---- (c2) THE LEG'S OWN PIN, DRIVEN.  A mutated producer must REFUSE, exit 2 --
printf '\n# (c2) mutation: one comment line, appended\n' >> "$CC/so3ar2_runScript.py"
C2_OUT=$(cd "$CC" && python3 so3ar2_collision_leg.py --root "$CC" 2>&1); C2_RC=$?
if [ "$C2_RC" -eq 2 ] && printf '%s\n' "$C2_OUT" | grep -q "SO3AR2_COLLISION REFUSE producer md5"; then
  ok "(c2) THE LEG'S PRODUCER PIN REFUSES A MUTATED PRODUCER: exit 2, and exit 2 is the ABSENCE of a reading -- neither green nor red"
else
  bad "(c2) a mutated producer was not refused by the collision leg (rc=$C2_RC)"
fi
cp -a "$HERE/so3ar2_runScript.py" "$CC/so3ar2_runScript.py"

# ---- (c3) THE CORE LEG: UNFIX THE PRODUCER AND REQUIRE RED -------------------
# ONE mutation: delete the `run_directory=` keyword from the builder call, which
# reconstructs SO-3aR's exact state.  The leg's own pin is re-pointed at the
# mutant so the refusal path cannot mask the finding -- a REFUSE here would be a
# leg that never got to look.
# THE KEYWORD ARGUMENT IS DELETED, NOT THE LINE.  In this file the
# `run_directory=` line also carries the call's closing paren, so a line-wise
# `grep -v` leaves a source that does not PARSE -- and a mutant that does not
# parse tests the reader's error handling, not the repair.  Caught by driving.
python3 - "$HERE/so3ar2_runScript.py" "$CC/so3ar2_runScript.py" <<'PYEOF'
import re, sys
t = open(sys.argv[1]).read()
t2, n = re.subn(r",\s*run_directory=RUN_DIRS\[sc\]", "", t, count=1)
assert n == 1, "expected exactly one run_directory keyword to delete, got %d" % n
open(sys.argv[2], "w").write(t2)
PYEOF
NEWP=$(md5sum "$CC/so3ar2_runScript.py" | cut -d' ' -f1)
python3 - "$CC/so3ar2_collision_leg.py" "$NEWP" <<'PYEOF'
import re, sys
p, new = sys.argv[1], sys.argv[2]
t = open(p).read()
t = re.sub(r'^PRODUCER_MD5 = "[^"]*"', 'PRODUCER_MD5 = "%s"' % new, t, count=1, flags=re.M)
open(p, "w").write(t)
PYEOF
C3_OUT=$(cd "$CC" && python3 so3ar2_collision_leg.py --root "$CC" 2>&1); C3_RC=$?
if [ "$C3_RC" -eq 1 ] && printf '%s\n' "$C3_OUT" | grep -q "L1 THE FIX DID NOT HOLD"; then
  ok "(c3) THE LEG GOES RED ON THE UNFIXED STATE: with the per-point run_directory removed by ONE mutation, rc=1 and the finding reads 'L1 THE FIX DID NOT HOLD -- the frozen producer collided'.  THIS IS THE WHOLE PROOF: a fix without a leg that fails on the unfixed state is not proved, and this lab has been killed three times by legs that read green on the state that kills the item"
else
  bad "(c3) the leg did NOT go red on an unfixed producer (rc=$C3_RC): $(printf '%s' "$C3_OUT" | grep FINDING | head -2)"
fi
cp -a "$HERE/so3ar2_runScript.py" "$CC/so3ar2_runScript.py"
cp -a "$HERE/so3ar2_collision_leg.py" "$CC/so3ar2_collision_leg.py"
C3B=$(md5sum "$CC/so3ar2_runScript.py" | cut -d' ' -f1)
if [ "$C3B" = "$(md5sum "$HERE/so3ar2_runScript.py" | cut -d' ' -f1)" ] && [ "$C3B" != "$NEWP" ]; then
  ok "(c3b) RESTORE PROVED BY HASH: after=$C3B equals the original and differs from the unfixed mutant $NEWP"
else
  bad "(c3b) restore not proved (after=$C3B mutant=$NEWP)"
fi

# ---- (c4) THE LAUNCHER LIMB: a run_directory nobody stages ------------------
# A `run_directory` DAFoam is told to use but the launcher never creates is a
# DIFFERENT failure wearing the SAME symptom, and L1-L3 cannot see it because
# they make the directories themselves.
# ONE mutation, and it lands on the STAGING loop only -- the launcher also
# iterates the same three directories in its G-IC0 block, and a whole-file
# substring check stayed GREEN on this exact mutation until it was driven.
python3 - "$HERE/so3ar2_run_arm.sh" "$CC/so3ar2_run_arm.sh" <<'PYEOF'
import re, sys
t = open(sys.argv[1]).read()
t2, n = re.subn(r'(for mp in mp0 )mp1 (mp2; do\n    cp -a "\$BASE/MESH")', r'\1\2', t, count=1)
assert n == 1, "expected exactly one staging-loop list to mutate, got %d" % n
open(sys.argv[2], "w").write(t2)
PYEOF
NEWL=$(md5sum "$CC/so3ar2_run_arm.sh" | cut -d' ' -f1)
python3 - "$CC/so3ar2_collision_leg.py" "$NEWL" <<'PYEOF'
import re, sys
p, new = sys.argv[1], sys.argv[2]
t = open(p).read()
t = re.sub(r'^LAUNCHER_MD5 = "[^"]*"', 'LAUNCHER_MD5 = "%s"' % new, t, count=1, flags=re.M)
open(p, "w").write(t)
PYEOF
C4_OUT=$(cd "$CC" && python3 so3ar2_collision_leg.py --root "$CC" 2>&1); C4_RC=$?
if [ "$C4_RC" -eq 1 ] && printf '%s\n' "$C4_OUT" | grep -q "L4 the launcher does not stage"; then
  ok "(c4) THE LAUNCHER LIMB GOES RED: with mp1 removed from the staging loop by ONE mutation, rc=1 and the finding names the unstaged directory.  The producer and the launcher have to AGREE, and this is the leg that makes them"
else
  bad "(c4) the launcher limb did not go red on an unstaged run directory (rc=$C4_RC)"
fi
cp -a "$HERE/so3ar2_run_arm.sh" "$CC/so3ar2_run_arm.sh"
cp -a "$HERE/so3ar2_collision_leg.py" "$CC/so3ar2_collision_leg.py"

# ---- (c5) RULE 3: the leg's own reader proved not blind ---------------------
if printf '%s\n' "$C1_OUT" | grep -q "L5 OK -- a reader that finds no builders REFUSES"; then
  ok "(c5) RULE-3 CONTROL ON THE LEG'S OWN READER: blinding the AST extractor makes the leg REFUSE, not report 'no collisions'.  A leg that extracts its subjects from the file under test cannot otherwise tell 'no collisions' from 'I could not read the file', and that is precisely how a zero becomes a statement about a rule set rather than about the code"
else
  bad "(c5) the blinding control did not fire: L5 did not report a refusal"
fi

# ---- (c6) THE PARENT'S BYTES ARE UNTOUCHED, RE-ASSERTED AFTER EVERYTHING ----
CP_END=$(md5sum "$HERE/../curriculum_SO3aR/so3ar_runScript.py" | cut -d' ' -f1)
if [ "$CP_END" = "$CP_SRC" ]; then
  ok "(c6) SO-3aR's frozen producer is BYTE-IDENTICAL after every leg above: md5 $CP_END, unchanged.  A closed item's record is read by its successor and never written by it"
else
  bad "(c6) SO-3aR's frozen producer CHANGED during this selftest ($CP_SRC -> $CP_END)"
fi
rm -rf "$CT"; trap - EXIT
else
  bad "(c1)-(c6) so3ar2_collision_leg.py is ABSENT -- the item's central instrument is not built"
fi

echo "SO3aR2 G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
