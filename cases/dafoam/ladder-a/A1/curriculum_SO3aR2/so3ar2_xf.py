#!/usr/bin/env python
"""Curriculum SO-3aR2 -- NACA0012, THE ALPHA-MULTIPOINT WEIGHTED OBJECTIVE
...
with EXACTLY the registered deltas below and no other.
  ...
  * `patchV` IS NOT A DESIGN VARIABLE IN THIS ITEM and is REMOVED from
    ...
    SO-3aR2's structural gate is G-MP-STRUCT, on the assembly identity, and the
  ...
  * THE STEP-BASED TRIVIAL BASELINE IS BACK, at h = 1e-8, and that is a
    ...
    bit-repeatable and h = 1e-8 never enters subtractive cancellation.  SO-3aR2's
  ...
  * Printed tokens SO2A_* -> SO3AR2_*; artefact names so2a_* -> so3ar2_*.
...
PRODUCER = "so3ar2_runScript.py"
# ---- THE PRODUCER PIN.  THIS CONSTANT IS THE WHOLE REASON SO-3aR2 EXISTS.
# ----
...
# ---- fourteenth that was not.  `so3ar2_pin_census.py` sweeps BY ROLE and
# ---- asserts, in A2, that a pin of this kind holds a real md5.
# ----
# ---- SET, and equal to md5(so3ar2_runScript.py).  It is asserted by A3 of the
# ---- census on every drive, and DRIVEN in both directions by (p1)-(p4) of
# ---- `so3ar2_groot5_selftest.sh`: REFUSE on a mutated producer, PASS on the
...
OUT_X = "so3ar2_X.json"
OUT_F = "so3ar2_F.json"
JSONL_F = "so3ar2_F.jsonl"
JSONL_X = "so3ar2_X.jsonl"
TERMINAL_X = "SO3AR2_X_WRITTEN"
TERMINAL_F = "SO3AR2_F_WRITTEN"
...
def parse_mode(argv):
    ...
    for i, a in enumerate(argv):
        if a == "-mode" and i + 1 < len(argv):
            mode = argv[i + 1]
    if mode not in ("X", "F"):
        sys.stderr.write("SO3AR2_XF usage: so3ar2_xf.py -mode X|F\n")
        sys.exit(64)
    return mode
...
def main():
    ...
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("SO3AR2_XF REFUSE producer md5 %s != frozen %s\n"
                         % (got, PRODUCER_MD5))
        sys.exit(2)
    ...
    if src.count(ANCHOR) != 1:
        sys.stderr.write("SO3AR2_XF REFUSE anchor %r appears %d times\n"
    ...
    ns = {"__name__": "so3ar2_frozen_header", "__file__": PRODUCER}
    ...
    if rank == 0:
        ...
        if not ok:
            sys.stderr.write("SO3AR2_XF REFUSE planted-zero control not seen on read-back: %s\n"
                             % json.dumps(detail, sort_keys=True, default=str))
            sys.exit(2)
        sys.stdout.write("SO3AR2_PLANTED_ZERO_CONTROL_SEEN n_zero_entries=%d want=%r\n"
