#!/usr/bin/env python3
"""SO-3aR2 PIN CENSUS -- A SWEEP BY **ROLE**, NEVER BY THE SHAPE OF A VALUE.

WHY THIS FILE EXISTS, AND IT IS THE WHOLE OF SO-3aR2's REASON TO EXIST
====================================================================
...
SCOPE.  This file is an INSTRUMENT of curriculum SO-3aR2 and speaks for no other
...
SOURCES = [
    ...
    "so3ar2_grade.py",
    "so3ar2_runScript.py",
    "so3ar2_aggregate_memory.py",
    "so3ar2_stop_marker.sh",
    "so3ar2_groot5_selftest.sh",
    "so3ar2_xf_selftest.py",
    "so3ar2_pin_census.py",
    # SO-3aR2's new instrument.  It carries THREE pins -- the producer, the
    # launcher, and SO-3aR's own frozen producer, which it reads READ-ONLY to
    # reproduce the death on the bytes that caused it.  It joins the census in
...
SOURCES_EXCLUDED = {
    "so3ar2_decomposeParDict": "an OpenFOAM dictionary, not a program; it holds "
                              "no constants and executes nothing",
    "so3ar2_derive_from_so3ar.sh": "the one-shot derivation script; it runs "
...
DISPOSITION = {
    ...
    "so3ar2_chain_driver.sh:MD5_LAUNCHER":     ("LOCAL", "so3ar2_run_arm.sh"),
    "so3ar2_chain_driver.sh:MD5_GRADER":       ("LOCAL", "so3ar2_grade.py"),
    "so3ar2_chain_driver.sh:MD5_RUNSCRIPT":    ("LOCAL", "so3ar2_runScript.py"),
    "so3ar2_chain_driver.sh:MD5_XF":           ("LOCAL", "so3ar2_xf.py"),
    "so3ar2_chain_driver.sh:MD5_AGG":          ("LOCAL", "so3ar2_aggregate_memory.py"),
    "so3ar2_chain_driver.sh:MD5_DECOMP":       ("LOCAL", "so3ar2_decomposeParDict"),
    "so3ar2_chain_driver.sh:MD5_STOP_MARKER":  ("LOCAL", "so3ar2_stop_marker.sh"),
    # ---- the chain driver's out-of-tree tutorial pins ----------------------
    "so3ar2_chain_driver.sh:MD5_TUT_RUNSCRIPT": ("TUTORIAL", "runScript.py"),
    "so3ar2_chain_driver.sh:MD5_TUT_GEN":       ("TUTORIAL", "genAirFoilMesh.py"),
    "so3ar2_chain_driver.sh:MD5_TUT_PREPROC":   ("TUTORIAL", "preProcessing.sh"),
    "so3ar2_chain_driver.sh:MD5_TUT_PS":        ("TUTORIAL", "profiles/NACA0012PS.profile"),
    "so3ar2_chain_driver.sh:MD5_TUT_SS":        ("TUTORIAL", "profiles/NACA0012SS.profile"),
    "so3ar2_chain_driver.sh:MD5_TUT_FFD":       ("TUTORIAL", "FFD/wingFFD.xyz"),
    ...
    "so3ar2_run_arm.sh:MD5_RUNSCRIPT":         ("LOCAL", "so3ar2_runScript.py"),
    "so3ar2_run_arm.sh:MD5_XF":                ("LOCAL", "so3ar2_xf.py"),
    "so3ar2_run_arm.sh:MD5_DECOMP":            ("LOCAL", "so3ar2_decomposeParDict"),
    ...
    "so3ar2_xf.py:PRODUCER_MD5":               ("LOCAL", "so3ar2_runScript.py"),
    # ---- THE COLLISION LEG'S THREE.  The third points OUT OF THIS DIRECTORY, at
    # ---- SO-3aR's own frozen producer -- the bytes that really died.  It is read
    # ---- READ-ONLY and never written (CLAUDE.md rule 6: a closed item's frozen
    # ---- file is never edited by a successor).  Registering it LOCAL with a
    # ---- relative target is deliberate: A3 then HASHES it and would go RED if
    # ---- anyone edited the parent, which is exactly the alarm this lane wants.
    "so3ar2_collision_leg.py:PRODUCER_MD5":        ("LOCAL", "so3ar2_runScript.py"),
    "so3ar2_collision_leg.py:LAUNCHER_MD5":        ("LOCAL", "so3ar2_run_arm.sh"),
    "so3ar2_collision_leg.py:PARENT_RUNSCRIPT_MD5": ("LOCAL",
                                                     "../curriculum_SO3aR/so3ar_runScript.py"),
    ...
    "so3ar2_grade.py:SO_MD5[SHIPPED]":         ("CONTAINER", "libidwarp.so @ SHIPPED image"),
    "so3ar2_grade.py:SO_MD5[PATCHED]":         ("CONTAINER", "libidwarp.so @ PATCHED image"),
    # ---- image digests: sha256, not md5, and an image is not a file ---------
    "so3ar2_grade.py:IMG_DIGEST[SHIPPED]":     ("DIGEST", "dafoam/opt-packages SHIPPED image"),
    "so3ar2_grade.py:IMG_DIGEST[PATCHED]":     ("DIGEST", "dafoam/opt-packages PATCHED image"),
    "so3ar2_run_arm.sh:IMG_SHIPPED_DIGEST":    ("DIGEST", "dafoam/opt-packages SHIPPED image"),
    "so3ar2_run_arm.sh:IMG_PATCHED_DIGEST":    ("DIGEST", "dafoam/opt-packages PATCHED image"),
}

# A pin whose in-repo file is STAGED into the run root under a DIFFERENT name.
# `so3ar2_decomposeParDict` is copied to `base/system/decomposeParDict` because
...
STAGED_AS = {
    "so3ar2_decomposeParDict": ["decomposeParDict"],
}
...
def run(root, tut_src_override=None):
    ...
    present = sorted(f for f in os.listdir(root)
                     if f.startswith("so3ar2_") and not f.endswith(".json")
    ...
    if tut_src is None:
        m = re.search(r"^TUT_SRC=(\S+)", sources_text["so3ar2_chain_driver.sh"], re.M)
        tut_src = m.group(1).strip("\"'") if m else None
    say("TUT_SRC (read from so3ar2_chain_driver.sh, not hard-coded) = %s" % tut_src)
