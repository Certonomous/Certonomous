#!/usr/bin/env python3
"""CURRICULUM SO-3aR2 -- THE FROZEN COMPARATOR.
...
frozen BEFORE its instruments existed.  SO-3aR2 freezes the document and the
...
file by md5, and `so3ar2_chain_driver.sh:MD5_GRADER` asserts it before staging.

Derived from `curriculum_SO2a/so2a_grade.py` with the deltas in
`so3ar2_grade_DELTAS_from_so2a.diff`.  FIVE of those deltas are the point of the
...
EXPECTATIONS -- a tautology on schema.  SO-3aR2's comparator is written AFTER two
of its consumers were frozen, so the contract runs the other way and is checkable
TODAY:

  * `so3ar2_chain_driver.sh:234` invokes `python3 $GRADER --root $BASE --out $GRADE_OUT`.
  ...
  * `so3ar2_stop_marker.sh:97,101,105` reads `verdict` at the TOP LEVEL, `rows` at
...
Neither fact is asserted in prose.  `selftest` RUNS `so3ar2_stop_marker.sh` on a
...
parent refuses on `ledger_row_absent` and on `arm_log_absent`.  **SO-3aR2 prints
...
--------------------------------------------------------------------------
Every X and F artefact in `_fix` is written by CALLING `so3ar2_xf.build_X_record`,
...
SOURCING `so3ar2_run_arm.sh`'s own `so3ar2_ledger_row`; when that launcher is absent
...
# ---- SO-3aR2's answer is not a better MAPPING, it is the ABSENCE of a second label
...
# SO-3aR2 REGISTERED DELTA.  This is the change that has to be argued, not just
...
# ---- SO-3aR2 rests on SO-1a's PATCHED row while SO-1a's ITEM verdict is GATE FAIL.
...
UPSTREAM = {
    ...
    "shipped_gate_detail": {
        ...
        "reading": "the SHIPPED toolchain row failed the FD band at a single point on this "
                   "very case; SO-3aR2 builds on the PATCHED row and the SHIPPED reading "
...
def compose_verdict_line():
    ...
    return ("UPSTREAM %s ITEM VERDICT GATE FAIL -- SHIPPED row GATE FAIL, PATCHED row PASS; "
            "SO-3aR2 builds on the PATCHED row and the SHIPPED GATE FAIL TRAVELS with every "
...
READERS = {
    "R1_read_ledger": {"produces": "core_min, rc, cpuset, DIGEST, oomkilled -> G1/G9/G10/G12",
                       "producer": "so3ar2_run_arm.sh:so3ar2_ledger_row",
    ...
    "R4_read_X": {"produces": "adjoint totals -> G5J, G5C, G-MP-STRUCT (ALL CAN PASS ON A SMALL NUMBER)",
                  "producer": "so3ar2_xf.build_X_record", "born_by": ["U60", "U61", "U62"], "born": True},
    "R5_read_F": {"produces": "FD derivative table -> G5J, G5C, G-TB",
                  "producer": "so3ar2_xf.build_F_record / build_fd_row / build_ctrl_row",
                  "born_by": ["U63", "U64", "U65"], "born": True},
    "R6_read_multipoint_identity": {"produces": "alphas and weights -> G-ALPHA",
                                    "producer": "so3ar2_xf.build_identity_block",
...
# (so3ar2_chain_driver.sh's `img_of`) was in shell -- so eight files were swept and
...
# SO-3aR2, 2026-08-31: BUMPED DELIBERATELY 92 -> 97, and the five are named so the
...
# form: it reads EVERY SO-3aR2 instrument and reports EVERY site that either
...
_ROW_SWEEP_PY = (
    ...
    (re.compile(r"""(?<![A-Za-z0-9_])(['"])(%s)\1""" % "|".join(ROW_SHORT_FORMS)),
     "a SHORT-FORM row literal; SO-3aR2 has ONE label set and no short form"),
...
_ROW_SWEEP_SH = (
    ...
    # unrepaired call site in this item -- `so3ar2_chain_driver.sh`'s `img_of`,
...
def so3ar2_instrument_files():
    ...
    # ELEVEN, not SO-3aR's ten and not SO-3a's eight.  `so3ar2_collision_leg.py`
    ...
    # cross to A1.
    names = ["so3ar2_grade.py", "so3ar2_xf.py", "so3ar2_runScript.py", "so3ar2_run_arm.sh",
             "so3ar2_groot5_selftest.sh", "so3ar2_chain_driver.sh", "so3ar2_stop_marker.sh",
             "so3ar2_aggregate_memory.py", "so3ar2_pin_census.py", "so3ar2_xf_selftest.py",
             "so3ar2_collision_leg.py"]
    present, absent = [], []
    for nm in names:
...
def grader_plant_X(base, xpath, tag, k, scale=None, plant_k=None):
    ...
    SO-3aR2 REGISTERED DELTA: `scale` is `d_ref` for the pair this plant lands on,
    ...
    if found:
        refuse("G-NOOPT", {"optimiser_evidence_in_the_artefacts": found[:10],
                           "note": ("Sanaa's `stands alone` is checked against the artefacts.  "
                                    "SO-3aR2's registered arm set contains no optimiser arm, so an "
...
def require_travelling_provenance():
    ...
    return {"verdict": "SATISFIED", "clauses": 5, "upstream": p,
            "note": ("the SHIPPED GATE FAIL travels with every claim SO-3aR2 makes; the ruling calls "
...
def grade(root):
    ...
    graded_k = {}
    # SO-3aR2 REGISTERED DELTA: `d_ref` is carried beside the index, because the
    ...
    # ---- so3ar2_stop_marker.sh:97,105 reads them there; `gates["G5J"]` carries the
    # ---- literal key so3ar2_stop_marker.sh:101 looks for.
...
LAUNCHER_PATH = os.path.join(_HERE, "so3ar2_run_arm.sh")
...
def _ledger_rows_via_launcher(k):
    ...
    OWN `so3ar2_ledger_row` -- the real producer's code -- not by a copy of its
    ...
    for arm in ARMS_DECLARED:
        ...
        cmd = ('. %s --source-only >/dev/null 2>&1; '
               'so3ar2_ledger_row %s %s img %s %d 60 %d %s %s 600 %s 12g "%s %s" 20.00 "%s" "%s" '
               '"%s" "" "" %s x'
...
def _fix(tmp, tweak=None):
    ...
    EVERY X AND F ARTEFACT IS WRITTEN BY `so3ar2_xf`'s OWN WRITERS and EVERY LEDGER
    ...
    if isinstance(rows_out, str):
        ...
        READERS["R1_read_ledger"]["not_born_reason"] = (
            "so3ar2_run_arm.sh:so3ar2_ledger_row could not be sourced (%s); the fixture fell back to a "
...
# ===================== the selftest ===================================================
STOP_MARKER_PATH = os.path.join(_HERE, "so3ar2_stop_marker.sh")


def _write_grade(root, rec, name="SO3aR2_grade_selftest.json"):
...
def stop_marker_basename():
    ...
    This lane's first draft hardcoded `SO3AR2_STOP_MARKER.json`; the frozen writer
    writes `SO3aR2_STOP_MARKER.json` (so3ar2_stop_marker.sh:59), so the check looked
...
def _run_stop_marker(root, gpath, executed=N_DECLARED):
    ...
    if not os.path.isfile(STOP_MARKER_PATH):
        return {"state": "NOT EXERCISED -- so3ar2_stop_marker.sh absent", "rc": None, "marker": None}
...
def selftest(tmp):
    ...
    unit("U11 SCHEMA the FROZEN so3ar2_stop_marker.sh reads this artefact's verdict",
    ...
    unit("U11b SCHEMA the marker ADDRESS is read out of the frozen writer, never guessed",
         stop_marker_basename() == "SO3aR2_STOP_MARKER.json"
         and sm["marker_path"].endswith("SO3aR2_STOP_MARKER.json"))
    ...
    gp_b = _write_grade(root0, r_break, "SO3aR2_grade_schemabreak.json")
    ...
    gp_b2 = _write_grade(root0, r_break2, "SO3aR2_grade_verdictbreak.json")
    ...
    # ---- SO-3aR2's PLANT-SIZING UNITS.  The plant is RELATIVE to the band it must
    ...
    present, absent = so3ar2_instrument_files()
    sweep = {os.path.basename(f): row_label_call_sites(f) for f in present}
    unit("U100 ROW-LABEL every built SO-3aR2 instrument is free of short-form row "
    ...
    # so3ar2_chain_driver.sh's `img_of` through eight files of sweeping.
...
def main():
    ...
    for i, a in enumerate(sys.argv):
        if a == "--root" and i + 1 < len(sys.argv):
            root = sys.argv[i + 1]
        # `--out` IS THE FROZEN DRIVER'S CONTRACT: so3ar2_chain_driver.sh:234 invokes
    ...
    if out is None:
        ...
        out = os.path.join(root, "SO3aR2_grade_%s.json" % stamp)
