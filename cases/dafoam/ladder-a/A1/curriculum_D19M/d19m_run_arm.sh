#!/usr/bin/env bash
# ===========================================================================
# Curriculum D19M ARM LAUNCHER -- NACA0012 SUBSONIC (DARhoSimpleFoam, M 0.288),
...
#       `libidwarp.so` md5 inside the container by `d19m_xf.py`.  Two readers,
...
#   (7) THE ENDPOINT ARMS STAGE `d19m_xopt.json` FROM THEIR OWN ROW'S O ARM.
#       Named explicitly, so the cold-start guard does not mistake an INPUT for
#       a pre-existing RESULT -- and XE/FE write `d19m_X.json`/`d19m_F.json`,
#       never `d19m_xopt.json`, so an input can never be read as this arm's own
#       output.
#
...
#
# `d19m_grade_selftest.py:_ledger_row_via_launcher` runs
#     bash -c '. d19m_run_arm.sh --source-only; d19m_ledger_row ...'
# so EVERY fixture row in the comparator's suite comes out of THESE bytes.  If
# this format and `d19m_grade.py:_LEDGER` ever diverge, the fixture rows stop
...
ITEM=D19M
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D19M-a1-naca0012-subsonic-multipoint
...
FORBIDDEN_ROOTS="/home/ubuntu/certonomous-runs/CURRICULUM-D19O-a1-naca0012-subsonic-optimisation
...
while IFS= read -r forb; do
  ...
  if [ "$BASE_REAL" = "$(realpath -m "$forb")" ]; then
    ...
    echo "  D19O's root holds this item's ENTIRE COST ANCHOR and its shape[7] endpoint"
    echo "  finding; D19R's holds d19r_selected_step.json, from which s* and the"
    echo "  shape[7] registration are read.  REFUSED."
    exit 3
  fi
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d19m_" | tr '\n' ',' | sed 's/,$//'
}
...
#   XE-S      1       12.0            540 s              12g
#   XE-P      1       12.0            540 s              12g
...
# ---- FROZEN INSTRUMENT HASHES (set at the freeze by d19m_repin.sh) --------
MD5_RUNSCRIPT=d1ae2ac5a620d9308b0d50059e944cf4
MD5_XF=3b029c4ee4032c459b116bcf128d802e
...
ARM="${1:-}"; IMG="${2:-}"
USAGE="usage: d19m_run_arm.sh <MESH|O-S|XE-S|FE-S|O-P|XE-P|FE-P> <image>"
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d19m_${ARM}_" 2>/dev/null | grep "^d19m_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
if [ -n "$LIVE_SAME_ARM" ]; then
  echo "ABORT G-ROOT.5 a RUNNING container already carries this item's prefix and arm: [$LIVE_SAME_ARM]"
  exit 3
fi
PIDFILE="$BASE/d19m_driver.pid"
if [ -f "$PIDFILE" ]; then
...
echo "$MD5_RUNSCRIPT  $BASE/d19m_runScript.py"          | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/d19m_xf.py"                        | md5sum -c - || { echo "ABORT xf md5"; exit 4; }
...
NAME="d19m_${ARM}_${STAMP}"
WORK="$BASE/$ARM"
LOG="$BASE/${ARM}_${STAMP}.log"

# ---- AGE GUARD DATUM: a LAUNCH SENTINEL, OUTSIDE every bind-mount source ----
# `d19m_age_guard.assert_sentinel_outside_mounts` REFUSES if the sentinel is at
...
import json, os, sys
sys.path.insert(0, os.environ["D19M_HERE"])
import d19m_age_guard as AGE
...
try:
    ...
    manifest = AGE.build_manifest(work, json.loads(os.environ["D19M_INPUTS"]))
except AGE.Refusal as exc:
    sys.stderr.write("ABORT age-guard datum: %s\n" % exc)
    sys.exit(1)
with open(os.path.join(work, ".d19m_manifest.json"), "w") as fh:
...
if [ "$ARM" = "MESH" ]; then
  ...
  export D19M_HERE="$HERE" D19M_INPUTS='["0.orig","constant","system"]'
  ...
  rm -f "$WORK/d19m_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" \
        "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.d19m_manifest.json" 2>/dev/null
  cp -a "$BASE/d19m_runScript.py" "$BASE/d19m_xf.py" "$BASE/d19m_stall.py" "$WORK/" \
    || { echo "ABORT stage instruments"; exit 4; }

  # (7) THE ENDPOINT ARMS' INPUT: their OWN row's final design point.
  if [ "$MODE" = "XE" ] || [ "$MODE" = "FE" ]; then
    OARM=$(o_arm_of "$ARM")
    test -f "$BASE/$OARM/d19m_xopt.json" || {
      echo "ABORT arm $ARM needs $BASE/$OARM/d19m_xopt.json from arm $OARM."
      ...
      exit 5; }
    cp -a "$BASE/$OARM/d19m_xopt.json" "$WORK/" || { echo "ABORT stage design point"; exit 4; }
  fi

  # COLD START, verified BEFORE the launch.  Every OUTPUT this item can write.
  # `d19m_xopt.json` is arm O's OUTPUT and is guarded like any other -- EXCEPT on
  # the endpoint arms, where it is a STAGED INPUT.  XE writes `d19m_X.json` and
  # FE writes `d19m_F.json`, never `d19m_xopt.json`, so an input can never be
  # mistaken for this arm's own result.  The exemption is BY ARM MODE and is
  # stated, not implicit.
  for bad in "$WORK/reports" "$WORK/d19m_O.json" "$WORK/d19m_X.json" "$WORK/d19m_F.json" \
             "$WORK/d19m_O.jsonl" "$WORK/d19m_X.jsonl" "$WORK/d19m_F.jsonl" \
             "$WORK/opt_IPOPT.txt" "$WORK/OptView.hst" "$WORK/dRdWColoring_1.bin"; do
    test -e "$bad" && { echo "ABORT G-COLD $bad exists"; exit 5; }
  done
  if [ "$MODE" = "O" ]; then
    test -e "$WORK/d19m_xopt.json" && { echo "ABORT G-COLD $WORK/d19m_xopt.json exists"; exit 5; }
  fi
  ...
  # ===========================================================================
  # (8) THE MULTIPOINT DELTA: ONE FULL CASE COPY PER OPERATING POINT.
  #
  # `d19m_runScript.py:RUN_DIRS` gives every scenario its own `run_directory`
  # (`mp0 mp1 mp2`) and `mesh_options_for` points that scenario's IDWarp at
  # `<work>/mp<i>`.  Each therefore needs a COMPLETE case, not a mesh link.
  #
  # WHY, AND IT IS NOT TIDINESS: SO-3aR DIED because three `DAFoamBuilder`s were
  # given NO `run_directory`, so three `DASolver` instances shared one case
  # directory and each renamed its solution to the same time
  # (`pyDAFoam.renameSolution`).  `d6r_run_arm.sh:313` solved it on A2 by staging
  # one directory per point; A1 did not carry it forward until SO-3, and this is
  # A1's compressible instance of the same repair.
  #
  # THE NAMES HERE MUST EQUAL `d19m_runScript.py:RUN_DIRS`'s VALUES.  That
  # agreement is DRIVEN by `d19m_run_arm_selftest.sh` leg (r9), which parses both
  # files, rather than being asserted in this comment.
  #
  ...
  # and nothing else.
  # ===========================================================================
  if [ "$MODE" = "O" ] || [ "$MODE" = "XE" ] || [ "$MODE" = "FE" ]; then
    for mp in mp0 mp1 mp2; do
      cp -a "$BASE/MESH" "$WORK/$mp" || { echo "ABORT stage $mp copy from MESH"; exit 4; }
      rm -f "$WORK/$mp/d19m_cmd.sh" "$WORK/$mp/checkMesh.log" \
            "$WORK/$mp/logMeshGeneration.txt" "$WORK/$mp/volumeMesh.xyz" \
            "$WORK/$mp/surfaceMesh.xyz" "$WORK/$mp/.d19m_manifest.json" 2>/dev/null
      test -n "$(ls -d "$WORK/$mp"/processor* 2>/dev/null)" && {
        echo "ABORT G-COLD $mp processor* present"; exit 5; }
      for d in "$WORK/$mp"/*/; do
        n=$(basename "$d")
        case "$n" in 0|0.orig) ;; [0-9]*) echo "ABORT G-COLD $mp time dir present: $n"; exit 5 ;; esac
      done
      { [ -f "$WORK/$mp/0/U" ] || [ -f "$WORK/$mp/0/U.gz" ]; } || {
        echo "ABORT G-COLD $mp carries neither 0/U nor 0/U.gz"; exit 5; }
      test -f "$WORK/$mp/constant/polyMesh/boundary" || {
        echo "ABORT $mp carries no polyMesh/boundary"; exit 5; }
    done
    echo "D19M_MULTIPOINT_STAGED points=3 dirs=[mp0 mp1 mp2] from=$BASE/MESH"
  fi
  export D19M_HERE="$HERE" D19M_INPUTS='["0.orig","constant","system","d19m_runScript.py","d19m_xf.py","d19m_stall.py"]'
  stamp_launch_datum || exit 5
fi
...
for n, w in sorted(want.items()):
    ...
    print('D19M_MESH_IDENTITY %s raw_md5=%s frozen=%s %s' % (n, g, w, 'OK' if g==w else 'MISMATCH'))
    if g != w: bad.append((n,g,w))
if bad:
    sys.stderr.write('D19M_MESH_IDENTITY REFUSE not byte-identical to the D15/D19R mesh: %r\n' % bad)
    sys.exit(6)
print('D19M_MESH_IDENTITY_ALL_OK')
EOF"
case "$ARM" in
  MESH) CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo D19M_CHECKMESH_RC \$?; grep -a 'cells:' checkMesh.log; grep -aq '^ *cells: *$MESH_CELLS\$' checkMesh.log || { echo D19M_CELLCOUNT_REFUSE expected $MESH_CELLS; exit 6; }; $MESH_ASSERT" ;;
  *)    CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d19m_xf.py -mode $MODE" ;;
esac

CMDFILE="$WORK/d19m_cmd.sh"
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/d19m_cmd.sh" > /dev/null 2>&1 \
...
# container when condition A fires: `d19m_stall.N_STALL` (=8) consecutive majors
...
if [ "$MODE" = "O" ]; then
  echo "D19M_STALL_WATCHDOG arm=$ARM armed=yes condition=A(alpha_pr<1e-3 x8) condition_B=NOT_EXERCISED"
else
  echo "D19M_STALL_WATCHDOG arm=$ARM armed=no reason=not_an_optimiser_arm"
fi
CAP_REPORTED=no
while true; do
...
" 2>/dev/null)
    if [ -n "$SR" ] && [ "$(python3 -c "import json;print(1 if json.loads('''$SR''')['fired'] else 0)" 2>/dev/null)" = "1" ]; then
      echo "D19M_STALL_FIRED arm=$ARM condition=A detail=$SR action=STOP core_min=$CM" | tee -a "$BASE/ledger.txt"
      sudo -n docker stop -t 30 "$NAME" >/dev/null 2>&1
      break
    fi
  fi
...
{
  d19m_ledger_row "$ARM" "$ROW" "$IMG" "$GOT_DIGEST" "$rc" "$WALL" "$RANKS" "$CORE_MIN" \
...
if [ "$GOT_SO" != "$WANT_SO" ]; then
  echo "D19M_G9_REFUSE arm=$ARM row=$ROW libidwarp.so md5 printed by the loading process is [$GOT_SO], registered [$WANT_SO]" | tee -a "$LEDGER"
  test "$rc" -eq 0 && rc=7
else
  echo "D19M_G9_OK arm=$ARM libidwarp_so_md5=$GOT_SO digest=$GOT_DIGEST" | tee -a "$LEDGER"
fi
