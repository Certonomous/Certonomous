#!/usr/bin/env bash
# ===========================================================================
# D19M CHAIN DRIVER -- the compressible single-point shape optimisation.
...
# and each row has its OWN final design point.  `XE-S` reads `O-S/d19m_xopt.json`
# and `XE-P` reads `O-P/d19m_xopt.json`; the launcher refuses to stage an
# endpoint arm whose own row's optimum does not exist, and `d19m_xf.py` refuses
...
#   setsid nohup bash d19m_chain_driver.sh MESH O-S XE-S FE-S O-P XE-P FE-P \
...
GRADER="$HERE/d19m_grade.py"
XF="$HERE/d19m_xf.py"
AGE="$HERE/d19m_age_guard.py"
STALL="$HERE/d19m_stall.py"
AGGMEM="$HERE/d19m_aggregate_memory.py"
MARKER="$HERE/d19m_stop_marker.sh"
RUNSCRIPT="$HERE/d19m_runScript.py"
DECOMP="$HERE/d19m_decomposeParDict"
...
# Set ONCE, for ALL of them together, by `d19m_repin.sh`.  A pin table filled in
...
MD5_LAUNCHER=267ec3fccb6058da3e37c92af691198b
MD5_GRADER=f7a4ffd5d85ea0b10644488fca1c828d
MD5_XF=3b029c4ee4032c459b116bcf128d802e
MD5_AGE=b61de171e111712fe106e5d43e8a0258
MD5_STALL=2c37822bcf6107fc368b5743701e3018
MD5_AGGMEM=b6c816491107740c297fc99c2ba2b46d
MD5_MARKER=656266a74eaf786fe32e8516b5c58196
...
test $# -ge 1 || { echo "ABORT usage: d19m_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"
for a in $ARMS; do
  case "$a" in
    MESH|O-S|XE-S|FE-S|O-P|XE-P|FE-P) ;;
    *) echo "ABORT arm '$a' is not a registered D19M arm."
       echo "  Registered: MESH O-S XE-S FE-S O-P XE-P FE-P"
       exit 64 ;;
  esac
done
STATUS="$BASE/STATUS.D19M_chain"; PIDFILE="$BASE/d19m_driver.pid"
cd "$HERE" || exit 4
...
echo "D19M_18_3_EXISTENCE_OK files=9 (asserted BEFORE any md5)"
...
for v in "$MD5_LAUNCHER" "$MD5_GRADER" "$MD5_XF" "$MD5_AGE" "$MD5_STALL" \
  ...
  case "$v" in
    *_UNSET) echo "ABORT a frozen pin is still the fail-closed sentinel ($v)."
             echo "  Run d19m_repin.sh and re-issue PREREGISTRATION.md section 7."
             exit 4 ;;
  esac
done
...
echo "D19M_PINS_OK nine files, existence asserted first, then md5"
...
# ===========================================================================
# D19O-DRIVER-DEF-1, REPAIRED HERE, WHERE IT IS FREE.
# D19O's driver wrote its pre-launch selftest output to `$HERE` -- THE GIT
# WORKING TREE -- leaving four untracked `.selftest.out` files beside the frozen
# instruments on every launch.  It changed no verdict and broke no pin, and it
# was NOT repairable in place because D19O's driver was pinned and the item had
# had first compute.  It is free in a NEW driver, so it is taken here: the output
# goes to the RUN ROOT, where run outputs belong (`FILING_CHARTER.md`: run
# outputs live under the run root, never beside the prose describing them).
#
# THE EVIDENCE IS NOT LOST BY MOVING IT -- that was the reason D19O left its
# files unswept.  It is now IN the run root, beside the arms it gated.
# ===========================================================================
# THE OUTPUT GOES TO A TEMP DIR FIRST AND IS MOVED INTO THE RUN ROOT AFTER
# STAGING, AND THAT DETOUR IS NOT FUSSINESS.  The selftests must run BEFORE
# staging -- that ordering is what stops a broken comparator from buying compute,
# and leg (c8) proves it in the byte offsets of this file.  But `mkdir -p
# "$BASE/selftests"` HERE WOULD CREATE THE RUN ROOT, and the staging block below
# is guarded by `if [ ! -d "$BASE" ]` -- so pre-creating it would silently skip
# staging entirely and every later md5 check would fail on files that were never
# copied.  Caught by reading the guard rather than by running into it.
SELFTEST_TMP=$(mktemp -d /tmp/d19m_selftests_XXXXXX) || { echo "ABORT mktemp"; exit 4; }
SELFTEST_OUT="$SELFTEST_TMP"
for t in "python3 $AGE --selftest" "python3 $STALL --selftest" \
         "python3 $HERE/d19m_xf_selftest.py" "python3 $HERE/d19m_grade_selftest.py"; do
  $t > "$SELFTEST_OUT/$(echo "$t" | md5sum | cut -c1-8).selftest.out" 2>&1 || {
    echo "ABORT an instrument SELFTEST refused: $t"
    echo "  Its controls do not fire, so it grades nothing.  No arm is launched."
    exit 2; }
done
echo "D19M_SELFTESTS_OK age_guard + stall + instrument + comparator"
...
if [ ! -d "$BASE" ]; then
  ...
  echo "ITEM=D19M" > "$BASE/ledger.txt"
...
D19M -- COST REGISTERED BEFORE LAUNCH (CLAUDE.md rule 12)
...
unit=core-minutes (wall_s x ranks / 60)
  ...
  FE-P     1        5.71     20.0
  ITEM             34.10    149.0
band (P_COST): [25.0, 85.0] core-min.
...
EST
  echo "D19M_ROOT_STAGED base=$BASE mode=$(stat -c '%a' "$BASE")"
  echo "D19M_ESTIMATE_REGISTERED 34.10 core-min predicted / 149.0 ceiling"
else
  echo "D19M_ROOT_PRESENT base=$BASE (not re-staged)"
fi

{ echo "$MD5_RUNSCRIPT  $BASE/d19m_runScript.py"; echo "$MD5_XF  $BASE/d19m_xf.py";
  echo "$MD5_STALL  $BASE/d19m_stall.py";
...
# ---- the selftest evidence moves INTO the run root, now that it exists ------
mkdir -p "$BASE/selftests" 2>/dev/null
mv "$SELFTEST_TMP"/*.selftest.out "$BASE/selftests/" 2>/dev/null
rmdir "$SELFTEST_TMP" 2>/dev/null
echo "D19M_SELFTEST_EVIDENCE_IN_RUN_ROOT $BASE/selftests ($(ls "$BASE/selftests" 2>/dev/null | wc -l) files)"
...
echo "D19M_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS]"
...
for ARM in $ARMS; do
  ...
  echo "D19M_H5_WINDOW arm=$ARM n=$N floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN samples_below_floor=$BELOW"
  ...
  echo "D19M_AGGMEM_GO arm=$ARM waited_s=$WAITED series=$(basename "$AGG_SERIES")"

  echo "D19M_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ)"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  EXECUTED=$((EXECUTED+1))
  echo "D19M_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
echo "D19M_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC executed=$EXECUTED/$DECLARED_ARMS"
