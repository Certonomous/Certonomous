#!/usr/bin/env python3
r"""Curriculum D19M -- the AGGREGATE MEMORY reader the chain driver executes.
...
registered aggregate ceiling, given what this box's containers already hold?
  ...
  d19m_aggregate_memory.py <want_gib> <ceiling_gib>
...
def main(argv):
    if len(argv) < 1:
        sys.stderr.write("usage: d19m_aggregate_memory.py <want_gib> [ceiling_gib]\n")
        return 2
    try:
        want = float(argv[0])
        ceiling = float(argv[1]) if len(argv) > 1 else CEILING_DEFAULT_GIB
    except ValueError:
        sys.stderr.write("D19M_AGG REFUSE non-numeric argument %r\n" % (argv,))
        return 2
...
if __name__ == "__main__":
    if "--selftest" in sys.argv:
        ok = True
        print("D19M AGGREGATE MEMORY -- CONTROLS")
        r = main(["12.0", "30.6"])
        ...
        ok = ok and (r4 == 2)
        print("D19M AGGREGATE MEMORY SELFTEST: %s"
