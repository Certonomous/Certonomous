#!/usr/bin/env python3
r"""Curriculum D19M -- the comparator's controls.

THE FIXTURES ARE WRITTEN BY THE REAL PRODUCERS.  Every FD row comes from
`d19m_xf.build_fd_row`, every control row from `d19m_xf.build_ctrl_row`, every
ledger row from `d19m_run_arm.sh`'s OWN `d19m_ledger_row` function, sourced and
...
comparator.  No arm of D19M has run; nothing here is validated against a real
D19M artefact, and that residual is R1 in PREREGISTRATION.md section 15.
"""
import json
import os
import shutil
...
import d19m_grade as G                                            # noqa: E402
...
# ---- the numbers this fixture stands on, re-read at this freeze -------------
# D19R's baseline adjoint (X2/d19r_X.json) for the four graded shape indices,
# used only to give the fixture realistic magnitudes.  The MULTIPOINT adjoint
# does not exist anywhere yet -- this item's own XE arms are the first -- so the
# per-scenario CD/CL arrays below are WRITER-BUILT and are labelled as such.
ADJ_J = {0: -0.007221766503489129, 3: 0.0092905364130168,
         6: -0.014133812719678937, 7: -0.00020994801762558475}
ADJ_CL = {0: 1.0735917332902383, 3: 1.368550326026575,
          6: -0.255135617198591, 7: 0.4888695416188753}
J0, CL0 = 0.0163267546, [0.42, 0.50, 0.58]


def _pair(dJ, dCLs, step, J0=J0, cl0=CL0):
...
def _ledger_row_via_launcher(**kw):
    ...
    r = subprocess.run(
        ["bash", "-c", '. "%s" --source-only; d19m_ledger_row "$@"'
         % os.path.join(HERE, "d19m_run_arm.sh"), "_"] + args,
...
def _alphas_block(bad=False):
    out = []
    for i, sc in enumerate(XF.SCENARIOS):
        a = XF.ALPHAS[i] + (1.0 if (bad and i == 0) else 0.0)
        out.append({"scenario": sc, "registered": XF.ALPHAS[i],
                    "read_dvs": a, "read_scenario": a,
                    "abs_err_dvs": abs(a - XF.ALPHAS[i])})
    return out
...
def build_root(tmp, *, converged=True, majors=12, reduction=6.0,
               ...
               plant_opt_in_endpoint=False, malform=None, alpha_bad=False,
               mp_bad=False):
    """A COMPLETE run root, written by the real writers."""
    arms = arms or list(G.ARMS_DECLARED)
    root = os.path.join(tmp, "CURRICULUM-D19M-a1-naca0012-subsonic-optimisation")
    ...
    for arm in arms:
        ...
        ledger.append("D19M_G9_OK arm=%s libidwarp_so_md5=%s digest=%s"
                      % (arm, so, G.DIGEST[row]))

        # ---- the arm's own log, carrying its terminal statement --------------
        term = ("D19M_MESH_IDENTITY_ALL_OK" if kind == "SCRIPT" else G.TERMINAL[kind])
        ...
        with open(os.path.join(d, ".d19m_manifest.json"), "w") as fh:
        ...
        if kind == "SCRIPT":
            ...
            doc = {"item": "D19M", "mode": "O", "row": row, "nprocs": rk,
                   "producer_md5": XF.PRODUCER_MD5,
                   "alphas": XF.ALPHAS, "weights": XF.WEIGHTS,
                   "scenarios": XF.SCENARIOS,
                   "alphas_read_back": _alphas_block(alpha_bad),
                   "J_baseline": repr(J0), "J_final": repr(J_f),
                   "CD_baseline": [repr(v) for v in CL0],
                   "CD_final": [repr(v * 0.9) for v in CL0],
                   "CL_baseline": [repr(v) for v in CL0],
                   "CL_final": [repr(v) for v in CL0],
                   "weighted_drag_reduction_pct": repr(reduction),
            ...
            with open(os.path.join(d, "d19m_O.json"), "w") as fh:
                json.dump(doc, fh, indent=1, sort_keys=True)
            with open(os.path.join(d, XF.XOPT), "w") as fh:
                json.dump({"item": "D19M", "row": row, "shape": xopt_shape,
                           "J_final": repr(J_f),
                           "CL_final": [repr(v) for v in CL0]}, fh, indent=1, sort_keys=True)
        elif kind == "XE":
            n = 8
            dJ = [ADJ_J.get(i, 1.0e-3 * (i + 1)) for i in range(n)]
            # THE CD ARRAYS ARE BUILT SO THAT SUM_i w_i dCD_i == dJ EXACTLY,
            # because that is what G-MP-STRUCT grades.  `mp_bad` breaks it.
            w = XF.WEIGHTS
            dCD = [[v * (1.0 + (0.0 if j == 0 else 0.0)) for v in dJ] for j in range(3)]
            if mp_bad:
                dCD[0] = [v * 1.5 for v in dCD[0]]
            dCL = [[ADJ_CL.get(i, 1.0 + i) + 0.01 * j for i in range(n)] for j in range(3)]
            with open(os.path.join(d, "d19m_X.json"), "w") as fh:
                json.dump({"item": "D19M", "mode": "XE", "row": row,
                           "alphas": XF.ALPHAS, "weights": XF.WEIGHTS,
                           "scenarios": XF.SCENARIOS,
                           "alphas_read_back": _alphas_block(alpha_bad),
            ...
            for dv, idx in XF.COMPONENTS:
                ...
                for st in list(XF.FD_STEPS_ENDPOINT[dv]) + [XF.TB_STEP]:
                    declared += 2
                    aJ = ADJ_J.get(idx, 1.0e-3 * (idx + 1))
                    aC = [ADJ_CL.get(idx, 1.0 + idx) + 0.01 * j for j in range(3)]
                    if st == XF.TB_STEP:
                        per[st] = _pair(aJ * 40.0, [v * 40.0 for v in aC], st)
                    elif (dv, idx) == ("shape", 7) and not shape7_perfect:
                        real = {1.0e-2: aJ * 1.0002, 1.0e-3: aJ,
                                1.0e-4: aJ * 1.25}       # fine side breaks the plateau
                        per[st] = _pair(real[st], aC, st)
                    else:
                        per[st] = _pair(aJ * 1.0002, [v * 1.0002 for v in aC], st)
                rows_out.append(XF.build_fd_row(dv, idx, per))     # THE REAL WRITER
            rows_out.append(XF.build_ctrl_row(J0, CL0))            # THE REAL WRITER
            with open(os.path.join(d, "d19m_F.json"), "w") as fh:
                json.dump({"item": "D19M", "mode": "FE", "row": row,
                           "alphas": XF.ALPHAS, "weights": XF.WEIGHTS,
                           "scenarios": XF.SCENARIOS,
                           "alphas_read_back": _alphas_block(alpha_bad),
                           "at_design_point": XF.XOPT, "no_optimiser_ran": True,
            ...
            if malform == arm:
                open(os.path.join(d, "d19m_F.json"), "w").write("{not json")
...
def main():
    print("D19M COMPARATOR SELFTEST")
    print("  d19m_grade.py md5 : %s" % G.md5_of(os.path.join(HERE, "d19m_grade.py")))
    print()
    tmp = tempfile.mkdtemp(prefix="d19m_grade_")
    try:
        ...
        print("     shape[7] planted PERFECT: rel=%.6f %% two_sided=%s"
              % (s7["J"]["rel_pct"], s7["J"]["plateau_two_sided"]))
        unit("B1-perfect-number", True, s7["J"]["rel_pct"] < 0.1,
             "(the planted row agrees to better than 0.1 %)")
        unit("B2-plateau-closes", True, s7["J"]["plateau_two_sided"],
             "(and its plateau CLOSES on this planted fixture)")
        unit("B3-STILL-not-a-result", "NOT A RESULT", s7["J"]["verdict"],
        ...
        unit("B6-three-graded", 3, gp["gates"]["G5_fd"]["PATCHED"]["n_graded_components"],
             "(FOUR components, one of which is the registered non-result)")
        unit("B7-published-anyway", True, s7["J"]["rel_pct"] is not None,
             "(its reading is PUBLISHED beside the aggregate, never instead of it)")
        agg = g["gates"]["G5_fd"]["PATCHED"]["aggregate_pct_excl_flagged_J"]
        print("     aggregate_pct_excl_flagged_J (3 components) = %.8f %%" % agg)
        unit("B8-agg-key-named", True,
             "aggregate_pct_excl_flagged_J" in g["gates"]["G5_fd"]["PATCHED"])
        ...
        unit("C7-cl-triple-travels", 3,
             len(g["gates"]["G-OPT9"]["PATCHED"]["CL_final"] or []),
             "(the THREE-CL TRIPLE travels with every weighted-drag number)")
        ...
        # ---- THE TWO GATES THAT ARE NEW IN THE MULTIPOINT --------------------
        print()
        print("E-MP. THE MULTIPOINT GATES -- the two defects only this item can carry")
        unit("E-MP1-alpha-green", "PASS", g["gates"]["G-ALPHA"]["verdict"],
             "(the three operating points read back to %g)" % G.ALPHA_ABS_TOL)
        ra = build_root(os.path.join(tmp, "alpha"), alpha_bad=True)
        ga, _ = _grade(ra)
        unit("E-MP2-alpha-red", "GATE FAIL", ga["gates"]["G-ALPHA"]["verdict"],
             "(point0 wired one WHOLE DEGREE off -- and note EVERY band still "
             "passes, because FD and adjoint are both taken at the same wrong "
             "angle. This is the one defect a multipoint item carries invisibly.)")
        unit("E-MP3-alpha-item", "GATE FAIL", ga["verdict"])
        unit("E-MP4-bands-blind", "PASS", ga["gates"]["G5_fd"]["PATCHED"]["verdict"],
             "(PROOF of the sentence above: the FD gate is GREEN on a wrongly-"
             "wired scenario. G-ALPHA is the only thing that sees it.)")

        unit("E-MP5-mpstruct-green", "PASS",
             g["gates"]["G-MP-STRUCT"]["PATCHED"]["verdict"],
             "(dJ/dx == SUM_i w_i dCD_i/dx to %g)" % G.MP_STRUCT_RTOL)
        rm2 = build_root(os.path.join(tmp, "mpstruct"), mp_bad=True)
        gm2, _ = _grade(rm2)
        unit("E-MP6-mpstruct-red", "GATE FAIL",
             gm2["gates"]["G-MP-STRUCT"]["PATCHED"]["verdict"],
             "(scenario 0's dCD scaled 1.5x -- J no longer reconstructs from its "
             "own components, which is the D6 `geometry_<pt>` defect)")
        ...
        gp_path = os.path.join(HERE, "d19m_grade.py")
        ...
        unit("G2-nine-readers", 9, b["n_readers"])
        ...
        blinds = [
            ...
            ("H3-blind-cells", "read_mesh_cells", lambda *a, **k: None,
             "ctrl_mesh_no_target"),
            # SHAPE-CORRECT BUT EMPTY -- the realistic blindness (a regex that
            # matches nothing returns the right container with nothing in it),
            # not a wrong-shaped return, which would be a programming error.
            ("H4-blind-X", "read_X", lambda *a, **k: {"J": [], "CD": [], "CL": []},
             "ctrl_X_reader_saw_no_J"),
            ("H5-blind-F", "read_F", lambda *a, **k: {},
             "ctrl_F_reader_saw_no_rows"),
        ...
        # R8: the operating-point reader blinded -- G-ALPHA's own hazard.
        real8 = G.read_alphas
        G.read_alphas = lambda *a, **k: []
        try:
            _g8, ref8 = _grade(rblind)
        finally:
            G.read_alphas = real8
        unit("H6b-blind-alphas", "CONTROL", ref8, "(blinded read_alphas)")
        ...
        print()
        if FAILED:
            print("D19M COMPARATOR SELFTEST: FAILED -- %d leg(s): %s" % (len(FAILED), FAILED))
            return 1
        print("D19M COMPARATOR SELFTEST: OK -- every leg driven, every red leg fired")
        print("R1, NAMED: no arm of D19M has run.  Nothing here is validated against a")
        print("real D19M artefact; a green suite is a result about the COMPARATOR.")
        return 0
    finally:
