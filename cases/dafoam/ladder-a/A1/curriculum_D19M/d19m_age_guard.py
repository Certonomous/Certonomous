#!/usr/bin/env python3
r"""Curriculum D19M -- THE REPAIRED AGE DATUM, AND THE CONTROLS THAT PROVE IT CAN
STILL REFUSE.

DERIVED from `curriculum_D19O/d19o_age_guard.py` (md5 RE-COMPUTED at this freeze
and recorded in PREREGISTRATION.md section 7) with EXACTLY these registered
deltas: the item id, the sentinel directory name, the fixture names, and the
graded artefact name (`d19m_F.json`).  NO CLAUSE IS WEAKENED, NO EXCLUSION IS
ADDED, AND NO TOLERANCE IS INTRODUCED.  The seven red legs and two green legs
below are the parent's, unchanged, and they are re-driven at this freeze.
D19O's file was itself derived from `curriculum_D19R/d19r_age_guard.py`, so the
seven red legs have now survived two derivations without a clause moving --
and THAT chain is stated rather than collapsed into a claim of direct descent
from D19R, which would be false.

THE ONE THING D19M ADDS, AND IT IS A REGISTRATION RATHER THAN A CODE CHANGE
===========================================================================
`D19R2` grading attempt 1 returned NOT A RESULT because `check_manifest`
refused arm `X2` with `MANIFEST_ENTRY_MUTATED` on `system/decomposeParDict`:
OpenFOAM's own `decomposePar` appends a `kahipCoeffs` default sub-dictionary
to the file it read, six seconds after the manifest was built.  THE GUARD WAS
RIGHT; the manifest pinned a path the run writes.

**D19M RUNS np = 1 ON EVERY ARM, SO `decomposePar` NEVER RUNS AND THE MUTATION
CANNOT OCCUR.**  That is a MEASUREMENT, not a hope: across D19R's six arms the
mismatch count is 1 on each of the three np=2 arms (`X2`, `S8`, `N2`) and
**0 on both np=1 arms** -- `MESH` (17 entries) and `S1` (27 entries).

SO NO EXCLUSION IS ADDED HERE.  Adding `system/decomposeParDict` to
`SOLVER_WRITE_TARGETS` would be a GATE-DESIGN DECISION, and `docs/LAB_STATE.md`
records that opening the compressible gate that way is reserved to Sanaa.  This
file does not take it.  Instead the pre-registration registers the falsifier:
if this guard refuses on `MANIFEST_ENTRY_MUTATED` for that path, the np = 1
premise is FALSE, the item is `NOT A RESULT`, and the finding is reported -- it
is never a reason to weaken the guard after the fact.
...
L1  THE DATUM LEAVES THE SOLVER'S WRITE SET, STRUCTURALLY.  The launcher stamps a
    ...
    `<parent of run root>/.d19m_datums/<ARM>.sentinel`.  This is ASSERTED, not
...
Usage:  d19m_age_guard.py --selftest
...
def _fixture(root, arm="S1"):
    ...
    _touch(os.path.join(d, "d19m_xf.py"), T, b"instrument\n")
    return d, T
...
INPUT_SUBDIRS = ["0.orig", "constant", "system", "d19m_xf.py"]


def selftest():
    ok = True
    tmp = tempfile.mkdtemp(prefix="d19m_guard_")
    try:
        run_root = os.path.join(tmp, "CURRICULUM-D19M-fixture")
        ...
        print()
        print("D19M PLANTED CONTROL -- RELATIVE, K REGISTERED AT FREEZE")
        band = 10.0                       # G19R-1b's band
        d_ref = -2.0648832854686106e-04   # RE-MEASURED at this freeze from D19R's own
                                  # S8 artefact, rows[shape,7].fd["0.001"].dCD --
                                  # NOT carried over from D19 (-2.065253e-04).
        ...
        print("  d_ref       %.6e   (D19R S8 rows[shape,7].fd[0.001].dCD, re-measured)" % d_ref)
        ...
        print()
        print("D19M AGE GUARD SELFTEST: %s" % ("OK -- every red leg fired, both green legs passed"
                                               if ok else "FAILED"))
