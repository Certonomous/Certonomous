...
# PREREGISTRATION.md section 7 and recorded in so1a_run_arm_DELTAS_from_d15.diff:
...
# `.so1a_age_datum_ref` beside the epoch, so the grader is not left inferring it.
#
...
# ===========================================================================
ITEM=SO1a
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO1a-a1-naca0012-dragmin-gradient
...
# does not apply (so1a_grade.py g_placement: dl_ok is True when ARM_RANKS == 1),
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^so1a_" | tr '\n' ',' | sed 's/,$//'
}
...
#   X-S      1       10.0            600 s              4g
#   X-P      1       10.0            600 s              4g
#   F-S      1       25.0           1500 s              4g
#   F-P      1       25.0           1500 s              4g
cap_core_min() {
  case "$1" in
    MESH)     echo 5.0 ;;
    X-S|X-P)  echo 10.0 ;;
...
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f   # so1a_runScript.py = byte copy of the shipped INCOMPRESSIBLE tutorial runScript.py
MD5_XF=f34bd5bf550786ff926aed81b342cced                                 # so1a_xf.py (set at freeze)
MD5_DECOMP=e6f1b0060944bc86d6dff56480ad2bd4      # so1a_decomposeParDict (numberOfSubdomains 1, scotch)

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: so1a_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: so1a_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^so1a_${ARM}_" 2>/dev/null | grep "^so1a_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/so1a_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/so1a_xf.py"                     | md5sum -c - || { echo "ABORT xf md5"; exit 4; }
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.so1a_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so1a_age_datum_ref"
  ...
  rm -f "$WORK/so1a_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.so1a_age_datum" "$WORK/.so1a_age_datum_ref" 2>/dev/null
  cp -a "$BASE/so1a_runScript.py" "$BASE/so1a_xf.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/so1a_X.json" "$WORK/so1a_F.json" "$WORK/so1a_F.jsonl" "$WORK/so1a_X.jsonl" "$WORK/dRdWColoring_2.bin"; do
  ...
  echo "$AGE_DATUM" > "$WORK/.so1a_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so1a_age_datum_ref"
  echo "SO1A_WRITE_COMPRESSION $(grep -a writeCompression "$WORK/system/controlDict" 2>/dev/null | head -1 | tr -d ';')"
  echo "D4_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM datum_file=$DATUM_NAME resolved_by=EXISTENCE"
fi

case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo SO1A_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so1a_xf.py -mode X" ;;
  F-S|F-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so1a_xf.py -mode F" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/so1a_cmd.sh" > /dev/null 2>&1 \
