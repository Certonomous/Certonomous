...
def build_X_record(nprocs, producer_md5, identity, mp_identity, adjoint,
    ...
    return {"item": ITEM, "mode": "X", "producer_md5": producer_md5, "nprocs": nprocs,
            ...
            # WHICH END OF THE OPTIMISATION THIS NUMBER CAME FROM.  `UNSPECIFIED`
            # is the fail-closed default and the comparator REFUSES on it: a
            # builder that silently defaulted to "BASELINE" would let an endpoint
            # artefact be graded as a baseline one, and section 9's whole point is
            # that those are DIFFERENT VERIFICATIONS.
            "design_point_record": (design_point
                                    or {"design_point": "UNSPECIFIED",
                                        "why": "the producer did not state which "
                                               "design point it evaluated at"}),
            "adjoint": adjoint,
            ...
            "optimiser_note": ("G-NOOPT-ENDPOINT: NO OPTIMISER RUNS IN THIS ARM.  MP-A1 is an "
                               ...
                               "and is what this gate now asserts, is narrower and still checkable: "
...
def build_F_record(nprocs, producer_md5, identity, mp_identity, rows,
    ...
    return {"item": ITEM, "mode": "F", "producer_md5": producer_md5, "nprocs": nprocs,
            ...
            "optimiser": None,
            "optimiser_note": ("G-NOOPT-ENDPOINT: no optimiser runs in THIS arm.  See "
                               "build_X_record's note for why the item-level claim is gone "
                               "and what replaced it; the comparator checks both artefacts")}
...
def load_xopt(path, ident):
    """Read the optimum and REFUSE unless its stamped row is the row THIS
    container's `libidwarp.so` proves.  Reading the other row's optimum would
    compare a patched gradient against a design neither toolchain produced, and
    the row is read from the LIBRARY'S BYTES, never from a flag or a filename --
    `DAFOAM_CHARTER.md` section 11 makes the hash the identity."""
...
def main():
    ...
    # ---- THE DESIGN POINT THIS ARM VERIFIES AT.  Baseline unless `-xopt` names
    # ---- the optimum, in which case EVERY adjoint total and EVERY FD step below
    # ---- is taken at the FINAL design point -- section 9's requirement, and the
    # ---- one thing SO-3aR2 could not supply.  The choice is EMITTED into the
    # ---- artefact, so a reader never has to infer which end of the run a number
    # ---- came from, and the grader refuses to compare two records that disagree
    # ---- about it.
    xopt_path = parse_xopt(sys.argv)
    if xopt_path:
        xrec, xrow = load_xopt(xopt_path, ident)
        ...
        design_point = {"design_point": "FINAL", "xopt_file": xopt_path,
                        "xopt_row": xrow, "xopt_sha256": sha256_of(xopt_path),
                        "xopt_linf": s_(float(np.max(np.abs(xshape)))),
                        "xopt_J_final": xrec.get("J_final")}
    else:
        design_point = {"design_point": "BASELINE", "xopt_file": None,
                        "xopt_row": None, "xopt_sha256": None,
                        "xopt_linf": None, "xopt_J_final": None}
    emit({"kind": "design_point", **design_point})
