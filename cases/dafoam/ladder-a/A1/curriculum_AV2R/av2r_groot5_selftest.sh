#!/usr/bin/env bash
# AV2R G-ROOT.5 SELFTEST -- the av1_groot5_selftest.sh pattern (0b3ebaa4, md5 dbb22dae4266f026d40fb10d386760f3),
# driven against AV2R's OWN launcher (G-ROOT.5 from birth, standing rule of 2026-08-26).
...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; L="$HERE/av2r_run_arm.sh"
...
echo "AV2R G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1)"
...
CN="av2r_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
...
echo "$SPID" > "$BASE/av2r_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; rm -f "$BASE/av2r_driver.pid"
# (b2) STALE pidfile (dead pid) must NOT block
echo "999999" > "$BASE/av2r_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
cp -a "$HERE/av2r_runScript.py" "$HERE/av2r_xf.py" "$BASE/" && cp -a /home/ubuntu/dafoam-tutorials/NACA0012_Airfoil/incompressible/system/decomposeParDict "$BASE/base/system/decomposeParDict"
...
rm -rf "$BASE/base" "$BASE/av2r_runScript.py" "$BASE/av2r_xf.py"
chmod 775 "$BASE"
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^av2r_" && bad "an av2r_ container survives" || ok "no av2r_ container survives"
echo "AV2R G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
