#!/usr/bin/env bash
# Curriculum AV2R arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam) baseline
# gradient, FORWARD-mode AD (ADF) against REVERSE-mode AD (ADR), np = 1, TWO ROWS
# (shipped + patched): the family's first dot-product/duality rung at the total
# level (ADJOINT_VERIFICATION_STANDARD.md section 2, v1.0a).
#
# DERIVED from `cases/dafoam/ladder-a/A1/curriculum_AV1/av1_run_arm.sh` (frozen at
# 0b3ebaa4, md5 9996cfddf5c1f8637a498027cd48385d) with the REGISTERED DELTAS listed
# in PREREGISTRATION.md section 7 and recorded in av2r_run_arm_DELTAS_from_av1.diff:
# item/root names; forbidden roots (+ AV1); arms MESH X-S FAD-S X-P FAD-P, ALL at
# one rank; cpuset 0,1 for every arm; memory 4g; cap table; G-ROW from the arm
# suffix; NO decomposeParDict overlay (np = 1 everywhere); the two-mode instrument
# av2r_xf.py (-mode X reverse totals, -mode FAD forward-mode totals per registered
# component).  No F arm: the reference here is forward-mode AD, not FD.
...
# ===========================================================================
ITEM=AV2R
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-AV2R-a1-naca0012-duality
...
# ---- can be correct and its ledger still be a foreign one moved in.  AV2R
# ---- (inherited from D15 via AV1): this item BUYS BOTH ROWS, so a ROW=SHIPPED row is NOT
...
#
# AV2R REGISTERED DELTA (PREREGISTRATION.md section 5): every arm is one rank on
# cpuset 0,1 (the supervisor's placement for the AV rungs); DISCLOSED co-tenancy
# with AV1's np <= 2 arms on the same pair, absorbed by the aggregate guard.
cpuset_of() {
  case "$1" in
    MESH|X-S|X-P|FAD-S|FAD-P) echo "0,1" ;;
    *) echo "" ;;
  esac
}
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^av2r_" | tr '\n' ',' | sed 's/,$//'
}
...
#   X-S      1       10.0            600 s              4g         0,1
#   FAD-S    1       25.0           1500 s              4g         0,1
#   X-P      1       10.0            600 s              4g         0,1
#   FAD-P    1       25.0           1500 s              4g         0,1
cap_core_min() {
  case "$1" in
    MESH)        echo 5.0 ;;
    X-S|X-P)     echo 10.0 ;;
    FAD-S|FAD-P) echo 25.0 ;;
    *)  echo "" ;;
  esac
}
cap_memory() {
  case "$1" in
    MESH|X-S|X-P|FAD-S|FAD-P) echo 4g ;;
    *) echo "" ;;
  esac
}
ranks_of() {
  case "$1" in
    MESH|X-S|X-P|FAD-S|FAD-P) echo 1 ;;
    *) echo "" ;;
  esac
}
...
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f   # av2r_runScript.py = byte copy of the shipped INCOMPRESSIBLE tutorial runScript.py
MD5_XF=32a755bc9fa84bc0e03ab02bb6ec3c3c                            # av2r_xf.py (set at freeze)
MD5_DECOMP_TUT=816f5ba44075fde47fa5db4269877bc8  # the tutorial's own decomposeParDict, unused at np = 1, asserted so the staged base is the tutorial's

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: av2r_run_arm.sh <MESH|X-S|FAD-S|X-P|FAD-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: av2r_run_arm.sh <MESH|X-S|FAD-S|X-P|FAD-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^av2r_${ARM}_" 2>/dev/null | grep "^av2r_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/av2r_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/av2r_xf.py"                     | md5sum -c - || { echo "ABORT xf instrument md5"; exit 4; }
echo "$MD5_DECOMP_TUT  $BASE/base/system/decomposeParDict" | md5sum -c - || { echo "ABORT base decomposeParDict is not the tutorial's"; exit 4; }
...
# DAFOAM_CHARTER.md §11 makes the HASH the identity.  Inherited from D15/AV1: the
...
# AV2R: every arm is np = 1; no decomposeParDict overlay (DAFoam does not read
# the dict at one rank).
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.av2r_age_datum"
  ...
  rm -f "$WORK/av2r_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.av2r_age_datum" 2>/dev/null
  cp -a "$BASE/av2r_runScript.py" "$BASE/av2r_xf.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/av2r_X.json" "$WORK/av2r_X.jsonl" "$WORK/av2r_X_planted.json" "$WORK/av2r_FAD.json" "$WORK/av2r_FAD.jsonl" "$WORK/av2r_FAD_planted.json" "$WORK/dRdWColoring_1.bin"; do
...
case "$ARM" in
  MESH)        CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo AV2R_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)     CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python av2r_xf.py -mode X" ;;
  FAD-S|FAD-P) CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python av2r_xf.py -mode FAD" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/av2r_cmd.sh" > /dev/null 2>&1 \
