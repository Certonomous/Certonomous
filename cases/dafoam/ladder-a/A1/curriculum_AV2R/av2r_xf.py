#!/usr/bin/env python
"""Curriculum AV2R -- NACA0012 INCOMPRESSIBLE (DASimpleFoam) baseline gradient: FORWARD-mode AD
...
with EXACTLY these registered deltas and no other (av2r_xf_DELTAS_from_av1_x.diff):
  * TWO MODES.  `-mode X` = AV1's X mode unchanged (reverse totals of CD and CL w.r.t.
    `shape` and `patchV` at the baseline, artefact-level planted control, av2r_X.json).
...
OUT_X, OUT_X_PLANTED, JSONL_X = "av2r_X.json", "av2r_X_planted.json", "av2r_X.jsonl"
OUT_F, OUT_F_PLANTED, JSONL_F = "av2r_FAD.json", "av2r_FAD_planted.json", "av2r_FAD.jsonl"
...
def parse_mode(argv):
    ...
    if mode not in ("X", "FAD"):
        sys.stderr.write("AV2R_XF usage: av2r_xf.py -mode X|FAD\n")
        sys.exit(64)
    return mode
...
def plant_and_readback(out, planted_path, orig_path, reader, mutate):
    ...
    if len(a) == 0 or len(a) != len(b):
        sys.stderr.write("AV2R_XF REFUSE planted control: %d vs %d values read back\n" % (len(a), len(b)))
        sys.exit(2)
    worst = max(abs((y - x) - PLANT) for x, y in zip(a, b))
    if worst > 1e-12:
        sys.stderr.write("AV2R_XF REFUSE planted control not seen on read-back: worst residual %r\n" % worst)
        sys.exit(2)
    sys.stdout.write("AV2R_PLANTED_CONTROL_SEEN n=%d worst_residual=%r plant=%r\n" % (len(a), worst, PLANT))
...
def main():
    ...
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("AV2R_XF REFUSE producer md5 %s != frozen %s\n" % (got, PRODUCER_MD5))
        sys.exit(2)
    with open(PRODUCER) as fh:
        src = fh.read()
    if src.count(ANCHOR) != 1:
        sys.stderr.write("AV2R_XF REFUSE anchor %r appears %d times\n" % (ANCHOR, src.count(ANCHOR)))
        sys.exit(2)
    header = src.split(ANCHOR)[0]

    saved_argv = list(sys.argv)
    sys.argv = [PRODUCER, "-task", "run_model", "-optimizer", "IPOPT"]
    ns = {"__name__": "av2r_frozen_header", "__file__": PRODUCER}
    ...
    if mode == "X":
        ...
        if rank == 0:
            ...
            sys.stdout.write("AV2R_X_WRITTEN %s nprocs=%d\n" % (OUT_X, nprocs))
    ...
    if rank == 0:
        ...
        with open(OUT_F, "w") as fh:
            json.dump(out, fh, indent=1, sort_keys=True); fh.flush(); os.fsync(fh.fileno())
        if control_fail:
            sys.stderr.write("AV2R_XF REFUSE forward-channel control: a forward value is 0.0, 1.0 or equal to the function value (silent no-op / seed echo / function echo)\n")
            sys.stdout.write("AV2R_FAD_CONTROL_REFUSED %s\n" % OUT_F)
            sys.exit(2)
        if [r for r in rows if r.get("status") == "MEASURED"]:
            ...
            sys.stdout.write("AV2R_PLANTED_CONTROL_SEEN n=0 note=every row blocked, nothing to plant\n")
        if blocked_any:
            sys.stdout.write("AV2R_FAD_BLOCKED rows_blocked=%d\n" % sum(1 for r in rows if r.get("blocked")))
        sys.stdout.write("AV2R_FAD_WRITTEN %s n_rows=%d blocked_any=%s\n" % (OUT_F, len(rows), blocked_any))
    MPI.COMM_WORLD.Barrier()
