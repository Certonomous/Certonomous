#!/usr/bin/env python3
"""Curriculum AV2R COMPARATOR -- NACA0012 INCOMPRESSIBLE (DASimpleFoam) baseline gradient,
...
shape are AV1's; G-NP is REPLACED by G-DP (av2r_grade_DELTAS_from_av1.diff).

WHAT IT GRADES (PREREGISTRATION.md section 3):
  G1   completion, ARM-KIND AWARE (AV1's form): kernel rc == 0, OOMKilled false, age guard,
       terminal marker (AV2R_X_WRITTEN / AV2R_FAD_WRITTEN) and the instrument's planted-control
       line (AV2R_PLANTED_CONTROL_SEEN) in every solver log; MESH needs `Mesh OK.`.
...
PLANTED CONTROLS (rule 3): (i) AV2R_PLANTED_CONTROL_SEEN required in every solver log; (ii)
...
ARTEFACT = {"MESH": "checkMesh.log", "X-S": "av2r_X.json", "FAD-S": "av2r_FAD.json", "X-P": "av2r_X.json", "FAD-P": "av2r_FAD.json"}
TERMINAL = {"X-S": "AV2R_X_WRITTEN", "FAD-S": "AV2R_FAD_WRITTEN", "X-P": "AV2R_X_WRITTEN", "FAD-P": "AV2R_FAD_WRITTEN"}
CONTROL_LINE = "AV2R_PLANTED_CONTROL_SEEN"
# --- SUCCESSOR DELTA (AV2R): the age-guard datum is resolved BY EXISTENCE, not by name ---
...
def arm_datum(base, arm):
    ...
    p = os.path.join(d, ".av2r_age_datum")
    if not os.path.isfile(p):
...
def grade(root):
    ...
    X = {"S": read_X(os.path.join(root, "X-S", "av2r_X.json")), "P": read_X(os.path.join(root, "X-P", "av2r_X.json"))}
    F = {"S": read_F(os.path.join(root, "FAD-S", "av2r_FAD.json")), "P": read_F(os.path.join(root, "FAD-P", "av2r_FAD.json"))}
    ...
    controls = {"forward_channel_S": forward_channel_control(F["S"], "S"), "forward_channel_P": forward_channel_control(F["P"], "P"),
                "grader_plant_XS": grader_plant_control(root, os.path.join(root, "X-S", "av2r_X.json"), "XS")}
...
def _fix(tmp, tweak=None):
    ...
    for arm in ARMS_REQUIRED:
        ...
        datfile = os.path.join(d, ".av2r_age_datum")
        ...
        if arm == "MESH":
            ...
            if arm.startswith("X"):
                ...
                json.dump({"item": "AV2R", "mode": "X", "identity": ident, "nprocs": 1, "CD_baseline": repr(k["CD"]), "CL_baseline": repr(k["CL"]), "adjoint": adj, "plant": PLANT}, open(art, "w"))
            else:
                rows_ = []
                for dv, idx in COMPONENTS_REGISTERED:
                    rev = J0[dv][idx]
                ...
                json.dump({"item": "AV2R", "mode": "FAD", "identity": ident, "nprocs": 1, "components_requested": COMPONENTS_REGISTERED,
...
def selftest(tmp):
    ...
    unit("U25 ast.Assert count = 0 in av2r_grade.py and av2r_xf.py, and the counter sees a planted assert (=1)",
         count_asserts(os.path.abspath(__file__)) == 0 and count_asserts(os.path.join(here, "av2r_xf.py")) == 0 and count_asserts(p) == 1)
    ...
    print("AV2R GRADER SELFTEST units=%d expected=%d failures=%d python_O=%s" % (n, EXPECTED_UNITS, len(fails), not __debug__))
    if n != EXPECTED_UNITS or fails:
        print("SELFTEST FAIL: %s" % (fails or "unit count %d != %d" % (n, EXPECTED_UNITS)))
        return 2
    print("AV2R GRADER SELFTEST PASS %d/%d (counted against the frozen EXPECTED_UNITS)" % (n, EXPECTED_UNITS))
    return 0
...
def main():
    ...
    if a.selftest:
        d = os.path.join(a.tmpdir, "av2r_selftest_%d" % os.getpid()); os.makedirs(d, exist_ok=True)
        return selftest(d)
    if not a.root:
        print("usage: av2r_grade.py --root <run root> [--out FILE] | --selftest"); return 64
    try:
        r = grade(a.root)
    except Refusal as e:
