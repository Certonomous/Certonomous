#!/usr/bin/env bash
# AV1R chain driver -- DERIVED from curriculum_D15/d15_chain_driver.sh (frozen at
# 8fc2bdeb, md5 4656f1a0e6381b9c03923cd2470d2814) with the REGISTERED DELTAS listed
# in PREREGISTRATION.md section 7 and recorded in av1r_chain_driver_DELTAS_from_d15.diff:
...
#       the ledger opens with an exact `ITEM=AV1R` line and a STAGED line
...
#   setsid nohup bash av1r_chain_driver.sh MESH X1-S X2-S X4-S X1-P X2-P X4-P > <root>/chain_launch.out 2>&1 &
...
LAUNCHER="$HERE/av1r_run_arm.sh"
GRADER="$HERE/av1r_grade.py"
...
MD5_LAUNCHER=4ce916895994ab87b458a3e5fab2de01
MD5_GRADER=b5c1d0092c6d2a2608ad3cc0899ed0fd
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f
MD5_XF=7313bab8b15629c9d872a39d0654aa08
...
test $# -ge 1 || { echo "ABORT usage: av1r_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/av1r_driver.pid"
cd "$HERE" || exit 4
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$HERE/av1r_runScript.py" "$HERE/av1r_x.py" "$HERE/av1r_decomposeParDict_np2" "$HERE/av1r_decomposeParDict_np4" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  ...
  echo "ITEM=AV1R" > "$BASE/ledger.txt"
  ...
  echo "AV1R_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "AV1R_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT  $BASE/av1r_runScript.py"; echo "$MD5_XF  $BASE/av1r_x.py"; echo "$MD5_DECOMP_NP2  $BASE/av1r_decomposeParDict_np2"; echo "$MD5_DECOMP_NP4  $BASE/av1r_decomposeParDict_np4"; echo "$MD5_DECOMP_NP4  $BASE/base/system/decomposeParDict";
...
echo "AV1R_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
for ARM in $ARMS; do
  ...
  echo "AV1R_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW file=$(basename "$H5_FILE")"
  ...
  while true; do
    AGG=$(python3 "$HERE/av1r_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
  ...
  echo "AV1R_AGGREGATE arm=$ARM waited=$WAITED $AGG"
  echo "AV1R_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "AV1R_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
if echo "$MD5_GRADER  $GRADER" | md5sum -c - > /dev/null; then
  python3 "$GRADER" --root "$BASE" --out "$BASE/AV1R_grade_${GSTAMP}.json" > "$BASE/AV1R_grade_${GSTAMP}.out" 2>&1; GRC=$?
  echo "grader_rc=$GRC stamp=$GSTAMP out=AV1R_grade_${GSTAMP}.json note=comparator-exit-status-NOT-the-verdict" >> "$STATUS"
else
  echo "grader_rc=NOT_RUN stamp=$GSTAMP note=grader-md5-drifted-at-chain-end" >> "$STATUS"
fi
echo "AV1R_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC"
exit "$CHAIN_RC"
