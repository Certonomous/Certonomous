#!/usr/bin/env bash
# AV1R G-ROOT.5 SELFTEST -- the d15_groot5_selftest.sh pattern (8fc2bdeb, md5 8a2a7df64b2db87615a20fab2d9208f8),
# driven against AV1R's OWN launcher (G-ROOT.5 from birth, standing rule of 2026-08-26).
...
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"; L="$HERE/av1r_run_arm.sh"
...
echo "AV1R G-ROOT.5 SELFTEST $(date -u +%Y-%m-%dT%H:%M:%SZ) launcher_md5=$(md5sum "$L" | cut -d' ' -f1)"
...
CN="av1r_${ARM}_selftest_$(date -u +%Y%m%dT%H%M%SZ)"
...
echo "$SPID" > "$BASE/av1r_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; rm -f "$BASE/av1r_driver.pid"
# (b2) STALE pidfile (dead pid) must NOT block
echo "999999" > "$BASE/av1r_driver.pid"
out=$(bash "$L" $ARM "$BOGUS" 2>&1); rc=$?
...
cp -a "$HERE/av1r_runScript.py" "$HERE/av1r_x.py" "$HERE/av1r_decomposeParDict_np2" "$HERE/av1r_decomposeParDict_np4" "$BASE/" && cp -a "$HERE/av1r_decomposeParDict_np4" "$BASE/base/system/decomposeParDict"
...
rm -rf "$BASE/base" "$BASE/av1r_runScript.py" "$BASE/av1r_x.py" "$BASE/av1r_decomposeParDict_np2" "$BASE/av1r_decomposeParDict_np4"
chmod 775 "$BASE"
...
sudo -n docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^av1r_" && bad "an av1r_ container survives" || ok "no av1r_ container survives"
echo "AV1R G-ROOT5 SELFTEST pass=$PASS fail=$FAIL $(date -u +%Y-%m-%dT%H:%M:%SZ)"
[ "$FAIL" -eq 0 ]
