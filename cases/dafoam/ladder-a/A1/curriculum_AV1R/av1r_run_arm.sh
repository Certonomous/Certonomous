#!/usr/bin/env bash
# Curriculum AV1R arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam) baseline
...
# in PREREGISTRATION.md section 7 and recorded in av1r_run_arm_DELTAS_from_d15.diff:
...
# X-only instrument av1r_x.py.  No F arm: this rung grades the adjoint against
...
# ===========================================================================
ITEM=AV1R
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-AV1R-a1-naca0012-npinv
...
# ---- can be correct and its ledger still be a foreign one moved in.  AV1R
...
#
# AV1R REGISTERED DELTA (PREREGISTRATION.md section 5): cpuset PER ARM -- 0,1 for
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^av1r_" | tr '\n' ',' | sed 's/,$//'
}
...
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f   # av1r_runScript.py = byte copy of the shipped INCOMPRESSIBLE tutorial runScript.py
MD5_XF=7313bab8b15629c9d872a39d0654aa08                             # av1r_x.py (set at freeze)
MD5_DECOMP_NP2=68ecc827562886fb43c3aedb0627b344  # av1r_decomposeParDict_np2 (numberOfSubdomains 2, scotch) = D15's dict
MD5_DECOMP_NP4=816f5ba44075fde47fa5db4269877bc8  # av1r_decomposeParDict_np4 (numberOfSubdomains 4, scotch) = the tutorial's own

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: av1r_run_arm.sh <MESH|X1-S|X2-S|X4-S|X1-P|X2-P|X4-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: av1r_run_arm.sh <MESH|X1-S|X2-S|X4-S|X1-P|X2-P|X4-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^av1r_${ARM}_" 2>/dev/null | grep "^av1r_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/av1r_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/av1r_x.py"                      | md5sum -c - || { echo "ABORT x instrument md5"; exit 4; }
echo "$MD5_DECOMP_NP2  $BASE/av1r_decomposeParDict_np2" | md5sum -c - || { echo "ABORT decomposeParDict_np2 md5"; exit 4; }
echo "$MD5_DECOMP_NP4  $BASE/av1r_decomposeParDict_np4" | md5sum -c - || { echo "ABORT decomposeParDict_np4 md5"; exit 4; }
...
# AV1R REGISTERED DELTA: the np = 2 / np = 4 arms overlay their REGISTERED
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.av1r_age_datum"
  ...
  rm -f "$WORK/av1r_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.av1r_age_datum" 2>/dev/null
  cp -a "$BASE/av1r_runScript.py" "$BASE/av1r_x.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  case "$ARM" in
    X2-*) cp -a "$BASE/av1r_decomposeParDict_np2" "$WORK/system/decomposeParDict" || { echo "ABORT overlay np2 dict"; exit 4; }
          echo "$MD5_DECOMP_NP2  $WORK/system/decomposeParDict" | md5sum -c - || { echo "ABORT overlaid np2 dict md5"; exit 4; } ;;
    X4-*) cp -a "$BASE/av1r_decomposeParDict_np4" "$WORK/system/decomposeParDict" || { echo "ABORT overlay np4 dict"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/av1r_X.json" "$WORK/av1r_X.jsonl" "$WORK/av1r_X_planted.json" "$WORK/dRdWColoring_1.bin" "$WORK/dRdWColoring_2.bin" "$WORK/dRdWColoring_4.bin"; do
...
case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo AV1R_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X1-S|X1-P|X2-S|X2-P|X4-S|X4-P) CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python av1r_x.py -mode X" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/av1r_cmd.sh" > /dev/null 2>&1 \
