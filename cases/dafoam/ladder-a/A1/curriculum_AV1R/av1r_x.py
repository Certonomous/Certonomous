#!/usr/bin/env python
"""Curriculum AV1R -- NACA0012 INCOMPRESSIBLE (DASimpleFoam) baseline gradient at np = 1 / 2 / 4.

DERIVED FROM `curriculum_D15/d15_xf.py` (8fc2bdeb, md5 8a664a976de77c5fe7c3fed5746be4de)
with EXACTLY these registered deltas and no other (av1r_x_DELTAS_from_d15_xf.diff):
  * PRODUCER is `av1r_runScript.py` -- a BYTE COPY of the shipped INCOMPRESSIBLE tutorial
  ...
  * ONE MODE, `-mode X`: the reverse-AD totals (compute_totals) of CD AND CL w.r.t.
    ...
    written to `av1r_X.json`.  The F mode (central FD) is REMOVED: this rung grades the
  ...
  * PLANTED CONTROL (CLAUDE.md rule 3), at the ARTEFACT level because this rung has no
    FD table to carry a CTRL component: the instrument writes `av1r_X.json`, then writes
    `av1r_X_planted.json` = the same artefact with PLANT = 1.234e-03 ADDED to every total,
...
def parse_mode(argv):
    ...
    for i, a in enumerate(argv):
        if a == "-mode" and i + 1 < len(argv):
            mode = argv[i + 1]
    if mode != "X":
        sys.stderr.write("AV1R_X usage: av1r_x.py -mode X\n")
        sys.exit(64)
    return mode
...
def main():
    ...
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("AV1R_X REFUSE producer md5 %s != frozen %s\n" % (got, PRODUCER_MD5))
        sys.exit(2)

    with open(PRODUCER) as fh:
        src = fh.read()
    if src.count(ANCHOR) != 1:
        sys.stderr.write("AV1R_X REFUSE anchor %r appears %d times\n" % (ANCHOR, src.count(ANCHOR)))
        sys.exit(2)
    header = src.split(ANCHOR)[0]

    saved_argv = list(sys.argv)
    sys.argv = [PRODUCER, "-task", "run_model", "-optimizer", "IPOPT"]
    ns = {"__name__": "av1r_frozen_header", "__file__": PRODUCER}
    ...
    if rank == 0:
        ...
        if len(a) == 0 or len(a) != len(b):
            sys.stderr.write("AV1R_X REFUSE planted control: %d vs %d values read back\n" % (len(a), len(b)))
            sys.exit(2)
        worst = max(abs((y - x) - PLANT) for x, y in zip(a, b))
        if worst > 1e-12:
            sys.stderr.write("AV1R_X REFUSE planted control not seen on read-back: worst residual %r\n" % worst)
            sys.exit(2)
        sys.stdout.write("AV1R_PLANTED_CONTROL_SEEN n=%d worst_residual=%r plant=%r\n" % (len(a), worst, PLANT))
        sys.stdout.write("AV1R_X_WRITTEN %s nprocs=%d\n" % (OUT_X, nprocs))
