#!/usr/bin/env bash
# Curriculum SO-2a arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam)
...
# PREREGISTRATION.md section 7 and recorded in so2a_run_arm_DELTAS_from_d15.diff:
...
# `.so2a_age_datum_ref` beside the epoch, so the grader is not left inferring it.
#
...
# `so2a_grade.py:_ledger_rows_via_launcher` runs
#     bash -c '. so2a_run_arm.sh --source-only; so2a_ledger_row ...'
...
# ===========================================================================
ITEM=SO2a
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO2a-a1-naca0012-geometric-constraint-gradient
...
#
# SO-2a REGISTERED DELTA (PREREGISTRATION.md section 5): cpuset 9, ONE core,
...
# does not apply (so2a_grade.py g_placement: dl_ok is True when ARM_RANKS == 1),
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^so2a_" | tr '\n' ',' | sed 's/,$//'
}
...
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f   # so2a_runScript.py = byte copy of the shipped INCOMPRESSIBLE tutorial runScript.py
MD5_XG=85b2a416e75141096967fe7e13b02e28      # so2a_xg.py (set at freeze)
MD5_DECOMP=e6f1b0060944bc86d6dff56480ad2bd4      # so2a_decomposeParDict (numberOfSubdomains 1, scotch)

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: so2a_run_arm.sh <MESH|X-S|G-S|X-P|G-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: so2a_run_arm.sh <MESH|X-S|G-S|X-P|G-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^so2a_${ARM}_" 2>/dev/null | grep "^so2a_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/so2a_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XG  $BASE/so2a_xg.py"                     | md5sum -c - || { echo "ABORT so2a_xg.py md5"; exit 4; }
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.so2a_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so2a_age_datum_ref"
  ...
  rm -f "$WORK/so2a_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.so2a_age_datum" "$WORK/.so2a_age_datum_ref" 2>/dev/null
  cp -a "$BASE/so2a_runScript.py" "$BASE/so2a_xg.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/so2a_X.json" "$WORK/so2a_F.json" "$WORK/so2a_F.jsonl" "$WORK/so2a_X.jsonl" "$WORK/dRdWColoring_2.bin"; do
  ...
  echo "$AGE_DATUM" > "$WORK/.so2a_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so2a_age_datum_ref"
  echo "SO2A_WRITE_COMPRESSION $(grep -a writeCompression "$WORK/system/controlDict" 2>/dev/null | head -1 | tr -d ';')"
  echo "D4_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM datum_file=$DATUM_NAME resolved_by=EXISTENCE"
fi

case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo SO2A_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so2a_xg.py -mode X" ;;
  G-S|G-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so2a_xg.py -mode F" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/so2a_cmd.sh" > /dev/null 2>&1 \
...
{
  so2a_ledger_row "$ARM" "$ROW" "$IMG" "$GOT_DIGEST" "$rc" "$WALL" "$RANKS" "$CORE_MIN" "$CAP" "$TMO" "$BACKCHECK" "$MEM" "$INSPECT" "$MEMAVAIL_GIB" "$MEMAVAIL_POST" "$CPUSET" "$DELIVERED" "$SIBLINGS_PRE" "$SIBLINGS_POST" "$(basename "$LOG")" "$STAMP"
