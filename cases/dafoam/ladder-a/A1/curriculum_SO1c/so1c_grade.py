#!/usr/bin/env python3
"""Curriculum SO-1c COMPARATOR -- NACA0012 INCOMPRESSIBLE drag-min-at-fixed-lift,
...
`so1c_grade_DELTAS_from_so1a_grade.diff`.  SO-1a's machinery is inherited
...
A MEASURED DEFECT:
  ...
  (D) G-MESHID -- "THE SAME MESH" IS ASSERTED, NOT ASSUMED.  The standard defines
      ...
      and image".  SO-1c regenerates its own mesh rather than reading one across
...
REGISTERED_NPROCS = 4
ARTEFACT = {"MESH": "checkMesh.log", "Ns-P": "so1c_N.json", "Ni-P": "so1c_N.json",
            "Ns-S": "so1c_N.json", "Ni-S": "so1c_N.json"}
...
STAGED_INPUTS = ("so1b_E.json", "so1c_runScript.py", "so1c_xn.py",
                 os.path.join("system", "decomposeParDict"))
TERMINAL = {"Ns-P": "SO1C_N_WRITTEN", "Ni-P": "SO1C_N_WRITTEN",
            "Ns-S": "SO1C_N_WRITTEN", "Ni-S": "SO1C_N_WRITTEN"}
DATUM_CANDIDATES = {"MESH": ("0.orig/U", "0.orig/U.gz"), "Ns-P": ("0/U", "0/U.gz"),
                    "Ni-P": ("0/U", "0/U.gz"), "Ns-S": ("0/U", "0/U.gz"),
                    "Ni-S": ("0/U", "0/U.gz")}
DATUM_FILE = ".so1c_age_datum"
...
EXPECTED_UNITS = 65          # 53 at the v1.0 freeze; +12 by AMENDMENT R6, 2026-08-28,
...
def g_xstar(root, meta, arm):
    ...
    if ref.get("row") != ARM_ROW[arm]:
        refuse("G-XSTAR", {"arm": arm, "staged_artefact_row": ref.get("row"),
                           "arm_row": ARM_ROW[arm],
                           "note": "a row is an image hash, never a directory name"})
...
def g_meshid(root):
    ...
    # `so1c_chain_driver.sh`'s `grep ... MESH_*.log | head -4`, effectively a
...
def _fix(tmp, tweak=None):
    ...
    for rowname in ("PATCHED", "SHIPPED"):
        ...
        json.dump({"item": "SO1b", "mode": "E", "row": rowname,
                   "nprocs": k["ref_np"],
    ...
    for arm in ARMS_REQUIRED:
        ...
        if arm == "MESH":
            ...
            json.dump({"item": "SO1c", "mode": "N", "row": ARM_ROW[arm], "identity": ident,
            ...
            text += "SO1C_DECOMP method=%s n_subdomains=%s\n" % (k["decomp"][arm], k["nprocs"][arm])
        if arm in k["fatal"]:
            text += k["fatal"][arm] + "\n"
...
def selftest(tmp):
    ...
    unit("U49 ast.Assert count = 0 in so1c_grade.py and so1c_xn.py (L-332)",
         count_asserts(os.path.abspath(__file__)) == 0
         and count_asserts(os.path.join(here, "so1c_xn.py")) == 0)
    ...
    print("\nSO1c grader selftest: %d units, %d fail" % (n, len(fails)))
    for f in fails:
        print("  FAIL: %s" % f)
    if n != EXPECTED_UNITS:
...
def main():
    ...
    if abs(sum(CAPS.values()) - ITEM_CEILING_CORE_MIN) > 1e-9:
        ...
        return 2
    if a.selftest:
        d = os.path.join(a.tmpdir, "so1c_selftest_%d" % os.getpid())
        os.makedirs(d, exist_ok=True)
        return selftest(d)
    if not a.root:
        print("usage: so1c_grade.py --root <run root> [--out FILE] | --selftest")
        return 64
    try:
        r = grade(a.root)
    except Refusal as e:
