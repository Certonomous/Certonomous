#!/usr/bin/env bash
# Curriculum SO-1c arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam),
...
# so1c_run_arm_DELTAS_from_so1a.diff:
...
# below, reproduced verbatim in PREREGISTRATION.md line 6's SO1C-CAP-MANIFEST,
...
# ===========================================================================
ITEM=SO1c
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO1c-a1-naca0012-dragmin-npinv
...
#
# SO-1c REGISTERED DELTA (PREREGISTRATION.md section 5b): THE PLACEMENT IS
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^so1c_" | tr '\n' ',' | sed 's/,$//'
}
...
#   MESH     1        5.0            300 s              4g
#   Ns-P     4       30.0            450 s              8g
#   Ni-P     4       30.0            450 s              8g
#   Ns-S     4       30.0            450 s              8g
#   Ni-S     4       30.0            450 s              8g
...
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f   # so1c_runScript.py = byte copy of the shipped INCOMPRESSIBLE tutorial runScript.py
MD5_XN=63d13c88fb915ab7695d6f1a9383a4ed                                # so1c_xn.py (set at freeze)
MD5_DECOMP_SCOTCH=816f5ba44075fde47fa5db4269877bc8   # so1c_decomposeParDict_scotch (numberOfSubdomains 4, method scotch)
MD5_DECOMP_SIMPLE=194c330803077f0ffa4341f468c09768   # so1c_decomposeParDict_simple (numberOfSubdomains 4, method simple, n (4 1 1))

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: so1c_run_arm.sh <MESH|Ns-P|Ni-P|Ns-S|Ni-S> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: so1c_run_arm.sh <MESH|Ns-P|Ni-P|Ns-S|Ni-S> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^so1c_${ARM}_" 2>/dev/null | grep "^so1c_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
#   (a) the SO1C-CAP-MANIFEST line in the document ON DISK must name exactly the
...
# ===========================================================================
PREREG="${PREREG:-/home/ubuntu/Certonomous/cases/dafoam/ladder-a/A1/curriculum_SO1c/PREREGISTRATION.md}"
PREREG_REL=cases/dafoam/ladder-a/A1/curriculum_SO1c/PREREGISTRATION.md
test -f "$PREREG" || { echo "ABORT G-CAP-PREREG the frozen pre-registration is absent: $PREREG"; exit 65; }
MAN_DISK=$(grep -a -o 'SO1C-CAP-MANIFEST v1 [^>]*' "$PREREG" | head -1 | sed 's/ *-->$//;s/ *$//')
test -n "$MAN_DISK" || { echo "ABORT G-CAP-PREREG no SO1C-CAP-MANIFEST line in $PREREG"; exit 65; }
MAN_HEAD=$(git show "HEAD:$PREREG_REL" 2>/dev/null | grep -a -o 'SO1C-CAP-MANIFEST v1 [^>]*' | head -1 | sed 's/ *-->$//;s/ *$//')
if [ -z "$MAN_HEAD" ]; then
...
echo "SO1C_G_CAP_PREREG_PASS arm=$ARM prereg=$PREREG head_channel=$MAN_HEAD_STATE sum_of_caps=$CAP_SUM ceiling=$CEIL_REG ranks=$RANKS manifest=[$MAN_DISK]"
...
echo "$MD5_RUNSCRIPT  $BASE/so1c_runScript.py"                    | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XN  $BASE/so1c_xn.py"                                 | md5sum -c - || { echo "ABORT xn md5"; exit 4; }
echo "$MD5_DECOMP_SCOTCH  $BASE/so1c_decomposeParDict_scotch"    | md5sum -c - || { echo "ABORT decomposeParDict_scotch md5"; exit 4; }
echo "$MD5_DECOMP_SIMPLE  $BASE/so1c_decomposeParDict_simple"    | md5sum -c - || { echo "ABORT decomposeParDict_simple md5"; exit 4; }
# and the dictionary THIS arm will actually run under, resolved from the arm name
case "$DECOMP" in
  scotch) DECOMP_SRC="$BASE/so1c_decomposeParDict_scotch" ;;
  simple) DECOMP_SRC="$BASE/so1c_decomposeParDict_simple" ;;
  *) echo "ABORT arm $ARM resolved to an unregistered decomposition [$DECOMP]"; exit 4 ;;
esac
echo "SO1C_DECOMP_SELECTED arm=$ARM decomposition=$DECOMP source=$DECOMP_SRC md5=$(md5sum "$DECOMP_SRC" | cut -d\  -f1)"
...
if [ "$ARM" != "MESH" ]; then
  ...
  test -f "$OPTREF" || {
    ...
    echo "  SO-1c verifies the gradient AT SO-1b's OPTIMUM; with no optimum there"
...
if d['row']!='$ROW': sys.stderr.write('G-OPTDEP artefact row %r != this arm row %r\n'%(d['row'],'$ROW')); sys.exit(1)
" || { echo "ABORT G-OPTDEP the optimum artefact for row $ROW is unparseable or is another row's. REFUSED."; exit 5; }
  echo "SO1C_G_OPTDEP_PASS arm=$ARM row=$ROW optimum=$OPTREF"
fi

STAMP=$(date -u +%Y%m%dT%H%M%SZ)_$$
NAME="so1c_${ARM}_${STAMP}"
WORK="$BASE/$ARM"
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.so1c_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so1c_age_datum_ref"
  ...
  rm -f "$WORK/so1c_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.so1c_age_datum" "$WORK/.so1c_age_datum_ref" "$WORK/so1c_N.json" "$WORK/so1c_N.jsonl" "$WORK/so1b_E.json" 2>/dev/null
  cp -a "$BASE/so1c_runScript.py" "$BASE/so1c_xn.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  echo "SO1C_STAGED_INPUT arm=$ARM file=so1b_E.json row=$ROW md5=$(md5sum "$WORK/so1b_E.json" | cut -d\  -f1) mtime=$(stat -c %Y "$WORK/so1b_E.json") class=STAGED_INPUT_NOT_A_PRODUCT"
  ...
  for bad in "$WORK/reports" "$WORK/so1c_N.json" "$WORK/so1c_N.jsonl" "$WORK/dRdWColoring_2.bin"; do
  ...
  echo "$AGE_DATUM" > "$WORK/.so1c_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so1c_age_datum_ref"
  echo "SO1C_WRITE_COMPRESSION $(grep -a writeCompression "$WORK/system/controlDict" 2>/dev/null | head -1 | tr -d ';')"
  echo "D4_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM datum_file=$DATUM_NAME resolved_by=EXISTENCE"
fi

case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo SO1C_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  Ns-P|Ni-P|Ns-S|Ni-S)
            CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so1c_xn.py -mode N -optimum so1b_E.json" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/so1c_cmd.sh" > /dev/null 2>&1 \
...
echo "SO1C_G_CLOCK arm=$ARM host_wall_s=$WALL container_wall_s=$CWALL ranks=$RANKS host_core_min=$CORE_MIN container_core_min=$CORE_MIN_CONTAINER clock_frame_delta_core_min=$CLOCK_DELTA cap_core_min=$CAP enforced_in_container_wall_s=$TMO gate_frame=CONTAINER cost_frame=HOST"
...
LEDGER="$BASE/ledger.txt"
{
  ...
  grep -a "D4S_CONTAINER_UID\|D4S_IDWARP_SO_MD5\|D4S_DEADLINE_IN_CONTAINER_S\|SO1C_DECOMP " "$LOG" | head -4   # the strings the container prints
} | tee -a "$LEDGER"
