#!/usr/bin/env python
"""Curriculum SO-1c -- NACA0012 INCOMPRESSIBLE drag-min-at-fixed-lift, the
...
DERIVED FROM `curriculum_SO1a/so1a_xf.py` (md5 pinned in so1c_chain_driver.sh)
with EXACTLY these registered deltas and no other
(so1c_xn_DELTAS_from_so1a_xf.diff):

  * ONE MODE `N` INSTEAD OF TWO.  SO-1a splits X (adjoint) and F (FD) into two
    arms because they answer two questions at one design point.  SO-1c must
  ...
  * THE DESIGN POINT IS SO-1b's OPTIMUM, READ FROM DISK, NEVER RE-DERIVED.
    ...
    SO-1b's chain reached and completed the arm that produced what SO-1c reads.
  ...
  * Printed tokens SO1A_* -> SO1C_*; artefact names so1a_* -> so1c_*.
...
def parse_args(argv):
    ...
    if mode != "N" or not opt:
        sys.stderr.write("SO1C_XN usage: so1c_xn.py -mode N -optimum <path to SO-1b so1b_E.json>\n")
        sys.exit(64)
    return mode, opt
...
def load_optimum(path, ident):
    ...
    try:
        ...
        sys.stderr.write("SO1C_XN REFUSE optimum artefact unreadable %s: %r\n"
                         % (path, exc))
        sys.exit(3)
    for key in ("design_point", "adjoint", "identity", "nprocs", "row"):
        if key not in src:
            sys.stderr.write("SO1C_XN REFUSE optimum artefact lacks %r: %s\n" % (key, path))
            sys.exit(3)
    if int(src["nprocs"]) != 1:
        sys.stderr.write("SO1C_XN REFUSE the comparison basis must be the np = 1 gradient; "
    ...
    if not want or not got or want != got:
        sys.stderr.write("SO1C_XN REFUSE G-ROWX: this process libidwarp md5 %r != the optimum "
...
def main():
    ...
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("SO1C_XN REFUSE producer md5 %s != frozen %s\n"
                         % (got, PRODUCER_MD5))
        sys.exit(2)
    ...
    if src.count(ANCHOR) != 1:
        sys.stderr.write("SO1C_XN REFUSE anchor %r appears %d times\n"
    ...
    ns = {"__name__": "so1c_frozen_header", "__file__": PRODUCER}
    ...
    if nprocs != REGISTERED_NPROCS:
        sys.stderr.write("SO1C_XN REFUSE G-NP: MPI reports nprocs=%d, registered %d\n"
                         % (nprocs, REGISTERED_NPROCS))
        sys.exit(4)
    if decomp.get("decomp_n_subdomains") != REGISTERED_NPROCS:
        sys.stderr.write("SO1C_XN REFUSE G-NP: decomposeParDict numberOfSubdomains=%r, "
    ...
    if not decomp.get("decomp_method"):
        sys.stderr.write("SO1C_XN REFUSE G-DECOMP: no decomposition method could be read "
    ...
    for k in ("shape", "patchV"):
        ...
        if len(vals) != defaults[k].size:
            sys.stderr.write("SO1C_XN REFUSE x* has %d %s components, this problem has %d\n"
    ...
    if rank == 0:
        ...
        if seen_zero != 0.0 or seen_plant is None or abs(seen_plant - want) > 1e-12 * abs(want):
            sys.stderr.write("SO1C_XN REFUSE planted-zero control not seen on read-back: "
                             "zero=%r plant=%r want=%r\n" % (seen_zero, seen_plant, want))
            sys.exit(2)
        sys.stdout.write("SO1C_PLANTED_ZERO_CONTROL_SEEN zero=%r plant=%r\n"
        ...
        with open(OUT_N, "w") as fh:
            ...
            fh.flush()
            os.fsync(fh.fileno())
        sys.stdout.write("SO1C_DECOMP method=%s n_subdomains=%s simple_n=%s\n"
                         % (decomp.get("decomp_method"),
                            decomp.get("decomp_n_subdomains"),
                            decomp.get("decomp_simple_n")))
        sys.stdout.write("SO1C_N_WRITTEN %s n_rows=%d\n" % (OUT_N, len(rows)))
