#!/usr/bin/env bash
# Curriculum SO-1b arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam)
...
# PREREGISTRATION.md section 7 and recorded in so1b_run_arm_DELTAS_from_d15.diff:
...
# `.so1b_age_datum_ref` beside the epoch, so the grader is not left inferring it.
#
...
# ===========================================================================
ITEM=SO1b
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO1b-a1-naca0012-dragmin-opt
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^so1b_" | tr '\n' ',' | sed 's/,$//'
}
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: so1b_run_arm.sh <MESH|O-P|E-P|O-S|E-S> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: so1b_run_arm.sh <MESH|O-P|E-P|O-S|E-S> <image>"; exit 64; }
...
# ===========================================================================
PREREG=/home/ubuntu/Certonomous/cases/dafoam/ladder-a/A1/curriculum_SO1b/PREREGISTRATION.md
REPO=/home/ubuntu/Certonomous
MANIFEST_TAG="SO1B-CAP-MANIFEST v1"
...
MAN_HEAD=$(git -C "$REPO" show "HEAD:cases/dafoam/ladder-a/A1/curriculum_SO1b/PREREGISTRATION.md" 2>/dev/null | grep -a -m1 -F "$MANIFEST_TAG" || true)
if [ -z "$MAN_HEAD" ]; then
...
echo "SO1B_G_CAP_PREREG_PASS arm=$ARM prereg=$PREREG head_channel=$MAN_HEAD_STATE sum_of_caps=$CAP_SUM ceiling=$CEIL_REG manifest=[$MAN_DISK]"
...
if [ -n "$NEEDO" ]; then
  ...
  echo "SO1B_G_EDEP_PASS arm=$ARM optimum_artefact=$NEEDO md5=$(md5sum "$NEEDO" | cut -d' ' -f1)"
fi
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^so1b_${ARM}_" 2>/dev/null | grep "^so1b_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.so1b_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so1b_age_datum_ref"
  echo "SO1B_WRITE_COMPRESSION $(grep -a writeCompression "$WORK/system/controlDict" 2>/dev/null | head -1 | tr -d ';')"
  echo "D4_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM datum_file=$DATUM_NAME resolved_by=EXISTENCE"
fi
