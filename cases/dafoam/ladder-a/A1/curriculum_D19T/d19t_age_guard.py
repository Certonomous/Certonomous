#!/usr/bin/env python3
r"""Curriculum D19T -- THE REPAIRED AGE DATUM, AND THE CONTROLS THAT PROVE IT CAN
STILL REFUSE.
...
graded artefact name (`d19t_F.json`).  NO CLAUSE IS WEAKENED, NO EXCLUSION IS
...
THE ONE THING D19T ADDS, AND IT IS A REGISTRATION RATHER THAN A CODE CHANGE
===========================================================================
...
**D19T RUNS np = 1 ON EVERY ARM, SO `decomposePar` NEVER RUNS AND THE MUTATION
...
L1  THE DATUM LEAVES THE SOLVER'S WRITE SET, STRUCTURALLY.  The launcher stamps a
    ...
    `<parent of run root>/.d19t_datums/<ARM>.sentinel`.  This is ASSERTED, not
...
Usage:  d19t_age_guard.py --selftest
...
def _fixture(root, arm="S1"):
    ...
    _touch(os.path.join(d, "d19t_xf.py"), T, b"instrument\n")
    return d, T
...
INPUT_SUBDIRS = ["0.orig", "constant", "system", "d19t_xf.py"]


def selftest():
    ok = True
    tmp = tempfile.mkdtemp(prefix="d19t_guard_")
    try:
        run_root = os.path.join(tmp, "CURRICULUM-D19T-fixture")
        ...
        print()
        print("D19T PLANTED CONTROL -- RELATIVE, K REGISTERED AT FREEZE")
        ...
        print()
        print("D19T AGE GUARD SELFTEST: %s" % ("OK -- every red leg fired, both green legs passed"
                                               if ok else "FAILED"))
