#!/usr/bin/env python
"""Curriculum SO-3a -- NACA0012, THE ALPHA-MULTIPOINT WEIGHTED OBJECTIVE
...
with EXACTLY the registered deltas below and no other.
  ...
  * `patchV` IS NOT A DESIGN VARIABLE IN THIS ITEM and is REMOVED from
    ...
    SO-3a's structural gate is G-MP-STRUCT, on the assembly identity, and the
  ...
  * THE STEP-BASED TRIVIAL BASELINE IS BACK, at h = 1e-8, and that is a
    ...
    bit-repeatable and h = 1e-8 never enters subtractive cancellation.  SO-3a's
  ...
  * Printed tokens SO2A_* -> SO3A_*; artefact names so2a_* -> so3a_*.
...
PRODUCER = "so3a_runScript.py"
# ---- THE PRODUCER PIN.  Section 18.3: NO MD5 IS CLAIMED FOR A FILE THAT DOES
# ---- NOT EXIST.  `so3a_runScript.py` is section 7 row 5 and is built AFTER this
# ---- file in the registered build order, so at this file's birth the pin is a
# ---- SENTINEL THAT CANNOT MATCH ANY MD5 -- 32 hex digits is the shape of an
# ---- md5 and this string is not one, so `md5_of() != PRODUCER_MD5` is true for
# ---- every possible producer and `main()` REFUSES (exit 2) rather than running
# ---- against an unpinned producer.  A placeholder that could PASS would be the
# ---- SO2a-DRIVER-DEF-1 shape: an agreement control reading agreement over a
# ---- file the frozen code executes.  The pin is set at the Stage-2 amendment.
...
OUT_X = "so3a_X.json"
OUT_F = "so3a_F.json"
JSONL_F = "so3a_F.jsonl"
JSONL_X = "so3a_X.jsonl"
TERMINAL_X = "SO3A_X_WRITTEN"
TERMINAL_F = "SO3A_F_WRITTEN"
...
def parse_mode(argv):
    ...
    for i, a in enumerate(argv):
        if a == "-mode" and i + 1 < len(argv):
            mode = argv[i + 1]
    if mode not in ("X", "F"):
        sys.stderr.write("SO3A_XF usage: so3a_xf.py -mode X|F\n")
        sys.exit(64)
    return mode
...
def main():
    ...
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("SO3A_XF REFUSE producer md5 %s != frozen %s\n"
                         % (got, PRODUCER_MD5))
        sys.exit(2)
    ...
    if src.count(ANCHOR) != 1:
        sys.stderr.write("SO3A_XF REFUSE anchor %r appears %d times\n"
    ...
    ns = {"__name__": "so3a_frozen_header", "__file__": PRODUCER}
    ...
    if rank == 0:
        ...
        if not ok:
            sys.stderr.write("SO3A_XF REFUSE planted-zero control not seen on read-back: %s\n"
                             % json.dumps(detail, sort_keys=True, default=str))
            sys.exit(2)
        sys.stdout.write("SO3A_PLANTED_ZERO_CONTROL_SEEN n_zero_entries=%d want=%r\n"
