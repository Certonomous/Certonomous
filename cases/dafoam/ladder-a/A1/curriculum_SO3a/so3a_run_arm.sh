#!/usr/bin/env bash
# Curriculum SO-3a arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam),
...
# PREREGISTRATION.md section 7 and recorded in so3a_run_arm_DELTAS_from_d15.diff:
...
# `.so3a_age_datum_ref` beside the epoch, so the grader is not left inferring it.
#
...
# `so3a_grade.py:_ledger_rows_via_launcher` runs
#     bash -c '. so3a_run_arm.sh --source-only; so3a_ledger_row ...'
...
# ===========================================================================
ITEM=SO3a
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO3a-a1-naca0012-alpha-multipoint-gradient
...
#
# SO-3a STAGE-2 PLACEMENT (PREREGISTRATION.md section 5: "cpuset fixed at Stage 2
...
# 14 is disjoint from both, so SO-3a does NOT inherit the parent's DISCLOSED
# OVERLAP with D4-SHIPPED's registered 5,6,7,9.  At np = 1 the grader's
# delivered-cores floor does not apply (so3a_grade.py g_placement: dl_ok is True
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^so3a_" | tr '\n' ',' | sed 's/,$//'
}

# ---- REGISTERED CAP TABLE (PREREGISTRATION.md §4, verbatim) ---------------
# SO-3a's registered caps (PREREGISTRATION.md section 4).  THIS TABLE IS A
...
# carried SO-3a's, so the two disagreed for a while and only the code was right.
# so3a_groot5_selftest.sh now PARSES BOTH and refuses if they diverge, because a
...
# before -- even though so3a_runScript.py and so3a_xf.py now exist and their md5s
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: so3a_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: so3a_run_arm.sh <MESH|X-S|F-S|X-P|F-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^so3a_${ARM}_" 2>/dev/null | grep "^so3a_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/so3a_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/so3a_xf.py"                     | md5sum -c - || { echo "ABORT so3a_xf.py md5"; exit 4; }
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.so3a_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so3a_age_datum_ref"
  ...
  rm -f "$WORK/so3a_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.so3a_age_datum" "$WORK/.so3a_age_datum_ref" 2>/dev/null
  cp -a "$BASE/so3a_runScript.py" "$BASE/so3a_xf.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/so3a_X.json" "$WORK/so3a_F.json" "$WORK/so3a_F.jsonl" "$WORK/so3a_X.jsonl" "$WORK/dRdWColoring_2.bin"; do
  ...
  echo "$AGE_DATUM" > "$WORK/.so3a_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so3a_age_datum_ref"
  echo "SO3A_WRITE_COMPRESSION $(grep -a writeCompression "$WORK/system/controlDict" 2>/dev/null | head -1 | tr -d ';')"
  echo "D4_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM datum_file=$DATUM_NAME resolved_by=EXISTENCE"
fi

case "$ARM" in
  MESH)     CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo SO3A_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so3a_xf.py -mode X" ;;
  F-S|F-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so3a_xf.py -mode F" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/so3a_cmd.sh" > /dev/null 2>&1 \
...
{
  so3a_ledger_row "$ARM" "$ROW" "$IMG" "$GOT_DIGEST" "$rc" "$WALL" "$RANKS" "$CORE_MIN" "$CAP" "$TMO" "$BACKCHECK" "$MEM" "$INSPECT" "$MEMAVAIL_GIB" "$MEMAVAIL_POST" "$CPUSET" "$DELIVERED" "$SIBLINGS_PRE" "$SIBLINGS_POST" "$(basename "$LOG")" "$STAMP"
