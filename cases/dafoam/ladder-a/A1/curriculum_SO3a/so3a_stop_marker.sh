#!/usr/bin/env bash
# SO-3a STOP MARKER WRITER -- PREREGISTRATION.md section 7 row 9.  NEW FILE, NO PARENT.
#
# WHY THIS EXISTS (section 2, limb 2).  SO-3a's duty to its successor is a FIXED
...
# rely on, and the successor must be able to distinguish "SO-3a said NOT A RESULT"
# from "SO-3a never got far enough to say anything" -- so both are RECORDED
...
# optimistic default.
#
# Usage:  so3a_stop_marker.sh <RUN_ROOT> <CHAIN_RC> <DECLARED> <EXECUTED> [GRADE_JSON]
set -uo pipefail

BASE="${1:-}"; CHAIN_RC="${2:-}"; DECLARED="${3:-}"; EXECUTED="${4:-}"; GRADE_JSON="${5:-}"
test -n "$BASE" || { echo "ABORT so3a_stop_marker.sh: no run root given"; exit 64; }
test -n "$CHAIN_RC" || { echo "ABORT so3a_stop_marker.sh: no chain rc given"; exit 64; }
test -n "$DECLARED" || { echo "ABORT so3a_stop_marker.sh: no DECLARED count given"; exit 64; }
test -n "$EXECUTED" || { echo "ABORT so3a_stop_marker.sh: no EXECUTED count given"; exit 64; }
test -d "$BASE" || { echo "ABORT so3a_stop_marker.sh: run root absent: $BASE"; exit 4; }
...
MARKER="$BASE/SO3a_STOP_MARKER.json"
...
TMP="$BASE/.SO3a_STOP_MARKER.json.tmp.$$"
...
if back.get("verdict") != rec["verdict"] or back.get("chain_rc") != rec["chain_rc"]:
    ...
    sys.exit(2)
print("SO3A_STOP_MARKER written=%s verdict=%s chain_rc=%d declared=%d executed=%d truncated=%s"
