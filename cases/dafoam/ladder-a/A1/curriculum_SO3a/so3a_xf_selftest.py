#!/usr/bin/env python3
"""SO-3a INSTRUMENT SELFTEST -- drives `so3a_xf.py` on the HOST.
...
OUT.append("SO-3a INSTRUMENT DRIVE -- so3a_xf.py, derived from so2a_xg.py")
...
# ---- A5. the producer pin FAILS CLOSED ---------------------------------------
import so3a_xf as XF
ok("A5 PRODUCER_MD5 is a SENTINEL that cannot match any md5 (not 32 hex digits), "
   "so main() REFUSES against every possible producer until the Stage-2 pin: %r"
   % XF.PRODUCER_MD5, re.fullmatch(r"[0-9a-f]{32}", XF.PRODUCER_MD5) is None)
...
if mutant == src:
    ...
    with tempfile.TemporaryDirectory() as td:
        shutil.copy(CHILD, os.path.join(td, "so3a_xf_mut.py"))
        open(os.path.join(td, "so3a_xf_mut.py"), "w").write(mutant)
        sys.path.insert(0, td)
        mut = importlib.import_module("so3a_xf_mut")
...
open(os.path.join(CASE, "so3a_xf_drive_evidence.txt"), "w").write(
    "SO-3a so3a_xf.py DRIVE EVIDENCE -- written by the drive, not by hand.\n"
