#!/usr/bin/env bash
# SO-3a chain driver -- DERIVED from curriculum_SO2a/so2a_chain_driver.sh
# (md5 asserted in so3a_chain_driver_DELTAS_from_so2a.diff) with the REGISTERED
...
#   (a) NAMES: so2a_ -> so3a_, SO2A_ -> SO3A_, SO2a -> SO3a; run root
#       CURRICULUM-SO3a-a1-naca0012-alpha-multipoint-gradient; the instrument is
#       so3a_xf.py (modes X and F), NOT so2a_xg.py.
...
#       THE EXPIRY TERMINATES.  IT DOES NOT BLOCK-AND-CONTINUE.  SO-3a registers
...
LAUNCHER="$HERE/so3a_run_arm.sh"
GRADER="$HERE/so3a_grade.py"
AGG_SCRIPT="$HERE/so3a_aggregate_memory.py"
STOP_MARKER="$HERE/so3a_stop_marker.sh"
...
test $# -ge 1 || { echo "ABORT usage: so3a_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/so3a_driver.pid"
cd "$HERE" || exit 4
EXECUTED=0
...
for _dep in "$LAUNCHER" "$GRADER" "$AGG_SCRIPT" "$STOP_MARKER" \
            "$HERE/so3a_runScript.py" "$HERE/so3a_xf.py" "$HERE/so3a_decomposeParDict"; do
  test -f "$_dep" || { echo "ABORT section 18.3 dependency ABSENT before any md5: $_dep"; exit 4; }
done
echo "SO3A_DEPS_EXIST n=7 checked_before_any_md5=yes"
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$HERE/so3a_decomposeParDict" "$BASE/base/system/decomposeParDict" || { echo "ABORT overlay decomposeParDict"; exit 4; }
  cp -a "$HERE/so3a_runScript.py" "$HERE/so3a_xf.py" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  ...
  echo "SO3A_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "SO3A_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT  $BASE/so3a_runScript.py"; echo "$MD5_XF  $BASE/so3a_xf.py"; echo "$MD5_DECOMP  $BASE/base/system/decomposeParDict";
...
echo "SO3A_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] declared=$DECLARED_ARMS permission=$PERMISSION"
...
for ARM in $ARMS; do
  ...
  while true; do
    ...
    echo "SO3A_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW waited_s=$H5_WAITED file=$(basename "$H5_FILE")"
  ...
  echo "SO3A_AGGREGATE arm=$ARM waited=$WAITED $AGG_JSON"
  echo "SO3A_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "SO3A_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
GRADE_OUT="$BASE/SO3a_grade_${GSTAMP}.json"
if echo "$MD5_GRADER  $GRADER" | md5sum -c - > /dev/null; then
  python3 "$GRADER" --root "$BASE" --out "$GRADE_OUT" > "$BASE/SO3a_grade_${GSTAMP}.out" 2>&1; GRC=$?
  echo "grader_rc=$GRC stamp=$GSTAMP out=SO3a_grade_${GSTAMP}.json note=comparator-exit-status-NOT-the-verdict" >> "$STATUS"
else
...
bash "$STOP_MARKER" "$BASE" "$CHAIN_RC" "$DECLARED_ARMS" "$EXECUTED" "$GRADE_OUT" \
  || echo "SO3A_STOP_MARKER_FAILED rc=$? -- the marker did not land; the successor has no fixed address for this run"
echo "SO3A_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC declared=$DECLARED_ARMS executed=$EXECUTED"
exit "$CHAIN_RC"
