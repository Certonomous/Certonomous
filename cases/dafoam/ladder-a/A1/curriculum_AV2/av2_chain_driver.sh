#!/usr/bin/env bash
# AV2 chain driver -- DERIVED from curriculum_AV1/av1_chain_driver.sh (frozen at
# 0b3ebaa4, md5 b070e40957b88e5d8b0ebba10075d500) with the REGISTERED DELTAS listed
# in PREREGISTRATION.md section 7 and recorded in av2_chain_driver_DELTAS_from_av1.diff:
...
#       the ledger opens with an exact `ITEM=AV2` line and a STAGED line
...
#   setsid nohup bash av2_chain_driver.sh MESH X-S FAD-S X-P FAD-P > <root>/chain_launch.out 2>&1 &
...
LAUNCHER="$HERE/av2_run_arm.sh"
GRADER="$HERE/av2_grade.py"
...
MD5_LAUNCHER=de540f10dd391f71606b157a11bc24f3
MD5_GRADER=4bde0ad7dbdd3e460dcef1fe6d063979
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f
MD5_XF=76bc93090062e0bf03de2344709e384f
...
test $# -ge 1 || { echo "ABORT usage: av2_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/av2_driver.pid"
cd "$HERE" || exit 4
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$HERE/av2_runScript.py" "$HERE/av2_xf.py" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  ...
  echo "AV2_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "AV2_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT  $BASE/av2_runScript.py"; echo "$MD5_XF  $BASE/av2_xf.py"; echo "$MD5_DECOMP_TUT  $BASE/base/system/decomposeParDict";
...
echo "AV2_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
for ARM in $ARMS; do
  ...
  echo "AV2_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW file=$(basename "$H5_FILE")"
  ...
  while true; do
    AGG=$(python3 "$HERE/av2_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
  ...
  echo "AV2_AGGREGATE arm=$ARM waited=$WAITED $AGG"
  echo "AV2_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "AV2_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
if echo "$MD5_GRADER  $GRADER" | md5sum -c - > /dev/null; then
  python3 "$GRADER" --root "$BASE" --out "$BASE/AV2_grade_${GSTAMP}.json" > "$BASE/AV2_grade_${GSTAMP}.out" 2>&1; GRC=$?
  echo "grader_rc=$GRC stamp=$GSTAMP out=AV2_grade_${GSTAMP}.json note=comparator-exit-status-NOT-the-verdict" >> "$STATUS"
else
  echo "grader_rc=NOT_RUN stamp=$GSTAMP note=grader-md5-drifted-at-chain-end" >> "$STATUS"
fi
echo "AV2_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC"
exit "$CHAIN_RC"
