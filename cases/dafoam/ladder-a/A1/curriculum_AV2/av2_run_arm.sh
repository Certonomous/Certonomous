#!/usr/bin/env bash
# Curriculum AV2 arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam) baseline
...
# in PREREGISTRATION.md section 7 and recorded in av2_run_arm_DELTAS_from_av1.diff:
...
# av2_xf.py (-mode X reverse totals, -mode FAD forward-mode totals per registered
...
# ===========================================================================
ITEM=AV2
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-AV2-a1-naca0012-duality
...
# ---- can be correct and its ledger still be a foreign one moved in.  AV2
...
#
# AV2 REGISTERED DELTA (PREREGISTRATION.md section 5): every arm is one rank on
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^av2_" | tr '\n' ',' | sed 's/,$//'
}
...
MD5_RUNSCRIPT=0557da51f6f179f6de865144343c499f   # av2_runScript.py = byte copy of the shipped INCOMPRESSIBLE tutorial runScript.py
MD5_XF=76bc93090062e0bf03de2344709e384f                            # av2_xf.py (set at freeze)
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: av2_run_arm.sh <MESH|X-S|FAD-S|X-P|FAD-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: av2_run_arm.sh <MESH|X-S|FAD-S|X-P|FAD-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^av2_${ARM}_" 2>/dev/null | grep "^av2_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/av2_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/av2_xf.py"                     | md5sum -c - || { echo "ABORT xf instrument md5"; exit 4; }
...
# AV2: every arm is np = 1; no decomposeParDict overlay (DAFoam does not read
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.av2_age_datum"
  ...
  rm -f "$WORK/av2_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.av2_age_datum" 2>/dev/null
  cp -a "$BASE/av2_runScript.py" "$BASE/av2_xf.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/av2_X.json" "$WORK/av2_X.jsonl" "$WORK/av2_X_planted.json" "$WORK/av2_FAD.json" "$WORK/av2_FAD.jsonl" "$WORK/av2_FAD_planted.json" "$WORK/dRdWColoring_1.bin"; do
...
case "$ARM" in
  MESH)        CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo AV2_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  X-S|X-P)     CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python av2_xf.py -mode X" ;;
  FAD-S|FAD-P) CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python av2_xf.py -mode FAD" ;;
esac
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/av2_cmd.sh" > /dev/null 2>&1 \
