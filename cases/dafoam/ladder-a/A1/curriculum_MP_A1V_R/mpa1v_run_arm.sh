#!/usr/bin/env bash
# Curriculum MP-A1V-R arm launcher (MP-A1V launcher-repair successor: L-504 runtime
# path refs age_guard/stall/output-artefact reconciled mpa1_->mpa1v_) -- NACA0012 INCOMPRESSIBLE (DASimpleFoam),
...
# ===========================================================================
ITEM=MPA1V
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-MP-A1V-R-a1-naca0012-multipoint-fixedlift-fdverify-directDVcentral
...
PIDFILE="$BASE/mpa1v_driver.pid"   # MUST match mpa1v_chain_driver.sh:PIDFILE (the driver WRITES this; G-ROOT.5 READS it)
if [ -f "$PIDFILE" ]; then
...
if [ "$ARM" = "MESH" ]; then
  ...
  # `mpa1v_stall.py` is staged INTO THE ARM because the producer imports it inside
  # the container to write its own stall reading; `mpa1v_age_guard.py` is NOT
  ...
  for bad in "$WORK/reports" "$WORK/mpa1v_X.json" "$WORK/mpa1v_F.json" "$WORK/mpa1v_F.jsonl" "$WORK/mpa1v_X.jsonl" \
...
import json; print(len(json.load(open('$WORK/mpa1_xopt.json')).get('shape',[])))")"
  ...
  # code does not honour.  `mpa1v_age_guard.py` leg (a3) proves a disagreement
  # between the two REFUSES.
  test -f "$BASE/mpa1v_age_guard.py" || { echo "ABORT age guard instrument absent: $BASE/mpa1v_age_guard.py"; exit 4; }
  python3 "$BASE/mpa1v_age_guard.py" pin "$WORK" "$WORK/.mpa1_manifest.json" \
...
case "$ARM" in
  O-S|O-P)
    if [ -f "$BASE/mpa1v_stall.py" ]; then
      (
        while sudo -n docker ps -q --filter "name=^${NAME}$" 2>/dev/null | grep -q .; do
          sleep 30
          [ -f "$WORK/opt_IPOPT.txt" ] || continue
          VERD=$(python3 "$BASE/mpa1v_stall.py" scan "$WORK/opt_IPOPT.txt" 2>/dev/null \
                 | python3 -c "
import sys, json
...
print('%s %s %s %s' % (d['stall'], d['stop_at_major'],
      ...
      STALLDOG=$!
      echo "MPA1_STALLDOG arm=$ARM armed=yes pid=$STALLDOG poll_s=30 detector=$BASE/mpa1v_stall.py"
    else
      ...
      echo "MPA1_STALLDOG arm=$ARM armed=NO reason=detector_absent_at_$BASE/mpa1v_stall.py -- THE STALL STOP IS NOT ARMED FOR THIS ARM"
    fi ;;
...
if [ -f "$WORK/.mpa1_manifest.json" ] && [ -f "$BASE/mpa1v_age_guard.py" ]; then
  DATUM_REF_FILE="$WORK/$(cat "$WORK/.mpa1_age_datum_ref" 2>/dev/null)"
  ARTS=""
  for a in "$WORK/mpa1v_X.json" "$WORK/mpa1v_F.json" "$WORK/mpa1_O.json" "$WORK/mpa1_xopt.json" "$WORK/checkMesh.log"; do
    [ -f "$a" ] && ARTS="$ARTS $a"
  done
  python3 "$BASE/mpa1v_age_guard.py" verify "$WORK" "$WORK/.mpa1_manifest.json" \
