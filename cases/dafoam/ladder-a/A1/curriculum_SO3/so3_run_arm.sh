#!/usr/bin/env bash
# Curriculum SO-3 arm launcher -- NACA0012 INCOMPRESSIBLE (DASimpleFoam),
...
# PREREGISTRATION.md section 7 and recorded in so3_run_arm_DELTAS_from_d15.diff:
...
# `.so3_age_datum_ref` beside the epoch, so the grader is not left inferring it.
#
...
# `so3_grade.py:_ledger_rows_via_launcher` runs
#     bash -c '. so3_run_arm.sh --source-only; so3_ledger_row ...'
...
# ===========================================================================
# THE REGISTERED TABLES LIVE ABOVE `--source-only`, AND THAT IS A SO-3 DELTA.
#
...
# Moving them above the early return exposes them to so3_groot5_selftest.sh's
...
cap_memory() {
  case "$1" in
    MESH|O-S|O-P|XE-S|XE-P|FE-S|FE-P) echo 12g ;;
    *) echo "" ;;
  esac
}
...
# statement about that np and is never carried to another."*  SO-3's FD
...
ranks_of() {
  case "$1" in
    MESH|O-S|O-P|XE-S|XE-P|FE-S|FE-P) echo 1 ;;
    *) echo "" ;;
  esac
}
...
# ===========================================================================
ITEM=SO3
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-SO3-a1-naca0012-alpha-multipoint-optimisation
...
#
# ---- SO-3 REGISTERED DELTA, AND IT IS A REPAIR OF A FICTION THIS FAMILY HAS NOW
...
# ----     sentence was TRUE OF SO-3a's ROOT and was carried across two renames
...
# ---- SO-3a, SO-3aR and SO-3aR2 -- are ADDED, because SO-3aR2's holds a GRADED,
...
# ---- `CURRICULUM-SO3F-...` is KEPT, because a name that is free today can be
...
# ---- `so3_run_arm_selftest.sh` leg (r6) drives every entry in this list against
...
/home/ubuntu/certonomous-runs/CURRICULUM-SO3F-a1-naca0012-alpha-feasibility
...
#
# SO-3 STAGE-2 PLACEMENT (PREREGISTRATION.md section 5: "cpuset fixed at Stage 2
...
# 14 is disjoint from both, so SO-3 does NOT inherit the parent's DISCLOSED
# OVERLAP with D4-SHIPPED's registered 5,6,7,9.  At np = 1 the grader's
# delivered-cores floor does not apply (so3_grade.py g_placement: dl_ok is True
...
# reporting the exposure.  G12 compares this value against so3_grade.py's
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^so3_" | tr '\n' ',' | sed 's/,$//'
}

# ---- REGISTERED CAP TABLE (PREREGISTRATION.md §4, verbatim) ---------------
# SO-3's registered caps (PREREGISTRATION.md section 4).  THIS TABLE IS A
...
# carried SO-3's, so the two disagreed for a while and only the code was right.
# so3_groot5_selftest.sh now PARSES BOTH and refuses if they diverge, because a
...
# block.  `so3_run_arm_selftest.sh` leg (r2) PARSES this comment and the code and
...
# before -- even though so3_runScript.py and so3_xf.py now exist and their md5s
...
# it.  so3_chain_driver.sh:60-71 makes the same choice for the same reason.
...
# so3_decomposeParDict is ADOPTED BYTE-IDENTICALLY from the parent (one line
# changed from the tutorial's: numberOfSubdomains 1), it EXISTS, and this is the
# value so3_chain_driver.sh:70 already pins it at.  It is not a partial table: it
# is the one file whose bytes are unchanged from an already-frozen ancestor.
MD5_DECOMP=e6f1b0060944bc86d6dff56480ad2bd4

ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: so3_run_arm.sh <MESH|O-S|XE-S|FE-S|O-P|XE-P|FE-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: so3_run_arm.sh <MESH|O-S|XE-S|FE-S|O-P|XE-P|FE-P> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^so3_${ARM}_" 2>/dev/null | grep "^so3_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "SO3_C188_FRAME arm=$ARM repaired_tmo_s=$TMO ancestor_tmo_s=$OLD_TMO"
echo "SO3_C188_FRAME arm=$ARM worst_case_host_core_min repaired=$NEW_WORST ancestor=$OLD_WORST cap=$CAP overhead_s=8_MEASURED_C188 kill_grace_s=$CAP_KILL_GRACE_S"
...
echo "$MD5_RUNSCRIPT  $BASE/so3_runScript.py"       | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_XF  $BASE/so3_xf.py"                     | md5sum -c - || { echo "ABORT so3_xf.py md5"; exit 4; }
...
# empty case and REFUSES, and it is the same mapping so3_grade.py registers as
# ARM_ROW (MESH/O-S/XE-S/FE-S -> SHIPPED, O-P/XE-P/FE-P -> PATCHED).
WANT_ROW=$(row_of "$ARM")
test -n "$WANT_ROW" || {
  echo "ABORT arm $ARM carries no registered row.  SO-3 declares MESH O-S XE-S FE-S O-P XE-P FE-P"
...
if [ "$ARM" = "MESH" ]; then
  ...
  echo "$AGE_DATUM" > "$WORK/.so3_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so3_age_datum_ref"
  ...
  rm -f "$WORK/so3_cmd.sh" "$WORK/checkMesh.log" "$WORK/logMeshGeneration.txt" "$WORK/volumeMesh.xyz" "$WORK/surfaceMesh.xyz" "$WORK/.so3_age_datum" "$WORK/.so3_age_datum_ref" 2>/dev/null
  # `so3_stall.py` is staged INTO THE ARM because the producer imports it inside
  # the container to write its own stall reading; `so3_age_guard.py` is NOT
  ...
  cp -a "$BASE/so3_runScript.py" "$BASE/so3_xf.py" "$BASE/so3_stall.py" "$WORK/" \
    || { echo "ABORT stage instruments"; exit 4; }
  # ---- SO-3 REGISTERED DELTA: ONE FULL CASE COPY PER OPERATING POINT ------
  ...
  # and MUST equal `so3_runScript.py:RUN_DIRS`'s values; `so3_collision_leg.py`
  ...
  for mp in mp0 mp1 mp2; do
    ...
    rm -f "$WORK/$mp/so3_cmd.sh" "$WORK/$mp/checkMesh.log" "$WORK/$mp/logMeshGeneration.txt" \
          "$WORK/$mp/volumeMesh.xyz" "$WORK/$mp/surfaceMesh.xyz" \
          "$WORK/$mp/.so3_age_datum" "$WORK/$mp/.so3_age_datum_ref" 2>/dev/null
  ...
  for bad in "$WORK/reports" "$WORK/so3_X.json" "$WORK/so3_F.json" "$WORK/so3_F.jsonl" "$WORK/so3_X.jsonl" \
             "$WORK/so3_O.json" "$WORK/so3_xopt.json" "$WORK/opt_IPOPT.txt" "$WORK/SO3_STALL_ABORT" \
  ...
  echo "$AGE_DATUM" > "$WORK/.so3_age_datum"
  echo "$DATUM_NAME" > "$WORK/.so3_age_datum_ref"
  echo "SO3_WRITE_COMPRESSION $(grep -a writeCompression "$WORK/system/controlDict" 2>/dev/null | head -1 | tr -d ';')"
  echo "D4_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM datum_file=$DATUM_NAME resolved_by=EXISTENCE"
  # ---- G-IC0's DATUM, PER POINT.  SO-3 REGISTERED DELTA -------------------
  ...
  for mp in mp0 mp1 mp2; do
    ...
    echo "$MPE" > "$WORK/$mp/.so3_age_datum"
    echo "$MPD" > "$WORK/$mp/.so3_age_datum_ref"
    md5sum "$WORK/$mp/0"/* > "$WORK/$mp/.so3_ic0_md5" 2>/dev/null || { echo "ABORT G-IC0 md5 $mp"; exit 5; }
    echo "SO3_G_IC0_DATUM point=$mp age_datum_epoch=$MPE datum_file=$MPD n_ic_fields=$(wc -l < "$WORK/$mp/.so3_ic0_md5")"
  done
  ...
  if [ -n "$XOPT_ARM" ]; then
    XOPT_SRC="$BASE/$XOPT_ARM/so3_xopt.json"
    test -f "$XOPT_SRC" || {
      echo "ABORT G-XOPT arm $ARM needs the final design point from arm $XOPT_ARM,"
      echo "  and $XOPT_SRC does not exist.  The endpoint FD check section 9"
      echo "  requires cannot be run against a design that was never produced."
      exit 5; }
...
print(d.get('row',''))" 2>/dev/null)
    test "$XOPT_ROW" = "$(row_of "$ARM")" || {
      ...
      exit 5; }
    cp -a "$XOPT_SRC" "$WORK/so3_xopt.json" || { echo "ABORT G-XOPT stage copy"; exit 4; }
    XOPT_SHA=$(sha256sum "$WORK/so3_xopt.json" | cut -d' ' -f1)
    echo "SO3_G_XOPT arm=$ARM row=$XOPT_ROW source_arm=$XOPT_ARM sha256=$XOPT_SHA n_dv=$(python3 -c "
import json; print(len(json.load(open('$WORK/so3_xopt.json')).get('shape',[])))")"
  fi
  ...
  # code does not honour.  `so3_age_guard.py` leg (a3) proves a disagreement
  # between the two REFUSES.
  test -f "$BASE/so3_age_guard.py" || { echo "ABORT age guard instrument absent: $BASE/so3_age_guard.py"; exit 4; }
  python3 "$BASE/so3_age_guard.py" pin "$WORK" "$WORK/.so3_manifest.json" \
...
case "$ARM" in
  MESH)       CMD="bash preProcessing.sh && checkMesh > checkMesh.log 2>&1; echo SO3_CHECKMESH_RC \$?; sha256sum constant/polyMesh/points* ; grep -a 'cells:' checkMesh.log" ;;
  O-S|O-P)    CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so3_runScript.py -task run_driver -optimizer IPOPT" ;;
  XE-S|XE-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so3_xf.py -mode X -xopt so3_xopt.json" ;;
  FE-S|FE-P)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python so3_xf.py -mode F -xopt so3_xopt.json" ;;
esac
test -n "${CMD:-}" || {
  echo "ABORT arm $ARM carries no registered command.  SO-3 declares"
  echo "  MESH O-S XE-S FE-S O-P XE-P FE-P and nothing else."
  exit 64; }
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/so3_cmd.sh" > /dev/null 2>&1 \
...
#
#   A. so3_stall.N_STALL (=8) consecutive majors with alpha_pr < 1e-3
...
# is section 9's and lives in `so3_grade.py`; a watchdog that also graded could
# launder a stop into a pass by relabelling it here.
# ===========================================================================
STALL_MARKER="$WORK/SO3_STALL_ABORT"
STALLDOG=""
case "$ARM" in
  O-S|O-P)
    if [ -f "$BASE/so3_stall.py" ]; then
      (
        while sudo -n docker ps -q --filter "name=^${NAME}$" 2>/dev/null | grep -q .; do
          sleep 30
          [ -f "$WORK/opt_IPOPT.txt" ] || continue
          VERD=$(python3 "$BASE/so3_stall.py" scan "$WORK/opt_IPOPT.txt" 2>/dev/null \
                 | python3 -c "
import sys, json
...
print('%s %s %s %s' % (d['stall'], d['stop_at_major'],
      d['condition_A_alpha_pr_run']['fired'], d['condition_B_inf_du_nondecreasing']['fired']))" 2>/dev/null)
          case "$VERD" in
            STALL\ *)
              set -- $VERD
              printf 'SO3_STALL_ABORT arm=%s stop_at_major=%s condition_A=%s condition_B=%s n_stall_window=8 alpha_pr_min=1e-3 stamp=%s\n' \
                     "$ARM" "$2" "$3" "$4" "$(date -u +%Y%m%dT%H%M%SZ)" > "$STALL_MARKER"
              echo "SO3_STALL_ABORT arm=$ARM stop_at_major=$2 condition_A=$3 condition_B=$4 action=DOCKER_STOP" | tee -a "$BASE/ledger.txt"
      ...
      STALLDOG=$!
      echo "SO3_STALLDOG arm=$ARM armed=yes pid=$STALLDOG poll_s=30 detector=$BASE/so3_stall.py"
    else
      ...
      echo "SO3_STALLDOG arm=$ARM armed=NO reason=detector_absent_at_$BASE/so3_stall.py -- THE STALL STOP IS NOT ARMED FOR THIS ARM"
    fi ;;
  *) echo "SO3_STALLDOG arm=$ARM armed=no reason=not_an_optimisation_arm" ;;
esac
...
if [ -f "$STALL_MARKER" ]; then
  echo "SO3_STALL_FIRED arm=$ARM $(cat "$STALL_MARKER")"
else
  echo "SO3_STALL_NOT_FIRED arm=$ARM -- the registered stall conditions did not fire; this is REPORTED, not read as convergence"
fi
...
if [ -d "$WORK/mp0" ]; then
  IC0="$BASE/${ARM}_IC0.json"
  {
    ...
    for mp in mp0 mp1 mp2; do
      ...
      if [ -f "$WORK/$mp/.so3_ic0_md5" ]; then
        PRE=$(cat "$WORK/$mp/.so3_age_datum" 2>/dev/null)
        NB=$(wc -l < "$WORK/$mp/.so3_ic0_md5")
        NC=$(md5sum -c "$WORK/$mp/.so3_ic0_md5" 2>/dev/null | grep -c ': FAILED$')
      fi
  ...
  echo "SO3_G_IC0 arm=$ARM file=$(basename "$IC0") $(python3 -c "
import json
...
if [ -f "$WORK/.so3_manifest.json" ] && [ -f "$BASE/so3_age_guard.py" ]; then
  DATUM_REF_FILE="$WORK/$(cat "$WORK/.so3_age_datum_ref" 2>/dev/null)"
  ARTS=""
  for a in "$WORK/so3_X.json" "$WORK/so3_F.json" "$WORK/so3_O.json" "$WORK/so3_xopt.json" "$WORK/checkMesh.log"; do
    [ -f "$a" ] && ARTS="$ARTS $a"
  done
  python3 "$BASE/so3_age_guard.py" verify "$WORK" "$WORK/.so3_manifest.json" \
          "$DATUM_REF_FILE" $ARTS > "$AGE_OUT" 2>&1
  echo "SO3_AGE_GUARD arm=$ARM rc=$? file=$(basename "$AGE_OUT") first_line=[$(head -1 "$AGE_OUT" 2>/dev/null)]"
else
  echo "SO3_AGE_GUARD arm=$ARM rc=NOT_RUN reason=manifest_or_instrument_absent -- REPORTED as NOT_MEASURED, never as a pass" | tee "$AGE_OUT"
fi
...
{
  so3_ledger_row "$ARM" "$ROW" "$IMG" "$GOT_DIGEST" "$rc" "$WALL" "$RANKS" "$CORE_MIN" "$CAP" "$TMO" "$BACKCHECK" "$MEM" "$INSPECT" "$MEMAVAIL_GIB" "$MEMAVAIL_POST" "$CPUSET" "$DELIVERED" "$SIBLINGS_PRE" "$SIBLINGS_POST" "$(basename "$LOG")" "$STAMP"
