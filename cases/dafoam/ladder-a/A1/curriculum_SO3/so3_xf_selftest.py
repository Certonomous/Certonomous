#!/usr/bin/env python3
"""SO-3 INSTRUMENT SELFTEST -- drives `so3_xf.py` on the HOST.
...
OUT.append("SO-3 INSTRUMENT DRIVE -- so3_xf.py, derived from so2a_xg.py")
...
if mutant == src:
    ...
    with tempfile.TemporaryDirectory() as td:
        shutil.copy(CHILD, os.path.join(td, "so3_xf_mut.py"))
        open(os.path.join(td, "so3_xf_mut.py"), "w").write(mutant)
        sys.path.insert(0, td)
        mut = importlib.import_module("so3_xf_mut")
...
open(os.path.join(CASE, "so3_xf_drive_evidence.txt"), "w").write(
    "SO-3 so3_xf.py DRIVE EVIDENCE -- written by the drive, not by hand.\n"
