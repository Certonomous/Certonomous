#!/usr/bin/env python3
"""CURRICULUM SO-3 -- THE FROZEN COMPARATOR.
...
THE FREEZE SHA IS DELIBERATELY NOT WRITTEN HERE.  SO-3a's copy of this line
cited `1a06a7d6`, its own Stage-1 freeze commit, because there its document was
frozen BEFORE its instruments existed.  SO-3 freezes the document and the
...
file by md5, and `so3_chain_driver.sh:MD5_GRADER` asserts it before staging.

Derived from `curriculum_SO2a/so2a_grade.py` with the deltas in
`so3_grade_DELTAS_from_so2a.diff`.  FIVE of those deltas are the point of the
...
EXPECTATIONS -- a tautology on schema.  SO-3's comparator is written AFTER two
of its consumers were frozen, so the contract runs the other way and is checkable
TODAY:

  * `so3_chain_driver.sh:234` invokes `python3 $GRADER --root $BASE --out $GRADE_OUT`.
  ...
  * `so3_stop_marker.sh:97,101,105` reads `verdict` at the TOP LEVEL, `rows` at
...
Neither fact is asserted in prose.  `selftest` RUNS `so3_stop_marker.sh` on a
...
parent refuses on `ledger_row_absent` and on `arm_log_absent`.  **SO-3 prints
...
--------------------------------------------------------------------------
Every X and F artefact in `_fix` is written by CALLING `so3_xf.build_X_record`,
...
SOURCING `so3_run_arm.sh`'s own `so3_ledger_row`; when that launcher is absent
...
import so3_stall as STALL

# ---- REGISTERED CONSTANTS (PREREGISTRATION.md; the document governs) ----------------
ITEM = "SO3"
BASE = "/home/ubuntu/certonomous-runs/CURRICULUM-SO3-a1-naca0012-alpha-multipoint-optimisation"
...
# ---- SO-3's answer is not a better MAPPING, it is the ABSENCE of a second label
...
# SO-3 arm therefore costs ~34 x 2.061 = 70.1 core-min, BELOW the converging
# prediction -- so the stall branch is a floor on the saving, not an extra cost,
# and the cap is sized on the CONVERGING branch.
CAPS = {"MESH": 5.0, "O-S": 240.0, "XE-S": 15.0, "FE-S": 40.0,
        "O-P": 240.0, "XE-P": 15.0, "FE-P": 40.0}
ITEM_CEILING_CORE_MIN = 595.0          # == sum(CAPS.values()), asserted in main()
PREDICTED_CORE_MIN = {"MESH": 0.19, "O-S": 103.0, "XE-S": 3.7, "FE-S": 7.5,
                      "O-P": 103.0, "XE-P": 3.7, "FE-P": 7.5}
PREDICTED_STALL_CORE_MIN = {"O-S": 70.1, "O-P": 70.1}
...
# SO-3 REGISTERED DELTA.  This is the change that has to be argued, not just
...
# that is not driven at an insufficient K is the same shape as SO-3a's leg `A5`,
...
# ---- SO-3 rests on SO-1a's PATCHED row while SO-1a's ITEM verdict is GATE FAIL.
...
# ---- SO-3 REGISTERED DELTA: THE PROVENANCE IS A CHAIN, NOT ONE LINK. --------
# SO-3's DIRECT upstream is SO-3aR2, not SO-1a, and SO-3aR2's ITEM VERDICT IS
...
UPSTREAM_CHAIN = [
    {"item": "CURRICULUM-SO3aR2",
     ...
     "relation": "DIRECT -- SO-3's optimisation is admissible only because SO-3aR2 "
     ...
     "patched_detail": {
         ...
         "endpoint_gap": "MEASURED AT ITERATION 0 ONLY.  DAFOAM_CHARTER section 9 requires "
                         ...
                         "47.  SO-3's XE/FE arms are what close that gap"}},
...
def compose_verdict_line():
    ...
    return ("UPSTREAM CHAIN %s -- EVERY LINK GATE FAIL AT ITEM LEVEL, SHIPPED row GATE FAIL "
            "at every link, PATCHED row PASS; SO-3 builds on the PATCHED row and the SHIPPED "
...
READERS = {
    "R1_read_ledger": {"produces": "core_min, rc, cpuset, DIGEST, oomkilled -> G1/G9/G10/G12",
                       "producer": "so3_run_arm.sh:so3_ledger_row",
    ...
    "R4_read_X": {"produces": "adjoint totals -> G5J, G5C, G-MP-STRUCT (ALL CAN PASS ON A SMALL NUMBER)",
                  "producer": "so3_xf.build_X_record", "born_by": ["U60", "U61", "U62"], "born": True},
    "R5_read_F": {"produces": "FD derivative table -> G5J, G5C, G-TB",
                  "producer": "so3_xf.build_F_record / build_fd_row / build_ctrl_row",
                  "born_by": ["U63", "U64", "U65"], "born": True},
    "R6_read_multipoint_identity": {"produces": "alphas and weights -> G-ALPHA",
                                    "producer": "so3_xf.build_identity_block",
...
# (so3_chain_driver.sh's `img_of`) was in shell -- so eight files were swept and
...
# SO-3, 2026-08-31: BUMPED DELIBERATELY 92 -> 97, and the five are named so the
...
# form: it reads EVERY SO-3 instrument and reports EVERY site that either
...
_ROW_SWEEP_PY = (
    ...
    (re.compile(r"""(?<![A-Za-z0-9_])(['"])(%s)\1""" % "|".join(ROW_SHORT_FORMS)),
     "a SHORT-FORM row literal; SO-3 has ONE label set and no short form"),
...
_ROW_SWEEP_SH = (
    ...
    # unrepaired call site in this item -- `so3_chain_driver.sh`'s `img_of`,
...
def so3_instrument_files():
    ...
    # ELEVEN, not SO-3aR's ten and not SO-3a's eight.  `so3_collision_leg.py`
    ...
    # cross to A1.
    names = ["so3_grade.py", "so3_xf.py", "so3_runScript.py", "so3_run_arm.sh",
             "so3_groot5_selftest.sh", "so3_chain_driver.sh", "so3_stop_marker.sh",
             "so3_aggregate_memory.py", "so3_pin_census.py", "so3_xf_selftest.py",
             "so3_collision_leg.py"]
    present, absent = [], []
    for nm in names:
...
def grader_plant_X(base, xpath, tag, k, scale=None, plant_k=None):
    ...
    SO-3 REGISTERED DELTA: `scale` is `d_ref` for the pair this plant lands on,
...
def g_noopt(recs, rec_kind, logs):
    ...
    SO-3 REGISTERED INVERSION, AND IT IS NAMED RATHER THAN QUIETLY NARROWED.
    SO-3aR2's version of this gate asserted that THE ITEM ran no optimiser, and
    that was right for a gradient rung.  SO-3 IS THE OPTIMISATION RUNG: its O-S
...
#      `so3_stall.py`, which is calibrated on 33 real IPOPT logs on this box:
...
#      UNCONSTRAINED in this item (so3_runScript docstring (C)), so a weighted-drag
...
OPT_INTERMEDIATE_THRESHOLD_PCT = 5.0   # weighted-drag reduction against the run's own baseline
...
def g_opt_section9(O, endpoint_row_verdict, cap_row):
    ...
    `O` is the row's `so3_O.json` record, or None if the arm did not run.
    ...
    return {
        ...
        "forbidden_readings": [
            ...
            "because CL is UNCONSTRAINED in this item",
...
# ===================== read the O artefact ============================================
def read_O(path):
    """The optimisation record `so3_O.json`, read by EXPLICIT keys.
...
def g_design_point(X, F, O):
    ...
    THE FAILURE THIS GATE EXISTS FOR.  SO-3's XE/FE arms are byte-for-byte the
    ...
    artefact that is SO-3aR2's baseline measurement under SO-3's arm name, and
    ...
    return {"verdict": "GATE FAIL" if bad else "PASS", "per_artefact": per,
            ...
            "why_it_is_gated": ("an XE/FE arm launched without -xopt yields a valid artefact that "
                                "is SO-3aR2's BASELINE measurement wearing SO-3's arm name, and "
...
def require_travelling_provenance():
    ...
    # ---- CLAUSE (6), SO-3's OWN: EVERY LINK IN THE CHAIN IS CHECKED, NOT JUST
    ...
    if not isinstance(chain, list) or len(chain) < 2:
        refuse("G-PROV", {"upstream_chain_absent_or_shorter_than_two_links": chain,
                          "note": "SO-3's direct upstream is SO-3aR2 and SO-3aR2 carries "
    ...
    return {"verdict": "SATISFIED", "clauses": 6, "upstream": p,
            "note": ("the SHIPPED GATE FAIL travels with every claim SO-3 makes; the ruling calls "
...
def grade(root):
    ...
    # gate in this file and would be SO-3aR2's number wearing SO-3's arm name.
    ...
    graded_k = {}
    # SO-3 REGISTERED DELTA: `d_ref` is carried beside the index, because the
    ...
    for rk in ROWS:
        ...
        # ---- iteration 0 and what SO-3 produces at the optimum.  It is NOT the
    ...
    # ---- so3_stop_marker.sh:97,105 reads them there; `gates["G5J"]` carries the
    # ---- literal key so3_stop_marker.sh:101 looks for.
    return {"item": "CURRICULUM-%s" % ITEM, "verdict": verdict,
            ...
            "capability_grid_cell": (
                ...
                "SO-3 sentence says `2-3 Mach/alpha` and THIS ITEM ANSWERS THE ALPHA HALF ONLY), "
                "np != 1 (registered at np = 1; DAFOAM section 5 forbids carrying the FD "
                "reference across np), and TRIMMED LIFT (CL is unconstrained here, so a drag "
                "reduction may have been bought by shedding lift and the CL pair travels with "
                "every drag number)")}
...
LAUNCHER_PATH = os.path.join(_HERE, "so3_run_arm.sh")
...
def _ledger_rows_via_launcher(k):
    ...
    OWN `so3_ledger_row` -- the real producer's code -- not by a copy of its
    ...
    for arm in ARMS_DECLARED:
        ...
        cmd = ('. %s --source-only >/dev/null 2>&1; '
               'so3_ledger_row %s %s img %s %d 60 %d %s %s 600 %s 12g "%s %s" 20.00 "%s" "%s" '
               '"%s" "" "" %s x'
...
def _fix(tmp, tweak=None):
    ...
    EVERY X AND F ARTEFACT IS WRITTEN BY `so3_xf`'s OWN WRITERS and EVERY LEDGER
    ...
    k = {"rc": {a: 0 for a in ARMS_DECLARED}, "ke": {},
         ...
         # ---- SO-3's OWN FIXTURE KNOBS: the optimisation arm's shape.
    ...
    def _dp(kk, a):
        ...
        return {"design_point": w,
                "xopt_file": ("so3_xopt.json" if w == "FINAL" else None),
    ...
    for arm in ARMS_DECLARED:
        ...
        if arm == "MESH":
            ...
            if arm.startswith("O-"):
                ...
                # and idwarp), so `so3_runScript_selftest.py` drives its
                ...
                stall = {"stall": stall_word, "n_majors_parsed": nm,
                         ...
                         "condition_B_inf_du_nondecreasing": {"fired": False,
                                                              "ends_at_major": None,
                                                              "window": None}}
                rec = {"item": "SO-3", "row": rk, "libidwarp_so_md5": k["so"][arm],
                       ...
                       "final": {"J": fin_J,
                                 ...
                                 "CL": [v * 0.97 for v in k["cl_base"]]},
                       "shape_final": [0.001 * (i + 1) for i in range(8)],
                       "weighted_drag_reduction_pct": red,
                       "CL_baseline": list(k["cl_base"]),
                       "CL_final": [v * 0.97 for v in k["cl_base"]],
    ...
    if isinstance(rows_out, str):
        ...
        READERS["R1_read_ledger"]["not_born_reason"] = (
            "so3_run_arm.sh:so3_ledger_row could not be sourced (%s); the fixture fell back to a "
...
# ===================== the selftest ===================================================
STOP_MARKER_PATH = os.path.join(_HERE, "so3_stop_marker.sh")


def _write_grade(root, rec, name="SO3_grade_selftest.json"):
...
def stop_marker_basename():
    ...
    This lane's first draft hardcoded `SO3_STOP_MARKER.json`; the frozen writer
    writes `SO3_STOP_MARKER.json` (so3_stop_marker.sh:59), so the check looked
...
def _run_stop_marker(root, gpath, executed=N_DECLARED):
    ...
    if not os.path.isfile(STOP_MARKER_PATH):
        return {"state": "NOT EXERCISED -- so3_stop_marker.sh absent", "rc": None, "marker": None}
...
def selftest(tmp):
    ...
    unit("U11 SCHEMA the FROZEN so3_stop_marker.sh reads this artefact's verdict",
    ...
    unit("U11b SCHEMA the marker ADDRESS is read out of the frozen writer, never guessed",
         stop_marker_basename() == "SO3_STOP_MARKER.json"
         and sm["marker_path"].endswith("SO3_STOP_MARKER.json"))
    ...
    gp_b = _write_grade(root0, r_break, "SO3_grade_schemabreak.json")
    ...
    gp_b2 = _write_grade(root0, r_break2, "SO3_grade_verdictbreak.json")
    ...
    # ---- SO-3's PLANT-SIZING UNITS.  The plant is RELATIVE to the band it must
    ...
    present, absent = so3_instrument_files()
    sweep = {os.path.basename(f): row_label_call_sites(f) for f in present}
    unit("U100 ROW-LABEL every built SO-3 instrument is free of short-form row "
    ...
    # so3_chain_driver.sh's `img_of` through eight files of sweeping.
...
def main():
    ...
    for i, a in enumerate(sys.argv):
        if a == "--root" and i + 1 < len(sys.argv):
            root = sys.argv[i + 1]
        # `--out` IS THE FROZEN DRIVER'S CONTRACT: so3_chain_driver.sh:234 invokes
    ...
    if out is None:
        ...
        out = os.path.join(root, "SO3_grade_%s.json" % stamp)
