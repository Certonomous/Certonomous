#!/usr/bin/env python3
"""SO-3 age guard: the SO-3aR2 datum form PLUS a manifest by NAME AND CONTENT HASH.
...
CASE**, and SO-3aR2 already solved the DAFoam-specific half of it:
  ...
  * the name actually used is RECORDED (`.so3_age_datum_ref`) rather than left
...
differences that matter:
  ...
  * **`opt_IPOPT.txt`, `SO3_STALL_ABORT`, `so3_O.json`, `so3_xopt.json`,
    `mphys.html`, `OptView.hst`, `reports/` are WRITE TARGETS** -- this item runs
    an optimiser and its ancestor did not.
  * **`so3_xopt.json` IS A WRITE TARGET ON THE O ARM AND A PINNED INPUT ON THE
...
# pinned by it.
_LAUNCHER_WRITES = frozenset({".so3_manifest.json"})

# The default role of `so3_xopt.json`: a WRITE TARGET (the O arm produces it).
...
def is_write_target(rel, xopt_is_input=False):
    ...
    if head.startswith("._"):           # AppleDouble spill
        return True
    if head.startswith(".so3_"):        # the launcher's own datum sidecars
        return True
...
def cmd_pin(case_dir, manifest_out, xopt_is_input=False):
    ...
    for must_pin in ("system/fvSchemes", "system/fvSolution", "0.orig/U",
                     "FFD/wingFFD.xyz", "so3_runScript.py", "so3_xf.py"):
...
def cmd_verify(case_dir, manifest_path, datum_path, artifacts):
    ...
    #     `so3_xopt.json` -- read from the manifest, never re-guessed here.
...
def cmd_selftest():
    ...
    must_exclude = ["0/U", "0/U.gz", "0/p", "0.0001/U.gz", "mp0/0/U.gz",
                    "mp2/constant/polyMesh/points.gz", "mp1/so3_cmd.sh",
                    "processor0/0/U.gz", "constant/polyMesh/points",
                    "system/decomposeParDict", "opt_IPOPT.txt", "so3_O.json",
                    "so3_X.json", "so3_F.jsonl", "SO3_STALL_ABORT",
                    "reports/r.html", "mphys.html", "OptView.hst",
                    "checkMesh.log", ".so3_age_datum", ".so3_manifest.json"]
    must_pin = ["system/fvSchemes", "system/controlDict", "system/fvSolution",
                ...
                "preProcessing.sh", "so3_runScript.py", "so3_xf.py",
                "so3_stall.py"]
    ...
    print("\n[a2] so3_xopt.json's role is PASSED IN, never guessed")
    good = is_write_target(XOPT, xopt_is_input=False)
    print("  %-52s %s" % ("O arm: so3_xopt.json is a WRITE TARGET",
                          "PASS" if good else "FAIL"))
    ok &= good
    good = not is_write_target(XOPT, xopt_is_input=True)
    print("  %-52s %s" % ("endpoint arm: so3_xopt.json is PINNED",
                          "PASS" if good else "FAIL"))
    ok &= good

    tmp = tempfile.mkdtemp(prefix="so3_guard_selftest_")
    try:
        ...
        for rel, body in (("system/fvSchemes", "schemes"),
                          ...
                          ("FFD/wingFFD.xyz", "ffd"),
                          ("so3_runScript.py", "producer"),
                          ("so3_xf.py", "instrument"),
                          ("so3_stall.py", "detector"),
                          ("0/U.gz", "field-U"),
                          ("mp0/0/U.gz", "mp0-U"),
                          ("mp1/0/U.gz", "mp1-U")):
        ...
        good = ("0/U.gz" not in m["pinned"] and "mp0/0/U.gz" not in m["pinned"]
                ...
                and "0.orig/U" in m["pinned"] and "so3_runScript.py" in m["pinned"])
        ...
        # (xopt an input) verified with that role must accept `so3_xopt.json`
        ...
        print("  %-52s %s" % ("endpoint manifest PINS so3_xopt.json",
                              "PASS" if good else "FAIL"))
        ok &= good
...
def main(argv):
    ...
    if len(a) == 2 and a[0] == "census":
        ...
        return 0
    print(__doc__)
    print("usage: so3_age_guard.py pin <case_dir> <manifest.json> [--xopt-is-input]\n"
          "       so3_age_guard.py verify <case_dir> <manifest.json> <datum_file> <artifact>...\n"
          "       so3_age_guard.py census <case_dir> [--xopt-is-input]\n"
          "       so3_age_guard.py selftest")
    return 1
