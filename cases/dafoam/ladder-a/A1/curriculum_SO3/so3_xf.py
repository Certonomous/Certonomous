#!/usr/bin/env python
"""Curriculum SO-3 -- NACA0012, THE ALPHA-MULTIPOINT WEIGHTED OBJECTIVE
...
with EXACTLY the registered deltas below and no other.
  ...
  * `patchV` IS NOT A DESIGN VARIABLE IN THIS ITEM and is REMOVED from
    ...
    SO-3's structural gate is G-MP-STRUCT, on the assembly identity, and the
  ...
  * THE STEP-BASED TRIVIAL BASELINE IS BACK, at h = 1e-8, and that is a
    ...
    bit-repeatable and h = 1e-8 never enters subtractive cancellation.  SO-3's
  ...
  * Printed tokens SO2A_* -> SO3_*; artefact names so2a_* -> so3_*.
...
PRODUCER = "so3_runScript.py"
# ---- THE PRODUCER PIN.  THIS CONSTANT IS THE WHOLE REASON SO-3 EXISTS.
# ----
...
# ---- fourteenth that was not.  `so3_pin_census.py` sweeps BY ROLE and
# ---- asserts, in A2, that a pin of this kind holds a real md5.
# ----
# ---- SET, and equal to md5(so3_runScript.py).  It is asserted by A3 of the
# ---- census on every drive, and DRIVEN in both directions by (p1)-(p4) of
# ---- `so3_groot5_selftest.sh`: REFUSE on a mutated producer, PASS on the
...
JSONL_F = "so3_F.jsonl"
JSONL_X = "so3_X.jsonl"
TERMINAL_X = "SO3_X_WRITTEN"
TERMINAL_F = "SO3_F_WRITTEN"
...
def build_X_record(nprocs, producer_md5, identity, mp_identity, adjoint,
    ...
    return {"item": ITEM, "mode": "X", "producer_md5": producer_md5, "nprocs": nprocs,
            ...
            "optimiser": None,
            "optimiser_note": ("G-NOOPT-ENDPOINT: NO OPTIMISER RUNS IN THIS ARM.  SO-3 is an "
                               ...
                               "so3_O.json and it is graded under DAFOAM_CHARTER section 9, not here")}
...
def parse_mode(argv):
    ...
    for i, a in enumerate(argv):
        if a == "-mode" and i + 1 < len(argv):
            mode = argv[i + 1]
    if mode not in ("X", "F"):
        sys.stderr.write("SO3_XF usage: so3_xf.py -mode X|F [-xopt so3_xopt.json]\n")
        sys.exit(64)
    return mode
...
def parse_xopt(argv):
    ...
    SO-3 REGISTERED DELTA.  `DAFOAM_CHARTER.md` section 9 requires the FD check
    ...
    only; this argument is how SO-3 supplies the other end."""
    for i, a in enumerate(argv):
...
def load_xopt(path, ident):
    ...
    if row_here is None:
        sys.stderr.write("SO3_XF REFUSE_XOPT_ROW libidwarp.so md5 %r matches neither "
                         "registered toolchain; this instrument will not verify an "
                         "endpoint it cannot attribute\n" % (got_md5,))
        sys.exit(2)
    if rec.get("row") != row_here:
        sys.stderr.write("SO3_XF REFUSE_XOPT_ROW %s is stamped row=%r but this "
...
def main():
    ...
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("SO3_XF REFUSE producer md5 %s != frozen %s\n"
                         % (got, PRODUCER_MD5))
        sys.exit(2)
    ...
    if src.count(ANCHOR) != 1:
        sys.stderr.write("SO3_XF REFUSE anchor %r appears %d times\n"
    ...
    if xopt_path:
        ...
        if xshape.shape != cur.shape:
            sys.stderr.write("SO3_XF REFUSE_XOPT_SHAPE %r vs %r\n"
    ...
    if rank == 0:
        ...
        if not ok:
            sys.stderr.write("SO3_XF REFUSE planted-zero control not seen on read-back: %s\n"
                             % json.dumps(detail, sort_keys=True, default=str))
            sys.exit(2)
        sys.stdout.write("SO3_PLANTED_ZERO_CONTROL_SEEN n_zero_entries=%d want=%r\n"
