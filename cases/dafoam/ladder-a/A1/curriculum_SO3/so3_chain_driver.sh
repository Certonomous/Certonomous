#!/usr/bin/env bash
# SO-3 chain driver -- DERIVED from curriculum_SO2a/so2a_chain_driver.sh
# (md5 asserted in so3_chain_driver_DELTAS_from_so2a.diff) with the REGISTERED
...
#   (a) NAMES: so2a_ -> so3_, SO2A_ -> SO3_, SO2a -> SO3; run root
#       CURRICULUM-SO3-a1-naca0012-alpha-multipoint-optimisation; the instrument is
#       so3_xf.py (modes X and F), NOT so2a_xg.py.
#   (b) ARMS: MESH O-S XE-S FE-S O-P XE-P FE-P.  DECLARED = 7, BOTH ROWS.
#       BOTH ROWS RUN, and the two-row verdict DAFOAM_CHARTER section 6 demands
#       is nearly free here: SO-3aR2's own MEASURED arm costs make SHIPPED the
#       CHEAPER side, not the dearer -- shipped 4.367 + 2.517 core-min against
#       patched 4.750 + 2.517.  A WORSE OR FAILED SHIPPED OPTIMISATION IS THE
#       RESULT, not waste: it is the finding this family exists to produce, and
#       SO-3aR2's own SHIPPED row already GATE FAILed at 31.498 % aggregate.
#       ORDER: MESH first (one mesh, generated once, in the SHIPPED image), then
#       the SHIPPED triple, then the PATCHED triple.  Each row's XE/FE arms read
#       THEIR OWN row's optimum, so O must precede XE/FE within a row -- that
#       dependency is enforced in so3_run_arm.sh:xopt_arm_of, not by this order.
...
#       THE EXPIRY TERMINATES.  IT DOES NOT BLOCK-AND-CONTINUE.  SO-3 registers
...
AGG_SCRIPT="$HERE/so3_aggregate_memory.py"
STOP_MARKER="$HERE/so3_stop_marker.sh"
...
DECLARED_ARMS=7                          # (e) section 5 Requirement 4
#
...
MD5_STOP_MARKER=4809ff569def1927e516685bb218e7bc  # so3_stop_marker.sh
MD5_STALL=c0719b7fad530a34391280139fe7aa6c       # so3_stall.py
MD5_AGEGUARD=1bcbe57cb708d8e6a7a89ceb2235a35c    # so3_age_guard.py
...
test $# -ge 1 || { echo "ABORT usage: so3_chain_driver.sh <MESH|O-S|XE-S|FE-S|O-P|XE-P|FE-P ...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/so3_driver.pid"
cd "$HERE" || exit 4
EXECUTED=0
...
for _dep in "$LAUNCHER" "$GRADER" "$AGG_SCRIPT" "$STOP_MARKER" \
            "$HERE/so3_runScript.py" "$HERE/so3_xf.py" "$HERE/so3_decomposeParDict" \
            "$HERE/so3_stall.py" "$HERE/so3_age_guard.py"; do
  test -f "$_dep" || { echo "ABORT section 18.3 dependency ABSENT before any md5: $_dep"; exit 4; }
done
echo "SO3_DEPS_EXIST n=9 checked_before_any_md5=yes"
...
echo "$MD5_STALL  $HERE/so3_stall.py" | md5sum -c - || { echo "ABORT stall-detector md5 drifted or UNSET"; exit 4; }
echo "$MD5_AGEGUARD  $HERE/so3_age_guard.py" | md5sum -c - || { echo "ABORT age-guard md5 drifted or UNSET"; exit 4; }
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$HERE/so3_decomposeParDict" "$BASE/base/system/decomposeParDict" || { echo "ABORT overlay decomposeParDict"; exit 4; }
  cp -a "$HERE/so3_runScript.py" "$HERE/so3_xf.py" "$HERE/so3_stall.py" "$HERE/so3_age_guard.py" \
       "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  echo "ITEM=SO3" > "$BASE/ledger.txt"
  ...
  echo "SO3_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "SO3_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT  $BASE/so3_runScript.py"; echo "$MD5_XF  $BASE/so3_xf.py"; echo "$MD5_DECOMP  $BASE/base/system/decomposeParDict";
  echo "$MD5_STALL  $BASE/so3_stall.py"; echo "$MD5_AGEGUARD  $BASE/so3_age_guard.py";
...
echo "SO3_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] declared=$DECLARED_ARMS permission=$PERMISSION"
...
# that so3_run_arm.sh's own G-ROW comment names as forbidden, in the words of the
...
for ARM in $ARMS; do
  ...
  while true; do
    ...
    echo "SO3_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW waited_s=$H5_WAITED file=$(basename "$H5_FILE")"
  ...
  echo "SO3_AGGREGATE arm=$ARM waited=$WAITED $AGG_JSON"
  echo "SO3_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "SO3_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
GRADE_OUT="$BASE/SO3_grade_${GSTAMP}.json"
if echo "$MD5_GRADER  $GRADER" | md5sum -c - > /dev/null; then
  python3 "$GRADER" --root "$BASE" --out "$GRADE_OUT" > "$BASE/SO3_grade_${GSTAMP}.out" 2>&1; GRC=$?
  echo "grader_rc=$GRC stamp=$GSTAMP out=SO3_grade_${GSTAMP}.json note=comparator-exit-status-NOT-the-verdict" >> "$STATUS"
else
...
bash "$STOP_MARKER" "$BASE" "$CHAIN_RC" "$DECLARED_ARMS" "$EXECUTED" "$GRADE_OUT" \
  || echo "SO3_STOP_MARKER_FAILED rc=$? -- the marker did not land; the successor has no fixed address for this run"
echo "SO3_DRIVER end=$(date -u +%Y%m%dT%H%M%SZ) chain_rc=$CHAIN_RC declared=$DECLARED_ARMS executed=$EXECUTED"
exit "$CHAIN_RC"
