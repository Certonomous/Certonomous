#!/usr/bin/env python3
"""SO-3 PIN CENSUS -- A SWEEP BY **ROLE**, NEVER BY THE SHAPE OF A VALUE.

WHY THIS FILE EXISTS, AND IT IS THE WHOLE OF SO-3's REASON TO EXIST
====================================================================
...
SCOPE.  This file is an INSTRUMENT of curriculum SO-3 and speaks for no other
...
SOURCES = [
    ...
    "so3_grade.py",
    "so3_runScript.py",
    "so3_aggregate_memory.py",
    "so3_stop_marker.sh",
    "so3_xf_selftest.py",
    "so3_pin_census.py",
    # ---- SO-3's OWN INSTRUMENTS.  Each joins the census in the commit that
    # ---- creates it, because a file that executes in this item and is not
    # ---- swept is exactly the hole SO-3a's sentinel went through: the census
    # ---- reported "13 declared, 13 driven", was right about thirteen constants
    # ---- in ONE file, and was silent about a fourteenth in a file it did not
    # ---- read.
    "so3_stall.py",
    "so3_age_guard.py",
    "so3_run_arm_selftest.sh",
    "so3_chain_driver_selftest.sh",
    "so3_runScript_selftest.py",
    "so3_grade_selftest.py",
    # The path census.  It carries no md5 pin, but it carries the PATHS -- and a
    # file that cites a run root and is not swept is how a falsified citation
    # travels three generations.  It joins on the same rule as everything else:
    # a file that executes in this item is swept.
    "so3_path_census.py",
    # The one-shot re-pinning script.  It is SWEPT rather than excluded because
    # it is the only thing that WRITES a pin, and a writer that could be edited
    # to write a constant of its own is precisely a pin site.
    "so3_repin.sh",
]
# ---- REMOVED FROM THE PARENT'S LIST, AND THE REMOVAL IS STATED RATHER THAN
# ---- SILENT.  SO-3aR2 swept `so3ar2_groot5_selftest.sh` and
# ---- `so3ar2_collision_leg.py`; SO-3 DOES NOT BUILD EITHER, and its own
# ---- ROW-LABEL sweep already reports them as `NOT YET BUILT` rather than
# ---- pretending to have swept them.  Leaving them in the list would have made
# ---- this census REFUSE on a file that does not exist -- which it did, on the
# ---- first drive, and that refusal is the census working.
SOURCES_RETIRED_FROM_PARENT = {
    "so3_groot5_selftest.sh": "SO-3aR2's 60 KB shell suite is NOT derived.  SO-3's "
                              "launcher legs live in so3_run_arm_selftest.sh, which "
                              "IS swept, and its chain legs in "
                              "so3_chain_driver_selftest.sh, which IS swept.",
    "so3_collision_leg.py":   "the per-point run_directory repair it proved is "
                              "INHERITED from SO-3aR2 unchanged and was graded there "
                              "(P-COLL HIT).  SO-3 re-proves nothing it did not "
                              "change; the invariant that RUN_DIRS is total and "
                              "injective over SCENARIOS is driven by "
                              "so3_runScript_selftest.py leg (p3).",
}
...
SOURCES_EXCLUDED = {
    "so3_decomposeParDict": "an OpenFOAM dictionary, not a program; it holds "
                              "no constants and executes nothing",
    "so3_derive_from_so3ar2.sh": "the one-shot derivation script; it runs "
...
#   CONTAINER_KEYED  the md5 is the KEY and the value is a ROW LABEL.  Both
#              halves are asserted: the key must be 32 hex, the value must be one
#              of the two registered row labels.  Used by the two md5 -> row
#              lookup tables through which the producer and the endpoint
#              instrument prove their row from the library's own bytes.
...
DISPOSITION = {
    ...
    "so3_chain_driver.sh:MD5_LAUNCHER":     ("LOCAL", "so3_run_arm.sh"),
    "so3_chain_driver.sh:MD5_GRADER":       ("LOCAL", "so3_grade.py"),
    "so3_chain_driver.sh:MD5_RUNSCRIPT":    ("LOCAL", "so3_runScript.py"),
    "so3_chain_driver.sh:MD5_XF":           ("LOCAL", "so3_xf.py"),
    "so3_chain_driver.sh:MD5_AGG":          ("LOCAL", "so3_aggregate_memory.py"),
    "so3_chain_driver.sh:MD5_DECOMP":       ("LOCAL", "so3_decomposeParDict"),
    "so3_chain_driver.sh:MD5_STOP_MARKER":  ("LOCAL", "so3_stop_marker.sh"),
    # ---- the chain driver's out-of-tree tutorial pins ----------------------
    "so3_chain_driver.sh:MD5_TUT_RUNSCRIPT": ("TUTORIAL", "runScript.py"),
    "so3_chain_driver.sh:MD5_TUT_GEN":       ("TUTORIAL", "genAirFoilMesh.py"),
    "so3_chain_driver.sh:MD5_TUT_PREPROC":   ("TUTORIAL", "preProcessing.sh"),
    "so3_chain_driver.sh:MD5_TUT_PS":        ("TUTORIAL", "profiles/NACA0012PS.profile"),
    "so3_chain_driver.sh:MD5_TUT_SS":        ("TUTORIAL", "profiles/NACA0012SS.profile"),
    "so3_chain_driver.sh:MD5_TUT_FFD":       ("TUTORIAL", "FFD/wingFFD.xyz"),
    ...
    "so3_run_arm.sh:MD5_RUNSCRIPT":         ("LOCAL", "so3_runScript.py"),
    "so3_run_arm.sh:MD5_XF":                ("LOCAL", "so3_xf.py"),
    "so3_run_arm.sh:MD5_DECOMP":            ("LOCAL", "so3_decomposeParDict"),
    ...
    "so3_xf.py:PRODUCER_MD5":               ("LOCAL", "so3_runScript.py"),
    # ---- SO-3'S TWO NEW CHAIN PINS.  They join the census in the commit that
    # ---- creates them; a file that executes in this item and is not pinned is
    # ---- how SO-2a read "eight of eight AGREE" over an absent file.
    "so3_chain_driver.sh:MD5_STALL":        ("LOCAL", "so3_stall.py"),
    "so3_chain_driver.sh:MD5_AGEGUARD":     ("LOCAL", "so3_age_guard.py"),
    # ---- THE TWO ROW-LOOKUP TABLES.  SO-3 REGISTERED ADDITION.  Both are keyed
    # ---- BY the libidwarp.so md5 the container prints, so the KEY is a
    # ---- CONTAINER pin and the VALUE is a row label.  This is how the producer
    # ---- and the endpoint instrument prove which row they are on WITHOUT a
    # ---- flag -- `DAFOAM_CHARTER.md` section 11 makes the library hash the
    # ---- identity, and a `-row PATCHED` flag would let a mislabelled launch
    # ---- stamp the wrong row into the artefact the endpoint arm then reads.
    # ---- They hold the SAME two values as so3_grade.py:SO_MD5[*], and A2 checks
    # ---- that; a third value appearing in either table is an unregistered
    # ---- toolchain and A1 refuses it.
    "so3_runScript.py:SO_MD5_ROW[85f59e87253e0a71a813f64ca6e4c425]":
        ("CONTAINER_KEYED", "libidwarp.so @ PATCHED image -> row label"),
    "so3_runScript.py:SO_MD5_ROW[f0fcb488e0e98156575cd19548e91663]":
        ("CONTAINER_KEYED", "libidwarp.so @ SHIPPED image -> row label"),
    "so3_xf.py:ROW_BY_SO_MD5[85f59e87253e0a71a813f64ca6e4c425]":
        ("CONTAINER_KEYED", "libidwarp.so @ PATCHED image -> row label"),
    "so3_xf.py:ROW_BY_SO_MD5[f0fcb488e0e98156575cd19548e91663]":
        ("CONTAINER_KEYED", "libidwarp.so @ SHIPPED image -> row label"),
    ...
    "so3_grade.py:SO_MD5[SHIPPED]":         ("CONTAINER", "libidwarp.so @ SHIPPED image"),
    "so3_grade.py:SO_MD5[PATCHED]":         ("CONTAINER", "libidwarp.so @ PATCHED image"),
    # ---- image digests: sha256, not md5, and an image is not a file ---------
    "so3_grade.py:IMG_DIGEST[SHIPPED]":     ("DIGEST", "dafoam/opt-packages SHIPPED image"),
    "so3_grade.py:IMG_DIGEST[PATCHED]":     ("DIGEST", "dafoam/opt-packages PATCHED image"),
    "so3_run_arm.sh:IMG_SHIPPED_DIGEST":    ("DIGEST", "dafoam/opt-packages SHIPPED image"),
    "so3_run_arm.sh:IMG_PATCHED_DIGEST":    ("DIGEST", "dafoam/opt-packages PATCHED image"),
}

# A pin whose in-repo file is STAGED into the run root under a DIFFERENT name.
# `so3_decomposeParDict` is copied to `base/system/decomposeParDict` because
...
STAGED_AS = {
    "so3_decomposeParDict": ["decomposeParDict"],
}
...
def python_consumers(sources_text, pin_ids):
    ...
    for fname, text in sources_text.items():
        ...
        # ---- A DICT LOOKUP IS A CONSUMPTION, AND `TABLE.get(x)` IS THE FORM
        # ---- THIS CENSUS COULD NOT SEE.  SO-3 introduced two md5-keyed ROW
        # ---- LOOKUP TABLES -- `so3_runScript.py:SO_MD5_ROW` and
        # ---- `so3_xf.py:ROW_BY_SO_MD5` -- whose entries are consumed by
        # ---- `TABLE.get(md5)` and never by a literal subscript.  The census
        # ---- called all four DEAD, which was a defect in the CENSUS's rule set
        # ---- and not in the code: exactly the error this file's own docstring
        # ---- warns about ("a census reporting a defect that is an artefact of
        # ---- its own rule set").  Recognising the form is the repair;
        # ---- registering an exemption would have been the weakening.
        for node in ast.walk(tree):
            if not (isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute)
                    and node.func.attr in ("get", "__getitem__")
                    and isinstance(node.func.value, ast.Name)):
                continue
            stem = "%s:%s" % (fname, node.func.value.id)
            hit = [p for p in pin_ids if p.startswith(stem + "[")]
            for pp in hit:
                sites.append({"file": fname, "line": node.lineno,
                              "pin": pp, "kind": "pin-operand-dict-lookup"})
...
def run(root, tut_src_override=None):
    ...
    present = sorted(f for f in os.listdir(root)
                     if f.startswith("so3_") and not f.endswith(".json")
    ...
    if tut_src is None:
        m = re.search(r"^TUT_SRC=(\S+)", sources_text["so3_chain_driver.sh"], re.M)
        tut_src = m.group(1).strip("\"'") if m else None
    say("TUT_SRC (read from so3_chain_driver.sh, not hard-coded) = %s" % tut_src)
    ...
    for pid in sorted(pins):
        ...
        kind, target = row
        note = ""
        # ---- CONTAINER_KEYED: THE ONE PIN SHAPE IN THIS ITEM WHERE THE md5 IS
        # ---- THE KEY AND THE VALUE IS A ROW LABEL.  SO-3's producer and its
        # ---- endpoint instrument prove which row they are on from the LIBRARY'S
        # ---- OWN BYTES -- `DAFOAM_CHARTER.md` section 11 makes the hash the
        # ---- identity -- so the table reads md5 -> row and its VALUE is
        # ---- correctly not an md5.  Registering these as CONTAINER made A2 go
        # ---- red on four correct pins, which is a census reporting a defect
        # ---- that is an artefact of its own rule set.  The repair is a KIND
        # ---- that asserts what is actually true of them, and it asserts MORE
        # ---- than CONTAINER did, not less: the key must be a real md5 AND the
        # ---- value must be one of the two registered row labels, so a third
        # ---- toolchain or a mistyped row cannot enter either table.
        if kind == "CONTAINER_KEYED":
            key = pid.split("[", 1)[1].rstrip("]") if "[" in pid else ""
            if not MD5_RE.match(key):
                fail.append("A2 KEY NOT AN MD5: %s at %s:%d -- a CONTAINER_KEYED "
                            "pin is keyed BY the library md5 the container prints"
                            % (pid, fname, line))
                note = "*** KEY NOT AN MD5 ***"
            elif value not in ("SHIPPED", "PATCHED"):
                fail.append("A2 VALUE NOT A REGISTERED ROW: %s = %r at %s:%d -- "
                            "the two row labels are the only legal values and "
                            "there is no short form" % (pid, value, fname, line))
                note = "*** NOT A ROW LABEL ***"
            else:
                note = "key = md5(%s)" % target
