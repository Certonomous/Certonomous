...
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D7FR-a3-m6-fd
...
echo "D7FR_G_ROOT_PASS item=$ITEM base=$BASE_REAL"
RANKS=4
...
# THE GRADER CHECKS THESE AGAINST THE DOCUMENT.  `d7fr_grade.py`'s
...
# inherited ones are.
MD5_LOCUS=ef2941bd8c4ba71ecb000de02799bba0
MD5_PHYSICAL=26b8265f30bcdd1a7612cd4a5d030e7f
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: d7fr_run_arm.sh <P1|X|ACC|F-S|F-P> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: d7fr_run_arm.sh <P1|X|ACC|F-S|F-P> <image>"; exit 64; }
...
if ls "$BASE/${ARM}_"*.log.ok.* "$BASE/${ARM}_"*.log.fail.* >/dev/null 2>&1; then
  echo "ABORT G-ROOT.4 arm $ARM carries a COMPLETION MARKER (.ok or .fail) in $BASE."
  echo "  That arm completed.  REFUSED before any deletion."
  exit 3
fi
echo "D7FR_G_ROOT4_PASS arm=$ARM no_prior_row no_success_marker"
...
echo "$MD5_LOCUS  $BASE/d7fr_endpoint_locus.py"       | md5sum -c - || { echo "ABORT locus md5"; exit 4; }
echo "$MD5_PHYSICAL  $BASE/d7fr_endpoint_physical.py" | md5sum -c - || { echo "ABORT physical md5"; exit 4; }
...
cp -a "$BASE/d7_opt_runScript.py" "$BASE/d7_fd_endpoint.py" "$BASE/d7_extract_endpoint.py" \
      "$BASE/d7fr_endpoint_locus.py" "$BASE/d7fr_endpoint_physical.py" \
      "$BASE/d7fr_accept_primal.py" "$BASE/d7fr_accept_compare.py" "$WORK/" \
...
for bad in "$WORK/reports" "$WORK/OptView.hst" "$WORK/opt_IPOPT.txt" \
           ...
           "$WORK/d7fr_accept_primal.json" "$WORK/d7fr_accept_verdict.json"; do
...
stage_endpoint() {
  ...
  echo "D7FR_H4_PASS arm=$ARM endpoint=D7R/O OptView.hst=$MD5_OPTVIEW opt_IPOPT.txt=$MD5_IPOPT"
}
...
case "$ARM" in
  ...
  F-S|F-P)
    ...
    test -f "$BASE/ACC/d7fr_accept_verdict.json" || { echo "ABORT arm $ARM requires ACC-1 to have run; $BASE/ACC/d7fr_accept_verdict.json absent (LIMIT 1)"; exit 5; }
    grep -q '"verdict": "PASS"' "$BASE/ACC/d7fr_accept_verdict.json" || { echo "ABORT arm $ARM: ACC-1 did not PASS, so the repair is NOT FROZEN and no FD number from it may be graded (LIMIT 1)"; exit 5; }
    echo "D7FR_LIMIT1_PASS $(python3 -c "import json;d=json.load(open('$BASE/ACC/d7fr_accept_verdict.json'));print('rel_CD=%r band=%r verdict=%s'%(d['rel_CD'],d['band_ACC1'],d['verdict']))")"
...
\" && python d7fr_endpoint_physical.py --age-datum $AGE_DATUM && python d7fr_endpoint_locus.py --gate . --runscript d7_opt_runScript.py --out d7fr_locus_gate.json" ;;
  ACC) CMD="rm -f d7_placement_rank*.json && mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings python -c \"
import json,os
from mpi4py import MPI
r=MPI.COMM_WORLD.rank
json.dump({'rank':r,'affinity':sorted(os.sched_getaffinity(0)),'n_cores':len(os.sched_getaffinity(0)),'pid':os.getpid()}, open('d7_placement_rank%d.json'%r,'w'))
\" && mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d7fr_accept_primal.py && python d7fr_accept_compare.py d7fr_accept_primal.json --out d7fr_accept_verdict.json" ;;
...
#
#   CAP     crossed -> D7FR_CAP_CROSSED written to the ledger, RUN CONTINUES.
...
echo "D7FR_RUNAWAY_GUARD arm=$ARM cap_core_min=$CAP ceiling_core_min=$CEILING mode=report_then_stop_at_ceiling"
CAP_REPORTED=no
CEILING_HIT=no
while true; do
  ...
  if [ "$CAP_REPORTED" = "no" ] && [ "$(python3 -c "print(1 if $CM > $CAP else 0)")" = "1" ]; then
    CAP_REPORTED=yes
    echo "D7FR_CAP_CROSSED arm=$ARM core_min=$CM cap=$CAP ceiling=$CEILING action=REPORTED_RUN_CONTINUES supervisor_decides" | tee -a "$BASE/ledger.txt"
  fi
  if [ "$(python3 -c "print(1 if $CM > $CEILING else 0)")" = "1" ]; then
    CEILING_HIT=yes
    echo "D7FR_CEILING_HIT arm=$ARM core_min=$CM ceiling=$CEILING action=HARD_STOP" | tee -a "$BASE/ledger.txt"
...
if [ "$ARM" = "P2" ] && [ "$rc" = "0" ] && [ -f "$WORK/dRdWColoring_${RANKS}.bin" ]; then
  ...
  echo "item=D7R arm=$ARM rc=$rc stamp=$STAMP md5=$(md5sum "$BASE/dRdWColoring_${RANKS}.bin" | awk '{print $1}')" > "$BASE/.d7fr_coloring_provenance"
  echo "D7FR_COLORING_PUBLISHED_FRESH $(cat "$BASE/.d7fr_coloring_provenance")" | tee -a "$LEDGER"
fi

# ---- THE COMPLETION MARKER, AND IT NOW MEANS WHAT ITS NAME SAYS ----------
# D7F-DEF-1, repaired here at its cause rather than worked around downstream.
# The predecessor wrote `test -s "$LOG" && touch "$LOG.ok.$STAMP"` -- a
# NON-EMPTINESS test wearing a success name.  A container that crashed after
# printing one banner line got a `.ok` marker, and the grader that cited it as
# terminal evidence would have read that crash as clean.
#
# THREE OUTCOMES ARE NOW DISTINGUISHABLE, and that is the point: the third one
# is what a bare `.ok`/no-`.ok` pair could never express.
#   .ok.<stamp>    the producer ran AND returned rc = 0
#   .fail.<stamp>  the producer ran AND returned rc != 0, with the rc IN THE FILE
#   NEITHER        this launcher itself died before it could record either, so
#                  the arm's outcome is UNKNOWN and must not be read as either
if [ "$rc" = "0" ]; then
  echo "rc=0 stamp=$STAMP arm=$ARM" > "$LOG.ok.${STAMP}"
else
  echo "rc=$rc stamp=$STAMP arm=$ARM" > "$LOG.fail.${STAMP}"
fi
