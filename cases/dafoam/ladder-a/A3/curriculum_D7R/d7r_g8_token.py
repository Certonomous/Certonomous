#!/usr/bin/env python3
"""
D7R G8 TOKEN WRITER -- repair of `D7-LAUNCHER-DEF-1`, authorised by the
...
GRADER = os.path.join(HERE, "..", "curriculum_D7", "d7_grade.py")
LAUNCHER = os.path.join(HERE, "d7r_run_arm.sh")

BASE = "/home/ubuntu/certonomous-runs/CURRICULUM-D7R-a3-m6-cdmin"
G8_WORK = os.path.join(BASE, "P1")
TOKEN = os.path.join(BASE, ".d7r_g8_pass")

# PREREGISTRATION.md Addendum 1 sec.A1.2 / Addendum 2 sec.A2.7
REGISTERED_LAUNCHER_MD5 = None   # set at the D7R freeze commit; None = HEAD-blob check only
...
def check_hashes():
    ...
    rel_l = "cases/dafoam/ladder-a/A3/curriculum_D7R/d7r_run_arm.sh"
    ...
    if dl != hl:
        refuse("C2 -- d7_run_arm.sh on disk is NOT the committed instrument",
               {"disk": dl, "HEAD": hl})
    if REGISTERED_LAUNCHER_MD5 is not None and dl != REGISTERED_LAUNCHER_MD5:
        refuse("C2 -- launcher md5 does not match the REGISTERED md5",
