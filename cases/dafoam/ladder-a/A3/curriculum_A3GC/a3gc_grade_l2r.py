...
def read_log(path):
    ...
    last = {}
    # ============================================================
    # A3GC-L2R AMENDMENT -- FIRST MATCH PER EQUATION PER BLOCK.
    #
    # a3gc_grade.py keeps the LAST match here.  That is correct for a log with
    # `nNonOrthogonalCorrectors 0`, where each equation prints exactly once per
    # block, and it stays frozen at md5 73dbe368934956700da87e5a1f44ea0c for the
    # runs it already grades (AR1, AR1C).  IT IS NOT CORRECT HERE.
    #
    # `DAUtility::primalResidualControl` is called INSIDE
    # `while (simple.correctNonOrthogonal())` (pEqnSimple.H:41,52) and prints one
    # `<eq> initRes:` line PER CORRECTOR.  With correctors on, the LAST line is
    # the FINAL corrector's initial residual -- measured off an almost-solved
    # field -- which says nothing about whether the OUTER iteration converged.
    #
    # FIRST is the semantically right quantity BY CONSTRUCTION, not by luck: it
    # is the outer iteration's initial residual BEFORE any corrector has acted,
    # which is exactly what "has this outer iteration converged" means.  The
    # maximum would USUALLY coincide with it, because the first corrector
    # normally carries the largest residual -- but "usually" is a property of
    # the arithmetic on a given day, not of the definition.  FIRST is right even
    # on a block where a later corrector happens to read higher.
    #
    # DIRECTION, AND IT IS ONE-WAY: this reader returns the residual BEFORE the
    # correctors reduced it, so it reads the HARDER number, never the easier
    # one.  THIS AMENDMENT CAN ONLY LOWER A VERDICT, NEVER RAISE ONE.
    # ============================================================
