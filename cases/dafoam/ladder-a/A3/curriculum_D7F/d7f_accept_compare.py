...
Reads `d7f_accept_primal.json` FROM DISK and grades it against the bands
...
def read_artifact(path):
    ...
    with open(path) as fh:
        doc = json.load(fh)
    if doc.get("kind") != "d7f_acceptance_primal":
        refuse("read_artifact", {"kind": doc.get("kind"),
                                 "required": "d7f_acceptance_primal"})
    for k in ("CD", "CL"):
        if k not in doc:
...
def blind_reader(path):
    ...
    return {"kind": "d7f_acceptance_primal", "CD": CD_OPT, "CL": CL_OPT,
...
def main():
    ...
    try:
        ...
        sys.stderr.write("D7F_ACCEPT_COMPARE REFUSE %s\n" % e)
        return 2
