...
IPOPT objective `2.3048932443550496e-02` by `d7f_accept_compare.py`, on a band
...
OUT = "d7f_accept_primal.json"


def refuse(msg):
    sys.stderr.write("D7F_ACCEPT REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    if rank == 0:
        ...
        with open(OUT, "w") as fh:
            ...
            fh.flush()
            os.fsync(fh.fileno())
        sys.stdout.write("D7F_ACCEPT_PRIMAL_WRITTEN %s CD=%r CL=%r wall_s=%.1f\n"
                         % (OUT, cd, cl, wall))
        sys.stdout.flush()
