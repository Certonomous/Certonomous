...
# ================================ GATES ====================================
def g1_completion(base, work, ledger, arms_expected):
    ...
    for arm in arms_expected:
        ...
        if rc != 0:
            ok = False
        # ---- THE TERMINAL CLAUSE, read from the PRODUCER'S OWN LOG FILE ----
        # Not from the ledger's summary of it: the ledger is the launcher's
        # claim, and rule 4 asks what the producer itself wrote.
        logname = rec.get("log")
        lpath = os.path.join(base, logname) if logname else None
        if not lpath or not os.path.isfile(lpath):
            out["arms"][arm]["log_terminal"] = "LOG_ABSENT"
            ok = False
        else:
            ltxt = open(lpath, errors="replace").read()
            has_end = any(l.strip() == "End" for l in ltxt.splitlines())
            marker = [f for f in os.listdir(base)
                      if f.startswith(logname + ".ok.")]
            out["arms"][arm]["log_terminal"] = {
                "log": logname, "End_line": has_end,
                "ok_marker": marker[0] if marker else None}
            if not (has_end and marker):
                ok = False
...
def grade(base, work, out_path, arms, cl_target_path, fd_paths, doc_path):
    ...
    g = {}
    g["G1"] = g1_completion(base, work, ledger, arms)
    ...
    doc = {"item": "curriculum_D7F", "base": base, "work": work,
...
def selftest():
    ...
    try:
        ...
        with open(led_p, "w") as fh:
            fh.write("ARM=O ROW=SHIPPED IMG=i DIGEST=d rc=0 wall_s=100 ranks=4 "
                     ...
                     "log=O.log\n")
        # THE PRODUCER'S OWN LOG, so the terminal clause has something to read.
        with open(os.path.join(tmp, "O.log"), "w") as fh:
            fh.write("Time = 1\nExecutionTime = 1 s\nEnd\n")
        open(os.path.join(tmp, "O.log.ok.STAMP1"), "w").close()
        ...
        unit("G1_FRESH_artifacts_pass_age_guard", True,
             bool(g1_completion(tmp, w, led, ["O"])["pass"]))
        unit("G1_MISSING_arm_is_absent_from_ledger_not_a_pass", True,
             bool(not g1_completion(tmp, w, led, ["O", "F-S"])["pass"]))

        # ---- D7R-GRADER-DEF-7's CLAUSE, DRIVEN IN BOTH DIRECTIONS ---------
        # The clause D7's grader named and never implemented.  Each mutant
        # asserts THAT THE MUTATION APPLIED before asserting the clause fired,
        # so no unit here can pass vacuously (L-335, L-302).
        okmark = os.path.join(tmp, "O.log.ok.STAMP1")
        os.remove(okmark)
        unit("G1_TERMINAL_mutation_applied__ok_marker_removed", False,
             os.path.exists(okmark))
        unit("G1_MISSING_ok_marker_FAILS_the_terminal_clause", True,
             bool(not g1_completion(tmp, w, led, ["O"])["pass"]))
        open(okmark, "w").close()
        logp = os.path.join(tmp, "O.log")
        with open(logp, "w") as fh:
            fh.write("Time = 1\nExecutionTime = 1 s\n")     # no End line
        unit("G1_TERMINAL_mutation_applied__End_line_removed", False,
             any(l.strip() == "End" for l in open(logp).read().splitlines()))
        unit("G1_LOG_WITHOUT_End_FAILS_the_terminal_clause", True,
             bool(not g1_completion(tmp, w, led, ["O"])["pass"]))
        os.remove(logp)
        unit("G1_ABSENT_LOG_FAILS_and_is_not_read_as_clean", True,
             bool(not g1_completion(tmp, w, led, ["O"])["pass"]))
        with open(logp, "w") as fh:
            fh.write("Time = 1\nExecutionTime = 1 s\nEnd\n")
        unit("G1_RESTORED_log_and_marker_pass_again", True,
             bool(g1_completion(tmp, w, led, ["O"])["pass"]),
             "the clause is not a one-way switch")
    ...
    sys.stdout.write("D7F_SELFTEST units=%d passed=%d failed=%d\n"
                     % (n, n_ok, n - n_ok))
    sys.stdout.write("D7F_SELFTEST_COVERAGE emitted=%d [%s]\n"
                     % (len(emitted), ",".join(emitted)))
    sys.stdout.write("D7F_SELFTEST_COVERAGE exercised=%d [%s]\n"
                     % (len(exercised), ",".join(exercised)))
    sys.stdout.write("D7F_SELFTEST_COVERAGE UNEXERCISED=%d [%s]\n"
    ...
    for u in units:
        sys.stdout.write("  %-46s %s  %s\n"
                         % (u["unit"], "ok" if u["ok"] else "FAILED",
                            u["detail"]))
    if n == 0:
        sys.stdout.write("D7F_SELFTEST REFUSE zero units ran -- a selftest that "
...
def main():
    ...
    if a.selftest:
        ...
        # else in this file: a graded run exits 0 (D7F_GRADER OK) or 2
        # (D7F_GRADER REFUSED).
        sys.exit(selftest())

    for req in ("base", "work", "out", "cl_target"):
        if not getattr(a, req.replace("-", "_")):
            sys.stderr.write("D7F_GRADER REFUSED missing --%s\n" % req.replace("_", "-"))
            sys.exit(2)
    fd_paths = {}
    if a.fd_shipped:
    ...
    try:
        ...
        sys.stderr.write("D7F_GRADER REFUSED %s\n" % exc)
        sys.exit(2)
    sys.stdout.write("D7F_GRADER OK verdict=%s out=%s\n"
