...
Self-test: `python3 d7f_endpoint_locus.py --selftest` builds deliberate mutants
...
def selftest():
    ...
    tmp = tempfile.mkdtemp(prefix="d7f_locus_st_")
    hdr = "U0 = 291.6\n"
    ...
    print("\nD7F_LOCUS_SELFTEST %d/%d PASS" % (len(ok), len(ok) + len(fail)))
    if fail:
...
def main():
    ...
    if a.selftest:
        return selftest()
    if not a.gate:
        sys.stderr.write("usage: d7f_endpoint_locus.py --selftest | "
    ...
    try:
        ...
        sys.stderr.write("D7F_LOCUS REFUSE %s\n" % e)
        return 2
