#!/usr/bin/env bash
# =============================================================================
# A3FL1 §2ba DETACHED AUTOGRADER  (mirrors d6rf10_autograde.sh)
# -----------------------------------------------------------------------------
# Polls the A3FL1 run-root ledger for completion, then grades each leg log with
...
# those are the dafoam-supervisor's calls after reading A3FL1_AUTOGRADE_DONE.txt.
...
# 1. Poll for ladder completion, bounded.  KEY ONLY on the ledger's own A3FL1_LADDER_DONE line.
while :; do
  if grep -qE 'A3FL1_LADDER_DONE' "$LEDGER" 2>/dev/null; then
    echo "TERMINATION seen (A3FL1_LADDER_DONE) $(date -u +%FT%TZ)" >> "$LOG"; break
  fi
  now=$(date +%s)
...
if [ "$GOT" != "$GRADER_MD5" ]; then
  ...
  {
    echo "A3FL1 AUTOGRADE REFUSED $(date -u +%FT%TZ)"
...
{
  echo "A3FL1_AUTOGRADE_DONE $(date -u +%FT%TZ)"
  echo "grader=$GRADER md5=$GOT (frozen, VERIFIED)  invocation: --log <leg log> --leg CONTROL|TEST [--fd-json ...]"
