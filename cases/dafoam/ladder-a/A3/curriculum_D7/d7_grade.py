...
CAPS = {"P1": 8.0, "P2": 60.0, "O": 600.0, "F-S": 130.0, "F-P": 130.0}  # sec.8
ITEM_CEILING_CORE_MIN = 928.0      # sec.8
...
# ================================ GATES ====================================
def g1_completion(work, ledger, arms_expected):
...
def g10_caps(ledger, arms_expected):
    """ENFORCED cap == REGISTERED cap, read BACK OUT OF THE LEDGER the launcher
    wrote -- NOT from the launcher's claim.  And actual <= cap.

    Cost constraints are LIFTED (Sanaa, 2026-08-25); caps are RUNAWAY GUARDS.
    A crossing is REPORTED, never silently continued and never silently
    stopped.  This gate reports it."""
    out = {"arms": {}}
    ok = True
    for arm in arms_expected:
        rec = ledger["arms"].get(arm)
        ...
        out["arms"][arm] = {"registered_cap": reg, "ledger_cap": cap,
                            ...
                            "cap_matches_registered": bool(agree),
                            "actual_within_cap": bool(within),
                            "overrun_core_min": (round(act - reg, 3)
                                                 if not within else 0.0)}
        if not agree:
            ok = False
...
# ============================== MAIN =======================================
def grade(base, work, out_path, arms, cl_target_path, fd_paths):
    ledger = read_ledger(os.path.join(base, "ledger.txt"))
    g = {}
    g["G1"] = g1_completion(work, ledger, arms)
    ...
    doc = {"item": "curriculum_D7", "base": base, "work": work,
...
def selftest():
    ...
    try:
        ...
        unit("G1_FRESH_artifacts_pass_age_guard", True,
             bool(g1_completion(w, led, ["O"])["pass"]))
        unit("G1_MISSING_arm_is_absent_from_ledger_not_a_pass", True,
             bool(not g1_completion(w, led, ["O", "F-S"])["pass"]))
        ...
        open(lp, "w").write(
            "ARM=O ROW=SHIPPED IMG=a DIGEST=d rc=0 wall_s=100 ranks=4 core_min=500.0 "
            "cap_core_min=600.0 enforced_core_min=600.000000 log=o.log\n")
        unit("G10_matching_cap_passes", True,
             bool(g10_caps(read_ledger(lp), ["O"])["pass"]))
        open(lp, "w").write(
            "ARM=O ROW=SHIPPED IMG=a DIGEST=d rc=0 wall_s=100 ranks=4 core_min=500.0 "
            "cap_core_min=1200.0 enforced_core_min=1200.000000 log=o.log\n")
        unit("G10_DRIFTED_cap_fails", True,
             bool(not g10_caps(read_ledger(lp), ["O"])["pass"]),
             "enforced 1200 vs registered %s" % CAPS["O"])
        open(lp, "w").write(
            "ARM=O ROW=SHIPPED IMG=a DIGEST=d rc=0 wall_s=100 ranks=4 core_min=700.0 "
            "cap_core_min=600.0 enforced_core_min=600.000000 log=o.log\n")
        r10 = g10_caps(read_ledger(lp), ["O"])
        unit("G10_OVERRUN_reported_as_a_runaway_not_absorbed", True,
             bool(r10["arms"]["O"]["overrun_core_min"] > 0
                  and not r10["arms"]["O"]["actual_within_cap"]),
             "overrun=%.1f core-min" % r10["arms"]["O"]["overrun_core_min"])
    ...
    sys.stdout.write("D7_SELFTEST units=%d passed=%d failed=%d\n"
                     % (n, n_ok, n - n_ok))
    sys.stdout.write("D7_SELFTEST_COVERAGE emitted=%d [%s]\n"
                     % (len(emitted), ",".join(emitted)))
    sys.stdout.write("D7_SELFTEST_COVERAGE exercised=%d [%s]\n"
                     % (len(exercised), ",".join(exercised)))
    sys.stdout.write("D7_SELFTEST_COVERAGE UNEXERCISED=%d [%s]\n"
    ...
    for u in units:
        sys.stdout.write("  %-46s %s  %s\n"
                         % (u["unit"], "ok" if u["ok"] else "FAILED",
                            u["detail"]))
    if n == 0:
        sys.stdout.write("D7_SELFTEST REFUSE zero units ran -- a selftest that "
...
def main():
    ...
    ap.add_argument("--arms", default="P1,P2,O,F-S,F-P")
    ...
    if a.selftest:
        ...
        # else in this file: a graded run exits 0 (D7_GRADER OK) or 2
        # (D7_GRADER REFUSED).
        sys.exit(selftest())

    for req in ("base", "work", "out", "cl_target"):
        if not getattr(a, req.replace("-", "_")):
            sys.stderr.write("D7_GRADER REFUSED missing --%s\n" % req.replace("_", "-"))
            sys.exit(2)
    fd_paths = {}
    if a.fd_shipped:
    ...
    try:
        ...
        sys.stderr.write("D7_GRADER REFUSED %s\n" % exc)
        sys.exit(2)
    sys.stdout.write("D7_GRADER OK verdict=%s out=%s\n"
