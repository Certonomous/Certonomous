...
# until arm P1 has written `.d7_g8_pass`.
stage_coloring() {
...
{
  echo "ARM=$ARM ROW=$ROW IMG=$IMG DIGEST=$GOT_DIGEST rc=$rc wall_s=$WALL ranks=$RANKS core_min=$CORE_MIN cap_core_min=$CAP enforced_wall_s=$TMO enforced_core_min=$BACKCHECK cap_exceeded=$OVER memory=$MEM inspect(exit,oomkilled)=[$INSPECT] memavail_pre_GiB=$MEMAVAIL_GIB memavail_floor_GiB=$FLOOR memavail_post_GiB=$MEMAVAIL_POST memavail_min_during=[$MEMMIN] cpuset=$CPUSET delivered_cores_mean=[$DELIVERED] siblings_pre=[$SIBLINGS_PRE] siblings_post=[$SIBLINGS_POST] log=$(basename "$LOG") stamp=$STAMP"
...
if [ "$ARM" = "P2" ] && [ "$rc" = "0" ] && [ -f "$WORK/dRdWColoring_${RANKS}.bin" ]; then
  ...
  echo "D7_COLORING_PUBLISHED md5=$(md5sum "$BASE/dRdWColoring_${RANKS}.bin" | awk '{print $1}')" | tee -a "$LEDGER"
fi
