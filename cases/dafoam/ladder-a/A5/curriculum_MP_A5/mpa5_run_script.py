...
parser.add_argument("-out", help="JSON record path", type=str, default="mpa5_out.json")
...
if args.dvFile:
    ...
    print("MPA5_DVFILE_LOADED: %s  n=%d  l2=%.10e" % (args.dvFile, len(_dv), float(np.linalg.norm(_dv))))
...
if args.task == "run_driver":
    try:
        prob.driver.add_recorder(om.SqliteRecorder("mpa5_hist.sql"))
    ...
    try:
        prob.cleanup()
        cr = om.CaseReader("mpa5_hist.sql")
...
if MPI.COMM_WORLD.rank == 0:
    ...
    print("MPA5_JSON_WRITTEN: " + os.path.abspath(args.out))
