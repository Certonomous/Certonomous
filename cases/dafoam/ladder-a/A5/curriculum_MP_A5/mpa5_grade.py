...
def rule4(root, arm, stamp):
    ...
    rec = os.path.join(armdir, "mpa5_out.json")
...
def control_record_reader(root, arm):
    ...
    rec = os.path.join(root, arm, "mpa5_out.json")
...
def main():
    ...
    if not (okA and okC):
        out["verdict"] = "REFUSED"
        emit(out, a.out)
        print("MPA5 COMPARATOR REFUSED: a planted control did not fire.  This is NOT a verdict.", file=sys.stderr)
        sys.exit(2)
    ...
    recs = {}
    for arm in ARMS:
        p = os.path.join(root, arm, "mpa5_out.json")
    ...
    emit(out, a.out)
    print("MPA5 VERDICT: %s" % out["verdict"])
    sys.exit(0)
