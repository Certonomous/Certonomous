    ...
    if not os.path.isfile(src):
        ...
        for h in FD_STEPS:
            ...
            if os.path.isfile(cand):
                ...
                break
    if not os.path.isfile(src):
        return None  # no endpoint record; the plant has nothing to plant into
    with open(src) as f:
        d = json.load(f)
    if "J_an" not in d:
        return None
...
def grade(root, cpuset, quiet=False):
    ...
    emit("G9-4", "PASS",
         ...
         "design point (l2(shapexUpper) = %.10e, %d components)"
...
def selftest():
    ...
    print("D9 GRADER SELFTEST: %d/%d %s" % (npass, len(results),
