#!/usr/bin/env bash
# CURRICULUM D10 CAPABILITY PROBE launcher.  Frozen instrument.
#
...
BASE="${BASE:-/home/ubuntu/certonomous-runs/CURRICULUM-PROBES-D10-D11-D12/D10}"
SRC="${SRC:-/home/ubuntu/Certonomous/cases/dafoam/ladder-a/A5/curriculum_D10_probe/d10_case}"
RUNPY="${RUNPY:-/home/ubuntu/Certonomous/cases/dafoam/ladder-a/A5/curriculum_D10_probe/d10_run_script.py}"
TMO="${TMO:-240}"
...
SPENT_CORE_MIN=0
PLANT_T="1.234"          # K, added to 0/T internalField in the plant stage only
BASE_T="293.15"
PLANTED_T="294.384"      # 293.15 + 1.234, written literally so the grader can read it back
...
timeout 300 sudo -n docker run --name "d10_mesh_${STAMP}" --user 0:0 --cpus=1 \
...
sudo -n docker rm "d10_mesh_${STAMP}" >/dev/null 2>&1
...
run_stage () {
  ...
  if [ "$PLANT" = "yes" ]; then
    sed -i "s/^internalField   uniform ${BASE_T};/internalField   uniform ${PLANTED_T};/" "$BASE/$STAGE/0/T" \
  ...
  timeout "$TMO" sudo -n docker run --name "$NAME" \
      ...
      "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
       mpirun --allow-run-as-root --bind-to none -np 1 python d10_run_script.py -task $TASK -out d10_${STAGE}.json" \
      > "$LOG" 2>&1
  rc=$?
  ...
  echo "STAGE=$STAGE TASK=$TASK rc=$rc wall_s=$WALL ranks=1 core_min=$CM inspect(exit,oomkilled)=[$INSPECT] json=$STAGE/d10_${STAGE}.json log=$(basename "$LOG")" | tee -a "$LEDGER"
