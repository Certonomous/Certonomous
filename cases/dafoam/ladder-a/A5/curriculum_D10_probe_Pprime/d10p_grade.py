#!/usr/bin/env python3
"""
CURRICULUM D10-Pprime CAPABILITY PROBE grader (the plant re-buy).  Frozen instrument.
...
PLANTED_T = "354.384"     # 353.15 + 1.234 K, planted on the HEATED WALL fixedValue in 0/T
BASE_T = "353.15"         # the unplanted heated-wall fixedValue
...
def grade(base, plant, clean):
    ...
    if plant_rel <= FLOOR_PLANT_REL:
        raise Refusal(
            "PLANTED-ZERO REFUSAL: a +1.234 K plant on the HEATED WALL boundary in 0/T moved HFX by only %.6e "
...
def selftest():
    ...
    print("D10-Pprime GRADER SELFTEST: 6/6 PASS (A healthy, B blind-plant refuses, C zero-adjoint "
          "GATE FAIL, D empty-set refuses, E noisy-clean refuses, F absent-function BLOCKED)")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--root", default="/home/ubuntu/certonomous-runs/CURRICULUM-PROBES-D10-D11-D12/D10P")
    ...
    try:
        ...
        base = read_json(os.path.join(a.root, "base", "d10p_base.json"))
        plant = read_json(os.path.join(a.root, "plant", "d10p_plant.json"))
        clean = read_json(os.path.join(a.root, "clean", "d10p_clean.json"))
        verdict, gates = grade(base, plant, clean)
    except Refusal as e:
        print("D10-Pprime PROBE VERDICT: NOT A RESULT")
        print("GRADER REFUSED (exit 2): %s" % e)
        return 2
    for k in sorted(gates):
        print("  %-8s %-12s %s" % (k, gates[k][0], gates[k][1]))
    print("D10-Pprime PROBE VERDICT: %s" % verdict)
    return 0
