...
# Every constant PREREGISTRATION.md must fix is written as __A5P2_UNFROZEN__.
...
# the counting pattern was bare '__A5P2_UNFROZEN__' against a threshold of >1,
...
# ===========================================================================
UNFROZEN_TOKENS=$(grep -cE '__A5P2_UNFROZEN__[A-Z0-9_]+' "${BASH_SOURCE[0]}" || true)
...
# ---------------------------------------------------------------------------
REGISTERED_BASE=/home/ubuntu/certonomous-runs/A5P2-ubend-plateau          # /home/ubuntu/certonomous-runs/A5P2-ubend-plateau
...
echo "A5P2_IMAGE arm=$ARM img=$IMG id=$GOT_DIGEST pinned_by=hash"
...
echo "A5P2_ONE_CHANGE arm=$ARM change=[$CHANGE] verified_in_staged_file=yes"
...
echo "A5P2_STAGED_MD5 arm=$ARM runScript.py=$GOT_MD5_RUNSCRIPT pinned=yes"
...
test "${PRE_ROWS:-0}" -gt 0 || {
  ...
  exit 5; }
echo "A5P2_LAUNCH_PRECAPTURE arm=$ARM capture=$(basename "$LAUNCH_PRE") rows=$PRE_ROWS staging_anchor_epoch=$AGE_DATUM comparison=strict_increase slack=none"
...
CID=$(sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     echo A5P2_CONTAINER_UID: \$(id -u) && \
     echo A5P2_DEADLINE_IN_CONTAINER_S: $TMO && \
...
echo "$CID" > "$CIDFILE"
echo "A5P2_CONTAINER_ID arm=$ARM name=$NAME cid=$CID launched_claim=NOT_YET_ASSERTED"
...
echo "A5P2_LAUNCH_MTIME_WITNESS arm=$ARM result=$LA_MTIME capture=$(basename "$LAUNCH_PRE") rows=$PRE_ROWS gating=no evidence=[$(printf '%s' "${LA_MTIME_EVIDENCE:-none}" | tr '\n' ';' | head -c 300)]"
if [ "$LA_RC" -eq 0 ] && [ "$LA_MTIME" = "unchanged_since_capture" ]; then
  echo "A5P2_LAUNCH_CONTRADICTION arm=$ARM the log witness fired but NOT ONE FILE in $WORK is strictly newer than the recorded capture.  A grader must treat this row as suspect." \
...
if [ "$LA_RC" -ne 0 ]; then
  ...
  EXITCODE=$(sudo -n docker inspect --format '{{.State.ExitCode}} {{.State.OOMKilled}}' "$CID" 2>>"$LA_TMP/la.stderr")
  {
    ...
    echo "A5P2_LAUNCH_REFUSED arm=$ARM rc=$LA_RC -- THE SOLVER'S FIRST ARTIFACT NEVER APPEARED.  This row is NOT A RESULT and no grading may read it."
    echo "A5P2_LAUNCH_READER_STDERR arm=$ARM: $(tr '\n' ' ' < "$LA_TMP/la.stderr" | head -c 600)"
  } | tee -a "$BASE/ledger.txt"
  sudo -n docker rm "$CID" >/dev/null 2>&1
  rm -f "$PIDFILE"
  exit "$LA_RC"
fi
echo "A5P2_LAUNCH_ASSERTED arm=$ARM launched: true reason=[$LA_REASON] waited_s=$LA_WAITED_S budget_s=$LAUNCH_BUDGET_S mtime_witness=$LA_MTIME precapture=$(basename "$LAUNCH_PRE") precapture_rows=$PRE_ROWS cid=$CID solver_call_seen=$LA_CORROBORATED" \
  | tee -a "$BASE/ledger.txt"
...
while :; do
  ...
  if [ "$OVER" = "1" ]; then
    KILLED=$(la_kill "$CID" "$LA_TMP/la.stderr")
    echo "A5P2_CAP_EXCEEDED arm=$ARM elapsed_s=$EL core_min=$COREMIN cap=$CAP_COREMIN container_kill=$KILLED -- an overrun STOPS the run; it does not get a new budget (CLAUDE.md rule 12).  This row is NOT A RESULT." \
      | tee -a "$BASE/ledger.txt"
    break
  fi
  sleep 10
...
SOLVER_RC=$(grep -a '^A5P2_SOLVER_RC: ' "$LOG" | tail -1 | awk '{print $2}')
sudo -n docker rm "$CID" >/dev/null 2>&1
...
echo "A5P2_AGE_GUARD arm=$ARM fields_at_${END_TIME}=$TOTAL newer_than_staging=$NEWER age_datum_epoch=$AGE_DATUM"
...
echo "  python3 <repo>/cases/dafoam/ladder-a/A5/curriculum_A5P/a5p2_grade.py \\"
