...
PLANT_P_FAIL = 9.876543e-03   # A5P2: the other-direction plant -- far ABOVE G1's 2.057e-05
...
def _plant_p_by_line_index(lines, value=None):
    ...
    if idx is None:
        return (None, None)
    out = list(lines)
    out[idx] = "p initRes: %.7e" % (PLANT_P if value is None else value)
    return (out, idx)
...
def control_plant_residual(lines, reader=None):
    ...
    # A5P2 REPAIR -- CONTROL A IS NOW DIRECTION-AWARE.
    #
    # A5P's control planted a PASSING value and demanded G1 flip False->True.
    # That is vacuous whenever the unplanted arm ALREADY passes G1 -- and it
    # happened on the first arm that mattered: A5P's P2 broke the plateau to
    # p initRes 1.449e-08, so the frozen grader REFUSED (exit 2) on its own
    # control rather than grade the one arm that worked.  The refusal was
    # CORRECT -- a plant that cannot change the answer proves nothing -- but a
    # control that only works when the arm FAILS cannot grade a success.
    #
    # The requirement is rule 3's, not one direction of it: the reader must be
    # shown able to return THE OTHER ANSWER, whichever answer it starts from.
    # So plant a FAILING value when the arm already passes, and a PASSING value
    # when it does not.  Either way G1 is shown to MOVE on real bytes.
    if g1_before is True:
        planted2, idx2 = _plant_p_by_line_index(lines, value=PLANT_P_FAIL)
        if planted2 is None:
            return (False, "no `p initRes:` line to plant the failing value into")
        after2 = reader(planted2)
        if not after2:
            return (False, "the reader saw nothing after the failing plant")
        seen2 = after2[-1][1]
        if abs(seen2 - PLANT_P_FAIL) > 1e-18:
            return (False, "reader returned %.9e, not the planted %.9e"
                           % (seen2, PLANT_P_FAIL))
        if eval_g1(seen2) is not False:
            return (False, "G1 did not FLIP True->False on the failing plant")
        return (True, "arm already PASSES G1 unplanted, so the control ran in "
                      "the OTHER direction: reader saw %.9e at line index %d; "
                      "G1 flipped True -> False" % (seen2, idx2))
...
# ===========================================================================
# A5P2'S ONE CHANGE FROM A5P (frozen da2e5065a): the field check reads the
# LAST WRITTEN time directory, not endTime.
#
# WHY: measured on A5P's own five arms -- DAFoam reaches `Time = 5000`, prints
# its ExecutionTime, and writes NO field directory there.  processor0/ holds
# 0 0.0001 1000 2000 3000 4000 under controlDict endTime 5000 / writeControl
# timeStep / writeInterval 1000 / purgeWrite 0.  Writing works (4000 carries
# the full field set); only the FINAL write is skipped.  A5P's F3 therefore
# asked for a directory this solver never creates, so every arm graded
# NOT A RESULT on a gate defect of the supervisor's, not on physics.
#
# THE FLOOR IS THE POINT: an unqualified "last written time" would be
# satisfied by a run that died at 1000.  MIN_FIELD_TIME refuses that instead
# of grading it.
# ===========================================================================
MIN_FIELD_TIME = 4000

def last_written_time(case_dir):
    """Largest numeric time dir under processor*/ or the case root, or None."""
    import os, glob as _g
    best = None
    roots = sorted(_g.glob(os.path.join(str(case_dir), "processor*"))) or [str(case_dir)]
    for r in roots:
        if not os.path.isdir(r):
            continue
        for e in os.listdir(r):
            if not os.path.isdir(os.path.join(r, e)):
                continue
            try:
                v = float(e)
            except ValueError:
                continue
            if v.is_integer() and (best is None or v > best):
                best = v
...
def grade_arm(arm, log_path, case_dir, rc, is_baseline, end_time=END_TIME,
    ...
    # F3b/c/d from disk
    if case_dir:
        f3t = last_written_time(case_dir)
        if f3t is None or f3t < MIN_FIELD_TIME:
            f_fails.append("F3: last written time %r is absent or below MIN_FIELD_TIME %d under %s -- a short run is refused, not graded" % (f3t, MIN_FIELD_TIME, case_dir))
            f3t = end_time
        lo, hi, n = field_minmax(case_dir, f3t, "nuTilda")
        if n == 0:
        ...
        um, nu = vector_mag_max(case_dir, f3t, "U")
        if nu == 0:
            f_fails.append("F3c: U not found at time %d under %s" % (f3t, case_dir))
        ...
        tlo, thi, nt = field_minmax(case_dir, f3t, "T")
        if nt == 0:
            f_fails.append("F3d: T not found at time %d under %s" % (f3t, case_dir))
