...
## NOTE 2 — 2026-09-11 — **THE TRAILING-EDGE DEVIATION IS SPAN-VARYING AND GROWS OUTBOARD. A3's VALIDATION IS FAITHFUL AT THE ROOT AND PROGRESSIVELY NOT FAITHFUL TOWARD THE TIP.** No verdict moves; one of my own arguments is restated on a narrower basis.

**cfd's M6C2 finding, INDEPENDENTLY CORROBORATED FROM MY OWN EARLIER MEASUREMENT.** The A3 surface holds trailing-edge thickness **constant in ABSOLUTE terms** where AR-138's conical loft requires `t_TE/c` constant. My own c0 reading — taken before and for another purpose — gives TE half-thickness **6.8879e-04 / 6.8603e-04 / 7.1352e-04 m** at root/mid/tip, a spread of **4.0 %**: flat in absolute terms, exactly as cfd says. With the M6's taper (root chord 0.8059 m, taper 0.562), that makes `t_TE/c` **~1.78× larger at the tip than at the root**. cfd measure 74 %; I get 78 % from different samples. ⚠ **The STRUCTURE agrees and the MAGNITUDE does not: our absolute thicknesses differ by ~21 % (1.378e-3 mine, 1.1366e-3 theirs), and cfd's own method note explains why — slab-sampling a 9-zone surface picks an arbitrary subset. MY root/mid/tip figures may carry the same defect, so I cite the structure as established and the magnitude as approximate.**

**WHAT IT DOES TO A3's GRADED RESULTS: nothing, and that is not a dismissal.** Every A3 row is what the solver did on the mesh it had, and all of them stand. What changes is what those rows are *evidence about*. A3's `Cp` validation is against **AGARD AR-138 Case 2308** at η = 0.20 … 0.99, so **the inboard stations compare a faithful geometry to experiment and the outboard stations do not.** The root matches the source to seven digits; by η ≈ 0.99 the trailing edge is up to ~74–78 % too thick in the ratio that governs the flow.

⚠ **AND IT FORCES ME TO RESTATE AN ARGUMENT OF MINE ON A NARROWER BASIS.** In NOTE 1 I set the TE aside as a candidate cause of the shock error because *"the TE geometry is span-uniform while the shock error changes sign, and a span-uniform cause cannot produce a sign reversal."* **That premise is now false: this defect is NOT span-uniform — it grows monotonically outboard.** The conclusion survives, but only on the replacement argument: the defect is **monotone in span**, while A3's shock error is **not monotone** — `+0.096, +0.074, +0.045, −0.001, +0.026, +0.042, +0.022` — falling to zero at η = 0.80 and rising again. **A monotone cause cannot produce a non-monotone error either.** Same conclusion, weaker footing, and I would rather say so than let a broken premise keep carrying it.

**AND ONE CORRELATION I CAN NO LONGER WAVE AWAY.** The **worst-agreeing station is also the most geometrically wrong station**: η = 0.99 carries the worst upper-surface RMS (0.1139) and the worst mean bias (+0.065), and it is where `t_TE/c` deviates most. I previously attributed that station wholly to tip-vortex RANS limits. **Both explanations are now live and I cannot separate them from what is on disk.** `UNMEASURED`, and it needs a run on a surface built to the reference loft.

**CONSEQUENCE FOR ANY DEMO CAPTION, and this is the operative part: A3 MUST NOT BE PRESENTED AS VALIDATING THE ONERA M6 OUTBOARD.** The defensible caption narrows to a **root-to-midspan** comparison against AR-138, with the outboard stations shown carrying a named geometry deviation — or the outboard stations are not shown. Presenting η = 0.90–0.99 as agreement-or-disagreement with experiment would attribute to the flow solver what is partly the mesh.

**cfd is extruding this surface only as a route probe, labelled NOT AN M6 MESH.** Nothing here asks them to change that.
