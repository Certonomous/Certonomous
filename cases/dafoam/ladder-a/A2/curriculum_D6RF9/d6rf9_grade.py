#!/usr/bin/env python3
"""Curriculum D6RF9 -- THE GRADING PATH for the A2-wing convergence probe
...
taken here. `python d6rf9_grade.py --drive` runs the planted-zero control and
...
written).  The DELTAS are enumerated in `d6rf9_grade.py_DELTAS_from_d6rf7.diff`
beside this file and are the supervisor's check-1 subject.

WHAT IS INHERITED VERBATIM (T25 -- no gate/threshold/floor moved):

  * THE ACCEPT FLOOR.  `d6rf9_accept_floor_control.py` is a BYTE-IDENTICAL copy
...
WHAT CHANGED, AND WHY (the registered DELTAS -- check-1 subject):
  ...
  D1  THE LEG DISCRIMINATOR IS `fvSolution`/`solverName`, NOT `fvSchemes`.
      ...
      by fvSchemes md5.  D6RF9 holds the D6RF7 LIMITED scheme FIXED and changes
...
LEG_BEGIN = "D6RF9_LEG_BEGIN"
LEG_END = "D6RF9_LEG_END"
# D6RF9's install marker echoes the OUTER-LOOP config, not an fvSchemes md5 (D1).
CONFIG_INSTALL_MARKER = "D6RF9_CONFIG_INSTALLED"
...
import d6rf9_accept_floor_control as afc                           # noqa: E402
    ...
    if inst is None:
        ...
        for key in ("solverName", "relax_p", "relax_eqn", "endTime"):
            ...
            if got is not None and got != exp:
                ok = False
                reasons.append("%s installed %r != registered %s"
                               % (key, got, exp))
...
def gate_conv(seg, rung):
    ...
    row = {"gate": "G-CONV_per_field_final_initRes", "bar": CONV_BAR,
           ...
           "bar_provenance": (
               ...
               "d6rf9_accept_floor_control.py refuses if it has moved."),
...
def freeze_check():
    ...
    afc_path = os.path.join(HERE, "d6rf9_accept_floor_control.py")
...
def drive():
    ...
    tmp = tempfile.mkdtemp(prefix="d6rf9_grade_drive_")
    rc = 0
    print("D6RF9 GRADE -- DRIVEN (no compute, PERMISSION=NOT_FROZEN)")
    try:
        fz = freeze_check()
...
def main():
    ...
    if a.drive:
        return drive()
    if a.log:
        legs = read_legs(a.log)
        if a.rung not in legs["segments"]:
            print(json.dumps({"verdict": "NOT A RESULT",
                              "reason": "LEG_NOT_IN_LOG", "rung": a.rung,
                              "legs_seen": legs["order"]}, indent=1))
            return 0
        seg = _bind(legs, a.rung)
        print(json.dumps(gate_conv(seg, a.rung), indent=1, default=str))
        return 0
    ap.print_help()
    return 64
