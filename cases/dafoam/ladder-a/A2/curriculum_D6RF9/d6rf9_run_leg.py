#!/usr/bin/env python
"""Curriculum D6RF9 -- THE PER-LEG PRIMAL DRIVER for the A2-wing convergence
...
(`d6rf9_run_arm.sh`) starts, once per leg, under `mpirun`.  The freeze, the
...
/ ladder machinery of D6RF7 is DROPPED -- D6RF9 delta D5: this is a single-arm
convergence probe, not an FD arm).  It CARRIES the two things that make the
container log gradeable by `d6rf9_grade.py:read_legs`:

  * THE LEG MARKERS.  `D6RF9_LEG_BEGIN <tag> mode=P_conv` ... `D6RF9_LEG_END
  ...
  * THE DAOption DUMP.  DAFoam prints its resolved `daOptions` -- carrying
    ...
    driver does NOT suppress it.  `d6rf9_accept_floor_control.run_floor_control`
...
driver runs, which is also where `D6RF9_CONFIG_INSTALLED` is emitted.  This
...
still closes `D6RF9_LEG_END`, and exits 0 so the container runs the next leg --
...
def _refuse(msg):
    sys.stderr.write("D6RF9_LEG REFUSE %s\n" % msg)
    sys.exit(2)
...
def main(tag, solver, runscript, endpoint_dvs):
    ...
    ns = {"__name__": "d6rf9_frozen_header", "__file__": runscript}
    ...
    # ---- emit exactly what d6rf9_grade.py parses.
    print("D6RF9_LEG_BEGIN", tag, "mode=P_conv"); _flush()
    t0 = time.time()
    raised = False
    err = None
    try:
        ...
        raised = True
        err = repr(exc)[:400]
    print("D6RF9_LEG_END", tag, "mode=P_conv",
    ...
    if rank == 0:
        ...
        with open("d6rf9_leg_%s.json" % tag, "w") as fh:
            ...
            fh.flush()
            os.fsync(fh.fileno())
    print("D6RF9_LEG_COMPLETE", tag, "primal_raised=%s" % raised); _flush()
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Run ONE D6RF9 P_conv leg and emit its grader markers.")
    ap.add_argument("--leg-tag", required=True,
                    help="the leg identity the grader binds (a rung R1..R4, or "
                         "a control tag); matches D6RF9_CONFIG_INSTALLED leg=")
