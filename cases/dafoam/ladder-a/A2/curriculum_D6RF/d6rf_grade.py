...
ARTEFACT_PRODUCER = {
    "d6rf_endpoint_dvs_PHYSICAL.json": "F_mp",
...
REGISTERED_PRODUCTS = {
    "F_mp": ["d6rf_endpoint_dvs_PHYSICAL.json", "d6r_endpoint_dvs_DRIVERSCALED.json",
...
def gate_dvl(root, census, runscript_d6r, runscript_d4):
    ...
    import d6rf_endpoint_locus as locus
    res = {"gate": "G-DVL_endpoint_locus", "arms": {}}
    verdicts = []
    for arm, art, rs in (("F_mp", "d6rf_endpoint_dvs_PHYSICAL.json", runscript_d6r),
...
def grade(root, out_path, skip_freeze=False, runscript_d6r=None,
    ...
    if not skip_freeze:
        frozen = freeze_check([
            "cases/dafoam/ladder-a/A2/curriculum_D6RF/PREREGISTRATION.md",
            "cases/dafoam/ladder-a/A2/curriculum_D6RF/d6rf_grade.py",
            "cases/dafoam/ladder-a/A2/curriculum_D6RF/d6rf_endpoint_locus.py",
            "cases/dafoam/ladder-a/A2/curriculum_D6RF/d6rf_endpoint_physical.py",
        ])
    rs6 = runscript_d6r or os.path.join(
        HERE, os.pardir, "curriculum_D6R", "d6r_opt_runScript.py")
    ...
    with open(out_path, "w") as fh:
        ...
        fh.flush()
        os.fsync(fh.fileno())
    print("D6RF_VERDICT %s  spend %.3f core-min of a %.1f ceiling  -> %s"
...
def main():
    ...
    a = ap.parse_args()
    if a.selftest:
        import d6rf_grade_selftest as st
        return st.run()
    if not a.root or not a.out:
        sys.stderr.write("usage: d6rf_grade.py --root <run root> --out <json>\n")
        return 64
    try:
        grade(a.root, a.out, a.skip_freeze, a.runscript_d6r, a.runscript_d4,
              a.d4_ref)
    except Refusal as e:
        sys.stderr.write("D6RF_GRADE REFUSED %s\n" % e)
        return 2
    return 0
