...
#   setsid nohup bash d6rf_chain_driver.sh F_mp REF_off > <root>/chain_launch.out 2>&1 &
# ).
#
...
LAUNCHER="$HERE/d6rf_run_arm.sh"
IMG=dafoam-idwarp-rot:v1
BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6RF-a2-wing-multipoint-fd
...
MD5_LAUNCHER=cecc53b2d12ba03ac8d94b2fbc4d140c
...
test $# -ge 1 || { echo "ABORT usage: d6rf_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"
for a in $ARMS; do
  case "$a" in
    F_mp|REF_off) : ;;
...
STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/d6rf_driver.pid"
cd "$HERE" || exit 4
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$D6R_CASE_DIR/d6r_opt_runScript.py" "$D6R_CASE_DIR/d6r_fd_endpoint.py" \
        "$D6R_CASE_DIR/d6r_extract_endpoint.py" "$D6R_CASE_DIR/d6r_ref_off.py" \
        ...
        "$HERE/d6rf_endpoint_locus.py" "$HERE/d6rf_endpoint_physical.py" \
        "$HERE/d6rf_units_assert.py" \
        "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
  # G-ROOT.3 accepts ONLY an exact `ITEM=D6RF` line; staging metadata goes on a
  # line that does not start with ITEM= (D5-DRIVER-DEF-1, inherited).
  echo "ITEM=D6RF" > "$BASE/ledger.txt"
  echo "STAGED stamp=$(date -u +%Y%m%dT%H%M%SZ) base_src=$D4_BASE_SRC permission=$PERMISSION" >> "$BASE/ledger.txt"
  echo "D6RF_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") permission=$PERMISSION"
else
  echo "D6RF_ROOT_PRESENT base=$BASE (not re-staged)"
fi
{ echo "$MD5_RUNSCRIPT6  $BASE/d6r_opt_runScript.py"
  echo "$MD5_FD  $BASE/d6r_fd_endpoint.py"
  echo "$MD5_EXTRACT6  $BASE/d6r_extract_endpoint.py"
  echo "$MD5_REFOFF  $BASE/d6r_ref_off.py"
  ...
  echo "$MD5_LOCUS6  $BASE/d6rf_endpoint_locus.py"
  echo "$MD5_PHYS6  $BASE/d6rf_endpoint_physical.py"
  echo "$MD5_UNITS  $BASE/d6rf_units_assert.py"; } | md5sum -c - || { echo "ABORT staged instrument md5"; exit 4; }
...
echo "D6RF_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
for ARM in $ARMS; do
  ...
  echo "D6RF_SPEND_CENSUS arm=$ARM spent_core_min=$SPENT this_arm_cap=$ACAP projected_worst_case=$PROJ item_ceiling=$ITEM_CEILING_CORE_MIN"
  ...
  while true; do
    ...
    echo "D6RF_H5_WINDOW arm=$ARM n=$N window_s=$H5_WINDOW_S floor_GiB=$H5_FLOOR_GIB min_GiB=$MIN max_GiB=$MAX samples_below_floor=$BELOW waited_s=$H5_WAITED file=$(basename "$H5_FILE")"
  ...
  echo "D6RF_AGGREGATE arm=$ARM waited=$WAITED $AGG"

  echo "D6RF_DRIVER arm=$ARM begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "D6RF_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
  ...
  if [ "$rc" = "77" ]; then
    echo "D6RF_UNITS_REFUSAL arm=$ARM rc=77 -- the design vector could not prove it was PHYSICAL and in bounds; THE MESH WAS NEVER TOUCHED.  This is a REGISTERED outcome, not an infrastructure failure." >> "$BASE/STATUS.$ARM"
  fi
  if [ "$rc" -ne 0 ]; then
...
echo "D6RF_SPEND_FINAL total_core_min=$FINAL_SPEND item_ceiling=$ITEM_CEILING_CORE_MIN" | tee -a "$BASE/ledger.txt"
