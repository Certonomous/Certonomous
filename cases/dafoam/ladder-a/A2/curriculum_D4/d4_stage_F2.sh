#!/usr/bin/env bash
# D4 ARM-F RE-RUN STAGING -- run ONLY after ACC-1 returned PASS.
#
...
# SECOND, STRICTLY LATER datum `.d4_stage_F2_copy_epoch` dates THIS arm, and
...
SRC="$BASE/O"
DST="$BASE/F2"
EV="$BASE/F2_STAGING_EVIDENCE.txt"

: > "$EV"
say() { echo "$*" | tee -a "$EV"; }

say "D4_STAGE_F2 utc=$(date -u +%Y%m%dT%H%M%SZ)"
...
say "D4_STAGE_F2 (a) OK src=$SRC exists, OptView.hst present, dst=$DST does not exist"
...
say "D4_STAGE_F2 (b) ASSERTION: count of pre-existing answer files under $SRC = $N (required 0)"
if [ "$N" -ne 0 ]; then
...
say "D4_STAGE_F2 (c) reference mesh md5 base=$MB src=$MS"
test "$MB" = "$MS" || { say "ABORT (c) $SRC carries a DEFORMED reference mesh -- staging from it would double-deform the wing"; exit 5; }
say "D4_STAGE_F2 (c) OK -- undeformed reference mesh, double-deformation confound eliminated by measurement"

# ---- (d) COPY, with mtime semantics PINNED and a second datum written ----
say "D4_STAGE_F2 (d) mtime semantics = cp -a, PRESERVE"
COPY_EPOCH=$(date -u +%s)
cp -a "$SRC" "$DST" || { say "ABORT (d) copy failed"; exit 4; }
echo "$COPY_EPOCH" > "$DST/.d4_stage_F2_copy_epoch" || { say "ABORT (d) cannot write copy datum"; exit 4; }
say "D4_STAGE_F2 (d) copied, copy_epoch=$COPY_EPOCH written to .d4_stage_F2_copy_epoch"
say "D4_STAGE_F2 (d) arm O age datum carried over = $(cat "$DST/.d4_age_datum" 2>/dev/null || echo ABSENT)"
...
say "D4_STAGE_F2 (a) POST-COPY: source intact"

N2=$(find "$DST" \( -name 'd4_endpoint_dvs*.json' -o -name 'd4_fd_endpoint.jsonl' -o -name 'd4_fd_endpoint.json*' \) 2>/dev/null | wc -l)
say "D4_STAGE_F2 (b) POST-COPY assertion: answer files under $DST = $N2 (required 0)"
...
for f in d4_endpoint_locus.py d4_endpoint_physical.py d4_fd_endpoint.py; do
  ...
  say "D4_STAGE_F2 (e) staged $f md5 $(md5sum "$BASE/$f" | cut -d' ' -f1)"
done

say "D4_STAGE_F2 READY -- invoke d4_run_F2.sh <image> <cpuset>"
