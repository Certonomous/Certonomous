#!/usr/bin/env bash
# D4 ARM-F RE-RUN LAUNCHER -- the endpoint FD table, at the CORRECTED point.
#
...
#
# THE LEDGER IS A SEPARATE FILE.  Rows go to `f2_ledger.txt`, NOT `ledger.txt`.
# MEASURED REASON, not caution: `d4_grade_SUPPLEMENT.py:630` refuses G10 with
# `unregistered_arm_in_ledger` on any arm outside {P1,P2,O,F}.  Writing an F2 row
# into `ledger.txt` would MOVE A GATE.  Rows go to `f2_ledger.txt`.
#
...
IMG="${1:-}"; CPUSET="${2:-}"
test -n "$IMG"    || { echo "ABORT usage: d4_run_F2.sh <image> <cpuset>"; exit 64; }
test -n "$CPUSET" || { echo "ABORT usage: d4_run_F2.sh <image> <cpuset>"; exit 64; }
...
test -d "$WORK" || { echo "ABORT $WORK absent -- run d4_stage_F2.sh first"; exit 5; }
test -f "$WORK/.d4_stage_F2_copy_epoch" || { echo "ABORT $WORK carries no F2 copy epoch -- the age guard would have no datum"; exit 5; }
AGE_DATUM=$(cat "$WORK/.d4_stage_F2_copy_epoch")
