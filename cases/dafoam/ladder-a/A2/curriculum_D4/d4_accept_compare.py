#!/usr/bin/env python3
"""Curriculum D4 -- THE ACCEPTANCE COMPARATOR for the D4-DEF-4 repair.

Reads `d4_accept_primal.json` FROM DISK and grades it against the bands
registered in `D4_DEF4_REPAIR_PREREGISTRATION.md` BEFORE the primal ran.

  ACC-1  THE GATE.  rel_CD = |CD_acc - CD_OPT| / CD_OPT <= 1.0e-3
         PASS -> the diagnosis is proved, the repair is frozen, arm F re-runs.
         GATE FAIL -> THE DIAGNOSIS IS WRONG.  The repair is withdrawn, D4 stays
...
PLANTED-ZERO CONTROL (CLAUDE.md rule 3), run BEFORE the number is believed:
  ...
  * the count channel: the artifact must carry the registered design-variable
    counts (7 twist, 96 shape, 2 patchV) or the comparator REFUSES BY COUNT,
...
CD_OPT = 2.1125978108239574e-02      # arm O IPOPT objective, O/opt_IPOPT.txt
CL_OPT = 0.49999992625227224         # frozen extractor _final_CL at the same row
...
N_REGISTERED = {"n_twist": 7, "n_shape": 96, "n_patchV": 2}
...
def read_artifact(path):
    ...
    with open(path) as fh:
        doc = json.load(fh)
    if doc.get("kind") != "d4_acceptance_primal":
        refuse("read_artifact", {"kind": doc.get("kind"),
                                 "required": "d4_acceptance_primal"})
    for k in ("CD", "CL"):
        if k not in doc:
...
def blind_reader(path):
    ...
    return {"kind": "d4_acceptance_primal", "CD": CD_OPT, "CL": CL_OPT,
            "n_twist": 7, "n_shape": 96, "n_patchV": 2,
...
def main():
    ...
    try:
        ...
        res = {
            ...
            "consequence": ("PASS: the D4-DEF-4 diagnosis is proved by an "
                            "instrument that grades nothing; the repair is "
                            "frozen and arm F re-runs."
        ...
        sys.stderr.write("D4_ACCEPT_COMPARE REFUSE %s\n" % e)
        return 2
