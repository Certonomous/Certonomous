#!/usr/bin/env python
"""Curriculum D4 -- THE ACCEPTANCE PRIMAL.  LIMIT 1 of the D4-DEF-4 ruling.

ONE primal at the CORRECTED design point.  Its `CD` is compared against arm O's
IPOPT objective `2.1125978108239574e-02` by `d4_accept_compare.py`, on a band
registered BEFORE this ran.

WHY IT EXISTS AND WHAT IT IS ALLOWED TO DECIDE.  The D4-DEF-4 diagnosis is
internally consistent -- a pinned variable reading back at exactly its scaler,
62 of 96 components outside their own bounds, and `scale=True`/`scale=False`
...
ENOUGH.  The supervisor made the reproduction a PRECONDITION of the freeze:
  ...
  * it reproduces inside the registered band  -> the diagnosis is proved by an
    instrument that grades nothing, the repair is frozen, arm F re-runs;
  * it does not reproduce                     -> THE DIAGNOSIS IS WRONG, the
    repair is WITHDRAWN, D4 stays BLOCKED and a second finding is recorded.
...
DESIGN RULES, all of them inherited from the FROZEN `d4_fd_endpoint.py` because
the point is to run the SAME model, not a lookalike:
  * The model definition is NOT COPIED.  This file reads `d4_opt_runScript.py`,
  ...
  * The design point is read from `d4_endpoint_dvs_PHYSICAL.json` and this file
...
DVS = "d4_endpoint_dvs_PHYSICAL.json"
OUT = "d4_accept_primal.json"


def refuse(msg):
    sys.stderr.write("D4_ACCEPT REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    if rank == 0:
        ...
        with open(OUT, "w") as fh:
            ...
            fh.flush()
            os.fsync(fh.fileno())
        sys.stdout.write("D4_ACCEPT_PRIMAL_WRITTEN %s CD=%r CL=%r wall_s=%.1f\n"
                         % (OUT, cd, cl, wall))
        sys.stdout.flush()
