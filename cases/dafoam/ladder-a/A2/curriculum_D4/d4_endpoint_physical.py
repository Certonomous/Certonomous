#!/usr/bin/env python
"""Curriculum D4 -- the D4-DEF-4 REPAIR, as a WRAPPER that edits zero bytes.

AUTHORISED BY `SUPERVISOR_D4DEF4_REPAIR_RULING.md` (commit dbb88eb4) under
`VERIFICATION_CHARTER.md` 2d.1.  THREE LIMITS BIND IT and each is enforced here
mechanically rather than promised in prose:
  ...
  LIMIT 1  The repair is NOT FROZEN until one primal at the corrected design
           ...
           BEFORE the run.  That is `d4_accept_primal.py` and
           `d4_accept_compare.py`, and this file is useless without them.
  LIMIT 2  ZERO BYTES of any frozen file are edited.  This wrapper INVOKES the
           committed `d4_extract_endpoint.py` blob (md5 asserted first, imported
           ...
           validated.  The scalers are READ from `d4_opt_runScript.py`, the file
...
WHAT IT DOES, in order, refusing (exit 2) at the first failure:
  ...
  C1  `d4_extract_endpoint.py` on disk must hash to the frozen md5.
  C2  `d4_opt_runScript.py` on disk must hash to the frozen md5.
  C3  the extractor is imported and run; its two outputs must exist AND be
      ...
      A stale `d4_endpoint_dvs.json` lying in the work directory from an earlier
  ...
  C4  the extractor's output is PRESERVED BYTE-FOR-BYTE as
      `d4_endpoint_dvs_DRIVERSCALED.json` and the copy is md5-verified against
      the original -- 2d.1 condition (4), the pre-repair values recorded beside
      the published ones.
  C5  the scalers, bounds and `U0` are PARSED from `d4_opt_runScript.py`; every
  ...
  C8  `d4_endpoint_dvs_PHYSICAL.json` is written -- the corrected artifact, a
      NEW FILE BESIDE the preserved original, carrying its own provenance.
  C9  `d4_endpoint_dvs.json` is then written with the PHYSICAL content, because
      the FROZEN `d4_fd_endpoint.py` reads that filename and this wrapper may
      not edit it.  NOTHING IS LOST: the driver-scaled pre-image is on disk at
      C4 with its md5 recorded in both files, and arm F's original crash
      artifact under `F/` is never touched by this file at all.
...
import d4_endpoint_locus as locus                                  # noqa: E402

EXTRACTOR = "d4_extract_endpoint.py"
RUNSCRIPT = "d4_opt_runScript.py"
# PREREGISTRATION.md 9a, verbatim.  These are the FREEZE, not a preference.
MD5_EXTRACT = "ee7d3c99fd716da23779cb651961918e"
MD5_RUNSCRIPT = "2906d52a5dbed2bacbaeaf85a37d3fe8"

SCALED_OUT = "d4_endpoint_dvs.json"                 # the frozen extractor writes this
PRESERVED = "d4_endpoint_dvs_DRIVERSCALED.json"     # 2d.1 condition (4)
PHYSICAL = "d4_endpoint_dvs_PHYSICAL.json"          # the corrected artifact
HISTORY = "d4_major_history.json"


def refuse(msg):
    sys.stderr.write("D4_ENDPOINT_PHYSICAL REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    for stale in (SCALED_OUT, PRESERVED, PHYSICAL, HISTORY):
        if os.path.exists(stale):
            os.remove(stale)
    spec = importlib.util.spec_from_file_location("d4_extract_frozen",
                                                  os.path.abspath(EXTRACTOR))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["d4_extract_frozen"] = mod
    ...
    print("  C9 %s rewritten with PHYSICAL content for the FROZEN "
          "d4_fd_endpoint.py; pre-image preserved at %s md5 %s"
          % (SCALED_OUT, PRESERVED, m_src))

    sys.stdout.write("D4_ENDPOINT_PHYSICAL_WRITTEN %s scalers=%s "
