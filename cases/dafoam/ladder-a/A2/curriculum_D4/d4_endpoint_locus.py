...
False)` argument is INERT.  `d4_fd_endpoint.py` then applies those values as
...
registration()` parses `d4_opt_runScript.py` with `ast` and takes `lower`,
...
Self-test: `python3 d4_endpoint_locus.py --selftest` builds deliberate mutants
...
# ------------------------------------------------------------- the gate mode
def gate(workdir, runscript, physical_json="d4_endpoint_dvs_PHYSICAL.json"):
    """G13 -- re-read the PUBLISHED physical artifact from disk and re-assert
...
def selftest():
    ...
    tmp = tempfile.mkdtemp(prefix="d4_locus_st_")
    hdr = "U0 = 100.0\n"
    ...
    scaled = {"twist": [-0.28020223162835683],
              ...
              "patchV": [10.0, 0.1195966323219245]}
    ...
    nopin = _mut_file(tmp, "nopin.py", hdr + _MUT_SRC.replace(
        'lower=[U0, 0.0], upper=[U0, 10.0]', 'lower=[0.0, 0.0], upper=[200.0, 10.0]'))
    ...
    check("descale/bound list length != extracted length",
          lambda: control_bounds({"twist": [0.0], "shape": [0.0],
                                  "patchV": [100.0, 1.0, 2.0]}, reg), True)

    # ---------- THE PARSER IS READING THE REAL FILE, not a fixture ----------
    here = os.path.dirname(os.path.abspath(__file__))
    real = os.path.join(here, "d4_opt_runScript.py")
    if os.path.isfile(real):
        rr = parse_registration(real)
        sc = {k: rr["dvs"][k]["scaler"] for k in sorted(rr["dvs"])}
        print("SELFTEST real d4_opt_runScript.py scalers = %s" % json.dumps(sc, sort_keys=True))
        print("SELFTEST real d4_opt_runScript.py U0       = %r"
        ...
        print("  PLANT: U0 read from the source, not typed here              %s"
              % ("PASS" if rr["module_constants"].get("U0") == 100.0 else "FAIL"))
        (ok if rr["module_constants"].get("U0") == 100.0 else fail).append("plant/real-U0")
    else:
        print("  NOTE: d4_opt_runScript.py not beside this file; real-source "
              "plant NOT RUN")

    print("\nD4_LOCUS_SELFTEST %d/%d PASS" % (len(ok), len(ok) + len(fail)))
    if fail:
...
def main():
    ...
    if a.selftest:
        return selftest()
    if not a.gate:
        sys.stderr.write("usage: d4_endpoint_locus.py --selftest | "
                         "--gate <workdir> [--runscript P] [--out F]\n")
        return 64
    rs = a.runscript or os.path.join(a.gate, "d4_opt_runScript.py")
    try:
        res = gate(a.gate, rs)
    except LocusRefusal as e:
        sys.stderr.write("D4_LOCUS REFUSE %s\n" % e)
        return 2
