#!/usr/bin/env bash
# D4 ARM-F RE-RUN LAUNCHER (F3) -- the endpoint FD table, at the CORRECTED point,
# in a tree that also carries the D4-DEF-6 staging repair.  Arm F2 ran with
# CORRECT UNITS and died in compute_totals on a renameSolution collision with
# arm O's 84 carried-over pseudo-time directories; d4_stage_F3.sh removes them
# from the COPY.  See that file for the full triage.
#
# RUN ONLY AFTER ACC-1 RETURNED PASS.  It did, at 2026-08-25T21:53Z:
# CD = 0.021130918911049287 against arm O's IPOPT objective
# 2.1125978108239574e-02, rel_CD = 2.3387e-04 inside the 1.0e-3 band registered
# BEFORE that primal ran.  The D4-DEF-4 repair is FROZEN.
#
# THE ARM'S CAP AND ITS BANDS ARE THE ONES ALREADY FROZEN IN PREREGISTRATION.md
# AND ARE NOT RE-DERIVED HERE: cap 120.0 core-min (section 8), memory 12g, bands
# C, D and E of section 4.  LIMIT 3 of the ruling: NO GATE, THRESHOLD, BAND, CAP
# OR LABEL MOVES.
#
...
#
# THE LEDGER IS A SEPARATE FILE.  Rows go to `f3_ledger.txt`, NOT `ledger.txt`.
# MEASURED REASON, not caution: `d4_grade_SUPPLEMENT.py:630` refuses G10 with
# `unregistered_arm_in_ledger` on any arm outside {P1,P2,O,F}.  Writing an F3 row
# into `ledger.txt` would MOVE A GATE.  Rows go to `f3_ledger.txt`.
#
...
#   F2        120.0          12g            53.0
# Both the cap and the prediction are PREREGISTRATION.md section 8's ARM-F row,
# carried over UNCHANGED.  RESULTS.md 10.8 states that 53.0 "was sound and
# remains the right estimate for the arm when it can be run".
CAP=120.0
MEM=12g
MEM_GIB=12.0
MEM_FLOOR_GIB=12.0
...
IMG="${1:-}"; CPUSET="${2:-}"
test -n "$IMG"    || { echo "ABORT usage: d4_run_F3.sh <image> <cpuset>"; exit 64; }
test -n "$CPUSET" || { echo "ABORT usage: d4_run_F3.sh <image> <cpuset>"; exit 64; }
...
test -d "$WORK" || { echo "ABORT $WORK absent -- run d4_stage_F3.sh first"; exit 5; }
test -f "$WORK/.d4_stage_F3_copy_epoch" || { echo "ABORT $WORK carries no F3 copy epoch -- the age guard would have no datum"; exit 5; }
AGE_DATUM=$(cat "$WORK/.d4_stage_F3_copy_epoch")
...
cp -a "$BASE/d4_opt_runScript.py" "$BASE/d4_extract_endpoint.py" \
      ...
      "$BASE/d4_fd_endpoint.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
