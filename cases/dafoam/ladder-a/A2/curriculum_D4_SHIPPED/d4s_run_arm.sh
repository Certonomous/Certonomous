...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d4_${ARM}_" 2>/dev/null | grep "^d4_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$ARM/d4s_cmd.sh" > /dev/null 2>&1 \
