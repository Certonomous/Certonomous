...
SO3DR-F4 SUCCESSOR (curriculum_SO3DR_stage2_R). This rig is byte-identical to the
FROZEN Stage-2 rig (../curriculum_SO3DR_stage2/so3dr_stage2_standalone_runScript.py,
md5 dc67cced46235f897f6257353402ea57) EXCEPT ONE ordering change: the rank-0
`injected_dv.json` echo-sidecar write is moved to BEFORE prob.run_model(), so it
survives DAFoam's EXPECTED post-End AnalysisError("Primal solution failed!") raise
-- the very outcome this experiment counts. In the parent the write sat AFTER
run_model(), so the 32 primal-raised legs never reached it and the frozen grader's
F4 echo-check hard-REFUSED (exit 2) on absent sidecars. Verification RULED that a
value-invariant §2d.1 repair was unavailable (the grader refused; there is no value
to be invariant to) and mandated a FRESH SUCCESSOR with a clean grading path. This
rig is that successor. The finding it must be able to REPRODUCE -- not manufacture --
is F3-borne: NOT A RESULT | HIGH-CONFOUNDED (r_succ 8/12 = 66.7% >> the 10% F3
threshold). The ordering-only fix touches NO value, NO solver byte, NO gate: the
sidecar dict's fields are all input-derived and pre-run knowable (confirmed
field-by-field). The invariant is enforced statically by
scripts/check_sidecar_before_run.py. Governing pre-registration is THIS dir's
PREREGISTRATION.md (NOT the parent's).
...
def main():
    ...
    # SO3DR-F4 SUCCESSOR ORDERING FIX (L-504 clause-c; enforced by
    # scripts/check_sidecar_before_run.py): the rank-0 echo-sidecar is written
    # HERE -- after the cold-start guard establishes run_dir_abs and BEFORE
    # prob.run_model() -- so the F4 echo survives DAFoam's EXPECTED post-End
    # AnalysisError("Primal solution failed!") raise (the very outcome this
    # experiment counts). Every field below is INPUT-DERIVED and PRE-RUN KNOWABLE:
    # args.* (argparse input), D6R_LOG_SHA256 / daOptions[...] (module constants),
    # patchv/shape/twist (parsed from the sha256-pinned D6R log at line 304,
    # above run_model), and the literal True. NONE depends on run_model output.
    # The parent frozen rig wrote this block AFTER prob.run_model(); on a
    # primal-raised leg it was never reached, the sidecar was absent, and the
    # grader's F4 hard-REFUSED. The fix touches no value and no gate.
