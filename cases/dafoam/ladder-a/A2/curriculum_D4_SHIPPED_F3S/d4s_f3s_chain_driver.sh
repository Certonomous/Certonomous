#!/usr/bin/env bash
# D4S-F3S chain driver -- D4-SHIPPED's d4s_chain_driver.sh (Addendum 2b/2d,
# md5 01ddb189...) parameterised for the two endpoint arms F-S (SHIPPED image)
# and F-P (PATCHED image) under ONE stationarity acceptance rule.  Runs the
...
LAUNCHER="$HERE/d4s_f3s_run_arm.sh"
STAGER="$HERE/d4s_f3s_stage_arm.sh"
GRADER="$HERE/d4s_f3s_grade.py"
...
# FROZEN instrument md5s (PREREGISTRATION.md section 6); asserted before EVERY arm.
MD5_LAUNCHER=d65da2e06d5ec171d1ae6501728d80b3
MD5_GRADER=2ed0651c786cb5bd8832f8660ba6c670
...
test $# -ge 1 || { echo "ABORT usage: d4s_f3s_chain_driver.sh <F-S|F-P ...>"; exit 64; }
for A in "$@"; do test -n "$(img_of "$A")" || { echo "ABORT unknown arm $A (F-S|F-P)"; exit 64; }; done
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/d4s_f3s_driver.pid"
cd "$HERE" || exit 4
...
if [ ! -d "$BASE" ]; then
  ...
  echo "D4SF3S_ROOT_CREATED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ)"
fi
if [ -f "$PIDFILE" ]; then
...
echo "D4SF3S_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
for ARM in $ARMS; do
  ...
  if [ ! -d "$BASE/$ARM" ]; then
    ...
    echo "D4SF3S_STAGED arm=$ARM copy_epoch=$(cat "$BASE/$ARM/.d4s_f3s_stage_${ARM}_copy_epoch") evidence=${ARM}_STAGING_EVIDENCE.txt"
  elif [ ! -f "$BASE/$ARM/.d4s_f3s_stage_${ARM}_copy_epoch" ]; then
  ...
  echo "D4SF3S_DRIVER arm=$ARM image=$IMG begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "D4SF3S_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
...
if echo "$MD5_GRADER  $GRADER" | md5sum -c - ; then
  python3 "$GRADER" --base "$BASE" --out "$BASE/d4s_f3s_grade_$GST.json" > "$BASE/grade_$GST.out" 2>&1
  grc=$?
  echo "grade_rc=$grc stamp=$GST out=d4s_f3s_grade_$GST.json note=grader-exit-status-INFRASTRUCTURE-L-342-not-the-verdict" >> "$STATUS"
else
