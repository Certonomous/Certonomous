#!/usr/bin/env python3
"""Curriculum D6RF4 -- `ACCEPT_FLOOR_UNMOVED`, this item's own non-regression
instrument (`PREREGISTRATION.md` section 7 instrument 4).

WHAT IT IS FOR, AND IT IS THE ONLY REASON IT EXISTS.  `D6RF4` is lawful as a
...
def _synthetic_log(path, tol="1e-08", diff="1000", dumps=3):
    ...
    block = ("    solverName      DARhoSimpleFoam;\n"
             ...
             "    adjUseColoring  1;\n" % (tol, diff))
    body = ["D6RF4_LEG_BEGIN baseline mode=P_conv"]
    ...
    body += ["", "Time = 1000", "",
             ...
             "D6RF4_LEG_END baseline mode=P_conv", "Finalising parallel run"]
...
def drive():
    ...
    tmp = tempfile.mkdtemp(prefix="d6rf4_floor_ctrl_")
    rc = 0
    print("D6RF4 ACCEPT_FLOOR_UNMOVED -- DRIVEN IN BOTH DIRECTIONS")
    ...
    if a.log:
        try:
            r = run_floor_control(a.ctrl_dir or tempfile.mkdtemp(
                prefix="d6rf4_floor_"), a.log)
        except AcceptFloorRefusal as e:
            sys.stderr.write("D6RF4_ACCEPT_FLOOR REFUSED %s\n" % e)
            return 2
