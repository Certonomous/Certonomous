#!/usr/bin/env python3
"""Curriculum D6RF4 -- `PREREGISTRATION.md` section 3f's TWO-DIRECTION MUTATION
...
rather than left as a preference: `d6rf4_grade.py` IS the grading path frozen by
...
# registered arm of D6RF4 (PREREGISTRATION.md section 8.2) and on a real
...
# rather than letting a reader infer that D6RF4 buys a REF_off arm.
...
def _fd_doc():
    """The shape `d6rf4_fd_endpoint.py` writes, with `points` (section 2a's
...
def _ledger(path, overrides=None):
    ...
    ov = overrides or {}
    with open(path, "w") as fh:
        fh.write("ITEM=D6RF4 staged=fixture\n")
...
def build(root, ledger_overrides=None):
    ...
    _w(os.path.join(f, "d6rf4_fd_endpoint.json"), _fd_doc())
    _w(os.path.join(f, "d6rf4_major_history.json"), _hist_doc())
    _w(os.path.join(f, "d6rf4_endpoint_dvs_PHYSICAL.json"), _dv_doc())
    _w(os.path.join(f, "d6rf4_endpoint_dvs_DRIVERSCALED.json"), _dv_doc())
    _w(os.path.join(f, "d6rf4_endpoint_dvs.json"), _dv_doc())
    _w(os.path.join(f, "d6rf4_f5_endpoint.json"), _fd_doc())
    r = os.path.join(root, "REF_off")
    _w(os.path.join(r, "d6rf4_ref_off.json"), _ref_doc())
...
def _cases(root, d4):
    ...
    f = os.path.join(root, "P_conv")
    r = os.path.join(root, "REF_off")
    fd = os.path.join(f, "d6rf4_fd_endpoint.json")
    ref = os.path.join(r, "d6rf4_ref_off.json")
    hist = os.path.join(f, "d6rf4_major_history.json")
    dv = os.path.join(f, "d6rf4_endpoint_dvs_PHYSICAL.json")
    rs = os.path.join(HERE, "d6rf4_opt_runScript.py")
...
# PREREGISTRATION.md section 3.4: "G-CONV and G-SOLN are wired into the same
# fold, so the two new gates inherit the restriction rather than sitting
...
LEG_LOG_CLEAN = """\
D6RF4_FVSOLUTION_INSTALLED leg=L1_L2 md5={fvmd5} sites=4 file=d6rf4_fvSolution_TIGHT
D6RF4_LEG_BEGIN baseline mode=P_conv
...
p initRes: {p} finalRes: 1.0e-10 nIters: 3
...
End
D6RF4_LEG_END baseline mode=P_conv J=0.0222 wall_s=118.300 primal_raised=False
Finalising parallel run
"""


def _leg_log(path, p="1.30e-07", nut="1.10e-07",
             cd="0.0184758685", cl="0.3999751808", fvmd5=None):
    # THE INSTALL MARKER IS PART OF THE FIXTURE, because `gate_conv` now
    # REFUSES a leg whose log does not show the registered fvSolution
    # installed before it -- and a fixture that omitted the marker would test
    # that refusal instead of the finiteness fold it is here for.  The md5 is
    # read FROM THE GRADER'S OWN REGISTERED TABLE, never retyped.
    with open(path, "w") as fh:
        fh.write(LEG_LOG_CLEAN.format(
            p=p, nut=nut, cd=cd, cl=cl,
            fvmd5=fvmd5 or G.FVSOL_MD5[G.LEG_FVSOLUTION["L1"]]))
    return path


def _drive_new_gates(root):
    """`G-CONV` and `G-SOLN`, driven in BOTH directions on a synthetic
    container log.  Returns (rc, n_direction1, n_direction2)."""
    rc, n1, n2 = 0, 0, 0
    tmpdir = os.path.join(root, "_leglogs")
    os.makedirs(tmpdir, exist_ok=True)
    print("  --- section 3.4 on the TWO NEW GATES: G-CONV and G-SOLN --------")
    cases = (
        ("G-CONV  p initRes    ", "conv", {"p": "nan"}, "L1.p.initRes"),
        ("G-CONV  p initRes    ", "conv", {"p": "inf"}, "L1.p.initRes"),
        ("G-CONV  nuTilda      ", "conv", {"nut": "-inf"}, "L1.nuTilda.initRes"),
        ("G-SOLN  CD           ", "soln", {"cd": "nan"}, "L1.CD"),
        ("G-SOLN  CL           ", "soln", {"cl": "inf"}, "L1.CL"),
    )
    # ---- DIRECTION 2 FIRST: the UNMUTATED control ------------------------
    clean = _leg_log(os.path.join(tmpdir, "clean.log"))
    legs = G.read_legs(clean)
    for name, fn in (("G-CONV", G.gate_conv), ("G-SOLN", G.gate_soln)):
        res = fn(root, CENSUS, legs)
    ...
    for label, which, kw, expect_key in cases:
        ...
        lg = G.read_legs(lp)
        res = (G.gate_conv if which == "conv" else G.gate_soln)(root, CENSUS, lg)
...
def drive():
    tmp = tempfile.mkdtemp(prefix="d6rf4_finmut_")
    root = os.path.join(tmp, "run")
    d4 = build(root)
    G.D4_OPT_IPOPT = d4
    print("D6RF4 SECTION 3f FINITENESS MUTATION HARNESS -- BOTH DIRECTIONS")
