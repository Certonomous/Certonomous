#!/usr/bin/env python3
"""D6RF4 -- THE CONTROL ON THE AGE-DATUM STAGING GUARD.
...
This file is that missing control for `D6RF4`, and it is written to be able to
...
====================================
It guards ONE line -- the production of the age datum in `d6rf4_run_arm.sh` --
...
============================================================
`datum_command()` reads `d6rf4_run_arm.sh` and pulls out its actual
...
synthetic pair.  (`d6rf4_grade_drive.py` writes its fixture datum as
...
DIRECTION.  `D6RF4`'s defect is FAIL-**OPEN**, not fail-closed.  `stat -c '%Y'`
...
Usage:
    python3 d6rf4_age_datum_control.py            # the four rows
    python3 d6rf4_age_datum_control.py --drive    # + the defective-launcher drive
"""
import argparse
import os
import re
...
def produce_datum(fmt, sentinel, work):
    ...
    as `d6rf4_grade.py` does.  The floor this control hunts crosses a FILE
...
def accepts(product, datum):
    """THE PREDICATE BOTH CONSUMERS USE, verbatim in sense: `d6rf4_grade.py`
    (`age_pass = age_pass and mt > datum`) and `d6rf4_endpoint_physical.py`
...
def rows(launcher=LAUNCHER, tmp=None, verbose=True):
    ...
    tmp = tmp or tempfile.mkdtemp(prefix="d6rf4_age_control_")
...
def drive(tmp=None):
    ...
    defect itself."""
    tmp = tmp or tempfile.mkdtemp(prefix="d6rf4_age_drive_")
    with open(LAUNCHER, encoding="utf-8") as fh:
        src = fh.read()
    ...
    bad = os.path.join(tmp, "d6rf4_run_arm_DEFECTIVE.sh")
...
def main(argv=None):
    ...
    print("D6RF4 AGE-DATUM CONTROL -- 0.000 SOLVER CORE-MIN, NOTHING HERE IS A "
