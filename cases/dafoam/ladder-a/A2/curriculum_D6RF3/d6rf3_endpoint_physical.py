#!/usr/bin/env python
"""Curriculum D6RF3 -- the MULTIPOINT endpoint repair, as a WRAPPER that edits
...
written); the deltas are in `d6rf3_endpoint_physical_DELTAS_from_d6rf2.diff`
...
are recorded here so a reader does not have to reconstruct why they moved:
  ...
  D1  `EXTRACTOR` / `MD5_EXTRACT` are RE-PINNED to `d6rf3_extract_endpoint.py`.
  ...
  D2  Every product name is this item's own (`d6rf3_*`).
  D3  ⚠ `PHYSICAL` IS `d6rf3_endpoint_dvs_PHYSICAL.json`, WHICH REPAIRS
      ...
      `d6rf3_grade.py` additionally carries an executable `PRODUCT_WRITER` check
...
import d6rf3_endpoint_locus as locus                                # noqa: E402

EXTRACTOR = "d6rf3_extract_endpoint.py"
RUNSCRIPT = "d6rf3_opt_runScript.py"
...
SCALED_OUT = "d6rf3_endpoint_dvs.json"                 # the frozen extractor writes this
PRESERVED = "d6rf3_endpoint_dvs_DRIVERSCALED.json"     # the pre-repair pre-image
PHYSICAL = "d6rf3_endpoint_dvs_PHYSICAL.json"         # the corrected artefact
HISTORY = "d6rf3_major_history.json"


def refuse(msg):
    sys.stderr.write("D6RF3_ENDPOINT_PHYSICAL REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    for stale in (SCALED_OUT, PRESERVED, PHYSICAL, HISTORY):
        if os.path.exists(stale):
            os.remove(stale)
    spec = importlib.util.spec_from_file_location("d6rf3_extract_frozen",
                                                  os.path.abspath(EXTRACTOR))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["d6rf3_extract_frozen"] = mod
    ...
    sys.stdout.write("D6RF3_ENDPOINT_PHYSICAL_WRITTEN %s scalers=%s "
