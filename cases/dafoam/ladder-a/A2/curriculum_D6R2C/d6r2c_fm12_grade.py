...
def selftest():
    ...
    try:
        ...
        check("--item accepts EVERY registered arm id, so a re-run id cannot "
              ...
              all(a in ARM_IDS for a in ("FM12", "FM12R2", "FM12R3")), True)
        check("...and the re-run ids are FM12's OWN, not FM11's -- a launcher "
              "that passed FM13 here would be refused rather than graded under "
              "a registration that is not this one",
              "FM13" in ARM_IDS, False)
    finally:
...
def main(argv=None):
    ...
    if a.print_cap:
        ...
        return 0
    if a.print_cost:
        print("D6R2C_FM12_COST core_min_per_objective_evaluation=%.3f "
