#!/usr/bin/env bash
# ===========================================================================
# d6r2c_fm9_run_arm.sh -- LAUNCHER for arm FM9 (the fresh mesh actually arrives)
# ===========================================================================
#
# Registered by PREREGISTRATION_FRESHMESH_ARRIVES.md sections 5, 8, 9 and 11,
# and IN THE SAME COMMIT, BEFORE ANY CONTAINER STARTS (CLAUDE.md rule 2).
#
# A FORK of d6r2c_fm6_run_arm.sh (md5 cead1008eeb4d151c0ccddae2653d64b), unedited
# on disk.  WHAT CHANGED, EXHAUSTIVELY:
#   E1 -- THE CONTAINER RUNS deform -> mesh -> STAGE -> solve.  `stage` is the one
#         registered change: it puts the fresh mesh into every condition case and
#         REMOVES the stale decomposition, so the solve phase's prob.setup()
#         decomposes THE FRESH MESH.  FM5, FM7 and FM8 had no such phase and each
#         solved on a decomposition of the BASE mesh built one phase earlier.
#   E2 -- NO FIELD TRANSFER.  d6r2c_fm6_init.py is NOT staged and NOT run: the
#         cell-for-cell index copy is invalid once the fresh mesh genuinely
#         arrives (measured warped-to-fresh displacement, median 17.8 first-cell
...
#         was right and the LABEL named the wrong file, and a constant sweep
#         cannot see a string inside an echo.
#
set -uo pipefail

ITEM=D6R2C-FM9
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6R2C-FM9-a2-wing-freshmesh-arrives
...
/home/ubuntu/dafoam-tutorials/MACH_Tutorial_Wing"
  ...
  echo "D6R2C_FM9_G_ROOT_PASS item=$ITEM base=$BASE_REAL"
  return 0
}
...
if missing:
    ...
    for x in missing:
        print("   " + x)
    sys.exit(1)
print("D6R2C_FM9_G_DEPS_PASS scanned=%d staged=%d" % (scanned, len(staged)))
DEPSPY
}
...
guard_freeze() {
  ...
  for f in d6r2c_fm9_grade.py d6r2c_fm9_stage.py d6r2c_freshmesh.py d6r2c_decomp.py; do
...
") || {
  ...
  echo "D6R2C_FM9_G_BOX_PASS load1=$load1 nproc=$nproc solver_swap_offenders=0 avail_gb=$avail_gb"
  return 0
}
...
seed_arm() {
  ...
  for bad in "$WORK/0" "$WORK/constant" "$WORK/mp04" "$WORK/d6r2c_freshmesh.json" "$WORK/d6r2c_fm9_stage.json"; do
  ...
  echo "D6R2C_FM9_G_COLD_PASS arm=$ARM age_datum_epoch=$AGE_DATUM"
  return 0
}
...
rotator() {
  ...
  while :; do
    ...
    mkdir -p "$D"
    for f in d6r2c_freshmesh.json d6r2c_fm9_stage.json h1.json d6r2c_x0_final.json mesh_rc.txt; do
...
if [ "$ARM" = "--selftest" ]; then
  ...
  [ "$(cap_core_min FM9)" = "34.200" ] && echo "SELFTEST ok the registered cap for FM9 is section 8's 34.200" || { echo "SELFTEST FAIL cap"; rc=1; }
  # ... and that it came from the GRADER, so the two cannot drift apart again.
  [ "$(cap_core_min FM9)" = "$(python3 "$SRC/d6r2c_fm9_grade.py" --print-cap FM9)" ] \
  ...
  grep -q "^# PIN d6r2c_fm9_grade.py " "$0" && grep -q "^# PIN d6r2c_fm9_stage.py " "$0" \
  ...
  PH=$(printf '%s\n' "$CMDBLOCK" | grep -E "^(mpirun|\[ .rc| *python)" | \
       grep -oE "phase deform|phase mesh|python d6r2c_fm9_stage\.py|phase solve" | tr '\n' ' ')
  [ "$PH" = "phase deform phase mesh python d6r2c_fm9_stage.py phase solve " ] \
    && echo "SELFTEST ok the container runs deform -> mesh -> STAGE -> solve, in that order" \
  ...
  # NO G-VERIFY CONTROLS (E2): there is no field transfer in this arm.  The
  # equivalent question -- DID THE FRESH MESH ARRIVE -- is M1, driven by the
  # grader's own selftest against a deliberately un-staged copy.
  # AND THE TRANSFER MUST NOT BE REFERENCED IN THE EXECUTED PATH: the index copy
  # is invalid once the mesh genuinely arrives, and staging it would invite it.
...
echo "D6R2C_FM9_CAP arm=$ARM cap_core_min=$CAP source=d6r2c_fm9_grade.py --print-cap"
LIVE=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d6r2c_fm9_${ARM}_" 2>/dev/null | head -3 | tr '\n' ',')
[ -n "$LIVE" ] && { echo "ABORT G-LIVE arm $ARM already running: $LIVE"; exit 3; }
mkdir -p "$BASE" && echo "ITEM=$ITEM" >> "$BASE/ledger.txt"
seed_arm "$ARM" || exit $?

STAMP=$(date -u +%Y%m%dT%H%M%SZ)_$$
NAME="d6r2c_fm9_${ARM}_${STAMP}"
WORK="$BASE/$ARM"
LOG="$BASE/${ARM}_${STAMP}.log"

CMDFILE="$WORK/d6r2c_fm9_cmd.sh"
cat > "$CMDFILE" <<'CMD'
set -uo pipefail
echo "D6R2C_FM9_DEADLINE_IN_CONTAINER_S: NONE"
rc=0
...
# ---- THE ONE REGISTERED CHANGE.  Serial: it is file work, not compute.  It puts
# ---- the fresh mesh into every condition case and REMOVES the stale
# ---- decomposition, so the solve phase's prob.setup() decomposes THE FRESH MESH.
# ---- Without this the solver reads the decomposition the DEFORM phase built from
# ---- the BASE mesh -- which is what FM5, FM7 and FM8 each did.
[ $rc -eq 0 ] && { python d6r2c_fm9_stage.py --arm-dir "$PWD" || rc=$?; }
...
echo "D6R2C_FM9_RC=$rc"
exit $rc
CMD
echo "D6R2C_FM9_CMD arm=$ARM md5=$(md5sum "$CMDFILE" | cut -d' ' -f1)"
...
sudo -n docker run -d --name "$NAME" \
    ...
    ". /home/dafoamuser/dafoam/loadDAFoam.sh && bash /mnt/$ARM/d6r2c_fm9_cmd.sh" \
    > /dev/null 2>&1 || { echo "ABORT docker run failed"; exit 6; }
echo "D6R2C_FM9_LAUNCHED name=$NAME arm=$ARM uid=${RUN_UID}:${RUN_GID}+${EXTRA_GID} ranks=$RANKS cpuset=$CPUSET"
...
{
  echo "ITEM=$ITEM"
  echo "D6R2C_FM9_ROW arm=$ARM name=$NAME rc=$RC wall_s=$WALL core_min=$CORE_MIN cap=$CAP root_owned=$ROOT_OWNED log=$LOG"
} >> "$BASE/ledger.txt"
awk -v c="$CORE_MIN" -v cap="$CAP" 'BEGIN{exit !(c>cap)}' && \
  echo "D6R2C_FM9_CAP_CROSSED arm=$ARM core_min=$CORE_MIN cap=$CAP -- the row is graded NOT A RESULT and THE CAP IS NEVER RAISED" | tee -a "$BASE/ledger.txt"
echo "D6R2C_FM9_DONE arm=$ARM rc=$RC core_min=$CORE_MIN cap=$CAP root_owned=$ROOT_OWNED"
# NO G-VERIFY (E2): there is no field transfer in this arm, so there is nothing
# for a warm-start check to verify.  The equivalent question here -- did the
# fresh mesh arrive -- is M1, in d6r2c_fm9_grade.py, and the grading line below
# hands it this arm's log for the corroboration limb.
echo "NOW GRADE:  python3 $SRC/d6r2c_fm9_grade.py --item FM9 \\"
echo "              --arm-dir $WORK --datum-file $WORK/.d6r2c_age_datum \\"
echo "              --core-min $CORE_MIN --rc $RC --log $LOG \\"
echo "              --out $BASE/FM9_GRADE.json"
exit 0
