#!/usr/bin/env python3
# ===========================================================================
# d6r2c_fm11_grade.py -- THE FM11 GRADER: G1, G2, D1, D2, M0, M1, W1, Q1, H4
# ===========================================================================
#
# Registered by PREREGISTRATION_FM11_MATCHED_LIFT.md sections 3, 5, 6 and 7, and
...
#       answered by d6r2c_fm11_prefreeze.sh, which hashes the table's own rows.
#   (2) the pre-freeze check drives THE CLI THE LAUNCHER EMITS -- answered by
#       the same script, which asks d6r2c_fm11_run_arm.sh --emit-grade-cmd for
...
ZB_TRIM_WALL_S = 574.5           # DEC7 STATE B
ZO_TRIM_WALL_S = 2400.3          # DEC7 STATE S
...
N_GRAD_EVALS_THIS_ARM = 0        # FM11 runs NO adjoint.  Registered so the zero
...
import d6r2c_fm11_states as prod       # noqa: E402
...
def load_inherited(evals_path, require_md5=True):
    ...
    def _J(rec):
        ...
        cd = {}
        for p in POINTS:
            for k in ("%s.aero_post.CD" % p, "%s_CD" % p, "CD_%s" % p):
                if k in fu:
                    cd[p] = fu[k]
                    break
        if len(cd) == len(POINTS):
            return weighted_J(cd), "recomputed from the per-condition CD"
        for k in ("obj", "J", "fun", "weighted_CD"):
            if k in fu:
                return _finite(fu[k], k), "the record's own %s" % k
...
def grade_fm11(arm_dir, datum_epoch, inherited, fm10, core_min, rc,
    ...
    # ---- THE LABEL ---------------------------------------------------------
    hard = {"G2": g2["pass"], "D2": d2["pass"], "M1": m1_ok, "W1": w1_ok,
            "H4": h4["pass"]}
    ...
    for s in SUB_ARMS:
        mm = (m0[s] or {}).get("max_point_difference_from_base_m")
        if s == "Zb" and mm not in (None, 0.0):
            findings.append("MESH REPRODUCIBILITY: the Zb mesh does not "
                            "reproduce the base mesh; max point difference "
                            "%.6e m" % mm)
...
def _synth_arm(root, J_base, J_opt, cl_base=None, cl_opt=None, cl_ez=None,
    ...
    """A complete synthetic FM11 arm: two sub-arms, four states, meshes the
                ...
                for i in range(N_PROCESSORS):
                    shutil.copyfile(
                        ...
                        os.path.join(d, mp, "processor%d" % i, "1000",
                                     "polyMesh", "points"))
        rows = []
        hdr = {"kind": "HEADER", "sub_arm": s, "uid": uid, "ranks": 4,
               "M0": {"max_point_difference_from_base_m": 0.0 if s == "Zb" else 1.5,
                      "differs_from_base": s == "Zo", "pass": True}}
        rows.append(hdr)
...
def selftest():
    ...
    tmp = tempfile.mkdtemp(prefix="d6r2c_fm11_grade_selftest_")
    try:
        ...
        check("--item accepts EVERY registered arm id, so a re-run id cannot "
              ...
              all(a in ARM_IDS for a in ("FM11", "FM12", "FM13")), True)
    finally:
        shutil.rmtree(tmp, ignore_errors=True)

    print("D6R2C_FM11_GRADE SELFTEST %s n=%d" % ("PASS" if ok else "FAIL", n))
...
def build_parser():
    ap = argparse.ArgumentParser(description="Grade arm FM11 -- the comparison "
                                             "at matched lift.")
...
def main(argv=None):
    ...
    if a.print_cap:
        ...
        return 0
    if a.print_cost:
        print("D6R2C_FM11_COST core_min_per_objective_evaluation=%.3f "
    ...
    try:
        ...
        print("D6R2C_FM11_GRADE REFUSED\n%s" % e)
        return 2
    ...
    g1 = rec["G1_detail"]
    print("D6R2C_FM11_GRADE item=%s label=%s R_fresh=%.9g R_def=%.9g "
          "window=[%.6f, %.6f] J_base=%.9g J_opt=%.9g"
          % (a.item, rec["label"], g1["R_fresh"], g1["R_def"],
             g1["window"][0], g1["window"][1], g1["J_base_fresh"],
             g1["J_opt_fresh"]))
    print("D6R2C_FM11_GRADE G1=%s G2=%s D1=%s D2=%s M1=%s W1=%s H4=%s "
          "Q1_no_breach=%s" % (rec["G1"], rec["G2"], rec["D1"], rec["D2"],
                               rec["M1"], rec["W1"], rec["H4"],
                               rec["Q1_reported_not_gated"]))
    for f in rec["findings"]:
        print("D6R2C_FM11_FINDING %s" % f)
    print("D6R2C_FM11_GRADE verdict=%s written=%s" % (rec["label"], out))
    return 0
