#!/usr/bin/env python3
# ===========================================================================
# d6r2c_dec4_grade.py -- THE GRADING PATH FOR THE SUCCESSOR TO AFTER-ITEM 8
# ===========================================================================
#
...
# this file carried `CAPS = {"DEC4": 407.303}` as a bare literal, taken from a
...
# CAPS["DEC4"] to build their own boundary -- self-referential, and therefore
...
# "PREDICTION 2084.6 s wall x 4 ranks / 60 = 138.973 core-min"  (sec 8)
PREDICTION_CORE_MIN = 138.973
# "REGISTERED CAP (3.00x) = 416.919 core-min"  (sec 8)
CAP_FACTOR = 3.00
CAPS = {"DEC4": round(PREDICTION_CORE_MIN * CAP_FACTOR, 3)}
...
def grade_item8(jsonl_path, arm_dir, datum_epoch, inherited, core_min,
    ...
    D2 = (d2_detail["rel_B_ok"] and d2_detail["rel_O_ok"]
          and d2_detail["B_CL_ok"] and d2_detail["O_CL_ok"] and D2GEO)
...
def _synth_decomp(tmp, JB, JT, JS, JF, JO, cl_miss=0.0, fail=0,
    ...
    d = os.path.join(tmp, "DEC4")
    os.makedirs(d, exist_ok=True)
    path = os.path.join(d, "d6r2c_dec4.jsonl")
...
def selftest():
    ...
    try:
        ...
        r = grade_item8(p, d, DATUM, inh, 300.0, 0)
        ok &= check("D2 J_B off by 2e-4 -> NOT A RESULT", r["label"], LABEL_NOT_A_RESULT)
        ok &= check("D2 false (J_B)", r["D2"], False)
        p, d = _synth_decomp(tmp + "/e", JB, JT, JS, JF, JO * (1 + 2.0e-4))
        ok &= check("D2 J_opt off by 2e-4 -> NOT A RESULT",
                    grade_item8(p, d, DATUM, inh, 300.0, 0)["label"], LABEL_NOT_A_RESULT)
        p, d = _synth_decomp(tmp + "/f", JB, JT, JS, JF, JO,
                             o_cl={p2: inh["CL_final"][p2] + 1e-5 for p2 in POINTS})
        ok &= check("D2 J_opt CL drift 1e-5 -> NOT A RESULT",
                    grade_item8(p, d, DATUM, inh, 300.0, 0)["label"], LABEL_NOT_A_RESULT)
        ...
        # from CAPS["DEC4"] and would pass for ANY value it held.  That is how a
        ...
        ok &= check("EXTERNAL: the prediction is section 8's 138.973",
                    PREDICTION_CORE_MIN, 138.973)
        ok &= check("EXTERNAL: the registered cap is section 8's 416.919",
                    CAPS["DEC4"], 416.919)
    ...
    print("D6R2C_DEC4_GRADE SELFTEST %s n=%d" % ("PASS" if ok else "FAIL", len(controls)))
...
def main(argv=None):
    ...
    try:
        ...
        kw = dict(jsonl_path=os.path.join(a.arm_dir, "d6r2c_dec4.jsonl"),
        ...
        print("D6R2C_DEC4_GRADE REFUSED\n%s" % e)
        return 2
    ...
    print("D6R2C_DEC4_GRADE item=8R2 label=%s table=%s" % (rec["label"], md))
    print("D6R2C_DEC4_GRADE verdict=%s written=%s" % (rec["label"], out))
    return 0
