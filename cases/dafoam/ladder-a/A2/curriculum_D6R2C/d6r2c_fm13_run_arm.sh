...
ITEM=D6R2C-FM13
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6R2C-FM13-a2-wing-matched-lift-guarded-write
FM12_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6R2C-FM12-a2-wing-matched-lift-provenance
...
STAGED_PY="d6r2c_opt_runScript.py d6r2c_fm13_deform.py d6r2c_freshmesh.py d6r2c_decomp.py d6r2c_fm9_stage.py d6r2c_fm12_stage.py d6r2c_dec5_decomp.py d6r2c_fm12_states.py"
...
PINNED_PY="d6r2c_fm13_grade.py d6r2c_fm13_deform.py d6r2c_fm12_states.py d6r2c_fm12_stage.py d6r2c_fm9_stage.py d6r2c_freshmesh.py d6r2c_decomp.py d6r2c_dec5_decomp.py"
SUB_ARMS="Zb Zo"
...
cap_core_min() {
  case "$1" in
    FM13|FM13R2|FM13R3) python3 "$SRC/d6r2c_fm13_grade.py" --print-cap "$1" 2>/dev/null ;;
    *) echo "" ;;
  esac
}
...
grade_cmd() {
  ...
  printf '%s' "python3 $SRC/d6r2c_fm13_grade.py --item $ARM --arm-dir $WORK \
...
container_cmd() {
  ...
  cat <<CMD
set -uo pipefail
echo "D6R2C_FM13_DEADLINE_IN_CONTAINER_S: NONE"
rc=0
...
cd /mnt/$ARM/Zo
# ---- THE ONE REGISTERED CHANGE OF THIS ARM.  d6r2c_freshmesh.py:268 executes
# ---- grid.writeToCGNS on ALL FOUR RANKS onto one path -- rank0 is computed at
# ---- :213 and first USED at :301, 88 lines after the write.  FM12 lost that
# ---- race (8192 bytes against 114688, rank 1 exit 1); FM10 won it.  This
# ---- wrapper guards the write to rank 0, barriers either side, and asserts the
# ---- result on EVERY rank.  d6r2c_freshmesh.py IS NOT EDITED (rule 6).
[ \$rc -eq 0 ] && { mpirun -np $RANKS python d6r2c_fm13_deform.py --phase deform --arm-dir "\$PWD" \\
...
if [ "$ARM" = "--emit-cmd" ]; then container_cmd "${2:-FM13}"; exit 0; fi
...
if [ "$ARM" = "--selftest" ]; then
  ...
  guard_root "$FM12_BASE" >/dev/null 2>&1 && { echo "SELFTEST FAIL G-ROOT ACCEPTED FM12'S ROOT, WHICH HOLDS THE PRESERVED 8192-BYTE CORRUPT CGNS -- THE EVIDENCE FOR THIS ARM'S EXISTENCE"; rc=1; } || echo "SELFTEST ok G-ROOT refuses FM12's root -- FM12 is graded BLOCKED and its Zo/surfaceMesh_final.cgns is this arm's KNOWN-BAD CONTROL, READ and never written"
  ...
  CAP11=$(cap_core_min FM13)
  [ -n "$CAP11" ] && echo "SELFTEST ok the cap comes from the grader: $CAP11 core-min" || { echo "SELFTEST FAIL no cap"; rc=1; }
  [ "$CAP11" = "$(cap_core_min FM13R2)" ] && [ "$CAP11" = "$(cap_core_min FM13R3)" ] \
    && echo "SELFTEST ok FM13R2 and FM13R3 carry the IDENTICAL registered cap -- a re-run id is the same number under another key" \
    || { echo "SELFTEST FAIL the re-run ids do not carry FM13's cap"; rc=1; }
  [ -z "$(cap_core_min FM12)" ] && [ -z "$(cap_core_min FM12R2)" ] \
    && echo "SELFTEST ok FM12's OWN arm ids get NO cap here, so an arm registered by ANOTHER document cannot be launched by this one" \
    || { echo "SELFTEST FAIL this launcher answers for an arm id it does not register"; rc=1; }
  # ---- THE ARM-ID COLLISION, ASSERTED RATHER THAN INHERITED -----------------
  # PREREGISTRATION_FM11_MATCHED_LIFT.md:349 registers FM13 as one of FM11's
  # re-run ids and d6r2c_fm11_run_arm.sh:99,:671 will launch it under FM11's cap.
  # FM11 is graded BLOCKED.  THIS DOCUMENT ALSO REGISTERS FM13.  The two cannot
  # be told apart by arm id, so they are told apart by RUN ROOT and by the
  # ledger's ITEM= line -- and that separation is asserted here, so the collision
  # is READ rather than discovered.
  [ "$REGISTERED_BASE" != "$FM11_BASE" ] && [ "$ITEM" = "D6R2C-FM13" ] \
    && echo "SELFTEST ok the FM13 arm id is shared with FM11's registration; this item is separated from it by run root ($REGISTERED_BASE) and by ITEM=$ITEM, never by arm id alone" \
    || { echo "SELFTEST FAIL the FM13 collision is not separated by root and ITEM"; rc=1; }
  ...
  for FLAG in $(printf '%s\n' "$GC" | tr ' ' '\n' | grep '^--'); do
    python3 "$SRC/d6r2c_fm13_grade.py" --help 2>/dev/null | grep -q -- "$FLAG" \
  ...
  PH=$(container_cmd FM13 | grep -oE "phase mesh|phase deform|--sub-arm Zb|--sub-arm Zo" | tr '\n' ' ')
  # THE DEFORM PHASE MUST RUN THE GUARDED PRODUCER, AND THE UNGUARDED ONE MUST
  # NOT APPEAR ON A DEFORM LINE ANYWHERE IN THE EMITTED BLOCK.
  container_cmd FM13 | grep -q "d6r2c_fm13_deform.py --phase deform" \
    && echo "SELFTEST ok the deform phase runs the GUARDED producer d6r2c_fm13_deform.py" \
    || { echo "SELFTEST FAIL the deform phase does not run the guarded producer"; rc=1; }
  container_cmd FM13 | grep -q "d6r2c_freshmesh.py --phase deform" \
    && { echo "SELFTEST FAIL the UNGUARDED d6r2c_freshmesh.py --phase deform is still emitted -- that is the collective write that blocked FM12"; rc=1; } \
    || echo "SELFTEST ok d6r2c_freshmesh.py --phase deform, the unguarded collective write that blocked FM12, IS NOT EMITTED BY THIS ARM"
  ...
  container_cmd FM13 | grep -q -- "--phase solve" \
  ...
  [ "$(bash "$0" --emit-cmd FM13)" = "$(container_cmd FM13)" ] \
  ...
  [ "$rc" -eq 0 ] && echo "D6R2C_FM13_LAUNCH SELFTEST PASS" || echo "D6R2C_FM13_LAUNCH SELFTEST FAIL"
  exit $rc
fi

test -n "$ARM" || { echo "ABORT usage: d6r2c_fm13_run_arm.sh <FM13|FM13R2|FM13R3> <image>  |  --selftest  |  --emit-cmd  |  --emit-grade-cmd"; exit 64; }
case "$ARM" in FM13|FM13R2|FM13R3) ;; *) echo "ABORT arm $ARM is not registered by this document"; exit 64 ;; esac
...
echo "D6R2C_FM13_CAP arm=$ARM cap_core_min=$CAP source=d6r2c_fm13_grade.py --print-cap"
python3 "$SRC/d6r2c_fm13_grade.py" --print-cost
...
CMDFILE="$WORK/d6r2c_fm13_cmd.sh"
container_cmd "$ARM" > "$CMDFILE"
echo "D6R2C_FM13_CMD arm=$ARM md5=$(md5sum "$CMDFILE" | cut -d' ' -f1)"
...
sudo -n docker run -d --name "$NAME" \
    ...
    ". /home/dafoamuser/dafoam/loadDAFoam.sh && bash /mnt/$ARM/d6r2c_fm13_cmd.sh" \
    > /dev/null 2>&1 || { echo "ABORT docker run failed"; exit 6; }
echo "D6R2C_FM13_LAUNCHED name=$NAME arm=$ARM uid=${RUN_UID}:${RUN_GID}+${EXTRA_GID} ranks=$RANKS cpuset=$CPUSET"
...
awk -v c="$CORE_MIN" -v cap="$CAP" 'BEGIN{exit !(c>cap)}' && \
  echo "D6R2C_FM13_CAP_CROSSED arm=$ARM core_min=$CORE_MIN cap=$CAP -- the row is graded NOT A RESULT and THE CAP IS NEVER RAISED" | tee -a "$BASE/ledger.txt"
echo "D6R2C_FM13_DONE arm=$ARM rc=$RC core_min=$CORE_MIN cap=$CAP root_owned=$ROOT_OWNED"
...
# FM12's stager, state producer and grader ARE REUSED UNCHANGED: the Zb sub-arm
# ran to rc=0 on them and its numbers stand.  THE TWO NEW INSTRUMENTS ARE
# d6r2c_fm13_deform.py and d6r2c_fm13_grade.py, and the ONE REGISTERED CHANGE --
# the CGNS write guarded to rank 0, barriered either side, and asserted on every
# rank -- is in d6r2c_fm13_deform.py, which WRAPS d6r2c_freshmesh.py rather than
# editing it.  The grader differs from FM12's ONLY in ARM_IDS and in
# ZB_TRIM_WALL_S, re-anchored 574.5 -> 61.113 s on FM12's own measured Zb.
# PIN d6r2c_fm13_grade.py 04a2735424527e2b2650a3ce3e13e4bd
# PIN d6r2c_fm13_deform.py bcb0685b56453bf96eda98b21d233556
...
# PIN d6r2c_fm12_run_arm.sh 97e7d57702d8ada8ccd1354ccc077798   (the file this one forks)
