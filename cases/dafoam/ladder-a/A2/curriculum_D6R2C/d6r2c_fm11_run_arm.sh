#!/usr/bin/env bash
# ===========================================================================
# d6r2c_fm11_run_arm.sh -- LAUNCHER for arm FM11 (the comparison at matched lift)
# ===========================================================================
#
# Registered by PREREGISTRATION_FM11_MATCHED_LIFT.md sections 2, 5, 6, 7 and 8,
# and IN THE SAME COMMIT AS THAT DOCUMENT, BEFORE ANY CONTAINER STARTS (rule 2).
#
# A FORK of d6r2c_fm9_run_arm.sh (md5 recorded at the PIN block below).  The
...
# never logic:
#   (i)   the printed token D6R2C_FM9_* became D6R2C_FM11_*;
...
#         (L-221/L-222);
#   (iii) the rotator walks $WORK/<sub-arm>/mp0*, because FM11's conditions sit
#         one level deeper than FM9's.
# seed_arm IS REPLACED, because FM11 seeds TWO SUB-ARMS rather than one arm.
#
...
# sec 22.3).  FM11 is the like-for-like evaluation FM10 failed to be: every solve
...
# `stage` as its own process between `mesh` and `solve`.  FM11's producer stages
...
ITEM=D6R2C-FM11
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6R2C-FM11-a2-wing-matched-lift
...
# d6r2c_freshmesh.py imports it; d6r2c_dec5_decomp.py because the FM11 producer
# imports the trim, the governor and the frozen-model loader from it.
STAGED_PY="d6r2c_opt_runScript.py d6r2c_freshmesh.py d6r2c_decomp.py d6r2c_fm9_stage.py d6r2c_dec5_decomp.py d6r2c_fm11_states.py"
...
PINNED_PY="d6r2c_fm11_grade.py d6r2c_fm11_states.py d6r2c_fm9_stage.py d6r2c_freshmesh.py d6r2c_decomp.py d6r2c_dec5_decomp.py"
SUB_ARMS="Zb Zo"
...
# THE CAP IS A PROPERTY OF THE ITEM, NOT OF THE ARM ID.  FM12 and FM13 are the
# registered re-run ids and carry THE IDENTICAL registered figure -- the same
...
cap_core_min() {
  case "$1" in
    FM11|FM12|FM13) python3 "$SRC/d6r2c_fm11_grade.py" --print-cap "$1" 2>/dev/null ;;
    *) echo "" ;;
  esac
}
...
grade_cmd() {
  ...
  printf '%s' "python3 $SRC/d6r2c_fm11_grade.py --item $ARM --arm-dir $WORK \
...
container_cmd() {
  ...
  cat <<CMD
set -uo pipefail
echo "D6R2C_FM11_DEADLINE_IN_CONTAINER_S: NONE"
rc=0
...
[ \$rc -eq 0 ] && { mpirun -np $RANKS python d6r2c_fm11_states.py --arm-dir "\$PWD" \\
...
/home/ubuntu/dafoam-tutorials/MACH_Tutorial_Wing"
  ...
  echo "D6R2C_FM11_G_ROOT_PASS item=$ITEM base=$BASE_REAL"
  return 0
}
...
if clash:
    ...
    sys.exit(1)
print("D6R2C_FM11_G_CPUSET_PASS cpuset=%s occupied=%s"
...
if missing:
    ...
    for x in missing:
        print("   " + x)
    sys.exit(1)
print("D6R2C_FM11_G_DEPS_PASS scanned=%d staged=%d" % (scanned, len(staged)))
DEPSPY
}
...
") || {
  ...
  echo "D6R2C_FM11_G_BOX_PASS load1=$load1 nproc=$nproc solver_swap_offenders=0 avail_gb=$avail_gb"
  return 0
}
...
seed_arm() {
  ...
  for SA in $SUB_ARMS; do
    ...
    for bad in "$SUB/0" "$SUB/constant" "$SUB/mp04" "$SUB/d6r2c_fm11.jsonl"; do
    ...
    echo "D6R2C_FM11_G_COLD_PASS arm=$ARM sub_arm=$SA"
  done
  ...
  echo "D6R2C_FM11_G_COLD_PASS arm=$ARM age_datum_epoch=$AGE_DATUM sub_arms=$SUB_ARMS"
  return 0
}
...
rotator() {
  ...
  while :; do
    ...
    mkdir -p "$D"
    for sa in Zb Zo; do
      for f in d6r2c_fm11.jsonl d6r2c_fm9_stage.json h1.json mesh_rc.txt; do
...
if [ "$ARM" = "--emit-grade-cmd" ]; then
  grade_cmd "${2:-FM11}" "${3:-$BASE/FM11}" "${4:-0.000}" "${5:-0}" \
            "${6:-$BASE/FM11.log}" "${7:-$BASE/FM11_GRADE.json}"; echo; exit 0
fi
if [ "$ARM" = "--emit-cmd" ]; then container_cmd "${2:-FM11}"; exit 0; fi
...
if [ "$ARM" = "--selftest" ]; then
  ...
  CAP11=$(cap_core_min FM11)
  [ -n "$CAP11" ] && echo "SELFTEST ok the cap comes from the grader: $CAP11 core-min" || { echo "SELFTEST FAIL no cap"; rc=1; }
  [ "$CAP11" = "$(cap_core_min FM12)" ] && [ "$CAP11" = "$(cap_core_min FM13)" ] \
    && echo "SELFTEST ok FM12 and FM13 carry the IDENTICAL registered cap -- a re-run id is the same number under another key" \
    || { echo "SELFTEST FAIL the re-run ids do not carry FM11's cap"; rc=1; }
  ...
  GC=$(grade_cmd FM11 /tmp/w 1.000 0 /tmp/l.log /tmp/o.json)
  case "$GC" in
    *"--item FM11"*) echo "SELFTEST ok the emitted grade command names the ARM, not a stale item id (L-570)" ;;
  ...
  [ "$(bash "$0" --emit-grade-cmd FM11 /tmp/w 1.000 0 /tmp/l.log /tmp/o.json)" = "$GC" ] \
  ...
  for FLAG in $(printf '%s\n' "$GC" | tr ' ' '\n' | grep '^--'); do
    python3 "$SRC/d6r2c_fm11_grade.py" --help 2>/dev/null | grep -q -- "$FLAG" \
  ...
  PH=$(container_cmd FM11 | grep -oE "phase mesh|phase deform|--sub-arm Zb|--sub-arm Zo" | tr '\n' ' ')
  ...
  container_cmd FM11 | grep -q -- "--phase solve" \
  ...
  [ "$(bash "$0" --emit-cmd FM11)" = "$(container_cmd FM11)" ] \
  ...
  if guard_deps d6r2c_fm11_states.py >/dev/null 2>&1; then
  ...
  case "$(guard_deps d6r2c_fm11_states.py 2>&1)" in
    *"d6r2c_fm11_states.py imports d6r2c_dec5_decomp"*)
  ...
  case "$PINNED_PY" in
    *d6r2c_fm11_states.py*d6r2c_fm9_stage.py*) echo "SELFTEST ok G-FREEZE pins the producer and the reader it imports" ;;
  ...
  for SA in $SUB_ARMS; do
    ARGS=$(container_cmd FM11 | tr '\n' ' ' | tr -d '\\' \
           | grep -oE "python d6r2c_fm11_states\.py [^|]*--sub-arm $SA [^|]*--x0 [^ ]+" | head -1)
    ARGS=${ARGS#python d6r2c_fm11_states.py }
    ARGS=$(printf '%s' "$ARGS" | sed 's|"\$PWD"|/tmp/fm11_selftest_arm|')
    OUT=$(cd "$SRC" && python3 d6r2c_fm11_states.py --parse-only $ARGS 2>&1)
    case "$OUT" in
      *"D6R2C_FM11_PARSE_ONLY sub_arm=$SA"*)
  ...
  (cd "$SRC" && python3 d6r2c_fm11_states.py --parse-only --sub-arm Zq >/dev/null 2>&1) \
  ...
  [ "$rc" -eq 0 ] && echo "D6R2C_FM11_LAUNCH SELFTEST PASS" || echo "D6R2C_FM11_LAUNCH SELFTEST FAIL"
  exit $rc
fi

test -n "$ARM" || { echo "ABORT usage: d6r2c_fm11_run_arm.sh <FM11|FM12|FM13> <image>  |  --selftest  |  --emit-cmd  |  --emit-grade-cmd"; exit 64; }
case "$ARM" in FM11|FM12|FM13) ;; *) echo "ABORT arm $ARM is not registered by this document"; exit 64 ;; esac
...
echo "D6R2C_FM11_CPUSET arm=$ARM chosen=$CPUSET nproc=$(nproc)"
echo "D6R2C_FM11_CPUSET   by container: ${OCC_DOCKER:-none}"
echo "D6R2C_FM11_CPUSET   by ${BUSY_PCT}%-busy sample over ${BUSY_INTERVAL}s: ${OCC_BUSY:-none}"

CAP=$(cap_core_min "$ARM"); test -n "$CAP" || { echo "ABORT no registered cap for arm $ARM"; exit 64; }
echo "D6R2C_FM11_CAP arm=$ARM cap_core_min=$CAP source=d6r2c_fm11_grade.py --print-cap"
python3 "$SRC/d6r2c_fm11_grade.py" --print-cost
LIVE=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d6r2c_fm11_${ARM}_" 2>/dev/null | head -3 | tr '\n' ',')
[ -n "$LIVE" ] && { echo "ABORT G-LIVE arm $ARM already running: $LIVE"; exit 3; }
mkdir -p "$BASE" && echo "ITEM=$ITEM" >> "$BASE/ledger.txt"
seed_arm "$ARM" || exit $?

STAMP=$(date -u +%Y%m%dT%H%M%SZ)_$$
NAME="d6r2c_fm11_${ARM}_${STAMP}"
WORK="$BASE/$ARM"
LOG="$BASE/${ARM}_${STAMP}.log"

CMDFILE="$WORK/d6r2c_fm11_cmd.sh"
container_cmd "$ARM" > "$CMDFILE"
echo "D6R2C_FM11_CMD arm=$ARM md5=$(md5sum "$CMDFILE" | cut -d' ' -f1)"
...
sudo -n docker run -d --name "$NAME" \
    ...
    ". /home/dafoamuser/dafoam/loadDAFoam.sh && bash /mnt/$ARM/d6r2c_fm11_cmd.sh" \
    > /dev/null 2>&1 || { echo "ABORT docker run failed"; exit 6; }
echo "D6R2C_FM11_LAUNCHED name=$NAME arm=$ARM uid=${RUN_UID}:${RUN_GID}+${EXTRA_GID} ranks=$RANKS cpuset=$CPUSET"
...
{
  echo "ITEM=$ITEM"
  echo "D6R2C_FM11_ROW arm=$ARM name=$NAME rc=$RC wall_s=$WALL core_min=$CORE_MIN cap=$CAP root_owned=$ROOT_OWNED log=$LOG"
} >> "$BASE/ledger.txt"
awk -v c="$CORE_MIN" -v cap="$CAP" 'BEGIN{exit !(c>cap)}' && \
  echo "D6R2C_FM11_CAP_CROSSED arm=$ARM core_min=$CORE_MIN cap=$CAP -- the row is graded NOT A RESULT and THE CAP IS NEVER RAISED" | tee -a "$BASE/ledger.txt"
echo "D6R2C_FM11_DONE arm=$ARM rc=$RC core_min=$CORE_MIN cap=$CAP root_owned=$ROOT_OWNED"
...
# ARE REUSED UNCHANGED -- deform, mesh, the OpenFOAM readers, the trim and the
# governor are the same bytes that ran FM10 and DEC7.  The two new instruments
# are the producer and the grader, and the ONE CHANGE is inside the producer.
# PIN d6r2c_fm11_grade.py ddceb552b4b4dcf749000d2b784f06d8
# PIN d6r2c_fm11_states.py 0cb143738c124b55067a7b19bc1a0c43
...
# PIN d6r2c_fm9_run_arm.sh 8694935d672e87109fad65dc6ef291e5   (the file this one forks)
