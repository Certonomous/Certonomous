#!/usr/bin/env python3
"""SELFTEST for `so3d_replay.py` -- every guard, refusal, control and gate of the
...
log, never creates the item's run root, and never writes `so3d_replay.json`
or `so3d_plant_report.json` anywhere the grading path would look.  It drives
...
commit (CLAUDE.md rule 2).  Giving `so3d_replay.py` a fixture-root override
...
_spec = importlib.util.spec_from_file_location("so3d_replay",
                                               os.path.join(HERE, "so3d_replay.py"))
...
def c_plant_control_passes_on_sound_reader():
    # cl06 owns the last banner; the early `End` the insertion targets belongs to
    # the PRETRIM record, so force the insertion into a real scenario record by
    # starting the End search inside the first iteration.
    ...
    try:
        ...
        return rep["status"] == "PASS", "plants=%s" % (
            {k: v["status"] for k, v in rep["plants"].items()})
    finally:
        _plant_restore(olds)
...
def c_plant_control_refuses_broken_attribution():
    ...
    lines = fixture(3, [(False, False, True), (False, True, True),
                        (True, False, True)])
    olds = _plant_env(lines)
    old_tol = R.AOA_MATCH_ABS_TOL
    R.AOA_MATCH_ABS_TOL = 1e-30
    try:
        return raises_refusal(R.score_plant_control, lines)
    finally:
        R.AOA_MATCH_ABS_TOL = old_tol
        _plant_restore(olds)
...
def c_no_assert_statements_in_reader():
    ...
    import ast
    bad = []
    for fn in ("so3d_replay.py", "so3d_replay_selftest.py"):
...
def main():
    ...
    print("SO3D READER SELFTEST -- __debug__=%s (python3 -O sets it False)" % __debug__)
