...
ITEM=D6RF2
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6RF2-a2-wing-multipoint-fd
...
echo "D6RF2_G_ROOT_PASS item=$ITEM base=$BASE_REAL ledger_clean=yes"
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d6rf2_" | tr '\n' ',' | sed 's/,$//'
}
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: d6rf2_run_arm.sh <F_mp|REF_off> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: d6rf2_run_arm.sh <F_mp|REF_off> <image>"; exit 64; }
...
# ---- any destructive act.
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d6rf2_${ARM}_" 2>/dev/null | grep "^d6rf2_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "D6RF2_G_ROOT5_PASS arm=$ARM live_same_arm_containers=none driver_pidfile=$([ -f "$PIDFILE" ] && echo present_owner_is_ancestor_or_stale || echo absent)"
...
echo "D6RF2_CAP_FRAME arm=$ARM registered_core_min=$CAP ranks=$RANKS deadline_in_container_s=$TMO frame_allowance_s=$FRAME_ALLOWANCE_S kill_grace_s=$KILL_GRACE_S worst_case_host_core_min=$BACKCHECK enforced_in_frame=container graded_in_frame=host_bracket_T0_T1"
...
echo "D6RF2_HOST_PRE arm=$ARM MemAvailable_GiB=$MEMAVAIL_GIB load1=$LOAD cpuset=$CPUSET siblings_pre=[$SIBLINGS_PRE]"
...
echo "$MD5_RUNSCRIPT6  $BASE/d6rf2_opt_runScript.py" | md5sum -c - || { echo "ABORT d6rf2_opt_runScript.py md5"; exit 4; }
echo "$MD5_FD  $BASE/d6rf2_fd_endpoint.py"           | md5sum -c - || { echo "ABORT d6rf2_fd_endpoint.py md5"; exit 4; }
echo "$MD5_EXTRACT6  $BASE/d6r_extract_endpoint.py"| md5sum -c - || { echo "ABORT d6r_extract_endpoint.py md5"; exit 4; }
echo "$MD5_REFOFF  $BASE/d6rf2_ref_off.py"           | md5sum -c - || { echo "ABORT d6rf2_ref_off.py md5"; exit 4; }
...
echo "$MD5_LOCUS6  $BASE/d6rf2_endpoint_locus.py"   | md5sum -c - || { echo "ABORT d6rf2_endpoint_locus.py md5"; exit 4; }
echo "$MD5_PHYS6  $BASE/d6rf2_endpoint_physical.py" | md5sum -c - || { echo "ABORT d6rf2_endpoint_physical.py md5"; exit 4; }
echo "$MD5_UNITS  $BASE/d6rf2_units_assert.py"      | md5sum -c - || { echo "ABORT d6rf2_units_assert.py md5"; exit 4; }
...
test "$ROW" = "PATCHED" || {
  ...
  exit 4; }
echo "D6RF2_G_ROW_PASS row=$ROW digest=$GOT_DIGEST"

STAMP=$(date -u +%Y%m%dT%H%M%SZ)_$$
NAME="d6rf2_${ARM}_${STAMP}"
WORK="$BASE/$ARM"
...
: > "$STAGE_EVID"
stage_say "D6RF2_STAGE arm=$ARM utc=$(date -u +%Y%m%dT%H%M%SZ) base=$BASE_REAL permission=$PERMISSION"
...
if [ "$ARM" = "F_mp" ]; then
  ...
  stage_say "D6RF2_STAGE_F_mp (S1) OK src=$SRC exists, OptView.hst present, dst=$WORK does not exist"
  # ------------------------------------------------------------------ S3
  ...
  stage_say "D6RF2_STAGE_F_mp (S3) OK undeformed reference mesh md5 $MD5_REF_MESH on base and on mp04 mp05 mp06 -- double-deformation confound eliminated by MEASUREMENT"
  # ------------------------------------------------------------------ S4
  # ===================================================================
  ...
  # d6rf2_endpoint_locus.py -- "a loop that can iterate zero times asserts
  ...
  stage_say "D6RF2_STAGE_F_mp (S4) source pre-copy: OptView.hst md5=$SRC_HST_MD5, entries under mp04/processor0 = $SRC_TIMEDIRS"
  ...
  COPY_EPOCH=$(date -u +%s); echo "$COPY_EPOCH" > "$WORK/.d6rf2_copy_epoch"
  ...
  stage_say "D6RF2_STAGE_F_mp (S4) OK cp -a (mtimes PRESERVED), copy_epoch=$COPY_EPOCH, SOURCE INTACT after the copy -- and the comparison COUNTED $SRC_TIMEDIRS entries on both sides, so it cannot have passed on a pair of false zeros"
  # ------------------------------------------------------------------ S5
  ...
  rm -f  "$WORK/mphys.html" "$WORK/opt_IPOPT.txt" "$WORK/d6rf2_cmd.sh" 2>/dev/null
  ...
  stage_say "D6RF2_STAGE_F_mp (S5) OK dropped $DROPPED of $BEFORE_DROP output time directories from the COPY, $REMAIN remain (required 0); 0/, 0.orig/ and processor*/0/ KEPT and asserted; OptView.hst and dRdWColoring_4.bin KEPT; SOURCE INTACT ($SRC_REMAIN pseudo-time dirs still under $SRC/mp04/processor0)"
  # ------------------------------------------------------------------ S6
  ...
  for p in d6rf2_endpoint_dvs_PHYSICAL.json d6r_endpoint_dvs_DRIVERSCALED.json \
           d6r_endpoint_dvs.json d6r_major_history.json d6r_fd_endpoint.json \
           d6r_fd_endpoint.jsonl; do
    rm -f "$WORK/$p"
  done
  stage_say "D6RF2_STAGE_F_mp (S7) OK this arm's six registered products swept from the copy"
else
  # ---------------------------------------------------------- REF_off, cold
  ...
  stage_say "D6RF2_D4_HST_STAGED arm=$ARM md5=$(md5sum "$WORK/OptView.hst" | cut -d' ' -f1) source=$D4_HST_SRC"
...
if [ -e "$WORK/$DVFILE" ]; then
  stage_say "D6RF2_UNITS_CALLSITE_1 arm=$ARM an endpoint artefact SURVIVED the sweep: $WORK/$DVFILE -- grading it before anything else runs"
  python3 "$WORK/d6rf2_units_assert.py" "$WORK/$DVFILE" --runscript "$WORK/$UNITS_RUNSCRIPT"
  UR=$?
  if [ "$UR" -ne 0 ]; then
  ...
  stage_say "D6RF2_UNITS_CALLSITE_1 arm=$ARM no endpoint artefact survived the sweep (expected on a first fire); call site 2 grades the one the wrapper writes"
fi
...
stage_say "D6RF2_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM (every registered product must be strictly newer)"
...
# ===========================================================================
case "$ARM" in
  F_mp)    CMD="python d6rf2_endpoint_physical.py --age-datum $AGE_DATUM && python d6rf2_units_assert.py d6r_endpoint_dvs.json --runscript d6rf2_opt_runScript.py && mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6rf2_fd_endpoint.py" ;;
  REF_off) CMD="python d4_endpoint_physical.py --age-datum $AGE_DATUM && python d6rf2_units_assert.py d4_endpoint_dvs.json --runscript d4_opt_runScript.py && mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6rf2_ref_off.py" ;;
esac
case "$CMD" in
  *d6rf2_units_assert.py*) : ;;
  *) echo "ABORT the arm command for $ARM does not invoke the units gate; call site 2 is missing (rule 14)"; exit 7 ;;
esac

CMDFILE="$WORK/d6rf2_cmd.sh"
printf '%s\n' "$CMD" > "$CMDFILE" || { echo "ABORT cmd file"; exit 4; }
echo "D6RF2_CMDFILE arm=$ARM md5=$(md5sum "$CMDFILE" | cut -d' ' -f1) deadline_in_container_s=$TMO units_gate=present"
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k $KILL_GRACE_S $TMO bash /mnt/$(basename "$WORK")/d6rf2_cmd.sh" > /dev/null 2>&1 \
...
echo "D6RF2_RUNAWAY_GUARD arm=$ARM cap_core_min=$CAP ceiling_core_min=$CEILING mode=report_then_graceful_stop_at_ceiling"
...
while true; do
  ...
  if [ "$CAP_REPORTED" = "no" ] && [ "$(python3 -c "print(1 if $CM > $CAP else 0)")" = "1" ]; then
    CAP_REPORTED=yes
    echo "D6RF2_CAP_CROSSED arm=$ARM core_min=$CM cap=$CAP ceiling=$CEILING action=REPORTED_RUN_CONTINUES supervisor_decides" | tee -a "$BASE/ledger.txt"
  fi
  if [ "$(python3 -c "print(1 if $CM > $CEILING else 0)")" = "1" ]; then
    CEILING_HIT=yes
    echo "D6RF2_CEILING_HIT arm=$ARM core_min=$CM ceiling=$CEILING action=GRACEFUL_STOP" | tee -a "$BASE/ledger.txt"
...
echo "D6RF2_CONTAINER_CLOCK arm=$ARM started_at=$CSTART finished_at=$CFIN container_wall_s=$CWALL host_wall_s_bracket_T0_T1=$WALL"
...
if [ "$rc" = "77" ]; then
  echo "D6RF2_UNITS_REFUSAL_IN_CONTAINER arm=$ARM rc=77 -- call site 2 refused the design vector; THE MESH WAS NEVER TOUCHED.  See $(basename "$LOG") for the failing check." | tee -a "$BASE/ledger.txt"
fi
...
LEDGER="$BASE/ledger.txt"
{
  ...
  grep -a "D4S_CONTAINER_UID\|D4S_IDWARP_SO_MD5\|D4S_DEADLINE_IN_CONTAINER_S\|D6RF2_UNITS_PASS" "$LOG" | head -4
} | tee -a "$LEDGER"
