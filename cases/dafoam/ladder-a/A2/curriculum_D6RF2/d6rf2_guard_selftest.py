...
`d6rf2_run_arm.sh` contains `sudo -n rm -rf "$WORK"`.  A launcher with a
...
LAUNCHER = os.path.join(HERE, "d6rf2_run_arm.sh")
DRIVER = os.path.join(HERE, "d6rf2_chain_driver.sh")
...
REG_ROOT = "/home/ubuntu/certonomous-runs/CURRICULUM-D6RF2-a2-wing-multipoint-fd"
...
def main():
    print("D6RF2 LAUNCHER/DRIVER GUARD SELFTEST -- driven, zero containers\n")
    before = census()
    ...
    for arm in ("F_mp", "REF_off"):
        ...
        m = re.search(r"D6RF2_CAP_FRAME arm=%s registered_core_min=(\S+) ranks=(\d+) "
    ...
    guard_names = ["G-ROOT.2a", "G-ROOT.1", "G-ROOT.2", "G-ROOT.3", "G-ROOT.5",
                   "D6RF2_CAP_FRAME", "L-251 run root mode", "G-ROW"]
    guard_lines = {}
    for n, t in lines:
    ...
    rec("U24 call site 1: the launcher runs the gate HOST-side at staging",
        'python3 "$WORK/d6rf2_units_assert.py" "$WORK/$DVFILE"' in src)
    ...
    rec("U27 call site 2: the gate sits BETWEEN the wrapper and the mpirun in "
        "BOTH arm commands",
        all(re.search(r"endpoint_physical\.py[^&]*&&[^&]*d6rf2_units_assert\.py"
    ...
    rec("U29 the in-container refusal rc 77 is NAMED on the ledger, not left bare",
        "D6RF2_UNITS_REFUSAL_IN_CONTAINER" in src and "rc=77" in src)
    ...
    files = {"MD5_RUNSCRIPT6": "d6rf2_opt_runScript.py",
             "MD5_FD": "d6rf2_fd_endpoint.py",
             "MD5_EXTRACT6": "../curriculum_D6R/d6r_extract_endpoint.py",
             "MD5_REFOFF": "d6rf2_ref_off.py",
             ...
             "MD5_LOCUS6": "d6rf2_endpoint_locus.py",
             "MD5_PHYS6": "d6rf2_endpoint_physical.py",
             "MD5_UNITS": "d6rf2_units_assert.py"}
    bad = []
    ...
    rec("U44 cumulative spend is asserted BEFORE every arm",
        "D6RF2_SPEND_CENSUS" in dsrc and "ABORT ITEM CEILING" in dsrc)
    ...
    if rec("U45 the `spent_core_min` reader was extracted from the driver",
        ...
        import tempfile
        td = tempfile.mkdtemp(prefix="d6rf2_guard_")
    ...
    if rec("U55 `count_src_entries` was extracted from the launcher",
           fn is not None):
        import tempfile
        td = tempfile.mkdtemp(prefix="d6rf2_s4_")
    ...
    if rec("U68 `field_path` was extracted from the launcher", fn is not None):
        ...
        # ---- NEGATIVE LEGS ---------------------------------------------
        td = tempfile.mkdtemp(prefix="d6rf2_gz_")
    ...
    rec("U53 no container carrying this item's prefix exists",
        not [c for c in after if c.startswith("d6rf2_")])
    ...
    print("\nD6RF2_GUARD_SELFTEST %d/%d PASS%s"
