#!/usr/bin/env python
"""Curriculum D6RF -- the MULTIPOINT endpoint repair, as a WRAPPER that edits
...
import d6rf2_endpoint_locus as locus                                # noqa: E402

EXTRACTOR = "d6r_extract_endpoint.py"
RUNSCRIPT = "d6rf2_opt_runScript.py"
# PREREGISTRATION.md section 7.  These are the FREEZE, not a preference.
MD5_EXTRACT = "1743dd4232a7f06785f71be2f285f08d"
MD5_RUNSCRIPT = "137539e0a99be27f27fdb69e063b2a87"

SCALED_OUT = "d6r_endpoint_dvs.json"                 # the frozen extractor writes this
PRESERVED = "d6r_endpoint_dvs_DRIVERSCALED.json"     # the pre-repair pre-image
PHYSICAL = "d6rf_endpoint_dvs_PHYSICAL.json"         # the corrected artefact
HISTORY = "d6r_major_history.json"


def refuse(msg):
    sys.stderr.write("D6RF_ENDPOINT_PHYSICAL REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    for stale in (SCALED_OUT, PRESERVED, PHYSICAL, HISTORY):
        if os.path.exists(stale):
            os.remove(stale)
    spec = importlib.util.spec_from_file_location("d6r_extract_frozen",
                                                  os.path.abspath(EXTRACTOR))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["d6r_extract_frozen"] = mod
    ...
    sys.stdout.write("D6RF_ENDPOINT_PHYSICAL_WRITTEN %s scalers=%s "
