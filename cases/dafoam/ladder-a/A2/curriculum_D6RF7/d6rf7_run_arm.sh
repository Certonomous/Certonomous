#!/usr/bin/env bash
# Curriculum D6RF7 arm launcher -- ONE ARM, P_conv: the CONVERGENCE PROBE.
#
# DERIVED from curriculum_D6RF3/d6rf3_run_arm.sh (md5
# 58684b91f6aced35f500fc947517a8e4) with the REGISTERED DELTAS of
# D6RF7 PREREGISTRATION.md sections 4, 5 and 8.2, enumerated in
# d6rf7_run_arm_DELTAS_from_d6rf5.diff beside this file:
...
#   B  THE LIMITED fvSchemes AND d6rf7_fvSolution (corr3) ARE INSTALLED AT STAGING (S9), at all four
...
#      prints `D6RF7_FVSCHEMES_INSTALLED leg=<L> md5=<m> sites=<n> ... nNonOrthogonalCorrectors=<k>` into the
...
#      d6rf7_accept_floor_control.py reads both values back out of the log this
...
#      `permission=` field of every `D6RF7_STAGE` line the launcher writes.
#      G-FREEZE IS DRIVEN IN BOTH DIRECTIONS BY `d6rf7_launcher_guard_drive.py`,
...
ITEM=D6RF7
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6RF7-a2-wing-convergence-probe
...
PERMISSION=cd1ad589  # FROZEN 2026-09-07 by dafoam-supervisor (freeze sha cd1ad589, its tree carries PREREGISTRATION.md, rule 2). Supervisor check-1 CLEAN on the grade.py + cd_plant_control DELTAS (comprehensive fail-closed present-but-empty handling; detector identity asserted; no gate/threshold/scheme touched; freeze-selfcheck + selftest incl. direction-4 fail-closed both rc 0; pins ALL match). Fixture-validated off the D6RF6 run root -> G-CONV GATE FAIL inside overall NOT A RESULT. Successor to D6RF6 (graded BLOCKED: planted-CD control REFUSED on the present-but-empty product because the P_conv baseline primal RAISED BY DESIGN and the guard wrote points={}; grading never reached G-CONV). D6RF7's single class of DELTA is grading FAILURE-PATH handling so the whole path SURVIVES the primal-raised present-but-empty product and produces the graded G-CONV verdict. Pin fixpoint ALL_PINS_MATCH TRUE (fd 6f4c8bcd==MD5_FD, physical 625bacf5==MD5_PHYS6, extract baedb673==MD5_EXTRACT==MD5_EXTRACT6, units 34f477f9, anchor_gate c549ec2e; runscript/locus/fvSchemes/fvSol carried BYTE-IDENTICAL). FIXTURE-VALIDATED off the D6RF6 run root (no re-solve): grade rc 0, cd/fd controls NOT EXERCISED (no refuse), G-OFF/G-PRICE/G-FD NOT A RESULT under ARM_RAN_PRIMAL_RAISED_POINTS_EMPTY, G-CONV GATE FAIL on L1 (binding p_first_uncorrected 1.6256e-05 = 1.626x the 1.0e-05 floor; nuTilda 1.408e-05 also over floor), F5 AS PREDICTED (L3 GATE FAIL), overall NOT A RESULT per the registered ladder rung 6 (want-of-input on the unbought FD/CD/off/price gates -- rule 5, NOT widened). No gate/threshold/band/cap/scheme/mesh changed (N-D43, T25). est 62.0 / cap 186.0 core-min; a deterministic re-run of the same scheme realises ~12-16 core-min (D6RF6 measured 12.133). Supervisor freezes after non-delegable check-1 of the grade.py/control DELTAS.
...
D6RF7_PREREG=cases/dafoam/ladder-a/A2/curriculum_D6RF7/PREREGISTRATION.md
D6RF7_REPO=/home/ubuntu/Certonomous
if command -v git >/dev/null 2>&1 && [ -d "$D6RF7_REPO/.git" ]; then
  RESOLVED=$(git -C "$D6RF7_REPO" rev-parse --verify --quiet "${PERMISSION}^{commit}" 2>/dev/null || true)
  if [ -z "$RESOLVED" ]; then
    echo "ABORT G-FREEZE-SHA PERMISSION='$PERMISSION' is hex of a plausible"
    echo "  length but names NO COMMIT in $D6RF7_REPO.  A freeze field that"
    echo "  resolves to nothing is not a freeze.  REFUSED."
    exit 3
  fi
  if ! git -C "$D6RF7_REPO" cat-file -e "${RESOLVED}:${D6RF7_PREREG}" 2>/dev/null; then
  ...
  PREREG_AT_FREEZE=$(git -C "$D6RF7_REPO" rev-parse "${RESOLVED}:${D6RF7_PREREG}")
...
echo "D6RF7_G_FREEZE_PASS permission=$PERMISSION assignments=$PERM_ASSIGNMENTS shape=${#PERMISSION}hex resolved=$RESOLVED prereg_blob_at_freeze=$PREREG_AT_FREEZE checked=$FREEZE_CHECKED"
...
# into a NEUTERED COPY (d6rf7_launcher_guard_drive.py), so nothing needs to
...
echo "D6RF7_G_ROOT_PASS item=$ITEM base=$BASE_REAL ledger_clean=yes"
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d6rf7_" | tr '\n' ',' | sed 's/,$//'
}

# ---- D6RF7 REGISTERED CAP TABLE (PREREGISTRATION.md section 7/10.6) -------
#   P_conv  62.00 core-min estimate (full run; L4-L8 GATED on G-CONV PASS) =
#       L1  D6RF7 baseline primal (LIMITED 0.333 + 3 correctors)  5.4 x 1.4 = 7.6
...
MD5_FD=6f4c8bcd91010141813e6b5a6736fa9c
MD5_EXTRACT6=baedb673e9c88291f6724794681bc9a7
MD5_LOCUS6=341189ca866f302a7e1bba8eefad3a57
# 2026-09-06 PRE-FIRST-COMPUTE: was ca75db3e036b9f4e7cfca6462037c021.  Repinned
# because `d6rf7_endpoint_physical.py`'s C3 gained the rule-14 assertion that its
...
# because d6rf7_endpoint_physical.py:95 MD5_EXTRACT was corrected from the stale
# PARENT d6rf4 value (c5aace65) to the real d6rf7 extractor (95630a22); that edit
# shifts d6rf7_endpoint_physical.py's own md5.  Fixpoint pair with that fix.
MD5_PHYS6=625bacf5b1489989f9a2638dd99e2e52
MD5_UNITS=34f477f92b23e27896b458475eef0f78
MD5_ANCHOR_GATE=c549ec2ee18d7c2bf3ae4244bb12bc78
# THE D6RF7 DISCRETISATION FILES, AND THE WHOLE ITEM TURNS ON THEM.
# d6rf7_fvSchemes_LIMITED carries `Gauss linear limited corrected 0.333` /
# `limited corrected 0.333` (Fix #1); d6rf7_fvSchemes_D6RF4_ORIGINAL carries
# D6RF4's `Gauss linear corrected` / `corrected` (the F5 wrong scheme);
# d6rf7_fvSolution carries nNonOrthogonalCorrectors 3 (Fix #2) on the tightened
...
# GRADER as G-DVL's D4-side reference (d6rf7_grade.py CROSS_ITEM), so its md5
...
# D6RF3's value `e823689c...` while the block above carries D6RF7's.  IN SHELL
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: d6rf7_run_arm.sh <P_conv> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: d6rf7_run_arm.sh <P_conv> <image>"; exit 64; }
...
case "$ARM" in
  ...
  F_mp|REF_off)
     ...
     echo "  D6RF7 registers ONE arm, P_conv (PREREGISTRATION.md section 8.2):"
...
if [ "$AF_REAL" -ne 0 ]; then
  ...
  echo "  control.  D6RF7 tightens the linear-solver stopping rule at an"
...
echo "D6RF7_G_ACCEPT_FLOOR_PASS planted_control_seen=$AF_RAW real_assignments=$AF_REAL -- the launcher sets neither primalMinResTol nor primalMinResTolDiff; the accept floor stays 1e-08 x 1000 = 1.0e-05"

# ---- G-ROOT.5 -- A LIVE ARM IS NEVER RE-STAGED.  Two live readings, BEFORE
# ---- any destructive act.
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d6rf7_${ARM}_" 2>/dev/null | grep "^d6rf7_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "D6RF7_G_ROOT5_PASS arm=$ARM live_same_arm_containers=none driver_pidfile=$([ -f "$PIDFILE" ] && echo present_owner_is_ancestor_or_stale || echo absent)"
...
echo "D6RF7_CAP_FRAME arm=$ARM registered_core_min=$CAP ranks=$RANKS deadline_in_container_s=$TMO frame_allowance_s=$FRAME_ALLOWANCE_S kill_grace_s=$KILL_GRACE_S worst_case_host_core_min=$BACKCHECK enforced_in_frame=container graded_in_frame=host_bracket_T0_T1"
...
echo "D6RF7_HOST_PRE arm=$ARM MemAvailable_GiB=$MEMAVAIL_GIB load1=$LOAD cpuset=$CPUSET siblings_pre=[$SIBLINGS_PRE]"
...
echo "$MD5_RUNSCRIPT6  $BASE/d6rf7_opt_runScript.py" | md5sum -c - || { echo "ABORT d6rf7_opt_runScript.py md5"; exit 4; }
echo "$MD5_FD  $BASE/d6rf7_fd_endpoint.py"           | md5sum -c - || { echo "ABORT d6rf7_fd_endpoint.py md5"; exit 4; }
echo "$MD5_EXTRACT6  $BASE/d6rf7_extract_endpoint.py"| md5sum -c - || { echo "ABORT d6rf7_extract_endpoint.py md5"; exit 4; }
echo "$MD5_LOCUS6  $BASE/d6rf7_endpoint_locus.py"   | md5sum -c - || { echo "ABORT d6rf7_endpoint_locus.py md5"; exit 4; }
echo "$MD5_PHYS6  $BASE/d6rf7_endpoint_physical.py" | md5sum -c - || { echo "ABORT d6rf7_endpoint_physical.py md5"; exit 4; }
echo "$MD5_UNITS  $BASE/d6rf7_units_assert.py"      | md5sum -c - || { echo "ABORT d6rf7_units_assert.py md5"; exit 4; }
echo "$MD5_ANCHOR_GATE  $BASE/d6rf7_anchor_gate.py" | md5sum -c - || { echo "ABORT d6rf7_anchor_gate.py md5"; exit 4; }
# THE D6RF7 DISCRETISATION FILES, ASSERTED AT $BASE BEFORE ANY IS INSTALLED.
echo "$MD5_FVSCHEMES_LIMITED  $BASE/d6rf7_fvSchemes_LIMITED" | md5sum -c - || { echo "ABORT d6rf7_fvSchemes_LIMITED md5 -- THE LIMITED NON-ORTH CORRECTION IS THE WHOLE ITEM"; exit 4; }
echo "$MD5_FVSCHEMES_ORIG  $BASE/d6rf7_fvSchemes_D6RF4_ORIGINAL" | md5sum -c - || { echo "ABORT d6rf7_fvSchemes_D6RF4_ORIGINAL md5 -- falsifier F5 must run D6RF4's ACTUAL scheme"; exit 4; }
echo "$MD5_FVSOL  $BASE/d6rf7_fvSolution" | md5sum -c - || { echo "ABORT d6rf7_fvSolution md5 -- nNonOrthogonalCorrectors 3 (Fix #2) is the paired remedy"; exit 4; }
...
test "$ROW" = "PATCHED" || {
  ...
  exit 4; }
echo "D6RF7_G_ROW_PASS row=$ROW digest=$GOT_DIGEST"

STAMP=$(date -u +%Y%m%dT%H%M%SZ)_$$
NAME="d6rf7_${ARM}_${STAMP}"
WORK="$BASE/$ARM"
...
: > "$STAGE_EVID"
stage_say "D6RF7_STAGE arm=$ARM utc=$(date -u +%Y%m%dT%H%M%SZ) base=$BASE_REAL permission=$PERMISSION"
...
if [ "$ARM" = "P_conv" ]; then
  ...
  stage_say "D6RF7_STAGE_P_conv (S1) OK src=$SRC exists, OptView.hst present, dst=$WORK does not exist"
  # ------------------------------------------------------------------ S3
  ...
  stage_say "D6RF7_STAGE_P_conv (S3) OK undeformed reference mesh md5 $MD5_REF_MESH on base and on mp04 mp05 mp06 -- double-deformation confound eliminated by MEASUREMENT"
  # ------------------------------------------------------------------ S4
  # ===================================================================
  ...
  # d6rf7_endpoint_locus.py -- "a loop that can iterate zero times asserts
  ...
  stage_say "D6RF7_STAGE_P_conv (S4) source pre-copy: OptView.hst md5=$SRC_HST_MD5, entries under mp04/processor0 = $SRC_TIMEDIRS"
  ...
  COPY_EPOCH=$(date -u +%s); echo "$COPY_EPOCH" > "$WORK/.d6rf7_copy_epoch"
  ...
  stage_say "D6RF7_STAGE_P_conv (S4) OK cp -a (mtimes PRESERVED), copy_epoch=$COPY_EPOCH, SOURCE INTACT after the copy -- and the comparison COUNTED $SRC_TIMEDIRS entries on both sides, so it cannot have passed on a pair of false zeros"
  # ------------------------------------------------------------------ S5
  ...
  rm -f  "$WORK/mphys.html" "$WORK/opt_IPOPT.txt" "$WORK/d6rf7_cmd.sh" 2>/dev/null
  ...
  stage_say "D6RF7_STAGE_P_conv (S5) OK dropped $DROPPED of $BEFORE_DROP output time directories from the COPY, $REMAIN remain (required 0); 0/, 0.orig/ and processor*/0/ KEPT and asserted; OptView.hst and dRdWColoring_4.bin KEPT; SOURCE INTACT ($SRC_REMAIN pseudo-time dirs still under $SRC/mp04/processor0)"
  # ------------------------------------------------------------------ S6
  cp -a "$BASE/d6rf7_extract_endpoint.py" "$BASE/d6rf7_fd_endpoint.py" \
        "$BASE/d6rf7_opt_runScript.py" "$BASE/d6rf7_endpoint_locus.py" \
        "$BASE/d6rf7_endpoint_physical.py" "$BASE/d6rf7_units_assert.py" \
        "$BASE/d6rf7_anchor_gate.py" \
        "$BASE/d6rf7_fvSchemes_LIMITED" "$BASE/d6rf7_fvSchemes_D6RF4_ORIGINAL" \
        "$BASE/d6rf7_fvSolution" \
        "$WORK/" || { stage_say "ABORT S6 stage instruments"; exit 4; }
  { echo "$MD5_EXTRACT6  $WORK/d6rf7_extract_endpoint.py"
    echo "$MD5_FD  $WORK/d6rf7_fd_endpoint.py"
    echo "$MD5_RUNSCRIPT6  $WORK/d6rf7_opt_runScript.py"
    echo "$MD5_LOCUS6  $WORK/d6rf7_endpoint_locus.py"
    echo "$MD5_PHYS6  $WORK/d6rf7_endpoint_physical.py"
    echo "$MD5_UNITS  $WORK/d6rf7_units_assert.py"
    echo "$MD5_ANCHOR_GATE  $WORK/d6rf7_anchor_gate.py"
    echo "$MD5_FVSCHEMES_LIMITED  $WORK/d6rf7_fvSchemes_LIMITED"
    echo "$MD5_FVSCHEMES_ORIG  $WORK/d6rf7_fvSchemes_D6RF4_ORIGINAL"
    echo "$MD5_FVSOL  $WORK/d6rf7_fvSolution"; } | md5sum -c - || { stage_say "ABORT S6 staged instrument md5 in the arm directory"; exit 4; }
  # `d6rf7_ref_off.py` IS NOT STAGED: `REF_off` is not a registered arm here,
  ...
  stage_say "D6RF7_STAGE_P_conv (S6) OK SEVEN instruments, BOTH fvSchemes files and d6rf7_fvSolution staged, every md5 asserted on BOTH sides of the copy; d6rf7_ref_off.py DELIBERATELY NOT STAGED (REF_off is not registered at this freeze)"
  # ------------------------------------------------------------------ S7
  UNITS_RUNSCRIPT=d6rf7_opt_runScript.py
  DVFILE=d6rf7_endpoint_dvs.json
  for p in d6rf7_endpoint_dvs_PHYSICAL.json d6rf7_endpoint_dvs_DRIVERSCALED.json \
           d6rf7_endpoint_dvs.json d6rf7_major_history.json d6rf7_fd_endpoint.json \
           d6rf7_fd_endpoint.jsonl d6rf7_f5_endpoint.json d6rf7_f5_endpoint.jsonl \
           d6rf7_fd_endpoint.json.partial d6rf7_f5_endpoint.json.partial; do
    rm -f "$WORK/$p"
  done
  stage_say "D6RF7_STAGE_P_conv (S7) OK this arm's registered products swept from the copy, INCLUDING the \`.partial\` temporaries the DEF-7 incremental writer renames from -- a stale \`.partial\` is a half-written product wearing a product's name"

  # ------------------------------------------------------------------ S9
  # ============ THE REGISTERED DISCRETISATION IS INSTALLED HERE ==========
  # THE STAGING DEFAULT for the P_conv L1/L2 legs: d6rf7_fvSchemes_LIMITED over
  # the base-case fvSchemes (Fix #1) and d6rf7_fvSolution (nNonOrthogonalCorrectors
  ...
  for sd in $FVSCHEMES_SITES; do
    ...
    test "$PRE" = "$MD5_FVSCHEMES_BASE" || {
      ...
      exit 5; }
    cp -a "$WORK/d6rf7_fvSchemes_LIMITED" "$tgt" || { stage_say "ABORT S9 fvSchemes install failed at $tgt"; exit 4; }
    ...
    stage_say "D6RF7_STAGE_P_conv (S9 fvSchemes) site $sd  pre=$PRE -> post=$POST"
  done
  ...
  # d6rf7_fvSolution (correctors 3) at every fvSolution site
  ...
  for sd in $FVSOL_SITES; do
    ...
    cp -a "$WORK/d6rf7_fvSolution" "$tgt" || { stage_say "ABORT S9 fvSolution install failed at $tgt"; exit 4; }
    POST=$(md5sum "$tgt" | cut -d' ' -f1)
    test "$POST" = "$MD5_FVSOL" || { stage_say "ABORT S9 fvSolution POST-IMAGE at $tgt is $POST, not $MD5_FVSOL"; exit 4; }
    FVSOL_DONE=$((FVSOL_DONE+1))
    stage_say "D6RF7_STAGE_P_conv (S9 fvSolution corr3) site $sd  post=$POST"
  done
  ...
  stage_say "D6RF7_STAGE_P_conv (S9) OK LIMITED fvSchemes installed at $FVSCHEMES_DONE of $FVSCHEMES_EXPECT ENUMERATED sites, pre-image asserted = $MD5_FVSCHEMES_BASE at every one; d6rf7_fvSolution (corr3) at $FVSOL_DONE sites; READ-BACK over the whole tree found $FOUND_LIMITED limited and 0 base"
else
  # ---------------------------------------------------------------------
...
#
# THE DEFECT THIS EXISTS FOR, 2026-09-03T22:13:11Z.  D6RF7 launched and aborted
# 70 s later at rc 4 on `md5sum: .../F_mp/d6rf7_anchor_gate.py: No such file or
...
# levels down: d6rf7_ref_off.py is named ONLY by d6rf7_anchor_gate.py's own
...
# 22:55:46Z state it does: d6rf7_anchor_gate.py -> d6rf7_ref_off.py, absent.
dangling = []
...
# $BASE is the md5-verified source: d6rf7_chain_driver.sh asserts all twelve
...
case "$DELIVERY" in
  MISSING*)
    ...
    stage_say "D6RF7_G_DELIVERY closure requires files the arm list did not stage: $NEED"
    for f in $NEED; do
      ...
      stage_say "D6RF7_G_DELIVERY staged $f from \$BASE, asserted byte-identical"
    done
    # THE READ-BACK: the same derivation, again, on the new disk state.
    DELIVERY=$(derive_delivery) ;;
esac
case "$DELIVERY" in
  OK\ *) stage_say "D6RF7_G_DELIVERY $DELIVERY" ;;
  MISSING*)
...
if [ -e "$WORK/$DVFILE" ]; then
  stage_say "D6RF7_UNITS_CALLSITE_1 arm=$ARM an endpoint artefact SURVIVED the sweep: $WORK/$DVFILE -- grading it before anything else runs"
  python3 "$WORK/d6rf7_units_assert.py" "$WORK/$DVFILE" --runscript "$WORK/$UNITS_RUNSCRIPT"
  UR=$?
  if [ "$UR" -ne 0 ]; then
  ...
  stage_say "D6RF7_UNITS_CALLSITE_1 arm=$ARM no endpoint artefact survived the sweep (expected on a first fire); call site 2 grades the one the wrapper writes"
fi
...
# ===========================================================================
echo "$MD5_ANCHOR_GATE  $WORK/d6rf7_anchor_gate.py" | md5sum -c - || { stage_say "ABORT G-ANCHOR gate md5"; exit 4; }
python3 "$WORK/d6rf7_anchor_gate.py" --work-dir "$WORK" || {
  ...
  exit 3; }
stage_say "D6RF7_G_ANCHOR OK arm=$ARM -- every anchor-scoped reader's sentinel is unique in the producer it will read, and the header it yields carries every symbol it execs"
...
# (`d6rf7_grade.py:1161`, `d6rf7_endpoint_physical.py:152`), so the guard was
...
# `d6rf7_age_datum_control.py`, which also drives this line in both directions.
...
stage_say "D6RF7_G_COLD OK arm=$ARM age_datum_epoch=$AGE_DATUM (every registered product must be strictly newer)"
...
# `d6rf7_fd_endpoint.py --mode F5_scheme` catches DAFoam's post-`End` refusal
...
install_fvschemes() {       # $1 = fvSchemes source in $PWD, $2 = expected md5, $3 = leg, $4 = nNonOrthogonalCorrectors
  ...
  for sd in system mp04/system mp05/system mp06/system; do
    [ -f "$sd/fvSchemes" ] || { echo "D6RF7_LEG_ABORT leg=$3 no fvSchemes at $sd"; return 5; }
    [ -f "$sd/fvSolution" ] || { echo "D6RF7_LEG_ABORT leg=$3 no fvSolution at $sd"; return 5; }
    cp -a "$1" "$sd/fvSchemes" || { echo "D6RF7_LEG_ABORT leg=$3 fvSchemes install failed at $sd"; return 5; }
    got=$(md5sum "$sd/fvSchemes" | cut -d' ' -f1)
    [ "$got" = "$2" ] || { echo "D6RF7_LEG_ABORT leg=$3 fvSchemes post-image $got != $2 at $sd"; return 5; }
    ...
    awk -v n="$4" '''BEGIN{b=0} /^SIMPLE/{b=1} b&&/nNonOrthogonalCorrectors/{sub(/[0-9]+;/, n";")} /^}/{if(b)b=0} {print}''' "$sd/fvSolution" > "$sd/fvSolution.tmp" && mv "$sd/fvSolution.tmp" "$sd/fvSolution" || { echo "D6RF7_LEG_ABORT leg=$3 corrector set failed at $sd"; return 5; }
    grep -qE "^SIMPLE" "$sd/fvSolution" && grep -qE "nNonOrthogonalCorrectors[[:space:]]+$4;" "$sd/fvSolution" || { echo "D6RF7_LEG_ABORT leg=$3 nNonOrthogonalCorrectors $4 not set at $sd"; return 5; }
    n=$((n+1))
  done
  [ "$n" -ge 1 ] || { echo "D6RF7_LEG_ABORT leg=$3 installed ZERO sites -- a swap that swapped nothing"; return 5; }
  echo "D6RF7_FVSCHEMES_INSTALLED leg=$3 md5=$2 sites=$n file=$1 nNonOrthogonalCorrectors=$4"
  return 0
}
python d6rf7_endpoint_physical.py --age-datum __AGE_DATUM__ && \
python d6rf7_units_assert.py d6rf7_endpoint_dvs.json --runscript d6rf7_opt_runScript.py && \
install_fvschemes d6rf7_fvSchemes_LIMITED __MD5_LIMITED__ L1_L2 3 && \
mpirun --allow-run-as-root -np __RANKS__ --bind-to core --report-bindings -x PYTHONPATH \
  python d6rf7_fd_endpoint.py --mode P_conv && \
install_fvschemes d6rf7_fvSchemes_D6RF4_ORIGINAL __MD5_ORIG__ L3 1 && \
mpirun --allow-run-as-root -np __RANKS__ --bind-to core --report-bindings -x PYTHONPATH \
  python d6rf7_fd_endpoint.py --mode F5_scheme
LEGEOF
case "$ARM" in
  P_conv)
...
for need in "--mode P_conv" "--mode F5_scheme" \
            "install_fvschemes d6rf7_fvSchemes_LIMITED" \
            "install_fvschemes d6rf7_fvSchemes_D6RF4_ORIGINAL"; do
  case "$CMD" in
    *"$need"*) : ;;
...
echo "D6RF7_CMD_LEGS_PRESENT arm=$ARM legs=L1,L2(--mode P_conv,LIMITED,corr3) L3(--mode F5_scheme,D6RF4_ORIGINAL,corr1) fvSchemes_swaps=2"

CMDFILE="$WORK/d6rf7_cmd.sh"
printf '%s\n' "$CMD" > "$CMDFILE" || { echo "ABORT cmd file"; exit 4; }
echo "D6RF7_CMDFILE arm=$ARM md5=$(md5sum "$CMDFILE" | cut -d' ' -f1) deadline_in_container_s=$TMO units_gate=present"
...
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k $KILL_GRACE_S $TMO bash /mnt/$(basename "$WORK")/d6rf7_cmd.sh" > /dev/null 2>&1 \
...
echo "D6RF7_RUNAWAY_GUARD arm=$ARM cap_core_min=$CAP ceiling_core_min=$CEILING mode=report_then_graceful_stop_at_ceiling"
...
while true; do
  ...
  if [ "$CAP_REPORTED" = "no" ] && [ "$(python3 -c "print(1 if $CM > $CAP else 0)")" = "1" ]; then
    CAP_REPORTED=yes
    echo "D6RF7_CAP_CROSSED arm=$ARM core_min=$CM cap=$CAP ceiling=$CEILING action=REPORTED_RUN_CONTINUES supervisor_decides" | tee -a "$BASE/ledger.txt"
  fi
  if [ "$(python3 -c "print(1 if $CM > $CEILING else 0)")" = "1" ]; then
    CEILING_HIT=yes
    echo "D6RF7_CEILING_HIT arm=$ARM core_min=$CM ceiling=$CEILING action=GRACEFUL_STOP" | tee -a "$BASE/ledger.txt"
...
echo "D6RF7_CONTAINER_CLOCK arm=$ARM started_at=$CSTART finished_at=$CFIN container_wall_s=$CWALL host_wall_s_bracket_T0_T1=$WALL"
...
if [ "$rc" = "77" ]; then
  echo "D6RF7_UNITS_REFUSAL_IN_CONTAINER arm=$ARM rc=77 -- call site 2 refused the design vector; THE MESH WAS NEVER TOUCHED.  See $(basename "$LOG") for the failing check." | tee -a "$BASE/ledger.txt"
fi
...
LEDGER="$BASE/ledger.txt"
{
  ...
  grep -a "D4S_CONTAINER_UID\|D4S_IDWARP_SO_MD5\|D4S_DEADLINE_IN_CONTAINER_S\|D6RF7_UNITS_PASS" "$LOG" | head -4
} | tee -a "$LEDGER"
