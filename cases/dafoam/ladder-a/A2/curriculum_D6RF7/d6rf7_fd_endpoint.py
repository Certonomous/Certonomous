#!/usr/bin/env python
"""Curriculum D6RF7 -- the CONVERGENCE PROBE `P_conv`, and (in `full` mode)
...
GUARD (`D6RF7 PREREGISTRATION.md` section 2), enumerated in
`d6rf7_fd_endpoint_DELTAS_from_d6rf5.diff` beside this file.  D6RF5's P_conv arm
...
force, are:
  ...
  R3  `F5_scheme` REPLACES `F5_loose`.  D6RF7's falsifier F5 re-runs the baseline
      ...
      LAUNCHER via d6rf7_fvSchemes_D6RF4_ORIGINAL, not by this script), everything
  ...
  N4  LEG MARKERS, CARRIED.  Each leg prints `D6RF7_LEG_BEGIN <leg>` /
      `D6RF7_LEG_END <leg> ...`, which is how the GRADER splits one container log
  ...
  N5  THE PER-FIELD G-CONV FIRST-P-SOLVE EXTRACTION LIVES IN THE GRADER, NOT
      ...
      `d6rf7_grade.py:read_legs` records the FIRST p line of the final Time block
...
PRODUCER = "d6rf7_opt_runScript.py"
...
OUT = "d6rf7_fd_endpoint.json"
JSONL = "d6rf7_fd_endpoint.jsonl"
...
MODE_OUT = {"P_conv": OUT, "F5_scheme": "d6rf7_f5_endpoint.json", "full": OUT}
MODE_JSONL = {"P_conv": JSONL, "F5_scheme": "d6rf7_f5_endpoint.jsonl",
              "full": JSONL}
...
def _require_bar_before_fd(mode):
    ...
    if BAR_LEG not in _LEGS_RUN:
        sys.stderr.write(
            "D6RF7_FD REFUSE mode=%s reached an FD leg with legs_run=%r -- "
...
def main(mode):
    if mode not in MODES:
        sys.stderr.write("D6RF7_FD REFUSE unknown mode %r; registered: %r\n"
                         % (mode, MODES))
        sys.exit(2)
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("D6RF7_FD REFUSE producer md5 %s != frozen %s\n" % (got, PRODUCER_MD5))
        sys.exit(2)
    with open(PRODUCER) as fh:
        src = fh.read()
    if src.count(ANCHOR) != 1:
        sys.stderr.write("D6RF7_FD REFUSE anchor %r appears %d times\n" % (ANCHOR, src.count(ANCHOR)))
        sys.exit(2)
    header = src.split(ANCHOR)[0]
    saved_argv = list(sys.argv)
    sys.argv = [PRODUCER, "-task", "run_model", "-optimizer", "IPOPT"]
    ns = {"__name__": "d6rf7_frozen_header", "__file__": PRODUCER}
    ...
    with open("d6rf7_endpoint_dvs.json") as fh:
        dvs = json.load(fh)
    for key in DV_KEYS:
        if key not in dvs:
            sys.stderr.write("D6RF7_FD REFUSE endpoint dv file missing %s\n" % key)
            sys.exit(2)
    ...
    _state = {
        ...
        "fvSolution_statement": (
            "THE ONLY THING D6RF7 CHANGES ABOUT THE PRIMAL IS THE LINEAR-"
            ...
            "in every leg of this item. d6rf7_accept_floor_control.py reads "
    ...
    def _write_out():
        ...
        leg_say("D6RF7_FD_ENDPOINT_WRITTEN", out_path,
                "legs_run=%s" % ",".join(_LEGS_RUN),
                "n_rows=%d" % len(_state.get("rows", [])))

    def primal(tag):
        t0 = time.time()
        leg_say("D6RF7_LEG_BEGIN", tag, "mode=%s" % mode)
        prob.run_model()
        ...
        leg_say("D6RF7_LEG_END", tag, "mode=%s" % mode, "J=%r" % j,
    ...
    # launcher stages d6rf7_fvSchemes_D6RF4_ORIGINAL + 1 corrector before it)
    ...
    if mode == "F5_scheme":
        try:
            ...
            emit({"kind": "primal_raised", "tag": "baseline", "mode": mode,
                  "error": repr(exc)[:800]})
            leg_say("D6RF7_LEG_END", "baseline", "mode=%s" % mode,
                    "primal_raised=True")
        _write_out()
        leg_say("D6RF7_MODE_COMPLETE", mode,
                "legs_run=%s" % ",".join(_LEGS_RUN))
        MPI.COMM_WORLD.Barrier()
        return

    # ===== D6RF7 DELTA vs D6RF5 (Supersedes D6RF5, BLOCKED) ==============
    ...
    # d6rf7_fd_endpoint.json was written, so the grader refused at G1 (product
    ...
    if mode == "P_conv":
        # THE REGISTERED ARM STOPS HERE.  D6RF7 buys a CONVERGENCE measurement,
    ...
    if rank == 0:
        ...
        with open(tmp, "w") as fh:
            ...
            os.fsync(fh.fileno())
        os.replace(tmp, out_path)
        leg_say("D6RF7_FD_ENDPOINT_WRITTEN", out_path, "n_rows=%d" % len(rows))
    leg_say("D6RF7_MODE_COMPLETE", mode, "legs_run=%s" % ",".join(_LEGS_RUN))
    MPI.COMM_WORLD.Barrier()
