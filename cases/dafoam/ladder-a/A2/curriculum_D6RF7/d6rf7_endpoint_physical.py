#!/usr/bin/env python
"""Curriculum D6RF7 -- the MULTIPOINT endpoint repair, as a WRAPPER that edits
...
written); the deltas are in `d6rf7_endpoint_physical_DELTAS_from_d6rf5.diff`
...
are recorded here so a reader does not have to reconstruct why they moved:
  ...
  D1  `EXTRACTOR` / `MD5_EXTRACT` are RE-PINNED to `d6rf7_extract_endpoint.py`.
  ...
  D2  Every product name is this item's own (`d6rf7_*`).
  D3  ⚠ `PHYSICAL` IS `d6rf7_endpoint_dvs_PHYSICAL.json`, WHICH REPAIRS
      ...
      `d6rf7_grade.py` additionally carries an executable `PRODUCT_WRITER` check
...
import d6rf7_endpoint_locus as locus                                # noqa: E402

EXTRACTOR = "d6rf7_extract_endpoint.py"
RUNSCRIPT = "d6rf7_opt_runScript.py"
# PREREGISTRATION.md section 7.  These are the FREEZE, not a preference.
MD5_EXTRACT = "baedb673e9c88291f6724794681bc9a7"  # 2026-09-07 D6RF7 derivation: re-pinned to d6rf7_extract_endpoint.py (rename d6rf6->d6rf7); parent D6RF6 value was c93adc8b (rule-2 pre-compute, single-variable successor changes ONLY the grading failure-path handling, no scheme/mesh/gate change)
MD5_RUNSCRIPT = "137539e0a99be27f27fdb69e063b2a87"

SCALED_OUT = "d6rf7_endpoint_dvs.json"                 # the frozen extractor writes this
PRESERVED = "d6rf7_endpoint_dvs_DRIVERSCALED.json"     # the pre-repair pre-image
PHYSICAL = "d6rf7_endpoint_dvs_PHYSICAL.json"         # the corrected artefact
HISTORY = "d6rf7_major_history.json"


def refuse(msg):
    sys.stderr.write("D6RF7_ENDPOINT_PHYSICAL REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    # `d6rf7_grade.py`'s age guard).  The launcher produces this value with
    ...
    for stale in (SCALED_OUT, PRESERVED, PHYSICAL, HISTORY):
        if os.path.exists(stale):
            os.remove(stale)
    spec = importlib.util.spec_from_file_location("d6rf7_extract_frozen",
                                                  os.path.abspath(EXTRACTOR))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["d6rf7_extract_frozen"] = mod
    ...
    sys.stdout.write("D6RF7_ENDPOINT_PHYSICAL_WRITTEN %s scalers=%s "
