#!/usr/bin/env python3
"""Curriculum D6RF5 -- `PREREGISTRATION.md` section 3f's TWO-DIRECTION MUTATION
...
rather than left as a preference: `d6rf5_grade.py` IS the grading path frozen by
...
import d6rf5_grade as G                                          # noqa: E402
...
# registered arm of D6RF5 (PREREGISTRATION.md section 8.2) and on a real
...
# rather than letting a reader infer that D6RF5 buys a REF_off arm.
...
def _fd_doc():
    """The shape `d6rf5_fd_endpoint.py` writes, with `points` (section 2a's
...
def _ledger(path, overrides=None):
    ...
    ov = overrides or {}
    with open(path, "w") as fh:
        fh.write("ITEM=D6RF5 staged=fixture\n")
...
def build(root, ledger_overrides=None):
    ...
    _w(os.path.join(f, "d6rf5_fd_endpoint.json"), _fd_doc())
    _w(os.path.join(f, "d6rf5_major_history.json"), _hist_doc())
    _w(os.path.join(f, "d6rf5_endpoint_dvs_PHYSICAL.json"), _dv_doc())
    _w(os.path.join(f, "d6rf5_endpoint_dvs_DRIVERSCALED.json"), _dv_doc())
    _w(os.path.join(f, "d6rf5_endpoint_dvs.json"), _dv_doc())
    _w(os.path.join(f, "d6rf5_f5_endpoint.json"), _fd_doc())
    r = os.path.join(root, "REF_off")
    _w(os.path.join(r, "d6rf5_ref_off.json"), _ref_doc())
...
def _cases(root, d4):
    ...
    f = os.path.join(root, "P_conv")
    r = os.path.join(root, "REF_off")
    fd = os.path.join(f, "d6rf5_fd_endpoint.json")
    ref = os.path.join(r, "d6rf5_ref_off.json")
    hist = os.path.join(f, "d6rf5_major_history.json")
    dv = os.path.join(f, "d6rf5_endpoint_dvs_PHYSICAL.json")
    rs = os.path.join(HERE, "d6rf5_opt_runScript.py")
...
LEG_LOG_CLEAN = """\
D6RF5_FVSCHEMES_INSTALLED leg=L1_L2 md5={fvmd5} sites=1 file=d6rf5_fvSchemes_LIMITED
D6RF5_LEG_BEGIN baseline mode=P_conv
...
End
D6RF5_LEG_END baseline mode=P_conv J=0.0222 wall_s=118.300 primal_raised=False
...
def drive():
    tmp = tempfile.mkdtemp(prefix="d6rf5_finmut_")
    root = os.path.join(tmp, "run")
    d4 = build(root)
    G.D4_OPT_IPOPT = d4
    print("D6RF5 SECTION 3f FINITENESS MUTATION HARNESS -- BOTH DIRECTIONS")
