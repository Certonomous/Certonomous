#!/usr/bin/env python3
"""Curriculum D6RF5 -- `G-ANCHOR`, THE REGISTERED PRE-COMPUTE GATE.
...
# ONE READER AT THIS FREEZE.  `d6rf5_ref_off.py` is DROPPED because `REF_off`
# is not a registered arm here (PREREGISTRATION.md section 8.2), and a gate
# that checks a file no arm runs is a gate reporting on nothing.  The count
# assertion below is what makes the drop visible rather than silent.
READERS = ("d6rf5_fd_endpoint.py",)
STAGED_PRODUCER = "d6rf5_opt_runScript.py"


def refuse(check, detail):
    sys.stderr.write("D6RF5_ANCHOR REFUSE %s %s\n" % (check, detail))
...
def gate(work_dir):
    print("D6RF5 G-ANCHOR -- %d anchor-scoped readers, on the bytes they will "
          "actually read" % len(READERS))
    res = [check_pair(os.path.join(work_dir, r), work_dir) for r in READERS]
    print("D6RF5_ANCHOR_PASS readers=%d producer=%s" % (len(res), STAGED_PRODUCER))
    return res
...
def selftest():
    ...
    print("D6RF5 G-ANCHOR SELFTEST -- both directions, on REAL bytes\n")
    ...
    td = tempfile.mkdtemp(prefix="d6rf5_anchor_")
    ...
    td2 = tempfile.mkdtemp(prefix="d6rf5_anchor2_")
    ...
    r2 = re.sub(r'PRODUCER_MD5 = "[0-9a-f]{32}"',
                ...
                open(os.path.join(HERE, "d6rf5_fd_endpoint.py")).read())
    open(os.path.join(td2, "d6rf5_fd_endpoint.py"), "w").write(r2)
    check("count == 1 but the anchor is at the TOP -> REFUSES at A4",
          lambda: check_pair(os.path.join(td2, "d6rf5_fd_endpoint.py"), td2), True)

    print("\nD. A2 -- the pin, which is the clause the ruling did not anticipate")
    td3 = tempfile.mkdtemp(prefix="d6rf5_anchor3_")
    shutil.copy2(os.path.join(HERE, STAGED_PRODUCER), os.path.join(td3, STAGED_PRODUCER))
    shutil.copy2(os.path.join(HERE, "d6rf5_fd_endpoint.py"),
                 os.path.join(td3, "d6rf5_fd_endpoint.py"))
    ...
    check("the producer drifts by one comment -> REFUSES at A2 (the md5), "
          ...
          lambda: check_pair(os.path.join(td3, "d6rf5_fd_endpoint.py"), td3), True)
    td4 = tempfile.mkdtemp(prefix="d6rf5_anchor4_")
    shutil.copy2(os.path.join(HERE, "d6rf5_fd_endpoint.py"),
                 os.path.join(td4, "d6rf5_fd_endpoint.py"))
    check("the producer is ABSENT from the work dir -> REFUSES at A2",
          lambda: check_pair(os.path.join(td4, "d6rf5_fd_endpoint.py"), td4), True)

    print("\nD6RF5_ANCHOR_SELFTEST %d/%d PASS" % (len(ok), len(ok) + len(fail)))
    if fail:
