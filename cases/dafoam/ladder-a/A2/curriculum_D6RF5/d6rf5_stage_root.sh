#!/usr/bin/env bash
# ===========================================================================
# D6RF5 RUN-ROOT STAGER.  IT LAUNCHES NOTHING.
#
# WHY THIS FILE EXISTS, STATED FIRST BECAUSE IT IS THE WHOLE POINT.
#
# D6RF5's P_conv arm was placed at 16:32:07Z and ABORTED `launcher_rc=4` at
# 16:33:09Z having burned ZERO SOLVER CORE-MINUTES, at d6rf5_run_arm.sh:515:
#
#     test "$(stat -c '%a' "$BASE")" = "777" || { echo "ABORT L-251 ..."; exit 4; }
#
# THE RUN ROOT NEVER EXISTED.  `d6rf5_run_arm.sh` contains ZERO `mkdir`: the
...
# STAGING on the first fire only".  D6RF5 registered ONE arm, dropped the chain
...
# discharged for D6RF5 in PREREGISTRATION.md's 2026-09-06 ROOT-STAGING
...
# NO instrument name and NO md5 of its own.  It PARSES `d6rf5_run_arm.sh` for
...
# asserted assigned EXACTLY ONCE in the launcher: `d6rf5_run_arm.sh:92-96`
...
# `d6rf5_run_arm.sh` IS NOT EDITED and its md5 is not re-pinned.  This file is
...
LAUNCHER="$HERE/d6rf5_run_arm.sh"
...
# `d6rf5_grade.py:184` registers `CAPS = {"P_conv": 54.00}` and `:696` computes
...
STAGE_EVID="${STAGE_EVID:-$HERE/D6RF5_ROOT_STAGING.txt}"
say() { echo "$*"; echo "$*" >> "$STAGE_EVID"; }

: > "$STAGE_EVID"
say "D6RF5_STAGE_ROOT begin utc=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ item=$ITEM arm=$ARM base=$BASE launches=NOTHING"
...
say "D6RF5_STAGE_ROOT launcher=$LAUNCHER md5=$LAUNCHER_MD5 (READ, not pinned here -- this file never re-pins the launcher)"
...
# d6rf5_run_arm.sh:92-96: MD5_ANCHOR_GATE was assigned TWICE in an early draft
...
say "D6RF5_STAGE_ROOT DERIVED $N_INSTR \$BASE-side assertion(s) from the launcher's own bytes, at lines [$ASSERT_LINES]; MD5_REF_MESH=$REFMESH_MD5"
...
for t in $TRIPLES; do
  ...
  say "D6RF5_STAGE_ROOT SOURCE OK $F md5=$M ($V)"
done
...
say "D6RF5_STAGE_ROOT SOURCE OK base=$D4_BASE_SRC mesh=$(basename "$SRC_MESH") md5=$SRC_MESH_MD5 FFD/wingFFD.xyz present"
...
if PS_OUT=$(sudo -n docker ps --format '{{.Names}}' 2>/dev/null); then
  ...
  LIVE=$(printf '%s\n' "$PS_OUT" | grep "^d6rf5_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
else
  DOCKER_READ=UNMEASURED
  LIVE=""
fi
say "D6RF5_STAGE_ROOT LIVE_READ docker=$DOCKER_READ same_arm_containers=[${LIVE:-none}]"
if [ -n "$LIVE" ]; then
...
# ---- FOREIGN ROOT, REFUSED BEFORE ANY WRITE AND BEFORE ANY D6RF5 ASSERTION.
...
# a D6RF5 md5 failure -- a true refusal with a false reason, which sends its
# reader hunting the wrong defect.  The check is hoisted here, ahead of every
# write and every D6RF5-shaped assertion.  The later ledger block is NOT
...
if [ ! -d "$BASE" ]; then
  ...
  # G-ROOT.3 accepts ONLY an exact `ITEM=D6RF5` line; staging metadata goes on a
  ...
  echo "STAGED stamp=$(date -u +%Y%m%dT%H%M%SZ) base_src=$D4_BASE_SRC launcher_md5=$LAUNCHER_MD5 stager=d6rf5_stage_root.sh" >> "$BASE/ledger.txt"
  STAGED_NOW=yes
  say "D6RF5_ROOT_STAGED base=$BASE stamp=$(date -u +%Y%m%dT%H%M%SZ) mode=$(stat -c '%a' "$BASE") instruments=$N_INSTR base_src=$D4_BASE_SRC"
else
  STAGED_NOW=no
  say "D6RF5_ROOT_PRESENT base=$BASE (NOT re-staged, NOTHING removed) -- every assertion below still runs"
fi
...
test "$MODE" = "777" || { say "ABORT L-251 run root mode $MODE != 777 at $BASE -- d6rf5_run_arm.sh:515 would refuse this root"; exit 44; }
say "D6RF5_STAGE_ROOT (1/3) OK $BASE exists, mode=$MODE -- d6rf5_run_arm.sh:515 satisfied"

# (2) :518-527 -- every asserted file present at $BASE with the launcher's md5.
for t in $TRIPLES; do
  V=${t%%:*}; rest=${t#*:}; F=${rest%%:*}; M=${rest##*:}
  echo "$M  $BASE/$F" | md5sum -c - >/dev/null 2>&1 || {
    say "ABORT STAGED md5 $F at $BASE ($V want=$M got=$(md5sum "$BASE/$F" 2>/dev/null | cut -d' ' -f1 || echo ABSENT)) -- d6rf5_run_arm.sh would refuse this at its \$BASE assertion"
    exit 41; }
done
say "D6RF5_STAGE_ROOT (2/3) OK all $N_INSTR staged instrument md5s match the launcher's own pins at $BASE -- d6rf5_run_arm.sh:$ASSERT_LINES satisfied"
...
test -n "$STAGED_MESH" || { say "ABORT STAGED $BASE/base/constant/polyMesh holds NEITHER points NOR points.gz -- d6rf5_run_arm.sh:632 would refuse"; exit 41; }
...
say "D6RF5_STAGE_ROOT (3/3) OK $STAGED_MESH md5=$STAGED_MESH_MD5, FFD/wingFFD.xyz present -- d6rf5_run_arm.sh:632 satisfied"
...
say "D6RF5_STAGE_ROOT LEDGER OK exact ITEM=$ITEM line present, no foreign item, no SHIPPED row -- G-ROOT.3 now passes on EVIDENCE, not on an absent file"
...
# both are the 54.00 core-min `d6rf5_grade.py:184` registers.  That is not a
...
say "D6RF5_STAGE_ROOT CEILING OK cap=$ARM_CAP_CORE_MIN ceiling=$ITEM_CEILING_CORE_MIN guard_rc=$GUARD_RC | $GUARD_LINE"

say "D6RF5_STAGE_ROOT COMPLETE base=$BASE staged_now=$STAGED_NOW instruments=$N_INSTR mode=$MODE utc=$(date -u +%Y%m%dT%H%M%SZ)"
say "D6RF5_STAGE_ROOT THIS FILE LAUNCHED NOTHING.  The re-fire is the dafoam-supervisor's decision."
exit 0
