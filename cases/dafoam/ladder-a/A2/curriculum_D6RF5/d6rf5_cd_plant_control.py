#!/usr/bin/env python3
"""Curriculum D6RF5 -- `PREREGISTRATION.md` section 3e's THIRD PLANTED-ZERO
...
cannot drift away from what it is controlling.  `d6rf5_grade.py` imports
...
def read_cd(path, point):
    ...
    artefact holds it -- a `repr()`'d string, because `d6rf5_fd_endpoint.py`
...
def run_cd_control(ctrl_dir, cd_path, reader=read_cd, point=PLANT_POINT):
    ...
    that is known to be wrong.  `d6rf5_grade.py` never passes it, so the graded
    ...
    planted_copy = os.path.join(ctrl_dir, "d6rf5_fd_endpoint.PLANTED.json")
...
def _synthetic_product(path):
    """A minimal artefact of the shape `d6rf5_fd_endpoint.py` writes.  Built
...
def drive():
    ...
    tmp = tempfile.mkdtemp(prefix="d6rf5_cd_ctrl_")
    prod = os.path.join(tmp, CD_ARTEFACT)
    cd = _synthetic_product(prod)
    print("D6RF5 CD PLANT CONTROL -- BOTH DIRECTIONS, no `assert` statement "
    ...
    if a.artefact:
        try:
            r = run_cd_control(a.ctrl_dir or tempfile.mkdtemp(
                prefix="d6rf5_cd_"), a.artefact)
        except CDRefusal as e:
            sys.stderr.write("D6RF5_CD_CONTROL REFUSED %s\n" % e)
            return 2
        print(json.dumps(r, indent=1, sort_keys=True, default=str))
        return 0
    sys.stderr.write("usage: d6rf5_cd_plant_control.py --drive "
                     "| --artefact <d6rf5_fd_endpoint.json>\n")
    return 64
