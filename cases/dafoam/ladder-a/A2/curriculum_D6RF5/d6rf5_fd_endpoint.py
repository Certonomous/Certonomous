#!/usr/bin/env python
"""Curriculum D6RF5 -- the CONVERGENCE PROBE `P_conv`, and (in `full` mode)
the endpoint finite-difference spot-check of the composite J, on the
NON-ORTHOGONAL-CORRECTION repair (successor to D6RF4, NOT A RESULT).

DERIVED FROM `curriculum_D6RF4/d6rf4_fd_endpoint.py`
(md5 `06ac0a171bde0ac3b17192f1819fa0ca`, verified on disk before this file was
written) with the REGISTERED DELTAS of `D6RF5 PREREGISTRATION.md` sections 2,
3, 4, 5 and 10, enumerated in `d6rf5_fd_endpoint_DELTAS_from_d6rf4.diff` beside
this file.  The FD MACHINERY IS CARRIED VERBATIM -- D6RF5 changes the
DISCRETISATION (staged fvSchemes/fvSolution), not the FD method.  The registered
deltas are:
  ...
  N4  LEG MARKERS, CARRIED.  Each leg prints `D6RF5_LEG_BEGIN <leg>` /
      `D6RF5_LEG_END <leg> ...`, which is how the GRADER splits one container log
  ...
  N5  THE PER-FIELD G-CONV FIRST-P-SOLVE EXTRACTION LIVES IN THE GRADER, NOT
      ...
      `d6rf5_grade.py:read_legs` records the FIRST p line of the final Time block
...
PRODUCER = "d6rf5_opt_runScript.py"
...
OUT = "d6rf5_fd_endpoint.json"
JSONL = "d6rf5_fd_endpoint.jsonl"
...
MODE_OUT = {"P_conv": OUT, "F5_scheme": "d6rf5_f5_endpoint.json", "full": OUT}
MODE_JSONL = {"P_conv": JSONL, "F5_scheme": "d6rf5_f5_endpoint.jsonl",
              "full": JSONL}
...
def _require_bar_before_fd(mode):
    ...
    if BAR_LEG not in _LEGS_RUN:
        sys.stderr.write(
            "D6RF5_FD REFUSE mode=%s reached an FD leg with legs_run=%r -- "
...
def main(mode):
    if mode not in MODES:
        sys.stderr.write("D6RF5_FD REFUSE unknown mode %r; registered: %r\n"
                         % (mode, MODES))
        sys.exit(2)
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("D6RF5_FD REFUSE producer md5 %s != frozen %s\n" % (got, PRODUCER_MD5))
        sys.exit(2)
    with open(PRODUCER) as fh:
        src = fh.read()
    if src.count(ANCHOR) != 1:
        sys.stderr.write("D6RF5_FD REFUSE anchor %r appears %d times\n" % (ANCHOR, src.count(ANCHOR)))
        sys.exit(2)
    header = src.split(ANCHOR)[0]
    saved_argv = list(sys.argv)
    sys.argv = [PRODUCER, "-task", "run_model", "-optimizer", "IPOPT"]
    ns = {"__name__": "d6rf5_frozen_header", "__file__": PRODUCER}
    ...
    with open("d6rf5_endpoint_dvs.json") as fh:
        dvs = json.load(fh)
    for key in DV_KEYS:
        if key not in dvs:
            sys.stderr.write("D6RF5_FD REFUSE endpoint dv file missing %s\n" % key)
            sys.exit(2)
    ...
    _state = {
        ...
        "fvSolution_statement": (
            "THE ONLY THING D6RF5 CHANGES ABOUT THE PRIMAL IS THE LINEAR-"
            ...
            "in every leg of this item. d6rf5_accept_floor_control.py reads "
    ...
    def _write_out():
        ...
        leg_say("D6RF5_FD_ENDPOINT_WRITTEN", out_path,
                "legs_run=%s" % ",".join(_LEGS_RUN),
                "n_rows=%d" % len(_state.get("rows", [])))

    def primal(tag):
        t0 = time.time()
        leg_say("D6RF5_LEG_BEGIN", tag, "mode=%s" % mode)
        prob.run_model()
        ...
        leg_say("D6RF5_LEG_END", tag, "mode=%s" % mode, "J=%r" % j,
    ...
    # launcher stages d6rf5_fvSchemes_D6RF4_ORIGINAL + 1 corrector before it)
    ...
    if mode == "F5_scheme":
        try:
            ...
            emit({"kind": "primal_raised", "tag": "baseline", "mode": mode,
                  "error": repr(exc)[:800]})
            leg_say("D6RF5_LEG_END", "baseline", "mode=%s" % mode,
                    "primal_raised=True")
        _write_out()
        leg_say("D6RF5_MODE_COMPLETE", mode,
                "legs_run=%s" % ",".join(_LEGS_RUN))
    ...
    if mode == "P_conv":
        # THE REGISTERED ARM STOPS HERE.  D6RF5 buys a CONVERGENCE measurement,
    ...
    if rank == 0:
        ...
        with open(tmp, "w") as fh:
            ...
            os.fsync(fh.fileno())
        os.replace(tmp, out_path)
        leg_say("D6RF5_FD_ENDPOINT_WRITTEN", out_path, "n_rows=%d" % len(rows))
    leg_say("D6RF5_MODE_COMPLETE", mode, "legs_run=%s" % ",".join(_LEGS_RUN))
    MPI.COMM_WORLD.Barrier()
