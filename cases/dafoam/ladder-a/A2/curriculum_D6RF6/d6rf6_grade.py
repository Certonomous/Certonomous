#!/usr/bin/env python3
"""Curriculum D6RF6 -- THE FROZEN GRADING PATH for `P_conv`.

DERIVED FROM `curriculum_D6RF4/d6rf4_grade.py`
(md5 `67e9508fab345d6d9387245d50af1fed`, re-hashed on disk before this file was
written) with the REGISTERED DELTAS of `D6RF6 PREREGISTRATION.md` sections 2, 3
and 10, enumerated in `d6rf6_grade_DELTAS_from_d6rf5.diff` beside this file.  The
SUBSTANTIVE D6RF6 deltas over the carried D6RF4 machinery are: G-CONV reads p's
...
in `PREREGISTRATION.md` before compute:
  ...
  N1  `ACCEPT_FLOOR_UNMOVED` (section 7 instrument 4).  `d6rf6_accept_floor_
...
`d6rf6_grade_DELTAS_from_d6rf5.diff` beside this file and there are SIX
mechanisms in them, all registered in `PREREGISTRATION.md` before compute:
  ...
  M3  A THIRD PLANTED-ZERO CONTROL (section 3e), because the source moved: the
      CD reader.  Its reader is IMPORTED from `d6rf6_cd_plant_control.py` and
...
# The gated source, FIXED BEFORE COMPUTE.  `d6rf6_major_history.json` is NOT
...
CD_MP_SOURCE_STATEMENT = (
    ...
    "d6rf6_fd_endpoint.py, at the endpoint design vector reconstructed to "
...
LAUNCHER = "d6rf6_run_arm.sh"
PRODUCT_WRITER = {
    "d6rf6_endpoint_dvs_PHYSICAL.json": "d6rf6_endpoint_physical.py",
    "d6rf6_endpoint_dvs_DRIVERSCALED.json": "d6rf6_endpoint_physical.py",
    "d6rf6_endpoint_dvs.json": "d6rf6_extract_endpoint.py",
    "d6rf6_major_history.json": "d6rf6_extract_endpoint.py",
    "d6rf6_fd_endpoint.json": "d6rf6_fd_endpoint.py",
    "d6rf6_f5_endpoint.json": "d6rf6_fd_endpoint.py",
}
...
ARTEFACT_PRODUCER = {
    "d6rf6_endpoint_dvs_PHYSICAL.json": "P_conv",
    "d6rf6_endpoint_dvs_DRIVERSCALED.json": "P_conv",
    "d6rf6_endpoint_dvs.json": "P_conv",
    "d6rf6_major_history.json": "P_conv",
    "d6rf6_fd_endpoint.json": "P_conv",
    "d6rf6_f5_endpoint.json": "P_conv",
    ...
    "d6rf6_ref_off.json": "REF_off",
}
REGISTERED_PRODUCTS = {
    "P_conv": ["d6rf6_endpoint_dvs_PHYSICAL.json", "d6rf6_endpoint_dvs_DRIVERSCALED.json",
               "d6rf6_endpoint_dvs.json", "d6rf6_major_history.json",
               "d6rf6_fd_endpoint.json", "d6rf6_f5_endpoint.json"],
}
...
# from `d6rf6_accept_floor_control` rather than re-declared, so this file and
...
# it is INAPPLICABLE to D6RF6, which DELIBERATELY changes the discretisation --
...
# own container log: the graded leg L1 must show d6rf6_fvSchemes_LIMITED
...
G_SCHEME_FILE = "d6rf6_fvSchemes_LIMITED"
...
# The arm runs ONE container and three legs inside it.  `d6rf6_fd_endpoint.py`
# prints `D6RF6_LEG_BEGIN`/`D6RF6_LEG_END` markers, which is how one container
...
LEG_BEGIN = "D6RF6_LEG_BEGIN"
LEG_END = "D6RF6_LEG_END"
LEGS = ("L1", "L2", "L3")
LEG_TAG = {"L1": "baseline", "L2": "baseline_repeat", "L3": "baseline"}
LEG_MODE = {"L1": "P_conv", "L2": "P_conv", "L3": "F5_scheme"}
# THE LEG DISCRIMINATOR IS fvSchemes, not fvSolution: D6RF6 changes the
...
LEG_FVSCHEMES = {"L1": "d6rf6_fvSchemes_LIMITED",
                 "L2": "d6rf6_fvSchemes_LIMITED",
                 "L3": "d6rf6_fvSchemes_D6RF4_ORIGINAL"}
# THE BYTES EACH LEG IS REGISTERED TO RUN.  The launcher installs the file and
# PRINTS `D6RF6_FVSOLUTION_INSTALLED leg=<L> md5=<m> sites=<n>` into the log;
...
FVSCHEMES_MD5 = {"d6rf6_fvSchemes_LIMITED": "8374443e7a374e9d353cffccdb654aaf",
                 "d6rf6_fvSchemes_D6RF4_ORIGINAL": "cf8f745dcc594ce7a9c67ad2d41e4b06"}
FVSCHEMES_INSTALL_MARKER = "D6RF6_FVSCHEMES_INSTALLED"
...
NULL_READINGS = {
    ...
    "G-SCHEME": {"run_derived_quantity": "installed fvSchemes md5 + p-solve "
                 ...
                 "bar_artefact": "the arm log segment between L1's LEG_BEGIN/END "
                                 "and its D6RF6_FVSCHEMES_INSTALLED marker",
                 "gated": True},
...
import d6rf6_cd_plant_control as cdc                               # noqa: E402
...
import d6rf6_accept_floor_control as afc                           # noqa: E402
...
# ------------------------------------------------------------ freeze check
ITEM_DIR_REL = "cases/dafoam/ladder-a/A2/curriculum_D6RF6"
...
FROZEN_PATHS = tuple("%s/%s" % (ITEM_DIR_REL, n) for n in (
    "PREREGISTRATION.md",
    "d6rf6_grade.py",
    "d6rf6_cd_plant_control.py",
    "d6rf6_accept_floor_control.py",
    "d6rf6_endpoint_locus.py",
    "d6rf6_endpoint_physical.py",
    "d6rf6_extract_endpoint.py",
    "d6rf6_fd_endpoint.py",
    "d6rf6_anchor_gate.py",
    "d6rf6_opt_runScript.py",
    "d6rf6_units_assert.py",
    "d6rf6_finiteness_mutation.py",
    "d6rf6_run_arm.sh",
    "d6rf6_fvSchemes_LIMITED",
    "d6rf6_fvSchemes_D6RF4_ORIGINAL",
    "d6rf6_fvSolution",
))
...
CROSS_ITEM = {
    ...
    # Opened by d6rf6_anchor_gate.py:167/:177 and d6rf6_endpoint_locus.py:554,
...
def frozen_path_coverage():
    ...
    imports and every `d6rf6_*` / `d4_*` filename literal it names, and REFUSES
    ...
    for node in _ast.walk(tree):
        if isinstance(node, _ast.Import):
            for a in node.names:
                if a.name.startswith(("d6rf6_", "d4_")):
                    imported.add(a.name + ".py")
        elif isinstance(node, _ast.ImportFrom) and node.module:
            if node.module.startswith(("d6rf6_", "d4_")):
                imported.add(node.module + ".py")
        elif isinstance(node, _ast.Constant) and isinstance(node.value, str):
            v = node.value
            if (v.startswith(("d6rf6_", "d4_"))
...
def imported_symbol_identity():
    ...
    import d6rf6_cd_plant_control as _cdc_again
    import d6rf6_accept_floor_control as _afc_again
    checks = {
...
def coefficient_equality_check():
    ...
    d6rf6_fvSchemes_LIMITED and returns the laplacian and snGrad non-orthogonal
...
def gate_g1(root, ledger_rows, census):
    ...
    for arm in ARMS:
        ...
        # other is `d6rf6_endpoint_physical.py`'s C3).  `float()` of a
...
def gate_dvl(root, census, runscript_d6r, runscript_d4):
    ...
    sys.path.insert(0, HERE)
    import d6rf6_endpoint_locus as locus
    ...
    # `root/F_mp/d6rf6_endpoint_dvs_PHYSICAL.json` on a run that writes to
    ...
    for arm, art, rs in ((ARMS[0], "d6rf6_endpoint_dvs_PHYSICAL.json", runscript_d6r),
        ...
        # `d6rf6_endpoint_locus.py`: a non-finite design component makes every
...
def gate_conv(root, census, legs):
    ...
    base = {"gate": "G-CONV_per_field_final_initRes",
            ...
            "bar_provenance": (
                ...
                "and d6rf6_accept_floor_control.py refuses if it has moved."),
            "graded_leg": "L1", "reported_legs": ["L2", "L3"],
            "d6rf4_measured": CONV_MEASURED_D6RF4,
            "per_leg": {}}
    producer = ARTEFACT_PRODUCER["d6rf6_fd_endpoint.json"]
...
def gate_scheme(root, census, legs, floor):
    ...
    the run's own container log, and reads NOT A RESULT otherwise:
      * L1's installed fvSchemes md5 == d6rf6_fvSchemes_LIMITED;
...
def gate_off(root, census):
    ...
    res = {"gate": "G-OFF_per_point",
           "artefacts": [CD_ARTEFACT, "d6rf6_ref_off.json"],
    ...
    if census["REF_off"]["state"] != "RAN":
        _r, _d = _no_input_reason(census, "REF_off", "d6rf6_ref_off.json", root)
        res.update(dict({"verdict": "NOT A RESULT", "reason": _r,
                         "artefact": "d6rf6_ref_off.json",
                         "per_point": {p: {"verdict": "NOT A RESULT",
                                           "reason": _r,
                                           "CD_mp_source": CD_MP_SOURCE_LABEL}
                                       for p in POINTS}}, **_d))
        res.update(NULL_READINGS["G-OFF"])
        return res
    rp = os.path.join(root, ARM_DIR["REF_off"], "d6rf6_ref_off.json")
    ...
    for p in POINTS:
        ...
        try:
            ...
            cd_ref = _ff(ref["points"][p]["CD"], "d6rf6_ref_off.json",
                         "points.%s.CD" % p)
            cl_ref = _ff(ref["points"][p]["CL"], "d6rf6_ref_off.json",
                         "points.%s.CL" % p)
        except cdc.CDRefusal as e:
    ...
    if cdp is None:
        ...
        if _r == ARM_RAN_ARTEFACT_NOT_WRITTEN:
            ...
            res["def7_statement"] = (
                ...
                "(d6rf6_fd_endpoint.py `_write_out`), so a run that dies at "
...
def report_reduction(root, census):
    ...
    try:
        j0 = _ff(J[0], "d6rf6_major_history.json", "J[0]")
        jf = _ff(J[-1], "d6rf6_major_history.json", "J[-1]")
    except NonFinite as e:
...
def compose(g1, dvl, fd, off, price, caps, tool, place, census,
    ...
    # d6rf6_fvSchemes_LIMITED with 3 correctors and equal 0.333 coefficients,
...
def grade(root, out_path, skip_freeze=False, runscript_d6r=None,
    ...
    if not skip_freeze:
        ...
        # written from memory -- `d6rf6_cd_plant_control.py` is on it because
    ...
    rs6 = runscript_d6r or os.path.join(HERE, "d6rf6_opt_runScript.py")
    ...
    ctrl_dir = os.path.join(os.path.dirname(os.path.abspath(out_path)),
                            "grader_controls")
    fd_path = None
    _fd_arm = ARTEFACT_PRODUCER["d6rf6_fd_endpoint.json"]
    if census[_fd_arm]["state"] == "RAN":
        fd_path = os.path.join(root, ARM_DIR[_fd_arm], "d6rf6_fd_endpoint.json")
    ...
    with open(out_path, "w") as fh:
        ...
        fh.flush()
        os.fsync(fh.fileno())
    print("D6RF6_VERDICT %s  spend %.3f core-min of a %.1f ceiling  -> %s"
...
def main():
    ...
    if a.freeze_selfcheck:
        try:
            ...
            sys.stderr.write("D6RF6_GRADE REFUSED %s\n" % e)
            return 2
        print("D6RF6 FREEZE SELF-CHECK -- no compute, no run root")
        ...
        if not coeff["equal"]:
            sys.stderr.write("D6RF6_GRADE REFUSED coefficient inequality %r\n" % coeff)
            return 2
        return 0
    if a.cap_arithmetic:
        try:
            ...
            sys.stderr.write("D6RF6_GRADE REFUSED %s\n" % e)
            return 2
        print("D6RF6 CAP / DEADLINE REACHABILITY -- ranks=%d frame_allowance_s=%d"
    ...
    if a.selftest:
        ...
        import d6rf6_finiteness_mutation as st
        rc = st.drive()
        rc2 = afc.drive()
        rc3 = cdc.drive()
        print("D6RF6 SELFTEST  finiteness=%d accept_floor=%d cd_plant=%d"
              % (rc, rc2, rc3))
        return max(rc, rc2, rc3)
    if not a.root or not a.out:
        sys.stderr.write("usage: d6rf6_grade.py --root <run root> --out <json>\n")
        return 64
    try:
