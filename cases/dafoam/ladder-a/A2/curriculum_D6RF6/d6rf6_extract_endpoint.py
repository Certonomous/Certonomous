#!/usr/bin/env python
"""Curriculum D6RF6 -- extract the optimiser's FINAL design point and the
...
`PREREGISTRATION.md` section 9 -- POSITION 1:
  ...
  * `J` IS KEPT.  `R-RED` (REPORTED, NOT GATED) reads it, and section 3c
    ...
    derived from a `NaN`.
  * Product names are this item's own (`d6rf6_*`), not `D6R`'s.  A product
...
HST = "OptView.hst"
OUT = "d6rf6_endpoint_dvs.json"
HIST_OUT = "d6rf6_major_history.json"
KEYS = ("twist", "shape", "patchV_cl04", "patchV_cl05", "patchV_cl06")


def refuse(msg):
    sys.stderr.write("D6RF6_EXTRACT REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    dropped = ("d6r_extract_endpoint.py:65-77 (per-point CL/CD) IS DROPPED in "
               ...
               "d6rf6_fd_endpoint.json points.<pt>.CD -- and NOT from here.")
    out["_cd_block_dropped"] = dropped
    hist["_cd_block_dropped"] = dropped
    out["_extractor"] = "d6rf6_extract_endpoint.py"
    hist["_extractor"] = "d6rf6_extract_endpoint.py"
    ...
    with open(OUT, "w") as fh:
        ...
        fh.flush()
        os.fsync(fh.fileno())
    sys.stdout.write("D6RF6_ENDPOINT_DVS_WRITTEN %s n_major_rows=%d n_shape=%d "
                     "history=%s\n"
