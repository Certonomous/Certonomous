...
the three points.  Writes `d6_endpoint_dvs.json` and `d6_major_history.json`.
...
HST = "OptView.hst"
OUT = "d6_endpoint_dvs.json"
HIST_OUT = "d6_major_history.json"
KEYS = ("twist", "shape", "patchV_cl04", "patchV_cl05", "patchV_cl06")
POINTS = ("cl04", "cl05", "cl06")


def refuse(msg):
    sys.stderr.write("D6_EXTRACT REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    with open(OUT, "w") as fh:
        ...
        fh.flush()
        os.fsync(fh.fileno())
    sys.stdout.write("D6_ENDPOINT_DVS_WRITTEN %s n_major_rows=%d n_shape=%d history=%s\n"
