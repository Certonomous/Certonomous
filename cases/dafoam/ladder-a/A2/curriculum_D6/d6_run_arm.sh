#!/usr/bin/env bash
# Curriculum D6 arm launcher -- multipoint cruise (CL 0.4/0.5/0.6) on D4's case.
#
...
# registered deltas of D6 PREREGISTRATION.md section 7, recorded in
# d6_run_arm_DELTAS_from_d5.diff.  Everything else is the family's bytes.
#
...
# the guard below is DRIVEN in d6_groot5_selftest.sh and shown to abort.
# ===========================================================================
ITEM=D6
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6-a2-wing-multipoint
...
if [ -f "$BASE/ledger.txt" ]; then
  ...
  if grep -aq "ROW=SHIPPED" "$BASE/ledger.txt" 2>/dev/null; then
    ...
    echo "  This launcher writes the PATCHED row only (D6 REGISTERED DELTA: the graded D4 row).  REFUSED."
    exit 3
  fi
fi
...
# D6 REGISTERED DELTA: 2,3,4,14 -- DISJOINT from every live container cpuset read
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d6_" | tr '\n' ',' | sed 's/,$//'
}

# ---- D6 REGISTERED CAP TABLE (PREREGISTRATION.md section 4) ----------------
#   arm      core-min cap   memory cap
#   O_mp        2000.0        20g
#   ACC_mp        30.0        20g   (three primals + three adjoints; 10.0 each)
#   F_mp         180.0        20g
#   REF_off       20.0        20g
...
MD5_EXTRACT6=a8f96e92ec03cd6773c7d8f82406254e
MD5_REFOFF=674e460e81c316487397eda49370cef6
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: d6_run_arm.sh <O_mp|ACC_mp|F_mp|REF_off> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: d6_run_arm.sh <O_mp|ACC_mp|F_mp|REF_off> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d6_${ARM}_" 2>/dev/null | grep "^d6_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/d6_opt_runScript.py"    | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_FD  $BASE/d6_fd_endpoint.py"             | md5sum -c - || { echo "ABORT fd md5"; exit 4; }
echo "$MD5_EXTRACT6  $BASE/d6_extract_endpoint.py"  | md5sum -c - || { echo "ABORT extract6 md5"; exit 4; }
echo "$MD5_REFOFF  $BASE/d6_ref_off.py"             | md5sum -c - || { echo "ABORT ref_off md5"; exit 4; }
...
test "$ROW" = "PATCHED" || {
  echo "ABORT G-ROW D6 is registered PATCHED-ONLY (the row D4 was GRADED on, C-97); got ROW=$ROW ($IMG)."
...
NAME="d6_${ARM}_${STAMP}"
WORK="$BASE/$ARM"
LOG="$BASE/${ARM}_${STAMP}.log"

# ---- stage a pristine copy of base/ for arms that need a cold case -------
# D6 REGISTERED DELTA: the multipoint model owns THREE full case copies mp04/
...
if [ "$ARM" != "F_mp" ]; then
  ...
  cp -a "$BASE/d6_opt_runScript.py" "$BASE/d6_fd_endpoint.py" "$BASE/d6_extract_endpoint.py" "$BASE/d6_ref_off.py" "$BASE/d4_extract_endpoint.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/OptView.hst" "$WORK/opt_IPOPT.txt" "$WORK/dRdWColoring_4.bin" "$WORK/d6_fd_endpoint.json" "$WORK/d6_ref_off.json"; do
  ...
  if [ "$ARM" = "REF_off" ]; then
    # D6 REGISTERED DELTA: D4's PATCHED history staged READ-ONLY, md5-asserted,
    ...
    echo "D6_D4_HST_STAGED arm=$ARM md5=$(md5sum "$WORK/OptView.hst" | cut -d' ' -f1) source=$D4_HST_SRC"
  fi
  ...
  cp -a "$BASE/d6_fd_endpoint.py" "$BASE/d6_extract_endpoint.py" "$WORK/" || { echo "ABORT stage F instruments"; exit 4; }
  rm -f "$WORK/d6_fd_endpoint.json" "$WORK/d6_fd_endpoint.jsonl" "$WORK/d6_endpoint_dvs.json" "$WORK/d6_major_history.json"
fi

case "$ARM" in
  O_mp)    CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6_opt_runScript.py -task run_driver -optimizer IPOPT" ;;
  ACC_mp)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6_opt_runScript.py -task compute_totals" ;;
  F_mp)    CMD="python d6_extract_endpoint.py && mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6_fd_endpoint.py" ;;
  REF_off) CMD="python d4_extract_endpoint.py && mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6_ref_off.py" ;;
esac
...
# bookkeeping runs in d6_chain_driver.sh, a setsid nohup session.
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     timeout -k 60 $TMO bash /mnt/$(basename "$WORK")/d6_cmd.sh" > /dev/null 2>&1 \
