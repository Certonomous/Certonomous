#!/usr/bin/env bash
# D6 chain driver -- DERIVED from curriculum_D5/d5_chain_driver.sh (itself the
...
# appended) with ONLY the registered deltas of D6 PREREGISTRATION.md section 7:
#   item names, run root, launcher md5, the D6 instruments (no FFD boxes -- D6
...
#   setsid nohup bash d6_chain_driver.sh O_mp ACC_mp F_mp REF_off > <root>/chain_launch.out 2>&1 &
...
test $# -ge 1 || { echo "ABORT usage: d6_chain_driver.sh <ARM...>"; exit 64; }
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/d6_driver.pid"
cd "$HERE" || exit 4
...
{ echo "$MD5_RUNSCRIPT  $BASE/d6_opt_runScript.py"; echo "$MD5_FD  $BASE/d6_fd_endpoint.py"; echo "$MD5_EXTRACT6  $BASE/d6_extract_endpoint.py"; echo "$MD5_REFOFF  $BASE/d6_ref_off.py"; echo "$MD5_EXTRACT4  $BASE/d4_extract_endpoint.py"; } | md5sum -c - || { echo "ABORT staged instrument md5"; exit 4; }
...
echo "D6_DRIVER start=$(date -u +%Y%m%dT%H%M%SZ) pid=$$ ppid=$PPID sid=$(ps -o sid= -p $$ | tr -d ' ') cwd=$(pwd) arms=[$ARMS] permission=$PERMISSION"
...
cap_mem_gib() { echo 20; }   # every D6 arm 20g (section 4); never 8g
for ARM in $ARMS; do
  ...
  while true; do
    AGG=$(python3 "$HERE/d6_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
  ...
  echo "D6_AGGREGATE arm=$ARM waited=$WAITED $AGG"
  echo "D6_DRIVER arm=$ARM begin=$(date -u +%Y%m%dT%H%M%SZ) ppid_now=$PPID permission=$PERMISSION"
  bash "$LAUNCHER" "$ARM" "$IMG" > "$BASE/${ARM}_launch.out" 2>&1
  rc=$?
  echo "D6_DRIVER arm=$ARM end=$(date -u +%Y%m%dT%H%M%SZ) rc=$rc"
