...
D6 PREREGISTRATION.md section 3:
  * PRODUCER is d6_opt_runScript.py (frozen md5 below); the header up to the
  ...
  * the endpoint DVs come from d6_endpoint_dvs.json (twist, shape,
    patchV_cl04/05/06);
...
OUT = "d6_fd_endpoint.json"
JSONL = "d6_fd_endpoint.jsonl"
...
def main():
    got = md5_of(PRODUCER)
    if got != PRODUCER_MD5:
        sys.stderr.write("D6_FD REFUSE producer md5 %s != frozen %s\n" % (got, PRODUCER_MD5))
        sys.exit(2)
    with open(PRODUCER) as fh:
        src = fh.read()
    if src.count(ANCHOR) != 1:
        sys.stderr.write("D6_FD REFUSE anchor %r appears %d times\n" % (ANCHOR, src.count(ANCHOR)))
        sys.exit(2)
    ...
    with open("d6_endpoint_dvs.json") as fh:
        dvs = json.load(fh)
    for key in DV_KEYS:
        if key not in dvs:
            sys.stderr.write("D6_FD REFUSE endpoint dv file missing %s\n" % key)
            sys.exit(2)
    ...
    if rank == 0:
        ...
        with open(OUT, "w") as fh:
            ...
            fh.flush()
            os.fsync(fh.fileno())
        sys.stdout.write("D6_FD_ENDPOINT_WRITTEN %s n_rows=%d\n" % (OUT, len(rows)))
