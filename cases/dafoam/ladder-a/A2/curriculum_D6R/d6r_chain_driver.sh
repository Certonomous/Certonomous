...
MD5_LAUNCHER=243f0f631719edf7ae354410276b3cfd
...
ARMS="$*"; STATUS="$BASE/STATUS.chain"; PIDFILE="$BASE/d6r_driver.pid"
cd "$HERE" || exit 4
...
if [ ! -d "$BASE" ]; then
  ...
  cp -a "$HERE/d6r_opt_runScript.py" "$HERE/d6r_fd_endpoint.py" "$HERE/d6r_extract_endpoint.py" "$HERE/d6r_ref_off.py" "$D4_CASE_DIR/d4_extract_endpoint.py" "$BASE/" || { echo "ABORT copy instruments"; exit 4; }
...
for ARM in $ARMS; do
  ...
  while true; do
    AGG=$(python3 "$HERE/d6r_aggregate_memory.py" "$(cap_mem_gib "$ARM")" "$AGG_CEILING_GIB")
