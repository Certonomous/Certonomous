#!/bin/bash
# so3dr_run_replay.sh -- detached launcher for the SO-3D Stage-1 reader.
#
...
# outcome. Every gate, threshold, plant and refusal lives in so3dr_replay.py.
#
...
# reader returns, and written to STATUS.SO3DR with the wall clock and a UTC stamp.
...
OUT="${CASE}/SO3DR_replay_${STAMP}.out"
RANKS=1

say() { printf '%s\n' "$*" >> "${STATUS}"; }

: > "${STATUS}"
say "SO3DR STAGE 1 -- launcher record"
say "launched_utc   ${STAMP}"
say "launcher       ${CASE}/so3dr_run_replay.sh"
say "reader         ${READER}"
...
say "stdout         ${OUT}"
say "result_json    ${RUN_ROOT}/so3dr_replay.json"
say "plant_json     ${RUN_ROOT}/so3dr_plant_report.json"
say ""
case "${RC}" in
  0) say "reader exit 0 -- every gate scored; verdicts are in so3dr_replay.json" ;;
