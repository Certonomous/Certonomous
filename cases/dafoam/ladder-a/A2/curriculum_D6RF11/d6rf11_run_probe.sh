#!/usr/bin/env bash
# D6RF11 probe launcher -- NEW FILE.  It edits no existing frozen launcher's
# guard; the shape a sibling lane and I were both refused tonight on D6RF10 is
# not attempted here.
#
# NO CLOCK KILL anywhere: no `timeout`, no `docker stop`, no ceiling that stops
# (Sanaa's NO-CAP ruling 6f3abf8a3).  rc is captured from the KERNEL via
# `docker inspect` before the container is removed.
set -uo pipefail

ITEM=D6RF11
BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6RF11-a2-wing-fd-simplec-probe
SRC=/home/ubuntu/certonomous-runs/CURRICULUM-D6RF3-a2-wing-multipoint-fd/F_mp
SCHEMES=/home/ubuntu/certonomous-runs/CURRICULUM-D6RF10-a2-wing-convergence-probe/P_conv/d6rf7_fvSchemes_LIMITED
SCHEMES_MD5=fbca617a0808c56113a34d156c5890b9
IMG=dafoam-idwarp-rot:v1
WANT=sha256:2927768a16acdea0330180fff95c8879c1dda9efcf6028728523b7dee30f6d35
RANKS=4
CPUSET=2,3,4,14
MEM=20g
ARM=F_probe
STAMP=$(date -u +%Y%m%dT%H%M%SZ)_$$
NAME="d6rf11_${ARM}_${STAMP}"
LOG="$BASE/${ARM}_${STAMP}.log"

# ---- G-ROOT: never stage over another item's graded row -------------------
for forb in /home/ubuntu/certonomous-runs/CURRICULUM-D6RF3-a2-wing-multipoint-fd \
            /home/ubuntu/certonomous-runs/CURRICULUM-D6RF10-a2-wing-convergence-probe \
            /home/ubuntu/certonomous-runs/CURRICULUM-D6R-a2-wing-multipoint \
            /home/ubuntu/certonomous-runs/CURRICULUM-D6R2-a2-wing-multipoint-transonic; do
  [ "$(realpath -m "$BASE")" = "$(realpath -m "$forb")" ] && { echo "ABORT G-ROOT BASE is $forb"; exit 3; }
done
# ---- G-ROOT.5: a live arm is never re-staged ------------------------------
LIVE=$(sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep "^d6rf11_${ARM}_" | head -1)
[ -n "$LIVE" ] && { echo "ABORT G-ROOT.5 a live container already carries this arm: $LIVE"; exit 3; }

mkdir -p "$BASE" || { echo "ABORT mkdir BASE"; exit 4; }
chmod 777 "$BASE"
W="$BASE/$ARM"
[ -e "$W" ] && { echo "ABORT G-COLD $W already exists -- refusing to re-stage"; exit 5; }

# ---- image identity BY DIGEST, never by tag ------------------------------
GOT=$(sudo -n docker image inspect --format '{{index .RepoDigests 0}}' "$IMG" 2>/dev/null | sed 's/.*@//')
[ "$GOT" = "$WANT" ] || { echo "ABORT digest mismatch got=$GOT want=$WANT"; exit 4; }
echo "D6RF11_IMAGE_OK row=PATCHED image=$IMG digest=$GOT"

# ---- stage: D6RF3's own arm directory, copied whole -----------------------
[ -d "$SRC" ] || { echo "ABORT source stage $SRC absent"; exit 4; }
cp -a "$SRC" "$W" || { echo "ABORT stage copy"; exit 4; }
# products of the FAILED predecessor are swept so nothing stale can be graded
rm -f "$W/d6rf3_fd_endpoint.jsonl" "$W/d6rf3_fd_endpoint.json" 2>/dev/null
rm -rf "$W/__pycache__" 2>/dev/null
rm -rf "$W"/processor* "$W"/mp04/processor* "$W"/mp05/processor* "$W"/mp06/processor* 2>/dev/null
# instrument identity, asserted on BOTH sides of the copy
for f in d6rf3_extract_endpoint.py d6rf3_endpoint_physical.py d6rf3_units_assert.py \
         d6rf3_fd_endpoint.py d6rf3_opt_runScript.py d6rf3_endpoint_dvs_PHYSICAL.json; do
  a=$(md5sum "$SRC/$f" | cut -d' ' -f1); b=$(md5sum "$W/$f" | cut -d' ' -f1)
  [ "$a" = "$b" ] || { echo "ABORT instrument $f md5 $a != $b across the copy"; exit 4; }
  echo "D6RF11_INSTRUMENT_OK $f md5=$a"
done
cp -a "$SCHEMES" "$W/d6rf7_fvSchemes_LIMITED" || { echo "ABORT fvSchemes stage"; exit 4; }
echo "$SCHEMES_MD5  $W/d6rf7_fvSchemes_LIMITED" | md5sum -c - || { echo "ABORT fvSchemes md5"; exit 4; }
# age datum: 0/ touched LAST, so every product must be strictly newer
touch "$W/0"/* || { echo "ABORT age datum"; exit 5; }
AGE=$(stat -c '%Y' "$W/0/U"); echo "$AGE" > "$W/.d6rf11_age_datum"
echo "D6RF11_G_COLD OK age_datum_epoch=$AGE"

# ---- the container command: D6RF10-R3's OWN install_config(), carried
# ---- unchanged, then D6RF3's OWN FD command, unchanged but for the datum.
CMDFILE="$W/d6rf11_cmd.sh"
sed -n '/^install_config() {/,/^}$/p' \
  /home/ubuntu/certonomous-runs/CURRICULUM-D6RF10-a2-wing-convergence-probe/P_conv/d6rf10_cmd_R3.sh \
  > "$CMDFILE" || { echo "ABORT could not carry install_config"; exit 4; }
grep -q 'a swap that swapped nothing' "$CMDFILE" || { echo "ABORT install_config carried WITHOUT its zero-site refusal"; exit 4; }
{
  echo 'set -o pipefail'
  echo "install_config R3 DARhoSimpleCFoam 12 0.70 0.70 2000 $SCHEMES_MD5 && \\"
  echo "python d6rf3_endpoint_physical.py --age-datum $AGE && \\"
  echo 'python d6rf3_units_assert.py d6rf3_endpoint_dvs.json --runscript d6rf3_opt_runScript.py && \\'
  echo "mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6rf3_fd_endpoint.py"
} >> "$CMDFILE"
echo "D6RF11_CMDFILE md5=$(md5sum "$CMDFILE" | cut -d' ' -f1)"

MEMAVAIL=$(python3 -c "print('%.2f' % ($(awk '/MemAvailable/{print $2}' /proc/meminfo)/1048576.0))")
echo "D6RF11_HOST_PRE MemAvailable_GiB=$MEMAVAIL load1=$(awk '{print $1}' /proc/loadavg) cpuset=$CPUSET"

T0=$(date -u +%s)
sudo -n docker run -d --name "$NAME" \
    --user 0:0 --cpus=$RANKS --cpuset-cpus=$CPUSET --memory=$MEM --memory-swap=$MEM --oom-score-adj=500 \
    -v "$BASE":/mnt -w "/mnt/$ARM" "$IMG" bash -lc \
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     echo D6RF11_CONTAINER_UID: \$(id -u) && \
     echo D6RF11_DEADLINE_IN_CONTAINER_S: NONE_NO_CAP_RULING_6f3abf8a3 && \
     bash /mnt/$ARM/d6rf11_cmd.sh" > /dev/null 2>&1 \
  || { echo "ABORT could not start container"; exit 4; }
echo "D6RF11_CONTAINER_STARTED name=$NAME"

# ---- wait.  REPORT the spend, stop nothing. ------------------------------
while [ "$(sudo -n docker inspect --format '{{.State.Running}}' "$NAME" 2>/dev/null)" = "true" ]; do
  sleep 20
done
sudo -n docker logs "$NAME" > "$LOG" 2>&1
rc=$(sudo -n docker inspect --format '{{.State.ExitCode}}' "$NAME" 2>/dev/null); [ -n "$rc" ] || rc=125
INSPECT=$(sudo -n docker inspect --format '{{.State.ExitCode}} {{.State.OOMKilled}}' "$NAME" 2>/dev/null)
T1=$(date -u +%s); WALL=$((T1-T0))
sudo -n docker rm "$NAME" >/dev/null 2>&1
sudo -n chown -R ubuntu:ubuntu "$LOG" "$W" 2>/dev/null
CORE_MIN=$(python3 -c "print(round($WALL*$RANKS/60.0,3))")
echo "$rc" > "$BASE/D6RF11_ARM_RC.txt"
{
  echo "ITEM=$ITEM"
  echo "ARM=$ARM ROW=PATCHED IMG=$IMG DIGEST=$GOT rc=$rc wall_s=$WALL ranks=$RANKS core_min=$CORE_MIN registered_prediction_core_min=1236.0 cap=NONE_NO_CAP_RULING_6f3abf8a3 memory=$MEM inspect(exit,oomkilled)=[$INSPECT] cpuset=$CPUSET log=$(basename "$LOG") stamp=$STAMP"
} | tee -a "$BASE/ledger.txt"
test -s "$LOG" && touch "$LOG.ok.${STAMP}"
echo "STAMP=$STAMP RC=$rc CORE_MIN=$CORE_MIN"
exit $rc
