...
#
# usage: d6r3_mp_r3_arm.sh <RANKS> [<EXCLUDE_CORES_CSV>]
set -uo pipefail
RANKS="${1:?ranks}"
EXCLUDE="${2:-}"
ARM=MP_R3
...
say(){ echo "D6R3_MPR3_$1 $2" | tee -a "$LEDGER"; }
...
for f in D6R3_MULTIPOINT_PREREGISTRATION.md d6r3_mp_r3_arm.sh d6r3_mp_stage_r3.py; do
...
python3 "$HERE/d6r3_mp_stage_r3.py" "$HERE/d6r3_opt_runScript.py" "$ARMDIR/runScript.py" 2>&1 | tee -a "$LEDGER"
...
[ "$NDIFF" -eq 11 ] || { say REFUSE "G-STAGE: $NDIFF changed lines, expected 11 (2 replaced + 5 added + 1 replaced, as < and >)"; exit 10; }
...
grep -q '"pcFillLevel": 2,' "$ARMDIR/runScript.py" || { say REFUSE "G-VERBATIM: pcFillLevel 2 not readable back"; exit 11; }
grep -q '"pcFillLevel": 1,' "$ARMDIR/runScript.py" && { say REFUSE "G-VERBATIM: pcFillLevel 1 still present"; exit 11; }
...
# BOTH gates are FIRST-CLASS.  A fill level that silently did not take is the same class of no-op as
# a misspelled ordering (which falls back to nested dissection without error).  Either one wrong and
# the arm is NOT A RESULT; no other number from it is offered as evidence.
LEVOK=1
...
case "$GOTFIL" in 2) : ;; "") say LEVER-FAIL "NO 'ILU PC Fill Level:' line -- UNPROVEN"; LEVOK=0 ;;
  *) say LEVER-FAIL "fill level printed '$GOTFIL', NOT 2 -- DID NOT ACT"; LEVOK=0 ;; esac
[ "$LEVOK" = "1" ] && say LEVER-OK "BOTH registered changes ACTED: rcm AND fill level 2" \
                   || say LEVER-VERDICT "NOT A RESULT -- a registered change is unproven"
exit $RC
