...
#
# usage: d6r3_mp_r4_arm.sh <RANKS> [<EXCLUDE_CORES_CSV>]
set -uo pipefail
RANKS="${1:?ranks}"
EXCLUDE="${2:-}"
ARM=MP_R4
MEMG=420  # CORRECTED: 120 GiB killed MP_R2 at iteration 400 via CONSTRAINT_MEMCG on a 739 GiB box (directive #17)
...
say(){ echo "D6R3_MPR4_$1 $2" | tee -a "$LEDGER"; }
...
for f in D6R3_MULTIPOINT_PREREGISTRATION.md d6r3_mp_r4_arm.sh d6r3_mp_stage_r4.py; do
...
python3 "$HERE/d6r3_mp_stage_r4.py" "$HERE/d6r3_opt_runScript.py" "$ARMDIR/runScript.py" 2>&1 | tee -a "$LEDGER"
...
[ "$NDIFF" -eq 10 ] || { say REFUSE "G-STAGE: $NDIFF changed lines, expected 10 (1 replaced->2 + 5 added + 1 replaced, as < and >)"; exit 10; }
...
grep -q '"asmOverlap": 2,' "$ARMDIR/runScript.py" || { say REFUSE "G-VERBATIM: asmOverlap 2 not readable back"; exit 11; }
...
GOTORD=$(grep -m1 "^Mat ReOrdering:" "$LOG" | sed "s/.*: //" | tr -d "\r")
GOTFIL=$(grep -m1 "^ILU PC Fill Level:" "$LOG" | sed "s/.*: //" | tr -d "\r")
say LEVER "Mat ReOrdering=<$GOTORD> ILU PC Fill Level=<$GOTFIL>"
# BOTH gates FIRST-CLASS.  MP_R2b's whole purpose is to measure pcFillLevel 1 against MP_R3's 2, so a
# fill level that silently did not take would destroy the comparison as surely as a wrong ordering.
GOTASM=$(grep -m1 "^ASM Overlap:" "$LOG" | sed "s/.*: //" | tr -d "\r")
say LEVER "ASM Overlap=<$GOTASM>"
# THREE gates, ALL first-class.  Every one of them fails SILENTLY: an unrecognised ordering falls
# back to nested dissection, and a fill level or overlap that did not take simply runs the old value.
LEVOK=1
case "$GOTORD" in rcm) : ;; "") say LEVER-FAIL "NO 'Mat ReOrdering:' line -- UNPROVEN"; LEVOK=0 ;;
  *) say LEVER-FAIL "ordering printed '$GOTORD', NOT rcm -- DID NOT ACT"; LEVOK=0 ;; esac
case "$GOTFIL" in 1) : ;; "") say LEVER-FAIL "NO 'ILU PC Fill Level:' line -- UNPROVEN"; LEVOK=0 ;;
  *) say LEVER-FAIL "fill level printed '$GOTFIL', NOT the published 1"; LEVOK=0 ;; esac
case "$GOTASM" in 2) : ;; "") say LEVER-FAIL "NO 'ASM Overlap:' line -- UNPROVEN"; LEVOK=0 ;;
  *) say LEVER-FAIL "ASM overlap printed '$GOTASM', NOT 2 -- THE RUNG-4 CHANGE DID NOT ACT"; LEVOK=0 ;; esac
[ "$LEVOK" = "1" ] && say LEVER-OK "ALL THREE gates ACTED: rcm AND fill level 1 AND ASM overlap 2" \
                   || say LEVER-VERDICT "NOT A RESULT -- a registered value is unproven"
exit $RC
