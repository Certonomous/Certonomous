#!/bin/bash
# D6R3 DIAGNOSTIC LAUNCHER -- arm DIAG_AGGLOM1 only.  The FROZEN producer runs, in the published
# order, with ONE staging change: the GAMGAgglomeration debug switch on.  DRAFT, nothing is sent or filed (rule 7).
# Registered by D6R3_DIAG_AGGLOM_PREREGISTRATION.md before any compute (rule 2).
...
# DIAG-AGGLOM: turn ON the GAMGAgglomeration debug switch so GAMGAgglomeration::New calls
# printLevels() and the coarse-grid hierarchy each instance builds is PRINTED.  Bit 0 only; the
# only other guarded block in this file is `debug & 2`, which stays off.  This is an instrument,
# and it carries its own control: instance 1 must still reproduce P00 bit-for-bit at step 1, or
# the instrument changed the physics and the arm is NOT A RESULT.
for d in mp04 mp05 mp06; do
  sed -i 's/^    SolverPerformance 0;$/    SolverPerformance 0;\n    GAMGAgglomeration 1;/' "$ARMDIR/$d/system/controlDict"
  grep -q "GAMGAgglomeration 1;" "$ARMDIR/$d/system/controlDict" || { say REFUSE "G-DEBUGSW: could not arm the GAMGAgglomeration debug switch in $d"; exit 10; }
done
for d in mp04 mp05 mp06; do
