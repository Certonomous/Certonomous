...
#
# usage: d6r3_mp_arm.sh <RANKS> [<EXCLUDE_CORES_CSV>]
set -uo pipefail
RANKS="${1:?ranks}"
EXCLUDE="${2:-}"
ARM=MP_R1
...
say(){ echo "D6R3_MP_$1 $2" | tee -a "$LEDGER"; }
...
for f in D6R3_MULTIPOINT_PREREGISTRATION.md d6r3_mp_arm.sh; do
...
cp "$HERE/d6r3_opt_runScript.py" "$ARMDIR/runScript.py"          # VERBATIM, byte-for-byte
...
say G-VERBATIM "runScript md5 $S == frozen; all 5 system dicts byte-identical to published; relTol 0.1, endTime 2000, primalMinResTol 1.0e-8 read back; primalFuncStdTol ABSENT; primalMinResTolDiff UNSET (published 1.0e2 stands)"
