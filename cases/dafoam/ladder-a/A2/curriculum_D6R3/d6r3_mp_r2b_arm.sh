...
#
# usage: d6r3_mp_r2b_arm.sh <RANKS> [<EXCLUDE_CORES_CSV>]
set -uo pipefail
RANKS="${1:?ranks}"
EXCLUDE="${2:-}"
ARM=MP_R2b
...
say(){ echo "D6R3_MPR2B_$1 $2" | tee -a "$LEDGER"; }
...
for f in D6R3_MULTIPOINT_PREREGISTRATION.md d6r3_mp_r2b_arm.sh d6r3_mp_stage_r2.py; do
...
case "$GOTFIL" in 1) : ;; "") say LEVER-FAIL "NO 'ILU PC Fill Level:' line -- UNPROVEN"; LEVOK=0 ;;
  *) say LEVER-FAIL "fill level printed '$GOTFIL', NOT 1 -- this arm is not the pcFillLevel 1 control"; LEVOK=0 ;; esac
[ "$LEVOK" = "1" ] && say LEVER-OK "BOTH gates ACTED: rcm AND fill level 1 (the control against MP_R3's 2)" \
