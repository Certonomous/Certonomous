...
# d6r2_run_arm_DELTAS_from_d6.diff.  Everything else is the family's bytes.
#
...
# the guard below is DRIVEN in d6r2_groot5_selftest.sh and shown to abort.
# ===========================================================================
ITEM=D6R2
REGISTERED_BASE=/home/ubuntu/certonomous-runs/CURRICULUM-D6R2-a2-wing-multipoint-transonic
...
container_census() {
  sudo -n docker ps --format '{{.Names}}' 2>/dev/null | grep -v "^d6r2_" | tr '\n' ',' | sed 's/,$//'
}
...
MD5_RUNSCRIPT=0abba50ab8baeefa3f59a3f1fc8f5336
MD5_FD=34d0cb87e47244bb460e6ff58e318302
MD5_EXTRACT6=07b89f6e7275868b55c2a32ff0d65b51
MD5_REFOFF=f4ea77e96fc577b24671dc17903c5b32
...
ARM="${1:-}"; IMG="${2:-}"
test -n "$ARM" || { echo "ABORT usage: d6r2_run_arm.sh <O_mp|ACC_mp|F_mp|REF_off> <image>"; exit 64; }
test -n "$IMG" || { echo "ABORT usage: d6r2_run_arm.sh <O_mp|ACC_mp|F_mp|REF_off> <image>"; exit 64; }
...
LIVE_SAME_ARM=$(sudo -n docker ps --format '{{.Names}}' --filter "name=^d6r2_${ARM}_" 2>/dev/null | grep "^d6r2_${ARM}_" | head -3 | tr '\n' ',' | sed 's/,$//')
...
echo "$MD5_RUNSCRIPT  $BASE/d6r2_opt_runScript.py"    | md5sum -c - || { echo "ABORT runScript md5"; exit 4; }
echo "$MD5_FD  $BASE/d6r2_fd_endpoint.py"             | md5sum -c - || { echo "ABORT fd md5"; exit 4; }
echo "$MD5_EXTRACT6  $BASE/d6r2_extract_endpoint.py"  | md5sum -c - || { echo "ABORT extract6 md5"; exit 4; }
echo "$MD5_REFOFF  $BASE/d6r2_ref_off.py"             | md5sum -c - || { echo "ABORT ref_off md5"; exit 4; }
...
if [ "$ARM" != "F_mp" ]; then
  ...
  cp -a "$BASE/d6r2_opt_runScript.py" "$BASE/d6r2_fd_endpoint.py" "$BASE/d6r2_extract_endpoint.py" "$BASE/d6r2_ref_off.py" "$BASE/d4_extract_endpoint.py" "$WORK/" || { echo "ABORT stage instruments"; exit 4; }
  ...
  for bad in "$WORK/reports" "$WORK/OptView.hst" "$WORK/opt_IPOPT.txt" "$WORK/dRdWColoring_4.bin" "$WORK/d6r2_fd_endpoint.json" "$WORK/d6r2_ref_off.json"; do
  ...
  cp -a "$BASE/d6r2_fd_endpoint.py" "$BASE/d6r2_extract_endpoint.py" "$WORK/" || { echo "ABORT stage F instruments"; exit 4; }
  rm -f "$WORK/d6r2_fd_endpoint.json" "$WORK/d6r2_fd_endpoint.jsonl" "$WORK/d6r2_endpoint_dvs.json" "$WORK/d6r2_major_history.json"
fi

case "$ARM" in
  O_mp)    CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6r2_opt_runScript.py -task run_driver -optimizer IPOPT" ;;
  ACC_mp)  CMD="mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6r2_opt_runScript.py -task compute_totals" ;;
  F_mp)    CMD="python d6r2_extract_endpoint.py && mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6r2_fd_endpoint.py" ;;
  REF_off) CMD="python d4_extract_endpoint.py && mpirun --allow-run-as-root -np $RANKS --bind-to core --report-bindings -x PYTHONPATH python d6r2_ref_off.py" ;;
esac
...
# bookkeeping runs in d6r2_chain_driver.sh, a setsid nohup session.
sudo -n docker run -d --name "$NAME" \
    ...
    "source /home/dafoamuser/dafoam/loadDAFoam.sh && \
     ...
     echo D4S_DEADLINE_IN_CONTAINER_S: NONE_NO_CAP_RULING_6f3abf8a3 && \
     bash /mnt/$(basename "$WORK")/d6r2_cmd.sh" > /dev/null 2>&1 \
...
echo "D4S_RUNAWAY_GUARD arm=$ARM cap_core_min=$CAP ceiling_core_min=$CEILING mode=report_only_NO_CAP_RULING_6f3abf8a3"
...
while true; do
  ...
  if [ "$CEILING_HIT" = "no" ] && [ "$(python3 -c "print(1 if $CM > $CEILING else 0)")" = "1" ]; then
    CEILING_HIT=yes
    echo "D4S_CEILING_CROSSED arm=$ARM core_min=$CM ceiling=$CEILING action=REPORTED_RUN_CONTINUES_NO_CAP_RULING_6f3abf8a3" | tee -a "$BASE/ledger.txt"
  fi
  sleep 10
done
