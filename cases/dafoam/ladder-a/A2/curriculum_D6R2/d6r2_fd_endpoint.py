...
D6 PREREGISTRATION.md section 3:
  * PRODUCER is d6r2_opt_runScript.py (frozen md5 below); the header up to the
  ...
  * the endpoint DVs come from d6r2_endpoint_dvs.json (twist, shape,
    patchV_cl04/05/06);
...
OUT = "d6r2_fd_endpoint.json"
JSONL = "d6r2_fd_endpoint.jsonl"
...
def main():
    ...
    with open("d6r2_endpoint_dvs.json") as fh:
        dvs = json.load(fh)
    for key in DV_KEYS:
        if key not in dvs:
