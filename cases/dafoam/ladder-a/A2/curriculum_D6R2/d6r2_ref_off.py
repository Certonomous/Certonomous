...
off-design reference that makes "single-point dominance" measurable (G-D6-1).
  ...
  * The model is the producer's own bytes: d6r2_opt_runScript.py is read, its md5
  ...
  * findFeasibleDesign solves CL(cl04)=0.4, CL(cl05)=0.5, CL(cl06)=0.6 on
    ...
    rank 0 (d6r2_ref_off.json), never recovered from stdout.
...
def refuse(msg):
    sys.stderr.write("D6R_REF_OFF REFUSE %s\n" % msg)
    sys.exit(2)
...
def main():
    ...
    ns = {"__name__": "d6r2_frozen_header", "__file__": PRODUCER}
    ...
    if rank == 0:
        with open(OUT, "w") as fh:
            ...
            fh.flush()
            os.fsync(fh.fileno())
        sys.stdout.write("D6R_REF_OFF_WRITTEN %s\n" % OUT)
