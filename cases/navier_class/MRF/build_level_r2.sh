#!/usr/bin/env bash
# MRF_R2 PER-LEVEL MESH BUILDER -- usage: build_level_r2.sh <RUNDIR> <NX> <NY> <NZ> <NCBL>
#
...
cp -r "$HERE/R2/system" "$RUNDIR/system"      || { echo "ABORT: R2 system copy"; exit 2; }
...
echo "=== MRF_R2 build $(date -u +%FT%TZ) rundir=$RUNDIR block=($NX $NY $NZ) NCBL=$NCBL"
