#!/bin/bash
# run_rung0.sh -- the RUNG 0 mesh-import driver.
#
# Registration: verification/campaign/RUNG0_MESH_IMPORT_PREREGISTRATION.md,
# FROZEN at d127d83d4ebf4caaaf0c12c39a063c5f624a03a4 (2026-09-03T17:17:27Z).
#
# WHAT IT RUNS.  For each of section 3's four committee grids: ugrid_to_foam.py, then
# checkMesh.  Then analyse_rung0.py, which runs section 7's planted controls FIRST and
# grades R0-G1, R0-G2a, R0-G3 and R0-G4.
#
# WHAT IT CANNOT RUN.  R0-G2b -- cases/committee-grids/foam_to_ugrid.py DOES NOT EXIST.
# NO `PASS` IS REACHABLE FROM THIS DRIVER.  The rung's verdict is section 4's
# CONJUNCTION and one conjunct is unbuilt; the comparator emits PENDING and refuses to
# print PASS.  That is registered, not discovered here.
#
# THE CAP IS STRUCTURAL, NOT A POLICY.  Section 13.1 fixes the hard cap at 23 core-min.
# At ranks = 1 that is 23 * 60 / 1 = 1380 wall seconds, and this script enforces it with
# a `timeout` budget RECOMPUTED BEFORE EVERY STEP from one total.  An overrun STOPS THE
# RUN; it does not get a new budget (CLAUDE.md rule 12).  A cap nothing enforces is not
# a cap.
#
# R0-G4, THE NO-CLOBBER GUARD.  The driver REFUSES (exit 3) if any target case directory
# already exists, writing nothing.  It stamps RUN_ROOT_CREATED_EPOCH BEFORE anything
# else, and every artifact the comparator grades must be NEWER than that stamp.
#
# rc IS CAPTURED INSIDE THIS SCRIPT, never around a `setsid` line: `setsid timeout cmd`
# exits 0 for every outcome, so a wrapper that reads the setsid parent's rc reads a
# constant zero.
set -u

RUN_ROOT=/home/ubuntu/Certonomous/verification/runs/RUNG0_MESH_IMPORT_runs
...
ANALYSE="$RUN_ROOT/analyse_rung0.py"

CAP_CORE_MIN=23
RANKS=1
BUDGET=$(( CAP_CORE_MIN * 60 / RANKS ))     # 1380 s. THE WHOLE RUN'S BUDGET.
START=$(date +%s)

STATUS="$RUN_ROOT/STATUS.RUNG0_MESH_IMPORT"
mkdir -p "$RUN_ROOT"
...
# converter rather than after it.
say "planted controls (registration section 7) -- before any conversion"
timeout "$(remaining)" python3 "$ANALYSE" --controls-only > "$RUN_ROOT/log.controls" 2>&1
RC=$?
[ $RC -eq 0 ] || fail 5 "planted controls did not all fire (rc=$RC) -- NOT A RESULT, and no core-minutes were spent on a conversion"
say "controls: all fired"
...
EOF
  ...
  R=$?
  [ $R -eq 0 ] || fail 8 "checkMesh rc=$R on $NAME (124 = cap stop). NOTE: a non-zero rc here is a PROCESS failure. checkMesh's own mesh VERDICT is never read by this rung."
  T1=$(date +%s)
  local CHECKS=$((T1-T0))
...
say "grading"
timeout "$(remaining)" python3 "$ANALYSE" --run-root "$RUN_ROOT" > "$RUN_ROOT/log.analyse" 2>&1
RC=$?
TOTAL=$(( $(date +%s) - START ))
CORE_MIN=$(python3 -c "print(round($TOTAL*$RANKS/60.0,4))")
say "TOTAL ${TOTAL} wall s at ${RANKS} rank = ${CORE_MIN} core-min (cap ${CAP_CORE_MIN})"
echo "rc=$RC total_wall_s=$TOTAL core_min=$CORE_MIN cap_core_min=$CAP_CORE_MIN" > "$STATUS"
[ $RC -eq 0 ] || fail 9 "analyse_rung0 rc=$RC -- see $RUN_ROOT/log.analyse"
say "DONE. Verdict is in $RUN_ROOT/RESULTS.json and log.analyse. NO PASS IS REACHABLE: R0-G2b's writer does not exist."
exit 0
