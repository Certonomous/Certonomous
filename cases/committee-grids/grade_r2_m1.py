#!/usr/bin/env python3
"""
R2-M1 COMPARATOR -- the frozen grading path for
`verification/campaign/RUNG2_CRM_M1_PREREGISTRATION.md`.

THIS FILE IS THE GRADING PATH. Rule 2: it is fixed at the pre-registration commit and the
driver verifies at run time that the file that ran IS the committed blob. It is NOT a copy
of `grade_r2_m0.py` and does not import it -- that file is frozen (rule 6) and is left
exactly as it is.

WHAT M1 GRADES THAT M0 COULD NOT
--------------------------------
M0's mechanism gate `R2-G2` came back `NOT A RESULT`. The reason is recorded in
`docs/COST_CALIBRATION.md` row C-20260906T161200.871248Z-5aae017a, and it is worth stating
precisely because M1's gate is built to avoid repeating it:

    M0's R2-G2 required "a per-iteration min/max line ... up to its last completed
    iteration" for EVERY arm. M0's A2 completed ZERO iterations -- it aborted inside
    Time = 1, before the first end-of-step write -- so it had no last completed iteration
    and no postProcessing directory at all. The gate was UNSATISFIABLE for that arm by
    construction, not by any defect of the writer.

M1's writer gate therefore asks a question an arm can actually answer:
`records_written == steps_completed`, exactly, per arm, per field. An arm that completed
zero steps satisfies it with zero records, and says so as `COMPLETED_STEPS=0` rather than
as a missing file. An arm that completed steps and wrote nothing FAILS it.

AND M1 ADDS THE INSTRUMENT M0 LACKED. An end-of-step writer can never record the step that
kills the run. Two independent readers are therefore graded:

    W1  fieldMinMax records -- end of each COMPLETED step. Cannot see the killing step.
    W2  the solver's own intra-step lines (`pressureControl: p min`, `time step continuity
        errors`) -- written DURING a step, so they survive the abort W1 cannot record.

Both were rehearsed pre-freeze, per arm, at 0.0079 core-min:
...
# same box, same day, one aborting and one clean. Positive and negative control.
CONTROL_ABORT_LOG = (
...
REPRODUCTION_ABORT_TIME = 2       # R2M1-G0: B0 must abort at Time <= this
ADMISSION_ENDTIME = 50            # R2M1-G4: B1's admission bar
MECHANISM_MIN_TIME = 3            # R2M1-G2: B2 must carry a record at Time >= this
MINMAX_FIELDS = ("T", "p", "rho", "mag(U)")
FO_NAME = "r2m1MinMax"

# The four cells of the 2x2. NAMED, NOT INTERPRETED -- see the registration section 9.
FACTORIAL = {"B5": ("e", "no"), "B3": ("h", "no"), "B4": ("e", "yes"), "B6": ("h", "yes")}
...
def read_minmax(dat_path, field):
    ...
    with a satisfied gate -- G1 compares it against steps_completed, so a zero passes
    only where the arm completed zero steps. The zero is plant-verified in selftest C8.
    """
...
def completion_clauses(case_dir, rc, facts, endtime):
    ...
    present = []
    for field in ("T", "U", "p", "k", "omega", "nut", "alphat"):
        if not os.path.exists(os.path.join(time_dir, field)):
            failed.append("field %s at endTime" % field)
        else:
            present.append(os.path.join(time_dir, field))

    # THE AGE GUARD. `0/T` is touched last at launch, so it dates the run that was
    # allowed to produce this answer. A field older than it is a leftover.
...
def prove_assert_free(own_path):
    """L-221/L-222 in the checking direction: this comparator must carry no bare
    `assert`, which vanishes under -O. Proved by reading its own source, and the
    detector is planted in C0 so a zero here is not a blind zero."""
    source = open(own_path).read()
    return len(re.findall(r"(?m)^\s*assert\s", source))
...
# evidence -- so every reader here is driven in BOTH directions, and the two GATE
# functions are driven in the REFUSING direction too (C10, C11).
# ---------------------------------------------------------------------------
...
def selftest(own_path):
    ...
    # C0 -- the assert detector, planted.
    real = prove_assert_free(own_path)
    with tempfile.TemporaryDirectory() as tmp:
        planted = os.path.join(tmp, "planted.py")
        open(planted, "w").write(open(own_path).read() + "\nassert True\n")
        seen = prove_assert_free(planted)
    record("C0", real == 0 and seen == 1,
           "assert-free proved (%d) and the detector sees a planted one (%d)" % (real, seen))
    ...
    # C10 -- THE WRITER GATE ITSELF, driven in the REFUSING direction. A reader that
    # discriminates is not enough; the GATE must be shown able to fail.
    ...
    record("C10", good and not short and not silent and zero,
           "writer gate PASSES 3-of-3, FAILS 1-of-3 and 0-of-3, and PASSES an arm that "
           "completed zero steps with zero records (M0's A2 case, which M0's gate could "
           "not express)")
...
def _completion_control():
    ...
    good = (ok_all and not ok_short and not ok_noend and not ok_rc and not ok_field
...
def writer_gate(steps_completed, w1, w2):
    """R2M1-G1, one arm. `records == steps`, exactly, per field, on BOTH instruments.

    An arm that completed ZERO steps satisfies this with ZERO records -- that is the
    case M0's A2 presented and M0's gate could not express (it demanded a record "up to
    its last completed iteration" of an arm that had none). An arm that completed steps
    and wrote fewer records FAILS.
    """
...
def grade(root):
    ...
    # -- R2M1-G0, reproduction control. A PRECONDITION: if it fails, nothing else stands.
    b0 = arm["B0"]
    if b0["state"] == "BLOCKED":
    ...
    status["R2M1_G0"] = "PASS" if g0 else "NOT A RESULT"
    lines.append("R2M1-G0: %s -- B0 rc=%s, class=%s, last Time=%s, thermo frames=%d"
                 % (status["R2M1_G0"], b0["rc"], b0["class"], b0["facts"]["last_time"],
                    b0["facts"]["thermo_frame"]))
    if not g0:
        lines.append("R2M1 VERDICT: NOT A RESULT -- the reproduction control did not "
                     "reproduce, so no conclusion is drawn from any other arm.")
        status["R2M1_VERDICT"] = "NOT A RESULT"
        return lines, status

    # -- R2M1-G1, writer liveness, PER ARM.
    g1_fail = []
    for name in ARMS:
        a = arm[name]
        if a["state"] == "BLOCKED":
    ...
    status["R2M1_G1"] = "PASS" if not g1_fail else "GATE FAIL"
    lines.append("R2M1-G1: %s -- writer liveness%s"
                 % (status["R2M1_G1"],
                    "" if not g1_fail else " FAILED on: %s" % ", ".join(g1_fail)))

    # -- R2M1-G2, mechanism. B2 runs with the FPE trap OFF, so the step that kills the
    #    other arms COMPLETES and its end-of-step state is recorded instead of guessed.
    b2 = arm["B2"]
    if b2["state"] == "BLOCKED" or status["R2M1_G1"] != "PASS":
        status["R2M1_G2"] = "NOT A RESULT"
        lines.append("R2M1-G2: NOT A RESULT -- %s"
                     % ("B2 was BLOCKED" if b2["state"] == "BLOCKED"
                        else "the writer gate did not pass, so no record is trusted"))
    else:
        reached = (b2["facts"]["last_time"] or 0) >= MECHANISM_MIN_TIME
        recorded = all(b2["w1"][f] >= MECHANISM_MIN_TIME for f in MINMAX_FIELDS)
        status["R2M1_G2"] = "PASS" if (reached and recorded) else "NOT A RESULT"
        lines.append("R2M1-G2: %s -- B2 (no FPE trap) last Time=%s, W1=%s, required >= %d"
                     % (status["R2M1_G2"], b2["facts"]["last_time"],
                        [b2["w1"][f] for f in MINMAX_FIELDS], MECHANISM_MIN_TIME))

    # -- R2M1-G3, the warm start MAPS AND EXERCISES.
    b1 = arm["B1"]
    mapped = os.path.isfile(os.path.join(root, "B1", "WARMSTART_MAPPED"))
    status["R2M1_G3"] = "PASS" if (mapped and b1["state"] != "BLOCKED") else "BLOCKED"
    lines.append("R2M1-G3: %s -- warm-start map marker %s, arm state %s"
                 % (status["R2M1_G3"], "present" if mapped else "ABSENT", b1["state"]))

    # -- R2M1-G4, ADMISSION. The only admission gate in this probe, and it is B1's alone.
    if status["R2M1_G3"] != "PASS":
        status["R2M1_G4"] = "BLOCKED"
        lines.append("R2M1-G4: BLOCKED -- the warm start did not exercise, so the last "
                     "untested remedy STILL has not been tested. This is not a GATE FAIL.")
    else:
        ...
        status["R2M1_G4"] = "PASS" if clean else "GATE FAIL"
        lines.append("R2M1-G4: %s -- B1 rc=%s last Time=%s%s"
                     % (status["R2M1_G4"], b1["rc"], b1["facts"]["last_time"],
                        "" if clean else "; failed clauses: %s" % ", ".join(why or ["nan/sigfpe"])))

    # -- R2M1-G5, the 2x2 CELLS. This gate reports four measurements. IT NAMES NO CAUSE.
    cells, missing = [], []
    for name, (energy, transonic) in FACTORIAL.items():
        a = arm[name]
        if a["state"] == "BLOCKED" or not writer_gate(a["facts"]["steps_completed"],
                                                      a["w1"], a["w2"]):
            missing.append(name)
            continue
        cells.append("%s(energy=%s,transonic=%s): abort_at_Time=%s steps=%d trap_site=%s"
                     % (name, energy, transonic, a["facts"]["last_time"],
                        a["facts"]["steps_completed"],
                        "thermo" if a["facts"]["thermo_frame"] > 0 else "NOT-thermo"))
    status["R2M1_G5"] = "PASS" if not missing else "NOT A RESULT"
    lines.append("R2M1-G5: %s -- 2x2 cells measured with a live writer%s"
                 % (status["R2M1_G5"],
                    "" if not missing else "; missing/unwritten: %s" % ", ".join(missing)))
    for cell in cells:
        lines.append("  " + cell)
    lines.append("  NOTE, REGISTERED BEFORE THE RUN: these four cells are MEASUREMENTS. "
                 "This comparator assigns NO cause to any difference between them, and a "
                 "trap site is where an FPE was CAUGHT, not where a bad value was BORN.")

    # -- the item verdict. R2M1-G4 carries it; G0 and the selftest can only turn it into
    #    NOT A RESULT, never the reverse.
    status["R2M1_VERDICT"] = status["R2M1_G4"]
    lines.append("R2M1 VERDICT: %s (carried by R2M1-G4, the admission gate)"
                 % status["R2M1_VERDICT"])
    lines.append("NO FORCE, DRAG, LIFT, MOMENT OR CRM CLAIM IS PRODUCED BY THIS PROBE. "
                 "Rung 2 (a) remains BLOCKED on the three grounds its own registration "
                 "names, binding ground (iii), the absent refinement triple.")
    return lines, status
...
def main(argv):
    ...
    if "--selftest" in argv:
        ...
        print("R2M1-G6: %s -- %d/%d controls fired."
              % ("PASS" if not bad else "NOT A RESULT",
                 len(results) - len(bad), len(results)))
        return 0 if not bad else 2

    if len(argv) < 2:
        print("usage: grade_r2_m1.py <run root> | --selftest")
        return 2
    root = argv[1]
    ...
    try:
        ...
        print("REFUSED before grading: %s" % exc)
        return 2
    bad = [t for t, ok, _ in results if not ok]
    if bad:
    ...
    for line in lines:
        print(line)
    with open(os.path.join(root, "STATUS.R2_M1"), "a") as handle:
