...
prob.model = Top()
prob.setup(mode="rev")
print("A5DOBJDXV === running primal ===", flush=True)
prob.run_model()
obj0_om = float(prob.get_val("OBJ.val")[0])
print("A5DOBJDXV === primal done t=%.1fs OBJ=%.15e ===" % (time.time() - t0, obj0_om), flush=True)

totals = prob.compute_totals(of=["OBJ.val"], wrt=["shapexUpper"])
dOBJ = np.array(totals[("OBJ.val", "shapexUpper")]).flatten()
print("A5DOBJDXV === adjoint done t=%.1fs, captured %d rev call(s) ===" % (time.time() - t0, len(_captures)), flush=True)
print("A5DOBJDXV dOBJ/dshapexUpper = %s" % np.array2string(dOBJ, precision=8, max_line_width=250), flush=True)
if not _captures:
    raise RuntimeError("A5DOBJDXV FAIL: warper rev hook never fired")
w_real = _captures[-1]
print("A5DOBJDXV ||w_real||=%.8e nXv=%d" % (float(np.linalg.norm(w_real)), w_real.size), flush=True)
...
if Xv0.size != w_real.size:
    raise RuntimeError("A5DOBJDXV FAIL: Xv %d vs w_real %d" % (Xv0.size, w_real.size))
...
def obj_at_Xv(Xv, tag):
    ...
    if DASolver.primalFail != 0:
        raise RuntimeError("A5DOBJDXV FAIL: primal failed at perturbed Xv (%s)" % tag)
    ...
    print("A5DOBJDXV_SOLVE %s calcFunction: TP1=%.15e TP2=%.15e OBJ=%.15e | "
...
# NOISE FLOOR: re-solve the IDENTICAL, unperturbed Xv0 repeatedly. Any spread is
# the primal's own irreproducibility -- the noise that every FD below divides by
# 2h. Measured, not assumed. Interleaved with a perturbed solve so the warm-start
# path differs between repeats exactly as it does in the real FD sweep.
reps = []
for k in range(4):
    reps.append(obj_at_Xv(Xv0, "NOISE rep=%d" % k))
    if k < 3:
        obj_at_Xv(Xv0 * 1.0 + 1e-6 * (Xv0 - Xv0.mean()), "NOISE scramble=%d" % k)
reps = np.array(reps)
print("A5NOISEFLOOR n=%d vals=%s spread_max_min=%.6e std=%.6e mean=%.15e openmdao=%.15e"
      % (len(reps), np.array2string(reps, precision=12), reps.max() - reps.min(),
         float(reps.std()), float(reps.mean()), obj0_om), flush=True)
for h in HS:
    print("A5NOISEFLOOR implied_FD_noise h=%.1e = %.6e (spread/(2h))"
          % (h, (reps.max() - reps.min()) / (2.0 * h)), flush=True)
obj0_direct = float(reps[0])
...
for idx in IDXS:
    for h in HS:
        ...
        print("A5DOBJDXV_RESULT idx=%d h=%.1e AN=%.12e FD=%.12e rel_err=%.6e sign=%s "
...
print("A5DOBJDXV_DONE t=%.1fs" % (time.time() - t0), flush=True)
