...
- A cross-case fingerprint: a35's three dominant entries are U2 rows of serial
  cells 338/407/275 — and re-examining A4's own record residual, its rank-3/4/5
  entries are U2 rows of the SAME serial cell ids (338/275/407, |r| =
  4.5/2.2/2.2), interior cells of the shared background mesh, beneath A4's
  dominant U0 pair (42.2). Where the two cases' scotch cuts pass through the
  same background-mesh region, they corrupt the same rows — the defect follows
  the cut, reproducibly.
...
**RESULT (`a4_inletOutlet_scotch.log`, 4.57 core-min): analytic dCD/dshape =
2.406182395247966e-01 — the no-defect class. PREDICTION HELD: the
`freestreamVelocity` farfield BC is the gating edit.** Two corroborating exact
agreements: (i) the papers-protocol arm WITH patchV registered read the same
partition's shape analytic as 2.4062e-01 — identical to this no-patchV arm at
the printed precision, so the input registration is measured inert for the
`dRdW^T` operator, exactly as the tape-structure reasoning said; (ii) CD0
1.5229e-01 differs from the established config's 1.5297e-01 by 0.44% (the BC
genuinely changes the flow slightly) while the analytic sits in the clean class,
9% away from the defect value on the same scotch partition.

**Mechanism implication (stated for the upstream report):** the wrong parallel
reverse-AD operator on A4 REQUIRES the `freestreamVelocity` boundary condition
in the recorded `updateStateBoundaryConditions`; with `inletOutlet` in its
place, the same mesh, same scotch cut, same tape machinery produce a clean
operator. Combined with the localization, the trigger condition sharpens to:
**reverse-AD of the freestream-family BC update, under a decomposition whose
cut geometry excites it, depositing O(1) residual error on interior
partition-interface momentum rows** (on a35, the three dominant cross-residual
cells are interior interface cells, not farfield-adjacent — the BC is a
necessary ingredient of the recorded computation, not the deposit site; two of
the next-rank cells ARE farfield-adjacent). `freestreamVelocity` is a
flux-switching (mixed-type) BC whose valueFraction depends on the face flux;
`inletOutlet` switches too, but on different recorded quantities — the
line-level reverse of these BCs inside the global tape is where an instrumented
rebuild should look first. This finding also explains at one stroke why A1, A2,
A5, CBFS and the sail stay clean at gradient level under every decomposition
tested: **none of them carries a freestream-family farfield BC on U — verified
from the on-disk 0.orig/U of every case this session** (A1: inletOutlet
farfield; A2: inletOutlet; A5: fixedValue inlet, internal flow; CBFS:
fixedValue inlet; sail: inletOutlet), **while exactly the two cases with a
measured operator defect (A4, Ahmed-35) carry `freestreamVelocity`**. The
correlate is perfect across all seven cases measured. A1's np=4-scotch
invariance also shows the BC is necessary-but-not-sufficient territory is
real: no clean case even hosts the necessary ingredient, so cut-geometry
sufficiency remains bracketed only within A4/a35 (np=2 clean, np=3/np=4 scotch
...
6. **The gating ingredient is the `freestreamVelocity` farfield BC (N9,
   ...
   vs 2.2086e-01 defective); the patchVelocity input registration is measured
   inert (2.4062e-01 with and without, printed precision). And the BC survey
   across all seven cases is a perfect correlate, verified from each case's
   on-disk 0.orig/U: the two operator-defect cases (A4, Ahmed-35) carry
   `freestreamVelocity`; the five clean cases (A1, A2, A5, CBFS, sail) carry
   inletOutlet/fixedValue. The trigger condition now reads: **freestream-family
   U BC in the recorded tape (necessary) x a cut geometry that excites the
   reverse halo accumulation (scotch np>=3 worst on these meshes), with the
   damage deposited on interior partition-interface momentum rows (the dominant
   a35 cells are NOT farfield-adjacent) and priced by the objective
   contraction.**
7. **A cross-case structural hint for the mechanism** (n=2, not overclaimed): the
...
analysis, the two crossres container runs, the CBFS FD pair, N8, and the N9 separating control. (2) Four
