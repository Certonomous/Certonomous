...
# PS parameters (refined: roughly 2x surface point density + finer LE clustering vs. original tutorial)
dX1PS = 0.0025  # first dx from the LE (was 0.005)
Alpha1PS = 1.15  # clustering from the LE (was 1.2, slightly finer growth for more LE points)
dX2PS = 1e-3  # first dx from the TE (was 2e-3)
Alpha2PS = 1.2  # clustering from the TE
dXMaxPS = 0.01  # max dx for PS (was 0.02)
# SS parameters
dX1SS = 0.0025  # first dx from the LE (was 0.005)
Alpha1SS = 1.15  # clustering from the LE (was 1.2)
dX2SS = 1e-3  # first dx from the TE (was 2e-3)
Alpha2SS = 1.2  # clustering from the TE
dXMaxSS = 0.01  # max dx for SS (was 0.02)
# TE parameters
NpTE = 5  # number of points for blunt TE
# 3D
NpExtrude = 65  # how many points to extrude for the 3D volume mesh in y (was 33, ~2x)
yWall = 2e-3  # first layer mesh length (was 4e-3, finer near-wall resolution)
