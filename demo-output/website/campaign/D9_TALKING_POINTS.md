# Founder talking points: the uncertainty band that called its shot

Spoken segment. Read the beats in order. Every number below is in the table
so nothing gets misquoted on camera.

## The line to land

Our uncertainty machinery predicted a known NASA model error, in writing,
before the run.

## Beats, in order

- We run a NASA benchmark called the wall mounted hump, a well studied
  separated flow case.
- Our baseline turbulence model over predicts where the flow reattaches.
...
- Tighten the band while it still contains the truth.
- Take the same machinery to another independently documented case and let
  it call its shot again.
