...
| 3.2 | `sdk/chief_engineer/uq.py` L345-351, L362-366 | the B-52 as one of **only two fixtures** calibrating the `extrapolation_sanity` tolerance 0.15 (*"clears the good case by 1.30× ... a factor of 16 below the bad case"*) | **A + ESCALATE** | **This is the most consequential item in the audit and it is not mine to close.** A tolerance separating two fixtures, one of which is now known to be noise-dominated, is calibrated on a 16× gap whose lower anchor may be an artifact. The threshold is probably still fine — the B-52 *should* be rejected — but the *margin* is no longer evidence of anything. **Recommend routing to the UQ/Infra family** alongside the lever-echo referral, with the ask: re-anchor 0.15 on a fixture whose mechanism is established. **Do not change 0.15 on the strength of this arm.** |
