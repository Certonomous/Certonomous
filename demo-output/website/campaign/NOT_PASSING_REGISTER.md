...
- **Root cause:** Mesh rungs still in pre-asymptotic regime; **[WITHDRAWN 2026-08-10 — chief ruling `7abb0ba3`. The B-52 ladder's "turn" is withdrawn as a claim: the published −4.055e-3 is max(rung 6) − min(rung 7) of eight same-recipe draws, and re-estimated from all of them the rung 6→7 increment is +8.2e-5 ± 1.2e-3 (t = 0.071) — 49× smaller, opposite in sign, and smaller than the resolution-mismatch bias. A selected extremum is not a measurement. See `campaign/B52_TURN_WITHDRAWAL_2026-08-10.md`. Original text retained below.]** Re-homed rather than deleted: `campaign/B52_RECIPE_NOTE_BACKGROUND_PRODUCT.md` measures a real stepwise interaction of exactly this kind — in the DELIVERED CELL COUNT, not in Cd.
oscillation suggests possible interaction between nearBody shell level 2 refinement and background blockMesh boundary-layer growth across the refinement sequence.
...
> **[RESTATED 2026-08-10 under `docs/charters/VERIFICATION_CHARTER.md` §17 — no draw-scatter evidence exists at the rung this feature turns on.]** The statement above is a claim about the SHAPE of a sequence of grid-refinement increments for **motorBike**. Under the adopted rule such a claim is published only with draw-scatter evidence at the deciding rung, or with the absence of that evidence stated on its face. **No replicate mesh has ever been drawn at this ladder's deciding rung.** Recipe class per `campaign/LADDER_RECIPE_CONSISTENCY_SWEEP_2026-08-10.md`: **UNDETERMINABLE** — the rung cases no longer exist, so neither the recipe nor the scatter can now be established. **This is not a withdrawal — the feature is unchecked, not shown false**; the remedy the rule specifies is exactly this sentence. Original text retained.
