...
- **Status:** Gate 2 remains FAIL. The follow-up converted an unconfirmed hypothesis into a refuted one plus a supported replacement — this is treated as a completed, understood finding, not an open action item.
- **Evidence:** `/home/ubuntu/Certonomous/demo-output/website/campaign/F9_pulsatile_valve.md` (§5, Gate 2); `demo-output/website/campaign/F9_work/f9_analysis.json`; `demo-output/website/campaign/F9_work/womersley_followup_results.json`.
