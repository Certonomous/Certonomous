...
**The LTS test (single variable, as instructed).** This case's `fvSchemes` uses `ddtSchemes { default localEuler; }` — LTS pseudo-time-stepping to a steady state, where each cell advances at its own locally-Courant-limited pseudo-rate, not a shared physical clock. Built `warmup20_bounded_realtime`, an exact copy with **only** `ddtSchemes` swapped to real time-accurate `Euler`, everything else identical (case preflight passed; foreground-then-backgrounded smoke test, resources checked clean before launch, the two protected long jobs undisturbed throughout). Result, run to `t≈1.21e-05s` (**~8x further in real physical time** than the LTS run's `t≈1.9e-06s`, reached in comparable or less wall-clock time):

| | LTS (`localEuler`), both fixes | Real time-accurate (`Euler`), both fixes |
|---|---|---|
| Physical time reached | 1.94e-06 s | 1.21e-05 s (≈8x further) |
| Bounded-cell fraction at run end | ~9,500 / 29,700 (**32%**), still climbing | ~2,650 / 29,700 (**9%**), plateaued |
| Spatial extent (x) of clamped cells | grew to `x∈[0.0003, 0.198]` — **nearly the whole domain, to the outlet** | plateaued at `x∈[0.0003, 0.087]` — **just past the flare corner (x=0.08), not spreading further** |
| Corner cell (`cell 0`) worst value | frozen to 9 sig figs, `-199551.388` repeated exactly, run after run | **not frozen** — drifts slowly (`-199523.25` → `-199524.7` over the observed window) |

**This substantially confirms the coordinator's hypothesis and requires retracting the strongest earlier claim.** The "spreading, consuming an ever-larger fraction of the domain, not settling" reading — reported as evidence of something fundamentally broken — was measured under LTS, where different cells advance at different local pseudo-rates; comparing "extent at time T" across an LTS run is not the same claim as it would be in a time-accurate run, and it does not reproduce under one. Under a fair, time-accurate reading covering *more* real physical time, the affected region **plateaus at ~9% of the mesh, localised near the flare corner and the inlet/farfield mesh corner**, not the domain-consuming defect reported earlier. **That earlier claim is retracted as stated.** The corner cell's bit-identical, frozen value was also an LTS artifact — under real time it drifts, so "the same value regardless of the fix" was partly an artifact of comparing frozen pseudo-time snapshots, not proof the value is externally imposed.

What survives: bounding is **still** required, on a real, non-trivial, non-single-cell fraction of the mesh (~9%, not 0%) even under the fairer test — this is not yet a clean case, and the earlier "not a single-cell excursion" conclusion is right, just for a smaller number than first reported. Two persistent sites remain, neither yet fully explained: the inlet/farfield corner cell (topology-based hypothesis unconfirmed — no BC value conflict found), and a second site at `x≈0.0003, r≈0.126` (`Y≈2.46cm` from the wall) that only became dominant once the wall-adjacent violation was addressed — sitting inside the already-flat (freestream) tail of the Table II profile, so not obviously a profile-interpolation kink either; cause not yet identified.

**Per the standing rule: the gate case has not moved to the stock solver.** Bounding is still load-bearing under the fairer (real-time) test too (~9% of cells, not settling to zero within the window observed), so the cause is not yet genuinely fixed. Next test, not yet run: extend the real-time-accurate run further to see whether the ~9% plateau continues shrinking toward zero (a slow-healing initial transient, in which case stock is close) or holds at a nonzero floor (in which case the two remaining sites need their own explanation before stock is appropriate).
