...
our margin over the leader is 0.001365, not 0.002878. Every probability below carries the same
...
\code{deb91557} --- \textbf{rank 1 of 5, scored locally}, 0.002878 below Reissmann, Fang \&
Sandberg's published 0.059525, and 45.3\% below the 0.1036 scored by submitting the
...
posterior beside it: \textbf{P(rank 1) = 68\%} over a 400{,}000-draw case-level bootstrap
(an eight-case sample cannot pin that tighter than \textbf{2--100\% at 95\%}), the
lead over the rank-2 entry \textbf{not statistically decided} (paired \(t=-0.495\), \(n=8\))
and the lead over rank 3 likewise (\(t=-0.953\)); the two remaining comparisons are decided on
...
\subsubsection*{The board moved on 2026-08-11, and every probability in this document is struck}
...
\textbf{What does not.} The margin over the leader is \textbf{0.001365}, not the 0.002878
...
\caption{The rank claim with its posterior attached (\artifact{campaign/PROBABILITY\_OF\_RANK\_2026-08-10.md},
400{,}000 case-level bootstrap resamples; the figure is publishable and \textbf{may not be
stated without its interval} --- see the caution below).
\(t\) is the paired per-case difference statistic, \(n=8\).}
\label{tab:prank}
\begin{tabular}{@{}lccl@{}}
\toprule
...
\textbf{no surface may state it without the interval} --- a bare 68\% sounds settled, and
...
\item[Round 5 --- 0.056647 ($\Delta$ $-$0.0088), the entry of record.] A duct-only change
  ...
  board --- at 68\% probability, an eight-case figure no tighter than 2--100\% at 95\%, with
  the lead over the rank-2 entry \textbf{not statistically decided} (Table~\ref{tab:prank}).
\end{description}
...
Table~\ref{tab:prank}: \textbf{P(rank 1) = 68\%}, an estimate eight cases pin no tighter than
\textbf{2--100\% at 95\%}, with the two nearest comparisons
\textbf{not statistically decided}.
...
\item \textbf{The pre-stated expectation band was wrong in our favour, and that is recorded
  ...
  is worth 68\% --- 2--100\% at 95\% on eight cases --- and its margin is
...
the case falls to 3 of 5, behind Reissmann and behind Wu \& Zhang's 0.0350; and our
best-on-board count drops from 5 of 8 to \textbf{4 of 8}. The risk-side signal was right
...
are the entry of record; round 2 is omitted for width and appears in \S3.1. The overall
row's rank carries \textbf{P(rank 1) = 68\%} (2--100\% at 95\%) and a lead over the published
rank-1 entry that is \textbf{not statistically decided} (Table~\ref{tab:prank}); the two
\code{Ret\_360} rows
...
\midrule
\code{alpha\_15\_13929\_4048} & 0.1320 & 0.0501 & 0.0501 & \textbf{0.0501} & 0.0592 & 0.0592 & \textbf{our model leads} \\
\code{alpha\_15\_13929\_2024} & 0.2049 & 0.1011 & 0.1011 & \textbf{0.1011} & 0.1339 & 0.1195 & \textbf{our model leads} \\
\code{alpha\_05\_4071\_4048} & 0.0461 & 0.0461 & 0.0461 & \textbf{0.0461} & 0.0606 & 0.0569 & \emph{baseline, declined} \\
\code{alpha\_05\_4071\_2024} & 0.0719 & 0.0719 & 0.0719 & \textbf{0.0719} & 0.0760 & 0.0760 & \emph{baseline, declined} \\
\code{AR\_1\_Ret\_360}       & 0.1288 & 0.0919 & 0.0811 & \textbf{0.0455} & 0.0387 & 0.0387 & 2nd of 5; ties rank 2 at published precision \\
\code{AR\_3\_Ret\_360}       & 0.1243 & 0.0862 & 0.0775 & \textbf{0.0400} & 0.0341 & 0.0341 & 3rd of 5, within published-precision noise of rank 2 \\
\code{AR\_14\_Ret\_180}      & 0.0590 & 0.0303 & 0.0325 & \textbf{0.0353} & 0.0325 & 0.0325 & 3rd of 5; the 0.00003 lead was priced and lost \\
\code{NASA\_2DWMH}           & 0.0621 & 0.0632 & 0.0632 & \textbf{0.0632} & 0.0412 & 0.0364 & behind, last on board \\
\midrule
\textbf{Overall} & \textbf{0.1036} & \textbf{0.0676} & \textbf{0.0654} & \textbf{0.056647} & \textbf{0.0595} & --- & rank 1 of 5, scored locally; P 68\% (2--100\% at 95\%) \\
\bottomrule
\end{tabular}}
\end{table}
...
Arithmetically we hold the best score on the public board on four of eight cases --- five
until round 5 spent one of them (\S\ref{sec:ar14}). That count was on our public page as a
...
\centering
\small
\caption{Attribution of the eight test cases at round 5, replacing the retired ``5 of 8''
...
Behind & 4 & \code{AR\_1\_Ret\_360}, \code{AR\_3\_Ret\_360}, \code{AR\_14\_Ret\_180}, \code{NASA\_2DWMH} \\
\bottomrule
\end{tabular}
\end{table}

\noindent The uncomfortable reading of that table is the one to state: \textbf{our overall
number is now first on the board while our own model holds a clear lead on two cases out of
eight.} The rank comes from having no bad case rather than from having many best ones ---
two declined baselines that beat every published entry, two ducts brought to the level of
...
\item \textbf{``4 of 8'' replaced ``5 of 8''} on the best-on-board count, by losing a row
  ...
  margin warning is left in place rather than deleted.
...
Table~\ref{tab:prank}: the 400{,}000-draw case-level bootstrap, the four intervals in order of
increasing honesty (Monte Carlo, leave-one-out, double bootstrap at 68\% and at 95\%), and the
head banner withdrawing the internal-only gate \\
...
finding that withdrew the gate \\
