...
probability is \textbf{P(rank 1) = 68\%}, with the lead over the rank-2 entry
...
posterior beside it: \textbf{P(rank 1) = 68\%} over a 400{,}000-draw case-level bootstrap, the
...
\textbf{P(rank 1) = 68\%}, and the two rows immediately below ours are
\textbf{not statistically decided} against it (Table~\ref{tab:prank}).}
\label{tab:board}
\begin{tabular}{clc}
\toprule
Rank & Entry & Overall \\
...
\textbf{P(rank 1) = 68\%} (67.6\%). The rest of the distribution is not a rounding error:
...
400{,}000 case-level bootstrap resamples; \textbf{internal figure} --- see the caution below).
...
\textbf{P(rank 1) = 68\%}; the leads over Reissmann and over Wu \& Zhang are
\textbf{not statistically decided} (Table~\ref{tab:prank}). \emph{That figure is an
internal one} --- it exists for this laboratory to reason with, is not to be quoted outside it,
attached to any package, or cited in any submission-facing document, and it does not travel to
any public surface derived from this report.
...
\item[Round 5 --- 0.056647 ($\Delta$ $-$0.0088), the entry of record.] A duct-only change
  ...
  board --- at 68\% probability, with the lead over the rank-2 entry
...
Table~\ref{tab:prank}: \textbf{P(rank 1) = 68\%}, with the two nearest comparisons
...
\item \textbf{The pre-stated expectation band was wrong in our favour, and that is recorded
  ...
  is worth 68\% and its margin is \textbf{not statistically decided}
  (Table~\ref{tab:prank}). The result landed at 0.056647, \emph{below the good
...
row's rank carries \textbf{P(rank 1) = 68\%} and a lead over the published rank-1 entry that
is \textbf{not statistically decided} (Table~\ref{tab:prank}); the two \code{Ret\_360} rows
...
\midrule
\textbf{Overall} & \textbf{0.1036} & \textbf{0.0676} & \textbf{0.0654} & \textbf{0.056647} & \textbf{0.0595} & --- & rank 1 of 5, scored locally; P 68\% \\
\bottomrule
\end{tabular}}
\end{table}
