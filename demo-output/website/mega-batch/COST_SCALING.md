...
**And 81 percent of its rows record a cost of exactly zero.** 56,190 of its
69,007 clean rows carry `wall_seconds` of exactly 0.0, against 12,817 carrying
a positive value up to 0.432 s. **27 percent of the entire ledger is rows with
a zero cost**, and any throughput figure that divides work by row count, rather
than by measured seconds, is counting them.

### 6a. Correction, 2026-08-01: the zeros are the stored precision

The paragraph above used to read that this "is not timer resolution rounding a
...
one of which records nothing." **That was wrong on both halves, and it is
withdrawn.** It was inferred from the range of the positive values; nobody had
looked at their mass, at the code that writes the field, or at whether the zero
rows produced results. Escalation #2 forced all three. Measured on 2026-08-01
against `demo-output/website/mega-batch/ledger.jsonl` (208,193 rows parsed, 91
failed, 1 torn line, spanning 2026-07-23T03:53:37Z to 2026-07-29T11:17:07Z):

* **The positive values are piled just above zero.** Of the 12,817 non-zero
  reduced-order rows, 8,300 are exactly 0.001 s, 3,279 are 0.002 and 733 are
  0.003 — **96.1 percent are at or below 0.003 s**, median 0.001, p99 0.006.
  Only 15 rows reach 0.1 s. The 0.432 s maximum is the tail, not the shape.
* **The smallest non-zero wall time anywhere in the ledger is exactly 0.001 s**,
  in every one of the six families. Nothing is ever stored between 0 and 0.001,
  which is what a rounding boundary looks like and what a genuine continuum does
  not.
* **There is no second path.** `run_task` in `sdk/workflows/mega_batch.py` starts
  one timer at line 719 before it dispatches, and writes
  `record["wall_seconds"] = round(time.time() - start, 3)` at line 746, on one
  line, for every solver, with no branch. A row whose true duration is under
  0.0005 s stores as 0.0 by that `round`, and there is no other way for the field
  to become zero.
* **The model costs less than that boundary.** Re-running the shipped ROM
  (`_run_valve`) on 600 sampled rows' own designs on 2026-08-01: median
  0.000323 s, p95 0.000339 s, and 598 of 600 replays finish under the 0.0005 s
  boundary.
* **The zero rows ran.** 400 of 400 sampled zero-wall rows reproduce their
  recorded `metrics` **exactly** — every field, to the stored decimal — when the
  ROM is re-run on that row's own design. So do 200 of 200 sampled non-zero
  rows. These are evaluations, not placeholders and not backfill.
* **The zero share tracks host load, in lockstep with the real solves that share
  its index rota.** On 2026-07-23 the box was busy: `openfoam-cylinder` averaged
  8.85 s a row and only 25.6 percent of reduced-order rows rounded to zero. From
  2026-07-26 the box was about 3.6x faster (`openfoam-cylinder` 1.99 s a row) and
  99.8 percent rounded to zero. The same arithmetic crossed the same boundary.
  Today's code reproduces both eras' metrics exactly, so the ROM itself never
  changed.

**What this costs the published numbers.** The 212.28 cleaned core-hours (239.259
gross) is *not* unaffected by construction — `ledger_summary` in
`sdk/chief_engineer/lab_stats.py` computes core-hours as the sum of these same
`wall_seconds` over 3600, so a zeroed field feeds it directly. It is unaffected
in magnitude, and now bounded: each zero row is under 0.0005 s by definition of
the `round`, so all 56,190 together hide **at most 28.1 s = 0.008 core-hours,
0.003 percent** of the total. The whole reduced-order family contributes 24.11
recorded seconds, 0.0028 percent of the 861,333.2 s of ok wall time.

**And the 208,102 evaluation headline stands.** Every zero row is a real
evaluation that produced a reproducible result; what it lacks is a duration
above the stored millisecond, not a duration. The count is measured. What must
not be claimed is a *measured cost* for those 56,190 rows: their recorded 0.000 s
is a floor. The credentials wall now carries that basis in its own footer
(`demo-output/website/wall/wall.html`, generated by
