...
| **C9** | AR_14 lead of 0.00003 | round-4 record | **PASS.** ~~Swept the corpus for "comfortab*": every occurrence in the closure line is the **prohibition** ("must not be reported as a comfortable win").~~ No surface claims a comfortable AR_14 lead. §0f, `PRODUCT_LIST` §4B and `ACTIVE_RESEARCH` all record the tie as **lost**. *[**Evidence corrected 2026-08-11** by the V6/V10/V14 closure pass, discharging V15 finding F9. **The verdict stands and the struck sentence was false.** Re-swept with `/bin/grep -rniI "comfortab" --exclude-dir=.git .` (frame: the whole working tree as text, `/bin/grep` not the ignore-honouring wrapper) — **76 hit lines tree-wide, 59 under `demo-output/website/`, 15 in the closure line** excluding this ladder's own reports. **Four are the prohibition**, not "every occurrence": `closure_challenge_trained_entry_round4_duct.json:88`, `CLOSURE_CHALLENGE_STATUS.md:394` and `:639`, `latex/closure_challenge_report.tex:864`; a fifth, `LADDER_V_TRIPLE_VERIFICATION.md:46`, restates the banned-list rule. **The remaining ten are ordinary uses**, and one of them is exactly what the claim denied existed: `campaign/PROBABILITY_OF_RANK_2026-08-10.md:151` — *"Larger and more comfortable than the Reissmann comparison"* — **a comfort descriptor applied to a margin**, in a file this pass edited in the same commit (the sentence does end *"still short of anything a statistician would call settled"*). The other nine are not win claims at all: `CLOSURE_RANK1_CAMPAIGN.md:341` calls an assumption *"the comfortable reading"* in a sentence **refusing** it; `closure_challenge_criterion_test_case_table.json:126` and `CLOSURE_CHALLENGE_STATUS.md:259, :261` describe cases sitting inside a **training regime**; `closure.html:181` and `report.tex:825, :905` read *"uncomfortable"*, all three concessions; `report.tex:754` says a result is *"not a reason to feel comfortable"*; `CLOSURE_METHOD_PRIORITY_REVIEW.md:242` is a prior-art verdict, *"PRIOR-EXISTS, comfortably"*. **What the sweep supports is the narrower claim, which is the one C9 needed**: no surface describes the AR_14 lead, or any lead, as a comfortable **win**. **Positive control, which the original sweep did not run**: a scratch file reading *"Our AR_14_Ret_180 lead of 0.00003 is a comfortable win over Reissmann"* was seeded under `demo-output/website/`, the sweep returned it, and the file was deleted with `git status` left clean. Provenance checked rather than assumed: the `PROBABILITY_OF_RANK` sentence entered at `b2aa6887` (2026-08-10 16:00) and the `CLOSURE_RANK1_CAMPAIGN` one at `0afd7e61` (2026-08-05 17:33), **both before this ladder** — so neither sentence is ladder-authored, and only this row's description of them was.]*
