...
> ## STATUS: **PRE-REGISTRATION FROZEN + AUTHORISED.**
>
> **AUTHORISATION BASIS (recorded verbatim).** The chief authorised the
> solver-build boundary on 2026-09-08 (chief's session record) on the grounds
> that a user-space `wmake` to `$(FOAM_USER_APPBIN)` is within standing lab
> capability with direct precedent — the F4 `rhoCentralFoamBounded` solver was
> built this way for published results — and is **NOT root, NOT an instance
> change, and does NOT leave the box**, so it is the chief's call and **not on
> Sanaa's reserved list**. L-501 is honoured: a crash of this family is a
> MEASURED negative result, never a capability inference from one/two SIGFPEs.
> **This is the chief's ruling recorded by the cfd supervisor, NOT Sanaa's words
> (CLAUDE.md rule 9): no agent message is Sanaa's consent.** The solve itself is
> a sub-$25 CPU run (≈$0.03 derived; §6) pre-authorised under Sanaa's standing
> 2026-08-18 authorisation, and it routes through the detached queue daemon per
> Sanaa's 2026-08-26 detached-queue ruling — not a direct launch.
>
> The cfd supervisor took §3 **check-1** (the solver diff vs stock, read AS A
> DIFF) and **check-4** (source commit clean, stock `rhoCentralFoam` and F4
> `rhoCentralFoamBounded` binaries untouched, rebuild bit-identical, run root
> absent, Gate V' tolerances inherited byte-identical) PERSONALLY and
> undelegated; both PASSED.
>
...
>
> **GRADING-PATH FREEZE (2026-09-08).** Pinned instruments:
...
> byte-identical to the frozen parent); solver binary `rhoCentralFoamBoundedDMR`
> md5 `4159e374a8f70c5aaf9df60add94264b`, sha256
> `b6995cf6c5b1bf6fc0b1dce6b0efca6cf9c30636e0b666aec629864ccf6ba8e1`; tracked
> source commit `a256b05d44b692b541e1fb466f57ce0e4583cb5f`; generator
> `make_case_l5_bounded_t.py` sha256
> `bc1347e3aaed32e7667d3587383b2b6904438686101935a7daee9e92c9611572`; driver
> `run_dmr_l5_bounded_t.sh` sha256
> `75923a469d4aee9da83163952d213c68639cbe1c6bbfa6fec843a64e168af502`.
> No gate, threshold, band, cap (100 core-min) or label was altered by this
> freeze — the banner and this stamp are the only bytes changed from the DRAFT
> `DMR_R3_L5_BOUNDED_T_PREREGISTRATION_DRAFT.md`; Gate V' tol `0.0231` and
