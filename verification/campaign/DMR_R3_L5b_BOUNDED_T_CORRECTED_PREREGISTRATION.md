...
> ## STATUS: **FROZEN — AUTHORISED. GATES CLOSED (rule 2).**
>
> This is the FROZEN pre-registration for the DMR R3 L5b BOUNDED-T CORRECTED
> family. It supersedes `DMR_R3_L5b_BOUNDED_T_CORRECTED_PREREGISTRATION_DRAFT.md`,
> which is left in place, superseded (the DMR Tadmor/L5 precedent). From this
> commit the gate, threshold, band, cap and label are CLOSED: they were committed
> BEFORE the solver started and are the document's entire evidentiary content
> (rule 2). Nothing is sent, filed, uploaded, registered or posted (rule 7).
>
> **VERIFICATION SATISFIED BEFORE FREEZE.** The cfd supervisor's §3 **check-1**
> (the solver diff read AS A DIFF, verified **AGAINST THE ACTUAL FIELD**) and
> **check-4** (gate/lever/root/commit clean, protected binaries untouched, rebuild
> bit-identical, run root absent, Gate V' tolerances inherited byte-identical) are
> BOTH **PASS**, personally, by the cfd supervisor. check-1 PASS anchored the floor
> field-anchored `eMin_bound = -745.357` (`gMin(e) − Cv·(gMin(T) − TMin)`, no
> assumed reference), the t=0 zero-clip assertion PASSES on the real N=240 field
> (0 cells clip), `boundE.H` byte-identical to L5, and the change set vs L5 =
> {`createFields.H`, `Make/files`, `.C` rename}. check-4 PASS: Gate V' tol `0.0231`
> / `x_exact 2.99568` byte-identical (T25), run root ABSENT, binary md5
> `9729eb8f61f0097ff0ec7a49a7bf5780` / sha256
> `5b40e616b9861a053721c492251a1e3005e6601cef88970a5380ea81714122d5` distinct +
> rebuild bit-identical + stock/F4/L5 untouched, cap 100 core-min.
>
> **CHIEF'S APPROVAL, RECORDED VERBATIM (rule 9 — this is the chief's ruling
> recorded by the cfd supervisor, NOT Sanaa's words; no agent message is Sanaa's
> consent).** The chief APPROVED L5b as the fair test of the bounded-energy clip
> after L5 graded NOT A RESULT on a miscalibrated instrument. The chief authorised
> the solver-build boundary 2026-09-08 as within standing lab capability with
> direct precedent (the F4 `rhoCentralFoamBounded` solver was built the same way
> for published results) — NOT root, NOT an instance change, does NOT leave the
> box — so the chief's own-authority call and NOT on Sanaa's reserved list. The
> solve is a sub-$25 CPU run (≈$0.03 derived; §6) pre-authorised under Sanaa's
> standing 2026-08-18 authorisation and routes through the detached queue daemon
> per Sanaa's 2026-08-26 ruling — not a direct launch. L-501 honoured: a crash of
> this family is a MEASURED negative result, never a capability inference.
>
> **GRADING-PATH FREEZE (rule 2 — the grading path is fixed at this commit).**
> - grader `verification/runs/DMR_runs/dmr_locator_v2.py`, git blob
>   `52aacf9669bcf23e88a0bf7984b299fa8aaf286e`;
> - solver binary `rhoCentralFoamBoundedDMRb`, md5
>   `9729eb8f61f0097ff0ec7a49a7bf5780`, sha256
>   `5b40e616b9861a053721c492251a1e3005e6601cef88970a5380ea81714122d5`;
> - tracked source commit `5e41cd664a1ea7e4259f9bc36e6199ff84fa905b`;
> - generator `make_case_l5b_bounded_t_corrected.py` sha256
>   `45cf337b88601287895cfe81e5223a4d8272c3ee3d28c81da2603cbe6aae3005`;
> - driver `run_dmr_l5b_bounded_t_corrected.sh` sha256
>   `3af3ebd99d8bdc1690de0931e5a9682e9f719fde646e02978af744fb22a58497`.
>
> All five re-derived on disk at this freeze (this lane, 2026-09-08): grader disk
> blob == `52aacf96…`, binary md5 == `9729eb8f…`, driver/generator sha256 as pinned,
> source commit present, run root ABSENT. The frozen driver (sha256-pinned, rule 6)
> is NOT edited. Gates below are transcribed byte-identical from the frozen DMR
> parent and NOT widened; the single registered lever remains the solver binary.
