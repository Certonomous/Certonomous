...
|---|---|---|---|
| `demo-output/website/benchmarks.html` (path corrected 2026-08-14 at `13b965dd`: ~~`demo-output/benchmarks.html`~~, a path that has never existed in this repo at any commit; the page carrying the sentence is the one now cited, and the claim about it is unchanged) | "a converged forward solve of the untrained QCR2000 physics term — nothing fitted to anything" ×2 | **a PUBLIC page.** `git log -S` shows this sentence was written by **`49f71b8c`** — the sibling Ladder V **V10** rung, 2026-08-08T02:08:13, i.e. **13 minutes before the V5 sweep committed** | **NEW since the 08-08 sweep's scope** |
