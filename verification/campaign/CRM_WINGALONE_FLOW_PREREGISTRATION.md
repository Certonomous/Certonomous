...
## ADDENDUM 2 — 2026-09-12 — **ARM P1C REGISTERED. THE INSTRUMENT MEASURED A DIFFERENT QUANTITY FROM THE ONE §2 REGISTERS. NO GATE, THRESHOLD, CAP OR LABEL IS ALTERED.**

**lines whose number changed above this section: 0.** §2's counts 11,136 / 11,136 / 14,144 are untouched; §2's thresholds **|r| ≤ 4.3** and **|r| ≥ 80** are untouched; §3–§9 are untouched; §10.2's signature, §10.4's check-1 finding and ADDENDUM 1 are untouched.

**CHECK 4 DISCHARGED PERSONALLY BY THE cfd-supervisor on this commit**, and the finding verified on my own read rather than relayed: `CRM_WINGALONE_FLOW_PREREGISTRATION.md` line 78 (HEAD blob) registers the wing assertion as *"within |r| ≤ 4.3 mesh-units **of the body axis**"*, while `verification/runs/CRM_WINGALONE_runs/P1/split_patches.py:31` computes `r = (cx*cx + cy*cy + cz*cz) ** 0.5` — **distance from the ORIGIN**. The instrument does not implement the registered quantity. **P1 and P1B reported honestly against the instrument they were given, twice; the instrument measured the wrong thing.**
...
*Arm P1C registered 2026-09-12 by a cfd `lab-lane`; check 4 signed by the cfd-supervisor on this commit. No compute has run under this addendum. Submissions parked. No agent's message is Sanaa's consent.*
