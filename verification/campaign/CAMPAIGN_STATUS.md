...
- **Verdict:** ~~**GATE REACHED.** Textbook signature of mesh-diffusion smearing on coarse grid (399,360 cells), not gross solver defect.~~ **STRUCK.** → **`NOT A RESULT`.** CD=0.02299556, CL=0.31311589 stand. **MOVED TO `NOT A RESULT` 2026-08-25 — ruling of the cfd supervisor on measured provenance, and the struck verdict above is preserved, not rewritten.** The comparison this cell reports is produced by `cases/dafoam/ladder-a/logs_A3/compare_cp.py` + `extract_cp.py` (tied to it by four exact figures: upper RMS 0.0491–0.1139, lower 0.0128–0.0265, η=0.99 RMS 0.1139 bias +0.0652, η=0.80 shift −0.00148). **BOTH SIDES OF THAT COMPARISON DEPEND ON ONE DOCUMENT THIS LAB DOES NOT HOLD:** `compare_cp.py:33-34` hard-codes the section→η mapping *“(Destarac & Dumont, NASA TMR ONERA_M6_Test_Case_TMR.pdf, Table 3)”* and `extract_cp.py:11-12` takes `b_semi = 1.1963` from the same source to place the seven cutting planes. Searched over all of `/home/ubuntu`: `ONERA_M6_Test_Case_TMR` **0**, `Destarac` **0**, `Dumont` **0**, under a **planted control** in which the same reader returns `case_2308` → **3**. The held artifact `case_2308.dat` carries **no spanwise coordinate at all**, so **without those imported values the comparison cannot be formed** — rule 15 title-page verification of the source is impossible, and rule 5's one-way door permits exactly this direction. **THIS SAYS NOTHING AGAINST THE SOLVE:** CD 0.02299556, CL 0.31311589 and every log stand exactly as recorded; it voids the **comparison built on them**. One import is corroborated on this box — `b_semi` against **1.19676 m measured independently from the registered STL, 0.039 %** — but **the seven η values have no check here and cannot have one**, and that is the load-bearing half. Ground: `verification/campaign/F1_CP_PROVENANCE_2026-08-25.md`.
...
> **[CORRECTED 2026-08-10, same day, before any compute was spent on it — the restatement above is WRONG for this ladder.]** Mesh-draw scatter is a property of a **nondeterministic** mesh generator: snappyHexMesh's castellation is nonlinear in the background lattice, so two meshes built to one recipe differ, which is what makes a *draw* a meaningful sample. **This ladder is generated by a deterministic structured `blockMesh` script** — the same parameters produce a byte-identical mesh every time. **There is no second draw to take, and the quantity the restatement says was never measured cannot exist for this generator.** The rule is satisfied here by stating that, not by measuring. See `docs/charters/VERIFICATION_CHARTER.md` §17's generator-class scope, added on this finding. Original restatement retained.
...
|---|---|---|---|---|---|---|---|---|---|---|
| **F1** | ONERA M6 | M=0.84, Re~1.5e7 | Transonic shock | 3D | Steady | Feasibility→Physics→**Gate** | Cp distribution (AGARD AR-138) | RMS 0.049–0.114, shock ±0.02–0.10 x/c | **`NOT A RESULT`** (was ~~GATE REACHED~~, moved 2026-08-25 — the reference values are not held on this box; see §F1 and `F1_CP_PROVENANCE_2026-08-25.md`. The SOLVE is untouched.) | 127.5 |
