#!/usr/bin/env python3
"""TADMOR-FLUX SUCCESSOR generator (DMR R3 Tadmor-flux successor pre-registration).

Derived from make_case_successor.py (the positivity successor generator, its
parent); generated case OUTPUT is byte-identical to that parent EXCEPT the SINGLE
registered lever of the Tadmor-flux family:
  * system/fvSchemes  fluxScheme Kurganov -> Tadmor

Nothing else changes.  Minmod / MinmodV reconstruction, maxCo 0.1, Euler ddt,
...
The one-word change is PROVED to move exactly one line of the generated tree by
diffing this generator's N=60 output against make_case_successor.py's N=60 output
(see run_dmr_tadmor_successor.sh selftest / the pre-registration §5 assertion).
