#!/usr/bin/env python3
"""POSITIVITY-LIMITED SUCCESSOR generator (DMR R3 positivity successor pre-registration).

Derived from make_case.py (its parent); generated case OUTPUT is byte-identical
to the parent EXCEPT the registered levers of the positivity-limited family:
  * reconstruct(rho) vanLeer  -> Minmod
  * reconstruct(U)   vanLeerV -> MinmodV
  * reconstruct(T)   vanLeer  -> Minmod
  * controlDict maxCo 0.2     -> 0.1
Nothing else changes: states, thermo, mesh, boundary conditions, ranks, write
times and end time are the parent's.  Kurganov flux, Euler ddt and Gauss-linear
div(tauMC) are UNCHANGED (the `Gauss Minmod` energy/velocity div lever is a
NON-registered fallback, not part of this freeze).
