#!/usr/bin/env python3
"""BOUNDED-T (L5b) generator -- DMR R3 L5b CORRECTED positivity-preserving clip.

Derived from make_case_l5_bounded_t.py (its immediate parent). Generated case
OUTPUT is byte-identical to that L5 generator EXCEPT the TWO controlDict values the
L5b family requires:
  * application  rhoCentralFoamBoundedDMR  ->  rhoCentralFoamBoundedDMRb  (the
    CORRECTED, field-anchored solver)
  * TMin 1e-3  ->  TMin 1e-2   (1% of ambient T=1.0; a pure positivity floor)

Nothing else changes. The NUMERICS are byte-identical to L4/L5: fluxScheme Tadmor,
...
thermo, mesh, ranks (4), write times and end time.
...
e-formula e = Cv*(T-298.15) (assumed Tref=298.15/eref=0); OpenFOAM's ACTUAL hConst
ambient e ~ -743.589, so the floor sat ABOVE the physical field and clipped 100% of
cells from the first step -> NOT A RESULT (DMR_R3_L5_BOUNDED_T_RESULTS.md). L5b's
solver re-derives eMin_bound = e_min_initial - Cv*(T_min_initial - TMin) anchored on
the MEASURED initial e/T field (NO assumed reference), so the floor sits below the
physical field by construction, and a MANDATORY t=0 zero-clip startup assertion
REFUSES (exit non-zero) if the floor is ever miscalibrated above the field. See
verification/runs/DMR_runs/rhoCentralFoamBoundedDMRb_src/createFields.H.

Generate a double-Mach-reflection case per DMR_PREREGISTRATION.md and the L5b prereg.

Usage: make_case_l5b_bounded_t_corrected.py <casedir> <N>   (N = cells/unit length)
...
application     rhoCentralFoamBoundedDMRb;
startFrom       startTime;
startTime       0;
stopAt          endTime;
endTime         0.2;
