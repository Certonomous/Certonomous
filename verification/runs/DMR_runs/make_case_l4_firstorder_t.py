#!/usr/bin/env python3
"""FIRST-ORDER-T (L4) generator (DMR R3 L4 first-order-T-reconstruction successor).

Derived from make_case_tadmor.py (the Tadmor-flux successor generator, its parent,
sha256 f08ddd6884f16ef89146723ba2ae79ba8efff639935b0a0dc1ac5b29b23bb0bc); generated
case OUTPUT is byte-identical to that parent EXCEPT the SINGLE registered lever of
the L4 family:
  * system/fvSchemes  reconstruct(T)  Minmod -> reconstruct(T)  upwind

Nothing else changes.  fluxScheme Tadmor RETAINED, maxCo 0.1 RETAINED,
reconstruct(rho) Minmod and reconstruct(U) MinmodV RETAINED, Euler ddt,
Gauss-linear div(tauMC), all boundary conditions, states, thermo, mesh, ranks,
write times and end time are the Tadmor successor's (and hence the parent DMR's).

WHY upwind IS FIRST-ORDER / MOST-DIFFUSIVE T RECONSTRUCTION (v2606, verified in
source).  In rhoCentralFoam the reconstruct(T) scheme governs the +/- face
interpolation of rPsi, e AND the cell-centre sound speed c
(rhoCentralFoam.C:109-113, 140, 146 -- all keyed on T.name()); this is the exact
positivity-critical path whose T undershoot below zero at the reflecting-wall foot
(t=0.150) is the measured mechanism.  `Foam::upwind<Type>`
(src/.../surfaceInterpolation/limitedSchemes/upwind/upwind.H) is a
limitedSurfaceInterpolationScheme -- the SAME base class and construction path as
Minmod -- whose limiter() returns ZERO and whose weights() return pos0(faceFlux):
the reconstructed face value is exactly the upwind CELL-CENTRE value
(piecewise-constant, no reconstructed slope).  That is the most-diffusive,
first-order T reconstruction, entirely in-solver: NO solver build.

The one-line change is PROVED to move exactly one line of the generated tree by
diffing this generator's N=60 output against make_case_tadmor.py's N=60 output
(see run_dmr_l4_firstorder_t.sh selftest / the pre-registration §5 assertion).

Generate a double-Mach-reflection rhoCentralFoam case per DMR_PREREGISTRATION.md.

Usage: make_case_l4_firstorder_t.py <casedir> <N>   (N = cells per unit length, e.g. 120)
