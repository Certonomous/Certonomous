#!/usr/bin/env python3
"""TADMOR-FLUX + maxCo-0.05 COURANT PROBE generator (DMR R3 L2 Courant probe).

Derived from make_case_tadmor.py (the Tadmor-flux family generator, sha256
f08ddd6884f16ef89146723ba2ae79ba8efff639935b0a0dc1ac5b29b23bb0bc); generated case
OUTPUT is byte-identical to that parent EXCEPT the SINGLE registered lever of the
L2 Courant probe:
  * system/controlDict  maxCo 0.1 -> 0.05

Nothing else changes.  Minmod / MinmodV reconstruction, Tadmor flux, Euler ddt,
Gauss-linear div(tauMC), all boundary conditions, states, thermo, mesh, ranks,
write times and end time are the positivity successor's (and hence the parent
DMR's).  Tadmor is confirmed a valid v2606 rhoCentralFoam fluxScheme
(readFluxScheme.H accepts only "Tadmor" or "Kurganov"); rhoCentralFoam.C:167 sets
the interface diffusion aSf = -0.5*amaxSf for Tadmor -- a symmetric split on the
single local max wave speed, strictly MORE numerical diffusion at the interface
than Kurganov's wave-speed-weighted split.

The single-line change is PROVED to move exactly one line of the generated tree by
diffing this generator's N=240 output against make_case_tadmor.py's N=240 output
(see run_dmr_l2_courant_probe.sh selftest / the pre-registration §4/§9 assertion):
the two trees differ by EXACTLY `maxCo 0.1;` -> `maxCo 0.05;` in system/controlDict.

Generate a double-Mach-reflection rhoCentralFoam case per DMR_PREREGISTRATION.md.

Usage: make_case_tadmor.py <casedir> <N>   (N = cells per unit length, e.g. 120)
