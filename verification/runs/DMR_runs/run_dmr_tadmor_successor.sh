#!/bin/bash
# DMR TADMOR-FLUX SUCCESSOR driver -- runs the fresh, self-contained three-level
# family R1t/R2t/R3t per
#   verification/campaign/DMR_R3_TADMOR_SUCCESSOR_PREREGISTRATION.md.
#
# This mirrors run_dmr_positivity_successor.sh's PROVEN idioms EXACTLY -- rc
# captured inside the wrapper at every step, ONE registered hard core-min cap that
# STOPS the run and writes a CAP_BREACH file (no new budget), the rule-4 ABSENT
# guard per level, and a per-level grade-path-hashed grading pass -- and changes
# only what the Tadmor family requires: the TADMOR generator make_case_tadmor.py,
# fresh Tadmor run roots, and the R1t/R2t/R3t labels.  The SINGLE registered lever
# vs the positivity successor is fvSchemes fluxScheme Kurganov->Tadmor, applied
...
# hard cap: total 60.0 core-min.  The per-level §6 figures (R1t ~0.85, R2t ~4.3,
# R3t ~32, overhead ~1.0; STEP 0 ~3 is a SEPARATE probe under its own cap) are
# MEASURED-ANCHORED ESTIMATES, NOT registered per-level caps.  Per rule 2/rule 12
# (do NOT invent a cap) this driver enforces the ONE registered hard cap via a
# SINGLE accumulator USED_CORESEC carried across ALL steps of ALL THREE levels.
# The §6 per-level estimates are printed as ADVISORY watermarks for rule-12
# calibration only.  core-min = wall_s * ranks / 60, accumulated per step with
# that step's OWN rank count -- serial steps count 1 rank, the solve counts 4.
#
...
# An R3t SIGFPE (rc 136) is a MEASURED negative result (Tadmor insufficient),
# recorded not softened -- under fix-until-runs it CONTINUES the ladder as a
# further dated successor, never a capability finding from one/two SIGFPEs (L-501).
#
...
# method-agnostic and grades the Tadmor family unchanged.

R=/home/ubuntu/Certonomous/verification/runs/DMR_R3_TADMOR_SUCCESSOR_runs
GEN=/home/ubuntu/Certonomous/verification/runs/DMR_runs/make_case_tadmor.py
...
# make_case_tadmor.py builds round(N/6) + round((4-1/6)*N) by N, i.e. (4N) x N:
# N=60 -> 240x60, N=120 -> 480x120, N=240 -> 960x240.
LVLS=(R1t R2t R3t)
NS=(60 120 240)
# ADVISORY ONLY -- §6 per-level core-min ESTIMATES, NOT caps.
EST_COREMIN=(0.85 4.3 32.0)
...
breach() {
  echo "CAP BREACH: ${USED_CORESEC}s core-seconds > ${CAP_CORESEC} (${CAP_COREMIN} core-min total)" \
       "-- TADMOR SUCCESSOR STOPPED at level '${LVL}' step '$1', no new budget" \
...
for i in 0 1 2; do
  ...
  # ---- generate the changed-method case via the TADMOR generator -----------
  python3 "$GEN" "$CASE" "$N" > "$R/log.makeCase_${LVL}" 2>&1 || {
    echo "ABORT: make_case_tadmor.py failed for ${LVL} (N=$N)" | tee "$R/${LVL}_REFUSED.txt"
...
sync
echo "TADMOR SUCCESSOR COMPLETE: used ${USED_CORESEC} core-seconds = $(echo "scale=2; $USED_CORESEC/60" | bc) core-min against the ${CAP_COREMIN} core-min total cap" \
  | tee -a "$R/PROGRESS.txt"
