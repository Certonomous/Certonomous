...
def run_case(case_dir, family, pair, level, instrumented=True):
    ...
    if family == "wedge":
        ...
        # UNIQUE-SELECTOR REPAIR of site run_f3s.py:255.  The glob is REPLACED by
        # os.listdir + the token test (sec.5.1): that fixes the dotfile blindness,
        # the unescaped-directory-metacharacter hazard, and turns the IndexError on
        # an empty match into a named refusal, all at once.
        rawp = select_one(surfd, "p")
...
def _wedge_series(case_dir, Lramp, end_time):
    ...
    for t, d in series_times(os.path.join(case_dir, "postProcessing", "surfSeries")):
        # UNIQUE-SELECTOR REPAIR of sites run_f3s.py:285 (the assignment) and :286
        # (the `if g:` guard, which is DELETED, not kept).  That guard made a
        # missing pressure sample a SILENTLY SKIPPED time point: the plateau series
        # that decides limb (1) of standing rule 5 shortened and nothing said so.
        praw = select_one(d, "p")
        t_p.append(t); v_p.append(p_wall_mean_of(praw, Lramp))
    t_b, v_b = [], []
