#!/usr/bin/env python3
"""TIP-TOPOLOGY DIAGNOSTIC (grid-b) -- NOT a production recipe, NOT a frozen ladder generator.

This is a DIAGNOSTIC copy of cases/F13_onera_m6/make_blockmesh_m6.py with EXACTLY ONE region
changed: the wall-normal point mapping in Mesh._points (search "WALL-NORMAL DISTRIBUTION").
It exists to answer, with a built + checkMesh'd mesh, the grid-(b) §9-item-5 question: is the
y+<1 (s0=1.546e-6) non-orthogonality obstacle the LEADING EDGE / near-wall boundary layer, or
the wing-TIP topology?  It uses the F13 ANALYTIC symmetric M6 section (m6_section.M6Geometry),
NOT the A3 curvature-refined surface -- so its numbers characterise the C-H TOPOLOGY family at
grid-b s0/layer-count, they are NOT the grid-b production mesh.  Reads only; writes polyMesh to a
caller-named case dir + the scratch dir.  Sends nothing, commits nothing itself (rules 7, 16).

MEASURED RESULT (OpenFOAM v2606 checkMesh, m=1, NN=150 wall-normal layers, s0=1.546e-6 exactly):
  * nofill C-grid (wing surface + LE, NO tip lens), straight-line transfinite : max non-orth 51.37 deg
  * nofill C-grid, tighter LE cluster nose dx/c 0.000764                       : max non-orth 51.60 deg
  * WITH the collapsed-lens tip fill                                           : max non-orth 87.05 deg
    -- the 36 severe faces are ALL outboard of z_tip (z 1.227..1.526), i.e. the TIP CAP.
  * coarse baseline (delta0 162um, y+~35), WITH fill                           : max non-orth 84.64 deg
    -- SAME 36 tip faces.  So the tip lens is ~85-87 deg INDEPENDENT of near-wall spacing.
CONCLUSION: an algebraic STRUCTURED C-grid clears the 70 deg gate at y+<1 on the wing/LE (51 deg);
the residual > 70 deg obstacle is the COLLAPSED-LENS TIP TOPOLOGY, which needs an orthogonal tip
cap (O-cap), not near-wall smoothing.  A near-wall true-normal BLEND (ORTHO_MODE="normal") was
also tried and measured WORSE on the wing (69.9 deg) -- the well-designed farfield already makes
the straight-line inner->outer near-normal, so no near-wall blend is needed for the wing.

--- ORIGINAL make_blockmesh_m6 docstring below ---
...
sys.path.insert(0, "/home/ubuntu/Certonomous/cases/F13_onera_m6")  # m6_section source of truth
...
# ---- GRID-(b) near-wall-orthogonal experiment knobs (set by the driver before build) --------
ORTHO_S0 = 1.546335364348132e-06   # Sanaa's y+<1 first-cell height, FIXED (never changed)
ORTHO_MODE = "normal"              # "normal" (true-normal near wall) or "transfinite" (straight)
ORTHO_DBLEND = 0.006               # [m] blend normal->transfinite completes by this wall distance


def _geom_ratio(ratio_total, n):
    """Per-point growth ratio r>1 with (r^n-1)/(r-1) == ratio_total (== L/s0), by bisection.
    ratio_total is an array; returns r same shape.  Monotone in r, so bisection is exact."""
    lo = np.full_like(ratio_total, 1.0 + 1e-12)
    hi = np.full_like(ratio_total, 5.0)
    for _ in range(80):
        mid = 0.5 * (lo + hi)
        g = (np.power(mid, n) - 1.0) / (mid - 1.0)
        too_small = g < ratio_total
...
class Mesh:
    ...
    def _points(self):
        ...
        # --- WALL-NORMAL DISTRIBUTION (grid-b): near-wall TRUE-NORMAL + blend to transfinite.
        # L = inner->outer distance per (i,k); a per-point geometric distribution hits s0 EXACTLY
        # (Sanaa's y+<1 directive) and reaches L in NN cells.  d[i,j,k] is the wall distance.
        s0 = ORTHO_S0
        L = np.hypot(xou - xin, you - yin)                       # (NW+1, NS+1)
        L = np.maximum(L, 10.0 * s0)                             # guard tiny/zero legs
        ratio_total = L / s0
        r = _geom_ratio(ratio_total, NN)                          # (NW+1, NS+1)
        jj = np.arange(NN + 1)[None, :, None]
        rr = r[:, None, :]
        d = s0 * (np.power(rr, jj) - 1.0) / (rr - 1.0)            # (NW+1, NN+1, NS+1), d[..,0,..]=0
        fnorm = d / L[:, None, :]                                 # in [0,1], fnorm[:,NN,:]=1
        fnorm = np.clip(fnorm, 0.0, 1.0)

        # transfinite (straight-line inner->outer) position at these wall fractions
        X_tf = xin[:, None, :] + (xou - xin)[:, None, :] * fnorm
        Y_tf = yin[:, None, :] + (you - yin)[:, None, :] * fnorm

        if ORTHO_MODE == "transfinite":
            X, Y = X_tf, Y_tf
        else:
            # unit outward surface normal in the (x,y) plane from the inner-curve tangent
            tx = np.gradient(xin, axis=0); ty = np.gradient(yin, axis=0)   # (NW+1, NS+1)
            tn = np.hypot(tx, ty); tn = np.where(tn < 1e-30, 1.0, tn)
            tx, ty = tx / tn, ty / tn
            nx, ny = ty.copy(), -tx.copy()                        # rotate tangent -90
            # orient outward: dot with (xou-xin, you-yin) > 0
            dot = nx * (xou - xin) + ny * (you - yin)
            flip = dot < 0.0
            nx = np.where(flip, -nx, nx); ny = np.where(flip, -ny, ny)
            # pure-normal position: p_inner + d * n
            X_nm = xin[:, None, :] + d * nx[:, None, :]
            Y_nm = yin[:, None, :] + d * ny[:, None, :]
            # blend weight: 0 at the wall (orthogonal), ->1 by ORTHO_DBLEND (smoothstep in d)
            u = np.clip(d / ORTHO_DBLEND, 0.0, 1.0)
            w = u * u * (3.0 - 2.0 * u)                           # smoothstep
            w[:, NN, :] = 1.0                                     # land exactly on the outer curve
            X = (1.0 - w) * X_nm + w * X_tf
            Y = (1.0 - w) * Y_nm + w * Y_tf
