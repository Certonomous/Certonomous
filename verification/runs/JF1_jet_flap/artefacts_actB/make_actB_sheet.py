...
def main() -> int:
    ...
    # THE CLIPPING CAVEAT, WHICH THIS SHEET HAS NEVER CARRIED. The solver
    # pushes turbulent kinetic energy back to zero whenever it goes negative,
    # and it had to do so on the great majority of iterations in every one of
    # the five solved conditions, right through the last one. A field held
    # non-negative by repeated clipping is not the same object as one that
    # converged without it, and that is a limitation of the numbers in Table 2,
    # not housekeeping.
    #
    # It was previously stated on the flow-picture figure ONLY, and only about
    # the companion run, which left the five conditions the customer actually
    # reads looking clean. Measured per iteration rather than per line, because
    # one iteration can clip several times and counting lines overstates it.
    census = [jf1num.bounded_k_census(os.path.join(
        os.path.dirname(SIXTH_CASE), name)) for _, name in jf1num.SWEEP_CASES]
