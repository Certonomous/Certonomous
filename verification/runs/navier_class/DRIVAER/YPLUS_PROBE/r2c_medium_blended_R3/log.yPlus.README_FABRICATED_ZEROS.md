...
```
Exec : postProcess -func yPlus -time 10000
```
...
```
Exec : simpleFoam -postProcess -func yPlus -time 10000      <-- log.yPlus2, THE VALID SPELLING
```

Representative reading from the valid arm: `patch top y+ : min = 12342.05937, max = 17545.71062, average = 15842.79606`. **0 of 52 patches are all-zero there.**
...
## 🔴 THE SOUND NUMBER FOR THIS DIRECTORY, AND IT DID NOT COME FROM EITHER LOG

The registered anchor is **`153.1325756`** — area-weighted median y+ over layered vehicle wall
faces, band `[30, 300]`, from **`YPLUS_AREA_WEIGHTED_R3.json`**, whose `instrument` is
`cases/navier_class/DRIVAER/yplus_area_weighted.py` over **99,600 dumped faces**, carrying its own
closure control: *sum of outward boundary area vectors == 0 on a closed mesh*, **relative residual
1.1894390551851534e-14**. **That anchor is SOUND and is unaffected by the fabricated log.** It is
recorded here because this directory holds a good number and a fabricated one side by side, and the
fabricated one is the file a reader reaches for by name.
