#!/usr/bin/env python3
"""T23G2Rn2 rule-4 completion instrument -- OPTION (b), the THIN WRAPPER.

======================================================================
DRAFT -- NOT FROZEN, NOTHING RUN
======================================================================
Authored by a heat-transfer lab-lane for the §2ba numerics successor T23G2Rn2
(T23G2Rn2_PREREGISTRATION.md section 5.1).  The supervisor picked OPTION (b): keep
the frozen `mark_done_t23.py` (blob 982e1db6622454c2e5cc3e9eb4d6811e87735b78)
BYTE-INVARIANT and reach the T23G2Rn2 levels through a thin import-wrapper instead
...
WHAT THIS FILE IS, AND WHAT IT DELIBERATELY IS NOT.
  ...
  * IT WIDENS THE ALLOW-LIST IN MEMORY ONLY.  It rebinds `mark_done_t23.CASES`
    ...
    `T23G2Rn2_L1/L2/L3`, then delegates to `mark_done_t23.main`.  `run()` resolves
...
FAIL-CLOSED EXACTLY AS THE BASE (nothing is relaxed):
  ...
  * clauses 1-4, the FIXED-dt clause 5 (`n_exec == round(endTime/deltaT)`, which
    is `== endTime` because T23G2Rn2 uses `deltaT 1` exactly as T23G2R --
    build_t23g2r.py:562, inherited unchanged by build_t23g2rn2.py), and the age
...
Usage (identical CLI to mark_done_t23.py):
    python3 mark_done_t23g2rn2.py [CASE ...] [--root DIR] | --selftest
...
# The T23G2Rn2 levels, added to the base's ALLOW-LIST in memory only.
T23G2RN_CASES = ("T23G2Rn2_L1", "T23G2Rn2_L2", "T23G2Rn2_L3")


def widen_cases():
    """Rebind the base module's CASES to include the T23G2Rn2 levels, IN MEMORY.
