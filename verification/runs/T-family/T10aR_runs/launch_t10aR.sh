#!/bin/bash
# Idempotent, collision-safe foreground launcher for ONE T10a-R case.
# Guards G1 (atomic mkdir lock), G2 (cwd scan), G3 (no stray numeric time dir),
# exactly as T10a_runs/launch_t10a.sh.  Touches no process it did not start.
...
cd "$HERE" || exit 4
setsid nohup "$HERE/run_one_t10aR.sh" "$CASE" > /dev/null 2>&1 &
WP=$!
