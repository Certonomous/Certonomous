#!/usr/bin/env python3
"""
Turn T10a-R STATUS files into DONE markers, under the strict rule of
mark_done_t10a.py, unchanged except for the case list and the dropped
optional-case clause (T10a-R has no optional case).
...
A case is DONE only if ALL of:
  ...
  6. every field in that time directory is NEWER than the case's own 0/T
     (run_one_t10aR.sh re-copies 0/ from 0.orig at the start of the run that
...
All three T10a-R cases are required; there is no optional case.
"""
import json
import os
import re
