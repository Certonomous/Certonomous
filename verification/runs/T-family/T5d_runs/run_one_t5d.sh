#!/bin/bash
# ---------------------------------------------------------------------------
# T5d PROVENANCE.  This file is `../T5b_runs/run_one_t5b.sh` -- the launcher
# whose cap enforcement was DRIVEN and PROVEN pre-freeze (T5b registration S10,
# three arms: stopped at the cap, SIGTERM-ignoring child still killed, child
# inside the cap untouched) -- with EXACTLY THREE mechanical substitutions and
# nothing else:
#
#     T5B_CAPS.txt   -> T5D_CAPS.txt      (this rung's own registered caps)
#     T5B_DETACHED   -> T5D_DETACHED      (the detach sentinel env var)
#     run_one_t5b.sh -> run_one_t5d.sh    (self-name in usage and re-exec)
#
# plus the two comment headings that name the rung.  No guard, no refusal, no
# cap arithmetic, no timeout, no age-guard line and no STATUS field is altered.
# The delta is verifiable: `diff` against the frozen T5b file, with those
# substitutions reversed, must return NO differences, and the registration
# states the diff line count it must produce.
#
# The cap enforced here is T5d's own, from the frozen T5D_CAPS.txt beside this
# file.  An argv still cannot widen it.
# ---------------------------------------------------------------------------
# T5d LAUNCHER -- rc IS THE SOLVER'S, AND THE CAP IS ENFORCED, NOT REPORTED.
#
# ===========================================================================
...
#    frozen `T5D_CAPS.txt` for this case.  It prints `CAP AGREES <case> <cap>`
...
usage() {
    cat >&2 <<'U'
usage: run_one_t5d.sh --case-dir DIR --cap-core-min N [--ranks N] [--solver NAME]
                      [--foam-bashrc PATH|none] [--no-detach]
       run_one_t5d.sh --drive-cap-kill        (driven proof that the cap KILLS)
  --cap-core-min  the REGISTERED cap in core-minutes.  Wall seconds are derived
                  HERE as cap*60/ranks and checked against T5D_CAPS.txt.
...
if [ "$DRIVE_KILL" = "1" ]; then
    echo "run_one_t5d.sh --drive-cap-kill"
...
if [ "$DETACH" = "1" ] && [ "${T5D_DETACHED:-}" != "1" ]; then
    T5D_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --cap-core-min "$CAP_CORE_MIN" \
...
# --- T5b/T5d PRECONDITION: the repaired function objects must be on disk ------
