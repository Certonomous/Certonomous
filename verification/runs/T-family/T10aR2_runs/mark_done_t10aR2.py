#!/usr/bin/env python3
"""Turn T10aR2 STATUS files into DONE markers under the STRICT COMPLETION RULE,
...
PHYSICS-CRITICAL (each a conjunct; any failure -> NOT DONE, no marker):
  1. STATUS.<case> EXISTS and carries the solver's own rc, written INSIDE the
     launch wrapper (launch_t10aR2.sh runs the solver in its foreground under
  ...
  4. every registered field is present at that time
     (T and qr, the two fields the frozen T10a comparator reads)
...
Usage:  python3 mark_done_t10aR2.py [CASE ...] [--root DIR] | --selftest
...
def main(argv):
    ...
    for c in want:
        if c not in CASES:
            refuse("%r is not one of the registered %s cases: %s" % (c, "T10aR2", " ".join(CASES)))
    rc = EXIT_OK
    for case in want:
        fails, nm = check(case)
