#!/usr/bin/env bash
# ==========================================================================
# T10aR2 CASE LAUNCHER -- modelled on scripts/launch_k0f.sh: rc-CAPTURING,
# optionally DETACHED, QUEUE-COMPATIBLE.
#
...
#   * rc is captured INSIDE and written to STATUS.<case> in the case's PARENT
#     directory, the file mark_done_t10aR2.py reads;
#   * `capped` = (wall_s >= timeout_s) is the INDEPENDENT expiry witness -- rc
#     124 collides with a solver that itself exits 124, and 137 with an OOM kill
#     (T4 section 9.3, measured); it is an INFRASTRUCTURE field (L-342): it
#     labels a non-zero rc, it never voids a run;
#   * a PRE-FLIGHT REFUSAL writes NO STATUS: nothing ran, so there is no rc, and
#     inventing one is the back-dating this file exists to prevent;
#   * the registered cap is READ from T10aR2_registered.json and a --timeout that
#     is not EQUAL to it is REFUSED (launch_t3_rff.sh form): a caller can
#     neither widen a cap that rule 12 says stops the run nor narrow one into a
#     manufactured cap-stop; --ranks must equal the registered 1;
#   * an existing STATUS.<case> is REFUSED before anything is written: a
#     completed run's record is never overwritten;
#   * `exit "$RC"` is the LAST line: the wrapper reports the solver's own status
#     to its caller, never 0.
#
# usage: launch_t10aR2.sh --case-dir DIR --timeout SECONDS [--ranks 1]
#                      [--solver buoyantSimpleFoam] [--foam-bashrc F]
#                      [--no-detach]
...
REGJSON="$SELF/T10aR2_registered.json"

usage() { sed -n '/^# usage:/,/^# ====/p' "$0" >&2; exit 2; }

CASE_DIR=""; TIMEOUT_S=""; RANKS=1
SOLVER="buoyantSimpleFoam"; DETACH=1
...
[ "$RANKS" = "1" ] || { echo "REFUSE: T10aR2 is registered SERIAL (ranks 1 on every level); --ranks=$RANKS" >&2; exit 2; }
...
# --- the registered cap: a --timeout above it is refused --------------------
if [ -f "$REGJSON" ]; then
    ...
    case "$REG_T" in
        UNREGISTERED) echo "REFUSE: $CASE is not a registered T10aR2 case in $REGJSON" >&2; exit 2 ;;
...
if [ "$DETACH" = "1" ] && [ "${T10AR2_DETACHED:-}" != "1" ]; then
    T10AR2_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" \
...
[ -f "$CASE_DIR/constant/F" ] && [ -f "$CASE_DIR/constant/globalFaceFaces" ] || { echo "REFUSE: $CASE has no constant/F + globalFaceFaces (viewFactorsGen preprocessing absent); no solver started, no STATUS written" >&2; exit 2; }
for f in T U p p_rgh qr; do
...
# guard below excluded only $$; measured: three zero-compute refusals, one per level.
