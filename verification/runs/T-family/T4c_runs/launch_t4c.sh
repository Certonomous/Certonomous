#!/usr/bin/env bash
# ==========================================================================
# T4c CASE LAUNCHER -- modelled on scripts/launch_k0f.sh: rc-CAPTURING,
...
#     directory, the file mark_done_t4c.py reads;
...
#   * the registered cap is READ from T4c_registered.json and a --timeout that
...
#
# usage: launch_t4c.sh --case-dir DIR --timeout SECONDS [--ranks 1]
...
REGJSON="$SELF/T4c_registered.json"
...
[ "$RANKS" = "1" ] || { echo "REFUSE: T4c is registered SERIAL (ranks 1 on every level); --ranks=$RANKS" >&2; exit 2; }
...
if [ -f "$REGJSON" ]; then
    ...
    case "$REG_T" in
        UNREGISTERED) echo "REFUSE: $CASE is not a registered T4c case in $REGJSON" >&2; exit 2 ;;
...
if [ "$DETACH" = "1" ] && [ "${T4C_DETACHED:-}" != "1" ]; then
    T4C_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" \
