#!/usr/bin/env python3
"""Turn T4c STATUS files into DONE markers under the STRICT COMPLETION RULE,
...
PHYSICS-CRITICAL (each a conjunct; any failure -> NOT DONE, no marker):
  1. STATUS.<case> EXISTS and carries the solver's own rc, written INSIDE the
     launch wrapper (launch_t4c.sh runs the solver in its foreground under
...
Usage:  python3 mark_done_t4c.py [CASE ...] [--root DIR] | --selftest
...
ROOT = HERE
CASES = ("T4c_IJ_c", "T4c_IJ_m", "T4c_IJ_f")
...
def main(argv):
    ...
    for c in want:
        if c not in CASES:
            refuse("%r is not one of the registered %s cases: %s" % (c, "T4c", " ".join(CASES)))
    rc = EXIT_OK
    for case in want:
        fails, nm = check(case)
