#!/usr/bin/env python3
"""Build the three registered T4c impinging-jet cases (successor of T4b, itself the
wall-resolved successor of T4).

T4c re-runs T4b (ERCOFTAC case025 ij2lr, H/D = 2, Re_D = 23 000, axisymmetric wedge,
steady buoyantBoussinesqSimpleFoam with beta = 0, kOmegaSST) with THREE registered
changes against build_t4b.py, each stated here and in T4c_PREREGISTRATION.md, and
...
wrong (T4b_RESULTS.md, gate_t4b.json):
  ...
  C. endTime schedule 20000 / 30000 / 40000 -> 30000 / 60000 / 64000, sized so the
     ...
     (T4c_PREREGISTRATION.md section 3).  writeInterval (2000 / 3000 / 4000) and
...
Usage:  build_t4c.py [--root <dir>] [--level c|m|f] ...
"""
import argparse
import math
import os
...
def block_mesh_dict(N, first_cell):
    ...
    nrj = (3 * N) // 2    # radial cells, jet core and pipe (T4b: 3N/4; DOUBLED, T4c Change A)
...
def system_files(endTime, writeInterval):
    ...
    under-relaxation factors are reduced 0.7 -> 0.6 (T4c Change B) to damp the
    ...
    frozen build_t4 writer derives writeInterval = endTime/10; T4c's endTimes
    (30000/60000/64000) would give 3000/6000/6400, but T4c keeps the T4b windows
...
def build(root, level):
    ...
    open(os.path.join(case, "CASE.txt"), "w").write("\n".join([
        ...
        "rung              T4c (successor of T4b; ERCOFTAC case025 ij2lr, H/D 2, Re 23000)",
