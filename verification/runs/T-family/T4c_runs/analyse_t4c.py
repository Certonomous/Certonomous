#!/usr/bin/env python3
"""T4c impinging jet -- the FROZEN comparator (T4b's, rename-only), with the section-9 controls ON THE
GRADING PATH.
...
REG = json.load(open(os.path.join(HERE, "T4c_registered.json")))
LEVELS = ("c", "m", "f")
CASES = {lv: "T4c_IJ_%s" % lv for lv in LEVELS}
...
if (float(_RF["STAGNANT_FLOOR"]), float(_RF["P_MIN"])) != (STAGNANT_FLOOR, P_MIN):
    print("REFUSE: T4c_registered.json roache_floors %r disagree with scripts/roache_triple.py "
...
def grade(a):
    prov = {"analyse_t4.py (frozen, imported)": sha256_of(os.path.join(T4DIR, "analyse_t4.py")),
            "analyse_t4c.py": sha256_of(os.path.abspath(__file__)),
            "T4c_registered.json": sha256_of(os.path.join(HERE, "T4c_registered.json"))}
    print("T4c comparator.  shared floors STAGNANT_FLOOR = %g, P_MIN = %g (scripts/roache_triple.py).  "
    ...
    for rid, rd, reffile, half in GRADED:
        ...
        print("%-3s r/D=%.1f fine=%.4f ref=%.4f band=[%.4f,%.4f] triple=%s p=%s GCI=%s -> %s%s"
              % (rid, rd, vals["f"], ref, ref - half, ref + half, state,
                 ...
                 (" [" + note + "]") if note else ""))
    out = dict(rung="T4c", STAGNANT_FLOOR=STAGNANT_FLOOR, P_MIN=P_MIN,
...
def drive_refusal(which, a):
    ...
    if which == "missing-done":
        d = tempfile.mkdtemp(prefix="t4c_nodone_")
        try:
            global HERE
            HERE = d
            for lv, case in CASES.items():
