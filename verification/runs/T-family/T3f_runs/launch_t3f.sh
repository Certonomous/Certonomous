...
REG_RANKS=8; REG_TIMEOUT_S=31115; REG_CAP_CORE_MIN=4148     # registered, T3f_PREREGISTRATION.md S5
...
[ "$RANKS" = "$REG_RANKS" ] || { echo "REFUSE: --ranks=$RANKS; R_fz is registered at $REG_RANKS ranks (T3f prereg S9)" >&2; exit 2; }
...
[ "$CASE" = "R_fz" ] || { echo "REFUSE: this launcher is registered for R_fz only; got $CASE" >&2; exit 2; }
...
[ -f "$CASE_DIR/constant/polyMesh/points" ] || { echo "REFUSE: no mesh -- the birth certificate (build_t3d.py R_fz) has not been issued" >&2; exit 2; }
