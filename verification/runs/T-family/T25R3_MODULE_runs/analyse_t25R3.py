...
FROZEN_DOC = "docs/campaigns/T-family/T25R3_PREREGISTRATION.md"
EXIT_REFUSE = 2
...
def patch_values(path, patch, nfaces):
    ...
    if kind == "uniform":
        v = float(_UNIFORM_RE.match(lines[first]).group(1))
        return [v] * nfaces
    if n != nfaces:
...
def tdir(case_dir, t):
    ...
    for cand in ("%g" % t, "%.1f" % t, "%.2f" % t):
        ...
        if os.path.isdir(p):
            return p
    refuse("case %s has no time directory for t = %g -- §9 registers writes "
           "every 5 s, so this time was registered and is ABSENT"
           % (case_dir, t))
