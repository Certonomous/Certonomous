#!/usr/bin/env bash
# ==========================================================================
# T4e CASE LAUNCHER -- modelled on scripts/launch_k0f.sh: rc-CAPTURING,
...
#     directory, the file mark_done_t4e.py reads;
...
#   * the registered cap is READ from T4e_registered.json and a --timeout that
...
#
# usage: launch_t4e.sh --case-dir DIR --timeout SECONDS [--ranks 1]
...
REGJSON="$SELF/T4e_registered.json"
...
[ "$RANKS" = "1" ] || { echo "REFUSE: T4e is registered SERIAL (ranks 1 on every level); --ranks=$RANKS" >&2; exit 2; }
...
if [ -f "$REGJSON" ]; then
    ...
    case "$REG_T" in
        UNREGISTERED) echo "REFUSE: $CASE is not a registered T4e case in $REGJSON" >&2; exit 2 ;;
...
if [ "$DETACH" = "1" ] && [ "${T4E_DETACHED:-}" != "1" ]; then
    T4E_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" \
