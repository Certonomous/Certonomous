#!/usr/bin/env python3
"""T4e impinging jet -- the FROZEN comparator (T4b's, rename-only), with the section-9 controls ON THE
GRADING PATH.
...
LEVELS = ("c", "m", "f")
CASES = {lv: "T4e_IJ_%s" % lv for lv in LEVELS}
...
if (float(_RF["STAGNANT_FLOOR"]), float(_RF["P_MIN"])) != (STAGNANT_FLOOR, P_MIN):
    print("REFUSE: T4e_registered.json roache_floors %r disagree with scripts/roache_triple.py "
...
def scratch_copy(case_dir, time):
    ...
    tmp = tempfile.mkdtemp(prefix="t4e_")
...
def grade(a):
    prov = {"analyse_t4.py (frozen, imported)": sha256_of(os.path.join(T4DIR, "analyse_t4.py")),
            "analyse_t4e.py": sha256_of(os.path.abspath(__file__)),
            "T4e_registered.json": sha256_of(os.path.join(HERE, "T4e_registered.json"))}
    print("T4e comparator.  shared floors STAGNANT_FLOOR = %g, P_MIN = %g (scripts/roache_triple.py).  "
          "provenance (sha256):" % (STAGNANT_FLOOR, P_MIN))
    for k, v in prov.items():
        print("  %-36s %s" % (k, v))
    for lv, case in CASES.items():
        if not os.path.isfile(os.path.join(HERE, "DONE.%s" % case)):
            refuse("no DONE.%s -- the whole rung is graded or none of it is (mark_done_t4e.py "
    ...
    for rid, rd, reffile, half in GRADED:
        ...
        print("%-3s r/D=%.1f fine=%.4f ref=%.4f band=[%.4f,%.4f] triple=%s p=%s GCI=%s -> %s%s"
              % (rid, rd, vals["f"], ref, ref - half, ref + half, state,
                 ...
                 (" [" + note + "]") if note else ""))
    out = dict(rung="T4e", STAGNANT_FLOOR=STAGNANT_FLOOR, P_MIN=P_MIN,
...
def selftest(a):
    ...
    chk("zero `assert` nodes in analyse_t4e.py (AST)", count_asserts(me) == 0)
...
def drive_refusal(which, a):
    ...
    if which == "missing-done":
        d = tempfile.mkdtemp(prefix="t4e_nodone_")
        try:
            global HERE
            HERE = d
            for lv, case in CASES.items():
...
def main():
    ...
    ap.add_argument("--json", default=os.path.join(HERE, "gate_t4e.json"))
