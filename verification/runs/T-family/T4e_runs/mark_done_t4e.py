#!/usr/bin/env python3
"""Turn T4e STATUS files into DONE markers under the STRICT COMPLETION RULE,
...
PHYSICS-CRITICAL (each a conjunct; any failure -> NOT DONE, no marker):
  1. STATUS.<case> EXISTS and carries the solver's own rc, written INSIDE the
     launch wrapper (launch_t4e.sh runs the solver in its foreground under
...
Usage:  python3 mark_done_t4e.py [CASE ...] [--root DIR] | --selftest
...
def main(argv):
    ...
    for c in want:
        if c not in CASES:
            refuse("%r is not one of the registered %s cases: %s" % (c, "T4e", " ".join(CASES)))
    rc = EXIT_OK
    for case in want:
        fails, nm = check(case)
