#!/usr/bin/env python3
"""Build the three registered T4e impinging-jet cases (successor of T4b, itself the
wall-resolved successor of T4).

T4e re-runs T4b (ERCOFTAC case025 ij2lr, H/D = 2, Re_D = 23 000, axisymmetric wedge,
steady buoyantBoussinesqSimpleFoam with beta = 0, kOmegaSST) with THREE registered
changes against build_t4b.py, each stated here and in T4e_PREREGISTRATION.md, and
...
wrong (T4b_RESULTS.md, gate_t4b.json):
  ...
  C. endTime schedule 20000 / 30000 / 40000 -> 30000 / 60000 / 64000, sized so the
     ...
     writeInterval (2000 / 3000 / 4000) is UNCHANGED, so C6.3 measures the IDENTICAL
     iteration window it measured in T4b -- the measurement is not shortened to
     manufacture a pass.

T4e DELTAS AGAINST ITS IMMEDIATE PARENT build_t4d.py (T4e_PREREGISTRATION.md; the
physics/design of A/B/C above are the byte-identical T4b/T4c/T4d inheritance; these
three are the ONLY substantive build changes vs build_t4d):

  D. FINE endTime 64000 -> 160000 (coarse 30000 and medium 60000 UNCHANGED).  T4d
     graded NOT A RESULT because the FINE C6.3 (0.01518, ~76x the 2e-04 tol) failed
     convergence at 64000 and all three Roache triples were non-CONVERGING.  T4e
     re-runs all three levels FRESH and extends ONLY the fine window so the fine
     C6.3 TRAJECTORY can be MEASURED far enough to discriminate D1 (a sustained
     limit cycle -> physical unsteadiness) from D2 (a slow monotone decay that still
     clears 2e-04 given room) from D3 (decays-but-not-cleared).  160000 is the HARD
     iteration cap; the fine leg may be terminated CLEANLY earlier on robust D1
     confirmation (trajectory_t4e.py; T4e_PREREGISTRATION.md sections 3a/9).

  E. purgeWrite 2 -> 0 (ALL levels), so EVERY written checkpoint is retained for the
     fine C6.3 trajectory (the sliding W=15 checkpoint window; with purgeWrite 2 only
     the last two survive and no trajectory could be built).

  F. runTimeModifiable false -> true (ALL levels), so the D1 early-terminate trigger
     can be applied at runtime by trajectory_t4e.py rewriting controlDict `stopAt`
     endTime -> writeNow: a CLEAN stop (fields written, an `End` line), NOT a kill.
     This changes NO band, threshold, floor, reference or prediction: it only enables
     a runtime-readable controlDict, and the D1 branch is a NON-CONVERGENCE finding
     (NOT A RESULT, unsteadiness), never a rule-4 completion (T4e_PREREGISTRATION.md
     section 3b, completion-semantics).
...
Usage:  build_t4e.py [--root <dir>] [--level c|m|f] ...
"""
import argparse
import math
import os
...
LEVELS = {"c": dict(N=48,  endTime=30000, writeInterval=2000, first_cell=1.2e-5),
          ...
          "f": dict(N=192, endTime=160000, writeInterval=4000, first_cell=3.0e-6)}
CASE_PREFIX = "T4e_IJ_"
...
def block_mesh_dict(N, first_cell):
    ...
    nrj = (3 * N) // 2    # radial cells, jet core and pipe (T4b: 3N/4; DOUBLED, T4e Change A)
...
def system_files(endTime, writeInterval):
    """The frozen build_t4.system_files() output with FIVE registered changes:
    (1) the residualControl block is removed (L-141); (2) the U/T/k/omega
    under-relaxation factors are reduced 0.7 -> 0.6 (T4e Change B) to damp the
    ...
    frozen build_t4 writer derives writeInterval = endTime/10; T4e's endTimes
    (30000/60000/160000) would give 3000/6000/16000, but T4e keeps the T4b windows
    2000/3000/4000 so C6.3 measures the IDENTICAL iteration window (the last two
    written checkpoints) it measured in T4b;  (4) purgeWrite 2 -> 0 (T4e Change E)
    so EVERY checkpoint is retained for the fine C6.3 sliding-window trajectory;
    (5) runTimeModifiable false -> true (T4e Change F) so the D1 clean-stop trigger
    can rewrite controlDict `stopAt` at runtime.  Refuses unless exactly one
    residualControl block, exactly four 0.7 relaxation factors, exactly one
    writeInterval line, exactly one `purgeWrite 2` line and exactly one
    `runTimeModifiable false` line were present and changed -- so a change to the
    inherited frozen writer cannot pass silently -- and unless endTime is a multiple
    of writeInterval (else the last checkpoint would not land on endTime)."""
    ...
    # T4e Change E: purgeWrite 2 -> 0 so every checkpoint survives for the trajectory
    cd, pw = re.subn(r"^(\s*purgeWrite\s+)2(\s*;)", r"\g<1>0\g<2>", cd, flags=re.M)
    if pw != 1:
        refuse("controlDict: expected exactly one 'purgeWrite 2' line to change to 0, "
               "found %d -- the frozen writer's purgeWrite default may have moved" % pw)
    if re.search(r"^\s*purgeWrite\s+2\s*;", cd, flags=re.M):
        refuse("controlDict still holds 'purgeWrite 2' after the registered change to 0")
    # T4e Change F: runTimeModifiable false -> true so the D1 stop trigger is readable
    cd, rtm = re.subn(r"^(\s*runTimeModifiable\s+)false(\s*;)", r"\g<1>true\g<2>", cd, flags=re.M)
    if rtm != 1:
        refuse("controlDict: expected exactly one 'runTimeModifiable false' line to change "
               "to true, found %d -- the frozen writer's default may have moved" % rtm)
...
def build(root, level):
    ...
    open(os.path.join(case, "CASE.txt"), "w").write("\n".join([
        ...
        "rung              T4e (successor of T4b; ERCOFTAC case025 ij2lr, H/D 2, Re 23000)",
        ...
        "endTime           %d   writeInterval %d   purgeWrite 0 (T4e Change E: retain all checkpoints)" % (spec["endTime"], spec["writeInterval"]),
        "runTimeModifiable true (T4e Change F: enables the D1 clean-stop trigger; stopAt endTime)",
