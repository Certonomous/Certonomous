#!/usr/bin/env bash
# ==========================================================================
# T4b CASE LAUNCHER -- modelled on scripts/launch_k0f.sh: rc-CAPTURING,
...
#     directory, the file mark_done_t4b.py reads;
...
#   * the registered cap is READ from T4b_registered.json and a --timeout that
...
#
# usage: launch_t4b.sh --case-dir DIR --timeout SECONDS [--ranks 1]
...
REGJSON="$SELF/T4b_registered.json"
...
[ "$RANKS" = "1" ] || { echo "REFUSE: T4b is registered SERIAL (ranks 1 on every level); --ranks=$RANKS" >&2; exit 2; }
...
if [ -f "$REGJSON" ]; then
    ...
    case "$REG_T" in
        UNREGISTERED) echo "REFUSE: $CASE is not a registered T4b case in $REGJSON" >&2; exit 2 ;;
...
if [ "$DETACH" = "1" ] && [ "${T4B_DETACHED:-}" != "1" ]; then
    T4B_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" \
...
#         (T4 AMENDMENT 2's defect class; launch_t3_rff.sh form; refusal writes no STATUS)
