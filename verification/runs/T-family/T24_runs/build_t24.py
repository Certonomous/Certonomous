...
T24: the TWELVE points T23_PREREGISTRATION.md section 6.4 deferred.  Gates FROZEN
at docs/campaigns/T-family/T24_PREREGISTRATION.md, commit b90574899143.

THIS FILE IS build_t23.py WITH FOUR EDITS AND NOTHING ELSE, each applied under an
exactly-once assertion so the "derived from" claim is checkable rather than
asserted: this docstring; P_LOSS and U_INF read from the environment instead of
being literals; and two comment strings naming the wrong builder.  NO GEOMETRY,
NO MESH PARAMETER, NO MATERIAL PROPERTY, NO SCHEME AND NO SOLVER SETTING IS
TOUCHED.  `diff build_t23.py build_t24.py` is the audit.
...
U_inf.  P_loss and U_inf come from T24_P_LOSS / T24_U_INF in the environment; the
builder REFUSES if either is absent, because a silent default would build a case
that is not the case the registration names.
...
# ------------------------------------------------------------- operating point
# THE TWO SWEPT SCALARS.  Read from the environment, never defaulted: T24 builds
# twelve cases from ONE set of bytes, and a default here would let a mistyped
# launch silently build the wrong operating point under the right case name.
# T24_PREREGISTRATION.md section 1 line 1 registers that these two enter ONLY
# constant/core/fvOptions and the fluid 0.orig values -- never blockMeshDict --
# which is why the mesh is identical across all twelve cases and to T23's.
def _swept(name):
    v = os.environ.get(name)
    if v is None:
        raise SystemExit("REFUSE: %s is not set; build_t24.py never defaults a swept scalar" % name)
    return float(v)

P_LOSS = _swept("T24_P_LOSS")   # W, FULL 360-degree motor loss  [REGISTERED, T23 section 2.6 levels]
U_INF = _swept("T24_U_INF")     # m/s                            [REGISTERED-by-owner, directive 3.3]
T_INF = 288.0            # K
...
def write_fv_options():
    ...
    4. THE WEDGE POWER FACTOR.  Directive 3.3's P_loss is the FULL 360-degree
       ...
       mesh."""
    w("constant/core/fvOptions", "dictionary", "fvOptions", """// Uniform volumetric loss in the motor core.  See build_t24.py:write_fv_options
...
def foam(cmd, log):
    ...
    if any(tool.endswith(b) or tool == b for b in ("chtMultiRegionSimpleFoam",
        ...
        raise SystemExit("REFUSE: build_t24.py never launches a solver (%s)" % tool)
