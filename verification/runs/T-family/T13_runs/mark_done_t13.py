#!/usr/bin/env python3
"""Turn T13 STATUS files into DONE markers under the STRICT COMPLETION RULE,
...
PHYSICS-CRITICAL (each a conjunct; any failure -> NOT DONE, no marker):
  1. STATUS.<case> EXISTS and carries the solver's own rc, written INSIDE the
     launch wrapper (run_one_t13.sh runs the solver in its foreground under
  ...
  4. every registered field is present at that time (T U p_rgh phi)
  5. the ExecutionTime line count == endTime (deltaT = 1 on every level)
...
ended_utc.  An absent or malformed infrastructure field is reported NOT
MEASURED and the grade PROCEEDS.  `capped` is NOT a conjunct (the T11/T4 form
that returned NOT DONE on a missing `capped` witness was audited CONFLATING,
L342_FIELD_CLASS_AUDIT_2026-08-26.md); it is still READ when present, because
it separates a cap-stop (rule 12: the run stops, NOT A RESULT) from a crash (a
finding needing triage) -- but it only LABELS a non-zero rc, never voids rc=0.

REFUSAL, NOT INFERENCE.  An ABSENT STATUS file is a REFUSAL (exit 2), never a
pass and never a NOT DONE a reader could mistake for a graded outcome (the K0d
L1 defect: a launcher wrote no rc and the right answer was to refuse rather
than infer rc=0 from an End line).
...
python3 -O alike; --root DIR points the reader at another tree (selftest use).

Usage:  python3 mark_done_t13.py [CASE ...] [--root DIR] | --selftest
...
ROOT = HERE
CASES = ("T13_VS_c", "T13_VS_m", "T13_VS_f")
...
def _forge(root, case, rc="0", end=40, n_exec=None, with_end=True, stale=False,
           status=True, infra=True, missing_field=None, capped="no"):
...
def selftest():
    ...
    print("mark_done_t13 selftest (planted controls; each clause drives the rule):")
    ...
    expect("a registered field missing at endTime -> NOT DONE", 1, False, missing_field=NEEDED[3])
...
def main(argv):
    ...
    for c in want:
        if c not in CASES:
            refuse("%r is not one of the registered T13 cases: %s" % (c, " ".join(CASES)))
    rc = EXIT_OK
    for case in want:
        fails, nm = check(case)
