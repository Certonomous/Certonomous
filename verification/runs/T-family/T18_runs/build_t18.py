#!/usr/bin/env python3
"""Build T18's registered cases: 3-D transient conduction in a cube (T11c).

SOLID ONLY -- there is no fluid in this rung.  An OCTANT of a cube of half-side
L, symmetry planes at x = 0, y = 0 and z = 0 (zeroGradient), convective Robin
faces at x = L, y = L and z = L, uniform initial theta = 1, T_inf = 0 so
theta = T directly.
...
Cases: T18_CU_c / T18_CU_m / T18_CU_f (N = 20 / 40 / 80 across the half-side,
r = 2 exactly) and T18_CU_f_CT, the temporal-bias control (fine mesh, deltaT
...
usage: build_t18.py --root DIR [--level c|m|f|f_CT ...] [--no-mesh] | --check-levels | --selftest
"""
import argparse
import os
import re
...
L = 0.01          # m, half-side of the cube
ALPHA = 1.0e-5    # m2/s
BI = 1.0
...
LEVELS = {"c": 20, "m": 40, "f": 80, "f_CT": 80}
DT_OF = {"c": DELTA_T, "m": DELTA_T, "f": DELTA_T, "f_CT": DELTA_T / 2.0}
CASE_OF = {lv: "T18_CU_%s" % lv for lv in LEVELS}
...
def write_case(root, level):
    ...
    open(os.path.join(case, "system", "blockMeshDict"), "w").write(
        ...
        "    symZ  { type patch; faces ( (0 3 2 1) ); }\n"
        "    faceZ { type patch; faces ( (4 5 6 7) ); }\n"
        ");\n\nmergePatchPairs ();\n"
        % (L, L, L, L, L, L, L, L, L, L, L, L, N, N, N))
    ...
    open(os.path.join(case, "0.orig", "T"), "w").write(
        ...
        % (T_INIT, robin, robin, robin))
    open(os.path.join(case, "CASE.txt"), "w").write(
        "case=%s\nlevel=%s\nN=%d\ncells=%d\ndx=%.17g\nvalueFraction=%.17g\nBi=%g\nL=%g\n"
        "alpha=%g\nendTime=%g\ndeltaT=%g\nFo_end=%.17g\nsolver=laplacianFoam\nranks=1\n"
        "decomposition=serial_1_rank_no_decomposition\n"
        % (CASE_OF[level], level, N, N ** 3, L / N, f, BI, L, ALPHA, END_T, dt, ALPHA * END_T / (L * L)))
    return case, N, f
...
def selftest():
    ...
    tmp = tempfile.mkdtemp(prefix="t18build_")
    try:
        case, N, f = write_case(tmp, "c")
        txt = open(os.path.join(case, "0.orig", "T")).read()
        vf = re.findall(r"valueFraction\s+uniform\s+([0-9.eE+-]+)", txt)
        ok = (len(vf) == 3 and all(abs(float(v) - BI / (BI + 2 * N)) < 1e-15 for v in vf)
              and not os.path.exists(os.path.join(case, "0")))
        print("  [%s] written 0.orig/T carries the derived valueFraction on ALL THREE Robin faces; no 0/ written"
              % ("ok " if ok else "FAIL"))
        if not ok:
            fails.append("write")
        ok = txt.count("zeroGradient") == 3
        print("  [%s] all THREE symmetry planes are zeroGradient" % ("ok " if ok else "FAIL"))
        if not ok:
            fails.append("sym")
        bmd = open(os.path.join(case, "system", "blockMeshDict")).read()
        ok = ("(%d %d %d)" % (N, N, N)) in bmd
        print("  [%s] blockMeshDict is N x N x N = %d^3" % ("ok " if ok else "FAIL", N))
        if not ok:
            fails.append("mesh-dims")
        meta = dict(re.findall(r"^([A-Za-z_]+)=(.*)$", open(os.path.join(case, "CASE.txt")).read(), re.M))
        ok = (int(meta["cells"]) == N ** 3 and meta["ranks"] == "1"
              and meta["decomposition"] == "serial_1_rank_no_decomposition")
        print("  [%s] CASE.txt records cells=N^3, ranks=1 and the decomposition seed" % ("ok " if ok else "FAIL"))
        if not ok:
            fails.append("case-meta")
    finally:
...
if __name__ == "__main__":
    ...
    for lv in (a.level or ["c", "m", "f", "f_CT"]):
        ...
        if not a.no_mesh:
            mesh(case)
        print("  built %s N=%d cells=%d valueFraction=%.8e deltaT=%g"
              % (os.path.basename(case), N, N ** 3, f, DT_OF[lv]))
