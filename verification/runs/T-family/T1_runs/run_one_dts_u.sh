#!/usr/bin/env bash
# ==========================================================================
# T1c-U CASE LAUNCHER (unheated-upstream arm, buoyantBoussinesqSimpleFoam) -- run
# ONE registered case of the six, capture the SOLVER's rc in this wrapper, write
# STATUS_u.<case>.  Modelled line-for-line on run_one_t14.sh (freeze 5a870e54),
# which is run_one_t13.sh (freeze 0d2dc150):
#
#   * the solver runs in THIS wrapper's FOREGROUND under `timeout`, with no
#     `setsid` between them, so `$?` is the solver's status;
#   * rc is captured INSIDE and written to STATUS_u.<case> in the run root -- a
#     SEPARATE namespace from the older arms' STATUS.<case>, which is never
#     touched (the
#     file mark_done_dts_u.py reads);
...
#     kill; it is an INFRASTRUCTURE field (L-342): it labels a non-zero rc, it
#     never voids a run;
#   * a PRE-FLIGHT REFUSAL writes NO STATUS: nothing ran, so there is no rc;
#   * the registered cap is READ from DTS_U_registered.json and a --timeout that
#     is not EQUAL to it is REFUSED; --ranks must equal the registered 1;
#   * time directories are matched by a REGEX with fullmatch semantics, NEVER a
#     shell glob (`[0-9]*` matches `0.orig`);
#   * an existing STATUS_u.<case> is REFUSED before anything is written;
#   * CELL CENTRES AND CELL VOLUMES ARE WRITTEN BY OpenFOAM ITSELF into 0/
#     before the solver starts (`postProcess -func writeCellCentres` /
#     `writeCellVolumes`).  T8's GROUND 2 was a mesh ratio ASSERTED from the
#     nominal spec and never measured (T8_VERDICT_2026-08-26.md section 4);
#     this arm's readers take Cx/Cy/Cz/V that OpenFOAM wrote, as the parent
#     arms already do (log.writeCellCentres beside every solved T1 case).
#     They are written BEFORE `0/T` is touched, so `0/T` still dates the run;
#   * `exit "$RC"` is the LAST line: the wrapper reports the solver's own
#     status to its caller, never 0.
#
# usage: run_one_dts_u.sh --case-dir DIR --timeout SECONDS [--ranks 1]
...
REGJSON="$SELF/DTS_U_registered.json"
...
[ "$RANKS" = "1" ] || { echo "REFUSE: the T1c-U arm is registered SERIAL (ranks 1); --ranks=$RANKS" >&2; exit 2; }

CASE_DIR="$(cd "$CASE_DIR" && pwd)"
ROOT="$(dirname "$CASE_DIR")"
CASE="$(basename "$CASE_DIR")"
STATUS="$ROOT/STATUS_u.$CASE"
...
if [ -f "$REGJSON" ]; then
    ...
    case "$REG_T" in
        UNREGISTERED) echo "REFUSE: $CASE is not a registered T1c-U case in $REGJSON" >&2; exit 2 ;;
...
if [ "$DETACH" = "1" ] && [ "${DTS_U_DETACHED:-}" != "1" ]; then
    DTS_U_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" \
...
POST_PATH="$(command -v postProcess 2>/dev/null || true)"
[ -n "$POST_PATH" ] || { echo "REFUSE: postProcess is NOT RESOLVABLE; the comparator's measured cell centres and volumes could not be written. Nothing ran, no STATUS written." >&2; exit 2; }
...
[ -f "$CASE_DIR/constant/polyMesh/owner" ] || { echo "REFUSE: $CASE has no mesh (constant/polyMesh/owner) -- run build_dts_u.py" >&2; exit 2; }
...
# LINEAGE-AWARE foreign-process guard (T10aR2 AMENDMENT 1, 9fa66065; the block
# copied from run_one_t14.sh:145-157): under the queue runner's launch form
# `setsid nohup bash -c 'cd <cwd>; <argv> ...'` the launcher's OWN ANCESTOR --
# the runner's wrapper shell -- holds the case directory as cwd; a guard that
# excluded only $$ refused its own launch (measured: three zero-compute refusals
# per rung, three rungs on 2026-08-26).  This launcher's own lineage (itself,
# its ancestors up to pid 1, its descendants) is excluded from the scan; ANY
# FOREIGN process whose cwd is the case directory is still refused.
...
# --- 3b. MEASURED geometry, written by OpenFOAM into 0/ before 0/T is touched -
if ! "$POST_PATH" -case "$CASE_DIR" -func writeCellCentres -time 0 > "$CASE_DIR/log.writeCellCentres" 2>&1; then
    rm -rf "$CASE_DIR/0"
    echo "REFUSE: writeCellCentres failed; the comparator would have no MEASURED cell centres and T8 GROUND 2 forbids asserting them. No solver started, no STATUS written." >&2; exit 2; fi
if ! "$POST_PATH" -case "$CASE_DIR" -func writeCellVolumes -time 0 > "$CASE_DIR/log.writeCellVolumes" 2>&1; then
    rm -rf "$CASE_DIR/0"
    echo "REFUSE: writeCellVolumes failed; the comparator would have no MEASURED cell volumes. No solver started, no STATUS written." >&2; exit 2; fi
for f in Cx Cz V; do
    [ -f "$CASE_DIR/0/$f" ] || { rm -rf "$CASE_DIR/0"; echo "REFUSE: 0/$f was not written; no solver started, no STATUS written" >&2; exit 2; }
done

# --- 3c. 0/T touched LAST: it is the age-guard datum -------------------------
sleep 1
