...
R3 = os.path.join(TF, "T25R3_MODULE_runs")
EXIT_REFUSE = 2
...
MESH_SRC = {
    ...
    "L3": os.path.join(R3, "_meshsrc_L3"),
}
...
RUNS = {
    ...
    # THE PROBE (section 2).  Leg A only, endTime 2.0 s = 100 steps at dt 0.02,
    # so it covers the t = 0 -> 1 s ramp, the hardest part of the transient.
    "P1":  ("L1", 0.02,  0.1,   15, 100,   0),
    "P2":  ("L2", 0.02,  0.1,   15, 100,   0),
    "P3":  ("L3", 0.02,  0.1,   15, 100,   0),
}
PROBE = ("P1", "P2", "P3")
PROBE_END = 2.0
...
# ==========================================================================
# THE T25R4 FIX -- section 1.  A FIXED REDUCTION FACTOR ON EVERY LEVEL, with a
# floor PROVED (section 1.2) not to bind on any of them.
#
# T25R3 registered `tolerance 1e-08; relTol 0` on p_rghFinal and
# `tolerance 1e-08; relTol 0.01` on the intermediate solves.  `tolerance` is
# ABSOLUTE, and the absolute scale of an OpenFOAM residual moves with cell
# count, so it is a DIFFERENT PHYSICAL DEMAND ON EVERY MESH.  Measured median
# reduction actually achieved: L1 x1.00, L2 x1.51, L3 x100.94.  ON L1 THE
# PRESSURE SOLVE DID NOTHING.
#
# NOTE WHAT IS AND IS NOT CHANGING.  `relTol 0.01` WAS ALREADY REGISTERED on the
# intermediate solves and NEVER BOUND, because the absolute 1e-08 short-circuited
# it.  Lowering the floor does not tighten the demand; IT STOPS THE FLOOR FROM
# CANCELLING THE DEMAND THAT WAS ALREADY THERE.
# ==========================================================================
...
def _patch_pressure_criterion(case):
    """Replace the two p_rgh solver entries in system/coolant/fvSolution.

    Structural, not textual: the two blocks are located by their own keys and
    replaced whole.  REFUSES if either key is absent or appears twice, because a
    silently-unpatched criterion is the exact defect this rung exists to repair
    and it would leave every other check green."""
    p = os.path.join(case, "system", "coolant", "fvSolution")
    txt = open(p).read()
    out, i, n_rep = [], 0, 0
    for key in ('"p_rgh.*"', "p_rghFinal"):
        if txt.count("\n    %s\n" % key) + txt.count("\n    %s\n" % key) == 0:
            refuse("%s: solver key %s not found; the pressure criterion cannot "
                   "be patched and an unpatched criterion IS the defect" % (p, key))
    lines = txt.split("\n")
    res, k = [], 0
    while k < len(lines):
        st = lines[k].strip()
        if st in ('"p_rgh.*"', "p_rghFinal"):
            depth, k2 = 0, k + 1
            while k2 < len(lines):
                depth += lines[k2].count("{") - lines[k2].count("}")
                if depth == 0 and "}" in lines[k2]:
                    break
                k2 += 1
            n_rep += 1
            if n_rep == 1:
                res.append(P_RGH_BLOCK.rstrip("\n"))
            k = k2 + 1
            continue
        res.append(lines[k]); k += 1
    if n_rep != 2:
        refuse("%s: expected exactly 2 p_rgh solver blocks, replaced %d"
               % (p, n_rep))
    open(p, "w").write("\n".join(res))
    chk = open(p).read()
    for want in ("tolerance       1e-13", "relTol          1e-3",
                 "relTol          0.01"):
        if want not in chk:
            refuse("%s: post-write check failed, %r absent" % (p, want))
    # PRECISION MATTERS IN A GUARD.  An earlier version of this check swept the
    # WHOLE solvers block for `tolerance 1e-08` and fired on the `rho` solver's
    # own perfectly legitimate 1e-08 -- aborting the stage half-way through and
    # looking exactly like the defect it was written to catch.  A guard that
    # fires on a healthy case is not strict, it is broken.  Check ONLY the two
    # blocks this function rewrote.
    # THIRD TIME THIS LANE HAS WRITTEN AN OVER-BROAD GUARD TODAY, SO THE RULE
    # GOES IN THE CODE: A GUARD THAT GREPS A REGION MUST DEFINE THAT REGION BY
    # ITS OWN DELIMITERS, NOT BY THE NEXT KEYWORD THAT HAPPENS TO APPEAR.
    #   - sweeping relaxationFactors for a quote caught its own explanatory
    #     COMMENT;
    #   - sweeping the solvers block for `tolerance 1e-08` caught the `rho`
    #     solver's legitimate value;
    #   - sweeping from "p_rgh.*" to the next "PIMPLE" caught
    #     `"(U|h|k|omega)Final"`'s legitimate `relTol 0`.
    # Each fired on a HEALTHY case and each looked exactly like the defect.
    # The precise check is that the block written above is present VERBATIM.
    if P_RGH_BLOCK.rstrip("\n") not in chk:
        refuse("%s: the registered p_rgh block is not present verbatim after "
               "the write -- an unpatched criterion IS the defect this rung "
               "exists to repair, and it would leave every other check green" % p)
    return True
...
def stage(run):
    ...
    _patch_pressure_criterion(case)
    ...
    endA = PROBE_END if run in PROBE else LEG_A_END
    w(case, "system/controlDict", "dictionary", "controlDict",
      control_dict("A", dtA, endA, "startTime", stepsA, run),
      location="system")
...
def selftest():
    ...
    for run, (lv, dtA, dtB, n, sa, sb) in RUNS.items():
        ...
        if run in PROBE:
            chk("%s: PROBE, leg A only, %d steps == %g/%g"
                % (run, sa, PROBE_END, dtA), abs(sa - PROBE_END / dtA) < 1e-6)
            continue
