...
REG_RANKS=8; REG_TIMEOUT_S=35424; REG_CAP_CORE_MIN=4723     # registered, T3e_PREREGISTRATION.md S5
...
[ "$RANKS" = "$REG_RANKS" ] || { echo "REFUSE: --ranks=$RANKS; R_fy is registered at $REG_RANKS ranks (T3e prereg S9)" >&2; exit 2; }
...
[ "$CASE" = "R_fy" ] || { echo "REFUSE: this launcher is registered for R_fy only; got $CASE" >&2; exit 2; }
...
[ -f "$CASE_DIR/constant/polyMesh/points" ] || { echo "REFUSE: no mesh -- the birth certificate (build_t3d.py R_fy) has not been issued" >&2; exit 2; }
