#!/usr/bin/env bash
# LAUNCHER for ONE registered T25R case.
#
# Registered at docs/campaigns/T-family/T25R_PREREGISTRATION.md sections 1, 6.4
# and 8.2.  Written and committed BEFORE ANY COMPUTE.
#
...
# `|| { echo ABORT ...; exit 1; }`.
#
# Usage: run_one_t25R.sh <CASE> <TIMEOUT_S> <CAP_CORE_MIN>
...
case "$CASE" in
  T25R_L1|T25R_L2|T25R_L2_DT025) ;;
  *) echo "ABORT: $CASE is not a registered T25R case (section 1)"; exit 1 ;;
esac
...
[ -d "$D/0.orig" ] || { echo "ABORT: no $D/0.orig -- run build_t25R.py C first"
                        exit 1; }
[ -d "$D/constant/coolant/polyMesh" ] && [ -d "$D/constant/module/polyMesh" ] || {
  echo "ABORT: both region meshes must exist -- run build_t25R.py B first"
  exit 1; }
...
# record; mark_done_t25R.py DERIVES conjunct 1 from log.solve regardless,
