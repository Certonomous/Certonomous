#!/usr/bin/env python3
"""STRICT COMPLETION (CLAUDE.md rule 4), all-or-nothing, for the THREE T25R runs.

Gates registered at `docs/campaigns/T-family/T25R_PREREGISTRATION.md` section 6.4.

THIS FILE IS FROZEN BEFORE ANY COMPUTE.  No solver, no `blockMesh`, no
`checkMesh` has run for T25R.  The completion rule therefore cannot have been
shaped by a result it had already seen, which is the whole evidentiary content of
CLAUDE.md rule 2.

PATTERN.  This is `verification/runs/T-family/T24_runs/mark_done_t24.py` in
shape -- the marker of the nearest multi-region conjugate rung on disk -- with
the registered differences named below.  T24's was T23's with the L-342
physics/infrastructure split, and that split is what the rc question turns on.
  ...
  DIFFERENCE 1.  THE FIELD TUPLE IS THIS RUNG'S, NOT AN INHERITED ONE.
    ...
    THAT TUPLE IS WRONG FOR T25R and is not inherited.  T25R has a fluid region.

  DIFFERENCE 2.  THE AGE-GUARD REFERENCE IS `0/module/T`, not `0/T`.  A
    multi-region case has no `0/T`.  `run_one_t25R.sh` touches `0/module/T`
  ...
  DIFFERENCE 3.  THE ExecutionTime CONJUNCT IS A STEP-COUNT IDENTITY, NOT A
    ...
    PER ADVANCED STEP WITH NO TRUNCATED TAIL.  T25R registers
...
THE CAP IS A NAMED OUTCOME, NOT A SURPRISE (T25R section 8.2, CLAUDE.md rule
...
Usage: python3 mark_done_t25R.py [CASE ...] [--root DIR] | --selftest
...
# T25R section 1, THE REGISTERED RUN SET.  Three, named, and closed.
...
AGE_REF = ("0", "module", "T")            # T25R section 6.4 conjunct 6
END_TIME = 900.0                          # T25R section 3.4
...
def check(root, case):
    ...
    if abs(et - END_TIME) > 1e-9:
        fails.append("controlDict endTime %g != the REGISTERED %g (T25R 3.4) -- "
    ...
    if dt <= 0.0 or abs(et / dt - want_steps) > 1e-6:
        fails.append("controlDict deltaT %g over endTime %g gives %.6f steps, "
                     "but T25R 6.4 registers %d for %s"
    ...
    if ats is not None and ats.lower() in ("yes", "true", "on", "1"):
        fails.append("controlDict has adjustTimeStep %s -- T25R 3.4 registers "
    ...
    # --- THE CAP, NAMED WHEN ITS EVIDENCE SURVIVES (T25R 8.2, rule 12).
    ...
    for case in want:
        ...
        if fails:
            ...
            if os.path.exists(marker):
                os.remove(marker)
                print("STALE     %-14s - DONE marker REMOVED: the case no "
                      "longer satisfies rule 4" % case)
        else:
            if not os.path.exists(marker):
                open(marker, "w").write("done\n")
            print("DONE      %-14s - all six conjuncts hold" % case)
    return rc
...
def selftest():
    ...
    root = tempfile.mkdtemp(prefix="t25Rmd_")
    try:
        C = "T25R_L2"
        _forge(root, C)
        ...
        chk("T25R_L2_DT025 at deltaT 0.25 / 3600 steps is DONE",
            run(root, [D]) == EXIT_OK)
        shutil.rmtree(os.path.join(root, D))
        _forge(root, D, dt=0.25, n_exec=1800)
        chk("T25R_L2_DT025 with 1800 ExecutionTime lines -> NOT DONE",
