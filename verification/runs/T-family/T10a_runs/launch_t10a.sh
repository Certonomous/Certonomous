#!/bin/bash
# Idempotent, collision-safe foreground launcher for ONE T10a case
# (same guards as T1_runs/launch_dts.sh; the box is shared with the T3 rung
# and NOTHING here may touch a process it did not start).
...
# Then detach run_one_t10a.sh (0/ re-copied from 0.orig, checkMesh ->
# log.checkMesh, buoyantSimpleFoam -> log.solve, STATUS.<case>
# "rc= wall= checkMesh_rc=") via setsid nohup.
...
cd "$HERE" || exit 4
setsid nohup "$HERE/run_one_t10a.sh" "$CASE" > /dev/null 2>&1 &
WP=$!
