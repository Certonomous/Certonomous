#!/usr/bin/env python3
"""T14 -- the FROZEN comparator.  2-D transient conduction in a square (T11b),
Bi = 1, Fo_end = 0.20, laplacianFoam, three levels N = 50/100/200.

SOLID ONLY.  This rung earns `V` for 2-D TRANSIENT CONDUCTION and nothing else.
...
def load_registered(root=None):
    p = os.path.join(root or HERE, "T14_registered.json")
    if not os.path.isfile(p):
        refuse("no T14_registered.json -- the gate is not registered")
...
def read_field(case_dir, time, N):
    """THE PRODUCTION READER: (N x N nested list, x-fastest) of T at `time`.
    ...
    return [vals[j * N:(j + 1) * N] for j in range(N)]     # [j][i] : y-row j, x-col i


def mean_of(F):
    n = len(F)
    return sum(sum(r) for r in F) / (n * n)
...
def theta_at(F, xs, ys):
    """Separable linear interpolation (linear extrapolation past the last
    centre) to a FIXED (x*, y*): the graded quantity is the same quantity on
    every level.  Interpolation error enters the triple and is disclosed."""
    n = len(F)
    cs = [(i + 0.5) / n for i in range(n)]
    col = [_interp1(cs, F[j], xs) for j in range(n)]
    return _interp1(cs, col, ys)


def symmetry_witness(F):
    """theta(x,y) - theta(y,x), max abs: the solution is symmetric under
    transposition, so this is a reader/solver sanity witness (W0)."""
    n = len(F)
    return max(abs(F[j][i] - F[i][j]) for j in range(n) for i in range(n))
...
def planted_zero_control(case_dir, time, N, readers):
    ...
    one-cell plant at the cell nearest their station (located by index, never
    by value); the AVERAGING reader (G1, mean over N*N cells) gets an ALL-CELL
    plant so the read moves by ~PLANT rather than PLANT/N^2.  Measured ladder.
    Copies first; never writes into the case."""
    tmp = tempfile.mkdtemp(prefix="t14pz_")
    try:
        ...
        dneg = max(abs(a - b) for ra, rb in zip(base, again) for a, b in zip(ra, rb))
        if dneg != 0.0:
        ...
        for name, fn, idxs in readers:
            ...
            if floor is None:
                refuse("planted-zero control %s POSITIVE ARM FAILED: no plant magnitude was visible -- the reader is BLIND" % name)
...
def grade(root, json_out, reg, ct=True):
    for lv, case in CASES.items():
        if not os.path.isfile(os.path.join(root, "DONE.%s" % case)):
            refuse("no DONE.%s -- the whole rung is graded or none of it is; mark_done_t14.py rules" % case)
    ...
    for lv in LEVELS:
        ...
        fields[lv] = F
    w0 = {lv: symmetry_witness(fields[lv]) for lv in LEVELS}
    ...
    print("  W0 symmetry witness: " + ", ".join("%s %.2e" % (lv, w0[lv]) for lv in LEVELS))

    Nf = Ns["f"]
    readers = (("G1", mean_of, list(range(Nf * Nf))),
               ("G2", lambda F: theta_at(F, 0.0, 0.0), [0]),
               ("G3", lambda F: theta_at(F, 1.0, 0.0), [Nf - 1]))
    pz = planted_zero_control(dirs["f"], times["f"], Nf, readers)

    rows = []
    R = reg["graded_rows"]
    ROWS = (("G1", mean_of, S.theta_mean(Fo)),
            ("G2", lambda F: theta_at(F, 0.0, 0.0), S.theta(0.0, 0.0, Fo)),
            ("G3", lambda F: theta_at(F, 1.0, 0.0), S.theta(1.0, 0.0, Fo)))
    for rid, fn, ref in ROWS:
        ...
        vals = {lv: fn(fields[lv]) for lv in LEVELS}
        tr = triple_of(vals)
        band = R[rid]["band_rel"]
    ...
    out = dict(rung="T14", scope="SOLID-ONLY 2-D transient conduction (T11b); NOT conjugate", Bi=Bi, Fo_end=Fo,
               N=Ns, refinement_ratio=REFINEMENT, dim=DIM, factor_of_safety=FS,
...
def _forge_case(root, lv, N, S, Fo, Bi, err=0.0, resid=1e-12, asym=0.0, with_ct=False):
    """A forged level: the analytic field at cell centres plus a smooth
    second-order-shaped deviation err*(1 + x*)(1 + y*)/N^2 (so p = 2)."""
    import time as _t
    case = "T14_SQ_%s" % lv
    d = os.path.join(root, case)
    ...
    open(os.path.join(d, "CASE.txt"), "w").write(
        "case=%s\nlevel=%s\nN=%d\ncells=%d\nBi=%g\nFo_end=%.17g\nvalueFraction=%.17g\nendTime=2\ndeltaT=0.0001\n"
        % (case, lv, N, N * N, Bi, Fo, Bi / (Bi + 2.0 * N)))
    vals = []
    for j in range(N):
        for i in range(N):
            xs, ys = (i + 0.5) / N, (j + 0.5) / N
            v = S.theta(xs, ys, Fo) * (1.0 + err * (1 + xs) * (1 + ys) / (N * N)) + asym * xs
            vals.append(v)
...
def selftest():
    ...
    for label, vals, want, want_gci in (
        ...
        if not ok:
            fails.append(label)
    # (iii) the VALUE CONTROL: a forged ladder from the analytic field grades PASS, p = 2
    Bi, Fo = reg["physics"]["Bi"], reg["physics"]["Fo_end"]
    S = EX.verify(Bi, Fo, quiet=True)
    def run_forged(**kw):
        tmp = tempfile.mkdtemp(prefix="t14forge_")
        try:
    ...
    if not ok:
        fails.append("resid")
    code, res = run_forged(err=1e-2, asym=1e-6)
    ok = (code == 0 and res and all(r["verdict"] == "NOT A RESULT" for r in res["rows"]) and not res["gate1"]["ok"])
    print("  [%s] symmetry witness W0 broken by 1e-6 -> gate (1) -> NOT A RESULT x3" % ("ok " if ok else "FAIL"))
    if not ok:
        fails.append("w0")
    code, res = run_forged(err=1.0)
    ok = (code == 0 and res and {r["row"]: r["verdict"] for r in res["rows"]}["G1"] == "GATE FAIL")
    print("  [%s] forged deviation 1.0/N^2 (2.5e-5 at f, outside the G1 band) -> GATE FAIL" % ("ok " if ok else "FAIL"))
    if not ok:
        fails.append("gate-fail")
    # (iv) a BLIND reader must be refused by the planted-zero control
    global read_field
    real = read_field
    tmp = tempfile.mkdtemp(prefix="t14blind_")
    fired = False
    try:
        _forge_case(tmp, "f", 200, S, Fo, Bi, err=1e-2)
        frozen = real(os.path.join(tmp, "T14_SQ_f"), "2", 200)
        read_field = lambda d, t, N: frozen           # ignores the disk
        try:
            planted_zero_control(os.path.join(tmp, "T14_SQ_f"), "2", 200,
                                 (("G1", mean_of, list(range(200 * 200))),))
        except SystemExit as e:
            fired = (e.code == EXIT_REFUSE)
    finally:
        read_field = real
    ...
    fired = False
    try:
        grade(HERE, os.path.join(tempfile.gettempdir(), "t14_never.json"), reg)
    except SystemExit as e:
        fired = (e.code == EXIT_REFUSE)
    ...
    ok = (n0 == 0 and n1 == 1)
    print("  [%s] AST assert count in this file = %d (counter sees a planted assert: %d)" % ("ok " if ok else "FAIL", n0, n1))
    if not ok:
        fails.append("ast")
...
def main():
    ...
    ap.add_argument("--json", default=os.path.join(HERE, "gate_t14.json"))
    a = ap.parse_args()
    if a.selftest:
        return selftest()
