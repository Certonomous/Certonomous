#!/usr/bin/env bash
# ==========================================================================
# T20 CASE LAUNCHER (lumped-capacitance control, chtMultiRegionFoam, SOLID
# ONLY -- regionProperties carries `fluid ()`).  Runs ONE registered case,
# captures the SOLVER's rc IN THIS WRAPPER, writes STATUS.<case>.
#
# Modelled line-for-line on run_one_t18.sh, whose pattern is proven on this
# family.  What changed for T20, and only these:
#   * the mesh and fields live under a REGION: constant/<region>/polyMesh and
#     0.orig/<region>/T, so the guards look there;
#   * checkMesh takes -region <region>;
#   * the solver is chtMultiRegionFoam.
#
# The properties inherited unchanged, each paid for by a specific failure:
...
#     around a setsid line reports 0 for every outcome;
#   * `capped` = (wall_s >= timeout_s) is an INDEPENDENT expiry witness -- rc
#     124 collides with a solver that itself exits 124, and 137 with an OOM
#     kill; `capped` is an INFRASTRUCTURE field (L-342): it LABELS a non-zero
#     rc, it never voids a run;
...
#   * the registered cap is READ from T20_registered.json and a --timeout that
#     is not EQUAL to it is REFUSED -- rule 12: a caller may neither widen a cap
#     that stops the run nor narrow one into a manufactured cap-stop.
...
#   * the OpenFOAM bashrc is sourced with `set -u` lifted;
...
#     rule 4's age-guard datum and must date the run allowed to produce the
#     answer;
#   * `exit "$RC"` is the LAST line: the wrapper reports the solver's own status
#     to its caller, never 0.
#
# usage: run_one_t20.sh --case-dir DIR --timeout SECONDS [--ranks 1]
#                       [--solver chtMultiRegionFoam] [--region cellRegion]
...
REGJSON="$SELF/T20_registered.json"

usage() { sed -n '/^# usage:/,/^# ====/p' "$0" >&2; exit 2; }

CASE_DIR=""; TIMEOUT_S=""; RANKS=1
SOLVER="chtMultiRegionFoam"; REGION="cellRegion"; DETACH=1
...
[ "$RANKS" = "1" ] || { echo "REFUSE: T20 is registered SERIAL (S1 line 6: 1 rank, no decomposeParDict); --ranks=$RANKS" >&2; exit 2; }
...
if [ -f "$REGJSON" ]; then
    ...
    case "$REG_T" in
        UNREGISTERED) echo "REFUSE: $CASE is not a registered T20 case in $REGJSON" >&2; exit 2 ;;
    ...
    REG_REGION=$(python3 -c "import json,sys; print(json.load(open(sys.argv[1]))['region'])" "$REGJSON" 2>/dev/null || echo UNREADABLE)
    [ "$REGION" = "$REG_REGION" ] || { echo "REFUSE: --region $REGION is not the registered region $REG_REGION" >&2; exit 2; }
    REG_SOLVER=$(python3 -c "import json,sys; print(json.load(open(sys.argv[1]))['solver'])" "$REGJSON" 2>/dev/null || echo UNREADABLE)
    [ "$SOLVER" = "$REG_SOLVER" ] || { echo "REFUSE: --solver $SOLVER is not the registered solver $REG_SOLVER. THE WHOLE POINT OF THIS FEASIBILITY RUN IS WHICH SOLVER IT IS; a substitution here would silently change what the rung demonstrates (S9)." >&2; exit 2; }
else
...
if [ "$DETACH" = "1" ] && [ "${T20_DETACHED:-}" != "1" ]; then
    T20_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" \
        --ranks "$RANKS" --solver "$SOLVER" --region "$REGION" \
        --foam-bashrc "$FOAM_BASHRC" --no-detach </dev/null >>"$CASE_DIR/log.launch" 2>&1 &
...
[ -d "$CASE_DIR/0.orig/$REGION" ] || { echo "REFUSE: $CASE has no 0.orig/$REGION to arm from" >&2; exit 2; }
[ -f "$CASE_DIR/constant/$REGION/polyMesh/owner" ] || { echo "REFUSE: $CASE has no mesh (constant/$REGION/polyMesh/owner) -- run build_t20.py" >&2; exit 2; }
[ -f "$CASE_DIR/constant/regionProperties" ] || { echo "REFUSE: $CASE has no constant/regionProperties -- the solid-only configuration is unstated" >&2; exit 2; }
for f in T p; do
    [ -f "$CASE_DIR/0.orig/$REGION/$f" ] || { echo "REFUSE: 0.orig/$REGION/$f missing -- the registered field set is incomplete" >&2; exit 2; }
done

# LINEAGE-AWARE foreign-process guard (T10aR2 AMENDMENT 1, 9fa66065): under the
# queue runner's launch form the launcher's OWN ANCESTOR holds the case directory
# as cwd; a guard that excluded only $$ refused its own launch.
...
# --- 1b. provenance: no cross-family token in this case ----------------------
PROV="/home/ubuntu/Certonomous/scripts/check_case_provenance.py"
[ -f "$PROV" ] || PROV="$SELF/../../../../scripts/check_case_provenance.py"
...
# --- 3. arm 0/ from 0.orig; 0/<region>/T touched LAST (the age-guard datum) --
