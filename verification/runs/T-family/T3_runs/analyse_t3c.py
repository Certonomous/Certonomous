#!/usr/bin/env python3
"""T3c successor comparator — the triple (R_m, R_f, R_ff) graded through the FROZEN
...
GATE_JSON = os.path.join(HERE, "gate_t3c.json")
...
def grade(root, ladder=LADDER, out=print):
    ...
    tally = {v: sum(1 for r in rows if r["kind"] == "GRADE" and r["verdict"] == v)
             for v in A.VERDICTS}
    result = dict(rung="T3c -- fourth level, repaired rule-3 control", ladder=ladder,
