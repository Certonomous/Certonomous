#!/usr/bin/env python3
"""T3g comparator — the triple (R_m, R_f, R_fz) graded through the FROZEN
...
LADDER level f R_ff -> R_fz, the output filename, the confound wording, the cost
key, and the rung label.  NO predicate, threshold, gate, band or verdict rule
differs.  `R_fz` is R_ff's SAME MESH continued from iteration 118 000, so this is
not a new level -- it is the same level iterated further, and the ladder's two
coarse members are byte-identical to T3c's.

Registered by T3g_PREREGISTRATION.md and frozen BEFORE R_fz has iterated: at the
freeze commit no R_fz time directory, STATUS or DONE marker exists.
...
LADDER = {"c": "R_m", "m": "R_f", "f": "R_fz"}
LEVELS = A.LEVELS
P_MIN = 0.5                     # the floor A.gci_unequal applies; T11 352aef0d
N_ULP = 32                      # smallest admissible power of two; see docstring
GATE_JSON = os.path.join(HERE, "gate_t3g.json")
CONFOUND = ("R_fz and R_ff ran decomposed (8 ranks, simple (8 1 1)); R_m and R_f ran serial. "
...
def grade(root, ladder=LADDER, out=print):
    ...
    cost = dict(R_fx_status=st, core_min_from_status=st.get("core_min"),
                cost_basis="wall x ranks / 60 from STATUS.R_fz; dollars derived, not measured")
    tally = {v: sum(1 for r in rows if r["kind"] == "GRADE" and r["verdict"] == v)
             for v in A.VERDICTS}
    result = dict(rung="T3g -- fourth level CONTINUED (R_fz), repaired rule-3 control", ladder=ladder,
