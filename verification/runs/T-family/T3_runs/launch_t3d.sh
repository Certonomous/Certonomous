...
REG_RANKS=8; REG_TIMEOUT_S=122445; REG_CAP_CORE_MIN=16326     # registered, T3d_PREREGISTRATION.md S5
...
[ "$RANKS" = "$REG_RANKS" ] || { echo "REFUSE: --ranks=$RANKS; R_fx is registered at $REG_RANKS ranks (T3d prereg S5)" >&2; exit 2; }
...
[ "$CASE" = "R_fx" ] || { echo "REFUSE: this launcher is registered for R_fx only; got $CASE" >&2; exit 2; }
...
[ -f "$CASE_DIR/constant/polyMesh/points" ] || { echo "REFUSE: no mesh -- the birth certificate (build_t3d.py R_fx) has not been issued" >&2; exit 2; }
...
if [ "$DETACH" = "1" ] && [ "${T3D_DETACHED:-}" != "1" ]; then
    T3D_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" --ranks "$RANKS" --no-detach \
