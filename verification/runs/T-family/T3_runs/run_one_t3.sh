#!/bin/bash
# Detached runner for one T3 case: 0.orig -> 0, checkMesh, solve, then write
# STATUS.<case> beside the case directory in the T1b pool format
# (rc= wall= checkMesh_rc=).  checkMesh already ran at build time; it is re-run
# here so checkMesh_rc in STATUS is measured by the run that produced the
# solution, not inherited from the build.
# no `set -u`: the openfoam2606 bashrc reads unset variables and aborts under nounset
#
# COLLISION GUARDS (defence in depth; launch_t3.sh applies the same guards in
...
#   G1  <case>/LAUNCH_LOCK must exist (created atomically by launch_t3.sh) and
...
#       (mark_done_t3.py test 6, L-143).  A pre-existing 0 means some earlier
#       run started; it is never overwritten or merged into.
# Nothing here ever kills a process.
...
[ -d "$LOCK" ] || refuse "G1: no LAUNCH_LOCK (not launched via launch_t3.sh)"
