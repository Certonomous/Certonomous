#!/usr/bin/env bash
# ==========================================================================
# T15 CASE LAUNCHER (UNSTEADY MTT plume, buoyantBoussinesqPimpleFoam) -- run
# the ONE registered level, capture the SOLVER's rc in this wrapper, write
# STATUS.<case>.  Modelled line-for-line on run_one_t14.sh (freeze 5a870e54),
...
#   * the registered cap is READ from T15_registered.json and a --timeout that
...
#     T15's comparator reads Cx/Cz/V that OpenFOAM wrote and asserts no ratio.
...
#
# usage: run_one_t15.sh --case-dir DIR --timeout SECONDS [--ranks 1]
...
REGJSON="$SELF/T15_registered.json"
...
[ "$RANKS" = "1" ] || { echo "REFUSE: T15 is registered SERIAL (ranks 1); --ranks=$RANKS" >&2; exit 2; }
...
if [ -f "$REGJSON" ]; then
    ...
    case "$REG_T" in
        UNREGISTERED) echo "REFUSE: $CASE is not a registered T15 case in $REGJSON" >&2; exit 2 ;;
...
if [ "$DETACH" = "1" ] && [ "${T15_DETACHED:-}" != "1" ]; then
    T15_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" \
...
[ -f "$CASE_DIR/constant/polyMesh/owner" ] || { echo "REFUSE: $CASE has no mesh (constant/polyMesh/owner) -- run build_t15.py then blockMesh" >&2; exit 2; }
for f in T U p_rgh k epsilon nut alphat; do
