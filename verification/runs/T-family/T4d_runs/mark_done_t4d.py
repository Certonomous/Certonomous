#!/usr/bin/env python3
"""Turn T4d STATUS files into DONE markers under the STRICT COMPLETION RULE,
...
PHYSICS-CRITICAL (each a conjunct; any failure -> NOT DONE, no marker):
  1. STATUS.<case> EXISTS and carries the solver's own rc, written INSIDE the
     launch wrapper (launch_t4d.sh runs the solver in its foreground under
...
Usage:  python3 mark_done_t4d.py [CASE ...] [--root DIR] | --selftest
...
ROOT = HERE
CASES = ("T4d_IJ_c", "T4d_IJ_m", "T4d_IJ_f")
...
def main(argv):
    ...
    for c in want:
        if c not in CASES:
            refuse("%r is not one of the registered %s cases: %s" % (c, "T4d", " ".join(CASES)))
    rc = EXIT_OK
    for case in want:
        fails, nm = check(case)
