#!/usr/bin/env bash
# ==========================================================================
# T4d CASE LAUNCHER -- modelled on scripts/launch_k0f.sh: rc-CAPTURING,
...
#     directory, the file mark_done_t4d.py reads;
...
#   * the registered cap is READ from T4d_registered.json and a --timeout that
...
#
# usage: launch_t4d.sh --case-dir DIR --timeout SECONDS [--ranks 1]
...
REGJSON="$SELF/T4d_registered.json"
...
[ "$RANKS" = "1" ] || { echo "REFUSE: T4d is registered SERIAL (ranks 1 on every level); --ranks=$RANKS" >&2; exit 2; }
...
if [ -f "$REGJSON" ]; then
    ...
    case "$REG_T" in
        UNREGISTERED) echo "REFUSE: $CASE is not a registered T4d case in $REGJSON" >&2; exit 2 ;;
...
if [ "$DETACH" = "1" ] && [ "${T4D_DETACHED:-}" != "1" ]; then
    T4D_DETACHED=1 exec setsid "$0" --case-dir "$CASE_DIR" --timeout "$TIMEOUT_S" \
