#!/usr/bin/env python3
"""T4d impinging jet -- the FROZEN comparator (T4b's, rename-only), with the section-9 controls ON THE
GRADING PATH.
...
REG = json.load(open(os.path.join(HERE, "T4d_registered.json")))
LEVELS = ("c", "m", "f")
CASES = {lv: "T4d_IJ_%s" % lv for lv in LEVELS}
...
if (float(_RF["STAGNANT_FLOOR"]), float(_RF["P_MIN"])) != (STAGNANT_FLOOR, P_MIN):
    print("REFUSE: T4d_registered.json roache_floors %r disagree with scripts/roache_triple.py "
...
def scratch_copy(case_dir, time):
    ...
    tmp = tempfile.mkdtemp(prefix="t4d_")
...
def grade(a):
    prov = {"analyse_t4.py (frozen, imported)": sha256_of(os.path.join(T4DIR, "analyse_t4.py")),
            "analyse_t4d.py": sha256_of(os.path.abspath(__file__)),
            "T4d_registered.json": sha256_of(os.path.join(HERE, "T4d_registered.json"))}
    print("T4d comparator.  shared floors STAGNANT_FLOOR = %g, P_MIN = %g (scripts/roache_triple.py).  "
    ...
    for rid, rd, reffile, half in GRADED:
        ...
        print("%-3s r/D=%.1f fine=%.4f ref=%.4f band=[%.4f,%.4f] triple=%s p=%s GCI=%s -> %s%s"
              % (rid, rd, vals["f"], ref, ref - half, ref + half, state,
                 ...
                 (" [" + note + "]") if note else ""))
    out = dict(rung="T4d", STAGNANT_FLOOR=STAGNANT_FLOOR, P_MIN=P_MIN,
...
def drive_refusal(which, a):
    ...
    if which == "missing-done":
        d = tempfile.mkdtemp(prefix="t4d_nodone_")
        try:
            global HERE
            HERE = d
            for lv, case in CASES.items():
