...
LEVEL_FACTOR = {"K2f_L1": 1.0, "K2f_L2": 1.5, "K2f_L3": 2.25}
LEVELS = ("K2f_L1", "K2f_L2", "K2f_L3")
TARGETS = {"K2f_L1": 59259, "K2f_L2": 200000, "K2f_L3": 675000}

MESHSIM_LO, MESHSIM_HI = 3.2063, 3.5438

# --- K2f ADDITIONS (registration sections 3, 9.4) --------------------------
# writeInterval WELL BELOW endTime so a trip leaves a restart point. K2d used
# writeInterval 3000 against endTime 3000, so K2f_L3's kill at iteration 995
# destroyed 100 % of 535.600 core-minutes having written nothing at all.
WRITE_INTERVAL = 500
MONITOR_INTERVAL = 50          # the registered G-CYCLE cadence (s.3, s.6.1)
HA_ZONE = "haZone"
...
def _system_and_constant(case, level):
    ...
    with open(os.path.join(sysd, "controlDict"), "w") as fh:
        fh.write(_hdr("dictionary", "controlDict", "system")
                 ...
                 % WRITE_INTERVAL
                 + _functions_block())
    # topoSetDict: the hot-aisle cellZone `U_ha` is volume-averaged over.
    # Box = full W_ha in y (2.3 -> 3.5), the rack row's first-to-last face in x
    # (0.6 -> 3.0), floor to rack top H_r in z (0 -> 2.0). Registration s.3.
    with open(os.path.join(sysd, "topoSetDict"), "w") as fh:
        fh.write(_hdr("dictionary", "topoSetDict", "system")
                 + "actions\n(\n"
...
def build(level, root=None, run_mesh=True):
    if level not in LEVELS:
        refuse("%r is not a K2f level %s" % (level, ", ".join(LEVELS)))
    root = root or HERE
    ...
    # THE HOT-AISLE CELLZONE. `U_ha` is a volAverage over a cellZone, so the
    # zone must exist before the solver starts. A build whose topoSet fails
    # produces a case whose G-CYCLE input CANNOT be written -- which is K2d's
    # defect in a different costume -- so this REFUSES rather than warning.
    r = _sh("topoSet > log.topoSet 2>&1", case)
    if r.returncode != 0:
        refuse("topoSet failed for %s (rc=%d); the %s cellZone does not exist "
               "and U_ha could not be written. See %s/log.topoSet"
               % (level, r.returncode, HA_ZONE, case))
    zpath = os.path.join(case, "constant", "polyMesh", "cellZones")
    if not os.path.exists(zpath):
        refuse("topoSet returned 0 but wrote no constant/polyMesh/cellZones for "
               "%s. UNMEASURED IS NOT PASSING: an rc of 0 is not evidence the "
               "zone exists." % level)
...
def selftest():
    ...
    try:
        res = build("K2f_L1", root=root, run_mesh=False)
        case = res["case"]
        ...
        arm_refuses("CLAUSE 7: a pre-existing 0/ -> builder REFUSES",
                    lambda: build("K2f_L1", root=root, run_mesh=False))
    ...
    try:
        res = build("K2f_L1", root=root2, run_mesh=True)
        case2 = res["case"]
