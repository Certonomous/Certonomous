#!/usr/bin/env python3
"""analyse_k2bU3R2.py -- grade the K2b-U3-R2 successor run (the un-confounded
Test D at the spec's own 60 mm coarse mesh, 80 s) against the SAME gate as its
frozen predecessor, plus the planted-zero control that predecessor lacks.

    python3 analyse_k2bU3R2.py            # grade the true run
    python3 analyse_k2bU3R2.py --selftest # prove the planted control fires

WHAT IS RE-USED, UNCHANGED, FROM THE FROZEN PREDECESSOR analyse_k2bU3.py
(git blob d1500a5f..., grading K2bU3R2_PREREGISTRATION.md, itself citing
...
K2bU3R2_PREREGISTRATION.md §5.1 requires and the frozen predecessor lacks
...
the case directory is re-pointed (60 mm K2bU3R2_D60) and the planted control is
...
# The successor's own 60 mm case (K2bU3R2_PREREGISTRATION.md §5.3), and the same
...
def planted_zero_control(src_log, rxs, reader=window_p2p_via_series):
    ...
    tmp = tempfile.mkdtemp(prefix="k2bU3R2plant_")
    try:
...
def main():
    print("=" * 78)
    print("K2b-U3-R2  DOES THE 2D LIMIT CYCLE SURVIVE IN 3D AT THE SPEC'S 60 mm MESH?")
    print("graded against K2bU3R2_PREREGISTRATION.md; gate re-used unchanged from")
    ...
    print("\nTEST D (R2) -- the full 3D module, N=4 racks, OPEN row ends, h=0.060 m.")
    d, dper = grade(CASE, "Test D R2 (3D, h=0.060)", RXS)
    if d is None:
    ...
    if d == "SURVIVES":
        ...
        print("two-dimensionality at the spec's own 60 mm module. O1's consequence")
        ...
        print("spec's own 60 mm module. The 2D finding STANDS as true of the 2D slice --")
...
def _selftest():
    ...
    tmp = tempfile.mkdtemp(prefix="k2bU3R2selftest_")
    try:
