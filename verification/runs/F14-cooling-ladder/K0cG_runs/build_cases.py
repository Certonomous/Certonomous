...
NX_FINE, NY_FINE = 307, 307   # K0cG: third level, 192 x 1.6
D1_COARSE = 2.5e-4          # first cell off every wall, coarse mesh, metres
REFINE = 2.56               # K0cG: 1.6^2, so d1 = 2.5e-4/2.56 = 9.7656e-05
...
def build():
    ...
    cases = {
        ...
        # K0cG: ONLY the third mesh level, for the two models K0cS graded.
        # LaunderSharmaKE is absent because K0cS REFUSED it, so it has no
        # two-level baseline for a third level to extend.
        "S_SST_x": dict(model="kOmegaSST", fine=True, end=40000),
        "S_KE_x":  dict(model="kEpsilon",  fine=True, end=40000),
    }
