...
def face_widths(centres, L):
    """Exact cell widths from cell centres on a 1D graded block.

    For faces f_0 = 0 .. f_n = L and centres c_j = (f_j + f_{j+1})/2, the
    faces follow exactly from f_{j+1} = 2 c_j - f_j.  Reconstructed rather
    than approximated by centre spacing, because on a 117:1 graded mesh the
    two differ by enough to matter in a wall integral.
    """
    f = [0.0]
    for c in centres:
        f.append(2.0 * c - f[-1])
    if abs(f[-1] - L) > 1e-9 * max(L, 1.0):
        refuse(f"REFUSE: face reconstruction closed at {f[-1]!r}, not {L!r}; "
               "the cell-centre list is not a single graded block.")
...
def measure(case_dir, case):
    ...
    def wall_nu(patch, axis, at_start):
        ...
        # AREA-WEIGHTED, NOT ARITHMETIC, AND THE MESH IS WHY.
        # Ampofo's "average Nusselt number" is the integral of the local
        # Nusselt over the wall divided by its length (p. 3564, and the
        # integration on Betts p. 681 is the same quantity).  This mesh is
        # graded to 0.25 mm at BOTH ends of every wall and 29 mm at the
        # middle, a ratio of 117, so an arithmetic mean over faces weights
        # the two corner regions -- where local Nu is 136 at one end and 17
        # at the other -- more than a hundred times too heavily.
        #
        # Caught by the heat balance, not by review: the arithmetic mean
        # closed the sealed cavity to only 2.5-8.4 percent, where the area
        # weighting closes it to a fraction of that.  The heat balance is a
        # near-identity and is NEVER a graded row, but it is exactly the
        # right instrument for finding an error in how a wall integral is
        # formed.
        tot = sum(widths)
        avg = sum(v * w for v, w in zip(vals, widths)) / tot
        return avg, vals, along
    ...
    return dict(
        ...
        # C4 replaces the horizontal walls with zeroGradient, so they carry no
        # wall temperature and wall_nu returns None rather than a fabricated
        # zero.  Their Nusselt IS zero by construction there, which is a
        # property of the boundary condition and not a measurement, so the
        # closure is formed from the two vertical walls alone and the fact is
        # recorded in the field name rather than hidden in a default.
        adiabatic_ends=(Nu_bot is None and Nu_top is None),
        heat_balance_pct=(
            100.0 * abs(Nu_hot + (Nu_bot or 0.0) - Nu_cold - (Nu_top or 0.0))
...
def main():
    ...
    # GATE_TABLE.md -- the committable verdict table, same artefact K0cT_runs
    # carries.  Generated from the graded rows, never hand-written.
    tl = ["# K0cS gate table (generated by analyse_k0cs.py -- do not hand-edit)",
          "",
    ...
    for model, (coarse, fine) in PAIRS.items():
        ...
        if not rs:
            tl += [f"## {model}", "",
                   "**REFUSED.** The fine mesh did not meet the convergence "
                   "criterion registered in K0cS_PREREGISTRATION.md section 4, "
                   "so no row of this model is graded.",
                   f"Measured peak-to-peak: "
        ...
        for g in sorted(rs, key=lambda z: int(z["row"][1:])):
            unit = " %" if g["kind"] == "REL" else ""
            tl.append(f"| {g['row']} | {g['quantity']} | {g['reference']:.4g} | "
                      f"{g['coarse']:.4g} | **{g['solved']:.4g}** | "
                      f"{g['deviation']:.3f}{unit} | {g['band']}{unit} | "
                      f"**{g['verdict']}** |")
        tl.append("")
    tl += ["## Reported, never graded (specification section 3.3)", "",
           "| case | theta centre | heat balance | nut/nu max | Nu hot |",
           "| --- | ---: | ---: | ---: | ---: |"]
    for c in GRADED + CONTROLS:
        mm = cases[c]["measure"]
        tl.append(f"| {c} | {mm['theta_centre']:.4f} | "
                  f"{mm['heat_balance_pct']:.4f} % | {mm['nut_over_nu_max']:.1f} | "
                  f"{mm['Nu_hot']:.3f} |")
    tl += ["",
           "Heat balance is a NEAR-IDENTITY on a sealed cavity and is reported, "
           "never gated (VERIFICATION_CHARTER.md:106-111). theta at the cavity "
           "centre is a near-identity for a Boussinesq solve (Tian p. 862) and "
           "is a measurement of the non-Boussinesq defect, not of a turbulence "
           "model.", ""]
