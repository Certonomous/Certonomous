...
def final_residuals(case):
    """Every solved field's last initial residual, against its own target.

    D407. What stood here before was a COMMENT inside measure() naming "the final
    initial residual of 9.6e-08" as the 64x64 leg's convergence evidence. Three
    things were wrong with it and only the first is obvious:

      - 9.6e-08 is the T residual, and the case's residualControl asks 1e-08 of
        T. The number offered as evidence of meeting a target stood 9.6x ABOVE it.
      - It was the SMALLEST of four. Ux and Uy sat 16x above their target and went
        unmentioned. Quoting the best of four without its target is a shape that
        reads as evidence and is not one.
      - IT WAS NEVER COMPUTED. Living in a comment, no run could contradict it, so
        it survived every re-run of this script unchallenged. A claim no execution
        can reach is not a measurement however true it happens to be.

    Reports per field: the last initial residual, the residualControl target that
    governs it, the ratio, and whether it met it. Plus `terminated_by`, read from
    the solver's own line rather than inferred -- a leg that ran out at endTime has
    NOT converged however small its residuals are, and that is a different fact
    from meeting a target.
    """
    log = None
    for name in ("log.buoyantBoussinesqSimpleFoam.continue",
                 "log.buoyantBoussinesqSimpleFoam", "log.solve"):
        cand = os.path.join(case, name)
        if os.path.exists(cand):
            log = cand
            break
    if log is None:
        return {"measured": False, "why": "no solver log found beside the case"}

    with open(log, errors="replace") as fh:
        text = fh.read()

    last = {}
    for field, resid in re.findall(
            r"Solving for (\w+), Initial residual = ([0-9.eE+-]+)", text):
        last[field] = float(resid)

    targets = {}
    fv = os.path.join(case, "system", "fvSolution")
    if os.path.exists(fv):
        block = re.search(r"residualControl\s*\{(.*?)\}", open(fv).read(), re.S)
        if block:
            for k, v in re.findall(r"(\w+)\s+([0-9.eE+-]+)\s*;", block.group(1)):
                targets[k] = float(v)

    def target_for(field):
        # `U` in residualControl governs every velocity component.
        if field in targets:
            return targets[field]
        if field.startswith("U") and "U" in targets:
            return targets["U"]
        return None

    per_field = {}
    for field, resid in sorted(last.items()):
        tgt = target_for(field)
        per_field[field] = {
            "final_initial_residual": resid,
            "residualControl_target": tgt,
            "ratio_to_target": (resid / tgt) if tgt else None,
    ...
    return {
        ...
        "fields_meeting_target": met,
        "fields_not_meeting_target": unmet,
        "per_field": per_field,
    }
...
def measure(case):
    ...
    # nothing about the end of the run. Its convergence evidence is now MEASURED
    # by final_residuals() -- all four fields against their own targets, plus how
    # the run actually terminated -- rather than asserted in a comment (D407).
    ...
    return dict(
        ...
        convergence=final_residuals(case),
        Pr=Pr, dT_K=dT,
