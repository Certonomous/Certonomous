...
CONTROL_TABLE = {             # frozen in freeze section 2.3
...
def main():
    ...
    for it, (tmx, tmn) in CONTROL_TABLE.items():
        got = d0.get(it)
        ok = got is not None and got["T_max"] == tmx and got["T_min"] == tmn
        tbl[str(it)] = {"registered": [tmx, tmn],
                        "measured": [got["T_max"], got["T_min"]] if got else None,
                        "exact": bool(ok)}
