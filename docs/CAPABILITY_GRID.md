...
**family table at HEAD: `b340e2bf`** (`docs/capability/heat-transfer_GRID.md`; every cell below is copied verbatim from that file — the family supervisor's words, not this script's).
...
| cell | verdict |
|---|---|
| **conduction · laminar (no flow) · 2D** | ~~**CAN DO — 2 cases**~~ ~~**CAN DO, CAVEATS — 2 cases**~~ ~~**CAN DO, CAVEATS — 4 cases**~~ **CAN DO, CAVEATS — 3 cases** *(corrected 2026-08-26, Correction 1 below: the strongest case's own headline is "Rung verdict: GATE FAIL — 1 of 3 graded rows failed", `T9a_RESULTS.md:13`; T11 has no rung-level record at HEAD)* — caveats: R1 interface `GATE FAIL` (−2.41 mK vs 0.92 mK band); R3/R4 `GATE REACHED` below the O(Bi) floor; T11 gate file uncommitted. Evidence (T9a composite wall + fin; T11 transient plane wall); **strongest case `docs/campaigns/T-family/T9a_RESULTS.md` @ `0cbaea26`**, comparator frozen at `239ed2b8` before any case existed (`:7`), pre-registration `docs/campaigns/T-family/T9a_PREREGISTRATION.md` @ `6753e912`; checked: exact-theory error with a CONVERGING triple + GCI per row — **R0 wall `q″` `PASS` 19.854991 vs 19.502682, dev 1.806 % on a 2.043 % GCI band, p 1.079; R2 interface-2 `T` `PASS` −0.51 mK on a 0.75 mK band, p 0.952** (`:38`, `:40`); planted-zero control in the comparator. Also on the record and stated so the cell cannot imply otherwise: **R1 hot-side interface `GATE FAIL` −2.41 mK against a 0.92 mK band (p 1.738), cause CONFIRMED as the interface scheme** (`:39`; T9aD @ `a74b2f61`, T9aH @ `b698dfc3` diagnose it and grade no T9a row); R3/R4 fin rows `GATE REACHED` — bands 0.00077 % / 0.00011 % below the 0.025 % O(Bi) floor, reported not graded (`:41-42`). T11: `PASS` ×3 EXACT tier, p 2.000 / 2.000 / 2.005, GCI 2.9e-07 / 1.4e-06 / 3.4e-06 on a ±1e-4 band, Robin BC, `P_MIN` ruled 0.5 — **but its `gate_t11.json`, `STATUS.T11_PW_{c,m,f}` and DONE markers are ON DISK and NOT AT HEAD** (`git ls-tree HEAD verification/runs/T-family/T11_runs/` holds only the instruments and the C-T control); the tracked witnesses are the calibration row `docs/COST_CALIBRATION.md` C-118 @ `6becf266` and the pre-registration @ `f5f67de7` (frozen `ca9aad86`, amendments `352aef0d`, `f5f67de7`). T11 is therefore counted as a case but is not the citation. **[ADDED 2026-08-27, heat-transfer — TWO CASES, AND THE STRONGEST CITATION IN THIS CELL IS NOW T14, NOT T9a.] T14 (2-D transient conduction in a square, T11b, EXACT tier): `PASS` x3, triples all CONVERGING, **p 1.9999 / 2.0000 / 2.0066**, GCI 2.3e-04 / 1.1e-03 / 1.9e-03 %, four registered predictions HIT; 6.884 core-min, ratio 0.950 (C-160). **This is the cell's cleanest evidence and the reason is the instrument, not the number:** `analyse_t14.py:205` carries NO exception of any kind -- a triple that is not CONVERGING returns `NOT A RESULT` unconditionally -- and its selftest at `:393` DRIVES the `exact (e21 = 0) -> NOT A RESULT` case. Rule 5 is fully encoded and was verified as such by the supervisor reading the source, not the summary. ~~**R1's GATE FAIL caveat above is NO LONGER OPEN, and is replaced by a NARROWER one.**~~ **[WITHDRAWN 2026-08-27 by the heat-transfer supervisor under VERIFICATION_CHARTER v1.16 section 2g. R1's GATE FAIL IS STILL OPEN. I claimed it retired; that claim required W1b's PASS, and W1b no longer has one. A successor that produces no graded result retires nothing.]** W1b (T9a-R1b, the one-row successor that changes a single `fvSchemes` line to the harmonic interface scheme T9a named as the cause) measures interface-1 `T` at **deviation 2.899e-12 K against a registered 1e-06 K floor** -- twelve orders below the parent's -2.41 mK failure, on a floor itself **900x tighter** than the 0.9 mK band it replaces. **The diagnosis in `T9a_RESULTS.md` 1.1 is therefore CONFIRMED by measurement.** **THE NARROWER CAVEAT, stated because the cell must not rest on it silently:** W1b's triple is measured **EXACT** (e21 -2.842e-13, e32 3.240e-12, no order and no GCI claimed), and `CLAUDE.md` rule 5 branch (2) makes an EXACT triple `NOT A RESULT`. W1b's `PASS` therefore stands on a **pre-registered floor exception to a standing rule** (`analyse_t9aR1b.py:209`; frozen at `3c39d08d` 2026-08-26T21:18Z, first compute 2026-08-27T08:54:19Z, so rule 2 is clean). ~~**That exception is REFERRED AND UNRULED, and this cell does not flip to an unqualified CAN DO on it.**~~ **[RULED 2026-08-27 — REFUSED. VERIFICATION_CHARTER v1.16 section 2g: "A pre-registration fixes the gate, threshold, cap and label FOR ITS CASE. It has no power to disapply a standing rule, and an exception to one is void however early it was frozen." The decisive ground is rule 5's own last sentence -- the gate is ONE-WAY, so an exception yielding PASS where rule 5 yields NOT A RESULT runs the gate BACKWARDS. Rule 2 being clean does not save it: freezing early proves the exception was not chosen to fit the answer, it does not confer the authority to write the exception. CONSEQUENCE, AND IT IS MINE TO APPLY BECAUSE THE ROW IS THIS TEAM'S: **W1b R1 reads `NOT A RESULT` on its Roache limb** and is **NOT COUNTED** among this cell's cases -- a NOT A RESULT is not a verified metric, and counting it would be the flattering error this family is on notice for. THE MEASUREMENT IS REPORTED, NOT DISCARDED (D534's row class): 2.899e-12 K against a 1e-06 K floor, three levels agreeing to 1e-13, 900x tighter than the band it replaces. VERIFICATION'S DIAGNOSIS, ADOPTED AND NOT MERELY ACCEPTED: **the wrong instrument was registered.** Where the discretisation error is below the floor at EVERY level, a Roache triple has nothing to measure -- p = ln|e32/e21| / ln r is noise divided by noise. Rule 5 refuses to compute an ORDER from nothing; it does not say the VALUE is worthless. THE REMEDY THIS TEAM OWES: a successor registering a FLOOR DEMONSTRATION with its own gate -- the discretisation error is below X at the COARSEST level, so the answer does not depend on the mesh at the resolution that matters -- which CAN PASS on its own pre-registered band without touching rule 5. That is a weaker claim than grid convergence and it is the claim the evidence actually supports. W1b stays VISIBLE in this cell as REPORTED and is never deleted.]** To its credit and recorded as such: the exception is the narrow one the argument actually supports -- `analyse_t9aR1b.py:217-225` **DERIVES** `EXACT` from `|e21|,|e32| < roundoff_K` rather than declaring it, and is named this family's pattern of record. |
...
| **natural conv · laminar · 2D** | **CAN DO, CAVEATS — 2 graded cases** (K0c differentially heated square cavity, `Ra` 1e3–1e6); **strongest case `docs/campaigns/F14-cooling-ladder/K0c_RESULTS.md` @ `8590c96a`**, gate specification written first at zero compute (`docs/campaigns/F14-cooling-ladder/K0c_DIFFERENTIALLY_HEATED_CAVITY_GATE.md` @ `6d149d51`), reference values and bands parsed from it at run time; checked: benchmark deviation — **"GATE PASS, 0 of 20 GRADED rows failed, with 4 identity rows reported and counted toward nothing"** (`:546`, the dated correction in force; the record's compound "GATE PASS" is the family's word for a whole-rung `PASS`), largest deviation 1.139 % on a 3 % band; planted control fires at 3.19 %. Caveats: **mesh PAIRS only — no Roache triple, no GCI, no observed order exists for K0c** (`docs/COVERAGE_MATRIX.md` §3.4: the "p 1.94–2.33" once attributed to it belongs to K0b, whose record disclaims the reference at `K0c_RESULTS.md:335`); **reference is SECONDARY** (de Vahl Davis via Han & Xie 2019; the 1983 original not read); the K0b capability rung's unaided reproduction **`GATE FAIL` V3 — 128×128 `Nu_avg_hot` 4.490 % below the published value at observed order −1.25 against +1.94** (`K0b_D403_RERUN_RESULTS.md:16` @ `3d28328c`), repaired to `PASS` ×4 W1–W4 bit-identical (`K0b_D406_REPAIR_RESULTS.md` @ `f279aac5`) — "graded against no published datum"; K0a/K0b feasibility rungs "validated against no published reference datum" (`verification/runs/THERMAL_K0_runs/README.md:12` @ `a1fbe127`); K2e Boussinesq-limit sweep is solver-backed, no gate on the world (`K2e_RESULTS.md` @ `b845b603`). **[ADDED 2026-08-27, heat-transfer — T13 SUPPLIES THE ONE THING THIS CELL'S OWN CAVEAT SAYS IT LACKS.] T13 (natural convection in a vertical slot, conduction regime, Batchelor parallel flow, EXACT tier): `PASS` x4. The caveat above records that **K0c is mesh PAIRS only -- no Roache triple, no GCI, no observed order.** T13 has one: **G1 CONVERGING p 2.0000, GCI 8.78e-02 %; G1b CONVERGING p 1.9975**, both graded through the FULL rule-5 gate with no exception; 442.167 core-min, ratio 3.174 (C-155). **So this cell now holds a converging three-level ladder with an observed order in the natural-convection mode, which it did not before.** **TWO CAVEATS TRAVEL WITH IT AND NEITHER IS DECORATIVE.** (1) T13 is EXACT tier: its referent is analytic, so under Sanaa's upheld V/P ruling it scores **V and can NEVER score P** -- it strengthens `V` and `G` in this cell and does nothing whatever for `P`. (2) T13's OTHER two rows, **G2 and G3, are `PASS` on ABSOLUTE FLOORS while their triples read `OSCILLATORY` and `DIVERGENT` (p -0.8931)**, because `analyse_t13.py:405` reads `if not exact_class and ...` -- a per-row flag, hard-coded at `:488`, that bypasses **the whole of rule 5 branch (2)** and is never confronted with the state actually measured. **The verdicts are substantively right and the reason is the floor, not the bypass:** the graded fine values are 6.263e-11 against a 1e-06 floor (**4.20 orders**) and 1.990e-10 against a 2e-04 floor (**6.00 orders**), and all five planted-zero controls PASS at a demonstrated detection floor of **1e-07** -- an order below G2's band -- so a band violation WOULD have been visible. **The comparator is nevertheless marked NOT-FOR-REUSE** (the residual hazard is reuse: lifting a sibling's `specs` block is how this family builds a rung), and `analyse_t9aR1b.py:217-225`, which MEASURES exactness where T13 DECLARES it, is the pattern of record. No retrofit is owed: the band verdict is computed first and the gate is one-way, so a repaired comparator reproduces every value at 0.000e+00 with every verdict unchanged. |
...
### heat-transfer family's corrections (appended below its footer at `b340e2bf`; supersede the table above where they strike it; reproduced verbatim)
...
### rulings applied to the heat-transfer table (appended to the family file at `b340e2bf`; amendments of record, reproduced verbatim; a ruling supersedes the cell text above where it says so)
...
Reading at assembly time (2026-08-27T20:29Z, HEAD `5e5cd10d`): **157 ok, 1 MISSING, 2 non-commit tokens, 160 distinct shas.** MISSING lines, each with the source that cites it — a token that does not resolve HERE is not automatically a lost commit: read the citing file, which may disclose it as a sha in a DIFFERENT repository, and this control deliberately never launders such a token into `ok`: `d3b7e38b` (cited by `docs/capability/dafoam_GRID.md`) Non-commit tokens: `dc2b4f74` (blob), `f4e5c350` (blob) — disclosed by the citing family file as not a commit sha.
