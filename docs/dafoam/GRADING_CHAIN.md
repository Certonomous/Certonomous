...
**And the IPOPT message that made this look like physics is misleading.** `EXIT: Invalid number in NLP function or derivative detected.` comes from a guard at `IpOrigIpoptNLP.cpp:487` that fails **disjunctively** — on a false *status* OR a non-finite *value* — and prints the same string either way.

**⚠ CORRECTION WITHIN THE CORRECTION, 2026-08-31T21:1xZ.** An earlier version of this paragraph asserted *"a sweep of every `CD:`/`CL:` print across 264,607 lines returns ZERO non-finite tokens."* **That over-states the record and is withdrawn.** What exists is an **ungated pre-freeze search for the literal token `nan`** returning 0 (`curriculum_SO3D/PREREGISTRATION.md` §3 fact 3) — and SO3D's own §2 says in terms: **"No non-finite census has been computed."** The full census is SO3D's registered prediction **`P1` / gate `G-SO3D-1`, still uncomputed**, with planted control `PLANT-A` waiting. Generalising `nan` to *all non-finite tokens*, and a declaration to a measurement, is **the same slip this document exists to end — committed inside it, by me, for the third time in one evening.** Recorded, not quietly edited away.

**What the record actually supports, and it is enough:** the measured A1 exception text is `success && IsFiniteNumber(ret) evaluated false: Error evaluating the objective function` — a disjunction, so it does not identify which limb tripped. **671 `Primal solution failed!` banners against 673 cutbacks** (near 1:1). The objective printed immediately before the final cutback is **finite at `0.0222388`**. And `D6RG_regrade.json` `grade/G-D6R-OPT` carries **`S1_line_search_failing: true`** with **`S2_dual_infeasibility_not_decreasing: false`** — which corroborates the failed-status reading over the bad-number one from the grading record itself.

**None of this is load-bearing for the class.** `BOOKKEEPING` rests on `ARM_DID_NOT_RUN` × 4 and `n_obj = 0`. The paragraph above is corroboration, and corroboration that over-reached is withdrawn without disturbing the finding.
