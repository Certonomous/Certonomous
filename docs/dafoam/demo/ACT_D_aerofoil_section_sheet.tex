% ---------------------------------------------------------------------------
% ACT D (AEROFOIL SECTION) -- MULTIPOINT SHAPE OPTIMISATION (INCOMPRESSIBLE)
% -- one-page customer-facing result sheet.  Written under the DEMO STANDARD v2
% output rules R1-R10.
%
% RENAMED 2026-09-01 from ACT_D_shape_optimisation_sheet.tex, for
% disambiguation only.  NO NUMBER, TABLE, FIGURE OR CLAIM ON THIS SHEET WAS
% TOUCHED BY THAT RENAME.  Two different subjects were both called "Act D" and
% the generic old filename had already misled a reader:
%   this one : 2D NACA0012 section, 4,032 cells, 10 m/s, SO-3aR2 run root,
%              gradients verified and NO OPTIMISER HAS RUN.
%   the other: ACT_D_reference_wing_sheet.tex -- three-dimensional reference
%              wing, 38,304 cells, 47 major iterations, 28.3% drag reduction.
% Their numbers belong to different cases and must never be merged.
%
...
% Compile: pdflatex ACT_D_aerofoil_section_sheet.tex
%
...
\begin{picture}(83,21)(-5,-10.5)
  ...
  \put(1.86,-2.05){{\color{warm}\vector(0,-1){7.50}}}
  \put(0,10.3){\makebox(0,0)[l]{\scriptsize\color{warm}leading edge}}
  ...
  % ---- axis label ---------------------------------------------------------
  \put(31,-9.3){\makebox(0,0)[c]{\scriptsize chord station $x/c$ --- section
...
$\bullet$ \textbf{One mesh, $4\,032$ cells.} The grid convergence study for this
case is running; the band lands in your inbox with the certificate.\\
$\bullet$ \textbf{Shape optimisation runs next.} No optimiser has run on this
section, so nothing here is an optimised result.}
...
Stage & \begin{tabular}{@{}r@{}}Stated upfront,\\ core-min\end{tabular}
      ...
      & \begin{tabular}{@{}c@{}}Live?\end{tabular} \\
\midrule
Feasibility check, ran first & $8.0$ cap      & $0.583$  & $35$  & $0.07$ & --- \\
Derivative verification      & $22.1$         & $14.318$ & $859$ & $\mathbf{0.65}$ & --- \\
\quad geometry and mesh      & ---            & $0.167$  & $10$  & ---   & yes \\
\quad three points re-solved, with gradients & --- & $2.517$ & $151$ & --- & yes \\
\midrule
Total                        & ---            & $14.901$ & $894$ & ---   & \\
\bottomrule
\end{tabular}}
...
\vspace{1pt}
{\scriptsize The indented rows are parts of the row above, not additions to it.
Tables 2 and 3 were computed ahead; the two indented stages are short enough to
repeat live. Figures are read from the run records; derived cost \$0.0127 at
\$0.0513 per core-hour --- derived, not metered.}

\vspace{1.5pt}
{\footnotesize \textbf{A short check ran before we spent the verification
...
count and with operating points.}
...
Table 2 shows why. One shape must serve all three, so the objective is their
...
mid-chord and a slightly fuller trailing edge. That is the local direction, not
the answer --- the optimisation that runs next follows it under your thickness
and area constraints and returns the shape and the saving. Want the trade made
against a lift constraint instead? The lift derivatives are already verified,
so it costs nothing to add.
...
$5\,\%$ limit written down before the run, not after seeing the answer; the
check was driven at a deliberately wrong step to prove it can fail, and it did.
What has no error bar is the mesh: one level, no refinement study --- say so and
we will price one. Every figure on this sheet is read from a run record
retained on disk.}
