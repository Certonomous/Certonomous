...
## AMENDMENT 3 — 2026-08-26 (POST-COMPUTE, DISCLOSURE ONLY): the ONE registered §7.1 extension (+20 000 iterations) is EXERCISED on all ten arms, in the form the frozen `mark_done_k0f.py` already prescribes; an extension launcher is added to the instrument set; no gate, band, threshold, floor, control, cap or label moves. Document v1.2 -> v1.3

**Lines whose number changed above this section: 0** (appended to the worktree copy after verifying it byte-identical to the HEAD blob `6f9a72b4`). **This amendment is POST-compute** (`CLAUDE.md` rule 2, second limb): 1 378.383 solver core-minutes are spent on the rung, all ten arms are `DONE` under the strict rule, and the frozen comparator's grade is on record (`K0f_RESULTS.md`, ledger row C-137, commit `db76091e`). Accordingly this section **cannot and does not** alter any gate, threshold, cap or label; it discloses how a clause that §7.1 registered **before** compute is now exercised, and adds one launcher — no reader, grader, mesh check or builder is touched (§7.7's `analyse_k0f.py` `764dedc6` and `mark_done_k0f.py` `f01e3fce` remain the grading path, byte-unchanged).

### A3.1 The ruling, and the prediction registered before the extension runs

**Ruled by the heat-transfer supervisor `[lab-attributed]`, relayed to this lane 2026-08-26 ~21:00Z, quoted as received:** *"the ONE registered §7.1 extension of +20 000 iterations is TAKEN for all ten arms, decided on the convergence numbers alone (max |dT| 0.14–13.7 K over the last 4 000 iterations against ~2e-5 K) — disclosed that the graded values were already printed by the comparator when this decision was made; I did not read them to decide. Registered prediction, written before the extension runs: NOT ONE level will meet §7.1 at 60 000 either (a 5-order gap does not close in 20 000 SIMPLE iterations; the field is moving, not settling), M2_f is the only arm that could surprise."* §7.1's own words are the licence: *"One extension of +20 000 iterations is registered; a second is not authorised. The decision to extend is taken on the convergence state alone, never with a graded value in view."* The disclosure in the ruling — the comparator had already printed the graded triples beside its `NOT A RESULT` rows when the decision was taken — is carried here verbatim rather than smoothed; the decision cites only the ten `converged=False` lines (§3 of `K0f_RESULTS.md`).

**Prediction P-A3.1 (scored at the extension's grading, HIT/MISS):** all ten arms still `converged=False` at 60 000 under §7.1; **P-A3.2:** if any arm converges it is `M2_f` (its max |ΔT| 0.1379 K was the smallest of the ten by an order of magnitude).

### A3.2 The restart form — the frozen instrument's, stated so nothing is improvised

`scripts/mark_done_k0f.py` (frozen, blob `f01e3fce`, §7.7) already carries the extension clauses, and they fix the form: the extension's rc is read from **`K0f_runs/STATUS.<case>.ext1`**; its log is **`<case>/log.solve.ext1`**; a `STATUS.<case>.ext2` **fails** the case (one extension only); clause 3 expects the last written time == **controlDict `endTime` + 20 000**, so the case's `controlDict` must read `endTime 40000` again after the run; clause 5 expects **60 000 `ExecutionTime` lines over both logs** and the extension's first `Time =` to be exactly **40 001**; clause 6's age datums for an extended case are **`0/T` and `STATUS.<case>`** (every field at 60 000 must be newer than both). Consequences, all mechanical: **`0/` is never touched, `STATUS.<case>` is never touched, `log.solve` is never touched**; the **40 000 checkpoint is the restart source** (`startFrom latestTime`), not an age datum; `writeInterval 4000` and `purgeWrite 2` are unchanged — OpenFOAM's purge queue (`TimeIO.C:565-578`, `previousWriteTimes_`) holds only times written by the running process, so `36000/` and `40000/` survive and the extension ends holding `56000/` and `60000/`, which is exactly the pair §7.1's criterion reads (`analyse_k0f.py` `is_converged` takes the latest time and latest − 4 000; the §V.4 equivalence gate samples the latest time). No registered instrument names a `STATUS_EXT.<case>` form; the supervisor's brief used that name and the frozen file's `STATUS.<case>.ext1` is followed instead, as rule 2 requires.

### A3.3 The extension launcher — `scripts/launch_k0f_ext1.sh` (git blob `49ad67c0`, full `49ad67c0a6ca952ef4661267201bdd6bfc84cef4`; sha256 `f574cdf6e055b0d1`; 149 lines; 0 `assert`)

Modelled on `launch_k0f.sh` (rc captured in the FOREGROUND under `timeout`, one `setsid` re-exec, atomic STATUS) and `T3_runs/run_one_t3_ext1.sh` (copy-edit + grep post-check on every `controlDict` edit). It **refuses (exit 2), before writing anything**: `--ranks ≠ 1`; no `log.solve`; no `STATUS.<case>` or one not `rc=0` (a failed first segment is not extended); an existing `STATUS.<case>.ext1`, `STATUS.<case>.ext2` or `log.solve.ext1` (the ONE extension); no `40000/`; no `0/T`; a `controlDict` whose `endTime` is not 40000 or whose `stopAt` is not `endTime`; any time directory beyond 40 000; a foreign process in the case directory (**G2, lineage-aware from birth** — own pid, ancestors to pid 1, descendants excluded, as `9fa66065` / `51618879` / `ba023a53`); an unresolvable solver; a failed `build_k0f.py --preflight`. Then: snapshot `system/controlDict` → `controlDict.pre_ext1` (`cp -p`); `startFrom latestTime` and `endTime 60000` by copy-edit, each grep post-checked with the snapshot restored on failure; solver in the foreground under `timeout` to `log.solve.ext1`; **restore the snapshot byte-for-byte, `cmp`-checked** (`controldict_restored=yes|no` in the STATUS, `no` appended to the note); write `STATUS.<case>.ext1` atomically in the pool format `rc= wall= checkMesh_rc=na timeout_s= ranks= solver= solver_path= case= note= segment=ext1 restart_from=40000 endTime_ext=60000 controldict_restored= started_utc= ended_utc=`; exit with the solver's rc (124 = cap, 128+n = signal, n = crash; `--preserve-status` not used, as `launch_k0f.sh` records). The snapshot file `system/controlDict.pre_ext1` is left in place as the edit's record.

**Driven on scratch copies of `M1_c` (with its `36000/`, `40000/` and `STATUS.M1_c`; outside the run tree; deleted afterwards) at 21:06Z:** (a) planted foreign `sleep` in the case directory → `REFUSE: G2: pid 439395 already running in M1_c`, rc 2; (b) the queue runner's exact form `setsid nohup bash -c 'cd <cwd>; launch_k0f_ext1.sh --case-dir … --no-detach > launcher.queue.out 2>&1'` with a fake solver on PATH that refuses unless `startFrom latestTime` and `endTime 60000`, writes 20 000 `Time =`/`ExecutionTime` lines from 40 001 and the `56000/` `60000/` checkpoints → guard passed, `STATUS.M1_c.ext1` `rc=0 … controldict_restored=yes`, `controlDict` md5 identical before and after (`0f6a1f8d…`), `log.solve.ext1` 20 000 `ExecutionTime` lines, first `Time = 40001`; **the frozen `mark_done_k0f.py --root <scratch> M1_c` then printed `1/1 cases meet the strict completion rule` and wrote `DONE.M1_c` "all seven clauses" — the frozen instrument accepts exactly this form**; (c) a second invocation → `REFUSE: … STATUS.M1_c.ext1 exists -- the ONE registered extension has already run`, rc 2; (d) fake solver exiting 7 → `rc=7 note=SOLVER_NONZERO_EXIT controldict_restored=yes`, controlDict `cmp`-identical to the live case's; (e) **the REAL `buoyantBoussinesqSimpleFoam` under `--timeout 20`** → the log's first line `Time = 40001` (a genuine restart from `latestTime` 40000), 225 iterations, `rc=124 wall=20 note=CAP_EXPIRED_or_child_exit_124 controldict_restored=yes`, no time directory beyond 40 000 written. Scratch spend ≈ 0.35 core-min, not charged to the rung. The live tree was not touched by any arm (`ls K0f_runs/M1_c` unchanged; no `*.ext1` anywhere under `K0f_runs/` at the time of this commit).

### A3.4 Queue form — the entry's cwd is NOT the case directory

`scripts/queue_entry_check.py` refuses any entry whose `cwd` holds a time directory (its age-guard check), and every K0f case holds `36000/` and `40000/`. The ten entries `verification/queue/heat-transfer/K0f_EXT_<case>.json` therefore set **`cwd` = `K0f_runs/ext/<case>/`** — an otherwise empty directory that receives only the runner's own records (`STATUS.K0f_EXT_<case>`, the launch argv's exit, an infrastructure record per L-342; `launcher.queue.out`) — and `launch_cmd` = `[scripts/launch_k0f_ext1.sh, --case-dir, K0f_runs/<case>, --timeout, <s>, --ranks, 1]`; the launcher `cd`s into the real case itself. `prereg_commit` = this amendment's commit; `ranks` 1; `memory_floor_gb` 1.0 (an estimate: §9 measured 128 MB RSS at 50 176 cells; L3 ≈ 250 MB). Dropped `M2_f` first (the one arm the prediction allows to surprise, and the longest), then the others oldest-first. The runner's `CAP_OVERRUN.txt` note at 1.10 × POINT is a report, never a kill; the wrapper's `timeout` at the cap below is the stop.

### A3.5 Cost — from the MEASURED segment-1 rates (C-137), not the registered basis

The extension is 20 000 of 40 000 iterations at each case's **own measured** rate, so POINT = 0.5 × the segment-1 core-minutes of C-137; per-case cap = 1.9 × POINT rounded up to whole core-minutes, `timeout = cap × 60 / 1` (rule 12: a wall cap converted, never assumed).

| case | segment 1 measured (core-min) | **ext POINT** | **ext cap (core-min)** | **`timeout` s** |
| --- | ---: | ---: | ---: | ---: |
| `M1_c` | 55.917 | **27.96** | **54** | **3240** |
| `M2_c` | 52.700 | **26.35** | **51** | **3060** |
| `M1_m` | 126.000 | **63.00** | **120** | **7200** |
| `M2_m` | 109.600 | **54.80** | **105** | **6300** |
| `B_hi` | 115.783 | **57.89** | **110** | **6600** |
| `I_hi` | 114.250 | **57.12** | **109** | **6540** |
| `M1_m_seed` | 119.083 | **59.54** | **114** | **6840** |
| `C_lam` | 95.700 | **47.85** | **91** | **5460** |
| `M1_f` | 290.800 | **145.40** | **277** | **16620** |
| `M2_f` | 298.550 | **149.28** | **284** | **17040** |
| **sum** | 1 378.383 | **689.19** | **1315** | |

**USD at the owner-stated $0.0513/core-h, DERIVED, NOT MEASURED: POINT $0.5893.** **The registered CEILING 2 750.70 core-min (A1.2) still binds the rung total:** 1 378.383 spent + 1315 cap sum = 2693.383 ≤ 2 750.70; the caps above are set so that even every arm reaching its cap cannot cross the ceiling. The §8.2 re-estimate trigger (1.6 × a case's line) and the per-case stop stand. Prediction P-A3.3 for the ledger: ratio actual/POINT ≈ 1.0 ± 0.1 per arm (the same solver, mesh and box as the rate was measured on, three hours later); contention is named separately at completion. **At completion:** `mark_done_k0f.py --root K0f_runs <case>` re-evaluated on all ten (the extended clauses now apply; the markers from segment 1 are not evidence for the extended case), then `analyse_k0f.py --expect-sha 764dedc6…` at ten arms, then a `K0f_RESULTS.md` update and a new ledger row. **No grade is read while any `STATUS.<case>.ext1` is absent.** §8.2 item 4 applies afterwards: an arm that cannot meet §7.1 after this one continuation has its rows reported as REFUSED with the measured spread — and no second extension exists.

### A3.6 What this amendment did not do

It moved no gate, band, threshold, floor, control, cap, label, closure, field set, mesh, `writeInterval`, `purgeWrite`, or `deltaT`; it edited no frozen instrument (§7.7's six comparator/builder/check rows and A2.3b's launcher rows are byte-unchanged); it retracted no `DONE` marker (`mark_done_k0f.py` never retracts — the re-evaluation at completion is a fresh check); it sent nothing, fetched nothing, and touched no permission setting, `CLAUDE.md` or `.claude/` config.

*Amendment drafted by a heat-transfer lane on the heat-transfer supervisor's ruling of 2026-08-26 `[lab-attributed]`, after first compute (1 378.383 core-min spent, disclosed above).*
