...
**THIS APPENDIX EXISTS BECAUSE §2 OF THIS FILE IS HEADED *"Completion — the
strict rule (rule 4), ten of ten"*, AND A READER IS ENTITLED TO KNOW WHICH OF
RULE 4's CLAUSES THAT SENTENCE ACTUALLY COVERS.** It covers the clauses
`mark_done_k0f.py` evaluates at grading. **It does not cover clause 7**, the
PRE-launch refusal of a case in which `0` or any numeric time directory already
exists — because clause 7 had no call site on any K0f launch path. §2 is not
struck and not rewritten (rule 6): *"1/1 cases meet the strict completion rule"*
is verbatim what the instrument printed, and the instrument printed it honestly
about the clauses it ran.

## D.0 RULE 6 COMPLIANCE, VERIFIED RATHER THAN CLAIMED

**lines whose number changed above this section: 0.**

**Verified BYTE-FOR-BYTE IN PYTHON against the `HEAD` blob `67c09b5b64b5b836f4d8206e2e96f0e4a261a240`**, by
asserting `new_bytes[:len(head_bytes)] == head_bytes` over all **22,559** bytes of
...
`scripts/launch_k0f_ext1.sh` carries 0 and 1. The extension segment recorded in
§E ran through the same path and is covered by the same statement.
...
- **NO VERDICT IS WITHDRAWN, MOVED, SOFTENED OR RE-OPENED.** The verdict at the head of this file — **`GATE REACHED`, tally
  **0 of 10**, every graded row `NOT A RESULT`** — **stands exactly as printed by
  the frozen comparator.** Ten of ten `DONE.*` markers stand. The cost accounting
  of §E6 stands. **No band was ever read**, so no number here could have been moved
  by anything clause 7 guards against.
...
**And the exposure on this rung is nil for a reason that belongs to the rung.**
The ordered gate stopped at limb (1) on all ten rows — no level iteratively
CONVERGED, arms sitting 1 193× to 727 875× their criterion even after the one
registered extension. **Not one value in this file was read from inside or
outside a band**, so not one could have been changed by a stray field.
...
frozen `K0f_PREREGISTRATION.md` §7.7 grading-path table, so a re-sequencing would
move a pinned grading-path blob after compute. And the cost of that
