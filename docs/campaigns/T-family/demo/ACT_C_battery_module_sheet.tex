...
channel faces, solved over the full 900 seconds. Every number below is read from
the solution; the one comparison not yet available is marked as such.}

% -------------------------------------------------- 1. confidence beat ------
\vspace{1pt}
\textbf{1. Before we spent anything on your module: what our transient
conduction machinery is worth}

\vspace{0.5pt}
{\footnotesize We solved a three-dimensional transient conduction problem with a
closed-form answer --- a uniform block, initially hot, cooled on its faces by a
fixed convective coefficient, the same physics as a cell losing heat to a
cooling channel --- and compared three independent readings against it.
Temperatures are the dimensionless excess
$\theta = (T-T_{\infty})/(T_{0}-T_{\infty})$ at $Bi = 1.0$, $Fo = 0.2$. Success
criteria and tolerances were fixed before the first solver started.}
...
\textbf{2. The module, the duty cycle, and what the eight cells did}
...
face against the interior cells' two, which gives them twice the time constant
and twice the settled temperature. \textbf{(3) The window closes before the
cells that matter stop rising.} The end cells reach $0.420\,\mathrm{K}$ at
$900\,\mathrm{s}$ and are heading for roughly $0.74\,\mathrm{K}$; at that point
the channel is still removing only $22.1\,\mathrm{W}$ of the $32\,\mathrm{W}$
going in. \textbf{$0.420\,\mathrm{K}$ is where the hottest cells are when the
clock stops, not where they are going.}\\[1.5pt]
\textbf{Energy conservation checked: closed to $99.85\,\%$.} Of the
$34\,080\,\mathrm{J}$ put in, $20\,212\,\mathrm{J}$ is stored in the cells and
$13\,818\,\mathrm{J}$ left through the channel faces --- measured separately,
agreeing to $0.15\,\%$.}
\vspace{1pt}
\end{minipage}}
...
cell its own cooling and nothing else, so a real pack with busbars, casing
conduction or coolant cross-talk would come in under it.\\
...
geometry and nothing else --- the six interior cells agree to six decimals and
so do the two end cells, which is how we know. It excludes coolant heating along
the channel, which this model cannot see and which would separate all eight.\\
$\bullet$ Properties are representative, not measured for your cells, and
conductivity is equal in all directions; the stack ends are insulated with no
plenum; there is no cell-to-cell contact path. \textbf{One mesh and one time
...
$512\,000$ cells; we said in advance it could be low by up to about $3\times$
and it was low by $1.43\times$.\\[2pt]
\emph{The module run:} upfront estimate $1.0$ core-minute with a ceiling of
$10.0$; \textbf{it used $0.017$ core-minutes} --- $1.7\,\%$ of the estimate.
We over-estimated by roughly $60\times$ and we say so: the estimate was
dominated by a guess at file-writing cost for a configuration we had never
measured, and the guess was far too pessimistic. The whole $900\,\mathrm{s}$
of module physics cost under a tenth of a penny.\\[2pt]
...
\vspace{0.5pt}
{\footnotesize
\rolesig{Lead Researcher.} The exact-solution check came first on purpose and it
licenses everything after it: heat generated in a solid, conducted out and
removed by a convective surface is exactly what a closed-form answer can test,
and we matched it to within $0.013\,\%$. The module result then said something
clear-cut --- the event you named the case after is not the event that sizes it.

\rolesig{Lead Engineer.} Size this pack against the cruise soak, not the takeoff
pulse: the pulse is a fifth of the energy. And the two end cells are your
problem --- one cooled face against two gives them twice the time constant and
twice the settled temperature of everything between them. A channel outboard of
cells 1 and 8 removes that asymmetry outright and is worth pricing first.

\rolesig{Lead Numericist.} Section 1 is a genuine verification; the module run is
not, and I will not dress it as one --- one mesh, one time step, no numerical
error bar on any module number. What it carries instead is an energy audit that
closes to $0.15\,\%$, and that is the check I would want before believing any of
it.\vspace{-2pt}}
