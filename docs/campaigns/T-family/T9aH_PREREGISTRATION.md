...
**`verification/runs/T-family/T9aH_runs/analyse_t9aH.py` — TWO `ast.Assert` nodes,
and BOTH are restoration guards on FROZEN state:**

```
270:    assert EX.WALL_LAYERS == ((0.05, 0.8), (0.10, 0.04), (0.02, 16.0)), \
           "frozen exact_t9a.WALL_LAYERS was NOT restored"
291:        assert A.REG["wall"]["layers"][1]["k"] == 0.04, \
           "frozen analyse_t9a.REG layer-k map was NOT restored"
```

**Why these are the most dangerous shape in this territory, and worse than a one-way
gate.** Both sit at the end of a deliberate **in-memory override of a frozen
reference** — `exact_wall()` overrides `EX.WALL_LAYERS` to derive the wall solution at
a trial conductivity, and `with_k2()` overrides `A.REG["wall"]["layers"][1]["k"]` so the
frozen `measure_wall` does not correctly refuse a 0.4 or 0.004 field. **The file on disk
is never touched — the whole safety of the manoeuvre rests on the restoration.** The
guard asserting that the restoration happened is the only thing standing between a
correct derivation and a comparator that silently grades every subsequent case against
a reference left at the wrong conductivity.

> **UNDER `-O`, A FAILED RESTORATION IS SILENT AND PERSISTENT. It does not corrupt one
> reading — it corrupts EVERY reading taken after it, in the same process, with the
> frozen reference still reporting itself as frozen.**

**`:291` is the sharper of the two: it sits inside a `finally:` block.** A `finally:`
guard is the one a reader is least likely to re-check, and it is the one that must hold
precisely when an exception is already in flight.

**Both are `UNJUDGED`, not shown harmed.** The 2026-08-25T22:48Z bound establishes that
no graded verdict on record was produced under `-O`, and **T9aH is not re-graded on this
ground.** The comment above `:270` already states the design intent — *"RESTORED, and
the restoration is asserted"* — and the disclosure here is that **"asserted" is exactly
the word that does not survive `-O`.**
...
**Specific to this rung:** the restoration guard must additionally **read
back the frozen module state and refuse on mismatch**, not merely test it — and the
successor should prefer **not overriding frozen state at all**, deriving at a trial
conductivity through an explicit parameter rather than by mutating a frozen module's
globals. **A guard that protects a mutation is weaker than not needing the mutation.**
