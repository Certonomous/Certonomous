#!/usr/bin/env python3
"""T23G2Rn COMPARATOR (DRAFT -- NOT FROZEN, NOTHING RUN).  Grades the §2ba NUMERICS
SUCCESSOR to T23G2R -- the SAME mesh, endTimes and every gate, with ONLY the p_rgh
GAMG linear-solver stopping tolerance dropped two decades below the G-CONV gate
(tolerance 1e-08 -> 1e-10, relTol 0.01 -> 0) so the p_rgh outer residual can
descend below the reused 1e-8 gate instead of floor-pinning at the linear
tolerance (T23G2R's G-CONV GATE FAIL; see T23G2Rn_PREREGISTRATION.md sections 1-2).

THE GRADING LOGIC IS BYTE-IDENTICAL TO FROZEN analyse_t23g2r.py.  The ONLY
differences are (a) the case-name / path constants (T23G2R -> T23G2Rn, RUNS,
...
until the supervisor pins the freeze commit; EXPECTED_SELF_BLOB = None).  Every
gate, threshold and band -- G-CONV's 1e-8 p_rgh threshold included -- is reused
VERBATIM and NOT RELAXED; the numerics fix lowers a LINEAR-SOLVER tolerance to
MEET the frozen gate, it does not move the gate (rule 2).
...
level names / endTimes point at T23G2Rn; (2) the ABSENT-primary-y+-instrument
...
RUNS = os.path.join(REPO, "verification/runs/T-family/T23G2Rn_runs")
...
# --------------------------------------------------------------------------
LEVELS = ("T23G2Rn_L1", "T23G2Rn_L2", "T23G2Rn_L3")
...
ENDTIME = {"T23G2Rn_L1": 8000, "T23G2Rn_L2": 16000, "T23G2Rn_L3": 28000}  # T23G2R 2.3
...
CELLS = {"T23G2Rn_L1": 40320, "T23G2Rn_L2": 90720, "T23G2Rn_L3": 204120}  # 2.1
...
GRADING_PATH_FREEZE_COMMIT = "2cba90ee619a1e8f4fbb51588ecf8d8751c10c4a"  # freeze commit 1/2 (T23G2Rn); this line set in commit 2/2 (VERIFICATION_CHARTER.md §2au.2). SELF member print-only (EXPECTED_SELF_BLOB=None), anchored by the FREEZE-PIN line in commit 2/2's message + grade-time byte-identity.
EXPECTED_SELF_BLOB = None          # section 2au.2 print-only; asserted only if the
                                   # supervisor ever sets it (see note above).
SELF_REL = "docs/campaigns/T-family/analyse_t23g2rn.py"
GRADING_PATH = (
    "docs/campaigns/T-family/T23G2Rn_PREREGISTRATION.md",
    "docs/campaigns/T-family/analyse_t23g2rn.py",
    "docs/campaigns/T-family/t23g_readonly_diagnosis.py",
    "verification/runs/T-family/T23_runs/mark_done_t23g2rn.py",
...
def case_dir(level):
    ...
    if not os.path.isdir(d):
        refuse("case directory %s does not exist; T23G2Rn has not been run" % d)
    return d
...
# ==========================================================================
def require_done():
    """`mark_done_t23g2rn.py` as a subprocess -- the thin OPTION (b) wrapper that
    ...
    T23G2Rn levels (T23G2Rn_PREREGISTRATION.md section 5.1).  T23G2's endTime is a
    ...
    md = os.path.join(T23_RUNS, "mark_done_t23g2rn.py")
...
def main(argv):
    ...
    note("=" * 74)
    note("T23G2Rn COMPARATOR -- gates REUSED VERBATIM from frozen T23G2 "
...
def selftest():
    ...
    note("=" * 74)
    note("T23G2Rn COMPARATOR --selftest : planted-control NON-VACUITY, RED/GREEN")
    ...
    try:
        cd = os.path.join(tmp, "T23G2Rn_L3")
        os.makedirs(cd)
...
# ==========================================================================
# AMENDMENT RECORD -- analyse_t23g2rn.py, 2026-09-09 (DRAFT -- NOT FROZEN).
#
# THIS FILE IS A COPY OF FROZEN analyse_t23g2r.py.  The grading LOGIC is
# byte-identical; the ONLY changes are (a) the case-name / path CONSTANTS
# (T23G2R -> T23G2Rn: RUNS, LEVELS, ENDTIME keys, CELLS keys, SELF_REL, the two
# GRADING_PATH members that name THIS file and THIS pre-registration, and the
# case-name strings in the runtime banners and the case_dir refusal) and (b) the
# SELF-FREEZE machinery: GRADING_PATH_FREEZE_COMMIT is the DRAFT placeholder
# "PIN-AT-FREEZE" (verify_self REFUSES to grade while unpinned) and
# EXPECTED_SELF_BLOB stays None (section 2au.2 print-only; a self-referential
# IDENTICAL assertion against its own freeze commit is a git pre-image).
#
# ENDTIME values (8000/16000/28000) and CELLS values are UNCHANGED from T23G2R:
# the §2ba numerics fix changes only the p_rgh linear-solver tolerance/relTol in
# the CASE (build_t23g2rn.py), NOT the iteration budget or the mesh, and the
# comparator's G-CONV 1e-8 p_rgh threshold is reused VERBATIM.
#
...
# to T23G2Rn would falsify a record, so they are not renamed.
#
# COMPLETION INSTRUMENT (T23G2Rn_PREREGISTRATION.md section 5.1): require_done()
# is BYTE-IDENTICAL to analyse_t23g2r.py -- it calls mark_done_t23.py.  That
# presupposes OPTION (a) (mark_done_t23.py's CASES additively extended with
# T23G2Rn_L*).  If the supervisor instead picks OPTION (b) (a separate thin
# mark_done_t23g2rn.py that imports mark_done_t23 and widens CASES in memory,
# leaving the frozen blob 982e1db6 invariant), require_done()'s `md` path and the
# mark_done member of GRADING_PATH re-point to the thin instrument at freeze.
# THIS LANE DOES NOT PICK; the supervisor decides in the §3 read.
#
