# T5. The rack physic: a heated wall-mounted cube against Meinders 1998 — pre-registration DRAFT

**DRAFT — NOT FROZEN — NOT COMMITTED — NOT BUILT — NOT RUN. Zero compute spent.**
Drafted by a T-family design lane 2026-08-22 for the supervisor and for Sanaa.
**It freezes on Sanaa's reading or on the supervisor's promotion of this file to
`T5_PREREGISTRATION.md`, and not before.** Every decision a reviewer might want
to change is labelled **INTERPRETATION n** in place and collected in §13.

Campaign T, rung **T5**, tier **ACQUIRE — primary HELD**. DC spine position 2
(Sanaa's H-2: `T3 → T5 → T8 → T12 → K2 rack row`). Verdict vocabulary fixed by
`VERIFICATION_CHARTER.md` §2 and by the lab constitution:
**PASS / GATE REACHED / GATE FAIL / NOT A RESULT / BLOCKED / PENDING.**
Templates: `T3_PREREGISTRATION.md` (row/gate/cost shape) and
`T10a_PREREGISTRATION_DRAFT.md` (INTERPRETATION discipline, intrinsic floors).

**No case directory exists.** Checked rather than asserted, at drafting time:

```
$ date -u +"%Y-%m-%dT%H:%M:%SZ"
2026-08-22T18:12:38Z
$ find verification/runs/T-family -maxdepth 1 -iname "*T5*"
rc=0            # no output above: no match
```

`verification/runs/T-family/` holds `T10aR_runs T10a_runs T1_runs T3_runs
T9a_runs` and nothing named T5. Charter §2b's condition for pre-first-compute
amendment is therefore satisfied and stated with the check that established it.

**This lane pre-registers. It launches nothing.** 14+ of 16 cores are committed
today; the launch decision is separate, is the supervisor's, and §11 costs the
rung so that decision can be taken on a number.
