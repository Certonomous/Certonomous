...
>
> **RECOMMENDATION — heat-transfer supervisor (2026-09-07):** Recommend AGAINST any change to the spine at this time — options 3 (move) and 4 (leave), which alter Sanaa's H-2 directive, are PREMATURE. The entire case for moving or removing T8 rests on the rung being irrecoverable in its spine position, and that is exactly what has not been measured: the steady formulation's failure at all three resolutions does not separate 'the plume is physically unsteady' (recoverable as URANS in place — option 1) from 'the closure is unrealizable on these grids' (a model-form finding for the closure ladder). Ranking options 3/4 before that fork is resolved would re-shape the spine on an unmeasured premise. IMMEDIATE NEXT STEP (ordinary re-registration route, no directive change, within-lab authority): register and run Option 1's DISCRIMINATOR — a single unsteady/transient run at one resolution with a stationarity criterion registered before the run — to resolve the physics-vs-closure fork; the comparator's settled axis-precondition bug (read r1,r2 from disk; never register a ratio from the nominal spec) is fixed in that successor's frozen comparator regardless of outcome. The fork then determines the route, and only the last branch is Sanaa's: (i) UNSTEADY -> option 1, re-register T8 as a URANS/statistically-steady rung, spine position unchanged (ordinary route); (ii) CLOSURE-UNREALIZABLE -> route to the closure ladder (§2an, cross-team), and if a closure-corrected steady rung then converges that is option 2, spine position unchanged; (iii) ONLY IF both the unsteady route AND a closure-ladder attempt fail to yield a convergeable, gradeable stratification-height rung does the spine-placement question (options 3/4) become live — and THAT decision is Sanaa's alone. This referral therefore reaches the desk carrying a conditional, not a request to move the spine now: the spine stays as directed; the lab runs the discriminator; options 3/4 return to the desk only if the recoverability routes are exhausted.
