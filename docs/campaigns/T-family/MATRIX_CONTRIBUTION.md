...
| **Impinging jet** | T4 (HAS RUN: T4a→T4d) | **PARTIAL** — ERCOFTAC case025 tabulated `Nu(r/D)` on disk (Re 23k/70k, H/D 2/6; one mislabeled header noted) + Martin correlation with stated validity from an open NREL report; **the `Nu` uncertainty is SECOND-HAND (2.4 %, KB Wiki quoting Baughn & Shimizu)**; graded rows need the closed ASME primaries. **2026-09-08 CORRECTION (V-129; was tier "NEVER RUN" — a fail-open, the verdict was invisible to the §2ay enforcer): this class HAS RUN and no longer belongs in §5. T4d (Re 23000, H/D 2) graded `NOT A RESULT` (whole rung) — `docs/campaigns/T-family/T4d_RESULTS.md`, commit `73e008dc`: fine level non-convergent (C6.3 field-change 0.01518 ≈ 76× the 2e-4 tol at endTime 64000, clean not capped) AND all three Roache radial triples non-CONVERGING (G1 OSCILLATORY, G2/G3 DIVERGENT; no GCI). STATE-(b) SUCCESSOR = T4e (run-length-vs-physical-unsteadiness discriminator; transient/URANS route endorsed by chief IF unsteadiness is measured) — DRAFTED, FREEZE PENDING, NOT yet run: the fail is now VISIBLE here but is NOT yet claimed discharged (honest — discharge requires the frozen+run successor).** | **NOT HELD** (`NOT A RESULT` under the binding triple gate, §0.2; state-(b) successor T4e freeze-pending, not yet discharged) |
