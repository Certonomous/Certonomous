#!/usr/bin/env python3
"""T23G2Rn2 COMPARATOR (DRAFT -- NOT FROZEN, NOTHING RUN).  Grades the dated §2ba
NUMERICS SUCCESSOR to T23G2Rn (itself the successor to T23G2R) -- the SAME mesh,
endTimes and every gate, with ONLY the p_rgh GAMG linear-solver settings changed:
tolerance 1e-08 -> 1e-09 (ONE decade below the 1e-8 G-CONV gate), an EXPLICIT
maxIter 100 INSERTED, and relTol KEPT at T23G2R's 0.01 (small-but-nonzero).  This
lets early outer steps exit cheaply on the relative criterion while, near
convergence, the sub-gate absolute floor (1e-9) binds and drives the p_rgh outer
residual below the reused 1e-8 gate instead of floor-pinning at the linear
tolerance (T23G2R's G-CONV GATE FAIL), and it does so at AFFORDABLE cost -- unlike
T23G2Rn's relTol 0, which removed the relative early-exit and ran GAMG to its
default maxIter (1000 V-cycles) EVERY step at 13-15x (T23G2Rn PENDING, cost miss;
LESSONS L-514).  See T23G2Rn2_PREREGISTRATION.md sections 1-2.

THE GRADING LOGIC IS BYTE-IDENTICAL TO FROZEN analyse_t23g2r.py.  The ONLY
differences are (a) the case-name / path constants (T23G2R -> T23G2Rn2, RUNS,
LEVELS, ENDTIME, CELLS, SELF_REL, GRADING_PATH) and (b) the self-freeze machinery
(GRADING_PATH_FREEZE_COMMIT = "PIN-AT-FREEZE", so verify_self REFUSES to grade
until the supervisor pins the freeze commit; EXPECTED_SELF_BLOB = None).  ENDTIME
and CELLS are UNCHANGED from T23G2R/T23G2Rn: the §2ba fix changes only the p_rgh
solver settings in the CASE, never the iteration budget or the mesh.  Every gate,
threshold and band -- G-CONV's 1e-8 p_rgh threshold included -- is reused VERBATIM
and NOT RELAXED; the numerics fix lowers a LINEAR-SOLVER tolerance and caps its
iterations to MEET the frozen gate, it does not move the gate (rule 2).

The inherited comments below (R1-R8, R6) describe the grading logic that is
carried over UNCHANGED from the T23G2 line and cite the frozen T23G2 /
T23G2R records that are their true provenance; they are left intact rather than
re-attributed, because renaming a record falsifies it.  Only three things differ
from analyse_t23g2.py in that inherited logic: (1) the run target /
level names / endTimes point at T23G2Rn2; (2) the ABSENT-primary-y+-instrument
...
RUNS = os.path.join(REPO, "verification/runs/T-family/T23G2Rn2_runs")
...
ENDTIME = {"T23G2Rn2_L1": 8000, "T23G2Rn2_L2": 16000, "T23G2Rn2_L3": 28000}  # T23G2R 2.3
...
CELLS = {"T23G2Rn2_L1": 40320, "T23G2Rn2_L2": 90720, "T23G2Rn2_L3": 204120}  # 2.1
...
GRADING_PATH_FREEZE_COMMIT = "PIN-AT-FREEZE"  # DRAFT placeholder: verify_self() REFUSES to grade while this reads PIN-AT-FREEZE. The supervisor sets this at freeze to commit 1/2's sha (VERIFICATION_CHARTER.md §2au.2). SELF member print-only (EXPECTED_SELF_BLOB=None), anchored by the FREEZE-PIN line in commit 2/2's message + grade-time byte-identity.
EXPECTED_SELF_BLOB = None          # section 2au.2 print-only; asserted only if the
                                   # supervisor ever sets it (see note above).
SELF_REL = "docs/campaigns/T-family/analyse_t23g2rn2.py"
GRADING_PATH = (
    "docs/campaigns/T-family/T23G2Rn2_PREREGISTRATION.md",
    "docs/campaigns/T-family/analyse_t23g2rn2.py",
    "docs/campaigns/T-family/t23g_readonly_diagnosis.py",
    "verification/runs/T-family/T23_runs/mark_done_t23g2rn2.py",
...
def case_dir(level):
    ...
    if not os.path.isdir(d):
        refuse("case directory %s does not exist; T23G2Rn2 has not been run" % d)
    return d
...
# ==========================================================================
def require_done():
    """`mark_done_t23g2rn2.py` as a subprocess -- the thin OPTION (b) wrapper that
    IMPORTS the frozen `mark_done_t23.py` (blob 982e1db6, byte-invariant) and
    reuses its six clauses verbatim, widening only its in-memory CASES for the
    T23G2Rn2 levels (T23G2Rn2_PREREGISTRATION.md section 5.1).  T23G2's endTime is a
    LEVEL VARIABLE (A1.6), unlike T23G's invariant 10000, so each level is checked
    against its own registered endTime and the value is passed explicitly rather
    than left to a default that would silently check the wrong time."""
    md = os.path.join(T23_RUNS, "mark_done_t23g2rn2.py")
...
def main(argv):
    ...
    note("=" * 74)
    note("T23G2Rn2 COMPARATOR -- gates REUSED VERBATIM from frozen T23G2 "
...
def selftest():
    ...
    note("=" * 74)
    note("T23G2Rn2 COMPARATOR --selftest : planted-control NON-VACUITY, RED/GREEN")
    ...
    try:
        cd = os.path.join(tmp, "T23G2Rn2_L3")
        os.makedirs(cd)
...
# ==========================================================================
# AMENDMENT RECORD -- analyse_t23g2rn2.py, 2026-09-09 (DRAFT -- NOT FROZEN).
#
# THIS FILE'S GRADING LOGIC IS BYTE-IDENTICAL TO FROZEN analyse_t23g2r.py (it was
# derived by a mechanical case-token rename of the proven analyse_t23g2rn.py
# template, whose own logic is byte-identical to analyse_t23g2r.py; the rename was
# verified LOSSLESS and REVERSIBLE).  The ONLY changes vs analyse_t23g2r.py are
# (a) the case-name / path CONSTANTS (T23G2R -> T23G2Rn2: RUNS, LEVELS, ENDTIME
# keys, CELLS keys, SELF_REL, the GRADING_PATH members that name THIS file, THIS
# pre-registration and the OPTION-(b) completion wrapper, and the case-name
# strings in the runtime banners and the case_dir refusal) and (b) the SELF-FREEZE
# machinery: GRADING_PATH_FREEZE_COMMIT is the DRAFT placeholder "PIN-AT-FREEZE"
# (verify_self REFUSES to grade while unpinned) and EXPECTED_SELF_BLOB stays None
# (section 2au.2 print-only; a self-referential IDENTICAL assertion against its
# own freeze commit is a git pre-image).
#
# ENDTIME values (8000/16000/28000) and CELLS values are UNCHANGED from T23G2R /
# T23G2Rn: the dated §2ba numerics fix changes only the p_rgh GAMG linear-solver
# tolerance (1e-08 -> 1e-09), INSERTS an explicit maxIter (100), and KEEPS relTol
# at 0.01, all in the CASE (build_t23g2rn2.py); it touches NEITHER the iteration
# budget NOR the mesh, and the comparator's G-CONV 1e-8 p_rgh threshold is reused
# VERBATIM (rule 2 -- the fix MEETS the gate, it does not move it).
#
# THE INHERITED R1-R8 / R6 AMENDMENT RECORDS ABOVE ARE LEFT INTACT.  They are the
# provenance of the grading logic this file carries unchanged; they cite the
# frozen T23G2 / T23G2R records that are their true source.  Re-attributing them
# to T23G2Rn2 would falsify a record, so they are not renamed.
#
# COMPLETION INSTRUMENT -- OPTION (b), ALREADY WIRED (T23G2Rn2_PREREGISTRATION.md
# section 5.1, mirroring the T23G2Rn supervisor decision).  require_done()'s `md`
# path and the FIRST mark_done member of GRADING_PATH name the thin wrapper
# mark_done_t23g2rn2.py (which IMPORTS the frozen mark_done_t23.py @982e1db6 and
# widens its CASES in memory only), and the frozen base mark_done_t23.py is ALSO
# carried as an explicit GRADING_PATH member so its blob is recorded on the
# comparator's face.  The frozen blob 982e1db6 is left BYTE-INVARIANT.  No clause
# of the six-clause completion logic is copied or changed; it lives once, in the
# frozen file, reached by import.
#
# NOT GRADED, NOT PINNED, NOT LAUNCHED by the lane that authored this file.
# ==========================================================================
