...
## AMENDMENT 8 — 2026-08-26 (PRE-FIRST-GRADED-COMPUTE): the S5.3 inflow map wrote to the WRONG REGION PATH; repaired, extrusion made mesh-derived, collinearity refusal added; the T5_C crash artefacts preserved

**Document version 1.6 -> 1.7. Lines whose number changed above this section: 0.** Disclosure only: no gate, band, threshold, cap or label moves. Ordered by the heat-transfer supervisor (`docs/LAB_STATE.md` addendum 17:43:22Z), executed and recorded by the T5 lane under Sanaa's standing orders (silence is approval); decisions `[lab-attributed]`. **The promotion of the repaired map PRECEDED the supervisor's read of its diff**: the diff is held at `verification/runs/T-family/T5_runs/map_inflow_t5.A8.diff` (blob `f64112a7`) for that read after the fact; the map blob in force is **`f4e5c350`** (was `dc2b4f74`). **Condition (`CLAUDE.md` rule 2), and how it was checked:** no graded case holds a time directory — `ls -d <case>/[0-9]*` over `T5_CUBE_c H_c T5_CUBE_m P_m L_m T5_CUBE_f` at 2026-08-26T20:52:02Z returns only `0.orig` in each; no `STATUS.*` or `DONE.*` for a graded case exists at the `T5_runs` root (only `STATUS.X_2d`, `DONE.X_2d`); the one graded launch on record (T5_C, 17:41:37Z) died at solver start with **zero iterations and 0 core-min** (`crash_1741Z/STATUS.T5_CUBE_c`: rc=1 wall_s=0), and nothing a verdict depends on has been read. Checked at 2026-08-26T20:53:20Z.

**(a) The defect, established from the artefacts and the reader's source — and it is NOT the collinearity the 17:43Z triage named.** The 1 200 points the `dc2b4f74` map wrote to `<case>/constant/boundaryData/inlet/points` at 17:41:02Z span **three z-stations (0, 0.0375, 0.075; 400 points each), x = −0.12, y ∈ [1e-6, 0.050999]** — a plane, not a line (measured on every one of the six files; the triage read only the first lines, all at z = 0). The solver's FATAL says *"only had **0** points"*, the same text it gave at 16:27Z when no boundaryData existed at all. **Cause:** openfoam-2606's `timeVaryingMappedFixedValue` reads through `PatchFunction1Types::MappedFile`, whose points path is `time.constant()/mesh.dbDir()/"boundaryData"/<patch>/points` (`src/meshTools/PatchFunction1/MappedFile/MappedFile.C:496-503`), and for a multi-region case `mesh.dbDir()` is the **region name** — the air-region reader opens `constant/air/boundaryData/inlet/points`, which did not exist; `rawIOField` held zero points and the planar interpolation refused. **F11 is restated accordingly: a map-script PATH defect, outside the freeze set; registration, meshes and comparator untouched.**

**(b) The repair, `map_inflow_t5.py` blob `dc2b4f74` -> `f4e5c350` (diff 162 lines, confined to `write_boundary_data()` and three new helpers; docstring and the constants line; 0 `ast.Assert`).** The fluid region is READ from `constant/regionProperties` (exactly one fluid region, else REFUSE) and `inlet` must exist in `constant/<region>/polyMesh/boundary`; the files go to `constant/<region>/boundaryData/inlet/`. The y-profile is extruded to **three z-stations spanning the patch width read from the region's `polyMesh/points` (streamed, min/max z only)** with a **0.1 mm outward margin** so every face centre lies inside the triangulated hull — the previous hard-coded `W = 0.075` with no margin is replaced. The points file is read back and the map **REFUSES (exit 2)** if the cloud is collinear or the count differs. Station selection, the 2 H guard, `re_theta()`, `sample()` and the log are byte-unchanged.

**(c) Executed.** The real map ran from `T5_runs` at 20:51:45Z (rc 0, `T5_runs/log.map_inflow_A8`; iteration appended to `T5_INFLOW_LOG.md`: Re_θ = 660 crossed at x/H 63.790 → plane 55.790 → station 56, Re_θ 592.1 — unchanged from 17:41Z, as the converged X_2d fixes it). In each of the six cases: **1 200 points, x = −0.12, y [1e-6, 0.050999], z [−0.0001, 0.0751]** at stations −0.0001 / 0.0375 / 0.0751 against a mesh z-extent [0, 0.075]; U/k/omega 1 200 values each; non-collinear (the refusal did not fire). **Arming (S16.11 item 3): `check_launcher_can_launch.py --one-iteration T5_CUBE_c --solver chtMultiRegionSimpleFoam` → `ARM 2 one-iteration: PASS -- solver rc=0 and reached Time = 1`** (20:51:59–20:52:02Z, scratch copy; the same result on a scratch copy driven with the proposed file before promotion). Housekeeping, disclosed: the T5_C crash artefacts were MOVED, never deleted, into `T5_CUBE_c/crash_1741Z/` (`0/`, `STATUS.T5_C`, `STATUS.T5_CUBE_c`, `launcher.queue.out`, `log.solve`, `log.checkMesh` — the last is the launcher's 17:41:38Z re-run of checkMesh, so the case root now carries no `log.checkMesh` until the next launch rewrites it); in all six cases the misplaced `constant/boundaryData` was renamed `constant/boundaryData.misplaced_1741Z` (not `.collinear_`, because it is not collinear; the name states what was measured).

**(d) Enqueue under this amendment.** The six entries are re-issued as `T5_<case>_v2.json` citing this amendment's commit as `prereg_commit`, argv per AMENDMENT 6(b), costs per AMENDMENT 2, in the order C, H_c, M, P_m, L_m, F. **`S_m` is NOT enqueued**: its launcher refuses without `M`'s surface T.

**Not done by this amendment:** the digitiser (S16.11 item 2) is untouched — AMENDMENT 7 remains reserved for it; `analyse_t5.py` stays blob `9c2c1d44`; `run_one_t5.sh` stays `313df45c`; every band, floor and gate of S7 stands.

---

## AMENDMENT 7 — 2026-08-26 (POST-FIRST-GRADED-COMPUTE): the digitiser reads the closed-box, inward-tick, line-connected figures; the reference is PRODUCED; §16.7's measured increment is superseded

**Document version 1.7 -> 1.8. Lines whose number changed above this section: 0.** **Disclosure only: no gate, no threshold, no band rule, no cap and no label moves.** §7.3's combination rule (`sqrt(stated² + digitisation²)`), §7.4's floors, §7.5's triple gate and its six-branch order, §7.2's row list and §16.4's ruling that the FACE-AVERAGED set is the graded `h` set all stand unaltered. What lands is the quantity §10 registered the digitiser to **measure** — its own increment — and the values it reads. Drafted by lab-lane `64b13819` (heat-transfer), the T5 reference lane, on the heat-transfer supervisor's ruling of 2026-08-26T17:02Z; **the supervisor's personal diff read of `verification/runs/T-family/T5_runs/digitise_t5.A7.diff` was done 2026-08-26 ~22:55Z and the amendment is ADOPTED** `[lab-attributed]`.

**Condition (`CLAUDE.md` rule 2), and how it was checked — this is POST-first-graded-compute and it is stated plainly.** `ls -d <case>/[0-9]*` over `T5_CUBE_c H_c T5_CUBE_m P_m L_m T5_CUBE_f` at **2026-08-26T22:39:29Z**: `T5_CUBE_c` `0/ 3000/ 4000/ 5000/`; `H_c` `0/ 3000/ 4000/ 5000/`; `T5_CUBE_m` `0/ 3000/ 4000/ 5000/`; `P_m` `0/ 3000/ 4000/`; `L_m` `0/ 1000/`; `T5_CUBE_f` `0/` only. **Five of six graded case directories hold a numeric time directory, so the pre-compute window is CLOSED and this amendment does not pretend otherwise.** It is admissible under §2's post-compute rule *only* because it is a dated addendum altering no gate, threshold, cap or label: it lands on the **reference-extraction path**, which §10 registered as a task to be performed *after* the comparator freeze and *before* grading, and which §16.11 item 2 recorded as still owed (*"The reference values are NOT YET DIGITISED"*). **§10's separation is met**: lane `64b13819` wrote neither `analyse_t5.py` nor `build_t5.py`, touched `build_t5.py` at no point, and **opened no T5 solver output**. The frozen reference of 16:27Z recorded in its own `provenance.digitiser` that the separation was NOT met; it is met here.

**(a) PATH 1 first, and it is refuted: THERE IS NO TABLE IN THE THESIS.** The ruling put a thesis TABLE ahead of any digitiser. Meinders 1998 contains **no numbered table at all**: the 281-page OCR sidecar carries **0** occurrences of `Table <n>` and **0** of `TABLE` against **172** figure captions in the same file (the reader is demonstrated able to see caption words — not a blind zero), the contents pages carry no *List of Tables*, and the only two `tabulat` hits are prose (*"tabulated criteria"*, p. 22; *"tabulated properties of epoxy"*, Appendix A). Every quantity T5 grades against is published **only as a plotted figure**. Title-page verification (rule 15 / L-144) was done by this lane, not inherited: PDF page 5 rendered and read by eye — *"Experimental study of heat transfer in turbulent flows over wall-mounted cubes / Proefschrift / … Technische Universiteit Delft … op maandag 2 november 1998 … door Erwin Rinaldo MEINDERS"*; PDF `sha256` `36c89a548030eae2aac84f2453c7c531632998433da660f4b655624cff6514cb`, identical to §7.6's registered value.

**(b) The instrument, PROMOTED.** The frozen `digitise_t5.py` (blob `e55d6208`) refused all three registered figures and **its refusals were correct**: it looks for an open L-frame with outward ticks, and the thesis plots are **closed boxes with inward ticks**, Figs 5.37/5.39 carrying **five line-connected symbol series** over a path `A|B|C|D` with full-height partition lines. Its first 506 lines — the open-frame reader, its raster control and its three selftest arms — are carried **verbatim and unedited**; everything else is added below them. Added: a closed-frame `find_frame` / `find_inward_ticks` / `calibrate_closed` (matching found ticks to **registered anchor VALUES**, never inferred from the image, refusing on a missing major, an ambiguous major/minor split, non-uniform spacing, a fit residual over tolerance, or an axis line off its first tick); a template **symbol reader** with an `explain_away` separation rule for touching and overlapping symbols; and a **RANK reader**.

**(c) The RANK reader exists because the symbol reader is MEASURED to fail on Fig. 5.37.** On the real `fig537_lower_crop.png` the symbol reader returns **66 circles (the true count), 1 square of 66 and 3 triangles of 66**: consecutive printed squares *touch* (side 10 px at an 11–12 px pitch, read off the deskewed raster), their edges merge into single runs, a ring template over a merged glyph reads recall 0.68 where 0.82 is required, and the thin-armed `+`/`×` templates then over-fire on the dense ink (**385** and **360** candidates for 66 true markers) so `explain_away` removes the real triangles as claimed by those spurious neighbours. Scans over symbol size 9–13 px, ring tolerance 0.75–1.5 px and ink dilation 1–2 px recovered at best 30 squares of 66. **A reader returning 3 of 66 markers of the registered series is not a reader, and turning its thresholds against the real page until it read would have been a reader fitted to the answer, not a measurement.** The rank reader uses a property of the PAGE, registered before any value was taken: Fig. 5.37 plots the same path for `T_co` = 60, 65, 70, 75, 80 °C, `T_sur` rises monotonically with `T_co`, and the five line-connected curves **never cross** — the thesis says so (printed p. 148: *"the temperature profiles are more or less congruent"*). It identifies **no symbols at all**: it scans column by column, merges ink groups closer than one symbol height (`RANK_MERGE_GAP_PX = 12`) into one curve, calls a column *resolved* only at exactly five groups, and takes the registered series by its **rank from the top**, which the legend on PDF page 151 fixes (`T_co = 80` uppermost, **75 second**). Because the in-frame `REAR`/`TOP`/`FRONT` labels sit in the top eighth of the frame and the hottest series runs *through* them, a pure rank read loses a band of columns (measured: a 0.174 path-unit hole in the control's `TOP` window), so the reader **tracks** the curve from a resolved seed, taking the group nearest the previous centre and stopping beyond `RANK_MAX_JUMP_PX = 25 px`. **The self-check is the point: at every column that does resolve into exactly five groups the tracked centre must still be that column's rank-2 centre within `RANK_ANCHOR_TOL_PX = 3.0 px`, else exit 2.** On the real crop the track disagrees at **0 of 146 / 182 / 134** resolved columns in `AB` / `BC` / `CD`. **The track is bounded to §7.1's central-80 % window exactly, with no margin, and that bound was not chosen to make the reader work**: with a 0.05-unit margin the track ran into the corner dip at `B`, where all five curves plunge and merge, and slid onto a neighbour — the anchor check **caught it and refused** (*"at column 394 all 5 curves resolve and the tracked curve sits 22.4 px from the rank-2 curve"*, rc 2). The margin removed was never part of §7.1's registered window.

**(d) Fig. 5.39 REFUSES, and that refusal is the answer.** Its five Reynolds-number series overlap and cross — §7.3 says so in advance (*"a small scanned plot carrying five overlapping Reynolds-number symbol series"*) — and the measurement agrees: **0 of 907** in-frame columns resolve into exactly five curves, against the registered `RANK_MIN_USABLE_FRAC = 0.50`. **No rank is registered for 5.39** and the reader exits 2 on it. Rows `G1`–`G3` are **ABSENT** in the reference, with the refusal text carried in the file. Under §16.4 those rows are **REPORTED**, not graded, so no graded row depends on them.

**(e) THREE PLANTED CONTROLS, all passing in the same run that produced the reference, and the plant is SIZED TO THE READER (L-340).** Every control travels the same channel as the real artifact, measured not assumed (`pdfimages` on PDF page 162: 1926 × 2816, **bpc = 1, CCITT G4, 301 × 300 ppi**); each is rendered at 300 dpi, skewed by the measured 0.1006°, hard-thresholded to 1 bit, speckled at the measured 4.40 % from a fixed seed and round-tripped through **CCITT Group 4**.

| control | what is planted | recovered vs planted | verdict |
|---|---|---|---|
| **closed-frame POINTS** (Fig. 5.45 shape) | 20 markers at `Re ≥ 3000` of five symbol series in a closed box with inward ticks, plus a 30-marker low-`Re` pile-up | **20 / 20**, 0 spurious of a graded series near the registered abscissa, 20 candidates rejected; **y MAX `0.1453 W/m²K` = `0.1615 %` of span**, RMS `0.0970`; x MAX `8.11` = `0.1352 %` | **PASS** |
| **closed-frame PATH, symbol reader** (Fig. 5.37 shape) | 66 markers of the registered series, 53 inside the central windows, at the page's measured pitch | **52 / 53** in-window, **0 spurious**, 956 candidates rejected; per-marker **y MAX `0.0533 °C` = `0.1776 %`**, RMS `0.0249`; partition-mean MAX **`0.0188 °C` = `0.0628 %`** | **PASS** |
| **closed-frame RANK** (Fig. 5.37 shape; **the reader that produces `G5a-c`**) | **the WHOLE CURVE** — 66 line-connected markers; 570/906 columns resolve (0.629); **250 usable columns per window** | partition means planted/recovered `58.5241/58.4279`, `60.1491/60.0508`, `58.5380/58.4258` → **MAX mean error `0.1122 °C` = `0.3740 %` of span (GATED)**; per-column MAX `0.5920 °C` = `1.9732 %`, RMS `0.1671` (**REPORTED, never gated**) | **PASS** |

**L-340, explicitly.** `G5a-c` are delivered by an **averaging** reader — the path-weighted mean over §7.1's central 80 %, here over **250 columns**. A single-point plant would be diluted by ~1/√N and would refuse a *working* reader: L-340's measured failure (VMFL011, a 1.234e-3 point plant moving an RMS over 401 points by 3.68e-7). **So the plant is the whole curve**, compared **both** per column (a point read) **and** as the partition mean (the averaged read the row uses). A shift of the planted curve moves the recovered mean **one for one**, and the selftest drives exactly that. **Why the rank control gates the MEAN and reports the per-column error:** the row takes the mean; no row takes a single column, and a column cuts the polyline vertically so where the curve steepens the ink in one column spans a large range — a property of a column read, not a defect. The per-column MAX is nevertheless carried **into** the digitisation increment, which makes the increment **larger, never smaller**.

**Instrument discipline, verified at this promotion.** `--selftest`: **3 frozen arms + 3 closed-frame controls + 16 driven refusals, 0 FAILED, rc 0 under BOTH `python3` and `python3 -O`**; `scripts/check_assert_guards.py --require-clean` returns `CLEAN`, **0 `ast.Assert` nodes**. Among the sixteen driven refusals: the planted curve shifted 5 % of span → rc 2; **the MUTATION CONTROL — track rank 1 (`T_co = 80`) and grade it against the rank-2 plant → rc 2**, so a reader returning anything rank-independent cannot pass; a raster with no five-curve structure → rc 2; no registered rank (the 5.39 case) → rc 2.

**(f) §16.7's MEASURED INCREMENT IS SUPERSEDED — STRUCK, NOT DELETED.** **Struck from §16.7:** ~~`h` digitisation error, MAX = `0.2263 W/m²K` = **`0.2514 %`** of the 0–90 span; RMS `0.1374`; `Re_H` MAX `7.07` = `0.1178 %`; planted/recovered 6/6~~ — measured on the **open-frame** control of the frozen instrument, i.e. on a channel shape the printed page does not have (open L-frame, outward ticks, isolated markers). **The words stay on the page; the number no longer governs.** §16.7.1's stated limit — that the figure was *"the error of an ISOLATED marker"* and that **marker overlap is the largest un-modelled error source** — is precisely why it had to be superseded. **REGISTERED IN ITS PLACE:**
...
**§16.7.2 stands unchanged: the EXPERIMENTAL (~5 % mid-face, ~10 % at edges, both hedged in the source) and the DIGITISATION components are named separately and never merged.**

**(g) The increments written into the reference, against §7.3's forecasts.** §10 registers `digitisation_increment = max(symbol radius, blind-repeat spread)` and requires the comparator to **refuse if the measured increment exceeds §7.3's forecast by more than 50 %**. This amendment **widens the rule and never narrows it**: the increment is `max(closed-control MAX error, symbol radius in data units, blind-repeat spread)`.
...
**Both are inside the registered 50 % refusal.**

**(h) The reference, PROMOTED into `T5_reference_primary.json`.** 9 rows, `digitised: true`.

| row | quantity | value | stated unc. | increment | source |
|---|---|---:|---:|---:|---|
...
| `G5a` | mid-line mean `T_sur`, **front** (`CD`) | **55.995 °C** | 0.4 °C | 0.5920 | Fig. 5.37, printed p. 149 (PDF 151), `T_co = 75` curve, 250 columns |
...
| `G1`,`G2`,`G3` | mid-line mean `h/h_tot` | **ABSENT** | — | — | Fig. 5.39 **REFUSED**; REPORTED-only under §16.4 |

`h̄_cube` reconstructed from the five faces of Fig. 5.45 (§7.3's registered route, **not** Fig. 5.41's log-log): **65.840 W/m²K** from front 83.654, top 55.224, rear 59.286, side (N) 65.728, side (S) 65.308. **Two independent checks the thesis's own prose supplies, neither available to the reader and neither used to select a value, and both hold:** *"The high heat transfer coefficient at the front face … is indicative for the impinging flow"* — the front face is the largest of the five, by 27 %; and *"Among these faces, the top face has the lowest averaged heat transfer coefficient"* (printed p. 157) — **the top face is the lowest of the five**. **Blind repeats** (§10's registered repeatability measurement, five perturbed re-reads each): `G5a` 55.963–56.034 (spread 0.071 °C), `G5b` 60.259–60.325 (0.066), `G5c` 58.475–58.546 (0.071).

**Disclosed, because the bytes matter:** the promoted JSON is **byte-identical** to the `T5_reference_primary.A7_PROPOSED.json` the supervisor read (blob `04dfd7e2`); it was **not regenerated at promotion**, so its `provenance.instrument` string still names `digitise_t5.A7_PROPOSED.py` — the pre-promotion filename of the file now frozen as `digitise_t5.py` blob `9a465d7b`. The file the supervisor read is the file that landed, and that chain is worth more than a cosmetically updated string.

**(i) §16.9 THE FREEZE SET — the digitiser row is superseded.** Quoted and struck, per rule 6:
...
| path | blob | role |
|---|---|---|
| `verification/runs/T-family/T5_runs/digitise_t5.py` | **`9a465d7bacb0`** | **produces the reference** |
| `verification/runs/T-family/T5_runs/T5_reference_primary.json` | **`04dfd7e2e56a`** | the reference (was `0ee14235bc8f`, `digitised: false`) |
| `verification/runs/T-family/T5_runs/analyse_t5.py` | `9c2c1d44cfeb` | unchanged (AMENDMENT 3/5) |
| `verification/runs/T-family/T5_runs/run_one_t5.sh` | `313df45c85b3` | unchanged |

`digitise_t5.A7_PROPOSED.py` and `T5_reference_primary.A7_PROPOSED.json` are **deleted at this promotion** (the A3 pattern). The check-1 artifact `verification/runs/T-family/T5_runs/digitise_t5.A7.diff` (2 399 lines, **+2 392 / −2**, blob `357d9693`) and the record `verification/runs/T-family/T5_runs/REFERENCE_ATTEMPT_2026-08-26.md` are retained.

**(j) A §16.9 FREEZE-SET FILE HAD DRIFTED IN THE WORKTREE; restored, the drift preserved.** Before any of the above, `T5_runs/digitise_t5.py` was found carrying **unfrozen bytes on disk** (worktree blob `ea789ea64d63`, mtime 16:48Z, **+929/−1** against HEAD), written by an earlier, now-dead A7 lane that had edited a frozen file in place. **No frozen file may carry unfrozen bytes on disk while a grade could read it.** Nothing was deleted: the worktree copy was hashed, **preserved** as `T5_runs/digitise_t5.WORKTREE_DRIFT_1648Z.py` (`mv`, never `rm`), and the frozen bytes restored from HEAD (`git hash-object` then read `e55d6208c511`, the §16.9 value) — *before* the promotion above replaced them with `9a465d7b`. **Descent:** the promoted file descends from the **FROZEN** `e55d6208`, not from the drift — its first 600 lines differ from the frozen file by 97 diff lines and from the drift by 387. The drift carries **one capability the promoted file lacks**: `pair_labels`, a `tesseract` OCR pairing of each printed tick **label** against the tick positions, refusing on an unpairable label, two labels sharing an anchor, and a paired count that is not the registered count. The promoted file instead matches found inward ticks to **registered anchor VALUES** and runs no OCR. The drift is preserved so it can be lifted into a later amendment; it is **NOT adopted**.

**Not done by this amendment:** `analyse_t5.py` is **not touched** and stays blob `9c2c1d44`; `run_one_t5.sh` stays `313df45c`; `build_t5.py` is untouched. Fig. 5.39 is not read and `G1`–`G3` stay absent. §10's **D4, D5, D6** (Figs 5.38, 5.41, 5.2, 5.3 — `R1`, the ±8 % `h̄_cube` cross-check, `I1`/`I2`) are **NOT digitised** and remain owed. **The `P` rows remain BLOCKED for a second, independent reason recorded here and ruled on separately: the frozen `analyse_t5.py` contains no reference reader and no grading driver at all — its `main()` prints the completion lines and then the fixed sentence *"No case has run: no rows are graded and no verdict is written."*** Every band, floor and gate of §7 stands. **Nothing here has been sent, filed, submitted, uploaded, registered or posted anywhere outside this box (`CLAUDE.md` rule 7).**

---

## AMENDMENT 9 — 2026-08-26 (PRE-FIRST-COMPUTE FOR `S_m`): the S8 `DS` fluid-only meshing branch IMPLEMENTED; `build_t5.py` promoted `c7b7063d` -> `9d7be1cb`; `S_m` built and enqueued
**Document version 1.8 -> 1.9. Lines whose number changed above this section: 0.** Disclosure only: no gate, band, threshold, floor, cap, label, control or verdict moves, for `S_m` or any other case. **ADOPTED by the heat-transfer supervisor after a personal read of the unified diff (`SUPERVISION_CHARTER.md` §3 check 1): supervisor's personal diff read done 2026-08-26 ~23:05Z; adopted `[lab-attributed]`.** Drafted by the T5 grading lane at `docs/campaigns/T-family/T5_AMENDMENT9_DRAFT.md`; promoted by grading lane 2 on the AMENDMENT 3 pattern.

**(a) Condition (`CLAUDE.md` rule 2), and how it was checked.** Compute HAS happened on this rung — `X_2d`, `T5_CUBE_c`, `H_c`, `T5_CUBE_m` and `P_m` are DONE, `L_m` and `T5_CUBE_f` are running — so this amendment is **pre-first-compute for `S_m` ONLY** and is held to the stricter standard. Checked at 2026-08-26T22:30:21Z by the drafting lane and re-checked at promotion by `ls`, not asserted: `verification/runs/T-family/T5_runs/S_m` **did not exist**; there was **no `STATUS.S_m` and no `DONE.S_m`** at the `T5_runs` root; **zero core-minutes had been spent on the `S_m` arm** — never built, never armed, never enqueued, its one drafted queue entry only ever REFUSED. **`build_t5.py` is NOT in the §16.9 freeze set**, which says so in terms: *"`build_t5.py` is NOT in this set and is NOT written"* — its blob was `c7b7063d`. Nothing a verdict depends on is read by this amendment.

**(b) The defect this repairs, stated as it actually is.** AMENDMENT 8(d) recorded *"`S_m` is NOT enqueued: its launcher refuses without `M`'s surface T."* **That was only one of two refusals, and it is the one that had since become clearable.** `build_t5.py` refused `S_m` twice: at l.575 for the missing `--s-m-tsurf` (clearable once `T5_CUBE_m` was DONE), and at l.624 with *"S_m fluid-only meshing (remove epoxy cells) is not implemented until T5_CUBE_m has run"* — **NOT clearable by `M` running**, because it is the `else` arm of the conjugate `splitMeshRegions` step and there was no fluid-only meshing code behind it at all. Its wording implied a precondition; what it guarded was an unwritten branch. **A9 fills that gap, and naming it correctly is half the point.**

**(c) What changed — 4 lines removed, 65 added, two hunks, confined to one branch and one new function.** Unified diff preserved at `verification/runs/T-family/T5_runs/build_t5.A9.diff` (85 lines, blob `f8d37ff3`) — this is the artefact the supervisor read.
- **Hunk 1, the meshing branch (l.618–628).** `splitMeshRegions -cellZonesOnly -useFaceZones -overwrite` moves OUT of `if conjugate:` and runs for both arms, because both arms need the same cut; the branch that follows chooses what to do with the result — `rename_interface_patches(case)` for the conjugate arm (byte-unchanged), `strip_solid_region(case)` for `S_m`. The l.624 `refuse` is removed. The cleanup loop below it is untouched and needs no change: its entries are existence-guarded.
- **Hunk 2, one new function `strip_solid_region()`**, inserted after `rename_interface_patches` and before `build_x2d`. It renames the four air-side interface patches `<face>_air_to_epoxy` -> `<face>` (the same bare names `analyse_t5.py`'s `YPLUS_WALLS` reads); inside those four patch blocks **only**, turns `type mappedWall;` into `type wall;` and drops `sampleMode` / `sampleRegion` / `samplePatch` / `offsetMode` / `offset`, which name a neighbour region that is gone; deletes `constant/epoxy`, `system/epoxy`, `0.orig/epoxy`, `0/epoxy` and the `cellToRegion` addressing left by the split; rewrites `constant/regionProperties` to the fluid-only form; and then **re-reads the bytes back from disk and REFUSES** on a cube patch absent, an un-renamed `_air_to_epoxy` name, a surviving `mappedWall`, a surviving `sampleMode`/`sampleRegion`/`samplePatch`, or a surviving `epoxy` region. **Six refusals, all on facts read back rather than on the code's intentions.**
- **The fluid-side cells are removed by `splitMeshRegions` itself** — the whole reason the conjugate cut is reused. `strip_solid_region` does not touch geometry.
- **What A9 does NOT change.** No band, floor, gate, threshold, cap, label or control. Not `analyse_t5.py` (`9c2c1d44`), not `mark_done_t5.py` (`a74ce20d`), not `run_one_t5.sh` (`313df45c`), not `digitise_t5.py`, and no §7 gate. Not `air_fields()` — its non-conjugate branch (`cube_T = ["type fixedValue;", "value uniform %g;" % t_surf]`) **already existed** and is used unchanged. Not `main()` — `--s-m-tsurf` already existed.

**(d) The registered input, MEASURED.** §8 `DS` registers the cube surface at *"a uniform temperature equal to the **area-averaged** conjugate surface temperature from the conjugate medium case."* Measured from `T5_CUBE_m` at `5000/`, on a scratch copy so the DONE case was not written into, with OpenFOAM's own `surfaceFieldValue` (`areaIntegrate` of `T`, `writeArea true`, per patch, air side): `cube_front` 0.03681321204 K·m² / 0.0001125 m²; `cube_rear` 0.03720046580 / 0.0001125; `cube_top` 0.03737210366 / 0.0001125; `cube_side_n` 0.07487779141 / 0.0002250; totals **0.18626357291 K·m² / 0.0005625 m²**. **Area-averaged `T_surf` = 331.135241 K**, between the registered inlet `T_IN = 293.65 K` and the core `T_CORE = 348.15 K` as a cube surface under a 9:1 conjugate jump must be.

**DISCLOSED, and not silently repaired.** `air_fields()` writes with `%g`, so `0.orig/air/T` carries `value uniform 331.135;` — **331.135, not 331.135241**. The loss is 0.24 mK on a driving difference of ≈ 37.5 K (6.4e-06 relative) and moves no registered band. **A9 does not change that format string**, because `%g` is shared by every field of every case and widening the diff to chase 0.24 mK would put the conjugate arms' bytes at risk for no physical gain. If full precision is wanted, the targeted change is `%g` -> `%.9g` on that one line, as its own amendment with its own byte-identity control.

**(e) Driven before proposal — two controls, and the second is the one that matters.** Both on scratch copies outside the run tree; **no solver ran**. Scratch spend ≈ 0.4 core-min, **not charged to the rung** (the AMENDMENT 8 precedent).
- **CONTROL 1 — the conjugate path is byte-for-byte unchanged.** `T5_CUBE_c` was built TWICE into separate scratch roots, once with the frozen `build_t5.py` and once with `build_t5.A9_PROPOSED.py`; `diff -r -x 'log.*'` between the two trees reports **NO DIFFERENCE**. A9 cannot alter a conjugate case, and this is measured rather than argued from the diff's shape.
- **CONTROL 2 — the mesh-quality result is the conjugate air region's own, not something A9 introduced.** `S_m`'s `checkMesh -allRegions -allTopology -allGeometry` reports `***Cells with small determinant (< 0.001) found, number of cells: 15727` and `Failed 1 mesh checks.`; **the conjugate `T5_CUBE_m` built with the same builder reports the SAME 15 727 cells and the same single failed check on its air region.** The two are the same mesh. This is AMENDMENT 5's already-disclosed mesh-quality mismatch — **expectation, not precondition** — unchanged, neither improved nor worsened, by A9. *(The `Mesh OK.` in the live `T5_CUBE_m/log.checkMesh` is not a contradiction: that file was rewritten by the launcher's own `checkMesh` at 20:57Z, which does not pass `-allGeometry`, so it never ran the determinant check.)*

**(f) THE PROMOTION, as executed 2026-08-26 ~22:50–22:52Z.** `build_t5.A9_PROPOSED.py` was copied over `verification/runs/T-family/T5_runs/build_t5.py` (mode preserved, `-rwxrwxr-x`) and the `_PROPOSED` copy deleted, exactly as AMENDMENT 3 promoted `analyse_t5.A3_PROPOSED.py`. **`build_t5.py` is now blob `9d7be1cb9b5e206c9bccbe610f7b24427b7f8e99` (was `c7b7063d99e2d056391c20d1f23af9bb3469c189`).** Re-verified at the promotion, not recalled: **0 `ast.Assert` nodes** (L-332); `--selftest` **PASS, 6 arms, 0 FAILED** under `python3` and again under `python3 -O`. **`build_t5.py` remains outside the §16.9 freeze set** — §16.9 is not amended by A9 and no freeze-set row moves. AMENDMENT 7 (commit `cd4a70ec`, landed at HEAD before this amendment was written) restated the freeze set as `digitise_t5.py` `9a465d7b` / `T5_reference_primary.json` `04dfd7e2` / `analyse_t5.py` `9c2c1d44` / `run_one_t5.sh` `313df45c`; **`build_t5.py` is absent from that restatement too**, checked by reading it at HEAD rather than assumed.

**(g) `S_m` BUILT in the run tree.** `python3 build_t5.py --case S_m --s-m-tsurf 331.135241` -> **rc 0, wall 8 s, 1 rank = 0.133 core-min**, not charged to the rung's solve budget. Verified by reading the built case, not by trusting the builder: `cells: 212942` in `S_m/log.checkMesh` — **exactly `T5_CUBE_m`'s air-region count** (the conjugate case is 212 942 air + 3 272 epoxy), so the epoxy cells are gone and no fluid cell moved; `constant/` holds `air g polyMesh regionProperties` and **`find` for `*epoxy*` over the whole case returns nothing**; `grep -c` for `mappedWall|sampleMode|sampleRegion|samplePatch` over `constant/air/polyMesh/boundary` = **0**; `constant/regionProperties` carries the fluid-only form; `0.orig/air/T` `cube_front` reads `type fixedValue; value uniform 331.135;`. **Age guard clean at the build: the case holds `0.orig` and NO `0/` and NO numeric time directory** (`0.orig` is the only entry the `[0-9]*` glob matches). The builder's own `foam_libs` NOTE (`system/controlDict has no FoamFile banner terminator; appending libs entry at end of file`) is recorded here rather than dropped; it is the L-221/L-222 insert-with-assert path, not a replacement.
