...
| **grey radiative exchange, curved geometry — inner sphere** | **T10a S0**, inner sphere `q` [W/m²] | **NOT A RESULT** | triple **DIVERGENT** (`p` −3.253), no band armed; the outer-sphere row-sum defect (4.3–4.8 %, non-converging under fixed quadrature) dominates | **H-3b characterisation**: mesh-family sweep, geometry sweep, reproducer from clean case; upstream candidate #4, **filing remains Sanaa's call**. REPORTED 2026-08-22 — cause identified (T10a-VF, D457): `viewFactorsGen` regularisation constant; workaround `alpha = exp(-3/2)` or `distTol = 1` documented, **not yet graded** |
| **grey radiative exchange, curved geometry — outer sphere** | **T10a S1**, outer sphere `q` [W/m²] | **NOT A RESULT** | triple **DIVERGENT** (`p` −1.838), no band armed | as above. REPORTED 2026-08-22 — cause identified (T10a-VF, D457): `viewFactorsGen` regularisation constant; workaround `alpha = exp(-3/2)` or `distTol = 1` documented, **not yet graded** |
