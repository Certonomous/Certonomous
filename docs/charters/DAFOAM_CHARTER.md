...
# 19. Amendment v1.0e — 2026-08-31: one additive clause, §18.6, on the provenance tag every relayed number carries

**Dated addendum, appended at the foot; append-only. ONE clause ADDED as §18.6 — a new number, so nothing above renumbers. NO existing gate, threshold, band, cap or label is altered, widened, narrowed or retired; no clause above is edited or struck; no verdict is affected; the verdict vocabulary of `CLAUDE.md` rule 1 and §8 is untouched. THE AMENDMENT IS ADDITIVE ONLY: it makes MORE reports refusable and makes NOTHING reportable that was not reportable before.** Retiring or widening a gate, threshold or charter clause is reserved to Sanaa (`CLAUDE.md` rule 9 and the FIRST-ACTION rule's reserved list); nothing here does either.

**Why the amendment record is §19 and the clause is §18.6.** §18 is the frozen v1.0d record and is not edited — rule 6. But readers cite *clauses* by `§18.x`, and this clause is of exactly the kind §18.1–§18.3 are: an additive disclosure duty with a stated, mechanical refusal ground. So the clause continues that series and the amendment record that carries it takes the next free section number. **Nothing in §18 is altered by this; §18.4's enforcement table is NOT edited, and §18.6 carries its own call-site statement in §18.4's voice instead.**

**Version: v1.0d → v1.0e.** As in §17, §18 and the `ADJOINT_VERIFICATION_STANDARD.md` v1.0a/v1.0b/v1.0c precedent, the version is carried in THIS heading and the `Version 1.0c` line at `:3` is deliberately NOT edited: editing it would renumber nothing but would rewrite a frozen line, and this addendum inserts nothing and edits nothing above itself.

**Lines whose number changed above this section: 0 — PROVED ON BYTES, not asserted.** Before the append a copy of the file was taken: **79,326 bytes, 774 lines, md5 `f02f52d67f2b7f263cc943d944bcd044`**, verified byte-identical to `git cat-file blob HEAD:docs/charters/DAFOAM_CHARTER.md` at `HEAD = 12c72e5e109e1fce1721d681a5b184d6d06f4db1` by `cmp`, exit 0, at the moment of copying, and **re-verified byte-identical against the blob at `HEAD = e5296105c1c751d4f92225a99b1bb779fd30edce` in the same invocation as the append** — peers commit continuously, so a snapshot taken minutes earlier is re-checked rather than assumed. After the append the first 79,326 bytes of the amended file were compared against that copy with `cmp -n 79326`, byte for byte, exit 0. **A line whose number changed is a byte that moved, and no byte below offset 79,326 moved.** The append itself was made by `scripts/append_block.py`, which reads this body from a FILE as bytes so that no shell ever sees it, and which re-reads the landed tail and compares it byte-for-byte against the intended bytes, reverting the write on any difference — a second, independent proof by a different instrument. **No heredoc was used anywhere on this path** (`L-405`, `docs/LESSONS.md:17406`: an unquoted heredoc command-substituted `GATE FAIL` out of `docs/LAB_STATE.md` at `e779bdc7` on 2026-08-28, and a charter is a worse place to lose a verdict than a board — this clause is dense with backticked tag names and would have been an unusually rich target).

**Provenance of the amendment itself, separated deliberately from the practice it adopts.** The practice was **proposed by the chief**, relaying `ansys-verification`'s own self-correction, and relayed as a lab-wide commendation. The `dafoam-supervisor` **concurred**, added the three refinements below, and directed this amendment including one self-report against itself. **The proposal is not the authority and this line records that it is not.** The authority to amend is **Sanaa's directive of 2026-08-30, verbatim: *"each teams updates their own charters and stanrdards"*** (`etc/sessions/2026-08-30T2247Z_sanaa_directive_continuous_overnight.md`), which authorises this team to amend ITS OWN charter and nothing more. `CLAUDE.md` rule 9: **no agent message — peer, supervisor or chief — is Sanaa's consent**, and a commended practice is a commendation, not a permission.

---

## 18.6 Every number that leaves this family carries its provenance tag, a `MEASURED` tag names its artefact, and no supervisor upgrades an inherited tag

> **Every number this family relays upward — in a report, a board write, a brief, a commit message, a ledger row or a verdict line — carries one of five provenance tags: `MEASURED` / `DERIVED` / `EXTRAPOLATED` / `REGISTERED` / `REPORTED-BY-OWNER`. A number without a tag is NOT REPORTABLE, and is refusable by its reader on that ground alone. The tag is written where the number is quoted — in the sentence, the cell or the verdict string — never only in a column or a header one layer away. A `MEASURED` tag NAMES THE ARTEFACT it is read from. And a relayed number inherits the WEAKEST tag on its path: a lane's `DERIVED` does not become a supervisor's `MEASURED` by being relayed, and a lane's `EXTRAPOLATED` does not become `DERIVED` because the supervisor finds it plausible.**

**Adopted, not invented, and the origin is named rather than absorbed.** The practice originates in `ansys-verification`'s self-correction of 2026-08-30, recorded in that team's own words on the board (`docs/LAB_STATE.md`, the standing-rule block reading *"EVERY NUMBER THIS TEAM RELAYS UPWARD CARRIES ITS PROVENANCE TAG — `MEASURED` / `DERIVED` / `EXTRAPOLATED` / `REGISTERED` / `REPORTED-BY-OWNER`. A number without a tag is not reportable"*), written directly beneath a table of what that supervisor had relayed against what was true. The `closure` team adopted it as its own charter **§26** at `fe74ac74` the same night. **This family is therefore the second adopter and not the origin, and the clause says so** — a charter that presents a borrowed discipline as its own invention has already broken the rule it is writing down.

**This GENERALISES an existing lab rule; it does not invent a duty.** `CLAUDE.md` rule 12 already binds the tag for **one** quantity class. Money: *"a cost is never called measured unless a record backs it"*; the box cannot read its own billing (`COMPUTE_BUDGET_CHARTER.md` §5), so *"any cost from it is **reported-by-owner, not measured**, and `cost_basis` must say so"*; and the calibration paragraph requires dollars *"derived at the recorded rate and labelled **derived, not measured**"*. **Three of the five tags are already lab law, for dollars, for a reason that is not peculiar to dollars.** The reason is that the reader cannot see the instrument from where they stand. That is true of a core-minute read from a ledger, a ratio computed from a denominator the reader never sees, a cell count copied from a dict, a residual quoted from a log tail and a file count taken from a manifest. **§18.6 extends the money discipline to every quantity, which is why it is easy to accept — nobody has to be persuaded that the tag is worth carrying, only that the argument for carrying it on dollars does not stop at dollars.**

### Refinement 1 — `EXTRAPOLATED` is the tag that was missing, and a right number with a false provenance is still a false claim

The three figures at the origin were not fabricated. The record's own sentence is the one this clause rests on: *"every number existed and was honestly computed — but the provenance is stripped in transit, and a fitted extrapolation reported as an observation is a false claim about evidence even when the number is right."* **Recorded precisely, because a clause must rest on what the document says and not on the shape of the story:** the record does **not** say all three relayed figures were numerically right. One of them — an asymptotic `deltaT` relayed as *"measured 0.011377 s"* — is recorded as **NEVER OBSERVED**, with the probe stopping at `deltaT = 0.01502` still falling and two defensible extrapolations bracketing it at 0.01176 and 0.01044. **The weaker, sounder statement is the one that carries the clause, and it is enough:** the failure was in the evidence claim, not in the arithmetic.

**Why that makes the tag load-bearing rather than decorative.** A reader told a number was `MEASURED` **will not re-derive it**. That is the whole point of the tag — it tells the reader where scrutiny is owed. So a wrong tag does not merely mislabel a number: it **disables the very scrutiny that would have caught the number if it HAD been wrong**. A `MEASURED` stamp on an extrapolation buys silence in both directions at once. **THE TAG IS NOT DECORATION ON THE NUMBER. IT IS A CLAIM ABOUT WHAT WOULD HAPPEN IF SOMEONE WENT LOOKING** — and it is checkable in exactly the way the number is not, because the artefact either exists and says that, or it does not.

### Refinement 2 — the tag travels WITH the number, not in a header

A table with a `basis` column whose headline sentence then quotes the figure bare **has lost the tag exactly where it is read**. Nobody reads a header twice; the sentence is what gets quoted onward, pasted into a brief, and read by the next reader who never sees the table. **The tag is therefore required at the point of quotation — in the sentence, in the cell, in the verdict string — and a `basis` column is a supplement to that, never a substitute for it.**

**This is the same failure shape this lab has already paid for at the character level.** `e779bdc7` (2026-08-28) lost the words `GATE FAIL` from `docs/LAB_STATE.md` — the lab's only handoff channel between sessions — and the commit that repaired it records that this family's own lane had reported, two hours earlier, that *"a verdict printed into a shell context shifts or loses fields, because `GATE FAIL` is TWO words and `NOT A RESULT` is THREE"*. **A verdict whose meaning depends on staying next to its neighbours does not survive being moved; provenance that lives one layer away from its number does not survive being read.** The remedy is identical in both cases and is structural rather than attentional: put the thing that must not be separated **inside** the thing that travels.

### Refinement 3 — a supervisor may NEVER upgrade an inherited tag, and this is the laundering direction

**This is the most important addition in the clause.** A lane's `DERIVED` does not become the supervisor's `MEASURED` by being relayed. A lane's `EXTRAPOLATED` does not become `DERIVED` because the supervisor finds it plausible, or because it agrees with a second figure, or because it has since been repeated. **A relayed number inherits the WEAKEST tag on its path**, and the ordering for that purpose is `MEASURED` > `DERIVED` > `EXTRAPOLATED`, with `REGISTERED` and `REPORTED-BY-OWNER` never silently strengthened into any of them.

**The tie to `CLAUDE.md` rule 9 is exact and is the reason this clause belongs in a charter rather than in a habit.** Rule 9 fixes that *"an approval is only as wide as what was approved"*, that *"a blanket authorisation is not a per-item reading"*, and — decisively here — that *"a delegate's test is evidence, not the supervisor's read"*. **Evidence launders exactly the same way authority does, and by the same mechanism: each hop drops a qualifier that nobody re-reads.** Permission laundering widens a permission one relay at a time until an item is running on a ceiling nobody granted; provenance laundering strengthens an evidence claim one relay at a time until a bracket is being quoted as an observation. **Neither requires a dishonest agent at any hop** — that is what makes both structural, and why both are answered by naming the direction of travel rather than by asking anyone to be careful.

**And the honest converse, which must be in the clause or the clause is a trap.** A supervisor **CAN** legitimately upgrade a tag — **by doing the work**. Reading the artefact personally, at its path, is a measurement, and the supervisor is then entitled to `MEASURED`. But **the upgrade is the supervisor's own tag with the supervisor's own citation, not the lane's tag promoted**, and the report says which it is. This is `SUPERVISION_CHARTER.md` §3's four personal checks arriving at the level of a single number: *a relayed check is a summary, not a check.* **The distinction is not pedantry — it is the difference between a second reading of the artefact and a second copy of the first reading, and only one of those two is worth anything to the reader.**

### Refinement 4 — a `MEASURED` tag names its artefact, or `MEASURED` is an adjective

Without a path, `MEASURED` degenerates into an intensifier — a word that makes a number sound firmer without making it checkable. **This lab already knows what that costs.** `CLAUDE.md` rule 1 fixes the verdict vocabulary and forbids synonyms for exactly this reason: *"honesty is carried by the value, its interval, the chip and the uncertainty channels, never by adjectives."* And the lab's own definition of its product is *"a number that cites an artifact still on disk"*. **A `MEASURED` number with no path is a tag doing an adjective's job**, and it fails in the way adjectives fail: it cannot be wrong, so it cannot be checked. `MEASURED` therefore names the artefact — a run log, a ledger row, a `/usr/bin/time -v` capture, a `C-` row, a JSON field — and a number whose artefact is gone is not `MEASURED` at any strength of recollection.

### Three worked examples from this family, on one night, and they are examples rather than praise because each one COST the report something

**(a) `AVWC` declined to call an unrecoverable execution measured, and paid for it with its subject.** `cases/dafoam/ladder-a/A1/curriculum_AVWC/RESULTS.md` §7 and `docs/COST_CALIBRATION.md` row `C-212`. The original 2026-08-28 execution's *"wall time was **never instrumented and is not recoverable**"* — the session died before it was recorded and the working directory was destroyed by the reboot — so **the entire cost table is labelled as measuring this lane's REPRODUCTION of the registered work rather than the execution the item actually ran**, under a heading that says so in the first line: *"A necessary honesty about which execution this measures."* Separately, the lane-added must-flag driver was run twice, *"the first run untimed by this lane's own oversight"*; that run's cost is stated **NOT MEASURED** and **bounded** at `≤ 0.02177 core-min` by the second, which performed identical work, and the item's total is carried as a bound — `≤ 1.37706 core-min, ≤ $0.0011774 DERIVED`. **A bound is what an honest tag leaves you when the measurement is gone, and it is worth more than a plausible figure because a reader can tell which way it can be wrong.**

> **PRECISION, recorded against this charter's own brief rather than smoothed away.** The description of this example that reached this clause said the untimed portion was *two selftest runs that predated the lane's timing harness*. **That is FALSE.** Both selftest runs were timed by `/usr/bin/time -v` and stand in the table as `MEASURED` — `python3` cold at **75.2358 wall s** and `python3 -O` warm at **3.7372 wall s** — and it is precisely their agreement on byte-identical work that produced `C-212`'s 20.1× cold/warm finding. The untimed items are the two named above. **This correction is disclosed because it is a SECOND INSTANCE OF THE CLASS THIS CLAUSE LEGISLATES:** a characterisation of a measurement, relayed one hop, arriving as a statement about the measurement. It was caught by opening the artefact, which is the only way any of them are caught.

**(b) `SO-2a`'s repair wrote an inequality sign, and the sign IS the tag.** `docs/COST_CALIBRATION.md` row `C-215`. The cleaned figure is **0.367 core-min at ranks 1** — the three drives that produced the committed evidence, 19.83 + 0.31 + 1.89 = 22.03 wall s. The gross figure is **`≥ 0.462 core-min`**, *"and the inequality is deliberate"*: four drives are timed and summed to 27.74 wall s, and *"one drive is NOT TIMED and is therefore not in the figure — the first, failing run of the refusal selftest, the one that exposed the harness's own `PERMISSION` defect. **It is named rather than estimated.**"* **That drive could have been estimated at a second's resolution and folded in silently, and no reader would ever have known.** Writing `≥` instead costs the row its exactness and buys the reader the one thing an exact figure could not have given them: **which direction the number can be wrong in, and why.**

**(c) `SO-2a` also WITHHELD its calibration ratio rather than divide by a denominator that priced a different program.** Same row. The predicted column reads **NONE REGISTERED — stated as absent rather than approximated**, because *"no pre-registration prices an instrument repair, so this row has no predicted figure and therefore no ratio"*, and the item's own registered **9.19 core-min at cap 79.0** is explicitly *"not the predicted column here and is **not** compared: comparing an item estimate against a repair's instrument time would be a **category error** and would corrupt the ledger's ratio series."* **This is §18.1's program statement arriving at the calibration end of the same document.** §18.1 forbids pricing an arm from a row that measured a different program; **§18.6 supplies the companion at the reporting end — a ratio whose denominator prices a different program is not reportable AT ANY TAG, because there is no tag that makes a category error true.** A withheld number and a tagged number are the same discipline: both refuse to let a reader believe something the artefact does not support.

> **PRECISION, again against this charter's own brief.** This example reached the clause attributed to `AV2RG`. **That is FALSE, and the artefact says the opposite:** `AV2RG` **published** its ratio — **≈ 0.033, 1.3 % of cap** (`docs/COST_CALIBRATION.md` row `C-216`; `cases/dafoam/ladder-a/A1/curriculum_AV2RG/RESULTS.md:201`) — and was right to, because its denominator prices its own registered program. The withheld ratio is `SO-2a`'s. **Two of the three worked examples that reached this charter came from the SAME ledger row, and a third item was named for doing the opposite of what was claimed of it.** Neither error changed a number and neither was dishonest; both are what a relay does to an attribution when nobody re-opens the artefact.

### And one against the supervisor of this family, because a charter that cites only its authors' successes is worthless

**The `dafoam-supervisor` relayed — to the chief, and in briefs — that D6R's `ACC_mp` cap *"cited `C-94`, a 45-second acceptance primal"*. That is FALSE, and it was verified false against the artefact by the lane that wrote this clause, independently of the supervisor's own correction:**

* `grep -c 'C-94' cases/dafoam/ladder-a/A2/curriculum_D6R/PREREGISTRATION.md` returns **0**. The string does not occur in that document.
* The anchor cell at `:270` reads **`9.0 × 1.6308`**. The `1.6308` does have a derivation, at `:258` — D6's registered `6.389 × 3 = 19.167`, measured/registered `= 1.6308×`. **The `9.0` has no row, no program and no provenance anywhere in the table.**
* The `C-94` trace belongs to **D5** and is quoted inside `C-139`, which is exactly what §18.1(i) already recorded when it was written: *"D6R's pre-registration does not cite `C-94` at all — the string occurs **zero** times in it."*

**What was relayed upward as a citation was a LANE'S CHARACTERISATION of a citation — a `REPORTED` quantity, tagged nowhere, arriving as an established fact about a document.** The supervisor caught it and recorded it against itself on the board under the heading *"MY OWN HEADLINE CLAIM WAS WRONG, AND THE TRUTH IS WORSE"*. **And the state of the board at `HEAD = 12c72e5e` when this clause was written is worth recording in the past tense, because it is the sharpest thing in this section:** the correction stood in one block of `docs/LAB_STATE.md` while **the uncorrected form still stood in another block of the same file**, reading *"`ACC_mp`'s 30.0 core-min cap traces to `C-94`, a 45-SECOND ACCEPTANCE PRIMAL"*. **A correction is not a repair of everything the error touched.** This lane reported that to the supervisor and did **not** edit it: the board is the supervisor's, and a lane silently amending a supervisor's self-report would destroy the only evidence that the error was ever made.

**Why the supervisor is the last place the tag can be lost — the structural reason, not the rhetorical one.** Below the supervisor, a lane's number sits beside the artefact that produced it, and a reader who doubts it can walk to the file. Above the supervisor, the artefact is out of view and **only the tag survives**. A qualifier dropped at any earlier hop can still be recovered by opening the case directory; a qualifier dropped at that hop is **gone**, because the reader above has no case directory to open. **That is why keeping the tag is the supervisor's duty and is NOT delegable to the lane that produced the number** — the lane cannot perform a check at a hop it is not standing on.

### CALL SITE — what a reader can refuse today, what a check would assert, and what is NOT automatic

**Refusable by the reader today, on stated mechanical grounds, any one sufficient:**

1. **A number quoted in an upward report, a brief, a board write or a commit message with no provenance tag adjacent to it in the same sentence, cell or verdict string.** The reader names the number and refuses it; the report is incomplete, not merely unpolished.
2. **A `MEASURED` figure whose line carries no artefact path.** `MEASURED` without a path is an adjective (Refinement 4).
3. **A supervisor's tag on a figure that is stronger than the tag the lane gave the same figure**, where the supervisor's line carries no independent artefact citation of its own. This is the laundering ground and it is the one worth building first.
4. **A ratio, rate or percentage whose denominator is not stated**, where the numerator and denominator could price different programs (§18.1's shape, at the reporting end).

**What a check would assert, stated so it is buildable.** For every figure appearing in both a lane's report and the supervisor's relay of it: parse the tag adjacent to each occurrence; assert `tag(supervisor) ≤ tag(lane)` on the ordering `MEASURED > DERIVED > EXTRAPOLATED`, unless the supervisor's occurrence carries a path token of its own. Separately, over any upward report: flag every numeric literal with no tag token within its sentence. **THE COMPARISON IS BUILDABLE AND DOES NOT EXIST.**

**NOTHING ENFORCES THIS CLAUSE. No script performs any part of it at this commit**, and it is said in those words rather than left to be discovered, exactly as §13 and §18.4 say it for the ten clauses before it. **§13's sentence stands and now covers one more row: *a clause nobody can fail is a preference.*** The difference between this and a preference is that all four refusal grounds above are mechanical tests over a specific artefact, and a reviewer can apply them tonight without any instrument being written.

**The planted control the first version of that check must be born on** (`CLAUDE.md` rule 3, applied to the checker rather than to a comparator — a checker reporting zero findings must first be shown able to report a non-zero): it **must flag** the sentence *"`ACC_mp`'s cap cited `C-94`, a 45-second acceptance primal"* when audited against `cases/dafoam/ladder-a/A2/curriculum_D6R/PREREGISTRATION.md`, where `C-94` occurs zero times; and it **must NOT flag** `C-215`'s *"`≥ 0.462 core-min` … the inequality is deliberate … it is named rather than estimated"*, which is a correctly tagged and correctly bounded figure. **A tag checker that cannot tell those two apart is measuring the presence of words, not the honesty of a claim.**

### What is forbidden

Quoting a number upward with no provenance tag. Reporting an extrapolation as an observation. Strengthening a lane's tag by relaying it. Writing `MEASURED` with no artefact path. Putting the tag in a column and the number in the sentence. Quoting a ratio whose denominator prices a different program at any tag. Treating a correction landing in one place as a correction of everywhere the error travelled.

### What this clause does NOT change

**No gate, threshold, band, cap or label moves. No verdict changes, and no verdict can be changed by it.** A provenance tag is a statement about a **number's evidence** and is never a verdict: the five tags are never written where one of `CLAUDE.md` rule 1's six tokens belongs, never used to soften a `GATE FAIL`, and never mixed into a verdict string in place of one. **`CLAUDE.md` rule 1 and §8 of this charter are untouched.** **Rule 12 stands unaltered in every part** — the unit is core-minutes, every run is costed in its pre-registration, an overrun stops the run and does not get a new budget, and the estimate-versus-actual calibration duty is unchanged; this clause adds a required disclosure alongside rule 12 and removes nothing from it. **§12, §18.1, §18.2 and §18.3 of this charter are untouched, and `docs/dafoam/ADJOINT_VERIFICATION_STANDARD.md` is NOT edited by this amendment.** **Nothing here permits an item, a report or a number that could previously have been refused to now pass.** Retiring or widening any of it is reserved to Sanaa.

---

| what this amendment did | figure |
| --- | --- |
| clauses ADDED (§18.6) | **1** |
| gates, thresholds, bands, caps or labels altered, widened, narrowed or retired | **0** |
| verdicts affected, and verdict-vocabulary tokens added, removed or redefined | **0** |
| clauses above weakened, edited or struck | **0** |
| reports or numbers that would previously have been refused and would now be accepted | **0** |
| other charters or standards edited by this amendment | **0** |
| factual claims in the directing brief verified against their artefacts before landing | **6** |
| of those, found FALSE and corrected in the clause rather than repeated | **2** |
| solver core-minutes spent to establish it | **0.000** |
| lines whose number changed above this section | **0 — proved on BYTES by `cmp -n 79326` against a pre-append copy, exit 0** |

# 20. Amendment v1.0f — 2026-08-31: one additive clause, §18.7, on the resource guard that discards a program silently, and on admitting a second concurrent item

**Dated addendum, appended at the foot; append-only. ONE clause ADDED as §18.7 — a new number, so nothing above renumbers. NO existing gate, threshold, band, cap, floor, ceiling or label is altered, widened, narrowed or retired; no clause above is edited or struck; no verdict is affected; the verdict vocabulary of `CLAUDE.md` rule 1 and §8 is untouched. THE AMENDMENT IS ADDITIVE ONLY: it makes MORE pre-registrations refusable and MORE completion reports refusable, and it makes NOTHING launchable or reportable that was not launchable or reportable before.** Retiring or widening a gate, threshold or charter clause is reserved to Sanaa (`CLAUDE.md` rule 9 and the FIRST-ACTION rule's reserved list); nothing here does either.

**Why the amendment record is §20 and the clause is §18.7.** §18 is the frozen v1.0d record and §19 the frozen v1.0e record; neither is edited — rule 6. Readers cite *clauses* by `§18.x`, and this clause is of exactly the kind §18.1–§18.3 and §18.6 are: an additive disclosure duty with a stated, mechanical refusal ground. So the clause continues that series and the amendment record that carries it takes the next free section number. **Nothing in §18 or §19 is altered by this; §18.4's enforcement table is NOT edited, and §18.7 carries its own call-site statement in §18.4's voice instead.**

**Version: v1.0e → v1.0f.** As in §17, §18, §19 and the `ADJOINT_VERIFICATION_STANDARD.md` v1.0a/v1.0b/v1.0c precedent, the version is carried in THIS heading and the `Version 1.0c` line at `:3` is deliberately NOT edited: editing it would renumber nothing but would rewrite a frozen line, and this addendum inserts nothing and edits nothing above itself.

**Lines whose number changed above this section: 0 — PROVED ON BYTES, not asserted.** Before the append a copy of the file was taken: **105,574 bytes, 884 lines, md5 `9aae5ace0c0afe90a5550d9c1814107b`**, verified byte-identical to `git cat-file blob HEAD:docs/charters/DAFOAM_CHARTER.md` at `HEAD = becd0863d98c20946ba05ab6f48c5094208869e6` by `cmp`, exit 0, at the moment of copying, and **re-verified byte-identical against the blob at `HEAD = ddf32f0cd4f05538b1ff1dfc2151dd550a05487e` in the same shell invocation as the append** — peers commit continuously, so a snapshot taken minutes earlier is re-checked rather than assumed. After the append the first 105,574 bytes of the amended file were compared against that copy with `cmp -n 105574`, byte for byte, exit 0. **A line whose number changed is a byte that moved, and no byte below offset 105,574 moved.** The append itself was made by `scripts/append_block.py`, which reads this body from a FILE as bytes so that no shell ever sees it, and which re-reads the landed tail and compares it byte-for-byte against the intended bytes, reverting the write on any difference — a second, independent proof by a different instrument. **No heredoc was used anywhere on this path** (`L-405`, `docs/LESSONS.md:17406`).

**Authority, and it is not a relay.** **Sanaa's directive of 2026-08-30, verbatim: *"each teams updates their own charters and stanrdards"*** (`etc/sessions/2026-08-30T2247Z_sanaa_directive_continuous_overnight.md`), which authorises this team to amend ITS OWN charter and nothing more. The clause below is written on the **`dafoam-supervisor`'s own reading** of this family's own incident of 2026-08-30/31. **`CLAUDE.md` rule 9: no agent message — peer, supervisor or chief — is Sanaa's consent**, and nothing in this amendment is recorded as authorised by any relay.

**Prior art, checked before novelty was claimed, because §18.6 was written by this family as a SECOND adopter and said so.** `grep` over all seventeen files in `docs/charters/` for an aggregate-resource, co-tenancy, sibling-admission or concurrency-starvation clause returns **nothing of the kind in any charter** — `COMPUTE_BUDGET_CHARTER.md` §4 owns the per-session three-number budget and the auto-stop contract and does not reach the aggregate across concurrent items; `CLOSURE_MODELLING_CHARTER.md` §26 is the provenance-tag clause and is unrelated. **No team got here first, and this clause is therefore not credited to one.** What DOES exist, and is credited below rather than absorbed, is this family's own **`L-262`** (2026-08-23) and **`L-239`** (2026-08-22, already carried in §13's PROPOSAL): the direct ancestors, and both are lessons with no call site — which is why the same class landed again.

---

## 18.7 A resource guard states, where it is registered, what a block DISCARDS; a second concurrent item is admitted against the aggregate of live siblings' caps; and a truncated program is never reported by a token that reads as completion

> **A pre-registration whose resource guard responds to a failing condition by BLOCKING ONE UNIT OF WORK AND CONTINUING states that in those terms, at the registration, together with the fraction of its declared program a single block discards and the fraction a sustained condition would discard. A guard for a TRANSIENT resource condition WAITS AND RE-POLLS under a bounded, registered deadline whose expiry TERMINATES the program with a non-zero rc; an item that registers block-and-continue instead states why the condition it guards is not transient. Before a SECOND item of this family is released to the queue while a first holds compute, the aggregate of live siblings' registered memory CAPS plus the candidate's own cap plus host non-container RSS is checked against the host ceiling, by the reading `cases/dafoam/ladder-a/A1/curriculum_SO1bR/so1br_aggregate_memory.py` implements. And every item reports, at completion, BOTH its DECLARED stage count and its EXECUTED stage count; a run whose executed count is below its declared count is NEVER reported by a token that reads as success, and the shortfall is READ BY A GATE, not merely recorded.**

**The incident, 2026-08-30/31, and it is this family's own allocation decision rather than anyone's bug.** Two dafoam items held compute simultaneously and one starved the other. `W3` (`/home/ubuntu/certonomous-runs/CURRICULUM-D12R2W3-cylinder-unsteady/ledger.txt`) holds **33 `STAGE=` lines of which 20 read `BLOCKED ... NOT LAUNCHED`** `[MEASURED, that path]`. The driver declares **33 stages** at `cases/dafoam/curriculum_D12R2/d12y_w3_chain_driver.sh:8` and the frozen registration declares the same 33-stage graph at `cases/dafoam/curriculum_D12R2/W3_PREREGISTRATION.md:12` `[REGISTERED]`. **So 13 of 33 executed — 60.6 % of the declared program never ran** `[DERIVED, 20/33]`. The 20 GiB that did it was `D6RACC2`'s container (`memory=20g`, run root `CURRICULUM-D6RACC2-a2-wing-multipoint`) `[MEASURED, that root's ledger.txt]`, whose own row names its victim in `siblings_pre=[d12y_w3_S3b_c0_am_20260830T231011Z_1280361]` `[MEASURED, same]`. **Both items were authorised by this family; the guard did exactly what it was registered to do; the registration design is what is at fault.**

**PRECISION 1, recorded against this charter's own directing brief and against `docs/LAB_STATE.md` §S-21, both of which state it the other way.** The twenty blocked rows do **NOT** each carry `memavail_GiB=13.5154`. They carry **twenty DISTINCT readings, 13.5007 to 13.5368 GiB**, each taken immediately before its own stage; `13.5154` occurs **exactly once**, on `S3b_c1_am` `[MEASURED, the twenty distinct memavail_GiB values enumerated from W3 ledger.txt]`. The substance survives — all twenty are below the registered floor of `14.0` — but a single value quoted twenty times would have been evidence of one reading reused, and it is not what the artefact says. **Recorded because §18.6 is this charter's own clause and a characterisation of a measurement relayed one hop is the class it legislates.**

### The asymmetry: same host condition, two registered behaviours, opposite costs

`SO2a` (`CURRICULUM-SO2a-a1-naca0012-geometric-constraint-gradient`) met the same condition on the same night, against the same 20 GiB sibling, and behaved **oppositely**. Its aggregate-memory gate held the `MESH` arm, verbatim at `cases/dafoam/ladder-a/A1/curriculum_SO2a/launcher.queue.out:15`:

```
SO2A_AGGREGATE arm=MESH waited=540 {"live_caps_GiB": 20.0, "live": ["d6ra2_ACC_mp_20260830T235332Z_1606819"], "this_cap_GiB": 4.0, "host_noncontainer_rss_GiB": 2.61, "aggregate_GiB": 26.61, "ceiling_GiB": 30.6, "ok": true}
```

**It waited 540 s, re-polled, got `ok: true`, launched, and lost nothing:** all five arms `rc=0` and `chain=COMPLETE` `[MEASURED, STATUS.chain in that root]`, with `waited=540` on `MESH` and `waited=0` on `X-S`, `G-S`, `X-P`, `G-P` `[MEASURED, launcher.queue.out]`. **`W3` blocked-and-continued:** each stage tested the floor once, failed, was marked `NOT LAUNCHED`, and the loop moved on.

**WAIT-AND-REPOLL COSTS WALL TIME AND PRESERVES THE PROGRAM. BLOCK-AND-CONTINUE COSTS THE PROGRAM AND PRESERVES WALL TIME** — and it does so with **no non-zero rc anywhere on the launcher's own path.** Read from the frozen bytes rather than supposed: the floor branch at `d12y_w3_stage_and_run.sh:362-365` `return 8`s, **but every phase-1 call site is `run_stage ... || echo "STAGE X NONZERO rc"`**, which consumes the non-zero; the loop completes; and the launcher prints its completion token and `exit 0`s at `:1011-1014`. `W3`'s chain carries `launcher_rc=2` `[MEASURED, cases/dafoam/curriculum_D12R2/STATUS.W3_chain]` — and **that 2 is the frozen comparator's, not any stage's.** Nothing in the execution path ever reported a failure.

### Requirement 1 — a guard whose failure mode is silent truncation declares that failure mode where it is registered

`W3`'s registration fixes `MEMAVAIL_FLOOR_GIB = 14.0` and states it is *"not lowered"* `[REGISTERED, W3_PREREGISTRATION.md:12]`. **What it does not state, anywhere, is what happens to a stage that fails it.** A reader of that registration cannot tell whether a breach stops the item, retries it, or discards one stage and continues — and those three have costs of 0 %, wall time, and 3.03 % of the declared program **per stage**, compounding to **60.6 % under a sustained condition**, which is what happened `[DERIVED, 1/33 and 20/33]`.

**So the duty is a disclosure, not a design mandate.** Block-and-continue remains a legal registered behaviour; it is sometimes the right one. What is forbidden is registering it **without naming it**, because the failure mode is silent data loss and **a failure mode discovered in the ledger afterwards was never disclosed at the only moment disclosure could have changed anything** — the pre-registration commit, where `SUPERVISION_CHARTER.md` §3 check 4 already places a supervisor's eyes. **A registration that names its floor and hides its response has told the reader the number and withheld the consequence.**

### Requirement 2 — the aggregate admission check, and the honest reason it is NOT simply "W3 lacked one"

**`W3` HAD an aggregate check, and it PASSED thirty-two times while the item starved.** `aggregate_cap_assert` (`d12y_w3_stage_and_run.sh:285-300`) sums live containers' registered caps plus its own and refuses if that sum exceeds physical `MemTotal`. Its own ledger rows read **`AGGREGATE_MEMORY_CAP peers=20.0 GiB + mine=8 GiB = 28.0 GiB vs physical 30.6440 GiB`, twenty-two times** (plus nine at `peers=0` and one at `peers=20.0625`) `[MEASURED, W3 ledger.txt]`. **28.0 is not greater than 30.644, so the check passed — correctly, on the question it asked — at the same moments the live `MemAvailable` was 13.5 GiB.**

**And `W3` HAD the waiting form too, bounded and registered** — `aggregate_cap_wait`, poll 30 s, bound `AGG_WAIT_MAX_S = 1200`, earned from `W2R-DEF-2` and written into the registration `[REGISTERED, W3_PREREGISTRATION.md:12]`. **It fired zero times**: `AGGREGATE_CAP_WAIT` occurs **0** times in the ledger `[MEASURED, W3 ledger.txt]`.

**THAT IS THE FINDING, AND IT IS SHARPER THAN "IT HAD NO GUARD".** The item put **the WAIT on the STABLE quantity and the BLOCK on the TRANSIENT one.** A sum of registered caps changes only when a container starts or stops; it is exactly the quantity a bounded wait can outlast, and it was armed with one. Live `MemAvailable` changes second by second as a peer's working set grows and shrinks; it is exactly the quantity that clears on its own, and it was armed with a one-shot block-and-continue. **A guard is not made correct by existing in the right form somewhere in the file. It is correct only when the form is attached to the quantity whose behaviour it assumes.**

**Hence the clause binds the READING, not merely the existence of a check, and names the reference implementation by path.** `cases/dafoam/ladder-a/A1/curriculum_SO1bR/so1br_aggregate_memory.py` computes `live container caps + this arm's cap + host NON-CONTAINER RSS` against a registered ceiling, prints one JSON line, and **fails closed** — any exception yields `ok: false`. The third term is the one `W3`'s arithmetic lacks. The driver's loop at `so1br_chain_driver.sh:201-218` supplies the behaviour: poll `AGG_POLL_S = 30`, bound `AGG_BOUND_S = 14400`, every wait a line in `STATUS.<arm>`, and **at the bound `rc=6`, `chain=BLOCKED_AGGREGATE`, `CHAIN_RC=6`, `break` — the chain STOPS with a non-zero rc and names its series.** It is live on disk and was exercised tonight: `SO1BR_AGGREGATE arm=MESH waited=0 {"live_caps_GiB": 0.0, "live": [], "this_cap_GiB": 4.0, "host_noncontainer_rss_GiB": 1.94, "aggregate_GiB": 5.94, "ceiling_GiB": 30.6, "ok": true}` `[MEASURED, cases/dafoam/ladder-a/A1/curriculum_SO1bR/launcher.queue.out:28]`. `SO2a`'s copy is the same reading at `curriculum_SO2a/so2a_aggregate_memory.py`.

**What is deliberately NOT claimed, because the margin is too thin to carry it.** Substituting the reference reading into `W3`'s own numbers gives `20.0 + 8.0 + host_noncontainer_rss` against a 30.6 ceiling, and `SO2a` measured that third term at **2.61–2.70 GiB** minutes after `W3`'s last blocked stage and against the same live sibling `[MEASURED, curriculum_SO2a/launcher.queue.out:15,20,25]` — so the aggregate computes to **≈ 30.61 GiB, over the ceiling by ≈ 0.01 GiB** `[EXTRAPOLATED — the third term was never measured inside W3's own process, the readings are adjacent in time rather than simultaneous, and 0.01 GiB is inside the noise of any of them]`. **A counterfactual that turns on 0.01 GiB is not evidence and this clause does not rest on it.** The clause rests on the quantity-versus-form mismatch established above, which no margin can move.

### Requirement 3 — a transient condition gets a bounded wait whose expiry TERMINATES, and block-and-continue is justified in the registration

The distinction that matters is not wait-versus-block. It is what happens when the bound expires. **`SO1bR`'s bound stops the chain at `rc=6`. `W3`'s floor breach discards a stage and the loop continues to the next one, and the next, twenty times, to `exit 0`.** One guard converts an unavailable resource into a *stopped program with a verdict*; the other converts it into a *silently shortened program with no verdict and a completion token*. `CLAUDE.md` rule 12 already fixes the analogue for money — *"an overrun stops the run; it does not get a new budget"* — and **memory deserves the same shape: a resource condition that cannot be waited out stops the item; it does not quietly buy a smaller one.**

Where an item genuinely wants block-and-continue — a sweep of independent probes, say, where 13 of 33 is a real partial result rather than a ruin — **it registers why the condition it guards is not transient, and Requirement 1's fractions make the price visible before the launch.** Nothing here forbids the design. It forbids choosing it silently.

### Requirement 4 — both counts reported, no success-reading token over a truncated program, and the shortfall must be READ BY A GATE

**The token, verified in the code before being cited.** `d12y_w3_stage_and_run.sh:1011` reads, unconditionally, at the foot of the phase-1 block and outside every conditional in it:

```
  echo "PHASE1_COMPLETE spent=$SPENT_CORE_MIN core-min" | tee -a "$LEDGER"
```

followed at `:1012-1014` by the `NEXT:` hint, the comparator command, and `exit 0`. **It printed `PHASE1_COMPLETE spent=63.2332 core-min` after blocking 20 of 33 stages** `[MEASURED, W3 ledger.txt:108]`. There is no branch on the blocked count, no branch on the manifest, and no branch on any stage's return value: the line executes on every path that reaches the end of the loop. **A phase that discarded 60.6 % of its declared work announced itself `COMPLETE` and exited zero.** So the requirement: the completion line carries **declared** and **executed** counts, and where they differ the token is one of `CLAUDE.md` rule 1's six — `NOT A RESULT` or `BLOCKED` — never a word whose plain reading is success.

**PRECISION 2, and it changes what this requirement has to ask for.** The directing brief and `docs/LAB_STATE.md` §S-21 both state that the block was *silent* and that the comparator caught a manifest/ledger disagreement. **Neither is what the artefacts say, and I checked both.** The launcher's floor branch **writes a manifest row before it returns** (`:364`, a `"blocked": true` row — the `W2R-DEF-1` repair, whose in-code comment says *"A GUARD THAT REFUSES MUST LEAVE THE SAME TRACE IN BOTH ARTIFACTS"*), and it worked: **`manifest.jsonl` holds 33 rows, of which exactly 20 carry `"blocked": true`, matching the ledger's 33 and 20 name for name** `[MEASURED, that manifest, parsed]`. **There is no manifest/ledger disagreement. Both artefacts recorded the truncation, honestly, completely, and in agreement.**

**What actually happened is worse, and it is the reason Requirement 4 ends where it does.** `d12y_grade_w3.py` hands `g0b_manifest_ledger_binding` a row set already filtered by `[r for r in _read_manifest_rows(...) if not r.get("blocked")]`, so `names_manifest` is **13**, the 20 blocked stages fall into `only_ledger`, and the refusal at `:590` reports *"the manifest and the ledger DISAGREE ... `manifest=13 rows, ledger=33 STAGE= lines`. A stage that ran and left no row is UNGRADED AND SILENT."* `[MEASURED, cases/dafoam/curriculum_D12R2/W3_plan.out]`. **Every clause of that diagnosis is false of these twenty stages: they did not run, they left rows in both artefacts, and the artefacts do not disagree.** The refusal is nonetheless CORRECT as a refusal — the item is ungradeable and `W3` is `NOT A RESULT` on any reading — and **this changes no gate and no verdict.**

**So the honest statement of the defect is not "the loss was silent". IT IS THAT THE LOSS WAS RECORDED PERFECTLY AND NOTHING READ IT.** Twenty rows in the ledger, twenty matching rows in the manifest, and **no gate anywhere takes the blocked count as an input.** The one gate that fired reached the right verdict by the wrong mechanism and handed the auditor a data-loss bug to hunt that does not exist — false comfort, which §18.1's incidents already establish is worse than silence. **That is why this requirement demands the shortfall be READ BY A GATE and not merely recorded: `W3` proves that recording it in two independent artefacts, correctly, is not enough.** A registration whose grading path filters blocked rows out before counting — which is right for every physics gate and wrong for this one — needs one limb that counts them **in**.

**PRECISION 3 — one claim in the directing brief is NOT REPEATED here, because no artefact on this box supports it.** The brief states that `PHASE1_COMPLETE` *"is what misled an earlier triage into calling it a normal completion."* **The token's unconditionality and its `exit 0` are verified above from the frozen code; the triage is not.** No record on this box was found in which a reader quotes that token and concludes normal completion. **The requirement therefore rests on what the token WOULD tell a reader, which is provable from `:1011` alone, and not on a reader it is alleged to have told** — an unverifiable link would have been the weakest sentence in the clause and the first one an auditor pulled.

### Attribution, and what this clause owes to whom

**Written on the `dafoam-supervisor`'s own reading**, under Sanaa's 2026-08-30 directive, from this family's own incident. **No relay is cited as authorising it** (`CLAUDE.md` rule 9).

**Its direct ancestor is this family's own `L-262`** (2026-08-23, `docs/LESSONS.md`): *"the launch gate is derived as floor + the largest measured own-peak among the arms + margin, and a registration whose gate cannot satisfy its own floor at the registered peaks is refused at review, not discovered at kill time"*, closing with *"The instrument was right; the registration design was wrong — spend the review on the pair, not the guard."* **That is this clause's thesis, written down eight days earlier, and the class landed again.** `L-262` bound the arithmetic to be self-consistent across ONE item's own arms; §18.7 adds the two dimensions it does not reach — **the SIBLING** (a second item's cap) and **the BEHAVIOUR ON FAILURE** (what a breach discards). **`L-239`** (2026-08-22), already carried in §13's 2026-08-22 PROPOSAL, is the third member of the family: a registered floor with no process able to execute it. **All three are lessons in prose with no call site, which is rule 14's shape and is exactly why §18.1 was written; this clause is the same remedy applied to the same disease.** No other team's work is borrowed here and none is credited, because the charter sweep found no equivalent clause anywhere in `docs/charters/` — stated as a measurement, not an assumption.

### CALL SITE — what a reviewer can refuse today, what a check would assert, and the planted control in both directions

**Refusable by a reviewer at the pre-registration commit, on stated mechanical grounds, any one sufficient:**

1. **A registered resource floor, ceiling or cap with no statement of the guard's response** — stop, wait-and-repoll, or block-and-continue — in the same section that registers the number.
2. **A registered block-and-continue with no discard fraction**: the fraction of the declared program one block discards, and the fraction a sustained condition discards.
3. **A registered block-and-continue for a condition the registration does not argue is non-transient.**
4. **A second item released to the queue while a sibling of this family holds compute, with no aggregate reading** of live siblings' caps + own cap + host non-container RSS against the ceiling — or with a reading that omits the third term.
5. **A guard whose bounded wait is attached to a quantity that cannot change within the bound, while a one-shot block is attached to a quantity that can** (the `W3` shape).
6. **A completion line carrying no declared/executed pair**, or carrying a success-reading token over a run whose executed count is below its declared count.
7. **A grading path that filters blocked rows out of every limb**, with no limb that counts them in.

**What a check would assert, stated so it is buildable.** Over an item's frozen launcher: locate each resource-guard branch; classify its exit as `return non-zero and propagate`, `sleep-and-retry`, or `return non-zero into a `|| echo` call site`; flag the third. Over the completion path: assert the terminal token is emitted under a condition on the executed count. Over the run root at grading: assert `declared == executed + blocked` and that at least one gate reads `blocked`. **THE CHECK IS BUILDABLE AND DOES NOT EXIST.**

**NOTHING ENFORCES THIS CLAUSE. No script performs any part of it at this commit**, and it is said in those words rather than left to be discovered, exactly as §13, §18.4 and §18.6 say it for the eleven clauses before it. **§13's sentence stands and now covers one more row: *a clause nobody can fail is a preference.*** The difference between this and a preference is that all seven refusal grounds above are mechanical tests over a specific, named artefact, and a reviewer can apply them tonight.

**The planted control the first version of that check must be born on** (`CLAUDE.md` rule 3, applied to the checker rather than to a comparator — a checker reporting zero findings must first be shown able to report a non-zero):

* **It MUST FLAG `W3`'s registration** — `cases/dafoam/curriculum_D12R2/W3_PREREGISTRATION.md` with `d12y_w3_stage_and_run.sh` — on grounds 1, 2, 5 and 6: a registered `MEMAVAIL_FLOOR_GIB = 14.0` with no stated response, no discard fraction, a bounded wait on the caps sum while a one-shot block sits on live `MemAvailable`, and an unconditional `PHASE1_COMPLETE` at `:1011`.
* **It MUST NOT FLAG `SO1bR`'s** — `cases/dafoam/ladder-a/A1/curriculum_SO1bR/` with `so1br_chain_driver.sh:201-218` and `so1br_aggregate_memory.py` — nor **`SO2a`'s** equivalent, both of which register the waiting form on the aggregate reading, with the third term present, a registered poll and bound, and a terminating `rc=6` at the bound.

**A checker that cannot tell those two apart is measuring the presence of a memory constant, not the design of a guard.**

### What is forbidden

Registering a resource floor without saying what a breach does. Registering block-and-continue without the discard fraction. Attaching a bounded wait to a quantity that cannot clear within the bound while a one-shot block guards the quantity that can. Releasing a second concurrent item of this family without the aggregate reading. Printing a completion token over a program that did not complete. Reporting an executed count without the declared count beside it. Inferring from two artefacts agreeing that the condition they agree on has been graded.

### What this clause does NOT change

**No gate, threshold, band, cap, floor, ceiling or label moves. No verdict changes, and `W3` is `NOT A RESULT` on the reading that stood before this clause and on the reading after it.** `W3`'s frozen `14.0` floor, its `900.0` core-min cap, its `1200 s` wait bound and every gate `G12R-0`…`G12R-11` are untouched and are not re-opened by this clause — the item is frozen and this amendment does not amend it. **`CLAUDE.md` rule 1 and §8's verdict vocabulary are untouched; no token is added, removed or redefined.** **Rule 12 stands unaltered in every part** — the unit is core-minutes, every run is costed in its pre-registration, an overrun stops the run and does not get a new budget, and the estimate-versus-actual calibration duty is unchanged. **§7's memory-envelope clause is untouched and is not widened**: §7 binds the *prediction* of an item's own peak before its own launch, and §18.7 adds a disclosure about the guard's *response* and an admission check across *siblings*, removing nothing from §7. **§12, §13, §18.1–§18.6 of this charter are untouched, `docs/dafoam/ADJOINT_VERIFICATION_STANDARD.md` is NOT edited, and no other team's charter or standard is edited by this amendment.** §13's 2026-08-22 PROPOSAL is neither ratified nor retired: it binds *every* registered threshold to name the process that can execute it, which is broader than anything taken here; **the narrower duty is taken, the broader one remains a PROPOSAL, unratified, and Sanaa's.** **Nothing here permits an item, a launch or a report that could previously have been refused to now pass.** Retiring or widening any of it is reserved to Sanaa.

---

| what this amendment did | figure |
| --- | --- |
| clauses ADDED (§18.7) | **1** |
| gates, thresholds, bands, caps, floors, ceilings or labels altered, widened, narrowed or retired | **0** |
| verdicts affected, and verdict-vocabulary tokens added, removed or redefined | **0** |
| clauses above weakened, edited or struck | **0** |
| **items that would previously have failed and would now pass** | **0** |
| other charters or standards edited by this amendment | **0** |
| files in `docs/charters/` swept for an existing aggregate-resource or concurrency clause before novelty was claimed (12 of them charters) | **17** |
| of those, found to carry an equivalent clause | **0** |
| factual claims in the directing brief verified against their artefacts before landing | **16** |
| of those, found FALSE or materially imprecise and corrected in the clause rather than repeated | **4** |
