...
- **REGISTERED (successor dir exists):** 5 — VMFL007-R2 → `VMFL007-R3`; VMFL011-R2 → `VMFL011-R3`;
  ...
  **VMFL010 → `VMFL010-R2` (self-similar hex triple REPOSITIONED to finest N=40 not N=80 + residualControl 1e-9; base was ALREADY self-similar r=2 per birth certs, so the oscillation was a ~1e-4 iterative perturbation on a finest level in the noise floor, NOT mesh non-similarity; prereg + comparator FROZEN 2026-09-07, freeze `934605f3`; --selftest 10/10; answer-blind finest smoke rc 0; gate 0.887 ±3% BYTE-IDENTICAL, L-487; LAUNCH HELD on Sanaa's desk)**.
...
| **VMFL010** | `NOT A RESULT` | #2 grid (triple OSCILLATORY) | **VMFL010-R2** — the base was ALREADY a self-similar structured-hex r=2 triple (birth certs: 3600/14400/57600 hex, non-orth 0, skew ~1e-13, junction included), so the "not cleanly refined" hypothesis is FALSIFIED by inspection; the split was textbook 2nd-order in magnitude (\|d32/d21\|=0.198, implied p=2.34) but SIGN-FLIPPED at a finest level (N=80) in a ~1e-4 iterative noise floor. R2 REPOSITIONS the same self-similar family so all three r=2 levels sit at N≤40 (L1/L2/L3 = 10/20/40, finest=base L2) where the leading term dominates, and tightens iteration to residualControl 1e-9; gate 0.887 ±3% BYTE-IDENTICAL (L-487). Directory `cases/ansys_verification/VMFL010-R2/` exists; prereg + comparator FROZEN at **`934605f3`** (comparator blob `44e4bc73`, driver blob `7f6d28a4`; --selftest 10/10; answer-blind finest smoke rc 0, 1490 iters to residualControl 1e-9). Re-runs. **LAUNCH HELD** (Sanaa's desk, rule 9). | REGISTERED |
