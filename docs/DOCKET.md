...
| B6 | **Shipped-helper blindness.** A check written with the same helpers as the thing it checks proves only transcription fidelity. Any load-bearing verification names its EXTERNAL referent, or declares it has none. | L-74 | **CLAIMED 2026-08-11 by session `64b13819`.** The last untouched item in this block. Tonight's audits keep landing on its shape from the side — the sweep helper measured its own frames, the absolutes checker measured its own false-positive rate, the dead-lever audit caught its own regex — and each time the *external* referent was what made the result credible. This item asks the question deliberately instead of by luck. |
