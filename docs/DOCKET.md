...
| B3 | **Absolutes.** No absolute claim ("never", "always", "cannot") ships in a docstring, comment, report or camera surface without an executed test named beside it. Surface list derived mechanically, never enumerated. | L-76 | In flight, session `64b13819`. Must publish a **measured false-positive rate** against a committed hand-labelled sample — the corpus is full of legitimate absolutes (quoted rules, definitional truths), and a checker with an unstated FP rate gets switched off in a day. |
