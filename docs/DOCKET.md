...
| D17 | **The referent declaration does not travel with the number, and the lab has already built the fix once without generalising it.** Five of B6's six live findings are the same shape: a correct referent declaration exists in the canonical record and is absent where a reader meets the figure. The closure score's canon declares *"a local scoring, not an official leaderboard placement"* and pins the scorer to commit `1c4e22c8`; the restatements in `LADDER_V` tables, `ACTIVE_RESEARCH.md`'s body and `PRODUCT_LIST.md` carry the figure alone. F6b Gate V's distributor is named one document away from the verdict (executed: a grep for `McConkey|rmcconke|closure-challenge-benchmark` over `F6b_ERCOFTAC_RESULTS.md` returns **0**), and the travelling phrasings compress to *"reproduces the known answer"* — which is another RANS code's answer. **The chief has already made the rank-1 companion clauses mandatory on every surface; no equivalent travelling rule exists for the referent.** | B6 external-referent audit, 2026-08-11 (§4.3, §4.4, §10) | Generalise the rank-1 companion-clause rule: a figure that carries a gate verdict carries its referent, or a pointer to the document that states it. This is the single finding of B6 that would close the other five. Zero compute; needs a ruling, not a run. | Katie / chief |
