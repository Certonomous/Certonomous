...
### 82. Supervisor's own reads: pass 10 BELIEVED; pass 8 gate arithmetic closed (2026-08-24T17:57:20Z)

**Pass 10 (`d74a36c2`, §73–81; C-35 `ac7de7e4`) — BELIEVED: SOUND WITH DISCLOSED DEVIATIONS, ONE DEFECT.** Read personally: `patched.log:904` "Main iteration 1000 KSP Residual norm 1.615247229756e-02" is digit-identical to `A3-rung3-n52/rung3_stage1.log:883`; line 903 is iteration 900 (the §2.1 citation range 893–903 is off by one, low — correct 894–904; every value reproduces; dafoam's dated correction under rule 6). Marker set read from the run root: the only `.patched` marker is `IDENTITY_CONFIRMED.patched`; `MEMORY_STOP_FIRED` exists only as the planted `gs_pos` selftest artifact. Freeze: two commits ever (`606930b4`, `8a0b440d`), Amendment 1 append-only over an 80,151-byte identical prefix and 107 s before the run root's birth time. Label `GATE FAIL — adjoint, inherited` frozen at prereg :260, 66 min before the gate; the stop at :562 registered; no stopped quantity reported as measured. Memory limb for the rule draft: registered [9.2, 15.0] GiB, measured peak 11.680 (73.0 % of the 16 GiB cap); min MemAvailable 15.287 GiB over 87 samples vs 8.0 floor, 0 strikes; gate opened on poll 1 at 27.19 GiB (+2.19 margin). No dafoam verdict moves. Owed to dafoam: the citation correction; a `D473` limb-2 fire at COST_CALIBRATION.md:87 untriaged (blame artifact class).

**Pass 8 — the last limb.** A read-only lane re-derived every gate from `/home/ubuntu/closure-data/tbnn_gpu/grading_gpu_ling.json` (26,981 B, sha256 f56dec94…, matching the witness) against the frozen thresholds at `e8309b6c` without importing the comparator: G0 PASS (rel err 1.599e-14), G1 NOT A RESULT (0/8 vs SST, b=0, train-mean; frozen :273), G2 GATE FAIL (margin −3.90e7), G3 NOT A RESULT (5/5 seeds over 2.374 % and 1.633; frozen :286–288), G4 capped (not run). All five agree with RESULTS.md:21–25. **Pass 8 BELIEVED in full.** Three verdict-neutral items for closure via the chief: RESULTS.md:22 "identical reading per seed" is an overstatement — seed 2 beats b=0 on `alpha_15_13929_4048` (0.310588 vs 0.312796); the G4 cell is an em-dash outside the rule-1 vocabulary; the G1 seed-aggregation convention and the G2 spread owner are unregistered degrees of freedom (harmless here; to close in the arm-2 prereg). Not re-derived: the b_rms values themselves from the .npz predictions.
...
### 90. Supervisor's own read of pass 11 — BELIEVED; the S0a ruling (2026-08-24T18:19:43Z)

**Pass 11 (`25f16019`, §83–89; C-41 `2153bd08`) — BELIEVED: SOUND WITH DISCLOSED DEVIATIONS, ONE DEFECT.** Read personally: the C1 fixture `verification/runs/F4_runs/swbli_cylflare/fixtures/bound_log_fixture.txt` is in the freeze commit `0bbac521` and places its single BOUND/BOUNDDIAG/BOUNDHIST block per timestep between `diagonal: Solving for rhoE` and `smoothSolver: Solving for e` (fixture lines 42–46, 55–59, 68–72) — event 1's position; `d457f612` §14 concedes the ambiguity in its own words ("fix the reading of a clause that is ambiguous as frozen", line 1416; "It presumes one", line 1423); `F4_SIGFPE_STEP01_RESULTS.md` (`5b5f5183`) line 231 shows §8.3 flipping INDETERMINATE (event 1) ↔ E-FIRST (event 2), and line 229 shows §8.1 flipping BASELINE-RECOVERED ↔ BASELINE-NOT-RECOVERED. Freeze limb: grading bodies first committed `4bf8138d` 17:32:25Z, +55 min 35 s after the earliest completion marker (16:36:50Z) — UNFROZEN on the freeze limb, disclosed; deferral declared at the freeze (`NotImplementedError("frozen skeleton …")` in the 480-line reader at `0bbac521`), every band and label fixed at `0bbac521`; reader hash `46f5c5af…` equal on disk, at `4bf8138d` and at HEAD. Planted control C4 fired in both steps' logs (nLow=424242, cell 12345); completion rule 8/8 both steps, age guard +4 min 56 s; every graded number reproduces to the digit. **DEFECT:** §14.4's "the choice is material in exactly one place" (repeated at RESULTS.md:239) is contradicted by the record's own table — §8.1 and §8.3 also flip; under event 2, §9.1 row 4 leaves §8.2/§8.3 as the only surviving reads and §8.3 would then attribute energy-first where event 1 attributes nothing. Owed to cfd: a dated correction in both files (rule 6). Minor: `append_record.py` reports a character count as bytes (128,088 B = 126,814 chars on that blob) — instrument hygiene, this team's.
