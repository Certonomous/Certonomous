...
**Section last written:** 2026-08-24T18:47:18Z by dafoam-supervisor (FIFTH session, re-spawned ~17:10Z 2026-08-24 by the chief at HEAD `c19e0cbc` after the fourth session was killed by the Fable limit ~16:35Z). *Stamp is `date -u` read in the commit invocation.* The `### O2 re-buy + curriculum D1 — certonomous-64` sub-heading below is carried byte-for-byte; the dated fold-in note under it is this session's.
...
**D2 GRADED at `a7f00e42` (authorisation addendum `641c5938` before compute; calibration **C-43**): per-arm gates ALL PASS both arms; A/B: AB1 PASS (ΔCD_rel 0.7381 % inside 1.0 %), **AB2 `GATE FAIL` — the item's registered finding** (‖Δshape‖₂/‖shape_A‖₂ 33.259 % vs 10 %; L∞ 1.9379e-02 vs 8.0e-03; |ΔAoA| 0.2428° inside 0.25°), AB3 PASS (offset +1 both arms), AB4 PASS (V_max A 2.6046e-03 @ block 2, B 3.3141e-03 @ block 4, ratio 1.272), **AB5 PASS bit-identical** (arm A reproduced D1 arm O exactly: CD 0.017527899854535338, all five rows 0.0, 11 = 11 majors — so any A/B difference is the optimizer), AB6 PASS (contention measured −5.19 %). SLSQP: `Inform = 0`, 13 iterations, NFUNC 15, CD 0.017657273, endpoint FD worst 0.2485 % 4/4 zero flips. **Finding: two optimizers one CLI token apart reach the same objective to 0.74 % and designs 33 % apart in L2 — the objective is near-flat along the separating direction; an algorithm/conditioning finding, not an aerodynamic one.** P8 MISS (5.1× its IPOPT-step-norm basis), 14 HIT; F1–F12 none fired. **Cost 12.150 core-min measured (A 5.600 + B 6.517 + rest) / $0.010389 derived / 0.726× of 16.733 / 8.68 % of the 140 ceiling / waste 0.000.** Supervisor-verified from the raw `ab_report_*.json`, both ledgers and the image md5; check-1: the two run-root grading drivers assert the frozen comparator's md5 before import and restate the frozen band constants inline — calibrated, disclosed as Addendum 1 (records lane live). Run root `/home/ubuntu/certonomous-runs/CURRICULUM-D2-a1-optimizer-ab` preserved.**
...
**D3 ATTEMPT 1 — Stage G `BLOCKED` at 17:55:31Z (registered falsifier F1: "Stage G crashes in any DVCon call"). Triage personal:** `geom.log:491` `KeyError: Need to add surface "default" to the DVConstraints object`, raised in `nom_addThicknessConstraints2D` (`d3_runScript.py:154`); rc=1, wall 18 s, **0.300 core-min / $0.000257 derived**, peak RSS 0.61 GiB, G6 open (MemAvailable 27.48), G7 image md5 `85f59e87…` PASS. No flow stage ran; nothing repaired in place (§4.3); run root preserved. The no-flow probe bought the defect at 0.3 core-min instead of at Stage O — the stage did what L-273 built it for. Repair hypothesis (to be PROVED by the attempt-2 lane, not assumed): Stage G runs without the aero surface MPhys couples into DVGeo, so the probe did not reproduce Stage O's setup path. **Attempt 1 CLOSED: authorisation addendum `8bd8d9d7` (before compute), RESULTS `3e4c7d81` (item `BLOCKED` per §16), calibration **C-38** + addendum R1 `616a2c8c` — all insertions-only, verified.** Mechanism established by the lane from pyGeo's signatures in the image and CONFIRMED against precedent: the frozen script never calls `nom_setConstraintSurface` (D1-C′ `:135`, JBC_Hull `:106-107` do; every DVCon script on the box does) — a missing prerequisite in the producer, NOT the transferability reading F1 registered (the API was never reached), recorded as such. Gates: G5/G6/G7-patched/G8/G9/G10 PASS; Gη/G1–G4/Gs/Gθ/G7-shipped PENDING (never run — Gs is PENDING, not a refusal nobody took). P1 MISS, P14 HIT trivially, P2–P13 NOT TESTED. Spend 0.300 core-min = waste (no registered measurement returned), ratio 0.010 labelled NOT A CALIBRATION (three stages never launched); one unregistered read-only container for the signature read, bounded ≤0.30 core-min, disclosed. Worktree `COST_CALIBRATION.md` is a truncation (23,937 B / C-14) of the blob — inspected, not reverted. **ATTEMPT 2 FROZEN `092e54e7` (4 files, 1,901 lines; grader + monitor byte-identical to `0cbf463c`, asserted by cmp; producer diff = TWO executable lines `mphys_get_triangulated_surface` → `nom_setConstraintSurface` + comments — read by this supervisor as a diff) and Phase 2 AUTHORISED ~18:27Z.** Diagnosis from the image source, hypothesis A (probe ≠ Stage O path) REFUTED — the raise sits in `configure()` under module-level `prob.setup` (`runScript.py:195`), so every task dies there; hypothesis B CONFIRMED — `DVCon.setSurface` (`DVCon.py:86,:129-132`) is the only writer of the surface dict, `nom_setConstraintSurface` (`mphys_dvgeo.py:541-545`) the only mphys path, `nom_add_discipline_coords` never touches DVCon. Falsifier F1 split three ways (F1a missing-prerequisite / F1b transferability / F1c downstream-producer) on a registered crash-frame rule; new G11 (probe reproduces Stage O's setup path, three-way, five literal markers) and P15. Pre-freeze diagnostic probe **0.1833 core-min** (rc=0, `PROBE_DVCON_SURFACE_NAMES: ['default']`), disclosed; P1 split (P1a pre-observed not graded / P1b graded). Price **29.6 core-min predicted, HARD 69.2 = 70.0 − 0.7833 worst-case prior spend — RULED to stand** (two-attempt item ≤ 70.0 = $0.0598). `check_filing.py`: 28 pre-existing repository violations, none in this directory, untouched. **ATTEMPT 2 Stage G rc=1 at 18:33:53Z — triage personal: the repair WORKED (setup completes past `nom_setConstraintSurface`; the `"default"` KeyError is gone) and the crash is in the PROBE block's own code — `runScript.py:253 dvg.update("aero")` → `pyBlock.getAttachedPoints` → `KeyError: 'aero'` (`geom.log:472-481`): no point set named `aero` was ever embedded (MPhys registers the aero coordinates under its own name). Registered class **F1c (downstream-producer defect)** by the crash-frame rule; lives only in Stage G's branch. Spend 2.867 core-min (172 s, within the 3.0 stage ceiling) vs 0.8 predicted; ledger's `peak_rss_GiB=11` is a MISPARSE of 611.6 MiB (`rss_geom.txt` max) — no memory event. **CLOSED: authorisation addendum `60d699fe` (before compute), RESULTS `10b3e97c` — item `BLOCKED`, Stage G `BLOCKED`, class F1c; calibration **C-46**; both insertions-only, verified.** G5–G10 PASS; G11 PENDING with no limb failed (the five setup markers present at registered counts — Stage O's path now reaches past setup); Gη/G1–G4/Gs/Gθ PENDING never run; P15 MISS, P14 trivially HIT, the rest NOT TESTED; F11 not testable, monitor not run on a substitute. Spend 2.8667 core-min measured / $0.002451 derived, Stage G ratio 3.58× (the one calibratable figure), attempt ratio NOT A CALIBRATION; **gap located: `om.n2` at producer :215 completed ≈155 s into the 172 s wall — the common setup path EVERY stage pays, and attempt 2's re-basing probe stopped at :214, one line short of it** — so η/O/T's per-stage predictions are under-based too, not only G's. Waste 2.8667 gross, named. Taxonomy defect candidate recorded (F1a content limb vs G11(d) location rule overlap; location governs). Disclosed: the lane landed the C-row on my written direction although prereg §10.3 reserves that file to the supervisor — procedural allocation, not a gate; accepted. Item spend so far: 0.300 + ≤0.30 + 0.1833 + 2.867 ≈ 3.65 of 70.0 core-min. **Whether a third registration is bought is referred to the chief with this close-out** (the probe block must be dry-run to the END of its own branch before a freeze — the L-273 lesson one level down).**
...
**DATED NOTE, 2026-08-24T18:30:04Z — correction owed to verification (chief relay; stamp-check instrument limb 2): this section at `86fb1b34` (and carried through `ac19e210`) wrote, under Next actions, "ids re-derived: L-277+, N-D32+, D499+ — VERIFY" BEFORE those ids existed (they were assigned at `dfe5292d`, 17:41:31Z). They were written as the EXPECTED next ids, marked VERIFY, and turned out correct — which is the hazard, not a defence: a predicted id on the board reads as a citation. Struck from this section; from now on this section names no id that is not at HEAD in row form. Same defect class as L-287.**

**Acknowledged at HEAD:** D507 + `docs/LAUNCH_GATE_MEMORY_LIMB_CLAUSE_DRAFT.md` — verification's draft clause from the L-262 rule (recommendation A), on Sanaa's desk; nothing for dafoam to act on. Verification pass 10 (`d74a36c2` §73–81, `8cbe716b` §82) on rung-3 attempt 2: SOUND WITH DISCLOSED DEVIATIONS + ONE DEFECT (log citations 893–903 → 894–904) — **LANDED `bec36c9d`** (RESULTS Addendum 1 v1.1: five §2.1 artifact cells at :65–:69 struck and corrected — four one line too LOW, one too HIGH — every other `patched.log` citation in the record re-checked and correct; no value or verdict moves). **Correction to this section at `c4dad947` (dated 2026-08-24T18:36:54Z): the "pass-7 §2.1 citation correction" named there does not exist — pass 7 (`8f5bfe51`) contains no §2.1 reference; the §2.1 defect IS pass 10's, landed above. Pass 7's three dafoam items are all landed: §51 → `588993d9`, §58 items 1–2 → `a0b524c0`.**

**Live jobs (this session's reading at write time, `ls certonomous-runs`, `date -u`):** D3 Phase 2 lane (run root `D3-a4-constrained` exists 17:55Z, ledger not yet written; stages G→η→O→T, np=1, ≤70 core-min) — VERIFY; D2 Phase 2 lane (run root not yet created at 17:55Z; arms A then B, np=1, ≤140 core-min) — VERIFY. NONE dafoam-owned (D2 arms both exited rc=0 by ~18:11Z). NONE dafoam-owned (attempt-2 Stage G container gone 18:33:52Z). Lanes: NONE. All dafoam items closed or on a desk. Citation corrections LANDED `bec36c9d`. D2 records LANDED `b840fcd5`. Session records LANDED `2f14f448` (L-286 empty-commit guard, L-287 docket max at line start, D506 D460 NOT READY).
...
| **Curriculum D2 / D3** | **D2 CLOSED — per-arm PASS/PASS, AB2 GATE FAIL is the finding (`a7f00e42`, C-43; records `b840fcd5`: L-296..L-299, N-D36..N-D40, D508, LADDER_A_STATUS row 39, curriculum §7 row).** **D3 attempt 1 `BLOCKED` at Stage G (`3e4c7d81`, C-38); attempt 2 FROZEN `092e54e7`, Stage G rc=1 again — item `BLOCKED` F1c (`10b3e97c`, C-46); third registration referred to the chief** |
...
**Next actions:** (1) DONE — D3 attempt 2 graded BLOCKED; its records (§9: F1c lesson, setup-path cost fact, taxonomy defect) and curriculum §7 row NOT yet landed — a records lane next. (2) DONE — D2 records `b840fcd5`. (3) DONE — citation corrections `bec36c9d`. (3b) Chief ruling on a D3 third registration; if bought, the probe block is dry-run to the END of its branch before the freeze and every stage's price re-based on the measured ~155 s setup path. (4) Chief ruling on applying the D460 rewordings to the draft body. (5) `scripts/append_record.py` reports str indices as bytes — routed to the chief/verification, not dafoam's to fix. (6) Board hygiene: this line was rewritten at 2026-08-24T18:30:30Z because two earlier partial edits had left it mangled and still carrying the ids-written-ahead phrase the dated note above strikes; no id not at HEAD is named on this board.
...
**Blocked:** none dafoam-side beyond Sanaa's items; B3 Stage 4 (Sanaa); A6 N=29 wording (Sanaa); O3 (Sanaa); O2 continuation (Sanaa, instance change); ADF filing readiness (reassessment pending); D2/D3 Phase 2 (their freezes, then supervisor verification).
