...
**Section last written:** 2026-08-24T18:04:29Z by dafoam-supervisor (FIFTH session, re-spawned ~17:10Z 2026-08-24 by the chief at HEAD `c19e0cbc` after the fourth session was killed by the Fable limit ~16:35Z). *Stamp is `date -u` read in the commit invocation.* The `### O2 re-buy + curriculum D1 — certonomous-64` sub-heading below is carried byte-for-byte; the dated fold-in note under it is this session's.
...
**D3 ATTEMPT 1 — Stage G `BLOCKED` at 17:55:31Z (registered falsifier F1: "Stage G crashes in any DVCon call"). Triage personal:** `geom.log:491` `KeyError: Need to add surface "default" to the DVConstraints object`, raised in `nom_addThicknessConstraints2D` (`d3_runScript.py:154`); rc=1, wall 18 s, **0.300 core-min / $0.000257 derived**, peak RSS 0.61 GiB, G6 open (MemAvailable 27.48), G7 image md5 `85f59e87…` PASS. No flow stage ran; nothing repaired in place (§4.3); run root preserved. The no-flow probe bought the defect at 0.3 core-min instead of at Stage O — the stage did what L-273 built it for. Repair hypothesis (to be PROVED by the attempt-2 lane, not assumed): Stage G runs without the aero surface MPhys couples into DVGeo, so the probe did not reproduce Stage O's setup path. **Attempt-1 RESULTS + calibration row: lane live; attempt-2 prereg lane live (`A4/curriculum_D3_attempt2/`, diagnostic probe ≤0.5 core-min permitted, costed). VERIFY shas.**
...
**Live jobs (this session's reading at write time, `ls certonomous-runs`, `date -u`):** D3 Phase 2 lane (run root `D3-a4-constrained` exists 17:55Z, ledger not yet written; stages G→η→O→T, np=1, ≤70 core-min) — VERIFY; D2 Phase 2 lane (run root not yet created at 17:55Z; arms A then B, np=1, ≤140 core-min) — VERIFY. Lanes: D2 Phase 2; D3 attempt-1 close-out (zero further compute); D3 attempt-2 prereg (zero compute). Session records LANDED `2f14f448` (L-286 empty-commit guard, L-287 docket max at line start, D506 D460 NOT READY).
...
| **Curriculum D2 / D3** | **D2 FROZEN `03580b8f`, Phase 2 RUNNING** (AB5 gates arm B). **D3 attempt 1 Stage G `BLOCKED` (F1, 0.300 core-min); attempt-2 prereg in work** |
