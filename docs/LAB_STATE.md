...
**Section last written:** 2026-08-24T17:51:09Z by dafoam-supervisor (FIFTH session, re-spawned ~17:10Z 2026-08-24 by the chief at HEAD `c19e0cbc` after the fourth session was killed by the Fable limit ~16:35Z). *Stamp is `date -u` read in the commit invocation.* The `### O2 re-buy + curriculum D1 — certonomous-64` sub-heading below is carried byte-for-byte; the dated fold-in note under it is this session's.
...
**Curriculum D2 (optimizer A/B on D1, ~140 core-min class) — PHASE-1 prereg lane live since 17:25Z, zero compute; VERIFY sha at next write.**
...
**D460 filing-readiness reassessment — lane live (zero compute, readiness document only; nothing sent, filing Sanaa's). VERIFY at next write.**

**Live jobs (this session's reading at write time):** D3 Phase 2 lane launching stages G→η→O→T under `0cbf463c` (np=1, run root `/home/ubuntu/certonomous-runs/D3-a4-constrained`, ≤70 core-min) — VERIFY state. No other dafoam solver. Lanes: D3 Phase 2, D2 prereg, D460 readiness.
...
| **Curriculum D2 / D3** | D2 prereg lane live. **D3 FROZEN `0cbf463c`, Phase 2 RUNNING** (η stop rule gates O) |
...
**Next actions:** (1) grade D3 on its lane's report — crash triage and per-stage numbers read by the supervisor. (2) D2 freeze: personal read of comparator/cost/gate, then Phase 2. (3) D460 readiness state to the board. (4) Lesson candidates queued: the `d99d82cd` empty-commit chain; the D901 phantom reader. (5) DONE — records landed via `append_record.py` (ids re-derived: L-277+, N-D32+, D499+ — VERIFY) and the `LADDER_A_STATUS` row 12b cell edit as a dated in-row note. (3) Verify D2/D3 freezes personally (diff read of comparators, cost, launch gate) before any Phase 2. (4) D460 filing-readiness reassessment, zero compute. (5) Cost-calibration note: a supervisor lane must decide whether the O2R-P2 addendum wants a C-row (zero compute; lane says none owed).

**On Sanaa's desk:** **NEW — D3 successor on the 45,760-cell A4 mesh (mesh change, UNPRICED, the only route to separation content).** Otherwise unchanged from the fourth session's list — D460 draft NOT FILED (class now CONDITIONING); A3 rung-1 §4 rule choice; D464 N=29 wording; R11 adoption evidence now THREE-sided (D1-C′: shipped and patched agree at a converged design point); MemAvailable floor 12 GiB; five upstream drafts NOT FILED (D-A/D-A2, D-B/D-B2, D-C, D-E, ADF `757eccf0`); B3 Stage 4; near-zero sign-flip class; charter §13 proposal; O3 guard authorization; O2's > 20 GiB-to-one-process instance question.

**Blocked:** B3 Stage 4 (Sanaa); A6 N=29 wording (Sanaa); O3 (Sanaa); O2 continuation (Sanaa, instance change); ADF filing readiness (reassessment pending); D2/D3 Phase 2 (their freezes, then supervisor verification).
...
**Section last written:** 2026-08-24T17:43:07Z by cfd-supervisor (**fourth owner session** — the third spawn after the ~16:35Z Fable usage-limit kill of the predecessor session; that predecessor's lanes **outlived it**, and their work was verified at HEAD before belief, not taken on the dead session's word). Stamp is `date -u` read in the committing shell invocation.

**Last commit:** F4 — `7cdb26f4` (step-0/1 **executed**: build, C0 twin, both steps COMPLETE/COMPLETE, 36 files), `d457f612` (prereg **ADDENDUM 2** §14, v1.1 → v1.2, POST-COMPUTE, 195 insertions: §8 reads **EVENT 1** = the `.C:267` `boundE.H` include, both readings disclosed — supervisor ruling on mechanism), `4bf8138d` (grading bodies +979/−42 plus `c0_twin_diag/compare_c0.py`; **supervisor read as a diff, cleared**). F5b — `c1ba1845` (Physics prereg **FROZEN STAGE 1** + reader skeleton, every §4 control passing on the Theodorsen fixture, 6,638 insertions, **zero compute**), `a80d5f36` (reader **STAGE 2** grading bodies +533/−37, zero compute; **supervisor read as a diff, cleared**). Freeze identities: F5b prereg blob `f1cbc96d`, md5 `81c32b83da0e9db280a550d2d74c8b29`; reader stage-2 blob `6c6d34d0`. Earlier this family: `290fcff2`, `0bbac521`, `f89aa7b4`, `7a96cf54`, `1135e3c5`, `3b9bcf31`, `b8fe7eea`.

**Live jobs:**
- **F4 grading lane (zero compute)** — writing `verification/campaign/F4_SIGFPE_STEP01_RESULTS.md` plus the `docs/COST_CALIBRATION.md` row against the frozen bodies at `4bf8138d`. **VERIFY on disk**: the results file was absent at the time this section was written; confirm it exists and cites the run tree before quoting any graded number from it.
- **F5b `physics_p1` launch AUTHORIZED** to a lane at ~17:45Z — cap **72.0 core-min** = **$0.0616 derived** at the recorded $0.0513/core-h (reported-by-owner, not measured), point estimate 35.0 core-min. **pid / ETA: VERIFY** — re-derive from `verification/runs/F5b_runs/physics_p1/{PID.txt,LAUNCH_LOAD.txt}` and `ps`; that directory did not yet exist when this section was written, which is also the standing freeze condition both F5b commits asserted.
...
- **F4 two-events ambiguity — RULED (check #2, crash triage).** The instrumented log carries two candidate SIGFPE events; the ruling is **event 1**, on mechanism grounds (the `.C:267` `boundE.H` include), landed as ADDENDUM 2 §14 at `d457f612` with **both readings disclosed**. Materiality is stated in §14.4: t\* at event 1 = 2668/29700 = **8.98 %**, inside the S0a pre-registered window **[6, 24] %**; event 2 = 749 = **2.52 %**, outside it. Gate quantities, bands, cap and labels unchanged — the addendum is post-compute and alters none of them.
- **F4 grading bodies — READ AS A DIFF (check #1), cleared.** All **42 deletions are skeleton placeholders** (`NotImplementedError` stubs and their docstring stubs); every constant in the new bodies matches the frozen number in the pre-registration. This discharges the standing "grading bodies are still `NotImplementedError`" hold from the previous session's board.
- **F5b Physics prereg — DRAFT READ IN FULL, and A-1 re-derived independently by this session** rather than relayed: Z = **5.0469847 + 0.0782415i**, A_att = **0.42900727**, G2 analytic supremum **0.2034845** against a brute-force **0.2033316**. **Three before-compute amendments ordered** by this supervisor: (1) A-1 re-attribution — the prior closure was credited to a usage-limit-killed session no reader can verify, withdrawn and replaced by the re-runnable derivation; (2) G3 must read the **lagged, logged** CoNum, not an inferred one; (3) header-**name** parsing, never positional.
...
- **F5b reader stage-2 — READ AS A DIFF (check #1), cleared**, including the outcome-map precedence, which follows standing rule 5 (a non-CONVERGING / non-converged input can only turn a PASS or GATE FAIL **into** NOT A RESULT, never the reverse).

**F4 step-0/1 — EXECUTED, NOT YET GRADED.** The following are **raw numbers and may be quoted as raw only** — no verdict attaches until the grading lane's record lands and is read. event1/event2 `nLow` at t\* = 1.8993308e-05: **step 0 = 2668 / 749**, **step 1 = 2668 / 775**; at window end: **6856 / 2019** (step 0) vs **6849 / 1913** (step 1). Cost: **9.86 core-min** in the solvers; **12.00 core-min gross / 11.59 cleaned** against the ≤15 envelope, with **0.40 core-min waste separately named** (never absorbed into the ratio); **$0.0103 derived**. The COST_CALIBRATION row is **not yet landed** — it is the grading lane's, appended from HEAD content.
...
| family | state |
|---|---|
| **F4** | Prereg `0bbac521` v1.0 → `290fcff2` v1.1 → **`d457f612` v1.2 (ADDENDUM 2, post-compute)**. **Executed** at `7cdb26f4` (COMPLETE/COMPLETE); grading bodies frozen at `4bf8138d` and diff-read. **Graded verdict PENDING on the lane.** θ=32.5°/35° gate held. Bounded-run logs GONE from disk — the 4–32 % fractions are record-quoted, artifact-missing; the VERIFY note landed in NOT_PASSING_REGISTER at `0ab22070`; Step 0 is the recovery path |
| **F5b** | Physics prereg **FROZEN** — stage 1 `c1ba1845` (blob `f1cbc96d`, md5 `81c32b83…`), reader stage 2 `a80d5f36` (blob `6c6d34d0`); both **zero compute**, `physics_p1` absent at each commit. **`physics_p1` launch AUTHORIZED** (cap 72.0 core-min). Feasibility PASS stands (`F5bc_unsteady_statistics.md`); Gate rung PENDING. Trap: `F5b_cylinder_re100_act.json` is a different case |
...
**Next actions:** (1) **F4 verdict** — grading lane's `F4_SIGFPE_STEP01_RESULTS.md` + calibration row → onto the board and into the report. (2) **F5b** — run `physics_p1` → grade against the frozen reader → results record → calibration row. (3) **D481 third-lever decision** — chief's. (4) **Lesson candidates for the next LESSONS append** (re-derive max+1 at commit time, from the tail): a crash-detecting regex that matches the solver's own startup banner is a 100 % false-positive detector; never `&&`-chain a command whose non-zero exit is expected; never read `coefficient.dat` positionally (v2606 writes 12 column-sorted coefficients, no α column); generic scratchpad message filenames collide across lanes.
