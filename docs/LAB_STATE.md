...
**Section last written:** 2026-08-23T20:12Z by dafoam-supervisor (session 15ac9361; the SECOND live session runs its own dafoam supervisor — see the claim ledger below).
...
**Claim addendum, 2026-08-23T20:05Z, by the `c8254a4a` session — the re-formed 64b13819 dafoam supervisor (session `01ENBw3KPr5gMaj8Vt7rcxSB`, pid 1100087), the predecessor's successor, not an intruder.** The split above is **CONFIRMED from this side**: ADF characterisation sweep 1 + full novelty sweep, the B3 peak-RSS watcher re-run (prereg `d062aace`), and the A3 rung-1/3 patched-column preregs are the other session's — this session will not touch them. This session holds:
(a) **W4 M1+M2** — the freeze of `c8254a4a` was verified by this supervisor personally (diff-tree confined to the one path, blob `e7b5f0d4` identical to the worktree file, memguard `f102eb52…` executable and self-tested), and **launch was AUTHORISED ~19:47Z**; Lane W is executing the six registered stages under the wired 60.0 core-min ceiling. Verdict lands in `cases/dafoam/ladder-b/W4_M1M2_RESULTS.md`.
(b) **`cases/dafoam/EXPERTISE_CURRICULUM.md` — CLAIMED 20:05Z, committing next.** Sanaa's 2026-08-23 directive (chief-relayed verbatim: *"DAFOAM we need to think of many advanced optimization cases the dafoam team can run so they become an expert"*): an advanced-optimization curriculum, tiered and costed. **Design work only — PROPOSED, awaiting Sanaa's ratification; NO compute is launched under that directive.** Distinct from heat-transfer's analogous `docs/campaigns/T-family/EXPERTISE_CURRICULUM.md` (their D468). Runs none of the other session's claimed items.
(c) **The predecessor's open 1.11e+02 question is RESOLVED — no correction needed, no quote-and-strike warranted.** The committed bytes were already correct: `ladder-a/A6/rung_n16_remaining_components/RESULTS.md:307` and L-242 (`docs/LESSONS.md:9593`) say "of order **1.1e+02**", and N-D15's two committed CD draws give δ_repeat 2.2104e-06 / 2e-8 = **110.52** — 1.1e+02 at 2 s.f., the `6c6de745` commit message's 1.11e+02 at 3 s.f., and between the two trivial-baseline draw magnitudes 28.75 and 152.94 exactly as claimed.
(d) **INCIDENT, disclosed by its author: `4932a7c3` (this session, ~19:57Z) committed a stale board.** The section build failed on this author's own wrong anchor string, the merge script then failed file-not-found, but the staging step picked up a **stale `LAB_STATE_merged.md` left in the session-shared scratchpad** (both dafoam sessions resume 64b13819 and share it — L-186's hazard, now with a measured cost) and the chain committed it: closure/dafoam/cfd/verification reverted to 08-22 states for a two-commit window until `537a52d5` rebuilt from a pre-clobber base. Residue check against `450735c1`: current HEAD differs only in the cfd section **plus one loss `537a52d5` itself introduced — heat-transfer's 20:15Z write is reverted to 19:45Z** (their own committed bytes survive at `450735c1:docs/LAB_STATE.md`; flagged to the chief, their section is not mine to restore). Repair adopted here: unique scratch filenames, existence-and-anchor asserts before staging, no `set -e` reliance. Lesson + docket rows owed by this session's next records commit.
